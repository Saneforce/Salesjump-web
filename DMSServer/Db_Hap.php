...
{
	//echo $_FILES["file"]["tmp_name"].$target_file_name;
...
function getAttndStatus(){
	...
	$Status = $_GET['Status'];
	$Priod = $_GET['Priod'];
    $query = "exec [GetAttnDetsByStatus] '" . $RSF . "','".$Status."','".$Priod."'";
    return performQuery($query);
}
...
function getAttndCnts(){
	...
	//if($RSF=='MGR0016' || $RSF=='MGR0013' || $RSF=='MGR0020') {//||$RSF=='MGR5146'||$RSF=='MGR0017'||$RSF=='MGR0018'||$RSF=='MR0032'){
		...
		*/
		...
		$aOnTm=FilterArry($res,"OnTm",11);
		if(count($aOnTm)>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'On-Time','StatusCnt' => count($aOnTm), 'StusClr' => '#089c08 !important');}
		$result[]=array('SF_Code' => $RSF, 'Status' => 'Weekly off ('.$DyDet['wkoff'].' days)','StatusCnt' => $wkCnt, 'StusClr' => '#ff920b !important');
		if($cONDy>0){
			$result[]=array('SF_Code' => $RSF, 'Status' => 'On-Duty','StatusCnt' => $cONDy,'StusClr' => '#ff920b !important');
		}
		$aOnTm=FilterArry($res,"AInSt",1);
		if(count($aOnTm)>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'Grace time','StatusCnt' => count($aOnTm), 'StusClr' => '#ff920b !important');}
		$phrs=0;foreach($res as $a){$phrs=$phrs+$a['Perm'];}
		if($phrs>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'Permission (2 Hrs)', 'StatusCnt' => $phrs, 'StusClr' => '#ff920b !important');}
		$aOnTm=FilterArry($res,"AInSt",2);
...
case "SSstockUpdation":
 ...
 $sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works, Hq_Id,Chilling_Id,SHift_Type_Id,Fromdate,Todate) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."',$Hq_Id,$Chilling_Id,$SHift_Type_Id,$Fromdate,$Todate";  
performQuery($sql);
...
$sql = "SELECT isNull(max(Sl_No),0)+1 as Sl_No FROM Expense_Start_Activity";
$topr = performQuery($sql);
$pk = (int) $topr[0]['Sl_No'];
    $sql = "insert into Expense_Start_Activity(Sf_code,Division_Code,Date_Time,Start_Km,Image_Url,Remarks,MOT,Approval_Flag,Sl_No) values( '$sfCode',".$Owndiv.",'".$Datetime."',".$vals['Km'].",'".$vals['Retailerphoto']['imgurl']."' ,".$vals['Remarks'].",".$vals['MOT'].",'1'," . $pk . ")";
		      performQuery($sql);
...
$sql = "update Expense_Start_Activity set Enddateand_time='".$Datetime."',End_Km=".$vals['Km'].",End_Image_Url='".$vals['Retailerphoto']['imgurl']."',End_MOT=".$vals['MOT'].",End_remarks=".$vals['Remarks']." where sf_Code='$sfCode' and convert(date,Date_Time)='".$Dateonly."'";
   	   performQuery($sql);
       ...
       $sql = "SELECT isNull(max(Sl_No),0)+1 as Sl_No FROM Trans_Product_Availability";
        ...
        case "TP_Attendance":
            ...
            //if($update==0){
                ...
                $sql ="exec Attendance_entry '$sfCode','$Owndiv','$eDate',$lat,$long,'$eDate','$slfy','".$wrkType."' ,'0' ,'".$Shift_Selected_Id."','$App_Version','".$PlcID."','".str_replace("'","''",$PlcNm)."','".str_replace("'","''",$vstRmks)."' ";
                $result=performQuery($sql);    
				$result["qry"]=$sqlh;			
           /* }
            else{
...
$va =$data[1][$tempp[0]];
            for ($i = 0; $i < count($vals); $i++) 
            {
                    $sql = "insert into Trans_Daily_User_Expense(SF_Code,eDate,expCode,expName,Amt,Division_Code,LUpdtDate,Ukey,Image_Url) select '" . $sfCode . "','".$dateTime."','" . $vals[$i]["ID"] . "','" . $vals[$i]["Name"]  . "','" . $vals[$i]["amt"] . "','" . $Owndiv . "',getDate(),'" . $va["Ukey"] . "' ,'" . $vals[$i]["imgData"] . "' " ;
                    performQueryWP($sql,[]);
            }
 ...
 $sql = "insert into Trans_TP_Hist(SF_Code,Tour_Month,Tour_Year,Submission_date,Tour_Date,WorkType_Code_B,Worktype_Name_B,Tour_Schedule1,Objective,Worked_With_SF_Code,Division_Code,Territory_Code1,Worked_With_SF_Name,TP_Sf_Name,HQ_Code,HQ_Name,Confirmed,Change_Status,JointWork_Name,Retailer_Code,Retailer_Name,submit_status,JointworkCode,Worktype_Flag,Hq_Id,Chilling_Id,SHift_Type_Id,Fromdate,Todate) select '" . $sfCode . "',MONTH($tourDate),YEAR($tourDate),getdate(),$tourDate,$worktype_code,$worktype_name,$RouteCode,$objective,$worked_with_code," . $divisionCode[0] . ",$RouteName,$worked_with_name,$sfName,$HQ_Code,$HQ_Name,0,0,$jointworkname,'".$RCode."','".$RName."','1',$jointWorkCode, $Flag,$Hq_Id,$Chilling_Id,$SHift_Type_Id,$Fromdate,$Todate";
		...
		case "TravelApproval":
            ...
            $Sl_No = $_GET['Sl_No'];
			$AAmount = $_GET['AAmount'];
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update Expense_Start_Activity set Approval_Flag=2,Approval_Amount='" . $AAmount . "'
			,Approved_Date='" .date('Y-m-d H:i:s'). "',
			Approved_by='" . $Mrsfcode . "' where Sl_No=$Sl_No";
            performQuery($sql);

            $sql = "SELECT DeviceRegId FROM Access_Table where sf_code=(select SF_Code from Expense_Start_Activity where Sl_No='$Sl_No')";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
				$msg = "Travel Allowance has been Approved";
                sendFCM_notify($reg_id, $msg,"Extended Shift Request",0,'#sign-in');
            }
		 break;
		case "TravelApprovalR":

            $Sl_No = $_GET['Sl_No'];
            $Mrsfcode = $_GET['sfCode'];
			$AAmount = $_GET['AAmount'];
            $sql = "update Expense_Start_Activity set Approval_Flag=3,Approved_Date='" .date('Y-m-d H:i:s'). "',Approved_by='" . $Mrsfcode . "',Reject_reason=" . $vals['reason'] . " where Sl_No=$Sl_No";
            performQuery($sql);
            ...
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
				$msg = "Travel Allowance has been Reject for \n" . $vals['reason'] . "";
                sendFCM_notify($reg_id, $msg,"Extended Shift Request",0,'#sign-in');
            }
		break;
		case "NTPApproval":
		...
		case "NTPApprovalR":
			...
			$Mr_Sfcode = $_GET['Mr_Sfcode'];
            $managerSf_Code = $_GET['sfCode'];
            $sql = "insert into TP_Reject_B_Mgr(SF_Code,Reject_date,Division_Code,Rejection_Reason) select '" . $Mr_Sfcode . "','" . date('Y-m-d H:i') . "',$Owndiv," . $vals['reason'] . ",'10','2020'";
			performQuery($sql);
			$sql = "delete from Trans_TP_One where  sf_Code='$Mr_Sfcode'  and convert(date,Tour_Date)='$Start_date' ";
		    performQuery($sql);
		...
		case "PermissionFormValidate":
			...
			$query = "insert into PermissionEntry  select '".$pk."','$sfCode','".$Owndiv."','".$vals['pdate']."','".$vals['start_at']."','".$vals['end_at']."','".$vals['No_of_Hrs']."','2','" . date('Y-m-d H:i') . "', '".$vals['Reason']."',' ','".$sf_name."','','' ";
            $results = performQuery($query); 
...
case "MissedPunchEntry":
			...
			,'".$vals['checkouttime']."','".$vals['reason']."','0','','','" . date('Y-m-d H:i') . "',null,'".$vals['Shift_Name']."','".$vals['checkinTime']."'";
			$results = performQuery($query); 
...
case "WeekofPunch":
            ...
            if($res[0]["Cnt"]>0){
				...
				if($res[0]["Cnt"]>=$DyDet['wkoff']){					
					...
					if(count($res)>0){				
						...
						$query = "insert into Trans_Weekoff  select '".$pk."','".$sf_name."','$sfCode','".$vals['WKDate']."','" . date('Y-m-d H:i') . "','".$Owndiv."','".$vals['reason']."' ";
						$results = performQuery($query); 
						$resp["success"] = true ;
					}
				}
  ...
  case "DCRMissedDates":
   ...
   case "LeaveForm":
	...
	select '$pk'," . $vals['Leave_Type'] . ",'" . $vals['From_Date'] . "'," . $vals['To_Date'] . "," . $vals['Reason'] . ",'$sfCode','$Owndiv','".$lvappp."','" . date('Y-m-d') . "'," . $vals['No_of_Days'] . "," . $vals['address'] . "," . $vals['HalfDay'] . "," . $vals['HalfDay_Type'] . "";
            performQuery($sql);
...
switch ($value[0]) {
	...
	case "ValidateExtended":
		...
		$result["success"] = true;
		$result["msg"]="";
		if(count($res)<1 and $sfcode!='MGR0016'  and $sfcode!='MGR4762'){
			$result["msg"]="Check-In Not Done.kindly do Check-In";
			$result["qry"]=$sql;
			$result["success"] = true;
		}
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
 ...
 $query = "update Mas_SF_SlNo set POrdSl=(isnull(POrdSl,0)+1) where SF_Code='" . $sfCode . "'";
    ...
    case "vwLeave":
        $sfCode = $_GET['sfCode'];
        $sql = "select SF_Mobile,Sf_Code,Leave_Id,Reason,Applieddate,Address,Address,FieldForceName,Reporting_To_SF,Designation,HQ,EmpCode,From_Date,To_Date,LeaveDays,Division_Code,sf_Designation_Short_Name,Leave_Type,Leave_Name from  (select * from vwLeave )A where Reporting_To_SF='$sfCode'";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		case "Get/WkoffStatus":
        //$sfCode = $_GET['sfCode'];
        $sql = "exec getWkoffStatus 'MGR0372'";
		...
		case "vwnativeExtended":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwnativeExtended_Approval where Reporting_To_SF='$sfCode' ";
        $vwExtended = performQuery($sql);
        outputJSON($vwExtended);
        break;
	    ...
	    $sql = "select count(Reporting_To_SF) TAC from  ViewTravelAllowance where Reporting_To_SF='$sfCode'";
        $TA = performQuery($sql);
		...
		$sql = "select count(Reporting_To_SF) Tour_Count from ViewTourPlan where Reporting_To_SF='$sfCode'";
		...
		$ViewAllCount['TravelCount']=$TA[0]['TAC'];
		...
		if($AMod==1){
			...
			case when Approval_Flag='2' then 'Pending'  when Approval_Flag='0' then 'Approved' else 'Reject' end PStatus,
			case when Approval_Flag='2' then 'rgb(255,165,0) !important'  when Approval_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
        ...
        $sql = "select Sf_Code,Sf_Name SFNm,Shift_Name,convert(varchar(10), Approved_Date, 103) Rejectdate,convert(varchar,Missed_punch_date,103) Missed_punch_date,Missed_punch_Flag,convert(varchar(10), Entry_Date, 120) Submission_date ,Reason,Checkin_Time,
		left(CONVERT(varchar,cast(replace(Checkout_Tme,'T',' ') as datetime),8),5) Checkout_Tme,
		case when Missed_punch_Flag='0' then 'Pending'  when Missed_punch_Flag='2' then 'Approved' else 'Reject' end MPStatus,
		...
		$sql="select convert(varchar, login_date, 23) name,
		 ...
		 where End_Time is null and Missed_punch_Flag is null";
		$GetMissed_Punch = performQuery($sql);
		//and convert(varchar, Start_Time, 23)=convert(varchar, Missed_punch_date, 23)
		...
		*/
		...
		$sql="select convert(varchar,Sf_TP_DCR_Active_Dt,23) SDt from Mas_Salesforce where SF_code='$sfCode' and Sf_TP_DCR_Active_Dt>'".$stDt."'";
		$stDts = performQuery($sql);
		if(count($stDts)>0){
			$stDt=$stDts[0]["SDt"];
		}
...
inner join Mas_Salesforce s on s.Sf_Code=TA.SF_Code where  WrkType>0 and TA.Approved_by='$rSF' order by login_date desc";
        ...
        $AMod = $_GET['AMod'];
		if($AMod==1){
        $sql = "select SF_Name SFNm,convert(varchar(10), l.Created_Date, 105)  Created_Date,Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), l.LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,No_of_Days,case when
		 Leave_Active_Flag='2' then 'Pending'  when Leave_Active_Flag='0' then 'Approved' else 'Reject' end LStatus ,
		 case when Leave_Active_Flag='2' then 'rgb(255,165,0) !important'  when Leave_Active_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
        ...
        $sql = "select convert(varchar(10), Created_Date, 105)  Created_Date,Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,No_of_Days,case when
		 Leave_Active_Flag='2' then 'Pending'  when Leave_Active_Flag='0' then 'Approved' else 'Reject' end LStatus ,
		 case when Leave_Active_Flag='2' then 'rgb(255,165,0) !important'  when Leave_Active_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
        ...
        $rSF = $_GET['rSF'];
        $Ta_Date = $_GET['Ta_Date'];
        $sql = "update Expense_Start_Activity set  summary_Flag='1' where sf_code='$sfCode' and convert(date,Date_Time)='$Ta_Date'";
		$TAStartandEnd = performQuery($sql);
		
		
		 $expense['TAStartandEnd']=true;
         $expense['User_Expense']=$sql;
        outputJSON($expense);
...
case "get/vwVstPendingPragnancy":
    ...
    case "save/track":
		...
		$lat = $data[0]['Latitude'];
		$sTime = $data[0]['Time'];
		$sql = "insert into tbTrackLoction(SF_code,DtTm,Lat,Lon,Addr,Auc,deg,DvcID) select '$sfCode','" . $sTime . "','$lat','$lng','','" . str_replace("'", "", $data[0]['Accuracy']) . "','" . str_replace("'", "", $data[0]['Bearing']) . "','" . str_replace("'", "", $data[0]['DvcID']) . "'";
...
case "get/RetailerPaymentCollection":
 ...
 case "get/GoogleLogin":
		...
		/* $sql = "SELECT  UsrDfd_UserName Sf_UserName,iif(SF_Status=3,3,SF_Status) SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,(select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,
		case when Approved_By='FF' then 1 else 0 end SFFType,case when Approved_By!='FF' and Approved_By!='I' then 1 else 1 end OTFlg,
		isnull(latlong,'12.91729615:80.23030043') AS HOLocation from mas_salesforce ms inner join Mas_HQuarters m on cast(HQ_ID as varchar)=SF_HQ  inner join access_master am on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT on At.Sf_code= ms.Sf_code where SF_Email='".$Email."' and SF_Status!=3 and Approved_By<>'I'"; */
		$sql = "SELECT  UsrDfd_UserName Sf_UserName,State_Code,iif(SF_Status!=0,3,SF_Status) SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,(select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,
		case when Approved_By='FF' then 1 else 0 end SFFType,sf_desgn SFDept,'1' DeptType,case when Approved_By!='FF' and Approved_By!='I' then 1 else 1 end OTFlg,
...
case "get/calpriod":
	...
	case "get/FieldForce_HQ":
		...
		$SF_code = $_GET['sfCode'];
		$Owndiv = (string) $divs[0];
		$query="select shift_type id,Typename name from Shift_Type where  Division_Code='" . $Owndiv . "' and Active_Flag=0 ";
		outputJSON(performQuery($query));
		break;
...
case "get/ClearData":
	...
	case "get/tknPerm":
		...
		$mDt = strtotime($mDt."- 1 day");
		$tDt=date('Y-m-d',$mDt);
		$query = "select sum(cast(Noof_hours as numeric)) tknHrS from PermissionEntry where Approval_Flag!=1 and SF_Code='".$sfCode."' and Permissiondate between '".$fDt."' and '".$tDt."'"; 
		//echo($query);
		outputJSON(performQuery($query));
		break;	
...
outputJSON($result);
	...
	case "Check/Version":
		...
		$tbdate = $_GET['Date'];
		//;
        $query = "select sf_code from TbMyDayPlan where sf_code='".$sfCode."' and  convert(date,Pln_Date)='" . $tbdate . "'"; 
		...
		$query="select * from vwTourPlan where sf_Code='".$sfCode."' and convert(date,[date])='" . $tbdate . "'";
