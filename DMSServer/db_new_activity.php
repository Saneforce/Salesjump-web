...
function saveNewDailyExp(){
	...
	for ($i = 0; $i < count($Add_Exp); $i++) 
    {
		...
		$expsql="insert into Trans_Additional_Expense(SlNo,Fk_Sl_No,Exp_Date,Exp_Code,Exp_Amt,Attachments,Sf_Code,Expense_Type,Ukey)values('".$TASlNo."','".$pk."','".$date."','".$Add_Exp[$i]["type"] ."','".$Add_Exp[$i]["total_amount"]."','".$Add_Exp[$i]["attach_count"]."','".$sfCode."','".$Add_Exp[$i]["exp_type"]."','".$Add_Exp[$i]["u_key"]."')";
		performQuery($expsql);
    ...
    for ($i = 0; $i < count($Other_Exp); $i++) {
		...
		$expsql="insert into Trans_Additional_Expense(SlNo,Fk_Sl_No,Exp_Date,Exp_Code,Exp_Amt,Attachments,Sf_Code,Expense_Type,Ukey)values('".$Fk_Sl_No."','".$pk."','".$date."','".$Other_Exp[$i]["type"] ."','".$Other_Exp[$i]["total_amount"]."','".$Other_Exp[$i]["attach_count"]."','".$sfCode."','".$Other_Exp[$i]["exp_type"]."','".$Other_Exp[$i]["u_key"]."')";
		performQuery($expsql);
    ...
    for ($k = 0; $k < count($Trv_details["trv_loca"]); $k++){
		...
		$sql="insert into Trans_Travelled_Loc(SlNo,FK_Trv_SlNo,From_P,To_P,Sf_Code,Mode,Fare,Ukey) select '" . $trvsl . "','".$pk."','".str_replace("'","''",$Traveled_Loc[$k]["from"])."','".str_replace("'","''",$Traveled_Loc[$k]["to"])."','".$sfCode."','".$Traveled_Loc[$k]["mode"]."','".$Traveled_Loc[$k]["fare"]."','".$Traveled_Loc[$k]["u_key"]."'";
		performQuery($sql);
	...
	if($Lodg_details["ldg_type"]!=""){
		...
		$Ldgsql="insert into Trans_Lodging_Details(SlNo,FK_SlNo,Lodging_Type,Eligible,Bill_Amt,WOB_Amt,Add_SF_Emp_ID,Ldg_Stay_Loc,Stay_Date,Driver_Ldg_Amount,Joining_Ldg_Amount,Total_Ldg_Amount,Attachment,Sf_Code,To_Date,NO_Of_Days,Ukey,Continuous_Stay,Early_Checkin,Late_Checkout,Erly_Check_in,Erly_Check_out,Ear_bill_amt,lat_chec_in,lat_check_out,lat_bill_amt) values(
				...
				'".str_replace(" ".$Lodg_details["to_dte"],"", $Lodg_details["toout_dte"])." ".$Lodg_details["to_dte"]."','".$Lodg_details["lat_bill_amt"]."')"; ///lat_check_out
		performQuery($Ldgsql); 
