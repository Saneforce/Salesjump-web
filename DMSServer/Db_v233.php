 ...
 $sql = "insert into Trans_TP_Hist(SF_Code,Tour_Month,Tour_Year,Submission_date,Tour_Date,WorkType_Code_B,Worktype_Name_B,Tour_Schedule1,Objective,Worked_With_SF_Code,Division_Code,Territory_Code1,Worked_With_SF_Name,TP_Sf_Name,HQ_Code,HQ_Name,Confirmed,Change_Status,JointWork_Name,Retailer_Code,Retailer_Name) select '" . $sfCode . "',MONTH($tourDate),YEAR($tourDate),getdate(),$tourDate,$worktype_code,$worktype_name,$RouteCode,$objective,$worked_with_code," . $divisionCode[0] . ",$RouteName,$worked_with_name,$sfName,$HQ_Code,$HQ_Name,0,0,$ww,'".$RCode."','".$RName."'";
        ...
        case "LeaveReject":
           ...
           $Mrsfcode = $_GET['sfCode'];
			...
			$Sl_No = $_GET['Sl_No'];
			$Mrsfcode = $_GET['sfCode'];
             $sql = "update Blocked_Entry set SF_Status=2,Release_Reason=" . $vals['reason'] . "  ,LastUpdt_Date='" .date('Y-m-d H:i:s'). "',Approved_to='" . $Mrsfcode . "' where Sl_No=$Sl_No ";
            performQuery($sql);
           ...
           /*  $sql = "SELECT DeviceRegId FROM Access_Table where sf_code=(select SF_Code from Blocked_Entry where Sl_No=$Sl_No)";
			...
			$Sl_No = $_GET['Sl_No'];
			$Mrsfcode = $_GET['sfCode'];
             $sql = "update Blocked_Entry set SF_Status=3 , AudiLstupdate='" .date('Y-m-d H:i:s'). "',Approved_to='" . $Mrsfcode . "' where Sl_No=$Sl_No ";
            performQuery($sql);
           ...
           /*  $sql = "SELECT DeviceRegId FROM Access_Table where sf_code=(select SF_Code from Blocked_Entry where Sl_No=$Sl_No)";
            ...
            $Sl_No = $_GET['Sl_No'];
			$Mrsfcode = $_GET['sfCode'];
            $sql = "update Blocked_Entry set SF_Status=2,AuditReason=" . $vals['reason'] . "  ,AudiLstupdate='" .date('Y-m-d H:i:s'). "',Approved_to='" . $Mrsfcode . "' where Sl_No=$Sl_No ";
            performQuery($sql);
        ...
        case "LeaveCancelReject":
           ...
           $Mrsfcode = $_GET['sfCode'];
            ...
            performQuery($sql);
            $sql = "update Leave_Cancel_Form set Leavecancel_Id=1 ,LastUpdt_Date='" .date('Y-m-d H:i:s'). "',Approved_to='" . $Mrsfcode . "' where Leave_Id=$leaveid ";
            performQuery($sql);
		...
		case "LeaveFormValidate":
            $leaveid = $_GET['leaveid'];
            $query = "exec iOS_getLvlValidate '$sfCode','" . $vals['From_Date'] . "'," . $vals['To_Date'] . "," . $vals['Leave_Type'] . ",'".$vals['PChk']."','".$vals['Shift']."'";
		...
		case "PermissionFormValidate":
			...
			case "GateOut":
			$sql = "SELECT isNull(count(Sl_No),0)+1 as RwID FROM QRGateEntry where  Sf_Code='$sfCode' convert(date,IEntryDate)=convert(date,'".$vals['time']."')";
            $tRw = performQuery($sql);
            $pk = (int) $tRw[0]['RwID'];
		    $divCode = $_GET['divisionCode'];
			...
			$updatequery = "update QRGateEntry set OHQLoc='".$vals['HQLoc']."',OLocation='".$vals['Location']."',OlatLng='".$vals['latLng']."',OEntryDate='".$vals['time']."',OServerDate='".date('Y-m-d H:i:s')."' where  Sf_Code='$sfCode' and convert(date,IEntryDate)=convert(date,'".$vals['time']."')";
...
case "WeekofPunch":
  ...
  case "DCRMissedDates":
   ...
   case "LeaveForm":
			...
			$sql = "insert into Mas_Deviation_Form(Deviation_Id,Sf_Code,Sf_Name,Deviation_Type,Deviation_Date,MobileDate,Server_Date,LatLng,Reason,Division_Code,Devi_Active_Flag)values('$pk','$sfCode','".$name."','" . $vals['Deviation_Type'] . "','" . $vals['From_Date'] . "'," . $vals['Time'] . ",'".$dt."',".$vals['LatLng']."," . $vals['reason'] . ", '$Owndiv','2')";
            performQuery($sql);
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
            ...
            case "vwleavetype":
                $query = "select Leave_Code id,Leave_Name name,Leave_SName,COffType from vwLeaveType where Division_code='" . $Owndiv . "'";
                $results = performQuery($query);
                break;
 ...
 $query = "update Mas_SF_SlNo set POrdSl=(isnull(POrdSl,0)+1) where SF_Code='" . $sfCode . "'";
    ...
    case "vwLeave":
        ...
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwAprovedBlockRelease where Reporting_To_SF='$sfCode' ";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
	...
	case "vwExtended":
        ...
        $sql = "select Sf_Code,Sf_Name SFNm,Shift_Name,convert(varchar(10), Approved_Date, 103) Rejectdate,Missed_punch_date,convert(varchar,Missed_punch_date,103) Missed_punch_Flag,convert(varchar(10), Entry_Date, 120) Submission_date ,Reason,Checkin_Time,
...
inner join Mas_Salesforce s on s.Sf_Code=TA.SF_Code where  WrkType>0 and TA.Approved_by='$rSF' order by login_date desc";
		...
		( select Sft_Name from Mas_Shift_Timings  where Sft_ID=TA.ShiftSelected_Id) as Shift_name,
				...
				from TP_Attendance_App  TA
inner join Mas_Salesforce s on s.Sf_Code=TA.SF_Code where  Duty_Mode>0 and TA.Approved_by='$rSF' order by login_date desc";
		}else{
		...
		( select Sft_Name from Mas_Shift_Timings  where Sft_ID=TA.ShiftSelected_Id) as Shift_name,
				...
				when Duty_Mode='2' then 'Approved' else 'Reject' end OStatus,
				case when Duty_Mode='1' then 'rgb(255,165,0) !important'  when Duty_Mode='2' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,Reject_reason
				from TP_Attendance_App  TA where  Duty_Mode>0 and Sf_Code='$sfCode' order by login_date desc";
        }
		$ondutystatus = performQuery($sql);
        outputJSON($ondutystatus);
        break;
...
case "get/RetailerPaymentCollection":
 ...
 case "get/GoogleLogin":
		...
		$sql = "SELECT  UsrDfd_UserName Sf_UserName,iif(SF_Status!=0,3,SF_Status) SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,(select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,
		...
		break;
		 ...
		 case "get/GoogleLoginpayroll":
		$Email = $_GET['username']; 
		$password = $_GET['Password'];		//CASE WHEN  UsrDfd_UserName='HAPHTNSAO2' THEN '18.7357416:78.9700081' ELSE '12.91729615:80.23030043' END 
		$DeviceRegId = $_GET['DvID'];
		$sql = "SELECT  UsrDfd_UserName Sf_UserName,iif(SF_Status!=0,3,SF_Status) SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,(select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,
		case when Approved_By='FF' then 1 else 0 end SFFType,case when Approved_By!='FF' and Approved_By!='I' then 1 else 1 end OTFlg,
		isnull(latlong,'12.91729615:80.23030043') AS HOLocation from mas_salesforce ms inner join Mas_HQuarters m on cast(HQ_ID as varchar)=SF_HQ  inner join access_master am on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT on At.Sf_code= ms.Sf_code where Sf_UserName='".$Email."' and Sf_Password='".$password."' and Approved_By<>'I'";
		$res = performQuery($sql);
...
case "get/ClearData":
	...
	case "get/tknPerm":
		...
		$mDt = strtotime($mDt."- 1 day");
		$tDt=date('Y-m-d',$mDt);
		$query = "select isnull(sum(cast(Noof_hours as int)),0)  tknHrS from PermissionEntry where Approval_Flag!=1 and SF_Code='".$sfCode."' and Permissiondate between '".$fDt."' and '".$tDt."'"; 
		//echo($query);
		outputJSON(performQuery($query));
		break;	
