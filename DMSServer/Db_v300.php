...
include "Checkutils.php";
 //$query = "insert into reqDetails select 'DBV300','".$_GET["axn"]."','".$_POST['data']."'"; 
 //performQuery($query);
...
function actionLogin() {
    ...
    if ($count == 1) {
        ...
        $dat=date('Y-m-d');
        $sql="select * from TP_Attendance_App WITH (NOLOCK) where Sf_Code='" . $arr[0]['SF_Code'] . "' and DATEADD(dd, 0, DATEDIFF(dd,0,Start_Time))='$dat'";
...
function outletbulksync($drCode){	
	$user_name = 'adminhap';
	$result=[];
	$password = '6MyBdmD4haWkTsG';
	$custcode=str_replace("'","",$drCode);
	$host="http://hapi.sanfmcg.com/api/sap/BulkPost_HAP_Portal?CustCode=$custcode";
	...
	$result["Msg"]=str_replace("\"","",$apiresult);
	curl_close( $ch );
	return $result;
}
function outletsync($drCode){	
	$user_name = 'adminhap';
	$password = '6MyBdmD4haWkTsG';
	$custcode=str_replace("'","",$drCode);
	$host="https://hapi.sanfmcg.com/api/sap/Post_HAP_Portal?CustCode=$custcode";
...
function getAttndStatus(){
    ...
    return performQuery($querystatus);;
}
...
function getAttndCnts(){
	...
	//if($RSF=='MGR0016' || $RSF=='MGR0013' || $RSF=='MGR0020') {//||$RSF=='MGR5146'||$RSF=='MGR0017'||$RSF=='MGR0018'||$RSF=='MR0032'){
		...
		*/
		...
		$query = "select count(Week_date) wkCnt from Trans_Weekoff WITH (NOLOCK) where sf_code='" . $RSF . "' and cast(Week_date as dateTime)>='".$fDt."' and cast(Week_date as dateTime)<='".$tDt."'";// MGR1510 MGR0017
		...
		if($wkCnt>0)
		$result[]=array('SF_Code' => $RSF, 'Status' => 'Weekly off ('.$DyDet['wkoff'].' days)','StatusCnt' => $wkCnt, 'StusClr' => '#089c08 !important');
		
		if($cONDy>0){
			$result[]=array('SF_Code' => $RSF, 'Status' => 'On-Duty','StatusCnt' => $cONDy,'StusClr' => '#ff920b !important');
		}
		$aOnTm=FilterArry($RRes,"AInSt",1);
		if(count($aOnTm)>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'Grace time','StatusCnt' => count($aOnTm), 'StusClr' => '#ff920b !important');}
		$phrs=0;foreach($RRes as $a){$phrs=$phrs+$a['Perm'];}
		if($phrs>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'Permission (2 Hrs)', 'StatusCnt' => $phrs, 'StusClr' => '#ff0000 !important');}
		...
		if($lvDys>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'Leave', 'StatusCnt' => str_replace(".0", " ",$lvDys), 'StusClr' => '#ff920b !important');}
		if($hlyDys>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'Holiday', 'StatusCnt' => str_replace(".0", " ", $hlyDys), 'StusClr' => '#ff920b !important');}
		if($lopDys>0){$result[]=array('SF_Code' => $RSF, 'Status' => 'LOP Leave','StatusCnt' => str_replace(".0", " ",$lopDys),'StusClr' => '#ff920b !important');}
		
		if($Devi_Count>0)
		$result[]=array('SF_Code' => $RSF, 'Status' => 'Deviation Entry','StatusCnt' => $Devi_Count, 'StusClr' => '#ff0000 !important');
...
function TpRouteNotify() {
  ...
  $today = date('Y-m-d H:i:s');
    ...
    $sql = "SELECT Sf_Name,Reporting_To_SF,sf_type FROM Mas_Salesforce WITH (NOLOCK) where Sf_Code='$sfCode'";

   $sfType = performQuery($sql);
   $sql = "SELECT  CONCAT('N',MAX(CONVERT(INT, RIGHT(NotifyID, LEN(NotifyID)-1)))+1) as id FROM Mas_AppNotifyList WITH (NOLOCK)";
...
$sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select Reporting_To_SF from mas_salesforce where Sf_Code='$sfCode')";
...
function getDailyAllow() {
	...
	$date=date('Y-m-d');
	$sfCode = $_GET['sfCode'];
    $query = "select ID,Allowance_Name Name,Max_Allowance,Attachemnt from Mas_Allowance_Type WITH (NOLOCK) where Type=1 and Division_Code=". $Owndiv ." and user_enter=1 and active_flag=0";
	$ExpenseWeb=performQuery($query);
	$query = "select * from Expense_Start_Activity WITH (NOLOCK) where Sf_code='$sfCode'  and convert(date,Date_Time)='".$date."'";
...
function getOfferNotify(){
    $sfCode = $_GET['CusCode'];
    $query = "select 'http://hap.sanfmcg.com/offerimg/'+offerimg offerimg from Mas_Dist_AppOffers where Eff_from<='".date("Y-m-d 00:00:00")."' and isnull(Eff_to,getdate())>='".date("Y-m-d 00:00:00")."'";
    return performQuery($query);
}
	...
	$sfCode=$Field_Code[0]['Field_Code'];
		...
		if (!is_null($sfCode)) {
			if ($tableName == 'vwTown_Master_APP' )                                  
				...
				$query .=" WHERE charindex('$sfCode',','+tab.Distributor_Code+',')>0";
...
function getEmpDetByID(){
	...
	$Empno=$_GET['rSF'];
	$dt=$_GET['desig'];
	$sql = "exec getEmpDetByID_old '" . $Empno . "','$dt'";//,'".$SF."'";
	return performQuery($sql);
}

function getEmpDetByName(){
	$Empno=$_GET['rSF'];
	$sql = "select * from mas_salesforce WITH (NOLOCK) where sf_emp_id='".$Empno."'";
	return performQuery($sql);
}
function saveAllowance($data){
	...
	$sql="";
		...
		$pk =  (int)$topr[0]['Sl_No'];
		$sqldub ="select Sf_code from Expense_Start_ActivityWITH (NOLOCK) where Sf_code='".$sfCode."' and Date_Time='".$Datetime."' and UKey='".$Ukey."'";
...
function addEntry() {
    ...
    $vals = $data[0][$temp[0]];
    $SFState='0';
    $sql = "SELECT Employee_Id,case sf_type when 4 then 'MR' else 'MGR' End SF_Type,State_Code FROM vwUserDetails WITH (NOLOCK) where SF_code='" . $sfCode . "'";
    $as = performQuery($sql);
...
case "SSstockUpdation":
 ...
 if( $vals['profilepic']!=null && $vals['profilepic']!="undefined")
			...
			if($vals["Button_Access"]==null)
                    $Button_Access="";
            else
                    $Button_Access=$vals["Button_Access"];
					
					$Hq_Id=$vals["HQid"];
					$Chilling_Id=$vals["Chilling_Id"];
					$Shift_Type_Id=$vals["Shift_Type_Id"];
					$today=str_replace("'", "", $today);
					$sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works, Hq_Id,Chilling_Id,SHift_Type_Id,Fromdate,Todate,Button_Access) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,'','$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."',$Hq_Id,$Chilling_Id,$Shift_Type_Id,'','','$Button_Access'";  
					performQuery($sql);

//$missedquery="exec svPostMissAndAuto  '$sfCode','".str_replace("'", "",$today)."','" . $Owndiv . "' ";
...
//performQuery($missedquery);
        ...
        case "newunlisted_doctor_master":
            if($vals["unlisted_cat_code"]==null)$vals["unlisted_cat_code"]=0;
            $Ukey=$vals['unlisted_doctor_name'].date('Y-m-d H:i:s');
            $divCode = $_GET['divisionCode'];
            $divisionCode = explode(",", $divCode);
            $sqlrnd="SELECT isnull(Retailer_add_ND,0) from Access_Master WITH (NOLOCK) where division_code='".$divisionCode[0]."'";
			...
			if(count($tr)>0&&($divCode=="4,"||$divCode=="29,")){
				$resp["success"] = false;
				$resp["sql"] = $sql;
				$resp["msg"]="Outlet already exists..";
				echo json_encode($resp);
				die;
			}
			$today=date('Y-m-d H:i:s');
			...
			$visitpath='';     
			$outletCategory=$vals["outlet_type_Details"];
			$freezer_files=$vals["freezer_file_Details"];
			$sqlsp=$vals['unlisted_doctor_name']." message:".$sql."sf_code=".$_GET['sfCode']."\n";
			$ListedDr_Active_Flag=str_replace("'","",$vals["flag"]);
			$freezerattach="";
			if($flagofretailerND==1){
				$ListedDr_Active_Flag=3;
			}
			for ($i = 0; $i < count($freezer_files); $i++) {
				$freezerattach=$freezerattach.",".$freezer_files[$i]["freezer_filename"];
			}
			
				$ListedDr_Active_Flag=0;
            $retsql = "exec svListedDR_APP_New '$sfCode','" . str_replace("'","",$vals['unlisted_doctor_name']) . "','" . str_replace("'","",$vals['unlisted_doctor_address']) . "'," . $vals['unlisted_doctor_phone'] . ",'" . str_replace("'","",$vals['unlisted_doctor_cityname']) . "','" . str_replace("'","",$vals['unlisted_doctor_areaname']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson']) . "', ' " . str_replace("'","",$vals['unlisted_doctor_designation']) . "','" . str_replace("'","",$vals['unlisted_doctor_gst']) . "','" . str_replace("'","",$vals['unlisted_doctor_pincode']) . "'," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",$ListedDr_Active_Flag,'" . date('Y-m-d  H:i:s') . "',$Owndiv,0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ",'" . str_replace("'","",$vals['unlisted_doctor_phone2']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_designation2']) . "','" .str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','','" . $visitpath . "','".str_replace("'","",$Lat)."','".str_replace("'","",$Long)."','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_phone3']) . "','" . str_replace("'","",$vals['CatUniverSelectId']) . "','" . str_replace("'","",$vals['AvailUniverSelectId']) . "','" . str_replace("'","",$vals['HatsunAvailswitch']) . "' ,'" . str_replace("'","",$vals['categoryuniverseswitch']) . "' ,'" . str_replace("'","",$vals['reason_category']) . "' ,'" . str_replace("'","",$vals['unlisted_doctor_Email']) . "' ,'" . str_replace("'","",$vals['Compititor_Id']) . "' ,'" . str_replace("'","",$vals['Compititor_Name']) . "' ,'".str_replace("'","",$vals['unlisted_Owner_name']) . "','".$vals['VechType']."','".str_replace("'","",$vals['img_name'])."','".$vals["OutletTypeCd"]."','".str_replace("'","",$vals["outstanding_amount"])."','" . str_replace("'","",$vals['State_Code']) . "','".str_replace("'","",$vals['unlisted_doctor_secondphone'])."','".str_replace("'","",$vals['customer_code'])."','".str_replace("'","",$vals['CategoryType'])."','".str_replace("'","",$vals['category_type'])."','".str_replace("'","",$vals['sub_category'])."','".str_replace("'","",$vals['unlisted_specialty_name'])."','".$freezerattach."','".str_replace("'","",$vals['expected_sales_value'])."','".str_replace("'","",$vals['fssai_number'])."','".str_replace("'","",$vals['pan_number'])."','".str_replace("'","",$vals['freezer_make'])."','".str_replace("'","",$vals['freezer_tagno'])."','".str_replace("'","",$vals['freezer_status'])."','".str_replace("'","",$vals['freezer_capacity'])."','".str_replace("'","",$vals['deposit_amount'])."','".str_replace("'","",$vals["freezer_required"]). "','".str_replace("'","",$vals["districtname"]). "'";
			$res = performQuery($retsql); 
			
			/*$sql = "SELECT ListedDrCode from Mas_ListedDr where Ukey='" . str_replace("'","",$Ukey) . "'";
			$res = performQuery($sql);*/
			if (count($res) > 0){
				$cCode=$res[0]["Drcode"];
				$distname=$res[0]["DistCode"];
				for ($i = 0; $i < count($outletCategory); $i++) {
					$sql="insert into Outlet_Category_Mapping(OutletCode,DistCode,OutletCat_Type,Category_Code,Category_Name,Sub_Category_Code,Sub_Category_Name,Price_List_No) 
						select '".$cCode."','".$distname."','".$outletCategory[$i]["type_name"]."','".$outletCategory[$i]["cat_id"]."','".$outletCategory[$i]["cat_name"]."','".$outletCategory[$i]["subcat_id"]."','".$outletCategory[$i]["subcat_name"]."',(select top 1 Price_List_Sl_No from Mas_Doctor_Speciality where Doc_Special_Code='".$outletCategory[$i]["subcat_id"]."' and Doc_Special_Active_Flag=0)";
					performQuery($sql);
				}
			...
			$resp["cCode"] = $res;
			$resp["success"] = true;
			$resp["msg"]="Outlet Added successfully";
			echo json_encode($resp);
			die;
        break;
        case "unlisted_doctor_master":
			...
			/*$myfile = fopen("../server/rlog/errlog_".date('Y_m_d_H_i_s').".txt", "a+");
			$sqlsp="log_".date('Y_m_d_H_i_s')." data:".$_POST['data'];
			fwrite($myfile, $sqlsp);
			fclose($myfile);*/
            ...
            $retsql = "exec svListedDR_APP '$sfCode','" . str_replace("'","",$vals['unlisted_doctor_name']) . "','" . str_replace("'","",$vals['unlisted_doctor_address']) . "'," . $vals['unlisted_doctor_phone'] . ",'" . str_replace("'","",$vals['unlisted_doctor_cityname']) . "','" . str_replace("'","",$vals['unlisted_doctor_areaname']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson']) . "', ' " . str_replace("'","",$vals['unlisted_doctor_designation']) . "','" . str_replace("'","",$vals['unlisted_doctor_gst']) . "','" . str_replace("'","",$vals['unlisted_doctor_pincode']) . "','" . str_replace("'","",$vals["unlisted_specialty_code"]) . "'," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",$ListedDr_Active_Flag,'" . date('Y-m-d  H:i:s') . "',$Owndiv,'".str_replace("'","'",$vals['sub_categoryId'])."',0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ",'" . str_replace("'","",$vals['unlisted_doctor_phone2']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_designation2']) . "','" .str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','','" . $visitpath . "','".str_replace("'","",$Lat)."','".str_replace("'","",$Long)."','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_phone3']) . "','" . str_replace("'","",$vals['CatUniverSelectId']) . "','" . str_replace("'","",$vals['AvailUniverSelectId']) . "','" . str_replace("'","",$vals['HatsunAvailswitch']) . "' ,'" . str_replace("'","",$vals['categoryuniverseswitch']) . "' ,'" . str_replace("'","",$vals['reason_category']) . "' ,'" . str_replace("'","",$vals['unlisted_doctor_Email']) . "' ,'" . str_replace("'","",$vals['Compititor_Id']) . "' ,'" . str_replace("'","",$vals['Compititor_Name']) . "' ,'".str_replace("'","",$vals['unlisted_Owner_name']) . "','".$vals['VechType']."','".str_replace("'","",$vals['img_name'])."','".$vals["OutletTypeCd"]."','".str_replace("'","",$vals["outstanding_amount"])."','" . str_replace("'","",$vals['State_Code']) . "','".str_replace("'","",$vals['unlisted_doctor_secondphone'])."','".str_replace("'","",$vals['customer_code'])."','".str_replace("'","",$vals['CategoryType'])."'";
			performQuery($retsql); 		   
			...
			'" .str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','','" . $visitpath . "',".$Lat.",".$Long.",'" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_phone3']) . "','" . str_replace("'","",$vals['CatUniverSelectId']) . "','" . str_replace("'","",$vals['AvailUniverSelectId']) . "','" . str_replace("'","",$vals['HatsunAvailswitch']) . "' ,'" . str_replace("'","",$vals['categoryuniverseswitch']) . "' ,'" . str_replace("'","",$vals['reason_category']) . "' ,'" . str_replace("'","",$vals['unlisted_doctor_Email']) . "' ,'" . str_replace("'","",$vals['Compititor_Id']) . "' ,'" . str_replace("'","",$vals['Compititor_Name']) . "' ,'".str_replace("'","",$vals['unlisted_Owner_name']) . "' )}";
			performQueryWP($sql,[]); */
			...
			if (count($res) > 0){
				...
				/*outletsync($cCode);
				$query="exec KTTSyncOutlet " . $cCode . "";
				$rss = performQuery($query);*/
				$vals["addrs"]="'".getaddress(str_replace("'","",$Lat),str_replace("'","",$Long))."'";
				$sql = "SELECT isNull(max(MapId),0)+1 as MapId FROM Map_GEO_Customers";
				$topr = performQuery($sql);
				$pk = (int) $topr[0]['MapId'];
       ...
       $sql = "SELECT isNull(max(Sl_No),0)+1 as Sl_No FROM Trans_Product_Availability";
        ...
        case "TP_Attendance":
			...
			*/
			...
			$HMSG="";
			if($On_Duty_Flag==1){
				$sql = "select count(sf_code) HCount from TP_Attendance_App WITH (NOLOCK) where sf_code='$sfCode' and convert(date,Start_Time)=convert(date,'$eDate')  and Duty_Mode=0";
				$device = performQuery($sql);
			...
			$sql = "select isnull(sum(No_of_Days),0)Lcount from mas_Leave_Form WITH (NOLOCK) where Sf_Code='$sfCode'  and convert(date,From_Date)=convert(date,getdate()) and Leave_Active_Flag=2 ";
            ...
            if($HCount==0 && ($Lcount==0|| $Lcount==0.5 ) ){
				...
				if($On_Duty_Flag==1){
					...
					$tDt=date('Y-m-d',$mDt);
					$ct=0;
					$query = "select count(distinct Login_date) Cnt from TP_Attendance_App WITH (NOLOCK) where Duty_Mode in (1,2) and SF_Code='$sfCode' and login_date between '".$fDt."' and '".$tDt."'";
					$res = performQuery($query);
					$ct=$res[0]["Cnt"];
					$query = "select count(distinct Week_date) Cnt from Trans_Weekoff WITH (NOLOCK) where SF_Code='$sfCode' and Week_date between '".$fDt."' and '".$tDt."'";
					$res = performQuery($query); 
					$ct=$ct+$res[0]["Cnt"];
					$DyDet=mWkDysCnt($fDt,$tDDt);
					
					
					$query = "select count(distinct HolidayDate) Cnt from Mas_Holiday_Dates_Detail H WITH (NOLOCK) inner join Mas_Salesforce S WITH (NOLOCK) on H.stateCode=S.State_Code where sf_Code='$sfCode' and HolidayDate between '".$fDt."' and '".$tDt."'";
					$res = performQuery($query);
					$ctn=$res[0]["Cnt"];
					$ctn=$ctn+$DyDet['wkoff'];
				...
				$ResultCom["call"]=saveAllowance($vals);
				}
				
				$HMSG="Your Check-In Submitted Successfully";
				$result["success"]=true;
				$ResultCom["success"]=true;
				}
...
$va =$data[1][$tempp[0]];
 ...
 if( $data[2]['ActivityCaptures']!=null && $data[2]['ActivityCaptures']!="undefined")
      ...
      case "Tour_Plan":
			...
			$Hq_Id	=$data[0]['Tour_Plan']['HQid'];
			$Chilling_Id	=$data[0]['Tour_Plan']['Chilling_Id'];
            $SHift_Type_Id	=$data[0]['Tour_Plan']['Shift_Type_Id'];
			$Fromdate	   =$data[0]['Tour_Plan']['Fromdate'];
			$Todate	       =$data[0]['Tour_Plan']['Todate'];
        ...
        case "TourPlanSubmit":
            $month = $_GET['month'];
            $year = $_GET['year'];
            $sql = "update Trans_TP_One set Change_Status=1,Confirmed=1,submit_status=2 where Tour_Month=$month and Tour_Year=$year and Sf_Code='$sfCode' and submit_status='1'";
            performQuery($sql);
            $resp["success"] = true;
			 $resp["query"] = $sql;
            echo json_encode($resp);
        ...
        case "TPApproval":
            ...
            $sql = "insert into Trans_TP(Division_Code,SF_Code,Worked_With_SF_Code,Worked_With_SF_Name,Tour_Date,Tour_Month,Tour_Year,WorkType_Code_B,Worktype_Name_B,WorkType_Code_B1,Worktype_Name_B1,WorkType_Code_B2,Worktype_Name_B2,Objective,Confirmed,Confirmed_Date,Rejection_Reason,Territory_Code1,Territory_Code2,Territory_Code3,TP_Sf_Name,TP_Approval_MGR,Tour_Schedule1,Tour_Schedule2,Tour_Schedule3,Submission_date,Change_Status,Worktype_Flag,Hq_Id,Chilling_Id,SHift_Type_Id,Fromdate,Todate,MOT,DA_Type,Driver_Allow,From_Place,To_Place,Mot_ID,To_Place_ID,Mode_Travel_Id,Button_Access)
                        select Division_Code,SF_Code,Worked_With_SF_Code,Worked_With_SF_Name,Tour_Date,Tour_Month,Tour_Year,WorkType_Code_B,Worktype_Name_B,WorkType_Code_B1,Worktype_Name_B1,WorkType_Code_B2,Worktype_Name_B2,Objective,Confirmed,GETDATE(),Rejection_Reason,Territory_Code1,Territory_Code2,Territory_Code3,TP_Sf_Name,TP_Approval_MGR,Tour_Schedule1,Tour_Schedule2,Tour_Schedule3,Submission_date,Change_Status,Worktype_Flag,Hq_Id,Chilling_Id,SHift_Type_Id,Fromdate,Todate,MOT,DA_Type,Driver_Allow,From_Place,To_Place,Mot_ID,To_Place_ID,Mode_Travel_Id,Button_Access
                            from Trans_TP_One where convert(date,Tour_Date)='$Tour_date' and Sf_Code='$Mr_Sfcode'";
            $trs = performQuery($sql);
            if (count($trs) > 0) {
                $sql = "delete from Trans_TP_One where convert(date,Tour_Date)='$Tour_date' and Sf_Code='$Mr_Sfcode'";
                performQuery($sql);
            }
            $resp["success"] = true;
            echo json_encode($resp);
        ...
        case "TPReject":
            ...
            $sql = "update Trans_TP_One set Change_Status=2,Confirmed=0,Rejection_Reason=" . $vals['reason'] . " where convert(date,Tour_Date)='$Tour_date' and Sf_Code='$Mr_Sfcode'";
            performQuery($sql);
            $resp["success"] = true;
			 $resp["query"] = $sql;
            echo json_encode($resp);
        ...
        case "LeaveApproval":
            ...
            performQuery($sql);
			$query = "exec UpdLvDays '$leaveid'";
			performQuery($query);*/

			$query = "exec UpdLvDays '$leaveid','" . $Mrsfcode . "'";
			performQuery($query);
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select SF_Code from Mas_leave_Form where Leave_Id='$leaveid')";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
        ...
        case "LeaveReject":
           ...
           $Mrsfcode = $_GET['sfCode'];
            ...
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select SF_Code from Mas_Deviation_Form where Deviation_Id=$deviationid)";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
           ...
           $Mrsfcode = $_GET['sfCode'];
            ...
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select Sf_Code from Trans_NewJoin where  where Sl_No=$Sl_No)";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
           ...
           $Mrsfcode = $_GET['sfCode'];
            ...
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select Sf_Code from Trans_NewJoin where Sl_No=$Sl_No)";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
		...
		case "PermissionApproval":
            ...
            $Sl_no = $_GET['Sl_no'];
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update PermissionEntry set Approval_Flag=0,LastUpdt_Date='" .date('Y-m-d H:i:s'). "',Approvedby='" . $Mrsfcode . "' where Sl_no=$Sl_no";
            performQuery($sql);
			
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select SF_Code from PermissionEntry where Sl_no='$Sl_no')";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
		...
		case "ondutyApproval":
            ...
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update TP_Attendance_App set WrkType=2,Approved_Date='" .date('Y-m-d H:i:s'). "',Approved_by='" . $Mrsfcode . "' where id=$duty_id";
            performQuery($sql);
			
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select SF_Code from TP_Attendance_App where id=$duty_id)";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
		 ...
		 case "HolidayApproval":
            ...
            $duty_id = $_GET['duty_id'];
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update TP_Attendance_App set Duty_Mode=2,Approved_Date='" .date('Y-m-d H:i:s'). "',Approved_by='" . $Mrsfcode . "' where id=$duty_id";
            performQuery($sql);
		 ...
		 case "MissedApproval":
            ...
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update Missed_punch_Entry set Missed_punch_Flag=2,Approved_Date='" .date('Y-m-d H:i:s'). "',Approved_by='" . $Mrsfcode . "' where Sl_No=$Sl_No";
            performQuery($sql);
            ...
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select SF_Code from Missed_punch_Entry where Sl_No='$Sl_No')";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
		 ...
		 case "ExtendedApproval":
            ...
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update Extended_entry set WrkType=2,Total_Numbers='" . $Numbers . "'
			,Approved_Date='" .date('Y-m-d H:i:s'). "',
			Approved_by='" . $Mrsfcode . "' where id=$Sl_No";
            performQuery($sql);
			
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code=(select SF_Code from Extended_entry where id='$Sl_No')";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
		...
		case "NTPApproval":
			...
			$Mr_Sfcode = $_GET['Mr_Sfcode'];
            $managerSf_Code = $_GET['sfCode'];
			
            $sql = "insert into Trans_TP(Division_Code,SF_Code,Worked_With_SF_Code,Worked_With_SF_Name,Tour_Date,Tour_Month,Tour_Year,WorkType_Code_B,Worktype_Name_B,WorkType_Code_B1,Worktype_Name_B1,WorkType_Code_B2,Worktype_Name_B2,Objective,Confirmed,Confirmed_Date,Rejection_Reason,Territory_Code1,Territory_Code2,Territory_Code3,TP_Sf_Name,TP_Approval_MGR,Tour_Schedule1,Tour_Schedule2,Tour_Schedule3,Submission_date,Change_Status,submit_status,MOT,DA_Type,Driver_Allow,From_Place,To_Place,Mot_ID,To_Place_ID,Mode_Travel_Id,Button_Access)
                        select Division_Code,SF_Code,Worked_With_SF_Code,Worked_With_SF_Name,Tour_Date,Tour_Month,Tour_Year,WorkType_Code_B,Worktype_Name_B,WorkType_Code_B1,Worktype_Name_B1,WorkType_Code_B2,Worktype_Name_B2,Objective,Confirmed,GETDATE(),Rejection_Reason,Territory_Code1,Territory_Code2,Territory_Code3,TP_Sf_Name,'" . $managerSf_Code . "',Tour_Schedule1,Tour_Schedule2,Tour_Schedule3,Submission_date,Change_Status,'3',MOT,DA_Type,Driver_Allow,From_Place,To_Place,Mot_ID,To_Place_ID,Mode_Travel_Id,Button_Access
                            from Trans_TP_One where sf_Code='$Mr_Sfcode' and convert(date,Tour_Date)='$Start_date'";
            ...
            if (count($trs) > 0) {
                $sql = "delete from Trans_TP_One where sf_Code='$Mr_Sfcode'  and convert(date,Tour_Date)='$Start_date' ";
                performQuery($sql);
            }
			$sql = "SELECT DeviceRegId FROM Access_Table where sf_code='" . $Mr_Sfcode . "'";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
				$msg = "Your Tour plan  has been Approved for \n" . $Start_date . "";
                sendFCM_notify($reg_id, $msg,"TP Approval",0,'#sign-in');
            }
		 break;
		...
		case "NTPApprovalR":
			...
			$Mr_Sfcode = $_GET['Mr_Sfcode'];
            $managerSf_Code = $_GET['sfCode'];
            $sql = "insert into TP_Reject_B_Mgr(SF_Code,Tour_Year,Division_Code,Rejection_Reason,Reject_Date) select '" . $Mr_Sfcode . "','" . date('Y-m-d H:i') . "',$Owndiv," . $vals['reason'] . ",'$Start_date'";
			performQuery($sql);
			$sql = "delete from Trans_TP_One where  sf_Code='$Mr_Sfcode'  and convert(date,Tour_Date)='$Start_date' ";
		    performQuery($sql);
            //$sql = "update Trans_TP_One set Change_Status=2,submit_status=4,TP_Approval_MGR='" . $managerSf_Code . "',Confirmed_Date='" . date('Y-m-d H:i') . "',Confirmed=0,Rejection_Reason=" . $vals['reason'] . " where sf_Code='$Mr_Sfcode'  and convert(date,Tour_Date)='$Start_date' ";
            $sql = "delete from Trans_TP where  sf_Code='$Mr_Sfcode'  and convert(date,Tour_Date)='$Start_date' ";
			performQuery($sql);
			$sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code='" . $Mr_Sfcode . "'";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
            if (!empty($reg_id)) {
				$msg = "Your PJP  has been Reject for \n" . $Start_date . "";
                sendFCM_notify($reg_id, $msg,"Reason:". $vals['reason'],0,'#sign-in');
            }
		 break;
		...
		case "LeaveFormValidate":
            $leaveid = $_GET['leaveid'];
            $query = "exec iOS_getLvlValidate '$sfCode','" . $vals['From_Date'] . "','" . str_replace("'", "",$vals['To_Date']) . "'," . $vals['Leave_Type'] . ",'".$vals['PChk']."','".$vals['Shift']."'";
		...
		case "PermissionFormValidate":
             ...
             $query = "exec iOS_getPermissionlValidate '$sfCode','" . str_replace("'","",$vals['pdate']) . "',".$vals['Noof_Count']."";
              $results = performQuery($query); 
             $resp['Msg'] = $results[0]['Msg'];
             $resp["success"] = true;
			 $resp["Query"] = $query;
			...
			$query = "insert into PermissionEntry  select '".$pk."','$sfCode','".$Owndiv."','".$vals['pdate']."','".$vals['start_at']."','".$vals['end_at']."','".$vals['No_of_Hrs']."','2','" . date('Y-m-d H:i') . "', '".str_replace("'","''",$vals['Reason'])."',' ','".$sf_name."','','' ";
            $results = performQuery($query); 
			...
			$resp["Msg"]="Permission Submited Successfully";
            $resp["success"] = true ;
			$resp["Query"] = $query ;
            echo json_encode($resp);
            die;
            ...
            $tRw = performQuery($sql);
            $pk = (int) $tRw[0]['RwID'];
			$query = "select Count(Leave_Id) Cnt from Leave_Cancel_Form WITH (NOLOCK) where Leave_Id='".$Leave_Id."'";
			$res = performQuery($query); 
			if($res[0]['Cnt']>0){
...
case "MissedPunchEntry":
			...
			$query = "select Count(Missed_punch_date) Cnt from Missed_punch_Entry WITH (NOLOCK) where sf_Code='$sfCode' and Missed_punch_date between '".$fDt."' and '".$tDt."' and Missed_punch_Flag!=3";
			$res = performQuery($query);
			if($vals['aflag']=='1'){
				$Datetime=date('Y-m-d H:i:s');
				$sql = "update Expense_Start_Activity set Enddateand_time='".$Datetime."',End_Km='".$vals['km']."',Personal_Km='".$vals['pkm']."',End_Image_Url='".$vals['url']."',End_MOT='".$vals['mod']."',End_remarks='".str_replace("'","''",$rmk)."',To_Place='".$vals['to']."',To_Place_Id='".$vals['to_code']."' where sf_Code='".$sfCode."' and convert(date,Date_Time)='".$vals['missed_date']."'";
			  performQuery($sql);
			}
...
case "WeekofPunch":
            ...
            $sql = "SELECT DeviceRegId FROM Access_Table WITH (NOLOCK) where sf_code='$sfCode'";
            $device = performQuery($sql);
            $reg_id = $device[0]['DeviceRegId'];
			$flag=0;
			$query = "select count(SF_code) Cnt from TP_Attendance_App WITH (NOLOCK) where SF_Code='$sfCode' and login_date='".$vals['WKDate']."'";
			$res = performQuery($query); 
            if($res[0]["Cnt"]>0){
				$resp["success"] = false;$flag=1;
				$resp["Msg"] = "Check-In Already Submited For ".$vals['WKDate']." Date.";
				/*if (!empty($reg_id)) {
					sendFCM_notify($reg_id, "Weeklyoff Can't Submit. ".$resp["msg"],"Notification",1,'#sign-in');
				}*/
			}
			$query = "select count(SF_code) Cnt from mas_Leave_Form WITH (NOLOCK) where SF_Code='$sfCode' and From_Date<='".$vals['WKDate']."' and To_Date>='".$vals['WKDate']."'";
			$res = performQuery($query); 
            if($res[0]["Cnt"]>0){
				$resp["success"] = false;$flag=1;
				$resp["Msg"] = "Leave Already Submited For ".$vals['WKDate']." Date.";
				/*if (!empty($reg_id)) {
					sendFCM_notify($reg_id, "Weeklyoff Can't Submit. ".$resp["msg"],"Notification",1,'#sign-in');
				}*/
			}
			$query = "select count(SF_code) Cnt from Trans_Weekoff WITH (NOLOCK) where SF_Code='$sfCode' and Week_date='".$vals['WKDate']."'";
			$res = performQuery($query); 
            if($res[0]["Cnt"]>0){
				$resp["success"] = false;$flag=1;
			...
			if($flag==0){	
				...
				$query="select CONVERT(varchar,Sf_Joining_Date,23)Joining_Date from Mas_Salesforce WITH (NOLOCK) where Sf_Code='$sfCode'";
				...
				$query = "select count(distinct from_Date) Cnt from Mas_Leave_Form WITH (NOLOCK) where SF_Code='$sfCode' and Leave_type=29 and from_Date between '".$fDt."' and '".$tDt."'";
				$res = performQuery($query);
				$ct=$ct+$res[0]["Cnt"];
				
				$query = "select count(distinct HolidayDate) Cnt from Mas_Holiday_Dates_Detail H WITH (NOLOCK) inner join Mas_Salesforce S WITH (NOLOCK) on H.stateCode=S.State_Code where sf_Code='$sfCode' and HolidayDate between '".$fDt."' and '".$tDt."' and HolidayDate>='".date('Y-m-d',resdt[0]["Joining_Date"])."'";
				$res = performQuery($query);
				$ctn=$res[0]["Cnt"];
				...
				$query = "select count(distinct Week_date) Cnt from Trans_Weekoff WITH (NOLOCK) where SF_Code='$sfCode' and cast(Week_date as date) between '".$fDt."' and '".$tDt."'";
				$res = performQuery($query); 
				$ct=$ct+$res[0]["Cnt"];
				...
				if($ct>=$ctn){						
					...
					$query = "select HolidayDate,HolidayName,HolidayRemarks from Mas_Holiday_Dates_Detail H WITH (NOLOCK) inner join Mas_Salesforce S WITH (NOLOCK) on H.stateCode=S.State_Code where sf_Code='$sfCode' and HolidayDate='".$vals['WKDate']."'";
					$res = performQuery($query);  
					if(count($res)>0){				
						$resp["success"] = false;
						...
						$query = "select Count(FK_SlNo) Cnt from Trans_Lodging_Details WITH (NOLOCK) where SF_Code='$sfCode' and cast(convert(varchar,To_Date,101) as datetime)='".$vals['WKDate']."'";
						$resl = performQuery($query); 
						$query = "select Count(FK_SlNo) Cnt from Trans_Lodging_Details WITH (NOLOCK) where SF_Code='$sfCode' and Continuous_Stay=1 and cast(convert(varchar,To_Date,101) as datetime)=dateadd(day,-1,'".$vals['WKDate']."')";
						$res = performQuery($query); 
						if($res[0]["Cnt"]>0 && $resl[0]["Cnt"]<1 ){
							$query = "select isnull(max(Sl_No),0)+1 Mx from Expense_Start_Activity";
							$res = performQuery($query); 
							$pkl=$res[0]["Mx"];
							$query = "select isnull(max(SlNo),0)+1 Mx from Trans_Lodging_Details";
							$res = performQuery($query); 
							$pkld=$res[0]["Mx"];
							$query ="insert into Expense_Start_Activity select '$sfCode','".$Owndiv."','".$vals['WKDate']."','','','','',null,null,null,null,null,1,null,'".$pkl."',null,null,null,null,From_Place,To_Place,Fare,Personal_Km,StEndNeed,dailyAllowance,driverAllowance,'',To_Place_Id,null,null,null,cast('".$vals['WKDate']."' as date) from Trans_Lodging_Details where SF_Code='$sfCode' and Continuous_Stay=1 and cast(convert(varchar,To_Date,101) as datetime)=dateadd(day,-1,'".$vals['WKDate']."')";
							performQuery($query);
							$query ="insert into Trans_Lodging_Details select '".$pkld."','".$pkl."',Lodging_Type,Eligible,Bill_Amt,WOB_Amt,Add_SF_Emp_ID,Ldg_Stay_Loc,Stay_Date,Driver_Ldg_Amount,Joining_Ldg_Amount,Total_Ldg_Amount,Attachment,Sf_Code,'".$vals['WKDate']."',NO_Of_Days,Ukey,Continuous_Stay,Early_Checkin,Late_Checkout,'".$vals['WKDate']."','".$vals['WKDate']."',Ear_bill_amt,'".$vals['WKDate']."','".$vals['WKDate']."',lat_bill_amt from Trans_Lodging_Details where SF_Code='$sfCode' and Continuous_Stay=1 and cast(convert(varchar,To_Date,101) as datetime)=dateadd(day,-1,'".$vals['WKDate']."')";
							performQuery($query);
						}
						$query = "insert into Trans_Weekoff  select '".$pk."','".$sf_name."','$sfCode','".$vals['WKDate']."','" . date('Y-m-d H:i') . "','".$Owndiv."','".str_replace("'","''",$vals['reason'])."' ";
						$results = performQuery($query); 
						$resp["Msg"] = "Weeklyoff Submitted Successfully.";
						$resp["success"] = true;
					}
				}
			}
  ...
  case "DCRMissedDates":
   ...
   case "LeaveForm":
            ...
            $sql = "SELECT LeaveAppproval FROM Access_Master WITH (NOLOCK) where division_code= '". str_replace(",", "",$divCode)."'";
            $LA = performQuery($sql);
            $leaveType= $LA[0]['LeaveAppproval'];
            $lvappp=2;
            if($leaveType==1) $lvappp=0;
			
            $sql = "SELECT Approved_By FROM Mas_Salesforce WITH (NOLOCK) where SF_Code='$sfCode'";
            $SFDet = performQuery($sql);
            ...
            $sql = "SELECT isNull(DedTyp,'2') DedTyp FROM mas_Leave_Type WITH (NOLOCK) where Leave_code=".$vals['Leave_Type']."";
            $tLt = performQuery($sql);
            $apl =$tLt[0]['DedTyp'];
			if($apl=='2'){ 
				$lvappp=0;
				//if($SFDet[0]["Approved_By"]!="I"){
					$query = "select HolidayDate,HolidayName,HolidayRemarks from Mas_Holiday_Dates_Detail H WITH (NOLOCK) inner join Mas_Salesforce S WITH (NOLOCK) on H.stateCode=S.State_Code where sf_Code='$sfCode' and HolidayDate='" . $vals['From_Date'] . "'";
					$res = performQuery($query); 
				
					$resp["HolidayQry"] = $query;
					if(count($res)<1){				
						$resp["success"] = false;
				...
				//if($SFDet[0]["Approved_By"]!="I"){
					$query = "select HolidayDate,HolidayName,HolidayRemarks from Mas_Holiday_Dates_Detail H WITH (NOLOCK) inner join Mas_Salesforce S WITH (NOLOCK) on H.stateCode=S.State_Code where sf_Code='$sfCode' and HolidayDate between '" . $vals['From_Date'] . "' and '" . str_replace("'","",$vals['To_Date']) . "'";
					$res = performQuery($query); 
					if(count($res)>0){				
						$resp["success"] = false;
			...
			$Todate=str_replace("'","",$vals['To_Date']);
			...
			{	
				...
				select '$pk1'," . $vals['Leave_Type'] . ",'" . $vals['From_Date'] . "','" . $Todate . "','" . str_replace("'","",$vals['Reason']) . "','$sfCode','$Owndiv','".$lvappp."','" . date('Y-m-d') . "'," . $vals['No_of_Days'] . "," . $vals['address'] . "," . $half_dayFlag . "," . $vals['HalfDay_Type'] . ",'" . $workeddate . "',
			...
			select '$pk'," . $vals['Leave_Type'] . ",'" . $vals['From_Date'] . "','" . $Todate . "','" . str_replace("'","",$vals['Reason']) . "','$sfCode','$Owndiv','".$lvappp."','" . date('Y-m-d h:i:s') . "'," . $vals['No_of_Days'] . "," . $vals['address'] . "," . $half_dayFlag . "," . $vals['HalfDay_Type'] . "";
            performQuery($sql);
			
			$resp["Msg"] = "Leave Submit Successfully";
            if($leaveType==1){
				...
				$sql = "select a.*,b.Leave_SName from MasEntitlement  a WITH (NOLOCK) inner join mas_Leave_Type   b WITH (NOLOCK) ON a.LeaveCode = b.Leave_code where a.SFCode='$sfCode' and DivisionCode='". str_replace(",", "",$divCode)."' and a.LeaveCode=" . $vals['Leave_Type'] . "";
...
/*                if ($ARDCd == "Exists") {
			...
			$totalproductitem = $data[0]['Activity_Report_APP']['totalproductitem'];
			...
			$Invoice_Flag = $data[1]["Activity_Doctor_Report"]['Invoice_Flag'];
			$TransSlNo = $data[1]["Activity_Doctor_Report"]['TransSlNo'];
			$CashDiscount=$data[1]["Activity_Doctor_Report"]['CashDiscount'];
			$Invoice_Amount=$data[1]["Activity_Doctor_Report"]['NetAmount'];
            if (count($transOrderDetails) > 0 ) {
                ...
                if(count($eRw)<1 || $eKeyID==""){
                    ...
                    performQuery($sql);
					if($Invoice_Flag=="'0'"){
                    $sql = "insert into Trans_Order_Head(Trans_Sl_No,Sf_Code,Cust_Code,Stockist_Code,Route_Target,Order_No,Collected_Amount,Order_Value,Order_Date,Route,DCR_Code,net_weight_value,discount_price,rateMode,Div_ID,OrderID,OrderType,TDPercent,Order_Flag,Sub_Total,No_Of_items) select '" . $pk . "','" . $sfCode . "'," . $custCode . "," . $stockistCode . ",'" . $routeTarget . "'," . $orderNo . ", $collectedAmount , $orderValue," . $orderDate . ", $townCode,'" . $DCRCode . "','$net_weight_value','$discount_price','$rateMode','$Owndiv','".$eKeyID."','".$POT."',".$Discountpercent.",'1',$orderValue,$totalproductitem";
                    performQuery($sql);
					  //$myfile = fopen("../server/Google_Details.txt", "a+");
							//$txt = "Vasan\n";
							//fwrite($myfile, $sql);
							//fclose($myfile);  
					}else if($Invoice_Flag=="'1'"){
					$sql = "update Trans_Order_Head set No_Of_items=$totalproductitem,Order_Flag='2',Order_Value=$orderValue,Dis_Total=$CashDiscount,Tax_Total=$Invoice_Amount where Trans_Sl_No='".$TransSlNo."'";
					performQuery($sql);	 
					/*   $myfile = fopen("../server/Google_Details.txt", "a+");
							$txt = "Vasan\n";
							fwrite($myfile, $sql);
							fclose($myfile); */ 
					$sql = "delete from  Trans_Order_Details where Trans_Sl_No='".$TransSlNo."'"; 
					performQuery($sql);
					$pk=$TransSlNo;
					}else{
						$sql = "insert into Trans_Order_Head(Trans_Sl_No,Sf_Code,Cust_Code,Stockist_Code,Route_Target,Order_No,Collected_Amount,Order_Value,Order_Date,Route,DCR_Code,net_weight_value,discount_price,rateMode,Div_ID,OrderID,OrderType,TDPercent,Order_Flag,Sub_Total,Dis_Total,Tax_Total,No_Of_items) select '" . $pk . "','" . $sfCode . "'," . $custCode . "," . $stockistCode . ",'" . $routeTarget . "'," . $orderNo . ", $collectedAmount , $orderValue," . $orderDate . ", $townCode,'" . $DCRCode . "','$net_weight_value','$discount_price','$rateMode','$Owndiv','".$eKeyID."','".$POT."',".$Discountpercent.",'2',$orderValue,$CashDiscount,$Invoice_Amount,$totalproductitem";
                    performQuery($sql);
					}
                    $totalval = 0;$orderValue=0;
                    for ($i = 0; $i < count($transOrderDetails); $i++) {
                        $Quantity = $transOrderDetails[$i]['Product_Rx_Qty'];
						$oldqty = $transOrderDetails[$i]['Product_RegularQty'];
                        $Product_Total_Qty = $transOrderDetails[$i]['Product_Total_Qty'];
						$value = $transOrderDetails[$i]['Product_Sample_Qty'];
						if($transOrderDetails[$i]['Product_Sale_Unit']==null)
                            $Product_Sale_Unit=0;
                        else
                            $Product_Sale_Unit=$transOrderDetails[$i]['Product_Sale_Unit'];
		...
		$sql = "select product_netwt from Mas_Product_Detail where Product_Detail_Code='" . $transOrderDetails[$i]['product_code'] . "'";
 		...
 		$net_weight=$Nwt[0]['product_netwt'];
			...
			//$productremarks=($transOrderDetails[$i]['rx_remarks']==null? 0:$transOrderDetails[$i]['rx_remarks']);
                        ...
                        if($Quantity>0 || $oldqty>0 || $free>0   || $cbQty>0  ||  $Ccb_qty>0 || $Promo>0  || $productremarks!==$remarkspd ||  $RcpaId>0 ){
				...
				/* $sql = "SELECT product_unit,Product_Sale_Unit FROM Mas_Product_Detail where Product_Detail_Code='" . $transOrderDetails[$i]['product_code'] . "'";
				 $MPD = performQuery($sql);
				$product_unit=$MPD[0]['product_unit'];
				$Product_Sale_Unit=$MPD[0]['Product_Sale_Unit']; */
						   
                           if($Quantity>0 || $Oldqty>0  || $free>0   || $cbQty>0 || $Promo>0   ||  $Ccb_qty>0   ||  $RcpaId>0  ){
                         
                            $sql = "insert into Trans_Order_Details(Trans_Order_No, Trans_Sl_No,Product_Code,Product_Name,Quantity,value,Rate,net_weight,free,discount,discount_price,rateMode,DistDisc,DistPicRate,MfgDt,ClStock,PromoVal,Div_ID,Offer_ProductNm,Offer_ProductCd,CClStock,New_Qty,unit,off_pro_unit,umo_unit,Total_Qty,Field_Code) 
							select '" . $pk1 . "','" . $pk . "','" . $transOrderDetails[$i]['product_code'] . "','" . $transOrderDetails[$i]['product_Name'] . "',$oldqty,$value,$rate,'$net_weight','$free','$discount','$discountprice','$rateMode',$Disc,$DistRate,'" . $transOrderDetails[$i]['Mfg_Date'] . "','". $cbQty  ."',$Promo,'$Owndiv','" . $Fname . "','" . $FreeP_Code . "' ,'". $Ccb_qty  ."' ,$Quantity,'".$Product_Sale_Unit."','0','0',$Product_Total_Qty,'" . $sfCode . "'";
                            performQuery($sql);
                    ...
                    $sql = "update Trans_Order_Head set Order_Value=((select sum(value) from Trans_Order_Details where Trans_Sl_No=Trans_Order_Head.Trans_Sl_No and Div_ID=Trans_Order_Head.Div_ID)-'$discount_price'),Sub_Total=((select sum(value) from Trans_Order_Details where Trans_Sl_No=Trans_Order_Head.Trans_Sl_No and Div_ID=Trans_Order_Head.Div_ID)-'$discount_price') where  Trans_Sl_No='" . $pk . "' and Div_ID='$Owndiv'";
                    $resp["HeadQryy"]=$today;
                    performQuery($sql);
                }
            }
            ...
            $sql = "SELECT Total_Order_Amount as totalOrder,Amt_Collect FROM Order_Collection_Details WITH (NOLOCK) where Sf_Code='$sfCode' and Stockist_Code=$stockistCode and Cust_Code=$custCode";
            $tRw = performQuery($sql);
            if (!empty($tRw)) {
...
performQuery($query);
 ...
 case "fileAttachment":
    ...
    case "imgTAupload":
		...
		//if($sf=="MGR0016"){
			move_uploaded_file($_FILES["imgfile"]["tmp_name"], "../TAphotos/" . $sf . "_" . $filNm);
		//}else{
...
break;
    ...
    case "table/list":
        ...
        switch (strtolower($data['tableName'])) {
			...
			case "get_compititordetails":
                $query = "select Id id ,name [name] from Competitor_details WITH (NOLOCK)";
                $results = performQuery($query);
                break;
            case "product_master":
                $desig = $data['desig'];
 ...
 case "pendingbils":
         ...
         $result['ProductCategoryRates'] = $ProductCategoryRates;
            ...
            case "category_master":
				...
				for ($i = 0;$i < count($results);$i++) {
					$cat_Id=$results[$i]['id'];
					$sql ="select  Product_Detail_Code  id,Product_Detail_Name name,Product_Brd_Code Product_Cat_Code,ROW_NUMBER() OVER (ORDER BY Product_Detail_Code) row_num,Product_Sale_Unit,product_unit,ISNULL(Unit_code,0)Unit_code,ISNULL(Default_UOMQty,0)Default_UOMQty,ISNULL(Default_UOM,0)Default_UOM   from  Mas_Product_Detail  where Product_Brd_Code='".$cat_Id."' order by Product_Detail_Name";
					
                 $results[$i]['Product']= performQuery($sql);
            } 
				...
				case "category_universe":
				$sfCode = $_GET['sfCode'];
                $query = "exec GetProdBrand_App '" . $sfCode . "'";
                $results = performQuery($query);

				break;
				case "getproduct_details":
					$sfCode = $_GET['sfCode'];
					/*$query = "select  Product_Detail_Code id,Product_Detail_Name name,Product_Brd_Code Product_Cat_Code,
								ROW_NUMBER() OVER (ORDER BY Product_Detail_Code) row_num,5 Rate,0 Qty,0 Amount,0 RegularQty,Product_Sale_Unit,product_unit,
								ISNULL(Unit_code,0)Unit_code,ISNULL(Default_UOMQty,0)Default_UOMQty,ISNULL(Default_UOM,0)Default_UOM   
								from  Mas_Product_Detail where Division_Code='3' order by Product_Code_SlNo";*/
					$query = "select  m.Product_Detail_Code id,Product_Detail_Name name,m.Product_Brd_Code Product_Cat_Code,
			...
			case  "getproductregularqty":
			 $Sf_code = $_GET['sfCode'];
			 $OutletCode = $_GET['OutletCode'];
			 $OrderDate = $_GET['OrderDate'];
			 $Outlet_Code = $_GET['Outlet_Code'];
			 $divisionCode = $_GET['divisionCode'];
			 $sql = "exec Get_Outlet_RegularQty  '" . $OutletCode . "','$OrderDate','".$divisionCode."','".$Sf_code."' ";
			 $results = performQuery($sql);
			break;
				
		case "getoutletviewreport":
			$Sf_code = $_GET['sfCode'];
			 $fromdate = $_GET['fromdate'];
			 $todate = $_GET['todate'];
			 $Outlet_Code = $_GET['Outlet_Code'];
			 $Stackist_Code = $_GET['Stackist_Code'];
			 $sql = "exec get_Sec_order '".$Sf_code."','".$Outlet_Code."','".$fromdate."','".$todate."' ";
			$results = performQuery($sql);
					
		break;
		case "gettotalorderbytoday":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];	
			...
			$type=str_replace('"', '', $loginType);
			if($type=="Distributor"){			
			
		     $sql = "exec Get_Sf_Total_Orders '".$distributorid."','".$fromdate."','".$todate."' ";
			//$sql = "exec Get_Primary_Orders '".$distributorid."','".$fromdate."','".$todate."' ";
			$resord = performQuery($sql);
			$sql = "exec Get_Sf_Total_Invoices '".$distributorid."','".$fromdate."','".$todate."' ";
			$resinv = performQuery($sql);
			//$results = array_merge($resord, $resinv);
			$res = array_merge($resord, $resinv);
			}else{
			$sql = "exec Get_Primary_Orders_Mod '".$distId."','".$fromdate."','".$todate."','Distributor' ";
			$resord = performQuery($sql);
			$res = $resord;
			}
			$results = $res;
		break;
		case "getsalesandstockreturn":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];	
			$distributorid = $_GET['distributorId'];
           
			$sql = "exec getSalesReturn_App '".$distributorid."','".$fromdate."','".$todate."' ";
			$resord = performQuery($sql);
			$sql = "exec getStockRotation_App '".$distributorid."','".$fromdate."','".$todate."' ";
			$resinv = performQuery($sql);
			$results = array_merge($resord, $resinv);
		...
		case "getindentdetails":
			...
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];	
			$distributorid = $_GET['distributorERP'];	
			
			$sql = "exec getStkIndentDetails '".$distributorid."','".$fromdate."','".$todate."' ";
			$resord = performQuery($sql);
			$sql = "exec getStkIndentInvoiceDetails '".$distributorid."','".$fromdate."','".$todate."' ";
			$resinv = performQuery($sql);
			//$sql = "exec getPrimaryIndent '".$distributorid."','".$fromdate."','".$todate."' ";
			//$respri = performQuery($sql);
			$results = array_merge($resord, $resinv);//,$respri);
		break;
		case "getprojectionhistory":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];
			
			$sql = "exec getProjectionDataApp '".$Sf_code."','".$fromdate."','".$todate."' ";
			$resord = performQuery($sql);
			$results = $resord;
		break;	
		case "getprojectiondetails":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];
			$orderID = $_GET['orderID'];
			 
			$sql = "exec Get_Projection_Details '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$results = performQuery($sql);
		break;	
		case "gettotalposorderbytoday":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];	
			$distributorid = $_GET['distributorId'];	
			
			$sql = "exec getPOSOrdersApp '".$distributorid."','".$fromdate."','".$todate."' ";
			$resord = performQuery($sql);
			$results = $resord;
		break;	
		case "gettotalposorderdetails":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];
			$orderID = $_GET['orderID'];
			...
			$sql = "exec Get_PosOrder_DetailsFF '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$results = performQuery($sql);
			
			$sql = "exec getPOSTAXDetails '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$taxresult = performQuery($sql);
			
			for($i=0;$i<count($results);$i++){
				$txtotal=$txtotal+$results[$i]["Tax_value"];
				$pid=$results[$i]["Product_Code"];
				$OrderNo=$results[$i]["Trans_POrd_No"];
		...
		case "gettotaloutletorders":
			$Sf_code = $_GET['sfCode'];
			 $fromdate = $_GET['fromdate'];
			 $todate = $_GET['todate'];
			 
			 $sql = "exec Get_Sf_Outlet_Orders '".$Sf_code."','".$fromdate."','".$todate."' ";
			$results = performQuery($sql);
					
		break;
		case "gettotalprimaryorderbytoday":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];	
			$distributorid = $_GET['distributorId'];	
			$loginType=$_GET['loginType'];
			//$sql = "exec Get_Primary_Orders '".$distributorid."','".$fromdate."','".$todate."' ";
			  $sql = "exec Get_Primary_Orders_Mod '".$distributorid."','".$fromdate."','".$todate."','".$loginType."' ";
			$resord = performQuery($sql);
			$results = $resord;
		break;		
		case "gettotalprimaryorderdetails":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];
			$orderID = $_GET['orderID'];
			...
			$sql = "exec Get_PriOrder_DetailsFF '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$results = performQuery($sql);
			
			$sql = "exec getPriOrderINVTaxDetails '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$taxresult = performQuery($sql);
		...
		break;	
		case "gettotalprimaryorderdetailsnew":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];
			$orderID = $_GET['orderID'];
			...
			$loginType=$_GET['loginType'];
			//$sql = "exec Get_PriOrder_DetailsFF '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$sql = "exec Get_PriOrder_DetailsFF_Mod '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."','".$loginType."'";
			$res = performQuery($sql);

			$sql = "exec getPriOrderINVTaxDetails '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$taxresult = performQuery($sql);
			
			$sql="select uom_name,uom_qty from Trans_Primary_Order_Uom where Trans_Sl_No='".$orderID."'";
			$uoms=performQuery($sql);
			for($i=0;$i<count($res);$i++){
				//$txtotal=$txtotal+$res[$i]["Tax_value"];
				$pid=$res[$i]["Product_Code"];
				$OrderNo=$res[$i]["Trans_POrd_No"];
				$new_array = array_values(array_filter($taxresult, function($obj)  use ($pid,$OrderNo){
							if(($obj["Product_Code"] === $pid) && ($obj["Trans_Order_No"] === $OrderNo)) return true;
						}));
				$res[$i]["TAX_details"]=$new_array;
			}
			$results["OrderData"]=$res;
			$results["uom_details"]=$uoms;
		break;	
		case "getpreviousorder":
			$Sf_code = $_GET['sfCode'];
			$sql="select top 1 Trans_Sl_No from Trans_PriOrder_Head WITH (NOLOCK) where Stockist_Code='".$Sf_code."' order by Order_Date desc";
			$lastorder=performQuery($sql);
			if(count($lastorder)>0)
			{
				$orderID = $lastorder[0]['Trans_Sl_No'];
				$txtotal = 0; 
				 
				$sql = "exec Get_PrePriOrder_Details '".$Sf_code."','".$orderID."'";
				$results = performQuery($sql);
				
				$sql = "exec get_PrePriOrderINVTaxDetails '".$Sf_code."','".$orderID."'";
				$taxresult = performQuery($sql);
		...
		break;
		case "getpreviousordernew":
			$Sf_code = $_GET['sfCode'];
			$GrpCode = $_GET['GrpCode'];
			$sql="exec getPreviousOrder '".$Sf_code."','".$GrpCode."'";
			$lastorder=performQuery($sql);
			if(count($lastorder)>0)
			{
				$orderID = $lastorder[0]['Trans_Sl_No'];
		...
		break;
		case "getpreviousinvoice":
			$Sf_code = $_GET['distributorId'];
			$RetailerID = $_GET['RetailerID'];
			$sql="select top 1 Trans_Inv_Slno from Trans_Invoice_Head WITH (NOLOCK) where Dis_Code='".$Sf_code."' and Cus_Code='".$RetailerID."' order by Invoice_Date desc";
			$lastorder=performQuery($sql);
			if(count($lastorder)>0)
			{
				$orderID = $lastorder[0]['Trans_Inv_Slno'];
				$txtotal = 0; 
				 
				$sql = "exec Get_PreSecInvoice_Details '".$Sf_code."','".$orderID."'";
				$results = performQuery($sql);
				
				$sql = "exec Get_PreSecInvoice_TaxDetails '".$Sf_code."','".$orderID."'";
				$taxresult = performQuery($sql);
		...
		break;
		case "gettodaysf_order_values":
			$Sf_code = $_GET['sfCode'];
			 $fromdate = $_GET['fromdate'];
			 $todate = $_GET['todate'];
			 
			 $sql = "exec Get_Sf_OrderValues '".$Sf_code."','".$fromdate."','".$todate."' ";
			$results = performQuery($sql);
					
		break;
		case "gettotalalldaysoutletorders":
			$Sf_code = $_GET['sfCode'];
			 $fromdate = $_GET['fromdate'];
			 $todate = $_GET['todate'];
			 
			 $sql = "exec Get_Sf_TotalOutlet_Orders '".$Sf_code."','".$fromdate."','".$todate."' ";
			$results = performQuery($sql);
					
		break;
		case "gettotalorderdetails":
			$Sf_code = $_GET['sfCode'];
			$fromdate = $_GET['fromdate'];
			$todate = $_GET['todate'];
			$orderID = $_GET['orderID'];
		...
		case "getorderdetailsforgrn":
			...
			$sql = "exec getInvDetailsForGRN '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$results = performQuery($sql);
			
			$sql = "exec getINVTaxDetailsforGrn '".$Sf_code."','".$fromdate."','".$todate."','".$orderID."'";
			$taxresult = performQuery($sql);
		...
		break;
		case "getoutletviewdetails":
		$Sf_code = $_GET['sfCode'];
		$Order_Id = $_GET['Order_Id'];
		$sql = "exec get_Sec_order_dts '".$Order_Id."','".$Sf_code."' ";
		$results = performQuery($sql);
		break;
            ...
            case "doctor_category":
                $query = "select Doc_Cat_Code id,Doc_Cat_Name name from Mas_Doctor_Category WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Doc_Cat_Active_Flag=0";
                $results = performQuery($query);
                break;
            case "doctor_specialty":
                $query = "select Doc_Special_Code id,Doc_Special_Name name,NeedApproval,ISNULL(','+Class_Code+',','')CategoryCode,ISNULL(DivErp,'')DivErp from Mas_Doctor_Speciality WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Doc_Special_Active_Flag=0";
                $results = performQuery($query);
                break;
            case "mas_doc_class":
                $query = "select Doc_ClsCode id,Doc_ClsName name,ISNULL(DivErp,'')DivErp from Mas_Doc_Class WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Doc_Cls_ActiveFlag=0";
                $results = performQuery($query);
                break;
            case "mas_doc_qualification":
                $query = "select Doc_QuaCode id,Doc_QuaName name from Mas_Doc_Qualification WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Doc_Qua_ActiveFlag=0";
                $results = performQuery($query);
                break;
           ...
           case "vwactivity_supercsh_detail":
              ...
              case "vwtourplan":
                ...
                $resultss = performQuery($query);
                 ...
                 $query = "select  * from vwTourPlan where   SF_Code='$sfCode' and Tmonth='".$CMn."' ";
                $results = performQuery($query); 
				
				 /* $query = "select  * from View_ReportingtoFieldforce where   Reporting_To_SF='$sfCode'  ";
                $results = performQuery($query); */
                outputJSON($results);
                die;
                break ;
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
            ...
            case "vwleavetype":
				$query="select isnull(SF_Per_City_Pincode,'S') MarriedFLag,isnull(sf_short_name,'M') MaleorFemale,Approved_By,ISNULL(cf.Paternity,0)Pflag from Mas_Salesforce ms WITH (NOLOCK) left join Mas_CombOff cf WITH (NOLOCK) on cf.Sf_Code=ms.Sf_Code where ms.Sf_Code='".$sfCode."'";
				$result = performQuery($query); 
				$MarriedFLag=$result[0]['MarriedFLag'];
				$MaleorFemale=$result[0]['MaleorFemale']; 
				$PaternityEligible=$result[0]['Pflag']; 
				if ($result[0]['Approved_By']=="I"){
					$query = "select Leave_Code id,Leave_Name name,Leave_SName,cast(COffType as varchar) COffType,MaxDays from vwLeaveType WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Leave_SName in ('LOP','PH')";
				}
				else{
                $query = "select id,name,Leave_SName,cast(COffType as varchar) COffType,MaxDays from ((select Leave_Code id,Leave_Name name,Leave_SName,COffType,MaxDays from vwLeaveType where Division_code='" . $Owndiv . "')
				union (select Leave_Code id,Leave_Name name,Leave_SName,COffType,isnull(MaxDays,1) MaxDays from mas_Leave_Type l WITH (NOLOCK) inner join Mas_CombOff c WITH (NOLOCK) on c.Comb_off=COffType and COffType=1 where Active_Flag=0 and SF_Code='$sfCode' and Division_code='" . $Owndiv . "')) as tb
                union(select Leave_Code id,Leave_Name name,Leave_SName,COffType,isnull(MaxDays,1) MaxDays from mas_Leave_Type WITH (NOLOCK) where Leave_code='38' and 'S'='$MarriedFLag')
				union(select Leave_Code id,Leave_Name name,Leave_SName,COffType,isnull(MaxDays,1) MaxDays from mas_Leave_Type WITH (NOLOCK) where Leave_code='37' and 'M'='$MarriedFLag' and 1=$PaternityEligible)";
				}
				$results = performQuery($query);
                //$query = "select Leave_Code id,Leave_Name name,Leave_SName from vwLeaveType where Division_code='" . $Owndiv . "'";
                //$results = performQuery($query);
                break;
            ...
            default:
				...
				$distcode=(isset($data['distributorId']) && $data['distributorId'] == 0) ? null : $_GET['distributorId'];
				...
				if(isset($data['today'])){
					if($data['today'] != 0) $today = $data['today'];
				}
                $or = null;
				if(isset($data['or'])){
					if($data['or'] != 0) $or = $data['or'];
				}
                $wt = null;
				if(isset($data['wt'])){
					if($data['wt'] == 0) $wt = $data['wt'];
				}
                $tableName = $data['tableName'];
				
                $coloumns = json_decode($data['coloumns']);
                $desig = $data['desig'];
				if ($desig == 'stockist' && $distcode!=0){
					$sfCode=$distcode;
				}
                ...
                if (!is_null($or)) {
					...
					if($tableName=="vwDoctor_Master_APP"){
						$query="exec getDistwiseOutlets_DMS '$sfCode'";
						$outlets=performQuery($query);
						/*$query="select o.OutletCode,o.OutletCat_Type,o.Category_Code,o.Category_Name,o.Sub_Category_Code,o.Sub_Category_Name from Outlet_Category_Mapping o inner join Mas_ListedDr dr on dr.ListedDrCode=o.OutletCode where DistCode='$sfCode' and dr.ListedDr_Active_Flag='0'";
						$catresult=performQuery($query);
						for($i=0;$i<count($outlets);$i++){
							$outletID=$outlets[$i]["id"];
							$new_array = array_values(array_filter($catresult, function($obj)  use ($outletID){
											if(($obj["OutletCode"] === $outletID)) return true;
										}));
							$outlets[$i]["outlet_type_Details"]=$new_array;
						}*/
						$results=$outlets;
...
echo json_encode($respon);
 ...
 $query = "update Mas_SF_SlNo set POrdSl=(isnull(POrdSl,0)+1) where SF_Code='" . $sfCode . "'";
    ...
    case "vwLeaveStatus":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwLeaveEntitle WITH (NOLOCK) where Sf_Code='$sfCode' and Trans_Year=year(getdate())";
    ...
    case "vwLeave":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwLeave WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		
		case "Dashboarddistcount":
        $sfCode = $_GET['sfCode'];
        $sql = "select top 1 'Distributor' Name,'10' CountAll  from mas_stockist";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		case "DashOutletC":
        $sfCode = $_GET['sfCode'];
        $sql = "select top 1 'DashOutletC' Name,'11' CountAll  from mas_stockist";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		case "DashProductC":
        $sfCode = $_GET['sfCode'];
        $sql = "select top 1 'Dairy Milk' Name,'12' CountAll  from mas_stockist";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		case "DashDaySummary":
        $sfCode = $_GET['sfCode'];
        $sql = "select top 1 'Day Summary' Name,'11' CountAll  from mas_stockist";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		...
		case "vwBlockRelease":
        $sfCode = $_GET['sfCode'];
        $sql = "select top 1* from vwBlockRelease WITH (NOLOCK)";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		case "vwReleasedfromMGR":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwAprovedBlockRelease WITH (NOLOCK) ";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		...
		case "vwDeviation":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwDeviation WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		
		case "GetModeoftravel":
        $sql = "select Sl_No id,MOT name,StEndNeed,DriverNeed from Mas_Modeof_Travel WITH (NOLOCK) where Active_Flag='0'";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		...
		case "vwNewJoin":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwNewJoin WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		...
		case "vwCancelLeave":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwCancelLeave WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $leave = performQuery($sql);
        outputJSON($leave);
        break;
		...
		case "ViewPermission":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from ViewPermission WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $Permission = performQuery($sql);
        outputJSON($Permission);
        break;
		case "vwOnduty":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwOn_Duty_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $vwOnduty = performQuery($sql);
        outputJSON($vwOnduty);
        break;
		case "vwHolidayEntry":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwWeekoffEntry_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $vwOnduty = performQuery($sql);
        outputJSON($vwOnduty);
        break;
		case "vwmissedpunch":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwOn_Missed_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $vwmissedpunch = performQuery($sql);
        outputJSON($vwmissedpunch);
        break;
	case "vwExtended":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwExtended_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode' ";
		...
		case "vwtourplan":
               ...
               /*  $query = "exec TourPlan_Holyday_Auto_Insert '$sfCode' ,'$CMn','$CYr',' $stateCode'";
                ...
                $query = "select  * from viewtourplan where   Reporting_To_SF='$sfCode' ";
				
                $results = performQuery($query);
                outputJSON($results);
                die;
                break;
		case "getreportingtosfname":
                $sfCode = $_GET['sfCode'];

               /*  $query = "exec TourPlan_Holyday_Auto_Insert '$sfCode' ,'$CMn','$CYr',' $stateCode'";
                $resultss = performQuery($query); */
 
               $query = "select  DateName(month , DateAdd( month , Tmonth , -1 )) Monthnameexample,cast (Tmonth-1 as varchar) Tmonth ,Sf_Code,cast(Tyear as varchar) Tyear ,FieldForceName from (select  * from viewtourplan where   Reporting_To_SF='$sfCode')A  group by Tmonth,FieldForceName,Tyear,Sf_Code  ";
                $results = performQuery($query);
                outputJSON($results);
                die;
                break;
	    ...
	    case "ViewAllCount":
		$sfCode = $_GET['sfCode'];
	    $sql = "select count(Reporting_To_SF) LeaveC from  vwLeave WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $leave = performQuery($sql);
		$sql = "select count(Reporting_To_SF) PermiC  from ViewPermission WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $Permission = performQuery($sql);
		$sql = "select count(Reporting_To_SF) OndutyC from vwOn_Duty_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $vwOnduty = performQuery($sql);
		$sql = "select count(Reporting_To_SF) MspC from vwOn_Missed_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $vwmissedpunch = performQuery($sql);
		$sql = "select count(Reporting_To_SF) ExtC from vwExtended_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $ExtendedCount = performQuery($sql);
		$sql = "select count(Reporting_To_SF) Tour_Count from ViewTourPlan WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $TountPlanCount = performQuery($sql);
		$sql = "select count(Reporting_To_SF) HolidayC from vwWeekoffEntry_Approval WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $HolidayCount = performQuery($sql);
		$sql = "select count(Reporting_To_SF) DeviationC from vwDeviation WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $Deviation_C = performQuery($sql);
		$sql = "select count(Reporting_To_SF) CancelLeave from vwCancelLeave WITH (NOLOCK) where Reporting_To_SF='$sfCode'";
        $CancelLeave = performQuery($sql);
		$sql = "select count(*)ExpList from vwexpenseapprovelist WITH (NOLOCK) where Reporting_To_SF='".$sfCode."'";
		$ExpList = performQuery($sql);
		$ViewAllCount=array();
		...
		$ViewAllCount['TountPlanCount']=$TountPlanCount[0]['Tour_Count'];
		
		$ViewAllCount['FlightAppr'] = 0;
		if($sfCode == "MGR0017" || $sfCode == "MGR5197"){			
			$sql = "select count(*) Cnt from Trans_Flight_booking_head where datediff(day,cast(BookReqDt as date),BookDt)<=5 and ApprvFlg=0";
			$FlightAppr = performQuery($sql);
			$ViewAllCount['FlightAppr'] = $FlightAppr[0]['Cnt'];
		}
		...
		$ViewAllCount['ExpList']=$ExpList[0]['ExpList'];
        ...
        $AMod = $_GET['AMod'];
		if($AMod==1){
        	$sql = "select p.Sf_Code,Sf_Name SFNm,convert(varchar(10), cast(Permissiondate as datetime), 105) Permissiondate,
			...
			Rejected_Reason from PermissionEntry p WITH (NOLOCK) where p.Approvedby='$rSF' order by cast(Permissiondate as datetime) desc";
		}else{
			$sql = "select Sf_Code,convert(varchar(10), cast(Permissiondate as datetime), 105) Permissiondate,
			...
			case when Approval_Flag='2' then 'Pending' when Approval_Flag='8' then 'Late Approval' when Approval_Flag='0' then 'Approved' else 'Reject' end PStatus,
			case when Approval_Flag='2' then 'rgb(255,165,0) !important'  when Approval_Flag='0' or Approval_Flag='8' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
			Rejected_Reason from PermissionEntry WITH (NOLOCK) where Sf_Code='$sfCode' order by cast(Permissiondate as datetime) desc";
		}
		...
		Reject_reason from Extended_entry EE WITH (NOLOCK)
		inner join Mas_Salesforce s WITH (NOLOCK) on s.Sf_Code=EE.SF_Code where EE.Approved_by='$rSF' order By Start_Time desc";
		}else{
        $sql = "select (select top 1 (select Sft_Name from Mas_Shift_Timings WITH (NOLOCK) where Sft_ID=ShiftSelected_Id) 
		from TP_Attendance_App where sf_code=EE.Sf_Code and convert(date,Start_Time)= convert(date,EE.Start_Time)) Shiftnames,
		...
		case when WrkType='0' then 'Pending'  when WrkType='2' then 'Approved' else 'Reject' end EStatus,
		case when WrkType='0' then 'rgb(255,165,0) !important'  when WrkType='2' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
		Reject_reason from Extended_entry EE WITH (NOLOCK) where Sf_Code='$sfCode' order By Start_Time desc";
        }
		$PermissionST = performQuery($sql);
        outputJSON($PermissionST);
        break;
        ...
        $sql = "select Sf_Code,Sf_Name SFNm,Shift_Name,convert(varchar(10), cast(Approved_Date as datetime), 103) Rejectdate, convert(varchar(10), Missed_punch_date, 103) Missed_punch_date, Missed_punch_Flag,convert(varchar(10), cast(Entry_Date as datetime), 103) Submission_date ,Reason,Checkin_Time,
		...
		from Missed_punch_Entry WITH (NOLOCK) where Approved_by='$rSF' order by Missed_punch_date desc";
        }else{
        $sql = "select Sf_Code,Shift_Name,convert(varchar(10), cast(Approved_Date as datetime), 103) Rejectdate,convert(varchar(10),Missed_punch_date,103) Missed_punch_date,Missed_punch_Flag,convert(varchar(10), cast(Entry_Date as datetime), 103) Submission_date ,Reason,Checkin_Time,
		left(CONVERT(varchar,cast(replace(Checkout_Tme,'T',' ') as datetime),8),5) Checkout_Tme,
		case when Missed_punch_Flag='0' then 'Pending'  when Missed_punch_Flag='2' then 'Approved'  when Missed_punch_Flag='8' then 'Late Approval'  else 'Reject' end MPStatus,
		case when Missed_punch_Flag='0' then 'rgb(255,165,0) !important'  when Missed_punch_Flag='2' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,RejectReason 
		from Missed_punch_Entry WITH (NOLOCK) where sf_Code='$sfCode' order by Missed_punch_date desc";
        }
		...
		$sfCode = $_GET['sfCode'];
		$sql="select convert(varchar,Sf_TP_DCR_Active_Dt,23) SDt from Mas_Salesforce WITH (NOLOCK) where SF_code='$sfCode' and Sf_TP_DCR_Active_Dt>'2020-07-22 00:00:00'";
		$stDts = performQuery($sql);
		$stDt="2020-07-22 00:00:00";
		if(count($stDts)>0){
		$stDt=$stDts[0]["SDt"];
		}
		$sql="select convert(varchar, login_date, 23) name,(select cast(count(Sf_code) as varchar) from Expense_Start_Activity  ex WITH (NOLOCK) inner join Mas_Modeof_Travel mt WITH (NOLOCK) on mt.Sl_No=ex.MOT where Sf_code='$sfCode' and ex.End_Km is null  and mt.StEndNeed='1' and convert(date,Date_Time)=convert(date,login_date)) Aflag,
		 ...
		 Min(Start_Time) Start_Time,Max(End_Time) End_Time,max(SFTE_Time) SFTE_Time,MAX(Devi_Active_Flag)Devi_Active_Flag from TP_Attendance_App T WITH (NOLOCK)
		 left outer join Mas_Shift_Timings WITH (NOLOCK) on ShiftSelected_Id=Sft_ID
		 left outer join Missed_punch_Entry m WITH (NOLOCK) on m.Sf_Code=T.Sf_Code and m.Missed_punch_date=login_date
		 and Missed_punch_Flag!=3 
		 left outer join mas_Leave_Form lf WITH (NOLOCK) on lf.sf_code=t.Sf_Code 
		...
		$sql="select convert(varchar, login_date, 23) name,
			...
			End_Time,convert(varchar, Start_Time, 24) Checkin_Time,replace(convert(varchar, SFTE_Time, 120),' ','T') COutTime
		 from (
		 select login_date,Sft_Name,max(Missed_punch_Flag) Missed_punch_Flag,max(WrkType) WrkTyp,
		 Min(Start_Time) Start_Time,Max(End_Time) End_Time,max(SFTE_Time) SFTE_Time from TP_Attendance_App T
		 ...
		 and Missed_punch_Flag!=3 where T.sf_code='$sfCode' 
		 and login_date>'$stDt' 
		 and getdate()>dateadd(hour,iif(ACtOffTyp=1,ACtOffHrs,0),SFTE_Time) 
		 and login_date<cast(convert(varchar,getdate(),101) as dateTime) 
		 group by login_date,Sft_Name) as t 
		 where End_Time is null and Missed_punch_Flag is null";//and convert(varchar, Start_Time, 23)=convert(varchar, Missed_punch_date, 23)
	   ...
	   $getWF = performQuery($sql);
		...
		$wkFl=0;
		if(count($getWF)>0){
		$sql="select iif(datediff(day,max(cast(Week_date as datetime)),'".$getWF[0]["WKDate"]."')>6,1,0) flg from Trans_Weekoff WITH (NOLOCK) where sf_Code='$sfCode'";
		...
		*/
		...
		$sql="select convert(varchar,Sf_TP_DCR_Active_Dt,23) SDt from Mas_Salesforce WITH (NOLOCK) where SF_code='$sfCode' and Sf_TP_DCR_Active_Dt>'".$stDt."'";
		$stDts = performQuery($sql);
		if(count($stDts)>0){
			$stDt=$stDts[0]["SDt"];
		}
		...
		$sql="select convert(varchar, login_date, 23) name,
			...
			isnull((select max(1) from Expense_Start_Activity ex WITH (NOLOCK) inner join Mas_Modeof_Travel mt WITH (NOLOCK) on mt.Sl_No=ex.MOT where sf_Code='$sfCode' and cast(convert(varchar,Date_Time,101) as datetime)=t.login_date and Enddateand_time is NULL and mt.StEndNeed='1'),0) Aflag 
		 from (
		 select login_date,Sft_Name,max(Missed_punch_Flag) Missed_punch_Flag,max(WrkType) WrkTyp,max(Leave_Active_Flag)Leave_Active_Flag,
		 Min(Start_Time) Start_Time,Max(End_Time) End_Time,max(SFTE_Time) SFTE_Time,MAX(Devi_Active_Flag)Devi_Active_Flag  from TP_Attendance_App T WITH (NOLOCK)
		 left outer join Mas_Shift_Timings WITH (NOLOCK) on ShiftSelected_Id=Sft_ID
		...
		$wkFl=0;
		if(count($getWF)>0){
		$sql="select iif(datediff(day,isnull(max(cast(Week_date as datetime)),dateadd(day,-8,'".$getWF[0]["WKDate"]."')),'".$getWF[0]["WKDate"]."')>6,1,1) flg from Trans_Weekoff WITH (NOLOCK) where sf_Code='$sfCode'";
	    $getWFD = performQuery($sql);
		$wkFl=$getWFD[0]["flg"];
		}
		
		if($sfCode=='MGR6396' || $sfCode=='MGR6395' || $sfCode=='MGR5257' || $sfCode=='MGR6398' || $sfCode=='MGR6465' || $sfCode=='MGR6464'|| $sfCode=='MGR6463' || $sfCode=='MGR2920'|| $sfCode=='MGR5246' ){
         $GetMissed['CheckWK']=[];
		}else{
        ...
        $AMod = $_GET['AMod'];
		if($AMod==1){
        	$sql = "select TA.Sf_Code,Sf_Name SFNm,WrkType,convert(varchar(20),cast(login_date as datetime), 105) login_date,iif(ODLocCd=0,'Other Location','HAP Location') ondutytype,
		isnull(convert(varchar(20), cast(Approved_Date as datetime), 105),'-') Approveddate,
		...
		isnull(( select Sft_Name from Mas_Shift_Timings  where Sft_ID=TA.ShiftSelected_Id),'-') as Shift_name,
				...
				from TP_Attendance_App  TA WITH (NOLOCK)
inner join Mas_Salesforce s WITH (NOLOCK) on s.Sf_Code=TA.SF_Code where  WrkType>0 and TA.Approved_by='$rSF' order by login_date desc";
		}else{
        $sql = "select Sf_Code,WrkType,convert(varchar(20),cast(login_date as datetime), 105) login_date,iif(ODLocCd=0,'Other Location','HAP Location') ondutytype,
		isnull(convert(varchar(20), cast(Approved_Date as datetime), 105),'-') Approveddate,
		ODLocName,Rmks Reason,convert(varchar(8),cast(Start_Time as time)) Start_Time, 
		convert(varchar(8), cast(End_Time  as time)) End_Time,(isnull(Start_Lat,'')+','+isnull(Start_Long,''))checkin,
		(isnull(End_Lat,'')+','+isnull(End_Long,''))checkout,
		...
		isnull(( select Sft_Name from Mas_Shift_Timings  where Sft_ID=TA.ShiftSelected_Id),'-') as Shift_name,
				...
				when WrkType='2' then 'Approved' when WrkType='8' then 'Late Approval' else 'Reject' end OStatus,
				case when WrkType='1' then 'rgb(255,165,0) !important'  when WrkType='2' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,Reject_reason
				from TP_Attendance_App  TA WITH (NOLOCK) where  WrkType>0 and Sf_Code='$sfCode' order by login_date desc";
        }
		$ondutystatus = performQuery($sql);
        outputJSON($ondutystatus);
        break;
		
		case "GetAdvance_Status":
        $sfCode = $_GET['sfCode'];
        $sfCode = $_GET['sfCode'];
        $rSF = $_GET['rSF'];
        $AMod = $_GET['AMod'];
		/*if($AMod==1){
        $sql = "select l.sf_code, cast (l.Leave_Id as varchar) Leave_Id ,SF_Name SFNm,case when convert(date,getdate())<= convert(date,From_Date) then '1' else '0' end showflag,convert(varchar(10), l.Created_Date, 105)  Created_Date,'0' cancelreason,Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), l.LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,No_of_Days,case when
		 ...
		 Rejected_Reason,Leave_Active_Flag from mas_Leave_Form l WITH (NOLOCK) inner join Mas_Salesforce s WITH (NOLOCK) on s.SF_Code=l.SF_Code
		 where Approved_to='$rSF'  order by cast(From_Date as datetime) desc";
        }else{*/
        $sql = "select SF,eKey,convert(varchar(10), eDate, 105) eDate,convert(varchar(10), AdvFrDt, 105) From_Date,convert(varchar(10), AdvToDt, 105) To_Date,
		 CHOOSE(AdvTyp,'Travel Advance','Expense Advance','Other Advance') AdvTyp,AdvLoc,AdvPurp,AdvAmt,convert(varchar(10), AdvSettle, 105) AdvSettle,
		 convert(varchar(10), ApprDt, 105) ApprDt,ApprBy,ApprByNm,flag,case flag when 0 then 'Pending' when 1 then 'Approved' when 2 then 'Reject' End LStatus,
		 case flag when 0 then '#ff920b !important'  when 1 then '#089c08 !important'  when 2 then '#ff0000 !important' end StusClr from Trans_AdvanceRequest WITH (NOLOCK) where SF='$sfCode' order by AdvFrDt desc";
        //}
        outputJSON(performQuery($sql));
        break;
		case "GetAdvance_Pending":
        $sfCode = $_GET['sfCode'];
        $rSF = $_GET['rSF'];
        $AMod = $_GET['AMod'];
		
        $sql = "select SF,eKey,SF_Name,SF_Emp_ID,HQ_Name,Designation_Name,SF_Mobile,convert(varchar(10), eDate, 105) eDate,convert(varchar(10), AdvFrDt, 105) From_Date,convert(varchar(10), AdvToDt, 105) To_Date,
		 CHOOSE(AdvTyp,'Travel Advance','Expense Advance','Other Advance') AdvTyp,AdvLoc,AdvPurp,AdvAmt,convert(varchar(10), AdvSettle, 105) AdvSettle,
		 convert(varchar(10), ApprDt, 105) ApprDt,ApprBy,ApprByNm,flag,case flag when 0 then 'Pending' when 1 then 'Approved' when 2 then 'Reject' End LStatus,
		 case flag when 0 then '#ff920b !important'  when 1 then '#089c08 !important'  when 2 then '#ff0000 !important' end StusClr from Trans_AdvanceRequest WITH (NOLOCK) inner join Mas_Salesforce s WITH (NOLOCK) on SF=SF_Code inner join Mas_HQuarters h WITH (NOLOCK) on h.HQ_ID=Sf_HQ inner join Mas_SF_Designation d WITH (NOLOCK) on d.Designation_Code=s.Designation_Code  where Reporting_To_SF='$rSF' and flag=0 order by AdvFrDt desc";
        ...
        $sql = "select sf_code,cast (Leave_Id as varchar) Leave_Id,case when convert(date,getdate())<= convert(date,From_Date) then '1' else '0' end showflag,convert(varchar(10), Created_Date, 105)  Created_Date,'0' cancelreason,  Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,No_of_Days,case when
		 Leave_Active_Flag='2'  or Leave_Active_Flag='4' then 'Pending'  when Leave_Active_Flag='0' then 'Approved' when Leave_Active_Flag='3'  then 'CancelPedning' when Leave_Active_Flag='8' then 'Late Approval'  else 'Reject' end LStatus ,
		 case when Leave_Active_Flag='2'  or Leave_Active_Flag='4' then 'rgb(255,165,0) !important'  when Leave_Active_Flag='3' then 'rgb(255,165,0) !important'  when Leave_Active_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
		 Rejected_Reason,Leave_Active_Flag from mas_Leave_Form WITH (NOLOCK) where Sf_Code='$sfCode'   order by cast(From_Date as datetime) desc";
        }
		$PermissionST = performQuery($sql);
        outputJSON($PermissionST);
        break;
        ...
        $AMod = $_GET['AMod'];
		if($AMod==1){
        $sql = "select l.sf_code,l.Leave_Id,SF_Name SFNm,case when convert(date,getdate())< convert(date,From_Date) then 1 else 0 end showflag,convert(varchar(10), l.Created_Date, 105)  Created_Date,'0' cancelreason,Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), l.LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,cast (No_of_Days as varchar)No_of_Days ,case when
		 ...
		 Rejected_Reason,Leavecancel_Id from Leave_Cancel_Form l WITH (NOLOCK) inner join Mas_Salesforce s WITH (NOLOCK) on s.SF_Code=l.SF_Code
		 where Approved_to='$rSF' and Leavecancel_Id<>'2'  order by cast(From_Date as datetime) desc ";
        }else{
        $sql = "select sf_code,Leave_Id,case when convert(date,getdate())< convert(date,From_Date) then 1 else 0 end showflag,convert(varchar(10), Created_Date, 105)  Created_Date,'0' cancelreason,  Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,cast (No_of_Days as varchar)No_of_Days,case when
		 Leavecancel_Id='2' then 'Pending'  when Leavecancel_Id='0' then 'Approved'   else 'Reject' end LStatus ,
		 case when Leavecancel_Id='2' then 'rgb(255,165,0) !important'  when Leavecancel_Id='3' then 'rgb(255,165,0) !important'  when Leavecancel_Id='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
		 Rejected_Reason,Leavecancel_Id from Leave_Cancel_Form WITH (NOLOCK) where Sf_Code='$sfCode'   order by cast(From_Date as datetime) desc";
        }
		$PermissionST = performQuery($sql);
        outputJSON($PermissionST);
        break;
        ...
        $AMod = $_GET['AMod'];
		if($AMod==1){
        $sql = "select l.sf_code,l.Deviation_Id,l.SF_Name SFNm,convert(varchar(10), cast(l.Server_Date as datetime), 105)  Created_Date,Reason,(select DeviationName from Mas_Devation where Deviation_code=Deviation_Type)Deviation_Type,convert(varchar(10), cast(Deviation_Date as datetime), 105)  Deviation_Date,convert(varchar(10), l.MobileDate, 105)  MobileDate,convert(varchar(10), cast(l.LastUpdt_Date as datetime), 105)  LastUpdt_Date,case when
		 ...
		 Rejected_Reason,Devi_Active_Flag from Mas_Deviation_Form l WITH (NOLOCK) inner join Mas_Salesforce s WITH (NOLOCK) on s.SF_Code=l.SF_Code
		 where Approved_to='$rSF' and Devi_Active_Flag<>3 order by cast(Deviation_Date as datetime) desc";
        }else{
        $sql = "select sf_code,Deviation_Id,convert(varchar(10), cast(Server_Date as datetime), 105)  Created_Date,Reason,(select DeviationName from Mas_Devation where Deviation_code=Deviation_Type)Deviation_Type,convert(varchar(10), cast(Deviation_Date as datetime), 105)  Deviation_Date,convert(varchar(10), cast(LastUpdt_Date as datetime), 105)  LastUpdt_Date,case when
		 Devi_Active_Flag='2' then 'Pending'  when Devi_Active_Flag='0' then 'Approved' else 'Reject' end DStatus ,
		 case when Devi_Active_Flag='2' then 'rgb(255,165,0) !important'  when Devi_Active_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
		 Rejected_Reason,Devi_Active_Flag from Mas_Deviation_Form WITH (NOLOCK) where Sf_Code='$sfCode'  and Devi_Active_Flag<>3 order by cast(Deviation_Date as datetime) desc";
        }
		$PermissionST = performQuery($sql);
        outputJSON($PermissionST);
        break;
	case "GetHelpList":
        $sfCode = $_GET['sfCode'];
        $sql = "select id,HlpCat_Name+ ' [ Click here ]' text from Mas_Help_Category WITH (NOLOCK) where Division_Code=3 and Active_Flag=0";
		$leave = performQuery($sql);
        outputJSON($leave);
        break;
	case "get/helpdet":
        $sfCode = $_GET['sfCode'];
        $id = $_GET['id'];
        $sql = "select Hlp_Name name,Hlp_Detail text from Mas_Help_Details WITH (NOLOCK) where Cat_Id='$id' and Division_Code=3 and Active_Flag=0 order by cast(Hlp_ID as numeric)";
		$leave = performQuery($sql);
        outputJSON($leave);
        break;
    case "Productdisplayapprovals":
    ...
    case "vwChkTransApprovalOne":
        ...
        $month = $_GET['month'];
        $year = $_GET['year'];
        $sql = "select Objective,Worked_With_SF_Name,Worktype_Name_B,Territory_Code1,convert(varchar,Tour_Date,103) Tour_Date,Button_Access from Trans_TP_One where Sf_Code='$sfCode' order by Tour_Date";
        $tp = performQuery($sql);
        outputJSON($tp);
        break;
...
case "get/vwVstPendingPragnancy":
    ...
    case "save/track":
		...
		$mock = $data[0]['mock'];
		$batt = $data[0]['batt'];
		$strV= $hAcc . "|" . $bear . "|" . $et . "|" . $alt . "|" . $vel . "|" . $vAcc . "|" . $sAcc . "|" . $bAcc;		
		/*$sql = "insert into tbTrackLoction(SF_code,DtTm,Lat,Lon,Addr,Auc,deg,Employee_Id,DvcID,Battery,Mock) select '$sfCode','" . $sTime . "','$lat','$lng','".$strV."','" . str_replace("'", "", $hAcc) . "','" . str_replace("'", "", $data[0]['bear']) . "','".$Speed."','" . str_replace("'", "", $data[0]['DvcID']) . "','".$batt."','".$mock."'";
    ...
    case "save/trackall":
        $TLocs = json_decode($_POST['data'], true);
        //$sfCode=$TrcLocs['SF_code'];
        ...
        for($ik=0;$ik<count($TLocs);$ik++){
			...
			$Speed = $TLocs[$ik]['Speed'];
			$hAcc = $TLocs[$ik]['hAcc'];
			$bear = $TLocs[$ik]['bear'];
			$et = $TLocs[$ik]['et'];
			$alt = $TLocs[$ik]['alt'];
			$vel ="";
			if(isset($TLocs[$ik]['vel']))
				$vel = $TLocs[$ik]['vel'];
			$vAcc = $TLocs[$ik]['vAcc'];
			$sAcc = $TLocs[$ik]['sAcc'];
			$bAcc = $TLocs[$ik]['bAcc'];
			...
			$batt = $TLocs[$ik]['batt'];
			$DvID="";
			if(isset($TLocs[$ik]['DvcID']))
				$DvID= str_replace("'", "", $TLocs[$ik]['DvcID']);
			$strV= $hAcc . "|" . $bear . "|" . $et . "|" . $alt . "|" . $vel . "|" . $vAcc . "|" . $sAcc . "|" . $bAcc;		
			$sql = "insert into tbTrackLoction(SF_code,DtTm,Lat,Lon,Addr,Auc,deg,Employee_Id,DvcID,Battery,Mock) select '$sfCode','" . $sTime . "','$lat','$lng','".$strV."','" . str_replace("'", "", $hAcc) . "','" . str_replace("'", "", $TLocs[$ik]['bear']) . "','".$Speed."','" .$DvID . "','".$batt."','".$mock."'";
            performQuery($sql);
        }
		$result=[];
		$result["Success"]=true;
...
outputJSON($result);
        break;
         case "upd/retailer":
        $data = json_decode($_POST['data'], true);
		$updsfcode=$_GET['sfCode'];
        $result = array();
        $query = "insert into updOutletHistory(log_Date,ListedDrCode,
ListedDr_Name,Allowance_Type,pin_code,gst,Owner_Name, ListedDr_Address1,Doc_ClsCode,doc_special_code, ListedDr_Phone,cityname, ListedDr_Email,
Compititor_Id,Compititor_Name,Category_Universe_Id,Hatsun_AvailablityId,reason_category,LastUpdt_Date,Hatsanavail_Switch,HatsanCategory_Switch,ListedDr_Class_Patients,ListedDr_Consultation_Fee  ,Outlet_Type,reject_remarks,ListedDr_Pin,Listeddr_App_Mgr,Dist_Name
) select getdate(),ListedDrCode,
ListedDr_Name,Allowance_Type,pin_code,gst,Owner_Name, ListedDr_Address1,Doc_ClsCode,doc_special_code, ListedDr_Phone,cityname, ListedDr_Email,
Compititor_Id,Compititor_Name,Category_Universe_Id,Hatsun_AvailablityId,reason_category,LastUpdt_Date,Hatsanavail_Switch,HatsanCategory_Switch,ListedDr_Class_Patients,ListedDr_Consultation_Fee
,Outlet_Type,reject_remarks,ListedDr_Pin,Listeddr_App_Mgr,Dist_name
from Mas_ListedDr where  ListedDrCode=" . $data["id"] . "";
        $temp = performQuery($query);
        $query = "update Mas_ListedDr set ListedDr_Name='".str_replace("'","",$data["unlisted_doctor_name"])."'";
		if(str_replace("'","",$data["img_name"])!="")
		{
			$query =$query.",ListedDr_Pin='".str_replace("'","",$data["img_name"])."'";
		}//". $data["unlisted_doctor_mobiledate"]."
		if($data["State_Code"]!="")
		{
			$query =$query.",State_Code='".str_replace("'","",$data["State_Code"])."'";
		}
		if($data["customer_code"]!="")
		{
			$query =$query.",CustomerCode='".str_replace("'","",$data["customer_code"])."'";
		}
		if(str_replace("'","",$data["sub_categoryId"])!="")
		{
			$query =$query.",Doc_Class_ShortName='".str_replace("'","",$data["sub_category"])."',Doc_ClsCode='".str_replace("'","",$data["sub_categoryId"])."'";
		}
		$query =$query.",Doc_Spec_ShortName='".$data["unlisted_specialty_name"]."',Outstanding='".str_replace("'","",$data["outstanding_amount"])."',Outlet_Type='".$data["OutletTypeCd"]."',Allowance_Type='".$data["VechType"]."' ,pin_code=".$data["unlisted_doctor_pincode"].",gst=".$data["unlisted_doctor_gst"].",Owner_Name=".$data["unlisted_Owner_name"].", ListedDr_Address1='".str_replace("'","",$data["unlisted_doctor_address"])."',doc_special_code=" . $data["unlisted_specialty_code"].", ListedDr_Mobile=" . $data["unlisted_doctor_phone"] . " ,cityname=" . $data["unlisted_doctor_cityname"] . ", ListedDr_Email=" . $data["unlisted_doctor_Email"] . ",Compititor_Id=" . $data["Compititor_Id"] . ",Compititor_Name=" . $data["Compititor_Name"] . ",Category_Universe_Id=" . $data["CategoryType"] . ",Hatsun_AvailablityId=" . $data["AvailUniverSelectId"].",reason_category=" . $data["reason_category"] . ",LastUpdt_Date=getdate(),Hatsanavail_Switch=". $data["HatsunAvailswitch"].",HatsanCategory_Switch=". $data["categoryuniverseswitch"].",ListedDr_Class_Patients=".$data["lat"].",Listeddr_App_Mgr='".$updsfcode."',ListedDr_Consultation_Fee=".$data["long"].", ListedDr_Phone='" . str_replace("'","",$data["unlisted_doctor_secondphone"]) . "',reject_remarks='" . str_replace("'","",$data["OutletTypeRmks"]) . "' where  ListedDrCode=" . $data["id"] . "";
        $temp = performQuery($query);
        $result["success"] = true;
		 $result["Query"] = $query;
		 //$result["sub_categoryId"] = str_replace("'","",$data["sub_categoryId"]);
		 /*outletsync($data["id"]);
		 $query="exec KTTSyncOutlet " . $data["id"] . "";
		 $rss = performQuery($query);*/
        outputJSON($result);
        break;  
        
        
       case "upd/newretailer":
        $data = json_decode($_POST['data'], true);
		$updsfcode=$_GET['sfCode'];
        $result = array();
		$outletCategory=$data["outlet_type_Details"];
        $query = "insert into updOutletHistory(log_Date,ListedDrCode,ListedDr_Name,Allowance_Type,pin_code,gst,Owner_Name, ListedDr_Address1,Doc_ClsCode,doc_special_code, ListedDr_Phone,cityname, ListedDr_Email,Compititor_Id,Compititor_Name,Category_Universe_Id,Hatsun_AvailablityId,reason_category,LastUpdt_Date,Hatsanavail_Switch,HatsanCategory_Switch,ListedDr_Class_Patients,ListedDr_Consultation_Fee  ,Outlet_Type,reject_remarks,ListedDr_Pin,Listeddr_App_Mgr,Dist_Name) select getdate(),ListedDrCode,ListedDr_Name,Allowance_Type,pin_code,gst,Owner_Name, ListedDr_Address1,Doc_ClsCode,doc_special_code, ListedDr_Phone,cityname, ListedDr_Email,Compititor_Id,Compititor_Name,Category_Universe_Id,Hatsun_AvailablityId,reason_category,LastUpdt_Date,Hatsanavail_Switch,HatsanCategory_Switch,ListedDr_Class_Patients,ListedDr_Consultation_Fee,Outlet_Type,reject_remarks,ListedDr_Pin,Listeddr_App_Mgr,Dist_namefrom Mas_ListedDr where  ListedDrCode=" . $data["id"] . "";
        $temp = performQuery($query);
        $query = "update Mas_ListedDr set ListedDr_Name='".str_replace("'","",$data["unlisted_doctor_name"])."'";
		if(str_replace("'","",$data["img_name"])!="")
		{
			//$query =$query.",ListedDr_Pin='".str_replace("'","",$data["img_name"])."'";
			$query =$query.",ListedDr_Pin='".str_replace("'","",$data["img_name"])."',Profile_Img='".str_replace("'","",$data["img_name"])."'";
		}
		$freezer_files=$data["freezer_file_Details"];
		$freezerattach="";
		for ($i = 0; $i < count($freezer_files); $i++) {
			$freezerattach=$freezerattach.",".$freezer_files[$i]["freezer_filename"];
		}
		if($freezerattach!=""){
			$query =$query.",freezer_attachments='".$freezerattach."'";
		}
		if($data["State_Code"]!="")
		{
			$query =$query.",State_Code='".str_replace("'","",$data["State_Code"])."'";
		...
		//$query =$query.",Doc_Spec_ShortName='".str_replace("'","",$data["category_type"])."',Outstanding='".str_replace("'","",$data["outstanding_amount"])."',Outlet_Type='".$data["OutletTypeCd"]."',Allowance_Type='".$data["VechType"]."' ,pin_code=".$data["unlisted_doctor_pincode"].",gst=".$data["unlisted_doctor_gst"].",Owner_Name=".$data["unlisted_Owner_name"].", ListedDr_Address1='".str_replace("'","",$data["unlisted_doctor_address"])."',doc_special_code=" . $data["unlisted_specialty_code"].", ListedDr_Mobile=" . $data["unlisted_doctor_phone"] . " ,cityname=" . $data["unlisted_doctor_cityname"] . ", ListedDr_Email=" . $data["unlisted_doctor_Email"] . ",Compititor_Id=" . $data["Compititor_Id"] . ",Compititor_Name=" . $data["Compititor_Name"] . ",Category_Universe_Id=" . $data["CategoryType"] . ",Hatsun_AvailablityId=" . $data["AvailUniverSelectId"].",reason_category=" . $data["reason_category"] . ",LastUpdt_Date=getdate(),Hatsanavail_Switch=". $data["HatsunAvailswitch"].",HatsanCategory_Switch=". $data["categoryuniverseswitch"].",ListedDr_Class_Patients=".$data["lat"].",Listeddr_App_Mgr='".$updsfcode."',ListedDr_Consultation_Fee=".$data["long"].", ListedDr_Phone='" . str_replace("'","",$data["unlisted_doctor_secondphone"]) . "',Category_Universe_Name='".str_replace("'","",$data["sub_category"])."',Category_Universe_Remarks='".str_replace("'","",$data["unlisted_specialty_name"])."',Freezer_status='".str_replace("'","",$data["freezer_status"])."',Freezer_Tag_no='".str_replace("'","",$data["freezer_tagno"])."',Freezer_Type='".str_replace("'","",$data["freezer_make"])."',Milk_Potential='".str_replace("'","",$data["expected_sales_value"])."',FssiNo='".str_replace("'","",$data["fssai_number"])."',Pan_No='".str_replace("'","",$data["pan_number"])."',ListedDr_Active_Flag='".str_replace("'","",$data["flag"])."',reject_remarks='" . str_replace("'","",$data["OutletTypeRmks"]) . "',freezer_required='".str_replace("'","",$data["freezer_required"]). "',freezer_capacity='".str_replace("'","",$data["freezer_capacity"])."',deposit_amount='".str_replace("'","",$data["deposit_amount"])."',CustomerCode='".str_replace("'","",$data["customer_code"])."',SDP='".str_replace("'","",$data["districtname"])."' where  ListedDrCode=" . $data["id"] . "";
		$query =$query.",Doc_Spec_ShortName='".str_replace("'","",$data["category_type"])."',Outstanding='".str_replace("'","",$data["outstanding_amount"])."',Outlet_Type='".$data["OutletTypeCd"]."',Allowance_Type='".$data["VechType"]."' ,pin_code=".$data["unlisted_doctor_pincode"].",gst=".$data["unlisted_doctor_gst"].", ListedDr_Address1='".str_replace("'","",$data["unlisted_doctor_address"])."',doc_special_code=" . $data["unlisted_specialty_code"].", ListedDr_Mobile=" . $data["unlisted_doctor_phone"] . " ,cityname=" . $data["unlisted_doctor_cityname"] . ", ListedDr_Email=" . $data["unlisted_doctor_Email"] . ",LastUpdt_Date=getdate(),ListedDr_Class_Patients=".$data["lat"].",Listeddr_App_Mgr='".$updsfcode."',ListedDr_Consultation_Fee=".$data["long"].", ListedDr_Phone='" . str_replace("'","",$data["unlisted_doctor_secondphone"]) . "',Milk_Potential='".str_replace("'","",$data["expected_sales_value"])."',FssiNo='".str_replace("'","",$data["fssai_number"])."',Pan_No='".str_replace("'","",$data["pan_number"])."',ListedDr_Active_Flag='".str_replace("'","",$data["flag"])."',reject_remarks='" . str_replace("'","",$data["OutletTypeRmks"]) . "',CustomerCode='".str_replace("'","",$data["customer_code"])."',SDP='".str_replace("'","",$data["districtname"])."',contactperson='".str_replace("'","",$data["unlisted_Owner_name"])."' where  ListedDrCode=" . $data["id"] . "";
        $temp = performQuery($query);
		$sql="delete from Outlet_Category_Mapping where OutletCode='".str_replace("'","",$data["id"])."'";
		performQuery($sql);
		$sql="select ListedDrCode,Dist_name from Mas_ListedDr where ListedDrCode='".str_replace("'","",$data["id"])."'";
		$res=performQuery($sql);
		if(count($res)>0){
			$cCode=$res[0]["ListedDrCode"];
			$distname=$res[0]["Dist_name"];
			for ($i = 0; $i < count($outletCategory); $i++) {
				$sql="insert into Outlet_Category_Mapping(OutletCode,DistCode,OutletCat_Type,Category_Code,Category_Name,Sub_Category_Code,Sub_Category_Name,Price_List_No) select '".$cCode."','".$distname."','".$outletCategory[$i]["type_name"]."','".$outletCategory[$i]["cat_id"]."','".$outletCategory[$i]["cat_name"]."','".$outletCategory[$i]["subcat_id"]."','".$outletCategory[$i]["subcat_name"]."',(select top 1 Price_List_Sl_No from Mas_Doctor_Speciality where Doc_Special_Code='".$outletCategory[$i]["subcat_id"]."' and Doc_Special_Active_Flag=0)";
				performQuery($sql);
			}
		}
        $result["success"] = true;
...
case "get/LeaveAvailabilityCheck":
    ...
    $query = "select SFCode,LeaveCode,LeaveValue,LeaveAvailability,LeaveTaken,Leave_SName,Leave_Name from MasEntitlement  inner join mas_Leave_Type l on Leave_code=LeaveCode where SFCode='".$sfCode. "' and year(createDate)=year(getdate()) and l.Leave_code not in (25,39)";
...
case 'get/retailercountfdc':
 ...
 $result = array();
  ...
  $query ="select count(ListedDrCode) lrdr_count from Mas_ListedDr Ld where ld.division_code = '" . $Owndiv . "' and cast(CONVERT(varchar, ListedDr_Created_Date, 101) as datetime) = '".$today."'" ;
  $query ="select count(ListedDrCode) lrdr_count from Mas_ListedDr Ld where ld.division_code = '" . $Owndiv . "' and cast(CONVERT(varchar, ListedDr_Created_Date, 101) as datetime) = '".$today."'" ;
...
case "get/RetailerPaymentCollection":
	...
	case "get/LitterVsValues":
		...
		outputJSON(performQuery($query));
    break;	
	case "apprv/flightbook": 
		$data = json_decode($_POST['data'], true);
		$sfCode = (string) $data['SF'];
		$BookID = (string) $data['BookID'];
		$Rmks = (string) $data['Rmks'];
		$flag = (int) $data['flag'];
		if($flag == 4)
			$msg="Rejected";
		else
			$msg="Approved";
		$query = "update Trans_Flight_booking_head set ApprvFlg=".$flag.",RejReason='".$Rmks."' where BookID='".$BookID."'";
		performQuery($query);
		$Res=[];
		$Res["Msg"]="Ticket has been ".$msg." Successfully";
		$Res["Success"]="true";
		$Res["Qry"]=$query;
		outputJSON($Res);
	break;
	case "cancel/flightbook": 
		$data = json_decode($_POST['data'], true);
		$sfCode = (string) $data['SF'];
		$BookID = (string) $data['BookID'];
		
		$query = "update Trans_Flight_booking_head set ApprvFlg=3 where BookSF='".$sfCode."' and BookID='".$BookID."'";
		performQuery($query);
		$Res=[];
		$Res["Msg"]="Ticket has been cancelled Successfully";
		$Res["Success"]="true";
		$Res["Qry"]=$query;
		outputJSON($Res);
	break;
	case "save/flightbook":
		$data = json_decode($_POST['data'], true);
		$sfCode = (string) $data['SF'];
		$sfName = (string) $data['SFNm'];
		$TrvTyp = (string) $data['TrvMdType'];
		$FrmPlc = (string) $data['TrvFrmPlc'];
		$ToPlc = (string) $data['TrvToPlc'];
		$edtDep = (string) $data['TrvDepDt'];
		$TrvMorEve = (string) $data['TrvSes'];
		
		$retFrmPlc = (string) $data['retTrvFrmPlc'];
		$retToPlc = (string) $data['retTrvToPlc'];
		$retedtDep = (string) $data['retTrvDepDt'];
		$retTrvMorEve = (string) $data['retTrvSes'];
		$Trvlr = $data['Traveller'];
		$xml="<root>";
		for($i=0;$i<count($Trvlr);$i++){
			$xml=$xml."<usr Typ=\"".$Trvlr[$i]["CmpTyp"]."\" EmpID=\"".$Trvlr[$i]["EmpID"]."\" TrvName=\"".$Trvlr[$i]["TrvName"]."\" TrvCmp=\"".$Trvlr[$i]["TrvCmp"]."\" CmpMob=\"".$Trvlr[$i]["CmpMob"]."\" CmpGen=\"".$Trvlr[$i]["CmpGen"]."\" Rmks=\"".$Trvlr[$i]["remarks"]."\" />";
		}
		$xml=$xml."</root>";

		$query = "exec svFlightBooking '". $sfCode ."','". $sfName ."','". $TrvTyp ."','". $FrmPlc ."','". $ToPlc ."','". $edtDep ."','". $TrvMorEve ."','". $retFrmPlc ."','". $retToPlc ."','". $retedtDep ."','". $retTrvMorEve ."','". $xml ."'";
		performQuery($query);
		/*
		...
		*/
		$Res=[];
		$Res["Msg"]="Booking Request Submitted Successfully";
		$Res["Success"]="true";
		$Res["Qry"]=$query;
		outputJSON($Res);
 ...
 case "get/GoogleLogin":
		$usrnm = $_GET['Usrnm'];
		$pwd = $_GET['Pwd'];   //CASE WHEN  UsrDfd_UserName='HAPHTNSAO2' THEN '18.7357416:78.9700081' ELSE '12.91729615:80.23030043' END 
		$DeviceRegId = $_GET['DvID'];
		$sql = "SELECT  UsrDfd_UserName Sf_UserName,sf_emp_id,(select DeptName from mas_department where DeptID=sf_desgn) DeptName,
		sf_Designation_Short_Name,iif(SF_Status!=0,3,SF_Status) SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,
		Reporting_To_SF RptCode,
		(select SF_Name from mas_salesforce where Sf_code=ms.Reporting_To_SF) RptName,
		(select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,
		case when Approved_By='FF' or Approved_By='F' or Approved_By='I' then 1 else 0 end SFFType,sf_desgn SFDept,
		case when sf_desgn='6' or sf_desgn='83' then '2' when sf_desgn='37' then '1' else '0' end DeptType,
		case when Approved_By!='FF' and Approved_By!='F' and Approved_By!='I' then 1 else 1 end OTFlg,'CheckIn' loginType,
		isnull(latlong,'12.91729615:80.23030043') AS HOLocation, cast(HQ_ID as varchar) HQID,HQ_Name,m.ERP_Code,'1000' HQCode,2 THrsPerm,Profile_Pic Profile,'http://hap.sanfmcg.com/SalesForce_Profile_Img/' ProfPath,'HAP Check-In' AppName,'ic_launcher.png' AppIcon,ISNULL(Freezer_Mandatory,0)Freezer_Mandatory,FlightAllowedType FlightAllowed,checkRadius,DisRad radius from mas_salesforce ms WITH (NOLOCK) inner join Mas_HQuarters m WITH (NOLOCK) on cast(HQ_ID as varchar)=SF_HQ  inner join access_master am WITH (NOLOCK) on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT WITH (NOLOCK) on At.Sf_code= ms.Sf_code where UsrDfd_UserName='".$usrnm."' and Sf_Password='".$pwd."'"; // and Approved_By<>'I'
		$res = performQuery($sql);
		...
		if (count($res) > 0) {
			...
			if($res[0]["SF_Status"]!=0){
				$result["success"]=false;
				$result["CInData"]=[];
				$result["BlockMsg"]='Your Check-In ID is blocked. For further details contact HO HR1 Department';
			}
			else 
			{
				$today = date('Y-m-d');
				$Flag=0;
				$query = "exec CheckClearFlag '" . $res[0]['Sf_code'] . "' ";  
				...
				if ($Flag==0){
					$query = "select top 1 isnull(Sft_Name,'ON-Duty') Sft_Name,Start_Time,convert(varchar, Start_Time, 8) Edate ,isNull(SFTE_Time,'') End_Time,isnull(cast(convert(varchar,login_date-1,101) +' '+ convert (varchar,sft_STime,108) as datetime),Start_Time) as Sft_STime,isnull(SFTE_Time,cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime)) sft_ETime,WrkType Wtp ,isnull(cast(Sft_ID as varchar),'') Sft_ID ,case when  isnull(End_Time,0)=0 then 0 else 1 end [Type],(case when ACtOffTyp=0 or ShiftSelected_Id=0 then cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime) else dateadd(hour,ACtOffHrs,cast(convert(varchar,(case when(Sft_STime>sft_ETime) then dateadd(day,1,login_date) else login_date end),101) +' '+ convert (varchar,sft_ETime,108) as datetime)) End) ACutOff,'1' DeviationNeed,'HAP SFA' AppName,'ic_launcher.png' AppIcon  from TP_Attendance_App Tp WITH (NOLOCK) left outer join mas_shift_timings mst WITH (NOLOCK) on  mst.Sft_ID=Tp.ShiftSelected_Id where sf_code='" . $res[0]['Sf_code'] . "'  order by  Start_Time  desc";//'".$today."'
		...
		// ISNULL(CONVERT(varchar,MAX(Tm.SlotTime),108),'23:59:00')
			//$sql="select Stockist_Code, '23:59:00' CutoffTime,Stockist_Code SF_Code,Stockist_Name,Stockist_Mobile,Stockist_Address,'Distributor' loginType,S.Division_Code+',' Division_Code,ERP_Code sf_emp_id,ERP_Code,Username,State_Code,subdivision_code,S.DivERP DivERP,3 SalesReturnImg,s.CusSubGrpErp,ISNULL(Freezer_Mandatory,0)Freezer_Mandatory from Mas_Stockist S WITH (NOLOCK) inner join Access_Master ac WITH (NOLOCK)  on ac.division_code=s.Division_Code left outer join VwRouteCustomerMapping Tm WITH (NOLOCK) on DistId=Stockist_Code  and Tm.RouteCode=S.RouteCode where s.Stockist_Active_Flag=0 and Username='".$usrnm."' and Password='".$pwd."' group by Stockist_Code,Stockist_Code,Stockist_Name,Stockist_Mobile,Stockist_Address,S.Division_Code,ERP_Code,ERP_Code,Username,State_Code,subdivision_code,S.DivERP,s.CusSubGrpErp,ISNULL(Freezer_Mandatory,0)";

			$sql="select Stockist_Code, '23:59:00' CutoffTime,Stockist_Code SF_Code,Stockist_Name,Stockist_Mobile,Stockist_Address,'Distributor' loginType,S.Division_Code+',' Division_Code,ERP_Code sf_emp_id,ERP_Code,Username,State_Code,subdivision_code,'' DivERP,3 SalesReturnImg,'' CusSubGrpErp,0 Freezer_Mandatory from Mas_Stockist s WITH (NOLOCK) inner join Access_Master ac WITH (NOLOCK)  on ac.division_code=s.Division_Code where s.Stockist_Active_Flag=0 and Username='".$usrnm."' and Password='".$pwd."' group by Stockist_Code,Stockist_Code,Stockist_Name,Stockist_Mobile,Stockist_Address,s.Division_Code,ERP_Code,ERP_Code,Username,State_Code,subdivision_code";			//convert(varchar,isnull(SlotTime,),8)
			$res = performQuery($sql);
			
				$result["Qry"]=$sql;
			/*$sql="select CONVERT(varchar,tm.SlotTime,108)Tm from Mas_Stockist S WITH (NOLOCK)
					inner join VwRouteCustomerMapping Tm WITH (NOLOCK) on DistId=Stockist_Code and tm.Plant_Code=S.Plant_id
					where s.Stockist_Active_Flag=0 and Username='".$usrnm."' and Password='".$pwd."' order by SlotTime desc";
			$cutoff = performQuery($sql);*/
			if (count($res)>0){
				$res[0]["SlotTime"]=[];//$cutoff;
				$result["Data"]=$res;
				if ($DeviceRegId!=null&&!empty($DeviceRegId)) {
					$sql = "update Access_Table set  DeviceRegId='$DeviceRegId' where sf_code='" . $res[0]['Stockist_Code'] . "'";
					performQuery($sql);
				}
			}
			else
			{
				$result["Query"]=$sql;
				$result["CInData"]=[];
				$result["BlockMsg"]='Your Check-In ID is blocked. For further details contact HO HR 2Department';
				$result["success"]=false;
			}
		}
        ...
        $sql="select S_No Stockist_Code, '23:59:00' CutoffTime,S_No SF_Code,S_Name Stockist_Name,Mobile Stockist_Mobile ,''Stockist_Address,'SuperStockist' loginType,s.Division_Code+',' Division_Code,ERP_Code sf_emp_id,ERP_Code,UsrDfd_UserName Username,s.State_Code,subdivision_code,'' DivERP,3 SalesReturnImg,'' CusSubGrpErp,0 Freezer_Mandatory,Act_Flg active_flag,Division_Name from  Supplier_Master s WITH (NOLOCK) inner join Access_Master ac WITH (NOLOCK)  on ac.division_code=s.Division_Code inner join Mas_Division md with (nolock) on s.Division_Code=md.Division_Code where  UsrDfd_UserName='".$usrnm."' and sf_Password='".$pwd."' group by S_No,S_No,S_Name,Mobile,s.Division_Code,ERP_Code,ERP_Code,UsrDfd_UserName,s.State_Code,subdivision_code,Act_Flg,Division_Name";
		$res = performQuery($sql);
		...
		if (count($res) > 0) {
			$result["Data"]=$res;
			$result["success"]=true;
			$result["ActiveFlag"]=$res[0]['active_flag'];
			$sql = "insert into LoginDetails select '" . $res[0]['Sf_code'] . "',getdate(),getdate()";
			performQuery($sql);
			$result["Qry"]=$sql;
			if ($DeviceRegId!=null&&!empty($DeviceRegId)) {
		...
		// ISNULL(CONVERT(varchar,MAX(Tm.SlotTime),108),'23:59:00')
			...
			$sql="select Stockist_Code, '23:59:00' CutoffTime,Stockist_Code SF_Code,Stockist_Name,Stockist_Mobile,Stockist_Address,'Distributor' loginType,S.Division_Code+',' Division_Code,ERP_Code sf_emp_id,ERP_Code,Username,s.State_Code,subdivision_code,'' DivERP,3 SalesReturnImg,'' CusSubGrpErp,0 Freezer_Mandatory,Stockist_Active_Flag active_flag,Division_Name from Mas_Stockist s WITH (NOLOCK) inner join Access_Master ac WITH (NOLOCK)  on ac.division_code=s.Division_Code inner join Mas_Division md with (nolock) on s.Division_Code=md.Division_Code where Username='".$usrnm."' and Password='".$pwd."' group by Stockist_Code,Stockist_Code,Stockist_Name,Stockist_Mobile,Stockist_Address,s.Division_Code,ERP_Code,ERP_Code,Username,s.State_Code,subdivision_code,Stockist_Active_Flag,Division_Name";			//convert(varchar,isnull(SlotTime,),8)
			$res = performQuery($sql);
 ...
 case "get/newapplogin":
		...
		(select SF_Name from mas_salesforce where Sf_code=ms.Reporting_To_SF) RptName,
		(select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,
		case when Approved_By='FF' or Approved_By='I' then 1 else 0 end SFFType,sf_desgn SFDept,
		case when sf_desgn='6' or sf_desgn='83' then '2' when sf_desgn='37' then '1' else '0' end DeptType,
		case when Approved_By!='FF' and Approved_By!='I' then 1 else 1 end OTFlg,
		isnull(latlong,'12.91729615:80.23030043') AS HOLocation,
		cast(HQ_ID as varchar) HQID,HQ_Name,m.ERP_Code,'1000' HQCode,2 THrsPerm,Profile_Pic Profile,'http://hap.sanfmcg.com/SalesForce_Profile_Img/' ProfPath from mas_salesforce ms WITH (NOLOCK) inner join Mas_HQuarters m WITH (NOLOCK) on cast(HQ_ID as varchar)=SF_HQ  inner join access_master am WITH (NOLOCK) on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT WITH (NOLOCK) on At.Sf_code= ms.Sf_code where SF_Email='".$Email."' and SF_Mobile='".$Pwd."' and Approved_By<>'I'";
		$res = performQuery($sql);
		$result["success"]=true;
		...
		if (count($res) > 0) {
			...
			else 
			{
				...
				if ($Flag==0){
					$query = "select top 1 isnull(Sft_Name,'ON-Duty') Sft_Name,Start_Time,convert(varchar, Start_Time, 8) Edate ,isnull(SFTE_Time,'') End_Time,isnull(cast(convert(varchar,login_date-1,101) +' '+ convert (varchar,sft_STime,108) as datetime),Start_Time) as Sft_STime,isnull(SFTE_Time,cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime)) sft_ETime,WrkType Wtp ,isnull(cast(Sft_ID as varchar),'') Sft_ID ,case when  isnull(End_Time,0)=0 then 0 else 1 end [Type],(case when ACtOffTyp=0 or ShiftSelected_Id=0 then cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime) else dateadd(hour,ACtOffHrs,cast(convert(varchar,(case when(Sft_STime>sft_ETime) then dateadd(day,1,login_date) else login_date end),101) +' '+ convert (varchar,sft_ETime,108) as datetime)) End) ACutOff  from TP_Attendance_App Tp WITH (NOLOCK) left outer join mas_shift_timings mst WITH (NOLOCK) on  mst.Sft_ID=Tp.ShiftSelected_Id where sf_code='" . $res[0]['Sf_code'] . "'  order by  Start_Time  desc";//'".$today."'
					$arr=performQuery($query);
				$result["Q"]=$query;
				}	
				else{
...
case "get/calpriod":
	...
	case "get/FieldForce_HQ":
		...
		$SF_code = $_GET['sfCode'];
		$Owndiv = (string) $divs[0];
		$query = "select Sf_HQ from mas_salesforce WITH (NOLOCK) where   sf_code='".$SF_code."'";
		$Sf_HQ=performQuery($query);

		$query = "select HQ_ID id,HQ_Name name,AvlShifts ODFlag,HQ_Type from Mas_HQuarters WITH (NOLOCK) where Division_code='" . $Owndiv . "' and HQ_Active_Flag=0 order by HQ_Name";
		outputJSON(performQuery($query));
		break;		
		case "get/Shift_type":
		$div = $_GET['divisionCode'];
		...
		$SF_code = $_GET['sfCode'];
		$Owndiv = (string) $divs[0];
		$query="select shift_type id,Typename name from Shift_Type WITH (NOLOCK) where  Division_Code='" . $Owndiv . "' and Active_Flag=0 ";
		outputJSON(performQuery($query));
		break;
		case "get/Chilling_Centre":
		$div = $_GET['divisionCode'];
		$divs = explode(",", $div . ",");
		$SF_code = $_GET['sfCode'];
		$Owndiv = (string) $divs[0];
		$query="select CCentre_ID id,CCentre_Name name from mas_chill_centre where  Division_Code='" . $Owndiv . "' and Active_Flag=0 ";
		outputJSON(performQuery($query));
		break;
	...
	case "get/Signout_Check":
		...
		if ($Flag==0){
			$query = "select top 1 isnull(Sft_Name,'ON-Duty') Sft_Name,Start_Time,  convert(varchar, Start_Time, 8) Edate ,End_Time,isnull(cast(convert(varchar,login_date-1,101) +' '+ convert (varchar,sft_STime,108) as datetime),Start_Time) as Sft_STime,isnull(sft_ETime,cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime)) sft_ETime,WrkType Wtp ,isnull(cast(Sft_ID as varchar),'') Sft_ID ,case when  isnull(End_Time,0)=0 then 0 else 1 end [Type],(case when ACtOffTyp=0 or ShiftSelected_Id=0 then cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime) else dateadd(hour,ACtOffHrs,cast(convert(varchar,(case when(Sft_STime>sft_ETime) then dateadd(day,1,login_date) else login_date end),101) +' '+ convert (varchar,sft_ETime,108) as datetime)) End) ACutOff  from TP_Attendance_App Tp WITH (NOLOCK) left outer join mas_shift_timings mst WITH (NOLOCK) on  mst.Sft_ID=Tp.ShiftSelected_Id where sf_code='".$SF_code."'  order by  Start_Time  desc";//'".$today."'
...
case "get/ClearData":
	...
	case "get/tknPerm":
		...
		$mDt = strtotime($mDt."- 1 day");
		$tDt=date('Y-m-d',$mDt);
		$query = "select isnull(sum(cast(isnull(Noof_hours,0) as numeric)),0) tknHrS from PermissionEntry WITH (NOLOCK) where Approval_Flag!=1 and SF_Code='".$sfCode."' and cast(Permissiondate as dateTime) between '".$fDt."' and '".$tDt."'"; 
		//echo($query);
		outputJSON(performQuery($query));
		break;	
	...
	case "get/Shift_timing":
		...
		$sfCode = $_GET['Sf_code'];
		//;
        $query = "select Sft_ID id,Sft_Name name,Sft_STime, cast(convert(varchar,(case when Sft_STime>Sft_ETime then dateadd(day,1,getDate()) else getdate() End),101) +' '+ convert (varchar,sft_ETime,108) as datetime) as   sft_ETime from mas_shift_timings WITH (NOLOCK) where Division_code='" . $Owndiv . "' and ActiveFlag=0 and Sft_ID<17"; 
		if ($sfCode!=null){
	...
	case "get/foodexp":
		...
		$foodexp=array();
		$item=array();
		$item["date"]="20/03/2021";
		$fdDet=array();
		$iitm=array();
		$iitm["name"]="Breakfast";
		$iitm["amount"]="50";
		array_push($fdDet,$iitm);
		$iitm=array();
		$iitm["name"]="Lunch";
		$iitm["amount"]="45";
		array_push($fdDet,$iitm);
		...
		$item["FoodDet"]=$fdDet;
		$item["amount"]="50";
		array_push($foodexp,$item);
		
		$item=array();
		$item["date"]="21/03/2021";
		$fdDet=array();
		$iitm=array();
		$iitm["name"]="Breakfast";
		$iitm["amount"]="50";
...
outputJSON($result);
	...
	case "Check/Version":
		...
		if($VERSION==$Ver){ //|| $VERSION=="Ver 2.3.1" || $VERSION=="Ver 2.3.2" || $VERSION=="Ver 2.2.2"
			$result["Msg"] = "";
			$result["success"] = true;
		}else{
		...
		$tbdate = $_GET['Date'];
		//;
		//$query = "select top 1 sf_code from TbMyDayPlan";
        $query = "select sf_code,(select count(SF_code) from Expense_Start_Activity WITH (NOLOCK) where sf_code=tb.sf_code and  convert(date,Date_Time)=convert(date,Pln_Date)) Cnt,wtype from TbMyDayPlan tb where sf_code='".$sfCode."' and  convert(date,Pln_Date)='" . $tbdate . "'"; 
		$CountofTb= performQuery($query);
		if(count($CountofTb)>0){			
			$query = "select Place_Involved from Mas_WorkType_BaseLevel where WorkType_Code_B='".$CountofTb[0]["wtype"]."'"; 
			$res= performQuery($query);
			if(count($res)>0){
				if($res[0]["Place_Involved"]=="N") $CountofTb[0]["Cnt"] = 1;
			}
		}
		$result["Checkdayplan"] = $CountofTb;
		
		$query="select isnull(count(Duty_Mode),0) Countd from TP_Attendance_App WITH (NOLOCK) where   Sf_Code='".$sfCode."'   and    convert(date,Start_Time)='" . $tbdate . "'  and Duty_Mode=1";
		$Checkonduty= performQuery($query);

		$query="select isnull(count(Duty_Mode),0) Todaycheckin_Flag from TP_Attendance_App WITH (NOLOCK) where   Sf_Code='".$sfCode."'   and    convert(date,Start_Time)='" . $tbdate . "'  ";
		$checkcheckin= performQuery($query);
		
		$query="select COUNT(*)cnt from TP_Attendance_App WITH (NOLOCK) where Sf_Code='".$sfCode."' and login_date=CONVERT(date,GETDATE())";
		//$query="select COUNT(*)cnt from Expense_Start_Activity where sf_code='".$sfCode."' and CONVERT(date,Date_Time)=CONVERT(date,GETDATE()) and MOT in (1,2) and ((MOT in (1,2) and End_Km is not null) or (End_Km is null and MOT not in (1,2)))";
		$checkMOT= performQuery($query);
		
		//$query="select COUNT(*)cnt from TP_Attendance_App where Sf_Code='".$sfCode."' and login_date=CONVERT(date,GETDATE()) and GETDATE()>SFTE_Time";
		//$query="select COUNT(*)cnt from Expense_Start_Activity where sf_code='".$sfCode."' and CONVERT(date,Date_Time)=CONVERT(date,GETDATE()) and MOT in (1,2) and ((MOT in (1,2) and End_Km is not null) or (End_Km is null and MOT not in (1,2)))";
		//$checkMOT= performQuery($query);
		
		
		$query="select COUNT(*)cnt from TP_Attendance_App WITH (NOLOCK) where Sf_Code='".$sfCode."' and login_date=CONVERT(date,GETDATE()) and End_Time is null";
		//$query="select COUNT(*)cnt from Expense_Start_Activity where sf_code='".$sfCode."' and CONVERT(date,Date_Time)=CONVERT(date,GETDATE()) and MOT in (1,2) and ((MOT in (1,2) and End_Km is not null) or (End_Km is null and MOT not in (1,2)))";
		$chkarr= performQuery($query);
		$query="select top 1 End_Km ckhend,convert(varchar,Date_Time,23)dt,convert(varchar,Date_Time,103)dtt from Expense_Start_Activity ex WITH (NOLOCK) inner join Mas_Modeof_Travel mt WITH (NOLOCK) on mt.Sl_No=ex.MOT where Sf_code='".$sfCode."' and mt.StEndNeed='1' and End_Km is null";  //and CONVERT(date,Date_Time)<>CONVERT(date,GETDATE())
		$checkEndMot= performQuery($query);	
		$query="select top 1 End_Km ckhend,convert(varchar,Date_Time,23)dt,convert(varchar,Date_Time,103)dtt from Expense_Start_Activity ex WITH (NOLOCK) inner join Mas_Modeof_Travel mt WITH (NOLOCK) on mt.Sl_No=ex.MOT where Sf_code='".$sfCode."' and mt.StEndNeed='1' and End_Km is null and CONVERT(date,Date_Time)<>CONVERT(date,GETDATE())";
		$TrvOth= performQuery($query);		
		if(count($checkEndMot)<1 || $chkarr[0]["cnt"]>0) 
			$result["CheckEndKM"]=0;
		else if(($checkEndMot[0]["ckhend"]==null && $chkarr[0]["cnt"]<1 && $checkMOT[0]["cnt"]>0) ||
			 (count($TrvOth)>0 && $TrvOth[0]["ckhend"]==null))
			$result["CheckEndKM"]=1;
		else
			$result["CheckEndKM"]=(($checkEndMot[0]["ckhend"]==null && $chkarr[0]["cnt"]<1) || ($checkMOT[0]["cnt"]>0 && $chkarr[0]["cnt"]>0))?0:1;//$checkEndMot[0]["ckhend"];
		$result["CheckEndDT"]=($checkEndMot[0]["dt"]==null)?'':$checkEndMot[0]["dt"];
		
		$result["CheckOnduty"]=$Checkonduty[0]["Countd"];
		$result["checkMOT"]=$checkMOT[0]["cnt"];
		$result["Todaycheckin_Flag"]=$checkcheckin[0]["Todaycheckin_Flag"];
		...
		/* if (!empty($reg_id)) {
			//$msg = $name . " Applied Leave for " . $vals['No_of_Days'] . " days";
			$msg = "Your Daily Inventory Approved...";
			sendFCM_notify($reg_id, "FMCG","Daily Inventory",0,'#sign-in');
		} */ 
		outputJSON($result);
		break;
		...
		$tbdate = $_GET['Date'];
		//;
		$query="select * from vwTourPlan WITH (NOLOCK) where sf_Code='".$sfCode."' and convert(date,[date])='" . $tbdate . "'";
		$Gettpdayplan= performQuery($query);
		
		$result["GettodayResult"] = $Gettpdayplan;
		$sql="select * from Tp_DynamicTable  where Ukey='".$Gettpdayplan[0]['Territory_Code2']."'";
		...
		if (count($res)>0) 
		{
			for ($il=0;$il<count($res);$il++)
			{
				...
				$res[$il]['inputs']=[];
				if($id=='10' ||  $id=='17' ||  $id=='7'){
					$query = "select top 100  ".$qucol.",$tblclmname from ".$tblclm." ";	
					$res[$il]['inputs']=(performQuery($query)==null?[]:performQuery($query));
				} 
				
			}
		} 		
		$result["DynamicViews"] = $res;
		outputJSON($result);
	break;
	case "Get/dayplanresult":
		$div = $_GET['divisionCode'];
		$divs = explode(",", $div . ",");
	    $Owndiv = (string) $divs[0];
		$sfCode = $_GET['sfCode'];
		$tbdate = $_GET['Date'];
		//;
		$query="select top 1 cluster,stockist,ClstrName,StkName,Button_Access,wtype from TbMyDayPlan WITH (NOLOCK) where sf_code='".$sfCode."' and convert(date,Pln_Date)='" . $tbdate . "'";
		outputJSON(performQuery($query));
	break;
	case "Get/MoreinfomonthName":
		$div = $_GET['divisionCode'];
		$divs = explode(",", $div . ",");
	    $Owndiv = (string) $divs[0];
		$sfCode = $_GET['Sf_code'];
		$tbdate = $_GET['Date'];
		$query=" select  cast (month(DATEADD(month, 0, getdate())) as varchar) CM ,cast (month(DATEADD(month, -1, getdate())) as varchar) NM ,cast (month(DATEADD(month, -2, getdate())) as varchar)PNM
			  union
			  select SUBSTRING(DATENAME(month,DATEADD(month, 0, getdate())),1,3) ,SUBSTRING(DATENAME(month,DATEADD(month, -1, getdate())),1,3), SUBSTRING(DATENAME(month,DATEADD(month, -2, getdate())),1,3) PNM
			  union
			  select  cast (year(DATEADD(month, 0, getdate())) as varchar) ,cast (year(DATEADD(month, -1, getdate())) as varchar),cast (year(DATEADD(month, -2, getdate())) as varchar)";
		$Gettpdayplan= performQuery($query);
		$result["Getmonthname"] = $Gettpdayplan;
		outputJSON($result);
	break;
	case "Get/sivacount":
		echo $_GET['sfCode'];
		...
		//;
		$queryn = "exec getContinuousStayDt '".$SF_code."','".$Selectdate."'";
		//$query="select top 1 cluster,stockist,ClstrName,StkName,Button_Access,wtype from TbMyDayPlan where sf_code='".$sfCode."' and convert(date,Pln_Date)='" . $tbdate . "'";
		outputJSON(performQuery(queryn));
	break;
	case "get/WrkDates":
        $Sf_Code = $_GET['Sf_code'];
		...
		outputJSON(performQuery($query));
	break;
	case "get/outletbulksync":
		$customerCode=$_GET['CustCode'];
		outputJSON(outletbulksync($customerCode));
	break;
	case "save/ondutyupdate":
		$data = json_decode($_POST['data'], true);
		$sfCode=$data["login_sfCode"];
		$eDate=$data['shift_date'];
		$DFlag=$data["Flag"];
		if($DFlag==1)
		{
			$sql = "select count(sf_code) HCount from TP_Attendance_App WITH (NOLOCK) where sf_code='$sfCode' and convert(date,Start_Time)=convert(date,'$eDate') and Duty_Mode=0 and WrkType=1";
			$device = performQuery($sql);
			$HCount = $device[0]['HCount'];
			if($HCount>0){
				$flag=0;
				...
				if($flag==0){
					$sql ="update TP_Attendance_App set Duty_Mode=1 where Sf_Code='$sfCode' and convert(date,Start_Time)=convert(date,'$eDate') and WrkType=1";
					performQuery($sql);
					$result["qry"]=$sql;
					$HMSG="Holiday Entry Created.";
					$result["success"]=true;
					$result["Msg"]=$HMSG;
				}
			}
			...
			{
				$HMSG="Holiday Entry Creation Failed.";
				$result["success"]=false;
				$result["Msg"]=$HMSG;
			}
		}
	...
	case "get/ondutystat":
		...
		$sql="exec getHolidayStat '$sfCode','$fDt','$tDt'";
		$res = performQuery($sql);
		if(count($res)>0){
			$result["success"]=true;
			$result["Data"]=$res;
