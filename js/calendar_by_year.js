...
function buildYearCalendar(year) {
  ...
  var populateMonth = function (year, month) { // Month range: 0-11
    ...
    if (lk < 6) { table_row = createTableRow(); $(table_row).append('<td>&nbsp;</td>'); }
  };
  ...
  MONTHS.forEach(function (month) {
      ...
      $(month_div).append('<table id="' + month + '" />');
  });
  $(calendar_year).append(month_div);
...
function dayClicked(that) {
  ...
  if ($('.selected.select-start').length === 0) {
    resetSelection();
  }
...
function updateSelectedDates() {
  ...
  if (start_date) {
    if (!end_date) {
        ...
        $('#datefrom').val(formatDate(start_date));
        $('#dateto').val(formatDate(start_date));
    } else {
        // date_text = 'Selected Dates: ' + start_date + ' to ' + end_date;
        date_text = '' + start_date + ' To ' + end_date;
        $('#datefrom').val(formatDate(start_date));
        $('#dateto').val(formatDate(end_date));
    }
  }
