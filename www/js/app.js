...
.state('fmcgmenu.TargetAndAchieve', {
    ...
    .controller('MainCtrl', ['$rootScope', '$scope', '$state', '$ionicModal', '$ionicScrollDelegate', '$ionicPopup', '$ionicPopover', '$location', 'fmcgLocalStorage', 'fmcgAPIservice', '$ionicSideMenuDelegate', 'geolocation', '$interval', 'notification', '$timeout', '$ionicLoading', '$filter', '$http', function($rootScope, $scope, $state, $ionicModal, $ionicScrollDelegate, $ionicPopup, $ionicPopover, $location, fmcgLocalStorage, fmcgAPIservice, $ionicSideMenuDelegate, geolocation, $interval, notification, $timeout, $ionicLoading, $filter, $http) {
        ...
        fmcgAPIservice.getPostData('POST', 'save/ver&ver=6.0.8', []).success(function(response) {}).error(function() {});
...
/*if($scope.fmcgData.FIRSTCALLERROR==0){
$scope.SlTyp = $scope.fmcgData.customer.selected.id;
                                if ($scope.SlTyp != 7) {
                                    if ($scope.fmcgData.doctor != undefined) {
                                        $scope.PrvData.id = $scope.fmcgData.doctor.selected.id;
...
_savLocal($scope, $state);
 ...
 if ($scope.SlTyp != 7 && $scope.SetupCallPreview != 0 && NETAMOUNT != undefined && NETAMOUNT > 0)
                                $state.go('fmcgmenu.prvcall');
        ...
        $scope.clearIdividual = function(value, total, CDId, cMod) {
            ...
            switch (value) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    fmcgAPIservice.getDataList('POST', 'table/list', ["vwstockiest_Master_APP", '["distributor_code as id", "stockiest_name as name","town_code","town_name","Addr1","Addr2","City","Pincode","GSTN","lat","long","addrs","Tcode"]', , '["isnull(Stockist_Status,0)=0"]'], CDId.replace(/_/g, ''))
                        .success(function(response) {
...
$scope.uploadPhotoretailer = function(imgId, imgData) {
        ...
        $scope.openModal = function(sdata, choice, repId) {
            ...
            if (typeof(sdata) == 'string') {
                ...
                if (tSdta == 'town_master' && choice != 32 && choice != 22) {
                    ...
                    if ($scope.view_MR == 1 || $scope.view_MR == 0) {
                        ...
                        if ((choice == 102 && $scope.DistBased == 1) || choice==506  ) {
            ...
            // else if (",4,14,".indexOf("," + choice + ",") > -1) {
             ...
             else if (",8,9,10,29,41,49,88,89,".indexOf("," + choice + ",") > -1) {
                $scope.SelMulti = 1;
                if (choice == 8)
...
/*if(window.innerHeight > window.innerWidth){
    ...
    .controller('myTodyPlCtrl', ['$rootScope', '$scope', '$state', '$ionicPopup', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', '$ionicLoading', '$filter', '$timeout', function($rootScope, $scope, $state,$ionicPopup, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification, $ionicLoading, $filter, $timeout) {
       ...
       $scope.setETime = function() {
        ...
        var $fl = 0;
               if(userData.divisionCode.replace(',','')==4 ||  userData.divisionCode.replace(',','')==57){
                   window.localStorage.removeItem("Tour_Plan_ND");
                               fmcgAPIservice.getDataList('POST', 'table/list&CMonth='+ (new Date().getMonth()+2)+'&CYr='+new Date().getFullYear(), ['vwtourplanfcd', '["date","remarks","worktype_code","worktype_name","RouteCode","RouteName","Worked_with_Code","Worked_with_Name","JointWork_Name"]'], $scope.Sf_Code)
                                .success(function(response) {
                                  
                                    if (response.length && response.length > 0 && Array.isArray(response))
                                        fmcgLocalStorage.createData("Tour_Plan_ND", response);
...
if($scope.view_MR != 1 && $scope.Supplier_Master==1){
...
if($scope.data.location==undefined || $scope.data.location=='' ){
    $scope.data.location=$scope.fmcgData.location;
            ...
            if((window.localStorage.getItem("Tour_Plan_ND")==null && cdate >$scope.TP_Mandatory_ND) ||(    [window.localStorage.getItem("Tour_Plan_ND")].length==0 &&cdate>$scope.TP_Mandatory_ND)){
                     $scope.MTPEnty.Month = (tDate.getMonth() + 2);
                ...
                if ((window.localStorage.getItem("Tour_Plan_ND")==undefined &&  cdate>=$scope.TP_Remainder_Date) || ( [window.localStorage.getItem("Tour_Plan_ND")].length==0 &&cdate>= $scope.TP_Remainder_Date  && $scope.TP_Mandatory_ND >= cdate)){
...
if(($scope.$parent.Myplns.length==0 || ($scope.$parent.Myplns.length>0 && $scope.CheckSelfie==1))&& userData.Selfie==1  && ($scope.photosList==undefined ||$scope.photosList.length==0 )){
        $scope.openCameraProfile();
        console.log("InsideOpen Camera");
        return false;
      }
...
*/
   ...
   $scope.$parent.fmcgData.BreedCategory=fmcgLocalStorage.getData("BreedCategory") || [];
...
//Tour Plan
 ...
 $scope.gridOptionsroute = {
    ...
    .controller('homeCtrl', ['$rootScope', '$scope', '$state', '$ionicPopup', 'fmcgAPIservice', 'fmcgLocalStorage', 'notification', '$ionicLoading', '$ionicModal', '$ionicScrollDelegate', function($rootScope, $scope, $state, $ionicPopup, fmcgAPIservice, fmcgLocalStorage, notification, $ionicLoading, $ionicModal, $ionicScrollDelegate) {
        ...
        if ($scope.Mypln.worktype.FWFlg == 'DH') {
            ...
            /*if($scope.sfCode!=undefined){
               ...
               return unique_array
                ...
                var data = fmcgLocalStorage.getData('doctor_master_' +$scope.sfCode) || [];

              $scope.TotalRoute = fmcgLocalStorage.getData("TodayTotalRoute") || [];
            $scope.TL = [];
            for (i = 0; i < removeDuplicates($scope.TotalRoute).length; i++) {
             var bb = data.filter(function(a) {
              return (a.town_code == removeDuplicates($scope.TotalRoute)[i])
             });
             $scope.TL.push(bb);
            }
            count = 0;
            for (i = 0; i < $scope.TL.length; i++) {
             count += $scope.TL[i].length;
            }
       ...
       //$scope.fmcgData.FIRSTCALLERROR=response.today.calls;
                $scope.labels = ["UnProductiveCalls", "Productive Calls"];
                $scope.data = [response.today.calls - response.today.order, response.today.order];
                $scope.monthdata = [response.month.calls - response.month.order, response.month.order];
                $scope.modal.monthvwCalls.productivity = parseFloat(($scope.modal.monthvwCalls.order / $scope.modal.vwCalls.totalRetailor) * 100).toFixed(2) + "%";
        ...
        $scope.disti=1;
         ...
         $scope.$on('setGeodistData', function(e) {
                    ...
                    /*$scope.geoDistributor = data.filter(function(a) {
                        return (a.lat != '' &&  (',' + $scope.geoTag.stockist_code.selected.id + ',').indexOf(',' + a.id + ',') > -1);
