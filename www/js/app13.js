var fmcg = angular.module('fmcg', ['ionic', 'fmcgServices', 'ngGrid', 'infinite-scroll', 'chart.js', 'flexcalendar', 'pascalprecht.translate', 'ion-sticky'])
    .config(function($stateProvider, $urlRouterProvider, $locationProvider) {
        $stateProvider
            ...
            .state('fmcgmenu.INHfeedbk', {
                url: "/INHfeedbk",
                views: {
                    'menuContent': {
                        templateUrl: "partials/InhouseFeedbk.html",
                        controller: "inhfeedbkCtrl"
                    }
                }
            })
            ...
            .state('fmcgmenu.dayFeedbk', {
                url: "/dayFeedbk",
                views: {
                    'menuContent': {
                        templateUrl: "partials/DayFeedback.html",
                        controller: "dayFeedbkCtrl"
                    }
                }
            })
            .state('fmcgmenu.custReview', {
                url: "/custReview",
                views: {
                    'menuContent': {
                        templateUrl: "partials/CallReview.html",
                        controller: "custReviewCtrl"
                    }
                }
            })
            .state('fmcgmenu.dcr', {
            ...
            .state('fmcgmenu.clview', {
                url: "/CLview",
                views: {
                    'menuContent': {
                        templateUrl: "partials/ViewCLDetails.html",
                        controller: "clViewCtrl"
                    }
                }
            });
    ...
    .controller('SignInCtrl', ['$rootScope', '$scope', '$state', 'fmcgAPIservice', 'fmcgLocalStorage', 'notification', function($rootScope, $scope, $state, fmcgAPIservice, fmcgLocalStorage, notification) {
        ...
        $scope.signIn = function(user) {
            ...
            if (flag == true && diffDays > 0) {
                ...
                window.localStorage.removeItem("INHFeeds");
    ...
    .controller('MainCtrl', ['$rootScope', '$scope', '$state', '$ionicModal', '$ionicScrollDelegate', '$ionicPopup', '$location', 'fmcgLocalStorage', 'fmcgAPIservice', '$ionicSideMenuDelegate', 'geolocation', '$interval', 'notification', '$timeout', '$ionicLoading', '$filter', function($rootScope, $scope, $state, $ionicModal, $ionicScrollDelegate, $ionicPopup, $location, fmcgLocalStorage, fmcgAPIservice, $ionicSideMenuDelegate, geolocation, $interval, notification, $timeout, $ionicLoading, $filter) {
        ...
        $scope.submit = function() {
            ...
            fmcgLocalStorage.createData("chkout", 0);
        ...
        $scope.EChmCap = userData.EChmCap;
        //thiruoutlet
        $scope.OUTletcount = "Outlet Review";
        // Chemist Caption
        ...
        loadSetups = function() {
            ...
            fmcgAPIservice.getDataList('POST', 'get/setup', [])
                .success(function(response) {
                    ...
                    $scope.RateEdit = 1; ///loginData.RateEdit;
        ...
        $scope.clearIdividual = function(value, total, CDId, cMod) {
            ...
            switch (value) {
                case 0:
                    fmcgAPIservice.getDataList('POST', 'table/list', ["vwDoctor_Master_APP", '["doctor_code as id", "doctor_name as name","town_code","town_name","lat","long","addrs","ListedDr_Address1","ListedDr_Sl_No","Mobile_Number"]', , '["isnull(Doctor_Active_flag,0)=0"]'], CDId.replace(/_/g, ''))
                        .success(function(response) {
                ...
                case 12:
                    if (cMod == true || total == 1) {
                        fmcgAPIservice.getDataList('POST', 'table/list', ["vwMyDayPlan", '["worktype","FWFlg","sf_member_code as subordinateid","cluster as clusterid","ClstrName","remarks","stockist as stockistid","worked_with_code","worked_with_name","dcrtype","location"]', , ])
                            .success(function(response) {
                                window.localStorage.removeItem("mypln");
        ...
        if ($scope.worktypes.length == 0) {
            ...
            if ($scope.Myplns.length == 0 && $scope.SFStat == '0') {
                ...
                fmcgAPIservice.getDataList('POST', 'table/list', ["vwMyDayPlan", '["worktype","FWFlg","sf_member_code as subordinateid","cluster as clusterid","stockist as stockistid","worked_with_code","worked_with_name","ClstrName","remarks"]', , ])
                    .success(function(response) {
                        if (response.length && response.length > 0 && Array.isArray(response)) {
                            $scope.fmcgData['jontWorkSelectedList'] = [];
                            if (response.length != 0) {
        ...
        $scope.openModal = function(sdata, choice, repId) {
            ...
            if (typeof(sdata) == 'string') {
                ...
                if (tSdta == 'town_master' && choice != 32 && choice != 22) {
                    ...
                    if ($scope.view_MR != 1) {
                        if (($scope.SF_type == 2 || $scope.SF_type == 1) && choice == 502) {
                            data1 = data.filter(function(a) {
                                return (a.stockist_code == $scope.Retailer.stockist.selected.id);
                            });
                            data = data1;
                        }
                        if (($scope.SF_type == 2 || $scope.SF_type == 1) && choice == 102 && $scope.BrndSumm.stockist == undefined) {
                            data1 = data.filter(function(a) {
                                return (a.stockist_code == $scope.Mypln.stockist.selected.id);
                            });
                            data = data1;
                        } else if (($scope.SF_type !== 2 || $scope.SF_type == 1) && choice == 2) {
            ...
            if (",5,6,7,25,26,36,106,503,".indexOf("," + choice + ",") > -1) {
                ...
                tCode = '';
                if (",6,36,503,".indexOf("," + choice + ",") > -1 && $scope.CusOrder == 1) {
                ...
                if (CurrentData[i].checked == true) {
                    if (",8,41,".indexOf("," + SlTyp + ",") > -1) {
                        ...
                        if (SlTyp == 9) {
                            ...
                            if (CurrentData[i].rate == undefined) CurrentData[i].rate = $scope.roundNumber($scope.RateByCd(CurrentData[i].id, $scope.fmcgData.customer.selected.id), 2) || 0.00;
        ...
        $scope.selectData = function(item) {
            ...
            switch ($scope.selectionType) {
                ...
                default:
                    ...
                    if ($scope.selectionType == 144) {
                        ...
                        if ($scope.selectionType == 206) {
                            ...
                            console.log($scope.Mypln.doctor.name)
                ...
                case 15:
                    ...
                    break;
                case 215:
                    $scope.INHFeed.remarks = item.name;
                    break;
                case 315:
                    $scope.dyFeedbk.remarks = item.name;
                    break;
                case 27:
                ...
                case 208:
                    ...
                    if ($scope.selectionType == 50) {
                        ...
                        $scope.$broadcast('DayReportHy');
                        $scope.$broadcast('getMonthReport');
                        $scope.$broadcast('getCLReport');
        ...
        function svQDatas() {
            ...
            if ($scope.onLine && navigator.onLine) {
                ...
                var Outletrevieww = fmcgLocalStorage.getData("Outlet") || [];
                if (Outletrevieww.length > 0 && $scope.savingData.Outlet == false) {

                    var i;
                    for (i = 0; i < Outletrevieww.length; i++) {




                        var Outletreview = Outletrevieww[i];


                        fmcgAPIservice.addMAData('POST', 'save/callreview', '29', Outletreview)
                            .success(function(response) {
                                if (response['success']) {
                                    ...
                                    //Toast('Local calls synced', true);
                                } else if (response.msg == "Outlet Review Already Entered") {
                                    Outletrevieww.shift();
                                    //  Toast(response.msg);
                                    $ionicLoading.hide();
                                } else {

                                    var Outlett = Outletrevieww.shift();

                                    Outletrevieww.push(Outlett);
                                }
                                fmcgLocalStorage.createData("Outlet", Outletrevieww);
        ...
        setInterval(function() {
            ...
            $scope.outboxCount += QueData.length;
            //thiru
            var QueData = fmcgLocalStorage.getData("Outlet") || [];
            $scope.outboxCount += QueData.length;
    ...
    .controller('myTodyPlCtrl', ['$rootScope', '$scope', '$state', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', '$ionicLoading', '$filter', '$timeout', function($rootScope, $scope, $state, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification, $ionicLoading, $filter, $timeout) {
        ...
        $scope.ChkInFlg = fmcgLocalStorage.getData("chkout") || '0';
        $scope.data = {};
        ...
        $scope.save = function() {
            ...
            if ($scope.Mypln.worktype.selected.FWFlg == 'DH') {
                ...
                if ($scope.view_MR != 1 && ($scope.Mypln.subordinate == undefined || $scope.Mypln.subordinate.selected == undefined || $scope.Mypln.subordinate.selected.id == undefined || $scope.Mypln.subordinate.selected.id == '')) {
                    Toast('Select the field force...');
                    return false;
                }
                if ($scope.Mypln.stockist == undefined || $scope.Mypln.stockist.selected == undefined || $scope.Mypln.stockist.selected.id == undefined || $scope.Mypln.stockist.selected.id == '') {
                    Toast("Select " + $scope.StkCap);
                    return false;
                }
                if ($scope.Mypln.cluster == undefined || $scope.Mypln.cluster.selected == undefined || $scope.Mypln.cluster.selected.id == undefined || $scope.Mypln.cluster.selected.id == '') {
                    Toast('Select the Cluster...');
                    return false;
                }
                if ($scope.Mypln.worktype.selected.FWFlg == 'F' && $scope.edit_sumry == 0 && ($scope.Mypln.dcrtypes == undefined || $scope.Mypln.dcrtypes.selected == undefined || $scope.Mypln.dcrtypes.selected.id == '')) {
                ...
                if ($scope.Mypln.worktype.selected.FWFlg == 'IH') {
                    ...
                    if ($scope.Mypln.doctor == undefined || $scope.Mypln.doctor.name == undefined || $scope.Mypln.doctor.name == '') {
                        Toast('Select the InHouse Retail / Place...');
                        return false;
                    }
                    if ($scope.Mypln.doctor.selected == undefined) $scope.Mypln.doctor.selected = {};
                    if ($scope.Mypln.doctor.selected.id == undefined || $scope.Mypln.doctor.selected.id == '') $scope.Mypln.doctor.selected.id = 'C-1';
            ...
            fmcgAPIservice.addMAData('POST', 'dcr/save', "3", $scope.data).success(function(response) {
                if (response.success) {
                    ...
                    if ($scope.data.FWFlg == "IH") fmcgLocalStorage.createData("chkout", 1);
    ...
    .controller('addULDocCtrl', ['$rootScope', '$scope', '$state', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', function($rootScope, $scope, $state, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification) {
        ...
        $scope.save = function() {
            ...
            if ($scope.fmcgData.class == undefined || $scope.fmcgData.class.selected == undefined) {
                Toast('Select the Retailer Class...');
                return false;
            }
            ...
            if ($scope.fmcgData.speciality == undefined || $scope.fmcgData.speciality.selected == undefined) {
                Toast('Select the Retailer Channel...');
                return false;
            }
    ...
    .controller('editRetailerCtrl', ['$rootScope', '$scope', '$state', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', '$ionicLoading', '$ionicScrollDelegate', '$timeout', function($rootScope, $scope, $state, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification, $ionicLoading, $ionicScrollDelegate, $timeout) {
        ...
        $scope.getDatas = function() {
            ...
            fmcgAPIservice.getPostData('POST', 'get/retailer', {
                    "id": $scope.Retailer.doctor.selected.id
                })
                .success(function(response) {
                    $scope.Retailer.Data.name = response[0].RetailerName;
                    $scope.Retailer.Data.address = response[0].Addr;
                    $scope.Retailer.Data.phone = response[0].PhoneNo;
                    $scope.Retailer.Data.class = {};
                    $scope.Retailer.Data.class.selected = {};
        ...
        $scope.SaveDatas = function() {
            ...
            if ($scope.Retailer.Data.address == "" || $scope.Retailer.Data.address == undefined) {
                if ($scope.NdrAddrNd == 1) {
                    Toast('Enter the Address...');
                    return false;
                } else
                    $scope.Retailer.Data.address = "";
            }
            if ($scope.Retailer.Data.class == undefined || $scope.Retailer.Data.class.selected == undefined) {
                Toast('Select the Retailer Class...');
                return false;
            }

            if ($scope.Retailer.Data.speciality == undefined || $scope.Retailer.Data.speciality.selected == undefined) {
                Toast('Select the Retailer Channel...');
                return false;
            }
    ...
    .controller('TourPlanCtrl', ['$rootScope', '$scope', '$state', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', '$ionicLoading', '$ionicModal', '$filter', function($rootScope, $scope, $state, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification, $ionicLoading, $ionicModal, $filter) {
                    ...
                    function updateData() {
                        ...
                        for (var key in tourPlan) {
                            if ($scope.data.date === (tourPlan[key]['date'])) {
                                ...
                                if (tourPlan[key]['worktype_name'] == "Field Work/Retailer Work") {
        ...
        $scope.save = function() {
            ...
            if ($scope.data.worktype_name == 'Field Work/Retailer Work') {
    ...
    .controller('dashbrdCtrl', ['$rootScope', '$scope', '$state', '$ionicPopup', 'fmcgAPIservice', 'fmcgLocalStorage', 'notification', '$ionicLoading', '$ionicModal', '$ionicScrollDelegate', function($rootScope, $scope, $state, $ionicPopup, fmcgAPIservice, fmcgLocalStorage, notification, $ionicLoading, $ionicModal, $ionicScrollDelegate) {
        ...
        $scope.sendWhatsApp = function() {
            ...
            $('body').scrollTop(0);
            SendPDFtoSocialShare($(xelem), "Summary Dashboard", "Dashboard", "w");
        }
        $scope.sendEmail = function() {
            ...
            $('ion-nav-view').css("height", $(xelem).height() + 300);
            $('body').css("overflow", 'visible');
            SendPDFtoSocialShare($(xelem), "Summary Dashboard", "Dashboard", "e");
        }
    }])
    .controller('homeCtrl', ['$rootScope', '$scope', '$state', '$ionicPopup', 'fmcgAPIservice', 'fmcgLocalStorage', 'notification', '$ionicLoading', '$ionicModal', '$ionicScrollDelegate', function($rootScope, $scope, $state, $ionicPopup, fmcgAPIservice, fmcgLocalStorage, notification, $ionicLoading, $ionicModal, $ionicScrollDelegate) {
        ...
        $scope.gotoCust = function(cus) {
            ...
            if ($scope.Mypln.worktype.FWFlg == 'F') {
        ...
        $scope.goToINHfeedbk = function() {
            if ($scope.cComputer == false) {
                ...
                if (_currentLocation.Latitude != undefined) {
                    $scope.INHFeed.location = _currentLocation.Latitude + ":" + _currentLocation.Longitude;
                }
            }
            $state.go('fmcgmenu.INHfeedbk');
        }
        $scope.goToCustReview = function() {
            $state.go('fmcgmenu.custReview');
        }
        $scope.goToCustReview = function() {
            $state.go('fmcgmenu.custReview');
        }
        ...
        if ($scope.Myplns.length > 0) {
            ...
            $scope.ViewDetail = function(Acd, SlTyp, Adt, type) {
                ...
                if ($scope.modal.type == 0)
                    $scope.modal.rptTitle = ((SlTyp == 1) ? $scope.EDrCap : (SlTyp == 2) ? $scope.EChmCap : (SlTyp == 3) ? $scope.EStkCap : (SlTyp == 4) ? $scope.ENLCap : (SlTyp == 5) ? 'In-Shop Sess ' : '') + ' Visit Details For : ' + Adt;
                ...
                if (SlTyp == 5) $scope.modal.rptTitle = 'In-Shop Sess Visit Details For : ' + Adt;
                $scope.modal.show();
                ...
                $scope.modal.sendWhatsApp = function() {
                    ...
                    $ionicScrollDelegate.scrollTop();
                    SendPDFtoSocialShare($(xelem), $scope.modal.rptTitle, "DayReport", "w");
                }
                $scope.modal.sendEmail = function() {
                    xelem = $(event.target).closest('.scroll').find('#vstpreview');
                    $ionicScrollDelegate.scrollTop();
                    SendPDFtoSocialShare($(xelem), $scope.modal.rptTitle, "DayReport", "e");
                }
    ...
    .controller('manageCtrl', ['$rootScope', '$scope', '$state', '$ionicLoading', 'fmcgAPIservice', 'fmcgLocalStorage', 'notification', function($rootScope, $scope, $state, $ionicLoading, fmcgAPIservice, fmcgLocalStorage, notification) {
        ...
        if ($scope.StkNeed != 0) {
            $scope.customers.push({
                'id': '5',
                'name': $scope.OUTletcount,
                'url': 'outleteditreview'
            });
            $scope.oCI = al;
            al++;
        }
        ...
        fmcgAPIservice.getDataList('POST', 'entry/count', []).success(function(response) {
            if (response['success']) {
                ...
                if ($scope.StkNeed != 0)
                    $scope.customers[$scope.oCI].count = response['data'][7]['outlet_review_Count'];
    ...
    .controller('inhfeedbkCtrl', function($rootScope, $scope, $location, $ionicPopup, $stateParams, $state, fmcgAPIservice, fmcgLocalStorage, generateUID, notification) {

        $scope.$parent.fmcgData.currentLocation = "fmcgmenu.INHfeedbk";
        $scope.$parent.navTitle = "In-Shop Feedback";
        $scope.LstINH = fmcgLocalStorage.getData("INHFeeds") || [];

        if ($scope.LstINH.length > 0)
            $scope.INHFeed.SessID = $scope.LstINH[$scope.LstINH.length - 1].SessID + 1
        else
            $scope.INHFeed.SessID = 1;

        if ($scope.cComputer) {
            tDate = new Date();
            $scope.INHFeed.eDtTM = tDate.getFullYear() + "-" + (tDate.getMonth() + 1) + "-" + tDate.getDate() + " " + tDate.getHours() + ":" + tDate.getMinutes() + ":" + tDate.getSeconds();
            $scope.INHFeed.eDt = tDate.getFullYear() + "-" + (tDate.getMonth() + 1) + "-" + tDate.getDate() + " 00:00:00.000"; // + endate.getHours() + ":" + endate.getMinutes() + ":" + endate.getSeconds();
        } else {
            window.sangps.getDateTime(function(tDate) {
                $scope.INHFeed.eDtTM = tDate.getFullYear() + "-" + (tDate.getMonth() + 1) + "-" + tDate.getDate() + " " + tDate.getHours() + ":" + tDate.getMinutes() + ":" + tDate.getSeconds();
                $scope.INHFeed.eDt = tDate.getFullYear() + "-" + (tDate.getMonth() + 1) + "-" + tDate.getDate() + " 00:00:00.000"; // + endate.getHours() + ":" + endate.getMinutes() + ":" + endate.getSeconds();
        ...
        if ($scope.Myplns[0].FWFlg == 'IH') {
            ...
            $scope.loadDatas(false, $scope.cDataID);
            $scope.INHFeed.dataSF = $scope.cDataID.replace(/_/g, '');
            $scope.INHFeed.Stkid = $scope.Myplns[0].stockistid;
            $scope.INHFeed.RouteID = $scope.Myplns[0].clusterid;
            $scope.INHFeed.RouteNm = $scope.Myplns[0].ClstrName

            if ($scope.Myplns[0].custid == undefined) {
                $scope.Myplns[0].custid = 'C-1';
            }
            $scope.INHFeed.DrCode = $scope.Myplns[0].custid;
            $scope.INHFeed.DrName = $scope.Myplns[0].custName;
            $scope.INHFeed.DrAddress = $scope.Myplns[0].address;
        }
        $scope.ClearFld = function() {
            $scope.INHFeed.CusMeet = "";
            $scope.INHFeed.IntCus = "";
            $scope.INHFeed.PCus = "";
            $scope.INHFeed.SalVal = "";
            $scope.INHFeed.remarks = "";
        }
        $scope.ClearFld();
        $scope.saveINHFeed = function() {

            if ($scope.INHFeed.CusMeet == undefined || $scope.INHFeed.CusMeet == "") {
                Toast("Enter the Total Customer Met");
                return false;
            }
            fmcgAPIservice.addMAData('POST', 'dcr/save', "30", $scope.INHFeed).success(function(response) {
                if (response.success)
                    Toast("This Session Feedback Entry Submited Successfully");
                $scope.LstINH.push($scope.INHFeed);
                fmcgLocalStorage.createData("INHFeeds", $scope.LstINH);
                $scope.INHFeed = {};
                $state.go('fmcgmenu.home');
            }).error(function() {
                Toast("No Internet Connection!");
    ...
    .controller('screen1Ctrl', function($rootScope, $scope, $location, $ionicPopup, $stateParams, $state, fmcgAPIservice, fmcgLocalStorage, generateUID, notification) {
        ...
        $scope.goNext = function() {
            ...
            if ($scope.$parent.fmcgData.worktype.selected.FWFlg == "F" || $scope.$parent.fmcgData.worktype.selected.FWFlg == "DH") {
                ...
                if (!$scope.fmcgData.stockist && proceed && $scope.$parent.fmcgData.worktype.selected.FWFlg != "DH") {
    ...
    .controller('screen3Ctrl', function($rootScope, $scope, $filter, $state, $ionicModal, $ionicSideMenuDelegate, fmcgAPIservice, fmcgLocalStorage, notification, $ionicLoading) {
                ...
                if ($scope.$parent.fmcgData.productSelectedList.length > 0) {
                    ...
                    for (i = 0; i < $scope.products.length; i++) {
                        ...
                        if (val == 0) {
                            ...
                            productData.pack_qty = $scope.products[i].pack_qty || 0;
        ...
        if (scrTyp == 1 || scrTyp == 4) {
            ...
            if ($scope.RateEdit == 0) {
                oRateEdit = true;
                edTmpl = "partials/productCellTemplate.html"; //
            } else {
                oRateEdit = false;
                edTmpl = 'partials/productCellTemplate.html';
            }
            $scope.packOrder = false;
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [{
                    ...
                    // { field: 'Rate', displayName: 'Rate', enableCellEdit: oRateEdit, cellTemplate: edTmpl, width: 60, cellClass: 'grid-alignr' },
                    {
                        field: 'Rate',
                        displayName: 'Rate',
                        enableCellEdit: oRateEdit,
                        width: 60,
                    ...
                    },
                    {
                        field: 'pack_qty',
                        displayName: 'O.Boxs',
                        enableCellEdit: "partials/cellEditTemplate.html",
                        width: 60,
                        "visible": $scope.packOrder,
                        cellClass: 'grid-alignr'
                    },
                    {
                        field: 'rx_qty',
    ...
    .controller('dayFeedbkCtrl', ['$rootScope', '$scope', '$state', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', '$ionicLoading', '$ionicModal', '$filter', function($rootScope, $scope, $state, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification, $ionicLoading, $ionicModal, $filter) {
        $scope.$parent.navTitle = "Day Feedback Form";

        $scope.GetFeedBkDBList = function() {
            var endate = new Date($scope.dyFeedbk.eDate);
            var dateStr = endate.getFullYear() + "-" + (endate.getMonth() + 1) + "-" + endate.getDate() + " 00:00:00.000"; // + endate.getHours() + ":" + endate.getMinutes() + ":" + endate.getSeconds();

            fmcgAPIservice.getDataList('POST', 'get/MyTbDB&Adt=' + dateStr, '')
                .success(function(response) {
                    $scope.dyFeedbk.DBLists = response || [];
                });
        }
        if ($scope.cComputer) {
            $scope.dyFeedbk.eDate = new Date();
            $scope.GetFeedBkDBList();
        } else {
            window.sangps.getDateTime(function(tDate) {
                $scope.dyFeedbk.eDate = tDate;
                $scope.GetFeedBkDBList();
            });
        }

        $scope.SaveCallReview = function() {
            /*if($scope.dyFeedbk.remarks==undefined || $scope.dyFeedbk.remarks==""){
                Toast("Enter the Remarks");
                return false;
            }*/

            $flg = 0;
            for ($il = 0; $il < $scope.dyFeedbk.DBLists.length; $il++) {
                tItem = $scope.dyFeedbk.DBLists[$il];
                if (tItem.DSRReVw > 0 || tItem.DBReVw > 0 || tItem.MKTReVw > 0) $flg = 1;
            }
            if ($flg == 0) {
                Toast("Your Feedback Not Given...");
                return false;
            }
            ...
            fmcgAPIservice.addMAData('POST', 'save/DyFeedBkrevw', "28", $scope.dyFeedbk).success(function(response) {
                Toast(response.Msg); //Call Review Submited Successfully
                $ionicLoading.hide();
                if (response.success) {
                    $state.go('fmcgmenu.home');
                }
    ...
    .controller('custReviewCtrl', ['$rootScope', '$scope', '$state', '$location', 'fmcgAPIservice', 'fmcgLocalStorage', '$ionicSideMenuDelegate', 'notification', '$ionicLoading', '$ionicModal', '$filter', function($rootScope, $scope, $state, $location, fmcgAPIservice, fmcgLocalStorage, $ionicSideMenuDelegate, notification, $ionicLoading, $ionicModal, $filter) {
        $scope.$parent.navTitle = "Outlet Review";
        if ($scope.view_MR == 1)
            $scope.cDataID = '_' + $scope.sfCode;
        else
            $scope.cDataID = '_' + $scope.Myplns[0].subordinateid; //alert($scope.cDataID);
        ...
        $scope.BrandList = fmcgLocalStorage.getData("category_master") || [];

        if ($scope.fmcgData.BrandList != undefined && $scope.fmcgData.BrandList.length > 0) {

            $scope.BrandList.shift();
            for ($il = 0; $il < $scope.fmcgData.BrandList.length; $il++) {
                tItem = $scope.fmcgData.BrandList[$il];
                if (tItem.Avail == "Y")
                    $scope.BrandList[$il].Avai = true;
                if (tItem.EC == "Y")
                    $scope.BrandList[$il].EC = true;
                if (tItem.Avail == "N")
                    $scope.BrandList[$il].Avai = false;
                if (tItem.EC == "N")
                    $scope.BrandList[$il].EC = false;
            }
        }
        ...
        $scope.SaveCallReview = function() {
            ...
            $scope.data.worktype = Wtyps[0]['id'];

            $scope.data.subordinate = $scope.fmcgData.subordinate.selected.id;
            $scope.data.stockist = $scope.fmcgData.stockist.selected.id;
            $scope.data.cluster = $scope.fmcgData.cluster.selected.id;

            var endate = new Date($scope.fmcgData.entryDate);
            var dateStr = endate.getFullYear() + "-" + (endate.getMonth() + 1) + "-" + endate.getDate() + " " + endate.getHours() + ":" + endate.getMinutes() + ":" + endate.getSeconds();
            $scope.data.entryDate = dateStr;

            if ($scope.fmcgData.doctor == undefined || $scope.fmcgData.doctor.selected == undefined) {
                Toast("Select the " + $scope.DrCap, true);
                return false;
            }
            $scope.data.doctorid = $scope.fmcgData.doctor.selected.id;
            $scope.data.photosList = $scope.fmcgData.photosList;
            $flg = 0;
            for ($il = 0; $il < $scope.BrandList.length; $il++) {
                tItem = $scope.BrandList[$il];
                if (tItem.Avai == undefined) $scope.BrandList[$il].Avai = false;
                if (tItem.EC == undefined) $scope.BrandList[$il].EC = false;
                if (tItem.Avai == true || tItem.EC == true) $flg = 1;
            }
            if ($scope.fmcgData.remarks != undefined && $scope.fmcgData.remarks != "") {
                $flg = 1;
            }
            if ($flg == 0) {
                Toast("Your Product Review Not Given...");
                return false;
            }
            $scope.data.BrandList = $scope.BrandList;

            /* if ($scope.fmcgData.remarks == undefined || $scope.fmcgData.remarks == "") {
                 Toast("Enter the Remarks");
                 return false;
             }*/
            $scope.data.remarks = $scope.fmcgData.remarks;
            ...
            $scope.SaveDataToLocalStorage = function(item) {

                var a = [];
                // Parse the serialized data back into an aray of objects
                a = JSON.parse(localStorage.getItem('Outlet'));
                ...
                a.push(item);
                if (a != undefined) {
                    localStorage.setItem('Outlet', JSON.stringify(a));
                    $state.go('fmcgmenu.home');
                }
            ...
            fmcgAPIservice.addMAData('POST', 'save/callreview', "29", $scope.data).success(function(response) {
                Toast(response.Msg); //Call Review Submited Successfully
                $ionicLoading.hide();

                $scope.$parent.fmcgData = [];
                if (response.success) {
                    $state.go('fmcgmenu.home');
                }
            }).error(function() {
                $ionicLoading.hide();
                // window.localStorage.setItem("Outlet", JSON.stringify(tm));
                //fmcgLocalStorage.createData("Outlet", tm);

                $scope.SaveDataToLocalStorage($scope.data);




                Toast("No Internet Connection! Outlet Review Saved in Localy");
            });
        }
    ...
    }).directive("entryDatePicker", [
        function() {
            return function(scope, element, attrs) {
                scope.$watch("stockUpdation.edate", (function(newValue, oldValue) {
                    scope.GetCurrentStock();
                    //scope.$apply();
                }), true)
                scope.$watch("stockUpdation.MinDate", (function(newValue, oldValue) {
                    element.datepicker('option', 'minDate', scope.stockUpdation.MinDate);
                }), true)
                scope.$watch("stockUpdation.dateRange", (function(newValue, oldValue) {
                    element.datepicker({
                        beforeShowDay: function(date) {
                            var dateString = jQuery.datepicker.formatDate('dd/mm/yy', date);
                            return [scope.stockUpdation.dateRange.indexOf(dateString) == -1];
                        }
                    })
                }), true)
                return element.datepicker({
                    ...
                    onSelect: function(date) {
                        scope.stockUpdation.edate = date;
                        scope.GetCurrentStock();
                        scope.$apply();
    ...
    .controller('orderEditCtrl', function($rootScope, $scope, $state, fmcgAPIservice, fmcgLocalStorage, $ionicLoading) {
        $scope.deleteOrder = function(item) {
            ...
            fmcgAPIservice.addMAData('POST', 'dcr/delorder', "24", item).success(function(response) {
                if (response.success)
                    Toast("Order Deleted Successfully");
            }).error(function() {
                Toast("No Internet Connection!.\n Try again later...");
            });
            window.location.href = "#/fmcg/manageDataView?myChoice=1";
        }
    ...
    .controller('clViewCtrl', function($rootScope, $scope, $state, $ionicPopup, $ionicLoading, fmcgAPIservice, fmcgLocalStorage, notification, $filter, $ionicScrollDelegate) {
        $scope.$parent.navTitle = "Closing Stocks View";
        $scope.ClDate = new Date().toISOString().slice(0, 10).replace(/-/g, '-');
        ...
        $scope.CLDet.vwCLList = [];
        $scope.CLDet.eFlag = 0;
        $scope.$on('getCLReport', function(evnt) {
            $scope.getCLView();
        });

        $scope.getCLView = function() {
            $scope.CLDet.eFlag = 0;
            fmcgAPIservice.getPostData('POST', 'get/clview&sSF=' + $scope.MnthSumm.subordinate.selected.id + '&Adt=' + $scope.ClDate, []).success(function(response) {
                $scope.CLDet.vwCLList = response;
                $scope.CLDet.eFlag = $scope.CLDet.vwCLList.length;
                $ionicLoading.hide();
            }).error(function() {
                Toast("No Internet Connection!");
                $ionicLoading.hide();
        ...
        $scope.getTotal = function(CQty, StkId) {
            var total = 0;
            var Wtyps = $scope.CLDet.vwCLList.filter(function(a) {
                return (a.StkId == StkId)
            });
        ...
        $scope.sendWhatsApp = function() {
            xelem = $(event.target).closest('.scroll').find('#vstpreview');
            $ionicScrollDelegate.scrollTop();
            SendPDFtoSocialShare($(xelem), 'Closing view For : ' + $scope.MnthSumm.subordinate.name + ' Date : ' + $scope.GetDate($scope.ClDate), "Closing view", "w");
        }
        $scope.sendEmail = function() {
            $scope.CLDet.eFlag = 1;
            xelem = $(event.target).closest('.scroll').find('#vstpreview');
            SendPDFtoSocialShare($(xelem), 'Closing view For : ' + $scope.MnthSumm.subordinate.name + ' Date : ' + $scope.GetDate($scope.ClDate), "Closing view", "e");
        }
    ...
    .controller('stockUpdationCtrl', function($rootScope, $scope, $state, $ionicPopup, $ionicLoading, fmcgAPIservice, fmcgLocalStorage, notification, $filter) {
        ...
        $scope.stockUpdation.dateRange = [];
        $scope.Last_Updation_Date = [];
        $scope.tdate = $filter('date')(new Date(), 'dd/MM/yyyy');
        tMindt = new Date().addDays(-2);
        ...
        function getStock() {
            if ($scope.view_STOCKIST == 1)
                ...
                if ($scope.prod == 0) $scope.$parent.fmcgData.productSelectedList = [];
            }
            if ($scope.LastUpdationDate != '' && $scope.LastUpdationDate != undefined) {
                spdt = $scope.LastUpdationDate.split("/");
                ldt = new Date(spdt[2] + '-' + spdt[1] + '-' + spdt[0] + ' 00:00:00.000');
                $scope.stockUpdation.MinDate = (ldt > tMindt) ? ldt : tMindt;
                if ($scope.stockUpdation.edate != undefined) {
                    spdt = $scope.stockUpdation.edate.split("/");
                    tedt = new Date(spdt[2] + '-' + spdt[1] + '-' + spdt[0] + ' 00:00:00.000');
                    if (ldt > tedt) {
                        $scope.stockUpdation.edate = $scope.LastUpdationDate;
                    }
                }
            } else {
                $scope.stockUpdation.MinDate = tMindt;
            }

            if ($scope.LastUpdationDate == $scope.stockUpdation.edate && $scope.stockUpdation.edate != undefined)
                $scope.editing = 0;
            else
                $scope.editing = 1;
        ...
        $scope.findTotalVal = function() {
            ...
            for (var key in productRate) {
                if (productRate[key]['Product_Detail_Code'] == product) {
                    distributerPrice = parseFloat(productRate[key]['DistCasePrice']);
                    if (isNaN(distributerPrice)) distributerPrice = 0;
                    distPPrice = parseFloat(productRate[key]['Distributor_Price']);
                    if (isNaN(distPPrice)) distPPrice = 0;
                }
            }
            ...
            var values = 0;
            for (var key in products) {
                val = parseFloat(products[key]['cb_qty']);
                if (isNaN(val)) val = 0;
                values = +val * distributerPrice;
                val = parseFloat(products[key]['pieces']);
                if (isNaN(val)) val = 0;
                values = +val * distPPrice;
            }
            $scope.fmcgData.value2 = 2
            $scope.fmcgData.value = Math.round(values * factor) / factor;
        };
            ...
            $scope.gridOptions = {
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        displayName: "Box",
                        enableCellEdit: true,
                        editableCellTemplate: "partials/cellEditTemplate6.html",
                        width: 55,
        ...
        $scope.save = function() {
            ...
            /*else
                $sCode = "0";*/
            console.log($scope.stockUpdation.edate)
            save = save + '&sCode=' + $sCode;
            save = save + '&eDt=' + $scope.stockUpdation.edate;
    ...
    .controller('manageDataViewCtrl', function($rootScope, $scope, $ionicLoading, $stateParams, $state, $ionicModal, $ionicPopup, $ionicScrollDelegate, fmcgAPIservice, fmcgLocalStorage, notification, $filter) {
        ...
        $scope.$parent.navTitle = "Submitted " + ((ch == 1) ? $scope.EDrCap : (ch == 2) ? $scope.EChmCap : (ch == 2) ? $scope.EStkCap : (ch == 2) ? $scope.ENLCap :   (ch == 5) ? $scope.OUTletcount : 'calls');
        $scope.clearData();
        $scope.modal = $ionicModal;
        ...
        $scope.loadEdDet = function(pklist, tempData) {
            switch ($scope.myChoice) {
                ...
                case "5":
                    $scope.customerDatas = fmcgLocalStorage.getData("chemist_master") || [];
                    fmcgAPIservice.getDataList('POST', 'table/list', ["getoutletallvalues", '["*"]', ]).success(function(response1) {
                        for (var j = 0, le = response1.length; j < le; j++) {
                            /* console.log(response1);*/



                            var prodNms = (response1[j]['BrndRevw'] + '$').split("$");

                            leg = prodNms.length;
                            for (var m = 0; m < leg; m++) {
                                if (prodNms[m] != "") {
                                    arr = prodNms[m].split('#');
                                    response1[j]['BrandReview'] = response1[j]['BrandReview'] || [];
                                    var pTemp = {};
                                    pTemp.product = arr[0].toString().trim();
                                    pTemp.product_Nm = arr[1].toString();
                                    pTemp.Avail = (arr[2] == "1") ? 'Y' : 'N';
                                    pTemp.EC = (arr[3] == "1") ? 'Y' : 'N';
                                    //pTemp.ABGcolor = (arr[2] == "1") ? '#39ac08' : '#ff0000';
                                    //pTemp.EBGcolor = (arr[3] == "1") ? '#39ac08' : '#ff0000';
                                    /*pTemp.Acolor = (arr[3] == "1") ? '#39ac08' : '#ff0000';
                                    pTemp.Ecolor = (arr[3] == "1") ? '#39ac08' : '#ff0000';*/
                                    response1[j]['BrandReview'].push(pTemp);
                                }
                            }
                            ...
                            tData = {};
                            tData = angular.copy(tempData);
                            tData['amc'] = response1[j]['RetID'];
                            tData['TDSlNo'] = response1[j]['TDSlNo'];
                            tData['customer'] = {};
                            tData['customer']['selected'] = {}
                            tData['customer']['selected']['id'] = "5";
                            tData['remarks'] = response1[j]['Rmks'];
                            tData['RetID'] = response1[j]['RetID'];
                            tData['subordinate'] = {};
                            tData['subordinate']['selected'] = {};
                            tData.subordinate.selected.id = response1[j]['SFCode'];

                            tData['doctor'] = {};

                            tData['doctor']['selected'] = {}
                            tData['doctor']['selected']['id'] = response1[j]['ListedDrCode'];
                            tData['doctor']['name'] = response1[j]['ListedDr_Name'];

                            tData['BrandList'] = response1[j]['BrandReview'];
                            tData['entryDate'] = response1[j]['Adt']['date'];
        ...
        $scope.edit = function(item) {
            ...
            if (TwnDet != undefined && TwnDet.length > 0) {
                $scope.loadDatas(false, '_' + item.subordinate.selected.id);
            } else if (TwnDet != undefined && TwnDet.length <= 0) {
            ...
            $scope.fmcgData.editableOrder = 1;
            //fmcgLocalStorage.createData('draft', $scope.drafts);
            if (item.customer.selected.id != 5) {
        ...
        $scope.preView = function(item) {
            ...
            $scope.modal.sendWhatsApp = function() { /*******/
                ...
                $ionicScrollDelegate.scrollTop();
                SendPDFtoSocialShare($(xelem), "Order Details", "OrderDetails", "w");
    ...
    .controller('BrndSummaryValue', function($rootScope, $ionicLoading, $ionicScrollDelegate, $ionicModal, $scope, $state, fmcgAPIservice, fmcgLocalStorage, notification) {
        ...
        $scope.sendWhatsApp = function() {
            xelem = $(event.target).closest('.scroll').find('#preview');
            $ionicScrollDelegate.scrollTop();
            SendPDFtoSocialShare($(xelem), 'Summary For : ' + userData.sfName + ' Date : ' + $scope.GetDate($scope.BrndDt), "BrandSummary", "w");
            /*$('.scroll').css('transform', 'translate3d(0px, 0px, 0px) scale(1)');
            $('ion-nav-view').css("height", $(xelem).height() + 300);
            $('body').css("overflow", 'visible');
            html2canvas($(xelem), {
        ...
        $scope.sendEmail = function() {
            xelem = $(event.target).closest('.scroll').find('#preview');
            SendPDFtoSocialShare($(xelem), 'Summary For : ' + userData.sfName + ' Date : ' + $scope.GetDate($scope.BrndDt), "BrandSummary", "e");
            /*$('.scroll').css('transform', 'translate3d(0px, 0px, 0px) scale(1)');
            $('ion-nav-view').css("height", $(xelem).height() + 300);
            $('body').css("overflow", 'visible');
            html2canvas($(xelem), {
    ...
    .controller('BrndSummaryLitter', function($rootScope, $ionicLoading, $ionicScrollDelegate, $ionicModal, $scope, $state, fmcgAPIservice, fmcgLocalStorage, notification) {
        ...
        $scope.$parent.navTitle = "Brandwise Liters Summary";
        $scope.clearData();
        $scope.BrndDt = new Date().toISOString().slice(0, 10).replace(/-/g, '-');
        var temp = window.localStorage.getItem("loginInfo");
        var userData = (temp != null && temp.length > 0) ? JSON.parse(temp) : null;
    ...
    .controller('mnthSummary', function($rootScope, $ionicLoading, $ionicScrollDelegate, $ionicModal, $scope, $state, fmcgAPIservice, fmcgLocalStorage, notification) {
        ...
        $scope.ViewDetail = function(Acd, SlTyp, Adt, type) {
            ...
            $scope.modal.sendWhatsApp = function() {
                xelem = $(event.target).closest('.scroll').find('#vstpreview');
                $ionicScrollDelegate.scrollTop();
                SendPDFtoSocialShare($(xelem), "Monthly Summary", "MonthlySummary", "w");
                /* $(event.target).closest('.modal').css("height", $(xelem).height() + 300);
                 $(event.target).closest('.ion-content').css("height", $(xelem).height() + 300);
                 $(event.target).closest('.scroll').css("height", $(xelem).height() + 300);
                 $(event.target).closest('.scroll').css('transform', 'translate3d(0px, 0px, 0px) scale(1)');
            ...
            $scope.modal.sendEmail = function() {
                xelem = $(event.target).closest('.scroll').find('#vstpreview');
                $ionicScrollDelegate.scrollTop();
                SendPDFtoSocialShare($(xelem), "Monthly Summary", "MonthlySummary", "e");
                /*$(event.target).closest('.modal').css("height", $(xelem).height() + 300);
                $(event.target).closest('.ion-content').css("height", $(xelem).height() + 300);
                $(event.target).closest('.scroll').css("height", $(xelem).height() + 300);
                $(event.target).closest('.scroll').css('transform', 'translate3d(0px, 0px, 0px) scale(1)');
        ...
        $scope.getMonthReports = function() {
            ...
            fmcgAPIservice.getDataList('POST', 'get/MnthSumm&rptSF=' + $scope.MnthSumm.subordinate.selected.id + '&rptDt=' + $scope.mnthDt, [])
                .success(function(response) {
                    $scope.MnthDCRList = response['dayrep'];
                    ...
                    $scope.brndwise = groupBy(brndwise, "Dt");
                    DCR_TLSD = response['DCR_TLSD'];
                    DCR_TLSD = groupBy(DCR_TLSD, "Dt");
    ...
    .controller('dayReport', function($rootScope, $ionicLoading, $filter, $ionicModal, $ionicScrollDelegate, $scope, $state, fmcgAPIservice, fmcgLocalStorage, notification) {
        ...
        $scope.ViewDetail = function(Acd, SlTyp, Adt, type, sfname) {
            $scope.modal.type = type;
            $scope.modal.SlTyp = SlTyp;
            $scope.modal.rptTitle = ((SlTyp == 1) ? $scope.EDrCap : (SlTyp == 2) ? $scope.EChmCap : (SlTyp == 3) ? $scope.EStkCap : (SlTyp == 4) ? $scope.ENLCap : '') + ' Visit Details For : ' + Adt;
            if ($scope.modal.type == 0)
                ...
                .success(function(response) {
                    if (SlTyp != 5) {
                        for (var j = 0, le = response.length; j < le; j++) {
                            var prods = (response[j]['Product_Code'] + ((response[j]['Product_Code'] == '') ? '' : '#') + response[j]['Additional_Prod_Code']).split("#");
                            //var prodNms = (response[j]['Product_Detail'] + ((response[j]['Product_Detail'] == '') ? '' : '#') + response[j]['Additional_Prod_Dtls']).split("#");
                            var prodNms = (response[j]['Product_Detail'] + '$').split("$");
                            leg = prodNms.length;
                            for (var m = 0; m < leg; m++) {
                                if (prodNms[m] != "") {
                                    arr = prodNms[m].split('#');
                                    response[j]['BrandReview'] = response[j]['BrandReview'] || [];
                                    var pTemp = {};
                                    pTemp.product = arr[0].toString().trim();
                                    pTemp.product_Nm = arr[1].toString();
                                    pTemp.Avail = (arr[2] == "1") ? 'Y' : 'N';
                                    ...
                                    /*pTemp.Acolor = (arr[3] == "1") ? '#39ac08' : '#ff0000';
                                    pTemp.Ecolor = (arr[3] == "1") ? '#39ac08' : '#ff0000';*/
                                    response[j]['BrandReview'].push(pTemp);
                                }
                            }
                            var prodNms = (response[j]['Product_Detail'] + ((response[j]['Product_Detail'] == '') ? '' : '#') + response[j]['Additional_Prod_Dtls']).split("#");
                                ...
                                if (prods[m].length > 0) {
                                    ...
                                    if (prod[1] != undefined) {
                                        if (prod[1].indexOf('$') > 0) {
                                            ...
                                            pTemp.rx_qty = prodQ[0];
                                            pTemp.product_netwt = prodQ[1];
                                            pTemp.total_values_of_product = pTemp.sample_qty / pTemp.rx_qty;
                                        ...
                                        if (idex > -1) {
                                            Prw = $scope.modal.vwProductSummry[idex];
                                            Prw.Qty = Prw.Qty + qt;
                                            Prw.TOTAL = Prw.Qty * pTemp.total_values_of_product;
                                            Prw.TOTAL = Prw.TOTAL.toFixed(2);
                                            ...
                                            pSumm.PName = prodNm[0].toString();
                                            pSumm.Qty = qt;
                                            pSumm.TOTAL = qt * pTemp.total_values_of_product;
            ...
            $scope.modal.getTotal = function() {
                var total = 0;
                var Wtyps = $scope.modal.vwProductSummry;
            ...
            $scope.modal.getqtyTotal = function() {
                var total = 0;
                var Wtyps = $scope.modal.vwProductSummry;


                angular.forEach(Wtyps, function(el) {
                    total = total + parseInt(el['Qty']);
                });
                return total;
        ...
        $scope.getDayReports = function() {
            ...
            if ($scope.MnthSumm == undefined) $scope.MnthSumm = {};
            if ($scope.MnthSumm.subordinate == undefined || $scope.MnthSumm.subordinate.selected == undefined || $scope.MnthSumm.subordinate.selected.id == undefined) {
                $scope.MnthSumm.subordinate = {};
                $scope.MnthSumm.subordinate.selected = {};
                $scope.MnthSumm.subordinate.selected.id = $scope.sfCode;
            }
            fmcgAPIservice.getDataList('POST', 'get/DayReport&rptDt=' + $scope.dyRptDt + '&rpSF=' + $scope.MnthSumm.subordinate.selected.id, [])
                .success(function(response) {
