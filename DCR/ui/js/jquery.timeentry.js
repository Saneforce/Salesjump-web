...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).timeEntry('setTime', null) */
		...
		_showTime: function(inst) {
		    var currentTime = (inst.options.unlimitedHours ? this._formatNumber(inst._selectedHour) :
		...
		_setTime: function (inst, time) {
			...
			ttime = new Date(0, 0, 0, 23, 59, 59);
			mtime = new Date(0, 0, 0, 0, 0, 0);

			if (time > ttime) { fields = [0, 0, 0]; }
			if (time < mtime) { fields = [23, 59, 59]; }
