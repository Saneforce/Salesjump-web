...
function getSlabDetails() {
    ...
    $Owndiv = (string) $divs[0];
    $sfCode = $_GET['sfCode'];
	$query="select Sf_HQ from mas_salesforce   where Sf_Code='" . $sfCode . "' ";
	$Sf_Hq=performQuery($query);
	if(strpos($Sf_Hq[0]['Sf_HQ'],"DELHI")>0){
		 $query = "select  RetSlabID id,SlabName name, SlabName  ,month(From_dt)SlabMonth,year(From_dt)SlabYear,SlabMinVal,SlabMaxVal,CONVERT(varchar,To_Dt, 101)To_Date  from Mas_Retail_Business_Slab where ActiveFlag='0' and  Division_Code='" . $Owndiv . "' and isnull(Claim_deadline,convert(date,getdate()))>=convert(date,getdate()) and RetSlabID not  in (89,90,91,92,93)";
	}else{
		$query = "select  RetSlabID id,SlabName name, SlabName  ,month(From_dt)SlabMonth,year(From_dt)SlabYear,SlabMinVal,SlabMaxVal,CONVERT(varchar,To_Dt, 101)To_Date  from Mas_Retail_Business_Slab where ActiveFlag='0' and  Division_Code='" . $Owndiv . "' and isnull(Claim_deadline,convert(date,getdate()))>=convert(date,getdate())";
...
function DateofExpense(){
	...
	if($Type=='1'){
  $query = "select case when   (WorkType='Meeting' or  WorkType='Admin Work') and ClstrName=''  and  Division_Code='109' then 'HQ' ELSE Allowance_Type end Allowance_Type,ClstrName,WorkType  from (select top 1 isnull(ClstrName,'') ClstrName ,Division_Code, (select  isnull(Allowance_Type,'HQ') from Mas_Territory_Creation where   CONVERT(VARCHAR(50),Territory_Code)=isnull(TB.cluster,'')) Allowance_Type , (select Worktype_Name_B from   Mas_WorkType_BaseLevel where WorkType_Code_B=TB.wtype ) WorkType from TbMyDayPlan    TB  where TB.sf_code='" . $sfCode . "' and convert(date,TB.Pln_Date)='" . $eDt . "' order by TB.Pln_Date ASC ) A ";
 }else{
   $query = "select case when   (WorkType='Meeting' or  WorkType='Admin Work') and ClstrName='' and  Division_Code='109'  then 'HQ' ELSE Allowance_Type end Allowance_Type,ClstrName,WorkType  from (select top 1 ClstrName,Division_Code,  isnull((select  Allowance_Type from Mas_Territory_Creation where CAST(Territory_Code AS varchar(50))=isnull(TB.cluster,'')),'HQ') Allowance_Type,(select Worktype_Name_M from   Mas_WorkType_Mgr where WorkType_Code_M=TB.wtype ) WorkType from TbMyDayPlan  TB  where TB.sf_code='" . $sfCode . "' and convert(date,TB.Pln_Date)='" . $eDt . "' order by TB.Pln_Date ASC) A ";
 }
    return performQuery($query);
}
...
performQuery($missedquery);
         ...
         if( $vals['profilepic']!=null && $vals['profilepic']!="undefined")
			...
			if($vals["WType_SName"]==null)
                    $WType_SName="";
              else
				$WType_SName=$vals["WType_SName"];
			
			$sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works,Appversion,Place_Inv,WType_SName) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."','".$Appversion."' , '".$Place_Inv."','".$WType_SName."'";  
			performQuery($sql);
...
if($vals['Retailerphoto']!=null && $vals['Retailerphoto']!="undefined")
         ...
         if($flagofretailerND==1){
          $ListedDr_Active_Flag=3;
            ...
            '" .str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','','" . $visitpath . "',".$Lat.",".$Long.",'" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_phone3']) . "' ,'" . str_replace("'","",$vals['unlisted_doctor_email']) . "'  )}";
           performQueryWP($sql,[]);
...
$val =$data[3][$temppp[0]];
 ...
 if( $data[2]['ActivityCaptures']!=null && $data[2]['ActivityCaptures']!="undefined")
           ...
           $sql ="exec Post_Expense '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$val["MOT"]."'";
           $result=performQuery($sql); 
                    ...
                    if (isset($tableData['Activity_Stk_POB_Report'])){
                        ...
                        for ($j = 0; $j < count($samp); $j++) {
						 ...
						 if($Pfree==null)
                            ...
                            $PPXML = $PPXML . "<Prod PCode=\"".$samp[$j]["product_code"]."\" PName=\"".$samp[$j]["product_Name"]."\" CQty=\"".$samp[$j]["Qty"]."\" PQty=\"".$samp[$j]["PQty"]."\" CL=\"".$cl_qty_stock."\"  Free=\"".$Free_qty."\"  discount_price=\"".$discount_price."\"  discount=\"".$discount."\"   Fname=\"".$Fname."\"  FreeP_Code=\"".$FreeP_Code."\"  Pfree=\"".$Pfree."\" />";
...
case "get/RetailerPaymentCollection":
	...
	$SF_code = $_GET['sfCode'];
	$query = "select  ListedDr_Name,Territory_Name from Mas_ListedDr  DR inner join Mas_Territory_Creation TC on TC.Territory_Code=DR.Territory_Code where   charindex(',$SF_code,',','+Dr.Sf_Code+',')>0  and ListedDr_Active_Flag=1";
	outputJSON(performQuery($query));
	break;
