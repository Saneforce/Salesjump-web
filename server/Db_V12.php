...
$sql = "insert into Trans_Compititor(Trans_Order_No,Trans_Sl_No,Sf_Code,Product_Code,Name,Cat_Id,Qty,Rate,Order_Date,Division_Code,Brand_Code) select '" .$tcpk1 . "','" . $pk . "','" . $sfCode . "','" . $Compititor_Product[$i]['Pid'] . "','" . $Compititor_Product[$i]['Name'] . "','" . $Compititor_Product[$i]['Cat_Id'] . "','" . $Compititor_Product[$i]['Rate'] . "','" . $Compititor_Product[$i]['Qty'] . "',$orderDate,'$Owndiv','" . $Compititor_Product[$i]['bid'] . "' ";
                            performQuery($sql);
		...
		$sql = "select product_netwt from Mas_Product_Detail where Product_Detail_Code='" . $transOrderDetails[$i]['product_code'] . "'";
 		...
 		$net_weight=$Nwt[0]['product_netwt'];
			...
			//$productremarks=($transOrderDetails[$i]['rx_remarks']==null? 0:$transOrderDetails[$i]['rx_remarks']);
                        ...
                        if($qty>0 || $free>0   || $cbQty>0 || $Promo>0  || $productremarks!==$remarkspd){
				...
				$Fname = $transOrderDetails[$i]['Fname'];
                          ...
                          if($productremarks!==$remarkspd){
							     //$productremarks_IdArrays =explode (",", "$productremarks_Id");
							   //$productremarksArrays = explode(",", "$productremarks");
     ...
     case "dcr/updateProducts":
       ...
       performQuery($sql);
        for ($i = 0; $i < count($transOrderDetails); $i++) {
            ...
            $sql = "insert into Trans_Order_Details( Trans_Order_No, Trans_Sl_No,Product_Code,Product_Name,Quantity,value,Rate,net_weight,free,discount,discount_price,rateMode,DistDisc,DistPicRate,MfgDt,ClStock,PromoVal,Div_ID,Offer_ProductNm,Offer_ProductCd) select '" . $pk1 . "','" . $pk . "','" . $transOrderDetails[$i]['product'] . "','" . $transOrderDetails[$i]['product_Nm'] . "', $qty,$value,$rate,$net_weight,'$free','$discount','$discountprice','$rateMode',$Disc,$DistRate,'" . $transOrderDetails[$i]['Mfg_Date'] . "','" . $cbQty . "','0','". $Div_ID."','0','0'";
            performQuery($sql);
        }
...
case "get/vwVstPendingPragnancy":
    ...
    $div = $_GET['divisionCode'];
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
        $query = "select top 1 Pln_Date,sf_code,cluster,(select count(ListedDrCode) from Mas_ListedDr MLDR where  charindex(','+ TP.cluster+',',','+ cast(MLDR.Territory_Code as varchar)+',')>0  and ListedDr_Active_Flag=0 and  division_code='".$Owndiv."' and charindex(','+  TP.sf_code+',',','+ sf_code+',')>0 group by Territory_Code) cnt  from TbMyDayPlan TP  where division_code='".$Owndiv."' and sf_code='".$sfCode."' and cast(convert(varchar,Pln_Date,101)as datetime)='".$date."'    order by Pln_Date desc";
