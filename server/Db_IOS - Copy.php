...
$sql = "SELECT ListedDrCode from Mas_ListedDr where Ukey=".$Field["Ukey"];
 ...
 performQuery($sql);
	...
	case "Activity_Report_APP":
		...
		{
			$aKeys = array_keys($data[$i]);
			$tables[$aKeys[0]] = $data[$i][$aKeys[0]];
		}
		$PPXML = '';$pProd = '';$npProd = '';$pGCd = '';$pGNm = '';$pGQty = '';$SPProds = '';$nSPProds = '';$Inps = '';$nInps = '';$vTyp = 0;$DemoTo='';
		
		if(isset($tables['Activity_Doctor_Report'])){
			$vTyp = 1;
			$DetTB = $tables['Activity_Doctor_Report'];
			
			$routewise="'".$DetTB['routeWise']."'";
			$docor_code=str_replace("'", "", $DetTB['doctor_code']);
			...
			if($Desig_Check[0]['sf_Designation_Short_Name']=='AIT') $AITEST=9;
		...
		for ($il = 0;$il < count($tables['PENDING_Bills']);$il++) {
			$Pendingbills = $tables['PENDING_Bills'][$il];
