...
worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works,Appversion,Place_Inv) 
					select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."','".$Appversion."' , '".$Place_Inv."'";
			performQuery($sql);
...
where sfcode= '" . $sfCode . "' and convert(date,EntryDt)='" . $today . "' and Prod_Code='" . $transOrderDetails[$i]['product_code'] . "' ";
		...
		$payType=$data[0]['Activity_Report_APP']['payment_mode_name'];
            $resp["Qry5"]=$PAYAmount;
		if($PAYAmount!="") {
			$sql = "exec saveAppVanPayment '" . $sfCode . "',''," . $custCode . ",$PAYAmount,$payType,'$dcr_activity_date','','$Owndiv','" . $pk . "'";
			$resp["Qry3"]=$sql;
			$res=performQuery($sql);
		}
}
                ...
                $productCode = $results[$i]['Product_Detail_Code'];
                    ...
                    for($j=0; $j<count($retList); $j++)
                    {
                        ...
                        if($priceListCode!=null && $priceListCode!=''){
                            $retRateQuery = "select pwd.Rate_in_piece,pwd.MRP_Price, pwd.Product_Code from Mas_Product_Wise_Bulk_rate_head pwh inner join Mas_Product_Wise_Bulk_rate_details pwd on pwh.Price_list_Sl_No = pwd.Price_list_Sl_No where pwd.Product_Code ='". $productCode ."' and pwh.Price_list_Sl_No ='". $priceListCode ."' and ('$date' >= Effective_From_Date and '$date'<=Effective_To_Date)";
...
RetailCasePrice, MRP_Price,VanSale_Price, Product_Detail_Code from vwProductStateRates where Division_code='" . $Owndiv . "' and State_Code=$State_Code";
                    ...
                    for($j=0; $j<count($distList); $j++)
                    {
                        ...
                        if($priceListCode!=null && $priceListCode!=''){
                            $distRateQuery = "select pwd.dis_rate_in_piece,pwd.Rate_in_piece,pwd.MRP_Price,pwd.Product_Code,pwd.Product_Name from Mas_Product_Wise_Bulk_rate_head pwh inner join
                                              ...
                                              where pwd.Product_Code ='". $productCode ."' and pwh.Price_list_Sl_No ='". $priceListCode ."' and ('$date' >= Effective_From_Date and '$date'<=Effective_To_Date)";
	...
	case "rejectExpense":
		...
		$query = "exec RejectExpense_Native '$sf_code','$month','$year','$date','$rej_type','$emp_id','$rej_remark','$PeriodID'";
		performQuery($query);
		$result['query'] = $query;
		$result['success'] = true;
		outputJSON($result);
	...
	case "get/CoverageDetails":
        ...
        where ms.Sf_Code in ($sfcode) and iif(mld.ListedDr_Active_Flag = 0,'$toDate',convert(date,ListedDr_Deactivate_Date)) >= '$toDate'";
        ...
        where ms.Sf_Code in ($sfcode) and iif(mt.Territory_Active_Flag = 0,'$toDate',convert(date,Territory_Deactive_Date)) >= '$toDate'";
