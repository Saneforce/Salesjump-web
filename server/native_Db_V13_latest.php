 ...
 case "LeaveFormValidate":
	...
	case "DCRMissedDates":
        ...
        case "LeaveForm":
            ...
            $tRw = performQuery($sql);
            $pk = (int) $tRw[0]['RwID'];
			$sql = "insert into Mas_Leave_Form(Leave_Id,Leave_Type,From_Date,To_Date,Reason,sf_code,Division_Code,Leave_Active_Flag,Created_Date,No_of_Days,Address,First_or_Second) select '$pk'," . $vals['Leave_Type'] . ",'" . str_replace("'", "",$vals['From_Date']) . "'," . $vals['To_Date'] . "," . $vals['Reason'] . ",'$sfCode','$Owndiv','".$lvappp."','" . date('Y-m-d') . "'," . $vals['No_of_Days'] . "," . $vals['address'] . ",".$halfday."";
            performQuery($sql);
			$resp["qry"]=$sql;
            if($leaveType==1){
                $sql = "SELECT sf_type FROM Mas_Salesforce where Sf_Code='$sfCode'";
