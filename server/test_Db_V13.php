            ...
            // if (isset($tableData['Activity_POB_Report'])) {
                    ...
                    if (isset($tableData['Activity_Stk_POB_Report'])){
                        ...
                        for ($j = 0; $j < count($samp); $j++) {
                            ...
                            $PPXML = $PPXML . "<Prod PCode=\"".$samp[$j]["product_code"]."\" PName=\"".$samp[$j]["product_Name"]."\" CQty=\"".$samp[$j]["Qty"]."\" PQty=\"".$samp[$j]["PQty"]."\" CL=\"".$cl_qty_stock."\"  Free=\"".$Free_qty."\"  discount_price=\"".$discount_price."\"  discount=\"".$discount."\"   Fname=\"".$Fname."\"  FreeP_Code=\"".$FreeP_Code."\"    tax=\"".$tax_price."\"  />";
...
case "get/vwVstPendingPragnancy":
    ...
    case "get/calls":
        ...
        $date = date('Y-m-d');
        $result = array();
        $query = "select convert(varchar,h.Activity_Date,103) Adate,cast(order_Value as decimal) orderValue,h.Trans_SlNo from vwActivity_Report h inner join vwActivity_MSL_Details d on h.Trans_SlNo=d.Trans_SlNo where h.Sf_Code='$sfCode' and h.Activity_Date='$date'";
