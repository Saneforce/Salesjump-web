...
break;
	...
	case "get/locations":
        ...
        if(count($Users_FieldWork)>0){
            $size = count($Users_FieldWork)-1;
            $result['size'] = $size;
            for($i=0;$i<count($Users_FieldWork);$i++){
                if($Users_FieldWork[$i]['FWFlg'] == 'F'){
                    if($size == $i){
            ...
            where ms.Sf_Code in ($all_sfcodes) and convert(date,tl.DtTm)='".$date."' group by tl.SF_code,ms.Sf_Name";
        }
...
$divisionCode=$_GET['divisionCode'];
	     ...
	     $query1 = "select  TOP 1 convert(varchar,cast(convert(varchar,Tour_Date,101)  as date),20) as date,Objective remarks,SF_Code,WorkType_Code_B worktype_code,Worktype_Name_B worktype_name,Tour_Schedule1 RouteCode,Territory_Code1 RouteRouteNameCode,Territory_Code1 RouteName,Worked_With_SF_Code Worked_with_Code,Worked_With_SF_Name Worked_with_Name,Confirmed,HQ_Code,HQ_Name,JointWork_Name,MONTH(Tour_Date) Tmonth,YEAR(Tour_Date) Tyear,Retailer_Code,Retailer_Name,isnull(Place_Inv,'')Place_Inv from Trans_TP
 where SF_Code='$sfCode' and cast(tour_date as date)=cast(getdate() as date) union 
select  TOP 1 convert(varchar,cast(convert(varchar,Tour_Date,101)  as date),20) as date,Objective remarks,SF_Code,WorkType_Code_B worktype_code,Worktype_Name_B worktype_name,Tour_Schedule1 RouteCode,Territory_Code1 RouteRouteNameCode,Territory_Code1 RouteName,Worked_With_SF_Code Worked_with_Code,Worked_With_SF_Name Worked_with_Name,Confirmed,HQ_Code,HQ_Name,JointWork_Name,MONTH(Tour_Date) Tmonth,YEAR(Tour_Date) Tyear,Retailer_Code,Retailer_Name,isnull(Place_Inv,'')Place_Inv from Trans_TP_One
 where SF_Code='$sfCode' and cast(tour_date as date)=cast(getdate() as date) and confirmed=1";
...
if($status[0]['Status']=="6"){
 ...
 $query = "select  TOP 1 *  from DCR_TPDev_Reason where SF_Code='$sfCode' and cast(activity_date as date)=cast(getdate() as date) order by activity_date desc";
         $dev= performQuery($query);
if(count($status)>0&&count($dev)==0&&$status[0]['Status']!="3"){
$status[0]['Status']=6;
}
		 $res=array();
				$res['status']=$status;
    ...
    case "table/list":
        ...
        switch (strtolower($data['tableName'])) {
			case "vwactivity_msl_details":
		     $sfCode = $_GET['Sf_Code'];
		     $query = $query = "select Order_Flag,Trans_Sl_No,Order_Value,Activity_Date,Trans_Detail_Name,j.Stockist_Name stockist_name from DCRMain_Trans h with(nolock) 
             inner join DCRDetail_Lst_Trans d on h.Trans_SlNo = d.Trans_SlNo 
