...
function getDistReport() {
    ...
    if ($filter['checkStock'] == 1)
        $sql = "select * from vwDistReport where cast(convert(varchar,Order_Date,101) as datetime)='$dyDt' and Dist_Name='$stockist'";
    if ($filter['checkStock'] != 1)
        $sql = "select * from vwDistReport where Sf_Code='$sfCode' and cast(convert(varchar,Order_Date,101) as datetime)='$dyDt' and Dist_Name='$stockist'";
    ...
    switch ($temp[0]) {
        case "stockUpdation":
            ...
            for ($i = 0; $i < count($vals); $i++) {
                ...
                if ($editable == 1) {
                    $updateDate = $vals[$i]['updateDate']['date'];
                ...
                $sql = "select Distributor_Price,Retailor_Price from vwProductStateRates where State_Code=$stateCode and Product_Detail_Code='" . $productCode . "'";
                $tr = performQuery($sql);
...
switch ($value[0]) {
    ...
    case "get/calls":
        ...
        $query = "select sum(POB_Value) orderValue,count(h.Trans_SlNo) Trans_SlNo from vwActivity_Report h inner join vwActivity_MSL_Details d on h.Trans_SlNo=d.Trans_SlNo where h.Sf_Code='$sfCode' and MONTH(Activity_Date) = MONTH(getDate()) and YEAR(Activity_Date) = YEAR(getDate())";
        $month = performQuery($query);
        $query = "select count(h.Trans_SlNo) Trans_SlNo from vwActivity_Report h inner join vwActivity_MSL_Details d on h.Trans_SlNo=d.Trans_SlNo where h.Sf_Code='$sfCode' and MONTH(Activity_Date) = MONTH(getDate()) and YEAR(Activity_Date) = YEAR(getDate()) and POB_Value>0";
