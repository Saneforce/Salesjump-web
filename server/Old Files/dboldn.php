    ...
    switch ($temp[0]) {
        ...
        case "stockUpdation":
            ...
            for ($i = 0; $i < count($vals); $i++) {
                ...
                $sampleErpCode = !empty($vals[$i]['conversionQty']) ? $vals[$i]['conversionQty'] : 0;
...
//                $sampleErpCode = $sampleCode[0]['Sample_Erp_Code'];
                ...
                if ($pieces > 0) {
                    ...
                    $salepieces = $sampleErpCode - $pieces;
                } else {
                    $salepieces = 0;
                    $saleQty = ($recvQty + $Op_Qty) - ($cbQty);
                }
                $saleQty = abs($saleQty);
                $sql = "insert into Trans_Secondary_Sales_Details(Sale_Code,Stockist_Code,Product_Code,Product_Name,Rec_Qty,Cl_Qty,pieces,Distributer_Rate,Retailor_Rate,Op_Qty,Sale_Qty,date,Conversion_Qty,sale_pieces,SfCode) select $pk,'" . $sfCode . "','" . $productCode . "','" . $productName . "',$recvQty,$cbQty,$pieces,$distPrice,$retPrice,$Op_Qty,$saleQty,'" . $date . "',$sampleErpCode,$salepieces,'$staffCode'";
                performQuery($sql);
            }
            $resp["success"] = true;
            echo json_encode($resp);
...
//            $market = $data[0]['Tour_Plan']['market'];
            ...
            $HQ_Name = $data[0]['Tour_Plan']['HQ_Name'];
...
switch ($value[0]) {
    ...
    case "vwChkTransApprovalOne":
        ...
        $month = $_GET['month'];
        $year = $_GET['year'];
        $sql = "select Objective,Worked_With_SF_Name,Worktype_Name_B,Territory_Code1,convert(varchar,Tour_Date,103) Tour_Date from Trans_TP_One where Sf_Code='$sfCode'";
        $tp = performQuery($sql);
        outputJSON($tp);
        break;
    case "tpview":
