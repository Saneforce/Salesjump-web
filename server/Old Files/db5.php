...
function getPreCallDet() {
    ...
    if (count($as) > 0) {
        ...
        $query = "select Stockist_Code,Last_Order_Amt,Out_standing_Amt from Order_Collection_Details where Cust_Code=$MSL order by Stockist_Code";
    ...
    switch ($temp[0]) {
        case "stockUpdation":
            ...
            for ($i = 0; $i < count($vals); $i++) {
                ...
                $cbQty = $vals[$i]['cb_qty'];
                $pieces = $vals[$i]['pieces'];
                $recvQty = $vals[$i]['recv_qty'];

				$sql = "select DistCasePrice,RetailCasePrice from vwProductStateRates where State_Code=$stateCode and Product_Detail_Code='" . $productCode . "'";
	            $tr = performQuery($sql);
	            $distPrice = $tr[0]['DistCasePrice'];
	            $retPrice = $tr[0]['RetailCasePrice'];
            
                $sql = "insert into Trans_Stock_Updation_Details(Tran_Slno,Stockist_Code,Product_Code,Product_Name,Rec_Qty,Cb_Qty,pieces,Division_Code,Distributer_Rate,Retailor_Rate,Purchase_Date) select '" . $pk . "','" . $sfCode . "','" . $productCode . "','" . $productName . "',$recvQty,$cbQty,$pieces," . $Owndiv . ",$distPrice,$retPrice,'" . date('Y-m-d H:i:s') . "'";
				performQuery($sql);
                $sql = "SELECT Stockist_Code  as stockist,Cb_Qty,pieces FROM Trans_Current_Stock_details where Stockist_Code='" . $sfCode . "' and Product_Code='" . $productCode . "'";
                $tRw = performQuery($sql);
                $Op_Qty = 0;
                if (!empty($tRw)) {
                    $Op_Qty = $tRw[0]['Cb_Qty'] . "." . $tRw[0]['pieces'];
                    $sql = "DELETE FROM Trans_Current_Stock_details where Stockist_Code='" . $sfCode . "' and Product_Code='" . $productCode . "'";
                    performQuery($sql);
                }
                $sql = "insert into Trans_Current_Stock_details(Stockist_Code,Product_Code,Product_Name,Rec_Qty,Cb_Qty,pieces,Division_Code) select '" . $sfCode . "','" . $productCode . "','" . $productName . "',$recvQty,$cbQty,$pieces," . $Owndiv . "";
                performQuery($sql);
                ...
                //$saleQty = abs($saleQty);
                $sql = "insert into Trans_Secondary_Sales_Details(Sale_Code,Stockist_Code,Product_Code,Product_Name,Rec_Qty,Cl_Qty,Distributer_Rate,Retailor_Rate,Op_Qty,Sale_Qty,date) select $pk,'" . $sfCode . "','" . $productCode . "','" . $productName . "',$recvQty,$cbQtySale,$distPrice,$retPrice,$Op_Qty,$saleQty,'" . date('Y-m-d H:i:s') . "'";
                performQuery($sql);
            }
            break;
        case "tbMyDayPlan":
...
switch ($value[0]) {
    ...
    case "get/calls":
        $sfCode = $_GET['sfCode'];
        $date = date('Y-m-d');
        $query = "select convert(varchar,Activity_Date,103) Adate,POB_Value orderValue,h.Trans_SlNo from vwActivity_Report h inner join vwActivity_MSL_Details d on h.Trans_SlNo=d.Trans_SlNo where h.Sf_Code='$sfCode' and Activity_Date='$date'";
