...
function getSlabDetails() {
    ...
    $query = "select  RetSlabID id,SlabName name,SlabDesc SlabName,SlabMinVal,SlabMaxVal,CONVERT(varchar,To_Dt, 101)To_Date,Claim_deadline  from Mas_Retail_Business_Slab where ActiveFlag='0' and  Division_Code='" . $Owndiv . "' ";
    return performQuery($query);
}
...
function GiftEnrolment() {
    ...
    $query = "select  GiftSlabID id,GiftName name,GiftDesc GiftName,CONVERT(varchar,To_Date, 101)To_Date  from mas_gift_slab where ActiveFlag='0' and Division_Code='" . $Owndiv . "'  ";
    return performQuery($query);
}
...
function  PrimarytargetDistiWise(){
    ...
    $query = "exec getPrimaryDistiWise '" . $sfCode . "' ,'" . $EMonth . "' ,'" . $EYear . "' ";
...
function DateofExpense(){
	...
	if($Type=='1'){
    $query = "select top 1 ClstrName,  (select Worktype_Name_B from   Mas_WorkType_BaseLevel where WorkType_Code_B=TB.wtype ) WorkType from TbMyDayPlan  TB  where TB.sf_code='" . $sfCode . "' and convert(date,TB.Pln_Date)='" . $eDt . "' order by TB.Pln_Date ASC ";
	}else{
		 $query = "select top 1 ClstrName,  (select Worktype_Name_M from   Mas_WorkType_Mgr where WorkType_Code_M=TB.wtype ) WorkType from TbMyDayPlan  TB  where TB.sf_code='" . $sfCode . "' and convert(date,TB.Pln_Date)='" . $eDt . "' order by TB.Pln_Date ASC ";
	}
    return performQuery($query);
}
...
function getPreCallDet() {
  ...
  $as = performQuery($query);
    ...
    if (count($as) > 0) {
        ...
        $result['Mobile'] = (string) $as[0]['Mobile'];
...
case "SSstockUpdation":
 ...
 $sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works,Appversion) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."','".$Appversion."'";  
performQuery($sql);
...
$val =$data[3][$temppp[0]];
 $countsql = "select count(sf_code) as [count] from Trans_Daily_User_Expense where sf_code='" . $sfCode . "' and convert(date,eDate)='".$val["Enterdate"]."'";
 $countresult=performQuery($countsql);
...
if($countresult[0]["count"]==0){
 ...
 if( $data[2]['ActivityCaptures']!=null && $data[2]['ActivityCaptures']!="undefined")
   ...
   $sql ="exec Post_Expense '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."'";
   $result=performQuery($sql); 
...
$resp["success"] = true;
	...
	$resp["msg"]= 'Already Submitted for the Date';
$resp["success"] = true;
            echo json_encode($resp);
            die;
}
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
 ...
 Aproval_Flag='0' then 'Pending'  when Aproval_Flag='1' then 'Approved' else 'Reject' end GStatus ,case when
 Gift_Type='1' then 'Display'  when Aproval_Flag='2' then 'Product Bill' else 'Slab' end GiftName,
 Reject_reason,Aproval_Flag from Trans_Gift_Claim where Sf_Code ='" . $sfCode . "'";
        ...
        $sfCode = $_GET['sfCode'];
		 $EYear = $_GET['EYear'];
		 $EMonth = $_GET['EMonth'];
        $sql = "exec Get_Expense_Details '" . $sfCode . "' ,'" . $EMonth . "' ,'" . $EYear . "' ";
        $View = performQuery($sql);
		$sql = "select Applied_ExpAmnt,Expense_Amt from   trans_expense_head1  where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'";
...
case "get/retailergiftStatus":
      ...
      $EYear = $_GET['EYear'];
      $EMonth = $_GET['EMonth'];
      $query = "select * from vwDoctor_SlabMaster_APP where  charindex(',$SF_code,',','+Field_Code+',')>0 and month(ListedDr_Created_Date)='".$EMonth."' and year(ListedDr_Created_Date)='".$EYear."'";
