...
function addEntry() {
    ...
    switch ($temp[0]) {
		...
		if($sfCode!="undefined")
		{
            ...
            break;
 case "doortodoor":
               $sql = "SELECT * FROM vwActivity_Report where SF_Code='" . $sfCode . "' and lower(Work_Type) <>lower(" . $vals["wtype"] . ")  and cast(activity_date as datetime)=cast('$today' as datetime)";
   ...
   $ARCd = "0";
			...
			$ww=$vals["worked_with"];
$sql = "{call  svDoorToDoor('" . $sfCode . "'," . $vals["sf_member_code"] . ",'" . date('Y-m-d H:i:s') . "'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,".$vals['numberofcoupens'].",'',".$vals['retailorcode'].", ".$vals['retailorname'].")}";
...
$sql = "SELECT * FROM vwActivity_Report where SF_Code='" . $sfCode . "' and lower(Work_Type) <>lower(" . $vals["worktype"] . ")  and cast(activity_date as datetime)=cast('$today' as datetime)";
            ...
            if (count($result1) > 0) {
                if (!isset($_GET['replace'])) {
                    ...
                    delAREntry($sfCode, $vals["worktype"], $today);
   $ARCd = "0";
                        $sql = "{call  svDCRMain_App(?,?," . $vals["worktype"] . ",'" . str_replace("'", "", $vals["cluster"]) . "',?,'" . str_replace("'", "", $vals["remarks"]) . "',?)}";
                        $params = array(array($sfCode, SQLSRV_PARAM_IN),
                            array($today, SQLSRV_PARAM_IN),
                            array($Owndiv, SQLSRV_PARAM_IN),
                            array($ARCd, SQLSRV_PARAM_OUT));
...
$sql = "SELECT * FROM vwActivity_Report where SF_Code='" . $sfCode . "' and lower(Work_Type) <>lower(" . $vals["worktype"] . ")  and cast(activity_date as datetime)=cast('$today' as datetime)";
            ...
            if (count($result1) > 0) {
                if (!isset($_GET['replace'])) {
                    ...
                    delAREntry($sfCode, $vals["worktype"], $today);
   $ARCd = "0";
                        $sql = "{call  svDCRMain_App(?,?," . $vals["worktype"] . ",'" . str_replace("'", "", $vals["sdp"]) . "',?,'" . str_replace("'", "", $vals["remarks"]) . "',?)}";
                        $params = array(array($sfCode, SQLSRV_PARAM_IN),
                            array($today, SQLSRV_PARAM_IN),
                            array($Owndiv, SQLSRV_PARAM_IN),
                            array($ARCd, SQLSRV_PARAM_OUT));
...
$RCPAdetails=$vals['RCPAdetails'];

            $sql = "{call  svSurvey('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . "," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["sdp"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . "," . $vals['Contact_Person_Name'] . "," . $vals['Worked_With'] . "," . $vals['dateStr'] . "," . $vals['mdateStr'] . ",'$lat','$lng','$address'," . $vals['sdp'] . "," . $vals['sdp_name'] . "," . $vals['DataSF'] . ",'$doctorCode')}";
...
$cup=$RCPAdetails[$i]['CpmptCup'];
 $sql = "insert into tbRCPADetails select '" . $sfCode . "','" . date('Y-m-d H:i:s') . "'," . $vals["RCPADt"] . ",'','$doctorCode','" . $name . "','','','','',''," . $Owndiv . "," .$litters. ",0,''," . $vals["unlisted_doctor_name"].",$pouch,$cup";
...
if($DetTB["net_weight_value"]==null)
$net_weight_value=0;
...
if($transOrderDetails[$i]['net_weight']==null)
$net_weight=0;
else
...
$net_weight=0;
 ...
 $net_weight = $transOrderDetails[$i]['net_weight'];
$net_weight_value=$net_weight_value+$net_weight;
