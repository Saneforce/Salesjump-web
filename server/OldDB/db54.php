...
function addEntry() {
    ...
    switch ($temp[0]) {
        ...
        case "tbMyDayPlan":
            ...
            $sql = "insert into tbMyDayPlan select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."'";
			performQuery($sql);
            if (str_replace("'", "", $vals["FWFlg"]) != "F") {
		...
		case "Activity_Report_APP":
            ...
            if ($vTyp != 0 && $vTyp!=7) {
				...
				if ($vTyp == 1)
					$sqlsp = $sqlsp . "'" . str_replace("'", "", $vals["Town_code"]) . "','" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,'" . str_replace("'", "", $vals["rx_t"]) . "'," . $DetTB["modified_time"] . ",?,?," . $vals["DataSF"] . ",'" . $DetTB["geoaddress"] . "'," . $POB_Value . ",$net_weight_value,".$DetTB['stockist_code'].",".$DetTB['stockist_name'].",$discount_price,'$rateMode')}";
				else if($vTyp==3){
...
/*                if ($ARDCd == "Exists") {
				...
				if($vTyp == 3){
		            $sqlOrd = "{call svPriOrder(?,$cCode,?," . $POB_Value . ",'" . $DetTB["intrumenttype"] . "','" . $DetTB["date_of_intrument"] . "'," . $pob . ",'" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,?,?)}";
		            $params1 = array(
						array($sfCode, SQLSRV_PARAM_IN),
		                array($today, SQLSRV_PARAM_IN),
            ...
            if (count($transOrderDetails) > 0) {
                ...
                for ($i = 0; $i < count($transOrderDetails); $i++) {
					if($qty>0 || $free>0  || $cbQty>0){
	                    ...
	                    $sql = "insert into Trans_Order_Details( Trans_Order_No, Trans_Sl_No,Product_Code,Product_Name,Quantity,value,Rate,net_weight,free,discount,discount_price,rateMode,DistDisc,DistPicRate,MfgDt,ClStock) select '" . $pk1 . "','" . $pk . "','" . $transOrderDetails[$i]['product_code'] . "','" . $transOrderDetails[$i]['product_Name'] . "', $qty,$value,$rate,'$net_weight','$free','$discount','$discountprice','$rateMode',$Disc,$DistRate,'" . $transOrderDetails[$i]['Mfg_Date'] . "','". $cbQty  ."'";
						performQuery($sql);
					}
                }
			}
