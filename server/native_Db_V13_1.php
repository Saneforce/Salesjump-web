...
$query="select dcr.Trans_SlNo,dcr.Trans_Detail_Slno,dcr.Trans_Detail_Name,dcr.Trans_Detail_Info_Code,dcr.SDP_Name,dcr.POB,dcr.Worked_with_Name,dcr.StartOrder_Time,head.Trans_Sl_No,head.DCR_Code,head.Stockist_Code,head.Order_Value,head.Order_In_Time,head.Order_Out_Time,dcr.Activity_Remarks
      from DCRDetail_Lst_Trans dcr left outer join Trans_Order_Head head on dcr.Trans_Detail_Slno=head.DCR_Code where dcr.Trans_SlNo='".$Tran_Slno."' ";
return performQuery($query);
}
...
function getInshopClStock(){
  $div=$_GET['divisionCode'];
  $divs = explode(",", $div . ",");
  $Owndiv = (string) $divs[0];
  $Sf_Code=$_GET['sfCode'];
  $Sl_No=$_GET['checkInNo'];
  $Ret_Code=$_GET['retCode'];
  
   $sqlqry="select * from Trans_Inshop_OP_Stock where CheckIn_SlNo='".$Sl_No."'";
   $eRww=performQuery($sqlqry);
   if(count($eRww)>0){
	   return [];
   }else{
	   // $sql="select MAX(Sl_No)SlNo  from Trans_Cus_Checkin where Sf_Code='".$Sf_Code."' and Retailer_Code='".$Ret_Code."' and Sl_No<>'".$Sl_No."'";
          $sql="select MAX(Sl_No)SlNo  from Trans_Cus_Checkin  inner join Trans_Inshop_Cl_Stock  on CheckIn_SlNo=Sl_No where Sf_Code='".$Sf_Code."' and Retailer_Code='".$Ret_Code."' and  Sl_No<>'".$Sl_No."'";
        $res=performQuery($sql);
        $preslno =$res[0]['SlNo'];
        $sqls="select * from Trans_Inshop_CL_Stock where CheckIn_SlNo='".$preslno."'";
		 return performQuery($sqls);
   }
  
	
}

function getInshopOpStock(){
  $div=$_GET['divisionCode'];
  $divs = explode(",", $div . ",");
  $Owndiv = (string) $divs[0];
  $Sf_Code=$_GET['sfCode'];
  ...
  // $sql="select * from Trans_Inshop_OP_Stock where CheckIn_SlNo='".$Sl_No."'";
   //$res=performQuery($sql);
 //  if(count($res)>0{
	   
      $sqls="select * from Trans_Inshop_OP_Stock where CheckIn_SlNo='".$Sl_No."'";
	   return performQuery($sqls);
   // }
	
}
function getInshopOrdValue(){
  $div=$_GET['divisionCode'];
  $divs = explode(",", $div . ",");
  $Owndiv = (string) $divs[0];
  $Sf_Code=$_GET['sfCode'];
  ...
  // $sql="select * from Trans_Inshop_Orders where CheckIn_SlNo='".$Sl_No."'";
   //$res=performQuery($sql);
 //  if(count($res)>0{
	   
      $sqls="select * from Trans_Inshop_Orders where CheckIn_SlNo='".$Sl_No."'";
	  return performQuery($sqls);
   // }
	
}
function getInshopClValue(){
  $div=$_GET['divisionCode'];
  $divs = explode(",", $div . ",");
  $Owndiv = (string) $divs[0];
  $Sf_Code=$_GET['sfCode'];
  ...
  // $sql="select * from Trans_Inshop_CL_Stock where CheckIn_SlNo='".$Sl_No."'";
   //$res=performQuery($sql);
 //  if(count($res)>0{
	   
      $sqls="select * from Trans_Inshop_CL_Stock where CheckIn_SlNo='".$Sl_No."'";
	  return performQuery($sqls);
   // }
	
}

function getPriReturnData(){
	$sfCode = $_GET['sfCode'];
	$CrntDt = $_GET['crntdt'];
	$stkCode = $_GET['stkcode'];
	$query = "select * from SalesStockistReturn_Head srh left join SalesStockistReturn_Detail srd on srh.Trans_SlNo= srd.Trans_SlNo where cast(RetDate as date)='".$CrntDt."' and srh.SF='".$sfCode."' and srh.Stk_Code='".$stkCode."'";
    return performQuery($query);
...
function getMnPriReturnData(){
	$sfCode = $_GET['sfCode'];
	$CrntTDt = $_GET['crntdt'];
	$stkCode = $_GET['stkcode'];
	$query = "select * from Trans_Primary_Return_Head srh left join Trans_Primary_Return_Details srd on srh.Serial_no= srd.Serial_no where cast(Return_Date as date)='".$CrntTDt."' and srh.sf_code='".$sfCode."' and srh.distributor_code='".$stkCode."'";
    return performQuery($query);
}
...
function getPendingPaymentDets_mod(){
      $div=$_GET['divisionCode'];
      $divs = explode(",", $div . ",");
      $Owndiv = (string) $divs[0];
      $stk_code=$_GET['stockist_code'];
    ...
    $type =  str_replace('"', '', $orderType);
      ...
      $sql="exec get_van_pending_payment_details_App '".$sfCode."','".$Owndiv."','".$cus_code."' ";
...
function getVanFieldForce(){
	  $div=$_GET['divisionCode'];
      $divs = explode(",", $div . ",");
	  $Owndiv = (string) $divs[0];
	  $stateCode=$_GET['stateCode'];
	  
	  $sql="select ms.Sf_Code id,ms.sf_name name,Territory town_name,Territory_Code town_code from Mas_Salesforce ms  inner join access_table ta on ta.sf_code=ms.Sf_Code
where ta.Div_Code='".$Owndiv."'  and  ta.Van_Sales='1' and SF_Status=0 and State_Code='".$stateCode."' and  CHARINDEX(','+'".$Owndiv."'+',',','+ms.Division_Code+',')>0 ";
	   return performQuery($sql);
	
}


function getVanCollectPaymentDets(){ 
     $stk_code=$_GET['stockist_code'];
	  $div=$_GET['divisionCode'];
      $divs = explode(",", $div . ",");
	  $Owndiv = (string) $divs[0];
	  ...
	  $sql="exec Sp_Get_van_Payment_View '".$stk_code."','".$Owndiv."','".$order_no."' ";
      return performQuery($sql);
	
}

function getVanCollectPaymentList(){
	 $stk_code=$_GET['stockist_code'];
	  $From_Date=$_GET['From_Date'];
      $To_Date=$_GET['To_Date'];
	   $sql="exec get_van_payment_list '".$stk_code."','".$From_Date."','".$To_Date."' ";
      return performQuery($sql);
}
...
function getRetAddress(){
	     $listedDrCode = $_GET['listedDrCode'];
		 $div = $_GET['divisionCode'];
         $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
		$id = $_GET['id'];
		$query="select * from Mas_RetDeliveryAddress where ListedDrCode = '$listedDrCode' and flag = '0' and Division_code='$Owndiv'";
		return performQuery($query);
		
}
function insertRetAddress(){
        $listedDrCode = $_GET['listedDrCode'];
		$address = $_GET['address'];
		$div = $_GET['divisionCode'];
		$category=$_GET['category'];
		$stateNm=$_GET['stateName'];
		$stateCd=$_GET['stateCode'];
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
		$inquery="insert into Mas_RetDeliveryAddress(ListedDrCode,Address,CrDate,ModDate,flag,Division_code,Category,State_Code,State_Name) values ('$listedDrCode', '$address', GETDATE(), null, 0,'$Owndiv','$category','$stateCd','$stateNm')";
		performQuery($inquery);
		$lastAddQuery= "select top 1 * from Mas_RetDeliveryAddress where ListedDrCode = '$listedDrCode' and Division_code='$Owndiv' order by id desc";
		return performQuery($lastAddQuery);	
}
function updateRetAddress(){
        $listedDrCode = $_GET['listedDrCode'];
	    $address = $_GET['address'];
	    $category=$_GET['category'];
		$id = $_GET['id'];
        ...
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
		$upQuery="update Mas_RetDeliveryAddress set Address = '$address',Category = '$category', ModDate = GETDATE(),State_Code='$stateCd',State_Name='$stateNm' where ListedDrCode = '$listedDrCode' and id = '$id'";
	    performQuery($upQuery);
		$res["success"] = true;
	    return $res;
}
function deleteRetAddress(){
         $listedDrCode = $_GET['listedDrCode'];
		$id = $_GET['id'];
		$delQuery="delete Mas_RetDeliveryAddress where ListedDrCode = '$listedDrCode' and id = '$id'";
		performQuery($delQuery);
		$res["success"] = true;	
		return $res;
}

function getCheckSumSHA256($msg, $checksumKey){
...
$checksum = hash_hmac('sha256',$msg,$checksumKey, false); 
$checksum = strtoupper($checksum);
return $checksum;

}

function PrimaryPaymentSaveBillDesk(){
	$data = json_decode($_POST['data'], true);
	$OrderID = (string) $data["OrderID"];
	$StockistCode = (string) $data["StockistCode"];
	$divisionCode = (string) $data["divisionCode"];
	$PaymentMode = (string) $data["PaymentMode"];
	$PaymentTypeName = (string) $data["PaymentTypeName"];
	$PaymentTypeCode = (string) $data["PaymentTypeCode"];
	$Amount =(string) $data["Amount"];
	$Attachement =(string) $data["Attachement"];
	$UTRNumber =(string) $data["UTRNumber"];
	$PaymentID =(string) $data["PaymentID"];
	$RazorOrderID =(string) $data["RazorOrderID"];
	$SignatureID =(string) $data["SignatureID"];
	$PaymentResponse =(string) $data["PaymentResponse"];


	$sql="{call savePrimaryOrderBillDeskPayment(?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	$params1 = array(
						array($divisionCode, SQLSRV_PARAM_IN),
						array($OrderID, SQLSRV_PARAM_IN),
						array($StockistCode, SQLSRV_PARAM_IN),
						array($PaymentMode, SQLSRV_PARAM_IN),
						array($PaymentTypeName, SQLSRV_PARAM_IN),
						array($PaymentTypeCode, SQLSRV_PARAM_IN),
						array($Amount, SQLSRV_PARAM_IN),
						array($Attachement, SQLSRV_PARAM_IN),
						array($UTRNumber, SQLSRV_PARAM_IN),
						array($RazorOrderID, SQLSRV_PARAM_IN),
						array($PaymentID, SQLSRV_PARAM_IN),
						array($SignatureID, SQLSRV_PARAM_IN),
						array($PaymentResponse, SQLSRV_PARAM_IN)
...
function  GetvanLedger(){
    ...
    $query = "exec van_stock_ledger_mod '" . $sfCode . "','" . $Owndiv . "','".$fromdate."','".$todate."','0'  ";
...
function getVstDetsNative() {
	...
	if($ACd=="sec_order"){
		$activityDate = $_GET['activityDate'];
		 $query = "select trans_slno from dcrmain_trans with(nolock) where sf_code='" . $sfCode . "' and cast(activity_date as date)='" . $activityDate . "'";
...
$ACd=$ACDD[0]['trans_slno'];
	...
	for ($j = 0; $j < count($viewResponse); $j++) {
		...
		$productListQuery = [];
             $Trans_Detail_Slno= $viewResponse[$j]["Trans_Detail_Slno"];
		$imageListQuery = [];
		...
		if($typ == 8){
			...
			$productListQuery = "exec get_pri_order_dts '" . $orderNo . "','" . $sfCode . "'";
                 ...
                 $imageListQuery="select * from Activity_event_captures with(nolock) where DCRDetNo='" . $Trans_Detail_Slno . "' and Identification='audio'";

			$viewResponse[$j]["productList"] = performQuery($productListQuery);
			$viewResponse[$j]["imageList"]=performQuery($imageListQuery);
	}
	//$viewResponse['sql']=$query;
	//$viewResponse['sqll']=performQuery($imageListQuery);
	...
	case "saleUpdation":
           ...
           // $editable = $_GET['editable'];
            ...
            if($sfCode!="undefined")
            {
				 ...
				 for ($i = 0; $i < count($vals); $i++)
                {
                    $sql = "SELECT isNull(max(Tran_Slno),0)+1 as RwID FROM Trans_Sale_Updation_Details";
                    $tRw = performQuery($sql);
                    $pk = (int) $tRw[0]['RwID'];
                    $productCode = $vals[$i]['product'];
                    $productName = $vals[$i]['product_Nm'];
                    $cbQty = !empty($vals[$i]['cb_qty']) ? $vals[$i]['cb_qty'] : 0;
                    $pieces = !empty($vals[$i]['pieces']) ? $vals[$i]['pieces'] : 0;
					$totalQty = !empty($vals[$i]['total_qty']) ? $vals[$i]['total_qty'] : 0;
					$sample_qty = !empty($vals[$i]['sample_qty']) ? $vals[$i]['sample_qty'] : 0;
                   $Mfg=($vals[$i]['Mgf_date']==null? '':$vals[$i]['Mgf_date']);
                   $batch_no=($vals[$i]['batch_no']==null? '':$vals[$i]['batch_no']);
                     if(!empty($vals[$i]['conversionQty']))
                    ...
                    $sql = "delete Trans_Sale_Updation_Details where Stockist_Code='" . $sfCode . "' and Product_Code='" . $productCode . "' and Purchase_Date='" . $date . "'";
                    performQuery($sql);



                    $sql = "insert into Trans_Sale_Updation_Details(Tran_Slno,Stockist_Code,Product_Code,Product_Name,Rec_Qty,Cb_Qty,pieces,Division_Code,Distributer_Rate,Retailor_Rate,Purchase_Date,Conversion_Qty,SfCode,Mgf_date,TotalValue,batch_no,Total_Qty) select '" . $pk . "','" . $sfCode . "','" . $productCode . "',N'" . $productName . "',$recvQty,$cbQty,$pieces," . $Owndiv . ",$distPrice,$retPrice,'" . $date . "',$sampleErpCode,'$staffCode','$Mfg',$sample_qty,'$batch_no','$totalQty'";
                    performQuery($sql);
					$sqlthiru= $sql;
				}
			 if( $data[1]['Activity_Event_Captures']!=null && $data[1]['Activity_Event_Captures']!="undefined")
            {
                ...
                for ($j = 0; $j < count($Event_Captures); $j++) {
                    ...
                    $sql = "insert into Activity_Event_Captures(Activity_Report_Code,imgurl,title,remarks,Division_Code,Identification,Insert_Date_Time,DCRDetNo) values( '".$sfCode."','" . $visitpath . "','" . $ev_title . "','" . $ev_remarks . "','".$Owndiv."','Closing Sale','" . $date . "','" .$Sales_Code."')";
                    performQuery($sql);
                }
            }
...
case "SSstockUpdation":
        ...
        case "tbMyDayPlan":
         ...
         if( $vals['profilepic']!=null && $vals['profilepic']!="undefined")
			...
			if($vals["cluster"]!=null&&$vals["cluster"]!="''"&&($vals["ClstrName"]==null ||$vals["ClstrName"]=="''")){
				$sql = "insert into testMyDayPlanData(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works,Appversion,Place_Inv) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . ",'" . str_replace("'","", $vals["remarks"]) . "','" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."','".$Appversion."' , '".$Place_Inv."'";
			    performQuery($sql);
 ...
 case "Activity_Return_Report_App":
	...
	$SfCode = $_GET['sfCode'];
	 ...
	 //  $time = $data[0]['Activity_Return_Report_APP']['CheckoutTime'];
	  ...
	  if($vals["editReturn"]==1){
				$qrya="delete from Trans_Primary_Return_Head where Serial_no='" . $vals["slno"] . "' and distributor_code ='" . $vals["distributor_id"] . "' and sf_code='" . $sfCode . "'";
				performQuery($qrya);
				$qryb="delete from Trans_Primary_Return_Details where Serial_no='" . $vals["slno"] . "'";
				performQuery($qryb);
				
			 $sql="insert into Trans_Primary_Return_Head Values('" . $vals["slno"] . "','".$SfCode. "', '" . str_replace("'", "", $vals["distributor_id"]) . "', '" . str_replace("'", "", $vals["CheckoutTime"]) . "')";
			  performQuery($sql);
			}else{
	...
	if (isset($tableData['Activity_Sample_Report'])){
        $samp = $tableData['Activity_Sample_Report'];
            ...
            for ($j = 0; $j < count($samp); $j++) {
				   ...
				   $sqlqry="Exec savePrimaryReturnStock '".$productName."','".$productCode."','".$ReturnQty."','".$spinPos."','".$ReturnType."','".$Remarks."','".$SfCode. "', '" . str_replace("'", "", $vals["distributor_id"]) . "' ";
				   performQuery($sqlqry);
...
$merchantFromSec=str_replace("'","",$vals["merchantFromSec"]);
$CheckoutTime=str_replace("'","",$vals["CheckoutTime"]);
...
$product_unit_conv=$samp[$j]["product_unit_conv"];
 ...
 for ($i = 1; $i < count($data); $i++) {
	...
	if($merchantFromSec=="1"){
		
		/*if (isset($tableData['Activity_Event_Captures_CheckIn'])){
                   $Event_Captures  = $tableData['Activity_Event_Captures_CheckIn'];
				    for ($j = 0; $j < count($Event_Captures); $j++) {
                    $ev_imgurl=str_replace("'", "",$Event_Captures[$j]["imgurl"]);
                    $ev_title=str_replace("'", "",$Event_Captures[$j]["title"]);
                    $ev_remarks=str_replace("'", "",$Event_Captures[$j]["remarks"]);
	...
	$sql="insert into Trans_Cus_Checkin(Sf_Code,Retailer_Code,CIn_Time,Entry_Date,eKey,COut_Time,C_Flag,Retailer_Name,Sf_Name,Entry_Type,Cin_Img_Url,Cout_Img_Url) Values('".$SfCode."','".str_replace("'", "", $vals["retailer_code"])."','".str_replace("'", "", $vals["checkin_time"])."','".str_replace("'", "", $vals["entry_date"])."','".str_replace("'", "", $vals["ekey"])."','".$sample."','".str_replace("'", "", $vals["c_flag"])."','".str_replace("'", "", $vals["retailer_name"])."','".str_replace("'", "", $vals["sf_name"])."','".str_replace("'", "", $vals["entry_type"])."','".str_replace("'", "", $vals["ev_imgurl"])."','')";
    performQuery($sql);
  ...
  $sql="insert into Trans_Pop_Ledger(Sf_Code,Stock_To,Pop_Code,Qty,Stock_Type,Ledg_Date,Ledg_Time,Merchant_Flag) values('".$SfCode."','".$SfCode."','".$productCode."','".$Qty."','".$StockType."','".$OrderDate."','".$OrderTime."','".$merchantFromSec."')";
                   performQuery($sql);
   $query="Exec saveOutletDcr'".$SfCode."','".$Owndiv."','".$RetailerCode."','".$productCode."','".$Qty."','".$pkk."','".$OrderDate."','".$OrderTime."','".$eKey."','".$merchantFromSec."' ";
  //$query11="insert into Trans_Pop_Dcr_Details Values('".$SfCode."','".$Owndiv."','".$RetailerCode."','".$productCode."','".$Qty."','".$pkk."','".$OrderDate."','".$OrderTime."','".$serialNo."','".$eKey."')";
    performQuery($query);  
 ...
 if (isset($tableData['Activity_Event_Captures'])){
	...
	$serialNoQuery="select Sl_No from Trans_cus_checkin where Sf_Code='".$SfCode."' and C_Flag='1' and Entry_Date='".str_replace("'", "", $vals["entry_date"])."' and Retailer_Code='".str_replace("'", "", $vals["retailer_code"])."' ";
	$tRw = performQuery($serialNoQuery);
    $serialNo =$tRw[0]['Sl_No'];
	$sqls="Update Trans_Cus_Checkin set COut_Time='".$CheckoutTime."',C_Flag='0' where Sl_No='".$serialNo."'";
    performQuery($sqls);
            ...
            for ($j = 0; $j < count($samp); $j++) {
                 ...
                 $productName=$samp[$j]["product_Name"];
                  ...
                  performQuery($sql);
			        ...
			        $sql="exec saveInshopOrders_mod '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_price."','".$product_unit_code."','".$product_unit_name."','" . $Owndiv . "' ";
                     performQuery($sql);
			     }
...
$resp["sql"] = $sql;
        ...
        die;
break;
case "Activity_Inshop_Order_Report_App_Mod":
   $SfCode = $_GET['sfCode'];
   $div = $_GET['divisionCode'];
   $divs = explode(",", $div . ",");
   $Owndiv = (string) $divs[0];
          ...
          $samp = $tableData['Activity_Sample_Report'];
            ...
            for ($j = 0; $j < count($samp); $j++) {
                 ...
                 $productName=$samp[$j]["product_Name"];
                  ...
                  $product_unit_name=$samp[$j]["product_unit_name"];
                  if($InshopField==1){
                  $sql="exec saveInshopOrders1 '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_price."','".$product_unit_code."','".$product_unit_name."' ";
                  performQuery($sql);
			      }else{
			        $sql="exec saveInshopOrders_mod1 '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_price."','".$product_unit_code."','".$product_unit_name."','" . $Owndiv . "' ";
                     performQuery($sql);
			     }
          ...
          $samp = $tableData['Activity_Sample_Report'];
            ...
            for ($j = 0; $j < count($samp); $j++) {
               ...
               performQuery($sql);
				 ...
				 $sql="exec saveInshopOpStock_mod '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_unit_code."','".$product_unit_name."','" . $Owndiv . "' ";
               performQuery($sql);   
...
case "Activity_Inshop_OpeningStock_Report_App_Mod":
   $SfCode = $_GET['sfCode'];
   $div = $_GET['divisionCode'];
   $divs = explode(",", $div . ",");
   $Owndiv = (string) $divs[0];
          ...
          $samp = $tableData['Activity_Sample_Report'];
            ...
            for ($j = 0; $j < count($samp); $j++) {
                 ...
                 $productName=$samp[$j]["product_Name"];
                  ...
                  $product_unit_name=$samp[$j]["product_unit_name"];
               if($InshopField==3){
                  $sql="exec saveInshopOpStock1 '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_unit_code."','".$product_unit_name."' ";
               performQuery($sql);
			   }else{
				 $sql="exec saveInshopOpStock_mod1 '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_unit_code."','".$product_unit_name."','" . $Owndiv . "' ";
               performQuery($sql);   
...
$resp["success"] = true;
        ...
        die;
break;
case "Activity_Inshop_ClosingStock_Report_App_Mod":
   $SfCode = $_GET['sfCode'];
   $OrderDate=  str_replace("'","",$vals["OrderDate"]); 
   $key=  str_replace("'","",$vals["ekey"]); 
for ($i = 1; $i < count($data); $i++) {
          ...
          $samp = $tableData['Activity_Sample_Report'];
            ...
            for ($j = 0; $j < count($samp); $j++) {
                 ...
                 $productName=$samp[$j]["product_Name"];
                  ...
                  $sql="exec saveInshopClStock_mod1 '".$SfCode."','".$OrderDate."','".$key."','".$productCode."','".$Qty."','".$product_unit_conv."','".$product_unit_code."','".$product_unit_name."' ";
               performQuery($sql);
...
case "Vansalesreject":
	...
	$sql = "select * from Trans_Prod_DailyInventoryNative where  SFCode='" . $ApprvSFCode . "' and ApproveFlag in ('1','2') and  convert(date,EntryDt)=convert(date,getdate())and CHARINDEX(','+cast(SLNo as varchar)+',',','+'".$vals['allLoadingStockId']."')>0 ";
    $eres = performQuery($sql);
	$resp['Qry'] =$sql;
    if(count($eres)>0){
	         $approveName =$eres[0]['ApproveByName'];
			 $flag=$eres[0]['ApproveFlag'];
			 $resp["success"] = false;
			 if($flag==1)
	           $resp['Msg'] ="Already Approved by".$approveName;
		    ELSE
				$resp['Msg'] ="Already Rejected by".$approveName;
	      }else{
			 for($il=0;$il<count($vals["InvProducts"]);$il++)
                {
                    $PDets=$vals["InvProducts"][$il];
                    $updateAprlQuery = "update Trans_Prod_DailyInventoryNative set ApproveFlag = '2',ApproveById='".$vals["RejectById"]."',ApproveByName='".$vals["RejectByName"]."' where SLNo ='". $PDets["id"] . "'";
 ...
 case "LeaveFormValidate":
  ...
  case "DCRMissedDates":
	...
	case "LeaveForm_New":
			...
			//$fflagg=$vals['half_flag'];
			if($vals['half_flag']==1){
			$querydel="delete from mas_Leave_Form where sf_code = '$sfCode'  and From_date ='".$vals['From_Date']."' and  Leave_Id = ".$vals['half_leave_id']." ";//and Leave_Active_Flag = 2
			$del=performQuery($querydel);
			}
			$query = "SELECT convert(nvarchar(MAX), From_Date, 105)LeaveDate,* from mas_Leave_Form where sf_code = '$sfCode' and Leave_Active_Flag in('0','2') and
			...
			$resp["leave_approve"] = false;
			$resp["Interval"] = dateDiffInDays("2020-07-01","2020-07-02");
		}else{
		$is_first_sec=0;
		$leaveId=0;
		if(($vals['No_of_Days']==1||$vals['No_of_Days']==0.5)&&str_replace("'", "",$vals['halfday'])!=$result[0]['First_or_Second']){
		$is_first_sec = $result[0]['First_or_Second'];
		$leaveId=$result[0]['Leave_Id'];
		}
        ...
        for($il=0;$il<count($result);$il++){
	      $leave_active_flag=$result[$il]['Leave_Active_Flag'];
		  if(($vals['No_of_Days']>1&&$leave_active_flag==0)||(($vals['No_of_Days']==1||$vals['No_of_Days']==0.5)&&$leave_active_flag==0&&str_replace("'", "",$vals['halfday'])==$result[0]['First_or_Second'])){
			$cnt++;
			$stringdate .= ",".$result[$il]['LeaveDate'];
		 ...
		 if($cnt==0){
			 ...
			 $resp["leave_approve"] = true;  
			 $resp["approve_date"]=substr($stringdate, 1);;
		 }
			...
			//$resp["leave_approve"] = false;
            $resp['query'] = $querydel; 
			$resp['halfdayFlag'] = $is_first_sec;
			$resp['halfLeaveId']=$leaveId;
		//	$resp['fflagg']=str_replace("'", "",$vals['halfday']);
		//	$resp['fflaggas']=$result[0]['First_or_Second'];
...
if($PDets["damageorderdate"]=="" || $PDets["damageorderdate"]==null) $PDets["damageorderdate"]="";
                ...
                $sql = "exec UpdRetReturnStock '".$pk."','" . $vals["StkId"] . "','" . $PDets["id"] . "','" . date('Y-m-d H:i:s') . "','".$PDets["S_name"]."'," . $Qtys . ",".$Divc.",'',''";
            ...
            if($Clim=="Y"){
                $sql = "insert into Customer_Ledger_Details(Cust_Code,Value,eType,eDate,EntryFrom,Remarks) select '" . $vals["DrId"] . "',".$tVal.",'C',getDate(),'Return',''";
                performQuery($sql);
            }
			...
			if($vals["editReturn"]==1){
				$qrya="delete from SalesStockistReturn_Head where Trans_SlNo='" . $vals["trans_slno"] . "' and Stk_Code ='" . $vals["StkId"] . "' and SF='" . $sfCode . "'";
				performQuery($qrya);
				$qryb="delete from SalesStockistReturn_Detail where Trans_SlNo='" . $vals["trans_slno"] . "'";
				performQuery($qryb);
...
if($PDets["damageorderdate"]=="" || $PDets["damageorderdate"]==null) $PDets["damageorderdate"]="";
 ...
 select ".$pk.",m.Product_Detail_Code," . $PDets["Qty"] . "," .
                ...
                //$Qtys=($rQty*$PDets["conversionQty"])+$rPQty;
                $sqlupd = "exec UpdStockistRetReturnStock '".$pk."','" . $vals["StkId"] . "','" . $PDets["id"] . "','" . date('Y-m-d H:i:s') . "','".$PDets["S_name"]."'," . $Qtys . ",".$Divc.",'',''";

                $sss=performQueryWP($sqlupd,[]);
            ...
            if($Clim=="Y"){
                $sql = "insert into Customer_Stockist_Ledger_Details select '" . $vals["StkId"] . "',".$tVal.",'C',getDate(),'Return',''";
                performQuery($sql);
            }
			
			                $result = array();
							// $result['sql'] = $sql;
                             $result['sqlupd'] = $sqlupd;
                             $result['sqlres'] = $sss;
                             // $result['Upsql'] = $Upsql;
                             $result["success"] = true;
                             outputJSON($result);
...
case "VanPendingPaymentDetailsNative":
                         $SfCode = $_GET['sfCode'];
						 $SfName=$_GET['sfName'];
						 $div = $_GET['divisionCode'];
                         $divs = explode(",", $div . ",");
                         ...
                         $Type=$_GET['Type'];
						  ...
						  $Route_code=$vals["Route_code"];
						  $eKey=$vals["eKey"];
						   $sql = "SELECT ISNULL(MAX(Sl_No),0)+1 as slNo FROM trans_van_payment_detail ";
                            $tRw = performQuery($sql);
                            $pk = (int) $tRw[0]['slNo'];
							
							  $sqlQry = "insert into trans_van_payment_detail(Sl_No,Sf_Code,Sf_Name,Cust_Id,Cus_Name,Amount,Pay_Mode,Pay_Date,Pay_Ref_No,Remarks,Distributor_Code,Route_code,eDate,PaymentName,advance_pay,div_code,invoice_no,Collected_by,type,eKey) Values('".$pk."','".$SfCode."','".$SfName."','".$Cust_ID."','".$Cust_Name."','".$Total_amount."','".$Mode."','". date('Y-m-d H:i:s') . "','".$Reference_No."','".$Remark."','".$SfCode."','".$Route_code."','".$Pay_Date."','".$Bk_name."','".$Advance_pay."','".$Owndiv."','".$invoice_no."','".$collect_by."','".$Type."','".$eKey."')";
                            ...
                            for($il=0;$il<count($vals["Payment_Details"]);$il++)
                            {
                              $PDets=$vals["Payment_Details"][$il];
							   ...
							   $Newsql = "insert into trans_van_payment_detail_view(Sl_No,bill_no,bill_date,bill_amt,Pen_amt,paid_amt) Values('".$pk."','" . $PDets["bill_no"] . "','". date('Y-m-d H:i:s') . "','" . $PDets["bill_amt"] . "','" . $PDets["Pen_amt"] . "','" . $PDets["paid_amt"] ."')";
							   
							   
							  performQuery($Newsql);

 $PPXML = $PPXML . "<Bill Sf_Code=\"".$SfCode."\" Cust_Code=\"".$Cust_ID."\" Received_amt=\"".$PDets["paid_amt"]."\" Pay_Date=\"".date('Y-m-d H:i:s')."\" Bill_No=\"".$PDets["order_no"]."\" Inv_no=\"".$PDets["bill_no"]."\" />";

							}
                                          $PPXML = $PPXML . "</ROOT>";
                              //$Upsql= "exec Update_Pending_bill '" .$PPXML. "', '".$Stk_Code."' ,'".$Cust_ID."','".$Total_amount."','".$Advance_pay."','" .$Type. "'";
                                $Upsql= "exec Update_van_Pending_bill '" .$PPXML. "', '".$SfCode."' ,'".$Cust_ID."','".$Total_amount."' ";
                               performQuery($Upsql);
	...
	case "DailyInventoryLoadingNativeMod":
                            $SFCd=$vals["SF"];
	                        ...
	                        if($vals["ApproveById"]==null || $vals["ApproveById"]=="")
								$vals["ApproveById"]="";
							
							if($vals["ApproveByName"]==null || $vals["ApproveByName"]=="")
								$vals["ApproveByName"]="";
								
								
							
	    $sql = "select * from Trans_Prod_DailyInventoryNative where  SFCode='" . $SFCd . "' and ApproveFlag in('1','2') and  convert(date,EntryDt)=convert(date,getdate())and CHARINDEX(','+cast(SLNo as varchar)+',',','+'".$vals['allLoadingStockId']."')>0 ";
    $eres = performQuery($sql);
	$result = array();
		$result['Qry'] =$sql;
    if(count($eres)>0&&$vals["AprlFlag"]=1){
	         $rejectName =$eres[0]['ApproveByName'];
			 $flag=$eres[0]['ApproveFlag'];
			 $result["success"] = false;
			 if($flag==2)
	         $result['Msg'] ="Already Rejected by".$rejectName;
		     ELSE
		     $result['Msg'] ="Already Approved by".$rejectName;
    }else{
							...
							if($Divc != 4){
                                    //$updateAprlQuery = "update Trans_Prod_DailyInventoryNative set ApproveFlag = '1' where SLNo ='". $PDets["loadingStockId"] . "'";
									 $updateAprlQuery = "update Trans_Prod_DailyInventoryNative set ApproveFlag = '1',ApproveById='" . $vals["ApproveById"] . "',ApproveByName='" . $vals["ApproveByName"] . "' where SLNo ='". $PDets["loadingStockId"] . "'";
                                    performQuery($updateAprlQuery);
                            }else{
                                        $dailyInvNewsql = "insert into Trans_Prod_DailyInventoryNative (SLNo, SFCode, EntryDt, Prod_Code, CaseQty, PiceQty, ConvQty, RateC_Retail, RateP_Retail, RateC_Dist, RateP_Dist, ApproveFlag, StkId, StkNm, Totalqty, TotalCase, TotalPcs, CarryFwdCase, CarryFwdPcs, Div_Id, Max_Time,ApproveById,ApproveByName) select '" . $pk ."','". $SFCd ."','". date('Y-m-d 00:00:00') ."','". $PDets["id"] . "','" . $PDets["opening_case_qty"] . "','" . $PDets["opening_piece_qty"]. "','" .$PDets["conversionQty"] . "',isnull(RetailCasePrice,0), isnull(Retailor_Price,0),isnull(DistCasePrice,0) DCP,isnull(Distributor_Price,0), 1,'". $vals["StkId"] . "','" . $vals["StkNm"] . "','" . $PDets["Totalqty"] . "','" . $PDets["total_case_qty"] . "','" .$PDets["total_piece_qty"]. "','" . $PDets["carry_fwd_case_qty"] . "','" .$PDets["carry_fwd_piece_qty"].  "','" . $PDets["Division_Code"] .  "', '". date('Y-m-d H:i:s') . "','" . $vals["ApproveById"] . "','" . $vals["ApproveByName"] . "'
 ...
 case "DailyInventoryUNLoadingNative":
                            ...
                            if($vals["SF"]==null || $vals["SF"]=="")
                             ...
                             $dt=$vals["UpdStkDate"];
							$dtms=$vals["UpdStkDateTime"];

                            if(count($vals["InvProducts"]) == 0){
                                $updateLatestTime = "update Trans_Prod_DailyEndInventoryNative set Max_Time = '".$dtms . "' where cast(EntryDt as date) = '". $dt . "'";
                            ...
                            for($il=0;$il<count($vals["InvProducts"]);$il++)
                                                        ...
                                                        $dailyInvNewsql = "insert into Trans_Prod_DailyEndInventoryNative (SLNo, SFCode, EntryDt, Prod_Code, CaseQty, PiceQty, ConvQty, RateC_Retail, RateP_Retail, RateC_Dist, RateP_Dist, ApproveFlag, StkId, StkNm, Totalqty, TotalCase, TotalPcs, CarryFwdCase, CarryFwdPcs, Div_Id, Max_Time ) select '" . $pk ."','". $SFCd ."','". $dt ."','". $PDets["id"] . "','" . $PDets["opening_case_qty"] . "','" . $PDets["opening_piece_qty"]. "','" .$PDets["conversionQty"] . "',isnull(RetailCasePrice,0), isnull(Retailor_Price,0),isnull(DistCasePrice,0) DCP,isnull(Distributor_Price,0), 0,'". $vals["StkId"] . "','" . $vals["StkNm"] . "','" . $PDets["Totalqty"] . "','" . $PDets["total_case_qty"] . "','" .$PDets["total_piece_qty"]. "','" . $PDets["carry_fwd_case_qty"] . "','" .$PDets["carry_fwd_piece_qty"].  "','" . $PDets["Division_Code"] .  "', '" . $dtms . "' from Mas_Product_Detail m left outer join vwProductStateRates v on m.Product_Detail_Code=v.Product_Detail_Code and m.Division_Code=v.Division_Code and v.State_Code=". $SFState." Where m.Product_Detail_Code='". $PDets["id"] . "'";
                                                        performQuery($dailyInvNewsql);

                            							//$dailyInvsql = "insert into Trans_Prod_DailyEndInventory (SLNo, SFCode, EntryDt, Prod_Code, CaseQty, PiceQty, ConvQty, RateC_Retail, RateP_Retail, RateC_Dist, RateP_Dist, ApproveFlag, StkId, StkNm, Totalqty) select '" . $pk ."','". $SFCd ."','". $dtms ."','". $PDets["id"] . "','" . $PDets["opening_case_qty"] . "','" . $PDets["opening_piece_qty"]. "','" .$PDets["conversionQty"] . "',isnull(RetailCasePrice,0), isnull(Retailor_Price,0),isnull(DistCasePrice,0) DCP,isnull(Distributor_Price,0), 0,'". $vals["StkId"] . "','" . $vals["StkNm"] . "','" . $PDets["Totalqty"] . "' from Mas_Product_Detail m left outer join vwProductStateRates v on m.Product_Detail_Code=v.Product_Detail_Code and m.Division_Code=v.Division_Code and v.State_Code=". $SFState." Where m.Product_Detail_Code='". $PDets["id"] . "'";
                            							//performQuery($dailyInvsql);
                                                        ...
                                                        $Ledgersql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm) select '" . $MaxRwID ."','". $vals["StkId"] ."','D','". $SFCd ."','F','" . $dtms . "','" . $PDets["id"] . "','" .$PDets["g_qty"]. "','". $Divc . "','EndInv','0','". $vals["StkNm"] ."'" ;
                                                        performQuery($Ledgersql);
														
														 $DQtys =$PDets["d_qty"];
                                                         if($DQtys>0){
														 $Ledgsql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm) select '" . $MaxRwID ."','". $vals["StkId"] ."','D','". $SFCd ."','F','" . $dtms . "','" . $PDets["id"] . "','" .$PDets["d_qty"]. "','". $Divc . "','DmgQty','0','". $vals["StkNm"] ."'" ;
                                                         performQuery($Ledgsql);
                                                         }
							...
							die;
							break;
case "DailyInventoryCarryFwdNative":
		                    $SFCd=$vals["SF"];
                            if($vals["SF"]==null || $vals["SF"]=="")
                                $SFCd=$vals["sfCode"];
                            else
                            ...
                            if($PDets["PQty"]=="") $PDets["PQty"]=0;
							     ...
							     $dailyInvNewsql = "insert into Trans_Prod_DailyInventoryNative (SLNo, SFCode, EntryDt, Prod_Code, OPCase,OPPcs, ConvQty, RateC_Retail, RateP_Retail, RateC_Dist, RateP_Dist, ApproveFlag, StkId, StkNm, OPTotalQty,Div_Id, Max_Time) select '" . $pk ."','". $SFCd ."','". date('Y-m-d 00:00:00') ."','". $PDets["id"] . "','" . $PDets["opening_case_qty"] . "','" . $PDets["opening_piece_qty"]. "','" .$PDets["conversionQty"] . "',isnull(RetailCasePrice,0), isnull(Retailor_Price,0),isnull(DistCasePrice,0) DCP,isnull(Distributor_Price,0), 3,'". $vals["StkId"] . "','" . $vals["StkNm"] . "','" . $PDets["Totalqty"] . "','" . $PDets["Division_Code"] .  "', '". date('Y-m-d H:i:s') . "' from Mas_Product_Detail m left outer join vwProductStateRates v on m.Product_Detail_Code=v.Product_Detail_Code and m.Division_Code=v.Division_Code and v.State_Code=". $SFState." Where m.Product_Detail_Code='". $PDets["id"] . "'";
                            performQuery($dailyInvNewsql);
                                                        ...
                                                        $Ledgersql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm) select '" . $MaxRwID ."','". $vals["StkId"] ."','D','". $SFCd ."','F','" . $dt . "','" . $PDets["id"] . "','" .$PDets["g_qty"]. "','". $Divc . "','CarryFwd','0','". $vals["StkNm"] ."'" ;
                                                        performQuery($Ledgersql);
        ...
        $result = array();
        $query = "select Trans_Sl_No,(Order_Value+isnull(discount_price,0)) Order_Value ,Order_Value netamount  from Trans_Order_Head where  OrderID='".$CallPreviewEkey."'  union select Trans_Sl_No,Order_Value,0 from Trans_PriOrder_Head where  OrderID='".$CallPreviewEkey."' union select Trans_Sl_No,Order_Value,0 from Trans_SPriOrder_Head where  OrderID='".$CallPreviewEkey."' union select Trans_Sl_No,Order_Value,0 from Trans_Pragnancy__Head where  OrderID='".$CallPreviewEkey."'";
...
'". $value."','" . $Rate . "','" . $free . "','" . $PieseRate . "','" . $sfCode . "'";
			...
			$resp=array();
				...
				die;
        break;
		case "common_dynamic_data":
		    $div = $_GET['divisionCode'];
            $divs = explode(",", $div . ",");
            $divisionCode = (string) $divs[0];
			$sfCode= $_GET['sfCode'];
			$dynamicData = $data[1]['dynamic_data_detail'];
			$eKey=$data[0]['common_dynamic_data']['eKey'];
			
		    //$retQuery = "select Top 1 ListedDrCode from Mas_ListedDr where charindex(',$sfCode,',','+Sf_Code+',')>0 order by ListedDr_Created_Date desc";
       // $retdata = performQuery($retQuery);
        //$retailer_code = $retdata[0] ['ListedDrCode'];
		if (count($dynamicData) > 0 ) {
			

         // $resp["params12345"] = $dynamicData;
			foreach ($dynamicData as $detail) {
            $groupId = $detail['groupId'];
            $grpTableName = $detail['grpTableName'];
            $insertData = [];
          //  $resp["params1234"] = $grpTableName;
            foreach ($detail['itemdetail'] as $item) {
                $columnName = $item['column_name'];
                $dataValue = $item['data_value'];
                $insertData[$columnName] = $dataValue;
            }
              $insertData['EntryID'] = $eKey;
			  $insertData['EntryBy'] = $sfCode;
			  $insertData['EntryDate'] = date('Y-m-d H:i:s');
			  
			 // $resp["params123"] = $insertData;
			
            if (!empty($insertData)) {
                $columns = implode(', ', array_keys($insertData));
                $values = "'" . implode("', '", $insertData) . "'";
                $insertQuery = "INSERT INTO $grpTableName ($columns) VALUES ($values)";
				//$resp["params12"] = $insertQuery;
			 ...
			 $resp["params1"] = $insertQuery;
		    echo json_encode($resp);
            die;
		break;
		
		case "retailer_dynamic_data":
		    $div = $_GET['divisionCode'];
            $divs = explode(",", $div . ",");
            $divisionCode = (string) $divs[0];
			$sfCode= $_GET['sfCode'];
        ...
        $retailer_code = $retdata[0] ['ListedDrCode'];
			 if (count($dynamicData) > 0 ) {
			/*for ($i = 0; $i < count($dynamicData); $i++) {
					  $column = $dynamicData[$i]['column_name'];
					  $datavalue = $dynamicData[$i]['data_value'];
				      $tablename = $dynamicData[$i]['table_name'];
					 
       $insertQuery = "insert into ".$tablename."($column) values( '".$datavalue."')";
        performQuery($insertQuery);
            ...
            foreach ($detail['itemdetail'] as $item) {
              ...
              $insertData['Retail_code'] = $retailer_code;
            if (!empty($insertData)) {
                $columns = implode(', ', array_keys($insertData));
                $values = "'" . implode("', '", $insertData) . "'";
                $insertQuery = "INSERT INTO $grpTableName ($columns) VALUES ($values)";
            ...
            die;
		break;
		case "distributor_dynamic_data":
		    $div = $_GET['divisionCode'];
            $divs = explode(",", $div . ",");
            $divisionCode = (string) $divs[0];
			$sfCode= $_GET['sfCode'];
			$dynamicData = $data[1]['dynamic_data_detail'];
		    $distQuery = "select Top 1 Stockist_Code from mas_stockist where charindex(',$sfCode,',','+Field_Code+',')>0 order by Created_date desc";
            $distdata = performQuery($distQuery);
            $dist_code = $distdata[0] ['Stockist_Code'];
			if (count($dynamicData) > 0 ) {
            ...
            die;
		break;
		case "route_dynamic_data":
		    $div = $_GET['divisionCode'];
            $divs = explode(",", $div . ",");
            $divisionCode = (string) $divs[0];
			$sfCode= $_GET['sfCode'];
			$dynamicData = $data[1]['dynamic_data_detail'];
		    $routeQuery = "select Top 1 Territory_Code from Mas_Territory_Creation where charindex(',$sfCode,',','+Sf_Code+',')>0 order by Created_date desc";
            $routedata = performQuery($routeQuery);
            $route_code = $routedata[0] ['Territory_Code'];
			if (count($dynamicData) > 0 ) {
            ...
            foreach ($detail['itemdetail'] as $item) {
              ...
              $insertData['RouteCode'] = $route_code;
            if (!empty($insertData)) {
                $columns = implode(', ', array_keys($insertData));
                $values = "'" . implode("', '", $insertData) . "'";
                $insertQuery = "INSERT INTO $grpTableName ($columns) VALUES ($values)";
	    ...
	    case "update_retailer_dynamic_data":
		    $div = $_GET['divisionCode'];
            $divs = explode(",", $div . ",");
            $divisionCode = (string) $divs[0];
			$sfCode= $_GET['sfCode'];
			$retailerCode=$data[0]['update_retailer_dynamic_data']['retailer_code'];
			$dynamicData = $data[1]['dynamic_data_detail'];
		...
		if (count($dynamicData) > 0 ) {
		   ...
		   foreach ($dynamicData as $detail) {
            if (isset($detail['grpTableName']) && isset($detail['itemdetail']) && is_array($detail['itemdetail'])) {
                $tableName = $detail['grpTableName'];
                foreach ($detail['itemdetail'] as $itemDetail) {
                    if (isset($itemDetail['column_name']) && isset($itemDetail['data_value'])) {
                        $columnName = $itemDetail['column_name'];
                        $value = $itemDetail['data_value'];

                        $updatesql = "UPDATE $tableName SET $columnName = '".$value ."' WHERE Retail_code='".$retailerCode."'"; 
...
$sql="exec  getMxH'" . $sfCode . "'," . $stockistCode . ",'".$Owndiv."'";
                    ...
                    $tRw = performQuery($sql);
                      $resp["tTransNo"] =performQuery($sql);
...
$vanSalesOrders=$data[0]['Activity_Report_APP']['vanSalesOrders'];
		...
		$payType=$data[0]['Activity_Report_APP']['payment_mode_name'];
            $resp["Qry5"]=$PAYAmount;
		if($PAYAmount!="") {
			$sql = "exec saveAppVanPayment '" . $sfCode . "',''," . $custCode . ",$PAYAmount,$payType,'". date('Y-m-d H:i:s') . "','','$Owndiv','" . $pk . "','".$eKeyID."'";
			$resp["Qry3"]=$sql;
			$res=performQuery($sql);
		}
}
...
RetailCasePrice, MRP_Price,VanSale_Price, Product_Detail_Code from vwProductStateRates where Division_code='" . $Owndiv . "' and State_Code=$State_Code";
		...
		case "get/distributorRate":
				...
				$State_Code=$head[0]["State_Code"];
				 ...
				 //$query = "exec getAppProdNative '" . $sfCode . "'";
				  $query = "select Product_Detail_Code from Mas_Product_Detail where Division_Code='" . $Owndiv . "' and Product_Active_Flag='0' and CHARINDEX(','+'".$State_Code."'+',' ,','+cast(State_Code as varchar)+',')>0";
                ...
                $productCode = $results[$i]['Product_Detail_Code'];
                    ...
                    for($j=0; $j<count($distList); $j++)
                    {
                        ...
                        if($priceListCode!=null && $priceListCode!=''){
							...
							$Division_Code=$Owndiv;
$query1 = "select isnull(Distributor_Price, '0') SS_Base_Rate,isnull(SS_Case_Rate, '0') SS_Case_Rate,State_Code,Division_Code,Distributor_Price,Retailor_Price,DistCasePrice,
RetailCasePrice, MRP_Price,VanSale_Price, Product_Detail_Code from vwProductStateRates where Product_Detail_Code='". $productCode ."' and Division_code='" . $Owndiv . "' and State_Code=$State_Code";
 $dist = performQuery($query1);
 if($dist[0]['SS_Base_Rate']!=null && $dist[0]['SS_Base_Rate'] !='')
 $SS_Base_Rate = $dist[0]['SS_Base_Rate'];
if($dist[0]['SS_Case_Rate']!=null && $dist[0]['SS_Case_Rate'] !='')
$SS_Case_Rate=$dist[0]['SS_Case_Rate'];
if($dist[0]['Distributor_Price']!=null && $dist[0]['Distributor_Price'] !='')
$Distributor_Price=$dist[0]['Distributor_Price'];
if($dist[0]['Retailor_Price']!=null && $dist[0]['Retailor_Price'] !='')
$Retailor_Price=$dist[0]['Retailor_Price'];
if($dist[0]['DistCasePrice']!=null && $dist[0]['DistCasePrice'] !='')
$DistCasePrice=$dist[0]['DistCasePrice'];
if($dist[0]['RetailCasePrice']!=null && $dist[0]['RetailCasePrice'] !='')
$RetailCasePrice=$dist[0]['RetailCasePrice'];
if($dist[0]['MRP_Price']!=null && $dist[0]['MRP_Price'] !='')
$MRP_Price=$dist[0]['MRP_Price'];
if($dist[0]['VanSale_Price']!=null && $dist[0]['VanSale_Price'] !='')
$VanSale_Price=$dist[0]['VanSale_Price'];
if($dist[0]['Product_Detail_Code']!=null && $dist[0]['Product_Detail_Code'] !='')
$Product_Detail_Code=$dist[0]['Product_Detail_Code'];
if($dist[0]['State_Code']!=null && $dist[0]['State_Code'] !='')
$State_Code=$dist[0]['State_Code'];
if($dist[0]['Division_Code']!=null && $dist[0]['Division_Code'] !='')
$Division_Code=$dist[0]['Division_Code'];

$results[$i]['SS_Base_Rate'] = $SS_Base_Rate;
$results[$i]['SS_Case_Rate'] = $SS_Case_Rate;
$results[$i]['Distributor_Price'] = $Distributor_Price;
$results[$i]['Retailor_Price'] = $Retailor_Price;
$results[$i]['DistCasePrice'] = $DistCasePrice;
$results[$i]['RetailCasePrice'] = $RetailCasePrice;
$results[$i]['MRP_Price'] = $MRP_Price;
$results[$i]['VanSale_Price'] = $VanSale_Price;
$results[$i]['Product_Detail_Code'] = $Product_Detail_Code;
$results[$i]['State_Code'] = $State_Code;
$results[$i]['Division_Code'] = $Division_Code;
$results[$i]['distList'] = $distList;
...
break;
	case "updateDispatchFlag":
 	$Trans_Detail_Slno= $_GET['Trans_Detail_Slno'];
	$query = "update vwActivity_MSL_Details  set Order_Flag='1' where Trans_Detail_Slno='". $Trans_Detail_Slno ."'";
	performQuery($query);
	$result['success'] = true;
    outputJSON($result);
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
                ...
                $current = performQuery($query);
				 $query = "select  TOP 1*  from Trans_TP where SF_Code='$sfCode' and Tour_Month=month(getdate())+1 and Tour_Year=year(getdate())  and
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' UNION
               select  TOP 1*  from Trans_TP_One where SF_Code='$sfCode' and Tour_Month=month(getdate())+1 and Tour_Year=year(getdate()) and Confirmed=1 and
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
    ...
    case "dcr/updateEntry":
     ...
     case "dcr/updateProducts":
	  ...
	  $no_of_items = $data['No_Of_items'];
		...
		$resp=array();
		$resp["OrderType"] = $OrderType;
		if($OrderType== "3"){
			$delqry="delete from Trans_VanStock_Ledger where RefNo='" .  $Trans_Sl_No . "' and from_loc='" . $sfCode . "' and Division_Code='". $Div_ID."'";
            $resp["delqry"] = $delqry;
            performQuery($delqry);
		   
		}
	   /* $lqry="select count(RefNo) Cnt from Trans_VanStock_Ledger where RefNo='" .  $Trans_Sl_No . "' and from_loc='" . $sfCode . "' and Division_Code='". $Div_ID."'";
		$tt=performQuery($lqry);
		$resp["count"]=$tt[0]['Cnt'];
		if($tt[0]['Cnt']>0){
			$delqry="delete from Trans_VanStock_Ledger where RefNo='" .  $Trans_Sl_No . "' and from_loc='" . $sfCode . "' and Division_Code='". $Div_ID."'";
            $resp["delqry"] = $delqry;
...
//PCode='" . $transOrderDetails[$i]['product'] . "' 
 $sql3 = "SELECT isNull(max(cast(Legr_ID as numeric)),0)+1 as MaxRwID FROM Trans_VanStock_Ledger";
								$tRww = performQuery($sql3);
								$MaxRwID = (int) $tRww[0]['MaxRwID'];
$insql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm,RefNo)  select '" . $MaxRwID . "' , '" . $sfCode . "','F' ," . $custCode . ",'R' , '" . $orderDate . "','" . $transOrderDetails[$i]['product'] . "',$qty,'". $Div_ID."','VanInv','0','" . $docor_Name . "','" . $pk . "'";
								 performQuery($insql);	
 ...
 $resp["insql"] = $insql ;								 
    ...
    case "vwLeave":
        $sfCode = $_GET['sfCode'];
        $sql = "select Leave_Id,Reason,Address,FieldForceName,Reporting_To_SF,Designation,HQ,EmpCode,Reporting_To_SF,Designation,cast(LeaveDays as int) LeaveDays,Division_Code,sf_Designation_Short_Name
 ...
 Reject_reason,Aproval_Flag from Trans_Gift_Claim where Sf_Code ='" . $sfCode . "'";
    ...
    case "vwChkExpenseApprovalOne":
        ...
        /*$sql = "select Dcr_Missed_Date,CONVERT(VARCHAR(10), Dcr_Missed_Date, 103) missed_date,s.sf_code,s.sf_name,sl_no,Deviation_Reason reason from DCR_MissedDates d
			...
			(select ROW_NUMBER() over(partition by sf_code,cast(Activity_Date as date) order by sf_code,cast(Activity_Date as date)) rw , sf_code, Activity_Date, Deviation_Reason  from DCR_TPDev_Reason) r on r.sf_code=d.sf_code and
...
break;
   ...
   break;
	...
	case "get/inshopClStock":
		outputJSON(getInshopClStock());
	break;
	case "get/inshopOpStock":
		outputJSON(getInshopOpStock());
	break;
	case "get/inshopOrdValue":
		outputJSON(getInshopOrdValue());
	break;
	case "get/inshopClValue":
		outputJSON(getInshopClValue());
	break;
	
	case "get/prireturndata":
		outputJSON(getPriReturnData());
	break;
	case "get/mnprimaryreturn":
		outputJSON(getMnPriReturnData());
	break;
    case "get/PendingPaymentDets":
          	outputJSON(getPendingPaymentDets());
			break;
case "get/PendingPaymentDets_mod":
          	outputJSON(getPendingPaymentDets_Mod());
			break;
    case "get/collectpaymentlist":
          outputJSON(getCollectPaymentList());
		break;
	case "get/vanfieldforce":
          outputJSON(getVanFieldForce());
		break;
	...
	case "get/collectvanpaymentlist":
          outputJSON(getVanCollectPaymentList());
		break;
    case "get/collectvanpaymentdets":
          outputJSON(getVanCollectPaymentDets());
		break;
...
/*if($sfCode=="MR3532"){
  ...
  if($_GET["dcr_activity_date"]!=null && $_GET["dcr_activity_date"]!=''){
    ...
    case "get/retailer":
		...
		case "get/Cusretailer":
		  ...
		  $queryGrp="select FieldGroupId,FGroupName,FGTableName from Mas_FieldGroupTable where ModuleId='3' ";
          $cus_view_query = "select ModuleId ModuleId,Field_Name Field_Name,Fld_Type Fld_Type,Fld_Symbol Fld_Symbol,Field_Col Field_Col,Fld_Length Fld_Length,Mandate Mandate,flag flag,Fld_Src_Name Fld_Src_Name,Fld_Src_Field Fld_Src_Field,FGTableName,FieldGroupId
          from Trans_Custom_Fields_Details where Div_code = '$div_code' and ModuleId = '3' and AccessPoint='1'";
		  
		  $cusviewlist=performQuery($cus_view_query);
		  for($j=0; $j<count($cusviewlist); $j++){
		     $fgtablename = $cusviewlist[$j]['FGTableName'];
		    $fgcolumnname = $cusviewlist[$j]['Field_Col'];
			 $data_value="";
			if($fgtablename!=null && $fgtablename!=''){
                   $valQuery = "select isnull($fgcolumnname,'')$fgcolumnname from $fgtablename  where Retail_code ='" . $id . "'";
                   $valuess = performQuery($valQuery);
					if(count($valuess)>0){
							
                        if($valuess[0]["$fgcolumnname"]!=null && $valuess[0]["$fgcolumnname"] !=''){
                           $data_value = $valuess[0]["$fgcolumnname"];
					    }
				    
					}
            }
							
            $cusviewlist[$j]['data_value'] = $data_value;
		 ...
		 $result['customGrp'] = performQuery($queryGrp);
         $result['Static_Data'] = performQuery($query);
         $result['Dynamic_View'] =$cusviewlist;
 ...
 case "get/Deactivate":
    ...
    case "get/misseddates":
        ...
        $query = "update TP_Attendance_App set Duty_Mode=$day,Rmks='" . $data["remarks"] . "',End_Time='" .  date("Y-m-d h:i:sa"). "' ,End_Lat= '" . $data["Lattitude"] . "' ,End_Long ='" . $data["Langitude"] . "' ,day_end_km='" . $data["day_end_km"] . "' where  Sf_Code='" . $sfCode . "'  and login_date='".date('Y-m-d')."'";
        performQuery($query);
			 //}
...
//"redeem_points":221, "redeem_flag":"1", "redeem_delivery_date":"2021-11-01", "redeem_cancel_date":"2021-11-01"}
	...
	case "get/paymentmode":
    	...
    	break;
	case "get/retAddress":
		outputJSON(getRetAddress());
		break;

	case "get/insertRetAddress":
		 outputJSON(insertRetAddress());
		break;

	case "get/updateRetAddress":
	   outputJSON(updateRetAddress());
		break;

	case "get/deleteRetAddress":
		outputJSON(deleteRetAddress());
		break;

   case "get/paymentResponse":
		$transactionAmt=$_POST['rate'];
		$orderID=$_POST['orderId'];
		$mobile=$_POST['mob'];
		$email=$_POST['email'];
		$transactionID=$orderID.date('YmdHis');
		$checksumKey = "G3eAmyVkAzKp8jFq0fqPEqxF4agynvtJ";
		//$msg="AIRMTST|ARP1523968042763|NA|2|NA|NA|NA|INR|NA|R|airmtst|NA|NA|F|NA|NA|NA|NA|NA|NA|NA|https://uat.billdesk.com/pgidsk/pgmerc/pg_dump.jsp";
		$msg="BDSKUATY|$transactionID|NA|$transactionAmt|NA|NA|NA|INR|NA|R|bdskuaty|NA|NA|F|$mobile|$email|NA|NA|NA|NA|NA|https://govind.sanfmcg.com/server/dms_db_V2.php";
		$CP_MSG = null;
		//echo getCheckSumSHA256($msg,$checksumKey);
		$finalmsg = $msg . "|" . getCheckSumSHA256($msg,$checksumKey);
		/*$url='https://www.billdesk.com/bdcs/BDCSMerchantHandler';
		$ch = curl_init();
		curl_setopt( $ch, CURLOPT_URL, $url );
		curl_setopt( $ch, CURLOPT_POST, true );
		curl_setopt( $ch, CURLOPT_POSTFIELDS, "msg=$finalmsg");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		$result = curl_exec( $ch );
		curl_close( $ch );*/
		$res["msg"]=$finalmsg;
		$res["input"]=$msg;
		$res["token"]="";
		$res["userMobile"]=$mobile;
		$res["userEmail"]=$email;
		outputJSON($res);
	break;
	case "get/paymentResponseLive":
    		$transactionAmt=$_POST['rate'];
    		$orderID=$_POST['orderId'];
    		$mobile=$_POST['mob'];
    		$email=$_POST['email'];
			...
			//$orderID.
    		//$checksumKey = "LTom0r6gUDFj2Lgi5MGCe2Hs0AQmSDVv";
			//$checksumKey = "wcosw7GSef49ZpiDGoQFNoOsmmRduUDS";
			$sql="exec getPaymentData '$orderID'";
			$payemntArr=performQuery($sql);
			if(count($payemntArr)>0)
			{
				$checksumKey=$payemntArr[0]["ChecksumKey"];
				$merchantID=$payemntArr[0]["MerchantID"];
				$merchantIDLowerCase=$payemntArr[0]["MerchantIDLowerCase"];
				//$msg="AIRMTST|ARP1523968042763|NA|2|NA|NA|NA|INR|NA|R|airmtst|NA|NA|F|NA|NA|NA|NA|NA|NA|NA|https://uat.billdesk.com/pgidsk/pgmerc/pg_dump.jsp";
				
				$msg="$merchantID|$transactionID|NA|$transactionAmt|NA|NA|NA|INR|NA|R|$merchantIDLowerCase|NA|NA|F|$mobile|$email|NA|NA|NA|NA|NA|https://govind.sanfmcg.com/server/dms_db_V2.php";
				
				//$msg="BOMGMMROM|$transactionID|NA|$transactionAmt|NA|NA|NA|INR|NA|R|bomgmmrom|NA|NA|F|$mobile|$email|NA|NA|NA|NA|NA|https://govind.sanfmcg.com/server/dms_db_V2.php";
				$CP_MSG = null;
				//echo getCheckSumSHA256($msg,$checksumKey);
				$finalmsg = $msg . "|" . getCheckSumSHA256($msg,$checksumKey);
				/*$url='https://www.billdesk.com/bdcs/BDCSMerchantHandler';
				...
				$res["orderID"]=$orderID;
				$res["transactionAmt"]=$transactionAmt;
				$res["transactionID"]=$transactionID;
				$res["checksumKey"]=$checksumKey;
				$res["Success"]=true;
				$res["msg"]=$finalmsg;
				$res["input"]=$msg;
				$res["token"]="";
				...
				$res["msg"]="Payment Information Not Available";
				$res["Success"]=false;
			}
//    echo str_replace(".000000","",json_encode_unicode($array));
 ...
 case "get/updateResponse":
		...
		$sql = "insert into PaymentResponse (response) values ('". $paymentResponse . "' )";
		performQuery($sql); 
		$res["success"]="true";
		...
		outputJSON($res);
	break;		
		case "save/primarypaymentBillDesk":
        	outputJSON(PrimaryPaymentSaveBillDesk());
        break;
	
	case "get/paymentkeys":
		$Sf_code = $_GET['Stockist_Code'];
		$Div = $_GET['DivCode'];		
		$divs = explode(",", $Div . ",");
		$Owndiv = (string) $divs[0];
		$sql = "select isnull(KeyID,'asdfagag')KeyID,isnull(Key_Secret,'safg')Key_Secret,Stockist_Code from Mas_Stockist st left outer join Mas_Payment_Keys ky on ky.Sl_No=st.Payment_Key_ID where Stockist_Code='$Sf_code'";
		$res = performQuery($sql);
		if (count($res) > 0) {
			$result["Data"]=$res;
			$result["success"]=true;
	...
	case "get/CustomDetails":
		...
		$queryGrp="select FieldGroupId,FGroupName,FGTableName from Mas_FieldGroupTable where ModuleId='$moduleId' ";
		$query = "select ModuleId ModuleId,Field_Name Field_Name,Fld_Type Fld_Type,Fld_Symbol Fld_Symbol,Field_Col Field_Col,Fld_Length Fld_Length,Mandate Mandate,flag flag,Fld_Src_Name Fld_Src_Name,Fld_Src_Field Fld_Src_Field,FieldGroupId,FGTableName
		from Trans_Custom_Fields_Details where Div_code = '$div_code' and ModuleId = '$moduleId' and AccessPoint='1'";
		$result['customGrp'] = performQuery($queryGrp);
		$result['customData'] = performQuery($query); 
		outputJSON($result);
	break;
	case "get/CustomModuleType":
	    $div = $_GET['divisionCode'];
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
		$StateCode=$_GET['State_Code'];
		
	    $query="select * from Mas_ModuleTable where ModuleId > 4";
		$results = performQuery($query);
		outputJSON($results);
	 ...
	 $results;
	  $sfCode = $_GET['sfCode'];
		...
		$fieldName=$_GET['fieldName'];
		 switch ($masterName) {
			case "Mas_Department":
			$query="select $fieldName from Mas_Department where Division_Code='$Owndiv'";
			$result = performQuery($query);
		    outputJSON($result);
			 break;
			case "Mas_District":
			$query="select $fieldName from Mas_District where Div_Code='$Owndiv'  and State_Code='$StateCode'";
			$result = performQuery($query);
		     outputJSON($result);
			 break;
			case "Mas_Division":
			$query="select $fieldName from Mas_Division where Division_Code='$Owndiv' and charindex(','+'$StateCode'+',',State_Code)>0";
			$result = performQuery($query);
		     outputJSON($result);
			 break;
			case "Mas_HQuarters":
			$query="select $fieldName from Mas_HQuarters where Division_Code='$Owndiv'";
			$result = performQuery($query);
		     outputJSON($result);
			 break;
			case "Mas_Salesforce":
			$query="select $fieldName from Mas_Salesforce where Division_Code='$div' and State_Code='$StateCode'";
			$result = performQuery($query);
		     outputJSON($result);
			 break;
			case "Mas_SF_Designation":
			$query="select $fieldName from Mas_SF_Designation where Division_Code='$Owndiv'";
			$result = performQuery($query);
		     outputJSON($result);
			 break;
			case "Mas_State":
			$query="select ms.State_Code,ms.StateName from Mas_Division md left join Mas_State ms on CHARINDEX(','+cast(ms.State_Code as varchar)+',',','+md.State_Code+',')>0 where md.Division_Code='$Owndiv'";
			$result = performQuery($query);
		    outputJSON($result);
			break;
			case "Mas_Territory":
			$query="select $fieldName from Mas_Territory where Div_Code='$Owndiv'";
			$result = performQuery($query);
		    outputJSON($result);
			break;
			case "Mas_Territory_Creation":
			$query="select $fieldName from Mas_Territory_Creation where Division_Code='$Owndiv'";
			$result = performQuery($query);
		    outputJSON($result);
			break;
	...
	case "get/customformdatadetail":
		  ...
		  for($j=0; $j<count($cusviewlist); $j++){
		    ...
		    $fgcolumnname = $cusviewlist[$j]['Field_Col'];
			 $data_value="";
			if($fgtablename!=null && $fgtablename!=''){
                   $valQuery = "select isnull($fgcolumnname,'')$fgcolumnname from $fgtablename  where EntryBy ='" . $id . "' and EntryId ='" . $entryid. "'";
                   $valuess = performQuery($valQuery);
					if(count($valuess)>0){
		...
		case "get/CustomEntryId":
		$sfCode = $_GET['sfCode'];
        $moduleId = $_GET['moduleId'];
        $fromDt = $_GET['fromDate'];
		$toDt = $_GET['toDate'];
		//$querys="exec getCustomEntryId '".$moduleId."','".$fromDt."','".$toDt."','".$sfCode."'";
		
		$query="select Top 1 FGTableName from Mas_FieldGroupTable where moduleId='".$moduleId."' order by FieldGroupId";
		$res=performQuery($query);
		$tbnm =$res[0]['FGTableName'];
		$qry="select EntryId from  $tbnm where EntryBy='".$sfCode."' and cast(EntryDate as date) between '".$fromDt."' and '".$toDt."'";
		$result = performQuery($qry);
		outputJSON($result);
