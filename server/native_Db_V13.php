...
function getInshopStockList(){
  ...
  $divCode="select Division_Code from Mas_Salesforce where Sf_Code='".$Sf_Code."'";
  $tRw = performQuery($divCode);
  $div =$tRw[0]['Division_Code'];
  $divs = explode(",", $div . ",");
  $Owndiv = (string) $divs[0];
  $query = "exec getInshopDet '".$Sf_Code."','" . $slNo . "','".$Owndiv."'"; 
  return performQuery($query);
...
$query="select dcr.Trans_SlNo,dcr.Trans_Detail_Slno,dcr.Trans_Detail_Name,dcr.Trans_Detail_Info_Code,dcr.SDP_Name,dcr.POB,dcr.Worked_with_Name,dcr.StartOrder_Time,head.Trans_Sl_No,head.DCR_Code,head.Stockist_Code,head.Order_Value,head.Order_In_Time,head.Order_Out_Time,dcr.Activity_Remarks,isnull(aec.imgurl,'')audioName,isnull(head.finalNetAmnt,'') finalNetAmnt,isnull(head.tlDisPercnt,0) tlDisPercnt,isnull(head.tlDisAmt,0) tlDisAmt,isnull(head.OrderType,0) OrderType from DCRDetail_Lst_Trans dcr left outer join Trans_Order_Head head on dcr.Trans_Detail_Slno=head.DCR_Code left join Activity_event_captures aec on aec.DCRDetNo=dcr.Trans_Detail_Slno and  aec.Identification='audio' where dcr.Trans_SlNo='".$Tran_Slno."' ";
return performQuery($query);
}
...
function getInshopDetails(){
 ...
 $sl_no=$_GET['slNo'];
  ...
  $divs = explode(",", $div . ",");
  $Owndiv = (string) $divs[0];
 $sql="exec getInshopDet '".$Sf_Code."','".$sl_no."','".$Owndiv."' ";
...
function actionLogin() {
    ...
    if ($res) {
		...
		performQuery($query);
$deviceIdQuery = "select isnull(DeviceId,'') DeviceId from Access_Table where sf_code = '" . $arr[0]['SF_Code'] . "'";
		 $deviceId=performQuery($deviceIdQuery);
		 if($DIVV!="156"){
		 ...
		 if($deviceId[0]['DeviceId']==""&&$DIVV=="156"&&$arr[0]['SF_Code']=="TSRMGR0023"){
		 $updatedeviceIdQuery = "update Access_Table set DeviceId = '". $loginDeviceId ."' where sf_code = '" . $arr[0]['SF_Code'] . "'";
		 performQuery($updatedeviceIdQuery);
		 }
		 if($deviceId[0]['DeviceId']!=""&&$deviceId[0]['DeviceId']!= $loginDeviceId&&$DIVV=="156"&&$arr[0]['SF_Code']=="TSRMGR0023"){
		 $respon['success'] = false;
        $respon['msg'] = "DeviceId Not Match..";
			 ...
			 $respon['TPDCR_Deviation'] = $arr[0]['TPDCR_Deviation'];
			  $respon['MyplnEventCapture'] = $arr[0]['Mypln_Event_Capture'];
               ...
               if(strtolower($code) == "admin"){
                $respon['success'] = false;
                $respon['msg'] = "Admin can login only through web";
               }
			   $sf_status = $arr[0]['SF_Status'];
               if($sf_status == 2){
                $respon['success'] = false;
                $respon['msg'] = "Account is Deactivated please contact admin";
               }elseif($sf_status == 1){
                $respon['success'] = false;
                $respon['msg'] = "Account is Vacant please contact admin";
               }elseif ($sf_status == 3) {
                $respon['success'] = false;
                $respon['msg'] = "Account is Blocked please contact admin";
               }
        return outputJSON($respon);
    } else {
        $respon['success'] = false;
        $respon['msg'] = "Invalid User and Password";
        return outputJSON($respon);
    }
}
...
$Owndiv = (string) $divs[0];
$sfCode = $_GET['sfCode'];
$query = "select  C.ListeddrCode cust_code,C.Listeddr_Name cust_name, GiftSlabID id,month(From_Date) Month, GiftMinVal MinVal,GiftMaxVal MaxVal,
...
GiftDesc Desc_Name,CONVERT(varchar,To_Date, 101)To_Date,CONVERT(varchar,From_Date, 101) From_Date
 ...
 where Doctor_Active_flag=0 and charindex(',$sfCode,',','+Field_Code+',')>0 and isnull(Claim_deadline,convert(date,getdate()))>=convert(date,getdate()) and claim_tab='S'";
return performQuery($query);
}
...
function GiftEnrolment() {
  ...
  $sfCode = $_GET['sfCode'];
	...
	$query = "select  C.ListeddrCode cust_code,C.Listeddr_Name cust_name, GiftSlabID id,month(From_Date) Month, GiftMinVal MinVal,GiftMaxVal MaxVal,
year(From_Date)GiftYear, GiftName name,isnull(Claim_deadline,convert(date,getdate())) Claim_deadline , 
DATEDIFF(day, getdate(), isnull(Claim_deadline,convert(date,getdate())))daydif, 
GiftDesc Desc_Name,CONVERT(varchar,To_Date, 101) To_Date,CONVERT(varchar,From_Date, 101) From_Date
 from  Mas_Gift_Slab S
 inner join Gift_Map_Customer C on c.Slab_Id=S.GiftSlabID
 inner join vwDoctor_Master_APP D with(nolock) on D.Doctor_Code=C.ListeddrCode
 where Doctor_Active_flag=0 and charindex(',$sfCode,',','+Field_Code+',')>0 and isnull(Claim_deadline,convert(date,getdate()))>=convert(date,getdate()) and claim_tab='P'";
   return performQuery($query);
}
...
function getDailyAllow() {
    ...
    $Owndiv = (string) $divs[0];
	 $sfCode = $_GET['sfCode'];
	$date=date('Y-m-d');
    $query = "select ID,Allowance_Name Name,isnull(Photo_Nd,0)Photo_Nd,isnull(Photo_Mandatory,0)Photo_Mandatory from Mas_Allowance_Type where Type=1 and Division_Code=". $Owndiv ." and user_enter=1 and active_flag=0";
    ...
    $order_no = $_GET['orderNo'];
	 $stockist = $_GET['stockist_code'];
	  $sf = $_GET['sfCode'];
       ...
       $query = "exec spGetVstSSOrderDetApp'" . $order_date . "','" . $order_no . "','" . $stockist . "','" . $sf . "'";
	  // else
    //$query = "exec spGetVstOrderDetApp_dms '" . $order_date . "','" . $order_no . "','" . $stockist . "','" . $sf . "'";
    return performQuery($query);
}
...
group by Product_Brd_Name";
 ...
 $lastprevorders['last_brands']=$brands1;
		...
		$query = "select ListedDrCode,Doc_Special_Code,ListedDr_Mobile,ListedDr_Phone  from Mas_ListedDr with(nolock)  where ListedDrCode='".$MSL."' ";
...
function getPreCallDet() {
	...
	$rvtQuery = "select top 1 convert(varchar, Last_Visit_Date,101) Last_Visit_Date, convert(varchar,Last_Order_Date,101) Last_Order_Date from retailer_visit_time where RetailerCode='" . $MSL . "' ";
	 if (count($as) > 0&& $as[0]['Division_Code']=="195"){
		 $rvtQuery = "select top 1 remarks,convert(varchar, Last_Visit_Date,101) Last_Visit_Date, convert(varchar,Last_Order_Date,101) Last_Order_Date from retailer_visit_time where RetailerCode='" . $MSL . "' ";
	 }		 
    ...
    if (count($as) > 0) {
		...
		$query="select top 1 Order_Value,Trans_Sl_No,stockist_code from trans_order_head  where sf_code='".$SF."' and Cust_Code='".$MSL."'    order by Order_Date desc";
...
//$query = "select *,case when (isnull(contactperson,'') +','+ isnull(contactperson2,'')+','+ isnull(contactperson3,'')=',,') then (' ') else isnull(contactperson,'') +','+ isnull(contactperson2,'')+','+ isnull(contactperson3,'') end  as ContactPersionn  from Mas_ListedDr with(nolock)  where ListedDrCode='".$MSL."' ";
       ...
       /* if (count($collectionOrder) > 0) {
           ...
           if (count($LASTORDER) > 0) {
                ...
                $query1 = "select Product_Code,Product_Name,Quantity,value from Trans_Order_details where Trans_Sl_No= '".$LASTORDER[0]['Trans_Sl_No']."'";
                $RES = performQuery($query1);
                $stockist[] = array(
                    "stockist_code" => $LASTORDER[0]['stockist_code'],
                   // "Sum" => $transHead[$i]['Order_Value'],
                   // "Avg" => floor($transHead[$i]['Order_Value'] / 6),
                   // "LastOrderAmt" => $collectionOrder[$i]['Last_Order_Amt'],
                  //  "outStandingAmt" => $collectionOrder[$i]['Out_standing_Amt'],
                    "OrderDetails" => $RES
                );
	...
	if (count($as) > 0&& $as[0]['Division_Code']=="195")
	$result['last_order_remarks'] =(string) $rvtQueryRes[0]['remarks'];
...
function getEntryCount() {
    $sfCode = $_GET['sfCode'];
    $today = date('Y-m-d 00:00:00');
	  if($_GET["dcr_activity_date"]!=null && $_GET["dcr_activity_date"]!=''){
                    $today=str_replace("'", "", $_GET["dcr_activity_date"]);
            }
						//$today="2023-04-02";
    $results = array();
...
function getFromTableWR($tableName, $coloumns, $divisionCode, $sfCode = null, $orderBy = null, $where = null, $join = null, $today = null, $wt = null) {
    ...
    if (!is_null($today)) {
        $today = date('Y-m-d 00:00:00');
		  if($_GET["dcr_activity_date1"]!=null && $_GET["dcr_activity_date1"]!=''){
                    $today=str_replace("'", "", $_GET["dcr_activity_date1"]);
            }
...
function getFromTable($tableName, $coloumns, $divisionCode, $sfCode = null, $orderBy = null, $where = null, $join = null, $today = null, $wt = null, $desig) {
    ...
    if (!is_null($where)) {
       // $query.=" and " . join(" and ", $where);
	    ...
	    $query.=" and " . join(" and ", str_replace("3","6",$where));
	}else{
                  $query.=" and " . join(" and ", $where);
	}
    ...
    if (!is_null($today)) {
        ...
        $query.=" and cast(tab.activity_date as date)=cast('$today' as date)";
    }
    if (!is_null($orderBy)) {
        $query .=" ORDER BY " . join(",", $orderBy);
    }
...
function orderDetailsDelete($sfCode, $custCode,$arc, $amc,$sec) {
	...
	if($sec!="1"){
		 $sql = "select Trans_Sl_No,Order_Value,Collected_Amount,OrderType,Div_ID from Trans_Order_Head where DCR_Code ='$amc'";
		 
	}else{
       $sql = "select Trans_Sl_No,Order_Value,Collected_Amount,OrderType,Div_ID from Trans_Order_Head where Trans_Sl_No ='$amc'";
    ...
    for ($t = 0; $t < count($trDet); $t++) {
		...
		$OrderType = $trDet[$t]['OrderType'];	
	    $DivID=$trDet[$t]['Div_ID'];
        ...
        $tr = performQuery($sql);
		 if($OrderType== "3"){	
			$delqry="delete from Trans_VanStock_Ledger where RefNo='" .  $Trans_Sl_No . "' and from_loc='" . $sfCode . "' and Division_Code='". $DivID."'";	
...
function addEntry() {
    ...
    switch ($temp[0]) {
        case "stockUpdation":
            ...
            if($sfCode!="undefined")
            {
                ...
                for ($i = 0; $i < count($vals); $i++)
                {
                   ...
                   $batch_no=($vals[$i]['batch_no']==null? '':$vals[$i]['batch_no']);
                    ...
                    $sql = "select isnull(cast(Distributor_Price as float),0) Distributor_Price,isnull(cast(Retailor_Price as float),0) Retailor_Price from vwProductStateRates where State_Code=$stateCode and Product_Detail_Code='" . $productCode . "'";
                    $tr = performQuery($sql);
                    if (empty($tr)) {
                            $distPrice = 0;
                            $retPrice = 0;
            ...
            echo json_encode($resp);closeConn();
            die;
            break;
...
case "SSstockUpdation":
        ...
        case "tbMyDayPlan":
         ...
         if( $vals['profilepic']!=null && $vals['profilepic']!="undefined")
			...
			$sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,
worked_with_name,dcrtype,location,CustId,CustName,StkName,Sprstk,Other_Works,Appversion,Place_Inv) 
					select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . ",'" . str_replace("'","", $vals["remarks"]) . "','" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."' ,'".$vals["superstockistid"]."' ,'".$OtherWors."','".$Appversion."' , '".$Place_Inv."'";
			performQuery($sql);
        ...
        case "unlisted_doctor_master":
            ...
            $flagofretailerND= performQuery($sqlrnd)[0]['Retailer_add_ND'];
			 ...
			 $sql="select State_Code from Mas_Salesforce where Sf_Code='".$sfCode."'";
			 $tr1=performQuery($sql);
			 $stateCode=$tr1[0]['State_Code'];
...
if($vals['Retailerphoto']!=null && $vals['Retailerphoto']!="undefined")
            {
                   ...
                   $visitpath=$sfCode.'_'.''.$vals['Retailerphoto']['imgurl'];
                    ...
                    $sql = "insert into Activity_Event_Captures(Activity_Report_Code,DCRDetNo,imgurl,Division_Code,Identification,Insert_Date_Time) values( '$sfCode','" . $stateCode . "','" . $vals['Retailerphoto']['imgurl'] . "','" . $Owndiv . "','Retailer Pic','".$today."')";
                    performQuery($sql);
	 ...
	 die;
     break;
     case "unlisted_doctor_master_new":
            if($vals["unlisted_cat_code"]==null)$vals["unlisted_cat_code"]=0;
            $Ukey=$vals['unlisted_doctor_name'].date('Y-m-d H:i:s');
...
if($vals['Retailerphoto']!=null && $vals['Retailerphoto']!="undefined")
		...
		$sql = "{call  svListedDR_APP_Native_New('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . ",'" . str_replace("'","",$vals['unlisted_doctor_phone']) . "','" . str_replace("'","",$vals['unlisted_doctor_cityname']) . "','" . str_replace("'","",$vals['unlisted_doctor_areaname']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson']) . "', ' " . str_replace("'","",$vals['unlisted_doctor_designation']) . "','" . str_replace("'","",$vals['unlisted_doctor_gst']) . "'," . $vals["unlisted_doctor_stateid"] . ",'" . str_replace("'","",$vals['unlisted_doctor_pincode']) . "'," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",$ListedDr_Active_Flag,'" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ",'" . str_replace("'","",$vals['unlisted_doctor_phone2']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_designation2']) . "', '" .str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','','" . $visitpath . "',".$Lat.",".$Long.",'" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_phone3']) . "' ,'" . str_replace("'","",$vals['unlisted_doctor_email']) . "','" . str_replace("'","",$vals['ListedDr_DOB']) . "','" . str_replace("'","",$vals['ListedDr_DOW']) . "','" . str_replace("'","",$vals['unlisted_doctor_address3']) . "','','','','". $vals['geoTaggingAddress'] . "','" . $vals['Retailerphoto']['imgurl'] . "','" . $vals['Retailerphoto']['unlisted_doctor_keyoutlet'] . "','" . str_replace("'","",$vals['unlisted_doctor_statename']) . "','" . str_replace("'","",$vals['unlisted_doctor_stateid']) . "'  )}";
		performQueryWP($sql,[]);
...
if(count($processUser)==0)
$results=array();
  echo json_encode($results);closeConn();
    die;
                break;
...
$product_unit_conv=$samp[$j]["product_unit_conv"];
  ...
  $sql="insert into Trans_Pop_Ledger(Sf_Code,Stock_To,Pop_Code,Qty,Stock_Type,Ledg_Date,Ledg_Time) values('".$SfCode."','".$SfCode."','".$productCode."','".$Qty."','".$StockType."','".$OrderDate."','".$OrderTime."')";
                   performQuery($sql);
...
$sql="insert into Trans_Pop_Stock_Loading Values('".$SfCode."','".$Owndiv."','".$StockDate."','".$StockTime."','".$productCode."','".$Stock_Qty."','".$StockDate."')";
                    ...
                    $sqlCode="insert into Trans_Pop_Ledger(Sf_Code,Stock_To,Pop_Code,Qty,Stock_Type,Ledg_Date,Ledg_Time) Values('".$SfCode."','".$SfCode."','".$productCode."','".$Stock_Qty."','".$StockType."','".$StockDate."','".$StockTime."')";
	...
	case "ProductAvailablity":
      ...
      $sql = "{call  svDCRMain_App(?,?,'" . $worktype . "','" . $cluster  . "',?,'PrimaryCLosing',?,'0')}";
        ...
        case "New_TP_Attendance":
		   ...
		   $srtEntry = $vals['srtEntry'];
		   $attach_need = $vals['attach_need'];
           $update = $_GET['update'];
           $div = $vals['division_code'];
           $today = str_replace("'", "", $vals['date_time']);
		   ...
		   $milli_sec = $vals['milli_sec'];
            ...
            if($update==0){
                ...
                $insertQuery = "exec INSERT_ExpenseStartAct '$sfCode','$div','$dateTime','$day_start_km','$img_url','$mod_id','$from_place','$to_place','$mod_name','$daily_allowance','$to_place_id','$today','$driver_allowance','$stEndNeed','$milli_sec','$srtEntry','$attach_need'";
...
$sql="select * from mas_leave_form where sf_code='$sfCode' and '$entryDate' between from_date and to_date and Leave_Active_Flag!=1";

				//print_r($sql);die;
		$leave=performQuery($sql);
			if(count($leave)>0){
				$resp["msg"]= "Leave Already Applied";
				$resp["Query"]= $sql;
				$resp["success"] = true;
				echo json_encode($resp);closeConn();
				die;
...
if($val['exp_auto'] == 2){
	 $query = "update Expense_Start_Activity set Start_Km = '".$val["srt_km"]."', End_Km = '".$val["end_km"]."' where CONVERT(date,Date_Time) = '".$val["Enterdate"]."' and Sf_code = '$sfCode'";
	 $sql = "exec Post_Expense_Common '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$val["MOT"]."','".$Billamount."','".$stayingtype."','0','".$val["mot_id"]."','".$val["st_endNeed"]."','".$val["max_km"]."','".$val["fuel_charge"]."','".$val["exp_km"]."','".$val["exp_amount"]."','".$val["period_name"]."','".$val["period_id"]."','".$val["from_date"]."','".$val["to_date"]."'";
}
else if($Owndiv=="156"){
	 $sql ="exec Post_Expense_Tsr '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$val["MOT"]."','".$Billamount."','".$stayingtype."','".$dailyEnteredExp."'";
           ...
           $resp["success"] = true;
			...
			if($resp["msg"] == 'Expense Submitted Successfully')
			{
				...
				for ($i = 0; $i < count($vals); $i++)
				{
                    $sql = "insert into Trans_Daily_User_Expense(SF_Code,eDate,expCode,expName,Amt,Division_Code,LUpdtDate,Image_Url,Remarks) select '" . $sfCode . "','". $val["Enterdate"]. "','" . $vals[$i]["ID"] . "','" . $vals[$i]["Name"]  . "','" . $vals[$i]["amt"] . "','" . $Owndiv . "',getDate(),'" . $vals[$i]["imgData"] . "','" . $vals[$i]["exp_remarks"] . "'" ;
                    performQueryWP($sql,[]);
				}
				$sql = "SELECT DeviceRegId FROM Access_Table where sf_code=(select Reporting_To_SF from mas_salesforce where Sf_Code='$sfCode')";
				$device = performQuery($sql);
 ...
 if( $data[2]['ActivityCaptures']!=null && $data[2]['ActivityCaptures']!="undefined")
            {
                ...
                for ($j = 0; $j < count($Event_Captures); $j++) {
                    ...
                    $ev_remarks=str_replace("'", "",$Event_Captures[$j]["remarks"]);
                    $visitpath=$sfCode.'_'.''.str_replace("'", "",$Event_Captures[$j]["imgurl"]);
                    $sql = "insert into Activity_Event_Captures(Activity_Report_Code,imgurl,Division_Code,Identification,Insert_Date_Time,Dateandtime) values( '".$sfCode."','" . $Event_Captures[$j]["imgurl"] . "','".$Owndiv."','Expense','".$dateTime."','".$val["Enterdate"]."')";
                    performQuery($sql);
                }
            }
...
if($val['exp_auto'] == 2){
	 $query = "update Expense_Start_Activity set Start_Km = '".$val["srt_km"]."', End_Km = '".$val["end_km"]."' where CONVERT(date,Date_Time) = '".$val["Enterdate"]."' and Sf_code = '$sfCode'";
	 if($val['exp_process_type'] == 1){
		 $sql = "exec Post_Expense_Common_Daily '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$val["MOT"]."','".$Billamount."','".$stayingtype."','0','".$val["mot_id"]."','".$val["st_endNeed"]."','".$val["max_km"]."','".$val["fuel_charge"]."','".$val["exp_km"]."','".$val["exp_amount"]."','".$val["period_name"]."','".$val["period_id"]."','".$val["from_date"]."','".$val["to_date"]."'";
	 }else{
	    $sql = "exec Post_Expense_Common '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$val["MOT"]."','".$Billamount."','".$stayingtype."','0','".$val["mot_id"]."','".$val["st_endNeed"]."','".$val["max_km"]."','".$val["fuel_charge"]."','".$val["exp_km"]."','".$val["exp_amount"]."','".$val["period_name"]."','".$val["period_id"]."','".$val["from_date"]."','".$val["to_date"]."'";
	 }
}
	 ...
	 $sql ="exec Post_Expense_Tsr '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$val["MOT"]."','".$Billamount."','".$stayingtype."','".$dailyEnteredExp."'";
		...
		//	echo json_encode($resp);
			...
			if($resp["msg"] == 'Expense Submitted Successfully')
			{
				...
				{
                    //$sql = "insert into Trans_Daily_User_Expense(SF_Code,eDate,expCode,expName,Amt,Division_Code,LUpdtDate,Image_Url,Remarks) select '" . $sfCode . "','". $val["Enterdate"]. "','" . $vals[$i]["ID"] . "','" . $vals[$i]["Name"]  . "','" . $vals[$i]["amt"] . "','" . $Owndiv . "',getDate(),'" . $vals[$i]["imgData"] . "','" . $vals[$i]["exp_remarks"] . "'" ;
                     $sql = "exec insertDailyUserExpense '" . $sfCode . "','". $val["Enterdate"]. "','" . $vals[$i]["ID"] . "','" . $vals[$i]["Name"]  . "','" . $vals[$i]["amt"] . "',
                    '" . $Owndiv . "','" . $vals[$i]["imgData"] . "','" . $vals[$i]["exp_remarks"] . "'";
                       performQueryWP($sql,[]);
				}
				$resp["exp_auto"]= $val['exp_auto'];
				if($val['exp_auto'] != 2){
				$sql = "SELECT DeviceRegId FROM Access_Table where sf_code=(select Reporting_To_SF from mas_salesforce where Sf_Code='$sfCode')";
				$device = performQuery($sql);
                $reg_id = $device[0]['DeviceRegId'];
...
$POB=$data[0]['Tour_Plan']['POB'];
		$SOB=$data[0]['Tour_Plan']['SOB'];
            $ww=$data[0]['Tour_Plan']['worked_with'];
				if($data[0]['Tour_Plan']['POB']==null)
              $POB="";
            else
            $POB=str_replace("'", "",$data[0]['Tour_Plan']['POB']);
		if($data[0]['Tour_Plan']['SOB']==null)
              $SOB="";
            else
            $SOB=str_replace("'", "",$data[0]['Tour_Plan']['SOB']);
...
case "Vansalesreject":
 ...
 case "LeaveFormValidate":
	...
	case "DCRMissedDates":
		...
		$sqlm = "insert into tbMyDayPlan (sf_code,Pln_Date,remarks,Division_Code,ClstrName,wtype,FWFlg)  select '" . $sfCode . "','".$vals['misseddate']."','".str_replace("'","", $Remarks)."','" . $Owndiv . "','Missed Entry','" . $Worktype . "','" . $WFlg . "'";
		performQuery($sqlm);
		...
		case "LeaveForm_New":
            ...
            $lvappp=2;
            if($leaveType==1) $lvappp=0;
			$query = "SELECT count(Leave_Id)leave_count from mas_Leave_Form where sf_code = '$sfCode' and Leave_Active_Flag = 2 and
            ((From_date BETWEEN '".$vals['From_Date']."' AND ".$vals['To_Date'].") OR 
            (To_date BETWEEN '".$vals['From_Date']."' AND ".$vals['To_Date'].") OR 
            (From_date <= '".$vals['From_Date']."' AND To_date >= ".$vals['To_Date']."))";
            $result = performQuery($query);
			$is_validate = $result[0]['leave_count'];
			$resp['query'] = $query;
			$resp['count'] = $is_validate;
            if($is_validate == 0){
			if($vals['halfday']==null){
				$halfdayFlag="";
				$halfday="''";
            ...
            $resp["success"] = true;
			$resp["leave_not_valid"] = false;
			$resp["Interval"] = dateDiffInDays("2020-07-01","2020-07-02");
		}else{
            $resp["success"] = true;
            $resp["leave_not_valid"] = true;
            $resp['query'] = $query;
        }
            echo json_encode($resp);
			closeConn();
 ...
 performQuery($sql);
	...
	for ($j = 0; $j < count($Event_Captures); $j++) {
		...
		performQuery($sql);
			...
			$resp["sql"] = $sql;
                echo json_encode($resp);closeConn();die;
            break;
        case "tbRemdrCall":
	...
	$result["Msg"] = $query;
	$result["success"] = true;
   echo json_encode($result);closeConn();die;
			break;
			 case "svSesFeedbk":
            ...
            // if (isset($tableData['Activity_POB_Report'])) {
                    ...
                    if (isset($tableData['Activity_Stk_POB_Report'])){
                        ...
                        for ($j = 0; $j < count($samp); $j++) {
							...
							$stringss = utf8_encode($samp[$j]["product_unit_name"]);
							$unitName=str_replace("\u00a0","",$stringss);
...
if (!empty($data[1]["Activity_Doctor_Report"]['superstockistid']))
 $superstk=$data[1]["Activity_Doctor_Report"]['superstockistid'];
            ...
            if ($vTyp != 0 && $vTyp!=7 && $vTyp != 9) {
                ...
                if ($vTyp == 1)
                    $sqlsp = $sqlsp . "'" . str_replace("'", "", $vals["Town_code"]) . "','" . str_replace("'", "''", $vals["Daywise_Remarks"]) . "',?,'" . str_replace("'", "", $vals["rx_t"]) . "'," . $DetTB["modified_time"] . ",?,?," . $vals["DataSF"] . ",'" . $DetTB["geoaddress"] . "'," . $POB_Value . ",$net_weight_value,".$DetTB['stockist_code'].",".$DetTB['stockist_name'].",$discount_price,'$rateMode','".$DemoTo."',$visit_id,$visit_name,'".$POT."','".$DetTB['retLatitude']."','".$DetTB['retLongitude']."')}";
                else if($vTyp==3){
                    if($DetTB["intrumenttype"]==null)
                        $DetTB["intrumenttype"]="";
                    if($DetTB["date_of_intrument"]==null)
                        $DetTB["date_of_intrument"]="";
                    $sqlsp = $sqlsp . "'" . str_replace("'", "", $vals["Town_code"]) . "','" . str_replace("'", "''", $vals["Daywise_Remarks"]) . "',?,'" . str_replace("'", "", $vals["rx_t"]) . "'," . $DetTB["modified_time"] . ",?,?," . $vals["DataSF"] . ",'" . $DetTB["geoaddress"] . "'," . $POB_Value . ",'" . $DetTB["intrumenttype"] . "','" . $DetTB["date_of_intrument"] . "','".$POT."')}";
                }
                else
...
/*                if ($ARDCd == "Exists") {
                  ...
                  /*  if($vTyp == 3){
                   ...
                   $sqlOrd = "{call svSuperStockist(?,$cCode,?," . $POB_Value . ",'" . $DetTB["intrumenttype"] . "','" . $DetTB["date_of_intrument"] . "'," . $pob . ",'" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,?,N'".$PPXML."','".$eKeyID."','".$POT."')}";
					...
					if($vTyp == 3){
                      ...
                      $sqlOrd = "exec svPriOrderNative '".$sfCode."',$cCode,'".$today."'," . $POB_Value . ",'" . $DetTB["intrumenttype"] . "','" . $DetTB["date_of_intrument"] . "'," . $pob . ",'" . str_replace("'", "''", $vals["Daywise_Remarks"]) . "','".$ARCd."','".$Owndiv."',N'".$PPXML."','".$eKeyID."','".$POT."',".$Super_Stck_code."";
                      $qryy= performQuery($sqlOrd);
                      $resp["priTransNo"] =$qryy;
            ...
            if(( $data[count($data)-4]["Activity_Event_Captures"]!=null && $data[count($data)-4]["Activity_Event_Captures"]!="undefined")||( $data[count($data)-3]["Activity_Event_Captures"]!=null && $data[count($data)-3]["Activity_Event_Captures"]!="undefined")||( $data[count($data)-5]["Activity_Event_Captures"]!=null && $data[count($data)-5]["Activity_Event_Captures"]!="undefined"))
            {
				if( $data[count($data)-3]["Activity_Event_Captures"]!=null && $data[count($data)-3]["Activity_Event_Captures"]!="undefined")
                $Event_Captures = $data[count($data)-3]["Activity_Event_Captures"];
			else if( $data[count($data)-5]["Activity_Event_Captures"]!=null && $data[count($data)-5]["Activity_Event_Captures"]!="undefined")
                $Event_Captures = $data[count($data)-5]["Activity_Event_Captures"];
			else
				 $Event_Captures = $data[count($data)-4]["Activity_Event_Captures"];
                for ($j = 0; $j < count($Event_Captures); $j++) {
                    $ev_imgurl=$Event_Captures[$j]["imgurl"];
                    $ev_title=$Event_Captures[$j]["title"];
                    $ev_remarks=$Event_Captures[$j]["remarks"];
					if($Event_Captures[$j]["DatewithTime"]!=null)
                    $DatewithTime=$Event_Captures[$j]["DatewithTime"];
		        	else
                   $DatewithTime="";
			   			   //$sql_event = "insert into Activity_Event_Captures(Activity_Report_Code,DCRDetNo,imgurl,title,remarks,Division_Code,Identification,Insert_Date_Time,lat,lon,Dateandtime) select '" . $ARCd . "','" . $ARDCd . "'," . $ev_imgurl . "," . $ev_title . "," . $ev_remarks . ",'" . $Owndiv . "' ,'DCR','".date('Y-m-d H:i:s')."','".$lat."','".$lng."' ,'".$DatewithTime."'";
 ...
 //$resp["callphoto"] =$data[count($data)-1]["Activity_Event_Captures_Call"];
			...
			$no_of_items = $data[1]["Activity_Doctor_Report"]['No_Of_items'];
			 $disPercnt = $data[1]["Activity_Doctor_Report"]['disPercnt'];
			$disValue = $data[1]["Activity_Doctor_Report"]['disValue'];
			$finalNetAmt = $data[1]["Activity_Doctor_Report"]['finalNetAmt'];
			$billingAddr=$data[1]["Activity_Doctor_Report"]['billingAddr'];
			$shippingAddr=$data[1]["Activity_Doctor_Report"]['shippingAddr'];
...
$sql="exec  getMxH'" . $sfCode . "'," . $stockistCode . ",'".$Owndiv."'";
				...
				//$SFSl=$tRw[0]['SLNo']."-";
                    ...
                    /*$sqlll = "insert into Trans_Order_Head(Trans_Sl_No,Sf_Code,Cust_Code,Stockist_Code,Route_Target,Order_No,Collected_Amount,Order_Value,Order_Date,Route,DCR_Code,net_weight_value,discount_price,rateMode,Div_ID,OrderID,OrderType,TDPercent,Order_In_Time,Order_Out_Time,Order_Flag) select '" . $pk . "','" . $sfCode . "'," . $custCode . "," . $stockistCode . ",'" . $routeTarget . "'," . $orderNo . ", $collectedAmount , $orderValue," . $orderDate . ", $townCode,'" . $DCRCode . "','$net_weight_value','$discount_price','$rateMode','$Owndiv','".$eKeyID."','".$POT."',".$Discountpercent.",'".$OrderStrat_Time."','".$OrderEnd_Time."','0'";*/
                     $sqlll = "insert into Trans_Order_Head(Trans_Sl_No,Sf_Code,Cust_Code,Stockist_Code,Route_Target,Order_No,Collected_Amount,Order_Value,Order_Date,Route,DCR_Code,net_weight_value,discount_price,rateMode,Div_ID,OrderID,OrderType,TDPercent,Order_In_Time,Order_Out_Time,Order_Flag,Sub_Total,Dis_Total,Tax_Total,No_Of_items,finalNetAmnt,tlDisPercnt,tlDisAmt,address,shippingAddress) select '" . $pk . "','" . $sfCode . "'," . $custCode . "," . $stockistCode . ",'" . $routeTarget . "'," . $orderNo . ", $collectedAmount , $orderValue," . $orderDate . ", $townCode,'".$DCRCode."','$net_weight_value','$discount_price','$rateMode','$Owndiv','".$eKeyID."','".$POT."',".$Discountpercent.",'".$OrderStrat_Time."','".$OrderEnd_Time."','0','".$subTotal."','".$discTotalValue."','".$taxTotalValue."','".$no_of_items."','".$finalNetAmt."','".$disPercnt."','".$disValue."','".$billingAddr."','".$shippingAddr."'";
 		...
 		$net_weight=$Nwt[0]['product_netwt'];
			...
			//$productremarks=($transOrderDetails[$i]['rx_remarks']==null? 0:$transOrderDetails[$i]['rx_remarks']);
				...
				$Fname = $transOrderDetails[$i]['Fname'];
                                ...
                                $competitorDetails = $data[2]['Activity_Sample_Report'][$i]['CompetitorDet'];
						 $resp["competitorDetails"] = $competitorDetails;
						
						if(count($competitorDetails)>0){
							 for ($j = 0; $j < count($competitorDetails); $j++) {
							 $qty = $competitorDetails[$j]['Prod_Qty'];
							 $rate = $competitorDetails[$j]['Prod_Rate'];
							 $value = $competitorDetails[$j]['Prod_Value'];
							 $competitor_name = $competitorDetails[$j]['Competitor_Name'];
							 $competitor_prod_name = $competitorDetails[$j]['Competitor_Product_Name'];
							 
							 
							 $cmpt="insert into tbrcpaDetails(SF_code,EntryDt,RCPADt,ListedDrCode,CmptrName,CmptrBrnd,CmptrPriz,ourBrnd,ourBrndNm,Division_Code,CmptrQty,CmptrPOB) select '" . $sfCode . "','" .  date("Y-m-d h:i:sa"). "','" .  date("Y-m-d h:i:sa"). "'," . $custCode . ",'".$competitor_name."','".$competitor_prod_name."','". $rate."','" . $transOrderDetails[$i]['product_code'] . "','" . $transOrderDetails[$i]['product_Name'] . "','$Owndiv','".$qty."','".$value."'";
						  ...
						  performQuery($sqlOrd);
							...
							if($transOrderDetails[$i]['vanSalesOrder']==null)
							 ...
							 if($qty > 0){
								...
								$qsql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm,RefNo)  select '" . $MaxRwID . "' , '" . $sfCode . "','F' ," . $custCode . ",'R' , " . $orderDate . ",'" . $transOrderDetails[$i]['product_code'] . "','".$qty ."','$Owndiv','VanInv','0','" . $docor_Name . "','" . $pk . "'";
								 performQuery($qsql);	 
								 }
							 ...
							 if($free > 0){
								...
								$fsql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm,RefNo)  select '" . $MaxRwID . "' , '" . $sfCode . "','F' ," . $custCode . ",'R' , " . $orderDate . ",'" . $transOrderDetails[$i]['product_code'] . "','" . $free . "','$Owndiv','Free','0','" . $docor_Name . "','" . $pk . "'";
								 performQuery($fsql);	 
								 }
							  ...
							  if($cbQty > 0){
								...
								$csql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm,RefNo)  select '" . $MaxRwID . "' , '" . $sfCode . "','F' ," . $custCode . ",'R' , " . $orderDate . ",'" . $transOrderDetails[$i]['product_code'] . "','" . $cbQty . "','$Owndiv','Return','0','" . $docor_Name . "','" . $pk . "'";
								performQuery($csql);	 
								}
...
where sfcode= '" . $sfCode . "' and convert(date,EntryDt)='" . $today . "' and Prod_Code='" . $transOrderDetails[$i]['product_code'] . "' ";
					...
					//$sql = "update Trans_Order_Head set Order_Value=".$orderValue-$disValue." where  Trans_Sl_No='" . $pk . "'";
					//performQuery($sql);
					//$sql = "update Trans_Order_Head set Order_Value=(select sum(value) from Trans_Order_Details where Trans_Sl_No='" . $pk . "'- //$disValue) where  Trans_Sl_No='" . $pk . "'";
				...
				$vanSalesOrders=$data[0]['Activity_Report_APP']['vanSalesOrders'];
			if($vanSalesOrders == 1){

               $PPXML = "<ROOT>";
	       for ($j = 0; $j < count($transOrderDetails); $j++) {
	        $PPXML = $PPXML . "<Prod PCode=\"".$transOrderDetails[$i]['product_code']."\" Qty=\"".transOrderDetails[$i]['Product_Rx_Qty']."\" Rate=\"".$transOrderDetails[$i]['Rate']."\" />";
	       }
		   $PPXML = $PPXML . "</ROOT>";
		   
		    $orderValues = $data[1]["Activity_Doctor_Report"]['orderValue'];
		$vanbill=   "insert into mas_van_pending_bills(SF,CustCode,BillNo,Inv_Dt,BillAmt,Coll_Amt,Bal_Amt,lastupdateddate,Invoice_No,div_code,Adv_amt,Adv_adj,inv_col,pay_col,cn_col) select '" . $sfCode . "'," . $custCode . ",'" . $pk . "','$dcr_activity_date','".$orderValues."',0,'".$orderValues."',getdate(),'" . $pk . "','$Owndiv',0,0,0,0,0 ";
		$resp["Qry4"]=$vanbill;
	     performQuery($vanbill);
	    
        $PAYAmount=$data[0]['Activity_Report_APP']['PayAmount'];
		$payType=$data[0]['Activity_Report_APP']['payment_mode_name'];
            $resp["Qry5"]=$PAYAmount;
		if($PAYAmount!="") {
			$sql = "exec saveAppVanPayment '" . $sfCode . "',''," . $custCode . ",$PAYAmount,$payType,'$dcr_activity_date','','$Owndiv','" . $pk . "','".$eKeyID."'";
			$resp["Qry3"]=$sql;
			$res=performQuery($sql);
		}
}
...
switch ($value[0]) {
    case "getDistReport":
        getDistReport();closeConn();
	break;
    case "login":
        actionLogin();closeConn();
	break;
	case "get/nonOutlets":
		//$SF=$_GET['SF_Code'];
		...
		$results = performQuery($query);
		echo json_encode($results);closeConn();die;
	break;
    case "get/track":
            $SF=$_GET['SF_Code'];
	...
	case "get/approvalList":
		...
		$query="exec getVanApproveList '" . $SF . "','".$divisionCode."'";
		$result = performQuery($query);
		outputJSON($result);
	break;
    case "deleteEntry":
	...
	break;
	case "get/LEOS":
		$Owndiv = $_GET['divisionCode'];
                 $divs = explode(",", $Owndiv . ",");
		$divisionCode = (string) $divs[0];
		// if($divisionCode=="107,")
		//$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code=107";
		//else
		$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code = '$divisionCode'";
		$det=performQuery($sql);
		if(count($det)==0){
			$div = $_GET['divisionCode'];
				$divs = explode(",", $div . ",");
	...
	case "get/Allow_Type":
		...
		$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code = '$division_code'";
		$det=performQuery($sql);
		if(count($det)==0){
			$div = $_GET['division_code'];
			$divs = explode(",", $div . ",");
			$Owndiv = (string) $divs[0];
			$sql="insert into Mas_Allowance_Types select 1,'HQ',$Owndiv,'HQ1'";
			performQuery($sql);
	...
	break;
    case "get/setup":
        outputJSON(getAPPSetups());closeConn();
	break;
  ...
  case "get/retailerRate":
               ...
               /* $query = "select isnull(SS_Base_Rate, '0') SS_Base_Rate,isnull(SS_Case_Rate, '0') SS_Case_Rate,State_Code, Division_Code,Distributor_Price,Retailor_Price,DistCasePrice,
 ...
 $query = "select Product_Detail_Code from Mas_Product_Detail where Division_Code='" . $Owndiv . "' and Product_Active_Flag='0' and CHARINDEX(','+'".$State_Code."'+',' ,','+cast(State_Code as varchar)+',')>0";
				...
				$retQuery = "select Price_List_Name,cat_id,stockiest_code from Retailer_cat_Rate ms right join vwstockiest_Master_APP vw on  vw.stockiest_code = ms.Dist_name where Stockist_Status =0 and vw.SF_Code ='". $sfCode ."' and ms.cat_id is not null ";
                ...
                $productCode = $results[$i]['Product_Detail_Code'];
                    ...
                    for($j=0; $j<count($retList); $j++)
                    {
                        ...
                        if($priceListCode!=null && $priceListCode!=''){
                            $retRateQuery = "select pwd.Rate_in_piece,pwd.MRP_Price, pwd.Product_Code,isnull(pwd.Unit,'')Unit from Mas_Product_Wise_Bulk_rate_head pwh inner join Mas_Product_Wise_Bulk_rate_details pwd on pwh.Price_list_Sl_No = pwd.Price_list_Sl_No where pwd.Product_Code ='". $productCode ."' and pwh.Price_list_Sl_No ='". $priceListCode ."' and ('".$date."' >= cast(Effective_From_Date as date) and '".$date."' <=cast(Effective_To_Date as date ))";
							...
							if(count($retRate)>0){
                                           ...
                                           if($retRate[0]['Unit']!=null && $retRate[0]['Unit'] !=''){
                                             $Product_Unit = $retRate[0]['Unit'];
							              }
										  ...
										  $retList[$j]['Product_Unit'] = $Product_Unit;
                                       //$retList[$j]['date'] = '$date';
                    }
	...
	case "get/distributorRate":
		...
		$State_Code=$head[0]["State_Code"];
		$query = "select isnull(SS_Base_Rate, '0') SS_Base_Rate,isnull(SS_Case_Rate, '0') SS_Case_Rate,State_Code, Division_Code,Distributor_Price,Retailor_Price,DistCasePrice,
		RetailCasePrice, MRP_Price,VanSale_Price, Product_Detail_Code from vwProductStateRates where Division_code='" . $Owndiv . "' and State_Code=$State_Code";
		$results = performQuery($query);
		...
		$distQuery = "select Price_List_Name, Stockist_Code from Mas_stockist ms right join
				...
				where Stockist_Status =0 and vw.SF_Code = '". $sfCode ."' and Price_List_Name is not null and Price_List_Name not in('','0')";
		...
		for($i=0;$i<count($results);$i++){
			...
			for($j=0; $j<count($distList); $j++)
			{
				...
				if($priceListCode!=null && $priceListCode!=''){
					$distRateQuery = "select pwd.dis_rate_in_piece,pwd.Rate_in_piece,pwd.MRP_Price, pwd.Product_Code, pwd.Product_Name,isnull(pwd.Unit,'')Unit from Mas_Product_Wise_Bulk_rate_head pwh inner join
									  ...
									  and ('".$date."' >= cast(Effective_From_Date as date) and '".$date."'<=cast(Effective_To_Date as date)) ";
					...
					if($distRate[0]['dis_rate_in_piece']!=null && $distRate[0]['dis_rate_in_piece'] !='')
					 ...
					 if($distRate[0]['Rate_in_piece']!=null && $distRate[0]['Rate_in_piece'] !=''){
								   ...
								   if($distRate[0]['MRP_Price']!=null && $distRate[0]['MRP_Price'] !=''){
								    ...
								    if($distRate[0]['Unit']!=null && $distRate[0]['Unit'] !=''){
                                             $Product_Unit = $distRate[0]['Unit'];
                ...
                $distList[$j]['Product_Unit'] = $Product_Unit;
	...
	break;
		...
		move_uploaded_file($_FILES["imgfile"]["tmp_name"], "../claim/" . $imgName);
		closeConn();
	break;
	case "multiple_imgUpload":
    $img = $_FILES['file'];
    $sf = $_GET['sf_code'];
    ...
    if(!empty($img)){
     for($i=0;$i<count($_FILES['file']['tmp_name']);$i++){
        ...
        $imgName = $sf . "_". $_FILES['file']['name'][$i];
		$response['names'] = $imgName;
        if(move_uploaded_file($_FILES['file']['tmp_name'][$i],"../photos/".$imgName)){
        $response['error'] = false;
        $response['message'] =  $response['message']. "uploaded sucessfully ::  ";
        }else{
        $response['error'] = true;
        ...
        break;
	case "temp_multiple_imgUpload":
		$result["error"] = false;
		outputJSON($result);
	break;
    case "web/notification_push":
        $sf_code = $_GET['sfcode'];
        $text = $_GET['msg'];
        $title = $_GET['title'];
        $event_id = $_GET['event_id'];
        $sql_query = "SELECT DeviceRegId FROM Access_Table where sf_code='$sf_code'";
        $device = performQuery($sql_query);
        $reg_id = $device[0]['DeviceRegId'];
        sendFCM_notify($reg_id, $text, $title,0,$event_id);
		$result['success'] = true;
		outputJSON($result);
        break;   
    case "get/jointwork":
	...
	case "ManagerAttendanceData";
		...
		outputJSON($result);
	break;
	case "getAttendanceDetailsMDB":
        $date = $_GET['date'];
        $sfcode= $_GET['sfcode'];
        $div_code= $_GET['division'];
        $wrk_type = $_GET['type'];
        //NLI means (NotLoggedIn) users
        if($wrk_type == "NLI"){
            $query = "exec getdashboard_wtype_nlg '$sfcode','$div_code','$date',''";
        }else{
            $query = "exec getdashboard_wtype_Oth '$sfcode','$div_code','$date','','$wrk_type'";
        }
        $result = performQuery($query);
        outputJSON($result);
    break;
	...
	case "get/fieldForceData";
		...
		COUNT(Trans_Detail_Info_Code) TC,COUNT(distinct Cust_Code) UPC,COUNT(Cust_Code)PC,cast(isnull(sum(order_Value),0) as decimal) orderValue,SUM(o.net_weight_value) weightValue from DCRMain_Trans h
		...
		$outlet_query = "select  count(distinct mld.ListedDrCode)total_outlet from Mas_SalesForce ms with(nolock) 
        inner join Mas_ListedDr mld with(nolock)  on  charindex(','+ms.Sf_Code+',',','+mld.Sf_Code+',')>0  
        where ms.Sf_Code in ($sfcode) and iif(mld.ListedDr_Active_Flag = 0,'$date',convert(date,ListedDr_Deactivate_Date)) >= '$date'";
	    $output['OUTLET'] = performQuery($outlet_query); 	
	...
	case "get/travel_data":
        ...
        $state_code = $_GET['state_code'];
        $query = "exec GetTravelApproval  '".$sf_code."','".$date."','".$div_code."','".$state_code."','".$dsg_code."'";
        $leave_query = "select COUNT(Leave_Type)leave_pending from mas_Leave_Form where Leave_Active_Flag = '2' and sf_code = '$sf_code' and '$date' between From_Date and To_Date";
        $result['travel_data'] = performQuery($query);
        $result['leave_validate'] = performQuery($leave_query);
        outputJSON($result);
    break;
	case "send_forApproval":
        ...
        $year = $_GET['year'];		
				if($divCode=="29,"||$divCode=="4,"){
$sql="select * from mas_leave_form where sf_code='$sf_code' and month(to_date)='$month' and year(to_date)='$year' and Leave_Active_Flag!=1";
        ...
        $query = "exec Update_Expense_ApprovalFlag '$sf_code','$period_id','$month','$year'";
		$result['msg'] = performQuery($query);
        $result["query"] = $query;
        $result["success"] = true;	
		$sql = "SELECT DeviceRegId FROM Access_Table where sf_code=(select Reporting_To_SF from mas_salesforce where Sf_Code='$sf_code')";
				$device = performQuery($sql);
                $reg_id = $device[0]['DeviceRegId'];
                if (!empty($reg_id)) {
					$sqlSfName = "select Sf_Name from Mas_Salesforce where Sf_Code='$sf_code'";
					$sfName = performQuery($sqlSfName);
        ...
        outputJSON($result);		
	break;
	case "get/expSubmitDates":
		$sf_code = $_GET['sf_code'];
		$from_date = $_GET['from_date'];
		$to_date = $_GET['to_date'];
		$period_id = $_GET['selected_period'];
		$month = $_GET['month'];
		$year = $_GET['year'];
		$sql_query="select CONVERT(varchar,convert(date,Date_Time),103)full_date,day(CONVERT(date,Date_Time))only_date from Expense_Start_Activity where CONVERT(date,Enddateand_time) between 
		'$from_date' and '$to_date' and Sf_code = '$sf_code' order by Enddateand_time";
		$query = "select convert(varchar,CONVERT(date,Expense_Date),103)full_date,day(CONVERT(date,Expense_Date))only_date from Trans_Expense_Detail_Periodic where convert(date,Expense_Date) between '$from_date' and '$to_date' and SF_Code = '$sf_code' order by Expense_Date";
		$plan_query = "select sf_code,FWFlg,max(convert(varchar,convert(date,Pln_Date),103))pln_date,day(CONVERT(date,Pln_Date))only_date from TbMyDayPlan where sf_code = '$sf_code' and CONVERT(date,Pln_Date) between '$from_date' and '$to_date' group by sf_code,FWFlg,convert(date,Pln_Date)";
		$rej_query = "select convert(varchar, CONVERT(date,RExpense_Date),103)full_date,day(CONVERT(date,RExpense_Date)) only_date from Trans_Rejected_Exp where Sf_Code = '$sf_code' 
		and CONVERT(date,RExpense_Date) between '$from_date' and '$to_date' order by RExpense_Date";
		$apr_query = "select Approve_Flag approve_flag from Trans_Expense_Head_Periodic where sf_code = '$sf_code' and Periodic_ID = '$period_id' and Expense_Month = '$month' and Expense_Year = '$year'";
		$srt_exp_query = "select CONVERT(varchar,convert(date,Date_Time),103)full_date,day(CONVERT(date,Date_Time))only_date from Expense_Start_Activity where CONVERT(date,Date_Time) between 
		'$from_date' and '$to_date' and Sf_code = '$sf_code' and isnull(Enddateand_time,0) = '0' order by Date_Time";
		$result['srt_exp'] = performQuery($srt_exp_query);
		$result['attance_flg']=performQuery($plan_query);
		$result['apr_flag']= performQuery($apr_query);
		$result['query2']=$srt_exp_query;
		$result['srt_end_exp']= performQuery($sql_query);
		$result['exp_submit']= performQuery($query);
		$result['rej_exp'] = performQuery($rej_query);
		outputJSON($result);
	break;
	case "get/periodicWise":
		 ...
		 $query2 = "select * from Approval_Hyr_Head Ahh WITH(Nolock) inner join Approval_Hyr_Detail Ahd WITH(Nolock) on Ahh.Slno=Ahd.FKSlno and sf_code='$sf_code' order by Appr_Lvl";
         $result['query'] = $query;
         $result['data'] = performQuery($query);
		 $result['query2'] = $query2;
		 $result['Hyr'] = performQuery($query2);
         outputJSON($result);
    break;
	case "get/Approval_Hyr":
        $sf_code = $_GET['sf_code'];
        $query = "select * from Approval_Hyr_Head Ahh WITH(Nolock) inner join Approval_Hyr_Detail Ahd WITH(Nolock) on Ahh.Slno=Ahd.FKSlno and sf_code='$sf_code' order by Appr_Lvl";
        outputJSON(performQuery($query));
    break;
	case "get/AllExpenseData":
	    $div_code = $_GET['div_code'];
		$period_id = $_GET['period_id'];
        $sf_code = $_GET['sf_code'];
	    $month = $_GET['month'];
		$year = $_GET['year'];
        $query = "select * from Trans_Expense_Head_Periodic with(nolock) where Appr_By = '$sf_code' and Periodic_ID = '$period_id' and Division_Code = '$div_code' and Approve_Flag = 0 and Expense_Month ='$month' and Expense_Year ='$year'";
		$result['query']= $query;
        $result['data'] = performQuery($query);
        outputJSON($result); 		
	break;
	case "get/Expense_Approved":
		$sf_code = $_GET['sf_code'];
		$date = $_GET['date'];
		$trans_slno = $_GET['trans_slno'];
		$approve_code = $_GET['approve_code'];
		$query = "update Trans_Expense_Head_Periodic set Approval_Date = '$date',Approved_By='$approve_code',Approve_Flag=1 where Trans_Sl_No = '$trans_slno' and SF_Code = '$sf_code'";
		performQuery($query);
		$result['query'] = $query;
		$result['success'] = true;	
		outputJSON($result);
	...
	case "Approve_Expense":
		...
		$date = $_GET['date'];
		$trans_slno = $_GET['trans_slno'];
		$approved_by = $_GET['approve_code'];
		$mn = $_GET['month'];
		$Yr = $_GET['year'];
		$pri_ID = $_GET['period_id'];
		$query = "exec update_expense_hyr '$sf_code','$date','$trans_slno','$approved_by','$mn','$Yr','$pri_ID'";
		performQuery($query);
		$result['query'] = $query;
		$result['success'] = true;	
		outputJSON($result);
	break;
	case "get/ApproveDailyExpense":
	    $sf_code = $_GET['sf_code'];
		$date = $_GET['date'];
		$approved_by = $_GET['approve_code'];
		$query = "exec update_daily_expense_hyr '$sf_code','$date','$approved_by'";
		performQuery($query);
		$result['query'] = $query;
		$result['success'] = true;
		outputJSON($result);
	break;
	case "get/RejectDailyExpense":
        $sf_code = $_GET['sf_code'];
		$date = $_GET['date'];
		$rej_remark = $_GET['remarks'];
		$query = "exec Reject_DailyExpense_Native '$sf_code','$date','0','$rej_remark','0'";
		performQuery($query);
		$result['query'] = $query;
		$result['success'] = true;
		outputJSON($result);
	...
	case "rejectExpense":
		...
		$year = $_GET['year'];
		$emp_id = $_GET['emp_id'];
		$rej_type = $_GET['rej_type'];
		$rej_remark = $_GET['remarks'];
		$PeriodID = $_GET['period_id'];
		$query = "exec RejectExpense_Native_Period '$sf_code','$month','$year','$date','$rej_type','$emp_id','$rej_remark','$PeriodID'";
		performQuery($query);
		$result['query'] = $query;
		$result['success'] = true;
		outputJSON($result);
	...
	case "get/Hyrsf":
	   $sf_code = $_GET['sf_code'];
	   $query = "select ms.Sf_Code,Sf_Name,Appr_Lvl from Approval_Hyr_Head Ahh WITH(Nolock) inner join Approval_Hyr_Detail Ahd WITH(Nolock) on Ahh.Slno=Ahd.FKSlno and Appr_SF='$sf_code' 
	             and Appr_Lvl>=0+1 and Appr_Type = 1 left join Mas_Salesforce ms on ms.Sf_Code = Ahh.Sf_Code group by ms.Sf_Code,Sf_Name,Appr_Lvl order by Appr_Lvl";
	   outputJSON(performQuery($query));
	break;
	case "get/ExpenseDetails":
		...
		$month = $_GET['month'];
		$year = $_GET['year'];
		$query = "exec getSfExpense_Approved_DemoApk '$sf_code','$month','$year','$trans_slno'";
		$add_query = "SELECT cast(convert(varchar,eDate,101) as datetime) as eDate,sum(Amt) as Amt FROM 
		 Trans_Daily_User_Expense d 
		 inner join Mas_Allowance_Type a on a.ID = d.expCode  
		 where d.sf_code='$sf_code' and  month(d.eDate)='$month' and year(d.eDate)='$year'
		 group by eDate,d.sf_code";
        $result['query'] = $query;
		$result['add_query'] = $add_query;
		$result['additional_data'] = performQuery($add_query);
		$result['data'] = performQuery($query);
		outputJSON($result);
	break;
	case "get/PendingExpenseApproval":
	     $sf_code = $_GET['sf_code'];
		 $query = "select convert(varchar,CONVERT(date,Expense_Date,101))full_date,SF_Name [name],ted.SF_Code sf_code,Town_Name from_place,Place_of_Work to_place,Expense_wtype work_type,Daily_Total amount,Expense_All_Type expense_type,Expense_DA da_amount,Expense_Distance travel_km,Expense_Amount travel_amount,Place_of_Work worked_place,sum(Amt) amt
			from Trans_Expense_Detail_Periodic ted 
			inner join Trans_Expense_Head_Periodic teh on teh.Trans_Sl_No = ted.trans_dt_slno
			inner join Trans_Daily_User_Expense d on d.SF_Code=teh.SF_Code and convert(date,ted.Expense_Date)=convert(date,d.eDate)
			where  ted.SF_Code = '$sf_code' and teh.Periodic_ID = 0 and Approve_Flag = 0 
			group by convert(varchar,CONVERT(date,Expense_Date,101)),SF_Name ,ted.SF_Code ,Town_Name ,Place_of_Work ,Expense_wtype ,Daily_Total ,Expense_All_Type ,Expense_DA ,Expense_Distance ,Expense_Amount ,Place_of_Work  ,Expense_Date
			order by Expense_Date";
		 $result['data'] = performQuery($query);
		 $result['query'] = $query;
		 outputJSON($result);
	break;
	case "get/PendingExpenseApprovalCount":
	     $sf_code = $_GET['sf_code'];
		 $query = "select teh.SF_Code sf_code,COUNT(teh.SF_Code) exp_count,SF_Name sf_name
					from Trans_Expense_Detail_Periodic ted 
					inner join Trans_Expense_Head_Periodic teh on teh.Trans_Sl_No = ted.trans_dt_slno
					where  ted.SF_Code in ($sf_code) and teh.Periodic_ID = 0 and Approve_Flag = 0 
					group by teh.SF_Code,SF_Name";
		 $result['data'] = performQuery($query);
		 $result['query'] = $query;
		 outputJSON($result);
	break;
	case "reject_travel":
        ...
        $amount = $_GET['amount'];
        $ukey = $_GET['Ukey'];
		$reason = $_GET['rej_reason'];
        $query = "update Expense_Start_Activity set Approval_Flag='2',Approval_Amount='$amount',Approved_Date='$date',Approved_by='$sf_code',Fare='$fare',Reject_reason = '$reason' where Ukey = '$ukey'";
        performQuery($query);
		$result['query'] = $query;
        $result['success'] = true;
		outputJSON($result);
	...
	case "approve_travel":
        ...
        $amount = $_GET['amount'];
        $ukey = $_GET['Ukey'];
        $query = "update Expense_Start_Activity set Approval_Flag='1',Approval_Amount='$amount',Approved_Date='$date',Approved_by='$sf_code',Fare='$fare' where Ukey = '$ukey'";
        performQuery($query);
		$result['query'] = $query;
        $result['success'] = true;
		outputJSON($result);
	...
	case "post/endDayExpense":
		...
		,End_MOT='$end_modId',Personal_Km='$personal_km',End_Entry = '$end_entry',Fare = '$end_fare' where Sf_code='$sf_code' and convert(date,ExpenseDate)='$date'";
		$atten_history = "update Attendance_history set End_Time='$date_time' where Sf_Code='$sf_code'  and cast(login_date as date) ='$date'";
		$tp_atten_app = "update TP_Attendance_App set End_Time='$date_time' where Sf_Code='$sf_code' and cast(login_date as date) = '$date'";
		performQuery($query);
		$result['success'] = true;
		outputJSON($result);
    break; 
	case "get/srtExpenseData":
		...
		$query = "select * from EXpense_Start_Activity esa inner join Mas_Modeof_Travel
             mmt on mmt.Sl_No = esa.MOT where convert(date,ExpenseDate)='$date' and sf_code= '$sf_code'";
		$result = performQuery($query);
		outputJSON($result);
    break;
	case "get/retailerWiseOrder":
		$retailer_id = $_GET['retailer_id'];
		$route_id = $_GET['route_id'];
		$sf_code = $_GET['sf_code'];
		$to_date = date("Y-m-d");
		$from_date = date("Y-m-01",strtotime($from_date." -6 months "));
		$query = "select Trans_Sl_No order_no,Convert(varchar,Order_Date,103) order_date_show,Order_Value order_value,Cust_Code retailer_id  from Trans_Order_Head where Sf_Code = '$sf_code' and Cust_Code = '$retailer_id' and CONVERT(date,Order_Date) between '$from_date' and '$to_date' order by order_date";
		$result = performQuery($query);
		outputJSON($result);
	break;
	case "get/van_target_sales":
	    $sf_code = $_GET['sfcode'];
		$month = $_GET['month'];
		$year = $_GET['year'];
		$from_date = $year."-".$month."-01";
		$to_date = $year."-".$month."-01";
		$ach_query = "select sum(Order_Value)value_o,Work_Type,dmt.Sf_Code from DCRMain_Trans dmt inner join DCRDetail_Lst_Trans dlt on dmt.Trans_SlNo = dlt.Trans_SlNo inner join Trans_Order_Head tod on dlt.Trans_Detail_Slno = tod.DCR_Code
        where month(Order_Date) = '$month' and YEAR(Order_Date) = '$year' and dmt.Sf_Code in ($sf_code) group by Work_Type,dmt.Sf_Code order by dmt.Sf_Code";
		$target_query = "select sum(target_Value) target_val,SF_Code sf_code from Trans_Field_Target_head  h inner join
		Trans_Field_Target_details d on d.TransFTslno=h.TransFTslno 
		where cast (cast(YEAR as varchar)+'-'+cast(month as varchar) +'-1' as date) between '$from_date' and '$to_date' and SF_Code in ($sf_code) group by SF_Code";
		$result['Achieved'] = performQuery($ach_query);
		$result['Target'] = performQuery($target_query);
		$result['target_query'] = $target_query;
		outputJSON($result);
	break;
	case "getExpenseReportDetails":
		$SF = $_GET['sf_code'];
		$frmdt = $_GET['from_date'];
		$todt = $_GET['to_date'];
		$Mn = $_GET['mon'];
		$Yr = $_GET['year'];
		$EmpID = $_GET['emp_id'];
		$PriID = $_GET['period_id'];
		$query = "exec getSfExpense_Apk_Periodic '$SF','$frmdt','$todt','$Mn','$Yr','$EmpID','$PriID'";
		$query2 = "select Amt amt,convert(varchar,eDate,103)edate,expName exp_name,Remarks remarks,Image_Url img_url from Trans_Daily_User_Expense where SF_Code = '$SF' and  Convert(date,eDate) between '$frmdt' and '$todt'";
		$query3 = "select Exp_Desc exp_desc,Add_Sub add_sub,Exp_Amnt exp_amnt from Expense_Additional ea inner join Trans_Expense_Head_Periodic thp on ea.transh_exp = thp.Trans_Sl_No where ea.Sf_Code = '$SF' and Mnth = '$Mn' and Yr = '$Yr' and Periodic_ID ='$PriID'";
		$query4 = "exec GetDesg_AllowanceDets_Pri '$SF','$Mn','$Yr','$PriID'";
		$result['exp_data'] = performQuery($query);
		$result['daily_exp'] = performQuery($query2);
		$result['add_sub_exp'] = performQuery($query3); 
		$result['sum_add_data'] = performQuery($query4);
		$result['query'] = $query3;
		$result['query2'] = $query4;
		outputJSON($result);
	break;
	case "getExpenseReportDetailsDaily":
		$SF = $_GET['sf_code'];
		$Mn = $_GET['mon'];
		$Yr = $_GET['year'];
        $div_code = $_GET['division_code'];
        $query = "exec getExpenseDet_SingleDay '$SF','$Mn','$Yr','$div_code'";
        $res = performQuery($query);
		$result['query'] = $query;
		$result['data'] = $res;
        outputJSON($result);
    break;
	case "get/productNewLaunch":
	     $div = $_GET['divisionCode'];
		  $divs = explode(",", $div . ",");
          $Owndiv = (string) $divs[0];
	     $sql="select File_Name offerimg from product_new_launch where  division_code='".$Owndiv."' and cast(Effective_From as date)<=CAST(getdate() as date) and CAST(getdate() as date)<=CAST(Effective_To as date)";
		 $result = performQuery($sql);
		outputJSON($result);
	break;
	...
	break;
        ...
        $div_code = $_GET['div_code'];
        $sf_code = $_GET['sf_code'];
        $query = "select ms.* from Mas_Division md inner join Mas_State ms on charindex(','+convert(varchar,ms.State_Code)+',',','+md.State_Code+',')>0 where md.Division_Code = '$div_code'";
        $result = performQuery($query);
	outputJSON($result);
        break;
	...
	case "get/locations":
        ...
        if(count($Users_FieldWork)>0){
            ...
            $query = "select  tl.Sf_Code sfcode, ms.Sf_Name username,MAX(tl.Lat) lat,MAX(tl.Lon) long,MAX(tl.DtTm) lastdate,isnull(MAX(ms.Profile_Pic),'') profilepic from Mas_Salesforce ms with (nolock) inner join tbTrackLoction  tl with (nolock) on tl.SF_code = ms.Sf_Code where ms.Sf_Code in ($all_sfcodes) and convert(date,tl.DtTm)='".$date."' and Lat <> '' and Lon <> '' group by tl.SF_code,ms.Sf_Name";
        }
        $result['result'] = performQuery($query);
        $result['query'] = $query;
        outputJSON($result);
    ...
    case "get/manager_performance":
        ...
        if($sec_or_pri>0){
            ...
            $achievedquery = "exec manager_performance_sec '$sf_code',$divCode,'$date','rsm'";
        }
        $result['Target'] = performQuery($targetquery);
        $result['Achieved'] = performQuery($achievedquery);
        outputJSON($result);
	...
	case "get/CoverageDetails":
        ...
        inner join Mas_ListedDr mld with(nolock)  on  charindex(','+ms.Sf_Code+',',','+mld.Sf_Code+',')>0  
        where ms.Sf_Code in ($sfcode) and iif(mld.ListedDr_Active_Flag = 0,'$toDate',convert(date,ListedDr_Deactivate_Date)) between '$fromDate' and '$toDate' and cast(ListedDr_Created_Date as date)<='$toDate'";
        ...
        where ms.Sf_Code in ($sfcode) and iif(mt.Territory_Active_Flag = 0,'$toDate',convert(date,Territory_Deactive_Date)) between '$fromDate' and '$toDate'  and cast(mt.Created_Date as date)<='$toDate'";
        ...
        where ms.Sf_Code in ($sfcode) and ISNULL(dis.Stockist_Active_Flag,0) = '0'   and cast(dis.Created_Date as date)<='$toDate'";
	...
	case "get/sfDetails":
        ...
        $div_code = $_GET['division_code'];
		$query = "exec GET_Filed_Force_Details '$div_code','','$sf_code','$selected_date'";
		$output = performQuery($query);
		outputJSON($output);
	break;  		
	case "get/circularlist":
        ...
        if (count($output) > 0) {
            for ($i = 0;$i < count($output);$i++) {
				   ...
				   if($size==false)
					   $size=0;
                   $results[] = array('FileName'=>$output[$i]['FileName'],'FileSubject'=>$output[$i]['FileSubject'],'ID'=>$output[$i]['ID'],'FileSize'=>$size,'FilePath'=>$path);
            }
        } else {
            $results = null;
        }
	...
	case "get/callpreviewhierarchy":
		...
		if($screentype == 1){
		$query = "select * from Trans_PriOrder_Details d with(nolock) inner join Mas_Product_Detail h with(nolock) on d.Product_Code = h.Product_Detail_Code where d.Trans_Sl_No = '".$trans_slno."' order by h.Product_Code_SlNo ";	
		}
		else{
		$query = "select * from Trans_Order_Details d with(nolock) inner join Mas_Product_Detail h with(nolock) on d.Product_Code = h.Product_Detail_Code where d.Trans_Sl_No = '".$trans_slno."' order by h.Product_Code_SlNo ";
		}
		$result = performQuery($query);
		outputJSON($result);
	break;
    ...
    case "table/list":
        ...
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
if($Owndiv==156&&strtolower($data['tableName'])=="vwactivity_msl_details"){
	$data['tableName']="vwactivity_msl_details_tsr";
}
        switch (strtolower($data['tableName'])) {
			case "vwactivity_msl_details":
		     $sfCode = $_GET['Sf_Code'];
		      $query = $query = "select Order_Flag,Trans_Sl_No,finalNetAmnt Order_Value,Activity_Date,Trans_Detail_Name,j.Stockist_Name stockist_name,OrderType OrderTyp from DCRMain_Trans h with(nolock) 
             inner join DCRDetail_Lst_Trans d on h.Trans_SlNo = d.Trans_SlNo 
           ...
           case "vwactivity_supercsh_detail":
            ...
            case "vwtourplan":
				...
				$query = "select  t.Rejection_Reason, CONVERT(VARCHAR(20),m.Sf_Joining_Date,111) joining_date,* from vwTourPlan v inner join Mas_Salesforce m on m.Sf_Code = v.SF_Code left join TP_Reject_B_Mgr t on v.Sf_Code = t.Sf_Code and v.Tmonth = t.Tour_Month and v.Tyear = t.Tour_Year
                          where v.SF_Code='$sfCode' and v.Tmonth=$CMn and v.Tyear=$CYr order by Reject_Date desc";
     ...
     case "vwtourplannew":
                ...
                //$query = "select  * from vwTourPlan where SF_Code='$sfCode' and Tmonth=$CMn and Tyear=$CYr";
				$query = "select  t.Rejection_Reason,v.* from vwTourPlan v left join TP_Reject_B_Mgr t on v.Sf_Code = t.Sf_Code and v.Tmonth = t.Tour_Month and v.Tyear = t.Tour_Year
                          where v.SF_Code='$sfCode' and v.Tmonth=$CMn and v.Tyear=$CYr order by Reject_Date desc";
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
	...
	case "dcr/updateProducts":
		...
		$disPercnt = $data['disPercnt'];
		$disValue = $data['disValue'];
		$finalNetAmt = $data['finalNetAmt'];
        ...
        if($data['discount_price']==null)
            ...
            $discount_price = $data['discount_price'];
        $query = "select Order_value pob_value,CL_Stock,Div_ID,OrderID,isnull(OrderType,0)OrderType from Trans_Order_Head WHERE Trans_Sl_No ='" . $Trans_Sl_No . "'";
    ...
    case "vwLeave":
        $sfCode = $_GET['sfCode'];
		 $divCode = $_GET['divisionCode'];
		 if( $divCode=='29,'){
        $sql = "select Sf_Code,Leave_Id,Reason,Address,FieldForceName,Reporting_To_SF,Designation,HQ,EmpCode,Reporting_To_SF,Designation,cast(LeaveDays as float) LeaveDays,Division_Code,sf_Designation_Short_Name
		,Leave_Type,Leave_Name,Leave_Id LeaveCode,From_Date,To_Date, case when First_or_Second='1' then 'First Half' when First_or_Second='2' then 'Second Half' when First_or_Second='3' then 'Full Day'  when First_or_Second='0' then 'Full Day' else isnull(First_or_Second,'') end leaveoption
		...
		,Leave_Type,Leave_Name,Leave_Id LeaveCode,From_Date,To_Date,'Full Day' leaveoption
		from vwLeave where Reporting_To_SF='$sfCode'";
		 }
		/* if($sfCode=="MGR1165"){
			$sql = "select Sf_Code,Leave_Id,Reason,Address,FieldForceName,Reporting_To_SF,Designation,HQ,EmpCode,Reporting_To_SF,Designation,cast(LeaveDays as int) LeaveDays,Division_Code,sf_Designation_Short_Name
		,Leave_Type,Leave_Name,Leave_Id LeaveCode,From_Date,To_Date,'Full Day' leaveoption
		from vwLeave where division_code=106";
		 }*/
        $leave = performQuery($sql);
		outputJSON($leave);
	break;
	...
	case "GetSecOrderConfirmView":
		...
		$sql = "select * from sec_order_val_confirmation where sf_code='$sfCode' and month=$Month and year=$Year and approve_flag!=2";
	...
	case "GetSecOrderConfirmView_all":
		...
		$sql = "select * from vwSecOrderConfirmApproval_all where (reporting_to_sf='$sfCode' or sf_code='$sfCode') and monthnum=$Month and year=$Year";
		$order = performQuery($sql);
	...
	case "GetExpenseView":
		...
		if($Owndiv=="107"){
			...
			$Total = performQuery($sql);
                        ...
                        $sql = "exec Get_Expense_Details_New '" . $sfCode . "' ,'" . $EMonth . "' ,'" . $EYear . "','".$Owndiv."' ";
			$View = performQuery($sql);
		...
		if(count($Total)>0){
			if($Owndiv!="109"){
				...
				$Total[0]['Expense_Amt']=$Total[0]['Expense_Amt'];
				$Total[0]['Applied_ExpAmnt']=$Total[0]['Applied_ExpAmnt'];
		}
		else{
			$Total[0]['Expense_Amt']="0";
			$Total[0]['Applied_ExpAmnt']="0";
		}
		if($Owndiv!="109"){
...
//	print_r($sql);
    ...
    case "vwChkTransApproval":
        ...
        outputJSON($tp);
	break;
	case "vwRejectLeaveDet":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwRejectLeaveDet where sf_code='$sfCode' and from_date1>=dateadd(month,-6,getdate()) order by from_date1 desc";
        $tp = performQuery($sql);
        outputJSON($tp);
	break;
    ...
    case "vwChkTransApprovalOne":
        ...
        $month = $_GET['month'];
        $year = $_GET['year'];
        $sql = "select Objective,SF_Code,Worked_With_SF_Name,Worktype_Name_B,Territory_Code1,convert(varchar,Tour_Date,103) Tour_Date,JointWork_Name,isnull(Worktype_Name_B2,'') JointWork_Name1,HQ_Code,isnull((TPOB+TSOB),0) targetVal,isnull(TPOB,0) TPOB,isnull(TSOB,0) TSOB from Trans_TP_One where Sf_Code='$sfCode' and Tour_Month='$month' and Tour_year=$year order by Tour_Date";
        $tp = performQuery($sql);
        outputJSON($tp);
	break;
        ...
        /*$sql = "select Dcr_Missed_Date,CONVERT(VARCHAR(10), Dcr_Missed_Date, 103) missed_date,s.sf_code,s.sf_name,sl_no,Deviation_Reason reason from DCR_MissedDates d
			...
			left join DCR_TPDev_Reason r on r.sf_code=d.sf_code and cast(Dcr_Missed_Date as date)=cast(Activity_Date as date)
             where d.status=3 and Reporting_To_SF='$sfCode' and month(Dcr_Missed_Date)=(case when 'All'='".$month."'then month(Dcr_Missed_Date) else '".$month."' end) and year(Dcr_Missed_Date)=(case when 'All'='".$year."'then year(Dcr_Missed_Date) else '".$year."' end)"; */
				$div = $_GET['divisionCode'];
				if($div=="29,"||$div=="4,")
				$sql = "exec getDeviationApprovalApp '".$sfCode."','".$month."','".$year."'";
			else{
...
/* $sql = "select Dcr_Missed_Date,CONVERT(VARCHAR(10), Dcr_Missed_Date, 103) missed_date,s.sf_code,s.sf_name,sl_no from DCR_MissedDates d
	...
	if($clsng=="1")
			...
			Conversion_Qty conversionQty,Mgf_date,(select Distributor_Price from vwProductStateRates where State_Code='".$stateCode."' and Product_Detail_Code=ts.Product_Code) Rate,CAST(TotalValue AS float)sample_qty,batch_no 
			from Trans_Stock_Updation_Details ts where (Purchase_Date = '".$CLDT."' or Purchase_Date = (select max(Purchase_Date) from Trans_Stock_Updation_Details where Stockist_code='".$SF."' and Purchase_Date <= '".$CLDT."' )) 
	...
	case "get/brandavail":
		...
		FROM ( select SFCode,cast(replace('<a><b>'+replace(replace(BrndRevw1,'#','</b><b>'),'$','</b></a><a><b>')+'</b></a>','<a><b></b></a>','') as xml) AS msg from Trans_OutletReview_Details
		...
		$tr=performQuery($sql);
        $sql="select l.ListedDr_Name,evnt.imgurl,outlet.Rmks from Trans_OutletReview_Details outlet
              inner join Activity_Event_Captures evnt on cast(outlet.TDSlNo as varchar(20))=evnt.DCRDetNo 
              inner join Mas_ListedDr l on cast(l.ListedDrCode as varchar(50))=outlet.RetID where evnt.Identification='[ Review ]' and 
              cast(convert(varchar,Adt,101) as datetime)='" . $dyDt . "' and sfcode='".$sf."'";

        $rev=performQuery($sql);
		$result1=array();
		if(count($tr)>0){
	...
	case "get/brandavail_tsr":
		$sf = $_GET['sfCode'];
		$div = $_GET['divisionCode'];
		$dyDt = $_GET['activityDate'];
               $sql1 = "exec getDayBrandReviewReportApp '" . $sf . "','" . $dyDt . "'";
...
$i=0;
   ...
   $result2[$i]['value']=$value;
$result2[$i]['name']=$value[0]['SF_Name'];
   $i++;
}
		outputJSON($result2);
	break;
case "get/brandavailmonth_tsr":
		$sf = $_GET['sfCode'];
		$div = $_GET['divisionCode'];
		$dyDt1 = $_GET['activityFromDate'];
$dyDt = $_GET['activityDate'];
               $sql1 = "exec getMonthBrandReviewReportApp '" . $sf . "','" . $dyDt1 . "','" . $dyDt . "'";
		$tr=performQuery($sql1);
$result1=array();
...
$i=0;
	foreach($result1 as $key => $value){
   $result2[$i]['value']=$value;
$result2[$i]['name']=$value[0]['ADate'];
   $i++;
}
		outputJSON($result2);
	break;
	case "get/brandavailret_tsr":
		$sf = $_GET['sf_code'];
		$div = $_GET['divisionCode'];
		$dyDt = $_GET['activityDate'];
		$BrandId = $_GET['BrandId'];
		
       $sql = "select SFCode,(select ListedDr_Name from Mas_ListedDr where ListedDrCode=RetId) BName,sum(Avail) Avail,sum(EC) EC,count(RetId) tc from  (select SFCode,RetId,Sum(Avail) Avail,Sum(EC) EC,0 tc from 
		(
		SELECT SFCode,RetId,s.c.value('(b)[1]', 'nvarchar(20)' ) AS BId,s.c.value('(b)[3]', 'int' ) AS Avail,s.c.value('(b)[4]', 'int' ) AS EC
		FROM ( select SFCode,RetId,cast(replace('<a><b>'+replace(replace(BrndRevw,'#','</b><b>'),'$','</b></a><a><b>')+'</b></a>','<a><b></b></a>','') as xml) AS msg from Trans_OutletReview_Details
		where cast(convert(varchar,Adt,101) as datetime)='" . $dyDt . "' and sfcode='".$sf."'
		) a
			CROSS APPLY a.msg.nodes('a') s(c)
		) as Stb
		where (Avail>0 or EC>0) and BId=".$BrandId."
		group by SFCode,BId,RetId) test  group by SFCode,RetId";
		
		// $sql = "exec getDayReviewReportApp '" . $sfCode . "','" . $dyDt . "'";
		$tr=performQuery($sql);
...
$query = "select Pln_Date Adate,'' Acode,
	...
	if($Owndiv=="162"||$Owndiv=="29"){
		...
		if($vanWorkFlag == "VTS"){
            $query = "select sum(Order_Value)order_value,Work_Type from DCRMain_Trans dmt inner join DCRDetail_Lst_Trans dlt on dmt.Trans_SlNo = dlt.Trans_SlNo inner join Trans_Order_Head tod on dlt.Trans_Detail_Slno = tod.DCR_Code
            where month(Order_Date) = '".date('m')."' and YEAR(Order_Date) = '".date('Y')."' and dmt.Sf_Code = '$sfCode' group by Work_Type,dmt.Sf_Code order by dmt.Sf_Code";
        }else{
		 $query = "exec Get_FieldForce_Acheive '".$sfCode."','".date('m')."','".date('Y')."','" . $Owndiv . "' ";
        }        
		$orderVal = performQuery($query);
        $result['month']['order'] = $tc_pc[0]['pc'];
        $result['month']['orderVal'] = $orderVal[0]['order_value'];
        $result['month']['workType'] = $orderVal[0]['Work_Type'];
        $result['month']['orderVal2'] = $orderVal[1]['order_value'];
        $result['month']['workType2'] = $orderVal[1]['Work_Type'];
        $result['month']['calls'] = $tc_pc[0]['tc'];
        $result['month']['TQty'] = 0;
        $result['month']['TAmt'] =0;
        $result['month']['AQty'] =0;
...
/*if($sfCode=="MR3532"){
  ...
  if($_GET["dcr_activity_date"]!=null && $_GET["dcr_activity_date"]!=''){
        ...
        $result["success"] = true;
		//  $result["dcrdate"] = $_GET["dcr_activity_date"];
...
if($data['date_time']!=null)
	...
	$logintime=date("h:i:sa");
		 if($SrtEndNd==0){
			// if($day==1){
			$queryy = "update Attendance_history set End_Time='" . $dateTime. "',End_Lat= '" . $data["Lattitude"] . "' ,End_Long ='" . $data["Langitude"] . "'    where  Sf_Code='" . $sfCode . "'  and cast(login_date as date) ='".$date."'";
			performQuery($queryy);
        ...
        $query = "update Login_Time_Table set End_Time='" . $dateTime. "' ,End_Lat= '" . $data["Lattitude"] . "' ,End_Long ='" . $data["Langitude"] . "'  where  Sf_Code='" . $sfCode . "'  and cast(login_date as date) ='".$date."'";
        ...
        $query = "insert into LogOutHistory (Sf_Code,Date_Time,Division_Code,Img_Url,Lat,Long) values ('". $sfCode ."','". $dateTime ."','". $Owndiv ."','','" . $data["Lattitude"] . "','" . $data["Langitude"] . "')";     
		performQuery($query);
        $result["success"] = true;
        $result["Query"] = $queryy;
        outputJSON($result);
...
Mas_ListedDr where listeddrcode='" . $data["id"] . "')";
    ...
    case "get/notify":
     ...
     case "notification_viewed":
        ...
        $query = "update Mas_AppNotifyList set Read_Sign = 1,RDt = GETDATE() where SF_Code = '$sf_code' and NotifyID = '$notify_id'";
        performQuery($query);
        $resp['query'] = $query;
        $resp['success'] = true;
        outputJSON($resp); 
    ...
    case "notification_delete":
        $sf_code = $_GET['sf_code'];
        $notify_id = $_GET['notify_ID'];
        $query = "update Mas_AppNotifyList set Clear_Sign = 1, ClDt = GETDATE() where SF_Code = '$sf_code' and NotifyID = '$notify_id'";
        performQuery($query);
        $resp['query'] = $query;
        $resp['success'] = true;
        outputJSON($resp);
...
case "get/EndInventoryCheck":
	...
	case "get/states":
				...
				$Owndiv = (string) $divs[0];
                $sfCode = $_GET['sfCode'];
			$query="select ms.State_Code,ms.StateName,md.Division_Code from Mas_Division md left join Mas_State ms on CHARINDEX(','+cast(ms.State_Code as varchar)+',',','+md.State_Code+',')>0 where md.Division_Code= '$Owndiv' ";
			$results = performQuery($query);
			outputJSON($results);
			
	break;
	case "get/ssorder_app":
       $SF_code = $_GET['sfCode'];
	    $stockistCode = $_GET['stockist_code'];
		$order_date=$_GET['order_date'];
		$div = $_GET['divisionCode'];
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
		$query = "exec getsuperstockistOrder_app '".$SF_code."','".$order_date."','".$Owndiv."'";
	    outputJSON(performQuery($query));
	break;
