...
public partial class MasterFiles_MR_ListedDoctor_RetailerAdd : System.Web.UI.Page
{
    ...
    {
        ListedDR lstDR = new ListedDR();
        dsListedDR = FetchTerritory(divcode, sf_type, sf_code);
        //dsListedDR = lstDR.FetchTerritory(divcode, sf_type, sf_code);
        return JsonConvert.SerializeObject(dsListedDR.Tables[0]);
    }
    public static DataSet FetchTerritory(string div_code, string sf_type, string sf_code)
    {
        ...
        if (sf_type == "4")
        {
            strQry = "SELECT 0 as Territory_Code,'---Select---' as Territory_Name " +
                    ...
                    " FROM  Mas_Territory_Creation where Division_Code = '" + div_code + "' AND territory_active_flag=0 and charindex(','+'" + sf_code + "'+',',',' + Dist_Name+',')>0 order by Territory_Name ";
        }
        else
        {
            strQry = " SELECT 0 as Territory_Code,'---Select---' as Territory_Name " +
                         " UNION " +
                     " SELECT Territory_Code,Territory_Name " +
                     " FROM  Mas_Territory_Creation where Division_Code = '" + div_code + "' AND territory_active_flag=0 order by Territory_Name ";
        }
        try
        {
    ...
    public static string FillCatagry(string divcode)
    {
        ...
        if (sf_type == "4")
        {
            ds=getDataSet("SELECT 0 as Doc_Cat_Code, '---Select---' as Doc_Cat_SName, '---Select---' as Doc_Cat_Name UNION  SELECT Doc_Cat_Code,Doc_Cat_SName,Doc_Cat_Name FROM  Mas_Doctor_Category  where division_Code = '" + div_code + "' and Doc_Cat_Active_Flag=0 order by Doc_Cat_Name");
    ...
    public static string FillSubCatagry(string divcode, string cat_code)
    {
        ...
        if (sf_type == "4")
        {
            ds = getDataSet("SELECT 0 as Doc_SubCatCode, '---Select---' as Doc_SubCatSName, '---Select---' as Doc_SubCatName UNION SELECT Doc_SubCatCode,Doc_SubCatSName,Doc_SubCatName FROM  Mas_Doc_SubCategory where Division_Code='" + div_code + "'  and Doc_SubCat_ActiveFlag=0 and charindex(','+'"+cat_code+"'+',',','+Doc_Cat_Code+',')>0 order by Doc_SubCatName");

           // ds = getDataSet("SELECT 0 as Doc_SubCatCode, '---Select---' as Doc_SubCatSName, '---Select---' as Doc_SubCatName UNION SELECT Doc_Special_Code Doc_SubCatCode, Doc_Special_SName Doc_SubCatSName, Doc_Special_Name Doc_SubCatName FROM Mas_Doctor_Speciality where Doc_Special_Active_Flag=0  and Division_Code='" + div_code + "'");
        }
        else
        {
            ds = getDataSet("SELECT 0 as Doc_SubCatCode, '---Select---' as Doc_SubCatSName, '---Select---' as Doc_SubCatName UNION SELECT Doc_Special_Code Doc_SubCatCode, Doc_Special_SName Doc_SubCatSName, Doc_Special_Name Doc_SubCatName FROM Mas_Doctor_Speciality where Doc_Special_Active_Flag=0 and Division_Code='" + div_code + "'");
    ...
    public static string Filldrslab(string divcode)
    {
        ...
        dsListedDR = getMasPurchase(div_code);
        //dsListedDR = dist.getMasPurchase(div_code);
        return JsonConvert.SerializeObject(dsListedDR.Tables[0]);
    }
    public static DataSet getMasPurchase(string div_code)
    {
        DataSet dsSF = null;
        DB_EReporting db = new DB_EReporting();
        strQry = "select * from mas_Purchase where Division_Code='" + div_code + "' and Active_flag=0 order by Name";
        try
        {
    ...
    public class SaveRetailerDataa
    {
        ...
        [JsonProperty("FRErbCode")]
        public string FRerbCode { get; set; }
                ...
                if (!RecordExist(sd.DR_Name, sd.retail_code, sd.div_code))
                {
                    if (!ERPRecordExist(sd.erbCode, sd.div_code, sd.retail_code))
                    {
                        try
                        {
                            ...
                            strQry = "select ms.subdivision_code from mas_stockist ms inner join Mas_subdivision msd on cast(msd.subdivision_code as varchar) = (select Replace(ms.subdivision_code,',',' ')) where Stockist_Code in (select top 1 * from SplitString('" + distname + "',','))";
                            ...
                            {
                                outletcode = "";
                                Subdivisioncode = dd1["subdivision_code"].ToString().Replace(",", "");
                            }
                            ...
                            //        new SqlParameter("@CusCode", sd.FRerbCode),
                            ...
                            //};
                            string sqlQry = "exec insertNewRetailerData '" + Listed_DR_Code + "','" + sd.DR_Name + "','" + sfcode + "','" + Listed_DR_Code + "', " +
                                "'" + sd.FRerbCode + "','" + sd.Mobile_No + "','" + sd.Contact_Person + "','" + sd.DR_Spec + "','" + sd.dr_spec_name + "'," +
                                "'" + sd.sales_Tax + "','" + sd.Tinno + "','" + sd.DR_Terr + "','" + sd.credit_days + "','" + sd.DR_Class + "'," +
                                "'" + sd.dr_class_name + "','" + sd.ad + "','" + sd.DR_Address1 + "','" + sd.DR_Address2 + "','" + Division_Code + "'," +
                                "'" + active_flag + "','" + sd.Milk_pon + "','" + sd.UOM + "','" + sd.UOM_Name + "','" + terr_code + "'," +
                                "'" + distname.TrimEnd(',') + "','" + sd.DDL_Re_Type + "','" + sd.outstandng + "','" + sd.creditlmt + "','" + sd.Cus_Alter + "'," +
                                "'" + sd.drcategory + "','" + sd.drcategoryName + "','" + sd.latitude + "','" + sd.longitude + "','" + sd.slbval + "'," +
                                "'" + sd.email + "','" + sd.pan_no + "','" + sd.Tds_App + "','" + sd.Tcs_App + "','" + sd.state_code + "'," +
                                "'" + sd.RetType + "','" + sd.FssiNo + "','" + sd.Taxgrp + "','" + sd.drSubcategory + "','" + sd.outletType + "'," +
                                "'" + sd.Delivery_Mode + "','" + sd.SecMobile_No + "','" + sd.Aadhar_No + "'";
                            try
                            {
                                int res = db.ExecQry(sqlQry);
                                if (res > 0)
    ...
    public static RetailerData[] getRetDets(string retcode)
    {
        ...
        foreach (DataRow row in dsm.Tables[0].Rows)
        {
            ...
            asd.FRErbCode = row["CustomerCode"].ToString();
    ...
    public static DataSet getRetailerDetails(string lstdrcode)
    {
        ...
        strQry = "select ListedDrCode,SF_Code,ListedDr_Name,ListedDr_Mobile,Code,CustomerCode,Contact_Person_Name,Doc_Special_Code,Doc_Spec_ShortName," +
            ...
            "ListedDr_Class_Patients,ListedDr_Consultation_Fee,Purchase_slab,ListedDr_Email,Pan_No,Tds,Tcs,isnull(State_Code,'')State_Code," +
            "Retailer_Type RetType,FssiNo as FssiNo, TaxGroup as Taxgrp, Doc_SubCatCode,Price_List_Name,Doc_QuaCode,Doc_Qua_Name,Outlet_Type,Delivery_Mode,Allowance_Type,Dist_name,isnull(ListedDr_Phone,'')SecMobile_No,isnull(Aadhar_No,'')Aadhar_No " +
