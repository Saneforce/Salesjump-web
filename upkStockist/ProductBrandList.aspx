<%@ Page Title="Product-Brand List" Language="C#" AutoEventWireup="true" MasterPageFile="~/Master_DIS.master" CodeFile="ProductBrandList.aspx.cs" Inherits="MasterFiles_ProductBrandList" EnableEventValidation="false" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div class="row">
        <div class="col-lg-12 sub-header">Brands List <span style="float: right; display: none"><a href="ProductDetail.aspx" class="btn btn-primary btn-update">Add New</a></span></div>
    </div>
        ...
        <div class="modal-dialog" role="document" style="width: 1200px !important">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Product Brand List</h5>
                    ...
                    <div class="row">
                        <div class="col-sm-12" style="padding: 15px">
                            <div class="tableBodyScroll">
                                <table class="table table-hover" id="Product_brand_Details_List">
                                    <thead class="text-warning">
                                        <tr>
                                             <th style="text-align: left">Sl.No</th>
    ...
    <script language="javascript" type="text/javascript">
        var AllOrders = []; var filter_product = [];
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "Product_Brd_SName,Product_Brd_Name,brd_count,";
        ...
        function ReloadTable() {
            ...
            for ($i = st; $i < st + PgRecords; $i++) {
                if ($i < Orders.length) {
                    tr = $("<tr></tr>");
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].Product_Brd_SName + "</td><td>" + Orders[$i].Product_Brd_Name + "</td><td class='clc'><a id='newsorder' onclick=get_cat(" + Orders[$i].Product_Brd_Code + ")  href='#'>" + Orders[$i].brd_count + "</a></td>");
        ...
        function View_Product_details(filter_product) {
            ...
            $("#Product_brand_Details_List TBODY").html("");
            //  st = PgRecords * (pgNo - 1);
            for ($i = 0; filter_product.length > $i; $i++) {
                if ($i < filter_product.length) {
                    ...
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + filter_product[$i].Product_Detail_Code + "</td><td>" + filter_product[$i].Product_Description + "</td><td>" + filter_product[$i].Sale_Erp_Code + "</td><td>" + filter_product[$i].Sample_Erp_Code + "</td>");
                    $("#Product_brand_Details_List TBODY").append(tr);
        ...
        function loadData() {
            $.ajax({
                ...
                //   async: false,
                url: "ProductBrandList.aspx/GetProductBrands",
        ...
        function get_cat(cat_no) {
            ...
            filter_product = Product_details.filter(function (a) {
                return (a.Product_Brd_Code == cat_no);
            });
