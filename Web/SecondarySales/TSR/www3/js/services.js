...
//                unlisted_doctor_master.unlisted_cat_code = addQuotes(data.category.selected.id);
            ...
            case "21":
                var ExpenseApproval = {};
                resultData.push({ 'ExpenseApproval': ExpenseApproval });
                break;
            case "22":
                var ExpenseReject = {};
                ExpenseReject.reason = addQuotes(data.reason);
                resultData.push({ 'ExpenseReject': ExpenseReject });
                break;
            case "23":
                var DCRSummary = {};
                ...
                resultData.push({ 'DCRSummary': DCRSummary });
                break;
        }
        $http.defaults.useXDomain = true;
        if (choice == "20") {
    ...
    fmcgAPI.saveDCRData = function(method, url, data, update) {
        ...
        if ((data['worktype']['selected']['FWFlg']).toString() !== "F" && (data['worktype']['selected']['FWFlg']).toString() !== "DH") {
            selectedC = "10";
        }
        else {
            for (var t = 0, jwlen = data.jontWorkSelectedList.length; t < jwlen; t++) {
        ...
        switch (selectedC) {
            ...
            case "7":
                ...
                pushKey(DCRDetail_Distributors_Hunting, { "Activity_Report_Code": "'Activity_Report_APP'" });
