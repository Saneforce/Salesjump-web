...
public partial class Default3 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if ((Convert.ToString(Session["sf_code"]) != null || Convert.ToString(Session["sf_code"]) != ""))
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                ff = viewnoti.countindashboard_MGR(div_code, sf_code);
                if (ff.Tables[0].Rows.Count > 0)
                {
                    retailer.Text = ff.Tables[0].Rows.Count.ToString();
                    Label2.Text = "Retailer";
    ...
    public static List<distributor> filldistr(string selectValue)
    {
        ...
        SqlConnection con = new SqlConnection(Globals.ConnString);
        con.Open();
        SqlCommand com = new SqlCommand("SELECT stuff(( SELECT ', ' + cast(QUOTENAME(d.sf_code,'''') as varchar(max)) FROM Mas_salesforce d where d.Reporting_to_sf='" + selectValue + "' FOR XML PATH('') ), 1, 2, '') ", con);
        SqlDataAdapter da = new SqlDataAdapter(com);
        da.Fill(df);

        if (df.Tables[0].Rows.Count > 0)
        {
            sff_code = df.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
            if (sff_code == "")
            {
                df.Clear();
                SqlCommand comf = new SqlCommand("select Stockist_Code,Stockist_Name from Mas_Stockist where Stockist_Active_Flag=0 and Division_Code ='" + div_code + "' and  Field_Code in ('" + selectValue + "') ", con);
                SqlDataAdapter da1 = new SqlDataAdapter(comf);
                da1.Fill(df);
            }
            else
            {
                df.Clear();
                SqlCommand comd = new SqlCommand("select Stockist_Code,Stockist_Name from Mas_Stockist where Stockist_Active_Flag=0 and Division_Code ='" + div_code + "' and  Field_Code in (" + sff_code + ") ", con);
                SqlDataAdapter da2 = new SqlDataAdapter(comd);
        ...
        {
            df.Clear();
            SqlCommand come = new SqlCommand("select Stockist_Code,Stockist_Name from Mas_Stockist where Stockist_Active_Flag=0 and Division_Code ='" + div_code + "'  and Field_Code in ('" + selectValue + "') ", con);
            SqlDataAdapter da3 = new SqlDataAdapter(come);
            da3.Fill(df);
        }
    ...
    public string Three_Tier()
    {
        ...
        //System.IO.File.WriteAllText(Server.MapPath(@"~\Data\DrillDownSSData.json"), ScriptValues);
    ...
    public string Three_Tier1()
    {
        ...
        ChartBAL BL = new ChartBAL();
        ChatBO.HeadDatas SFDet = BL.get_Sale_Cat(Year);
        //List<ChatBO.MainDatas> Setup = BL.get_sale_Cat_item(div_code, SFCode);
        List<ChatBO.MainDatas> Setup = BL.get_sale_Cat_item_MGR(div_code, Year, sf_code);
        ChatBO.HeadDatas SFDet1 = BL.get_Sale_Brands(Year);
        List<ChatBO.MainDatas> Setup1 = BL.get_Pur_Sale_item(div_code, Year);
        ChatBO.HeadDatas SFDet2 = BL.get_Sale_Prod(Year);
        List<ChatBO.MainDatas> Setup2 = BL.get_Sale_Prod_item(div_code, Year);
        ScriptValues += "{\"chart\":" + JsonConvert.SerializeObject(SFDet) + ",\"data\":" + JsonConvert.SerializeObject(Setup) + "," + "\"linkeddata\":[{" + "\"id\": \"2010Quarters\"," + "\"linkedchart\":{ \"chart\":" + JsonConvert.SerializeObject(SFDet1) + "," + "\"data\":" + JsonConvert.SerializeObject(Setup1) + "," + "\"linkeddata\": [{\"id\": \"2010Q1\",\"linkedchart\":{ \"chart\":" + JsonConvert.SerializeObject(SFDet2) + "," + "\"data\":" + JsonConvert.SerializeObject(Setup2) + "}} ]  }}]}";
        //string json = JsonConvert.SerializeObject(ScriptValues);
    ...
    protected void ExportToExcel(object sender, EventArgs e)
    {
        ...
        DataTable dt = LstDoc.getListedDr_new1_ex_MGR(div_code, sf_code);
    ...
    #region ExportToExcel2
    protected void ExportToExcel2(object sender, EventArgs e)
    {
        Stockist sk = new Stockist();
        Response.ClearContent();
        Response.Buffer = true;
        Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "Dist.xls"));
        Response.ContentType = "application/ms-excel";
        DataTable dt = sk.getStockist_Ex_MGR(div_code, sf_code, "", "", "0");
        string str = string.Empty;
        foreach (DataColumn dtcol in dt.Columns)
        {
            Response.Write(str + dtcol.ColumnName);
