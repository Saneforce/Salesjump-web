...
public partial class MIS_Reports_order_detailviewchgd : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        sf_code = Request.QueryString["FO_Val"].ToString();
        ...
        if (!IsPostBack)
        {
            ...
            myTemplate.HeaderText = "Order Value";
            myTemplate.Visible = false;
            gvorderview.Columns.Add(myTemplate);
            //gvorderview.Columns.Insert(7, myTemplate);
           
          
            gridorder_detail();
        }

        lblHead.Text = "ORDER DETAIL VIEW for the Month of " + strFMonthName + " " + FYear;
    ...
    private void gridorder_detail()
    {
        ...
        DCR dc = new DCR();
        dsGV = dc.view_order_detail_view(div_code, distributor, route, FMonth, FYear, sf_code);
        if (dsGV.Tables[0].Rows.Count > 0)
        {
            ...
            //dsGV.Tables[0].Columns[7].SetOrdinal(1);
            gvorderview.DataSource = dsGV;
            gvorderview.DataBind();
           
        }
        else
        {
            gvorderview.DataSource = null;
            gvorderview.DataBind();
        }
    ...
    protected void OnRowCreated(object sender, GridViewRowEventArgs e)
    {
        ...
        DataSet ff = new DataSet();
             ff = SF.GetProduct_for_orderdetail(div_code, distributor, route, FMonth, FYear, sf_code);
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            HeaderCell.BackColor = System.Drawing.Color.FromName("#3F729B");
            ...
            gvorderview.Controls[0].Controls.AddAt(0, HeaderGridRow0);
        }
    }
    protected void MainGridView_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            string DCR_CODE= e.Row.Cells[1].Text;
            string order_date = e.Row.Cells[2].Text;
            string retailer =e.Row.Cells[3].Text;;
            //GridViewRow rows = new GridViewRow();
            LinkButton myLink = new LinkButton();
          
            //myLink.PostBackUrl="Order_Detail_View.aspx?id=" + h.ToString();
            myLink.ID = "LinkButton1";
            myLink.Text = e.Row.Cells[7].Text;

            //string h = e.Rows.Cells[3].Text;
           //// myLink.Click += new EventHandler((s, e1) => myLink_Click(s, e1,h));
           // myLink.OnClientClick = String.Format("window.open('{0}')", "rpt_Order_Detail_View1.aspx?id=" + J.ToString() + "&Distributor_code=" + distributor + "&,  target=_blank");
           // myLink.Click += myLink_Click(e.Row.Cells[9].Text);
            myLink.OnClientClick = String.Format("javascript:window.open('{0}')", "rpt_Order_Detail_View1.aspx?DCR_code=" + DCR_CODE.ToString() + "&Distributor=" + distributor + "&ORDER_DATE=" + order_date + "&Territory_Name=" + route + "&ListedDr_Name=" + retailer + "&FMonth=" + FMonth + "&FYear=" + FYear + "", "target=_blank", "popUpWindow", "location=1,status=1,scrollbars=1,  width=300,height=100");
    ...
    protected void myLink_Click(object sender, EventArgs e,string id)
    {
        Response.Redirect("Order_Detail_View.aspx?id=" + id.ToString());
    }
    //protected void myLink_Click(object sender, EventArgs e)
    //{
    //    Response.Redirect("Order_Detail_View.aspx?id=" + h.ToString());
    //}
