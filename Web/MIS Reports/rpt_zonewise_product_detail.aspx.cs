...
public partial class MIS_Reports_rpt_zonewise_product_detail : System.Web.UI.Page
{
...
dtype="total";
                sURL = "rpt_zone_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&Zone_Code=" + zonecode + "&Zone_name=" + zone_name + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                     ...
                     //   dsDrr = sd.zonewise_purchase_productdetail_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString());
          dsDrr = sd.Zonewise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, product_code: drdoctor["Product_Detail_Code"].ToString(),week:i);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                   ...
                   //     dsDrr = sd.zonewise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), zonecode);
   dsDrr = sd.Zonewise_purchase_value(zonecode: zonecode, div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, product_code: drdoctor["Product_Detail_Code"].ToString(),week:i);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
