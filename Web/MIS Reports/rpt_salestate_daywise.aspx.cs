...
public partial class MIS_Reports_rpt_salestate_daywise : System.Web.UI.Page
{
...
sf_code = Session["sf_code"].ToString();
    ...
    dtype = Request.QueryString["Dtype"].ToString();
        ...
        lblHead.Text = "Sale Register statewise Product Detail for the Period of " + strFMonthName + " " + Year + " ";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                  ...
                  //  dsDrr = sd.Sale_Statewise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), statecode);
        dsDrr = sd.Sale_Statewise_value(div_code: div_code, subdivision: subdivision, sf_code: sf_code, cyear: FYear, cmonth: FMonth,day:jj, statecode: statecode, product_code: drdoctor["Product_Detail_Code"].ToString());
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    string jj = ht.ToString("yyyy-MM-dd");
                    //dsDrr = sd.purchase_disributor_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
                //    dsDrr = sd.Sale_Statewise_purchase_productdetail_total_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString());
dsDrr = sd.Sale_Statewise_value(div_code: div_code, subdivision: subdivision, sf_code: sf_code, cyear: FYear, cmonth: FMonth, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString());
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
