<%@ Page Title="" Language="C#" MasterPageFile="~/Master.master" AutoEventWireup="true" CodeFile="Rpt_DayWisePrdt.aspx.cs" Inherits="MIS_Reports_Rpt_DayWisePrdt" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="row">
        <div class="col-lg-12 sub-header">
            DayWise Productivity Report<span style="float: right; margin-right: 15px;">
    ...
    <script type="text/javascript">
        ...
        var Allclustr = [], Allsf = [], Allsf_Suvdiv = [], Allstate = [], SubDivision = [], filter_subdiv_det = [], AllMGR = [];
        var DayWisePrdCnt = []; var styl = '', trstyl = '', str = '', MnVal = 0;
        ...
        $('#ddlstate').on('change', function () {
            ...
            if (statecode != '' || statecode != 'Select State') {
                $.ajax({
                    ...
                    async: false,
                    url: "Rpt_DayWisePrdt.aspx/Getcluster",
                    ...
                    success: function (data) {
                        ...
                        Select_st = DayWisePrdCnt.filter(function (a) {
        ...
        $('#ddlclstr').on('change', function () {
            ...
            if (terr_code == '' || terr_code == 'Select cluster') {
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "Rpt_DayWisePrdt.aspx/getsubDiv",
                    ...
                    success: function (data) {
                        ...
                        Select_trr = DayWisePrdCnt.filter(function (a) {
        ...
        $('#ddldiv').on('change', function () {
            ...
            if (SubDiv_Code == '') {
                ...
                Select_div = DayWisePrdCnt.filter(function (a) {
                    return a.subdivision_code.indexOf(SubDiv_Code) > -1;
                });
                ReloadTable(Select_div);
            }
        });
        ...
        function GetMGR() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_DayWisePrdt.aspx/GetMGR",
                dataType: "json",
                success: function (data) {
        ...
        function FilterDetails(sf_code) {
            Filteritems = DayWisePrdCnt.filter(function (a) {
        ...
        function fillstate() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_DayWisePrdt.aspx/getStates",
        ...
        function GetDiv() {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "Rpt_DayWisePrdt.aspx/fillsubdivision",
                datatype: "json",
                success: function (data) {
        ...
        function loadData() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_DayWisePrdt.aspx/DayWisePrd_Cnt",
                dataType: "json",
                data: "{'Fdt':'" + FDt + "','Tdt':'" + TDt + "'}",
                success: function (data) {
                    DayWisePrdCnt = JSON.parse(data.d) || [];
                    loadHeader();
                    ReloadTable(DayWisePrdCnt);
                    $('#loadover').hide();
                },
                error: function (res) {
                    alert(res);
        ...
        function loadHeader() {
            ...
            str = "<tr " + trstyl + " ><th " + styl + " rowspan='2'>Date</th><th colspan='3' " + styl + ">" + ((strt != end) ? strt + "  -  " + end : strt) + "</th>";
            str += "<tr><th " + trstyl + styl + ">Total Calls</th><th " + trstyl + styl + ">Productive Call</th><th " + trstyl + styl + ">%</th></tr>";
            str += "</tr>";
            $('#tblRet_wise thead').append(str);
            str = '';
        }

        function ReloadTable(DayWisePrd_filterCnt) {
            str = ''; var Prdval = 0, totlval = 0, Chkdt = '', Grndttl = 0, Grndttlprd = 0, Grndprcnt = 0;
            $("#tblRet_wise TBODY").html("");
            if (DayWisePrd_filterCnt.length > 0) {
                for (var i = 0; DayWisePrd_filterCnt.length > i; i++) {
                    if ((DayWisePrd_filterCnt[i].Order_Date).toString() != Chkdt) {
                        Chkdt = (DayWisePrd_filterCnt[i].Order_Date).toString();
                        DistinctDate = DayWisePrd_filterCnt.filter(function (a) {
                            return a.Order_Date == DayWisePrd_filterCnt[i].Order_Date;
                        });
                        Prdval = 0; totlval = 0;
                        $.each(DistinctDate, function (key, val) {
                            Prdval += val.callcnt;
                            totlval += val.DCRm;
                        });
                        Grndttl += totlval;
                        Grndttlprd += Prdval;
                        var Prcnt = (Prdval != 0) ? (Prdval / totlval) * 100 : 0;
                        Grndprcnt += Prcnt;
                        str += "<tr><td>" + (DayWisePrd_filterCnt[i].Order_Date).toString() + "</td><td>" + totlval + "</td><td>" + Prdval + "</td><td>" + parseFloat(Prcnt).toFixed(2) + "</td></tr>";
                    }
                }
                str += "<tr><td style='background-color: black;color: #fff;'>Grand Total : </td><td>" + Grndttl + "</td><td>" + Grndttlprd + "</td><td>" + parseFloat(Grndprcnt).toFixed(2) + "</td></tr>";
            }
            else {
