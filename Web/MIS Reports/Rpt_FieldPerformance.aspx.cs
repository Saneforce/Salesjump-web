...
public partial class MIS_Reports_Rpt_FieldPerformance : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["div_code"] != null)
        {
            ...
            hidsfname.Value = Request.QueryString["SF_Name"].ToString();
    ...
    public static string getleave()
    {
        ...
        SqlConnection con = new SqlConnection(Globals.ConnString);
        con.Open();
        SqlCommand cmd = new SqlCommand("Select convert(varchar(10), Activity_Date, 126)  dt, Sf_Code, FieldWork_Indicator from DCRMain_Trans Where sf_code = '" + sfcode + "' "+
                                        "and FieldWork_Indicator = 'L' and convert(date, Activity_Date) between '" + FDT + "' and '" + TDT + "'", con);
        SqlDataAdapter da = new SqlDataAdapter(cmd);
        da.Fill(ds);
        con.Close();
        return JsonConvert.SerializeObject(ds);
