<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Rpt_distbeatwise_card.aspx.cs" Inherits="MIS_Reports_Rpt_distbeatwise_card" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Distributor/BeatWiseScoreCard</title>
...
<body>
    <form id="form1" runat="server">
        ...
        <div class="row">
            <div class="col-sm-8">
                <asp:Label ID="Label1" runat="server" Text="Distributor/Beat Wise Score Card" Style="margin-left: 10px; font-size: x-large"></asp:Label>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#loadover').show();
            setTimeout(function () {
                $.when(gettcpobval(), getnewstore(), getunbilledstore(), getUsers(), getbeat()).then(function () {
                    ReloadTable();
                });
            }, 1000);
        ...
        function gettcpobval() {
            $.ajax({
                ...
                async: false,
                url: "Rpt_distbeatwise_card.aspx/gettcpob",
        ...
        function getUsers() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_distbeatwise_card.aspx/getSFdets",
                data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {
        ...
        function getbeat() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_distbeatwise_card.aspx/getSFroute",
                data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {

                    bt = JSON.parse(data.d);
                }
            });
        }
        ...
        function getnewstore() {
                ...
                contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "Rpt_distbeatwise_card.aspx/getnewstor",
                    data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {
        ...
        function getunbilledstore() {
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                    url: "Rpt_distbeatwise_card.aspx/gettlsdcount",
                    data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {
        ...
        function ReloadTable() {
            ...
            var sth = '';
            var str = '';
            sth += "<tr><th rowspan='2'>S.NO</th><th rowspan='2'>Distributor Name</th><th rowspan='2'>Beat Name</th><th rowspan='2'>AC</th>";
            ...
            for (var i = 0; i < sfusers.length; i++) {
                ...
                var totshponle = 0;
                var pertoac = 0;
                let filtrout = bt.filter(function (a) {
                    return a.stockist == sfusers[i].Stockist_Code;
                });
                str += "<tr colspan=2><td>" + (i + 1) + "</td><td>" + sfusers[i].Stockist_Name + "</td><td>" + (filtrout.length > 0 ? filtrout[0].Territory_Name : '') + "</td><td>" + sfusers[i].ac + "</td>";
               
                let filtarr = tcpob.filter(function (a) {
                    return a.stockist_code == sfusers[i].Stockist_Code;
                    });
                ...
                var varnews = lenns.filter(function (el) {
                    const val = mymaps.get(el.Stockist_Code);
                    if (val) {
                        return false;
                    }
                    mymaps.set(el.Stockist_Code, el.Sf_Code);
                    return true;
                }).map(function (a) { return a; });
