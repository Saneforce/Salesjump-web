<%@ Page Language="C#" AutoEventWireup="true" CodeFile="view_point_sum_rep.aspx.cs" Inherits="MIS_Reports_view_point_sum_rep" %>
...
<body>
                ...
                <table class="table table-hover" id="OrderList" style="font-size: 12px">
                    <thead class="text-warning">
                        <tr style="background-color: #37a4c6; color: #fff; white-space: nowrap;">
                            ...
                            <th style="text-align: left; color: #fff">Phone</th>
                            <th style="text-align: left; color: #fff">Value</th>
                            <th style="text-align: left; color: #fff">Points</th>
                          </tr>
                    </thead>
                    <tbody>
                    </tbody>
    ...
    <script language="javascript" type="text/javascript">
        var AllOrders = []; var Allsf = []; var hqsf = []; var Allstate = []; var Ostate = []; var AllHQ = []; var SFHQ = []; var pnt = []; var pntv = '';
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "retailername,route,";
        var routes = [];
        var filtrkey = '0';
        ...
        function ReloadTable() {
            ...
            for ($i = st; $i < st + Number(PgRecords); $i++) {
                if ($i < Orders.length) {
                    point = parseInt(Orders[$i].Order_Value / pntv);
                    tr = $("<tr rname='" + Orders[$i].retailername + "' rocode='" + Orders[$i].ListedDrCode + "'></tr>");
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].retailername + "</td><td>" + Orders[$i].route + "</td><td>" + Orders[$i].ListedDr_Mobile + "</td><td>" + Orders[$i].Order_Value + "</td><td>" + point + "</td>");
        ...
        function fillgift() {
            $.ajax({
                ...
                async: false,
                url: "view_point_sum_rep.aspx/getfilldata",
        ...
        function fillpoint() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "view_point_sum_rep.aspx/getfillpoint",
                data: "{'divcode':'<%=Session["div_code"]%>'}",
                dataType: "json",
                success: function (data) {
                    pnt = JSON.parse(data.d) || [];
                    pntv = pnt[0].Point_Value;
                  }
            });
        }
