...
public partial class MIS_Reports_rpt_value_primary_vs_secondary : System.Web.UI.Page
{
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                     ...
                     if (stype == "3")
                        {
                            dsDrr = sd.purchase_value_product_weekwise(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), stockist);
foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
                            ...
                            try
                            {
                                ...
                                lit_det_dr_name.Text = per.ToString("0.00");
                            }
                            catch
                            {
                                lit_det_dr_name.Text = drrdoctor["value"]==DBNull.Value ? drrdoctor["value"].ToString(): Convert.ToDecimal( drrdoctor["value"]).ToString("0.00");
                            }
...
tc_det_dr_name.HorizontalAlign = HorizontalAlign.Right;
                        ...
                        {
                            dsDrr = sd.sale_value_product_weekwise(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), stockist);
foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
                            TableCell tc_det_dr_name = new TableCell();
                            Literal lit_det_dr_name = new Literal();
                            try
                            {
                                ...
                                lit_det_dr_name.Text = per1.ToString("0.00");
                            }
                            catch
                            {
                                lit_det_dr_name.Text = drrdoctor["value"]==DBNull.Value ? drrdoctor["value"].ToString(): Convert.ToDecimal( drrdoctor["value"]).ToString("0.00");
