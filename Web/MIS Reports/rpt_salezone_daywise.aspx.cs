...
public partial class MIS_Reports_rpt_salezone_daywise : System.Web.UI.Page
{
...
dtype = Request.QueryString["Dtype"].ToString();
        ...
        lblHead.Text = "Purchase Register Zonewise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
...
tc_det_SNo.Style.Add("text-align","right");
				...
				//   dsDrr = sd.sale_zonewise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), zonecode);
                    dsDrr = sd.sale_Zonewise_purchase_value( div_code: div_code, cmonth: FMonth, cyear: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(), zonecode: zonecode,sf_code:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    //dsDrr = sd.purchase_disributor_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
                 //   dsDrr = sd.sale_zonewise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(),zonecode);
dsDrr = sd.sale_Zonewise_purchase_value(div_code: div_code,cmonth: FMonth,cyear: FYear,day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(),sf_code:sf_code);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
