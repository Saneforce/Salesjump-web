...
public partial class MIS_Reports_rpt_sale_purchase_view : System.Web.UI.Page
{
...
sf_code = Session["sf_code"].ToString();
 ...
 subdivision = Request.QueryString["subdivision"].ToString();
        ...
        lblHead.Text = "Sales Register Distributorwise Product Detail for the Month of " + strFMonthName + " " + FYear;
...
leave_utility.Style.Add("text-align","center");
  ...
  dtype="total";
                sURL = "rpt_saledistributor_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&distributor=" + distributor + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                      ...
                      //  dsDrr = sd.sales_register_productdetail_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString());
   dsDrr = sd.sales_register_productdetail(divcode: div_code, Month: FMonth, Year: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), sf_code: sf_code);
...
leave_utility.Style.Add("text-align","center");
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
                ...
                lit_catg_namee.Text = " Week-" + i.ToString();
  dtype="value";
                sURL = "rpt_saledistributor_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&distributor=" + distributor + "&stockist_name="+ Stock_name+"&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
                lit_catg_namee.Attributes["onclick"] = "javascript:window.open('" + sURL + "' ,'','resizable=yes,toolbar=no,menubar=no,scrollbars=1,status=no,width=500,height=600,left=0,top=0,zoom=50%');";
                lit_catg_namee.Style.Add("Color", "Blue");
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                  //      dsDrr = sd.sales_register_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor);
   dsDrr = sd.sales_register_productdetail(divcode: div_code, Month: FMonth, Year: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), distributor: distributor, sf_code: sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
