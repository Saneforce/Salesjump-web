...
public partial class MIS_Reports_rpt_sale_zonewise_product_detail : System.Web.UI.Page
{
...
string sURL = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        zonecode = Request.QueryString["Zone_Code"];
        zone_name = Request.QueryString["Zone_name"];
 ...
 subdivision = Request.QueryString["subdivision"].ToString();
   ...
   if (stype == "1")
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Sale Register Zonewise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
            lblProd.Font.Bold = true;
            lblname.Text = zone_name;
...
dtype="total";
                sURL = "rpt_salezone_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&Zone_Code=" + zonecode + "&Zone_name=" + zone_name + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                      ...
                      //  dsDrr = sd.sale_zonewise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), zonecode);
                        dsDrr = sd.sale_Zonewise_purchase_value( div_code: div_code,cmonth: FMonth,cyear: FYear,week: i, product_code: drdoctor["Product_Detail_Code"].ToString(),sf_code:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                     ...
                     //   dsDrr = sd.sale_zonewise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), zonecode);
                        dsDrr = sd.sale_Zonewise_purchase_value(div_code: div_code, cmonth: FMonth,cyear: FYear,week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), zonecode: zonecode,sf_code:sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
