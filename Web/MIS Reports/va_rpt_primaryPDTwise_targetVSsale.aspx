<%@ Page Language="C#" AutoEventWireup="true" CodeFile="va_rpt_primaryPDTwise_targetVSsale.aspx.cs" Inherits="MIS_Reports_va_rpt_primaryPDTwise_targetVSsale" %>
        ...
        $(document).ready(function () {
            ...
            genReport = function () {
                if (pDs.length > 0) {
                    ...
                    var len = 1;
                    var fd = new Date(tYear + '/' + fMonth + '/' + "01/");
                    var td = new Date(tYear + '/' + tMonth + '/' + "01/");
                    cd = fd;
                    var str = '<th rowspan="2">Field_Force Name</th><th rowspan="2">Designation</th><th rowspan="2">State</th>';
                    var st2 = '';
                    var stkDt = '';
                    var st4 = '<th colspan="3" style="text-align: left;">Total </th>';

                    while (cd <= td) {
                        str += '<th colspan="2"> ' + month[cd.getMonth()] + ' - ' + cd.getFullYear() + ' </th>';
                        stkDt += '<th>Target Value</th><th>Sale Value</th>';
                    ...
                    $(tbl).find('thead').append('<tr style="background-color: #496a9a; color: white;">' + stkDt + '<th>Target Value</th><th>Sale Value</th></tr>');
                   ...
                   var ogVSTot = [];
                    ...
                    for (var i = 0; i < pDs.length; i++) {
                        var str = '<td>' + pDs[i].sf_name + '</td><td>' + pDs[i].Designation + '</td><td>' + pDs[i].StateName + '</td> ';
                        md = new Date(tYear + '/' + fMonth + '/' + "01/");
                        var j = 0;
                        TT = 0;
                        TS = 0;
                        TTQ = 0;
                        ...
                        while (md <= td) {
                            cR = Dtls.filter(function (obj) {
                                return (Number(obj.cmonth) === Number((md.getMonth() + 1)) && Number(obj.cyear) === Number(md.getFullYear()) && obj.pCode === pDs[i].SF_Code);
                            });
                            var T = '';
                            var S = '';
                        ...
                        $(tbl).find('tbody').append('<tr>' + str + ' <td style="background-color: #b3a46d; color: white;"> ' + TT.toFixed(2) + ' </td><td style="background-color: #b3a46d; color: white;">' + TS.toFixed(2) + '</td> </tr> ');
            ...
            $.ajax({
                ...
                async: false,
                url: "va_rpt_primaryPDTwise_targetVSsale.aspx/getProductTargetSale",
                data: "{'sf_Code':'" + sfCode + "','fMonth':'" + fMonth + "','tYear':'" + tYear + "','tMonth':'" + tMonth + "'}",
                dataType: "json",
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "va_rpt_primaryPDTwise_targetVSsale.aspx/getdata",
                data: "{'sf_Code':'" + sfCode + "','SubDiv':'" + SubDiv + "','fMonth':'" + fMonth + "','tYear':'" + tYear + "','tMonth':'" + tMonth + "'}",
                dataType: "json",
                success: function (data) {
                    pDs = data.d;
                    console.log(data.d);
