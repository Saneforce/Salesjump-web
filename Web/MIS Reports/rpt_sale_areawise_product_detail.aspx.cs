...
public partial class MIS_Reports_rpt_sale_areawise_product_detail : System.Web.UI.Page
{
  ...
  subdivision = Request.QueryString["subdivision"].ToString();
   ...
   if (stype == "1")
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Sale Register Areawise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
...
leave_utility.Style.Add("text-align","center");
      ...
      dtype="total";
                sURL = "rpt_salearea_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&Area_Code=" + areacode + "&Area_Name=" + area_name + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    //    dsDrr = sd.sale_areawise_purchase_productdetail_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString());
    dsDrr = sd.sale_areawise_purchase_value(div_code: div_code, cmonth: FMonth,cyear: FYear,week: i, product_code: drdoctor["Product_Detail_Code"].ToString(),sf_cdoe:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                   ...
                   //     dsDrr = sd.sale_areawise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), areacode);
        dsDrr = sd.sale_areawise_purchase_value( div_code: div_code, cmonth: FMonth, cyear: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), areacode: areacode ,sf_cdoe:sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
