...
public partial class MIS_Reports_rpt_zone_daywise : System.Web.UI.Page
{
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                  ...
                  //  dsDrr = sd.zonewise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), zonecode);
    dsDrr = sd.Zonewise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, zonecode:zonecode, product_code: drdoctor["Product_Detail_Code"].ToString(), day: jj);
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    string jj = ht.ToString("yyyy-MM-dd");
                    //dsDrr = sd.purchase_disributor_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
                  //  dsDrr = sd.zonewise_purchase_productdetail_total_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString());
dsDrr = sd.Zonewise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code,  product_code: drdoctor["Product_Detail_Code"].ToString(), day: jj);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
