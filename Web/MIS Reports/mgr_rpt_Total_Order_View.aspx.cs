...
public partial class MIS_Reports_mgr_rpt_Total_Order_View : System.Web.UI.Page
{
    ...
    private void FillSF()
    {
        ...
        {
            dsGV = dc1.view_total_order_view(divcode, sfCode, Date);
            if (dsGV.Tables[0].Rows.Count > 0)
            {
                gvtotalorder.DataSource = dsGV;
                gvtotalorder.DataBind();
            ...
            dsGc = dc1.view_total_order_view_categorywise(divcode, sfCode, Date, subdivision_code);
            ...
            valTotal = 0;
            foreach (DataRow drFFs in dsGc.Tables[0].Rows)
            {

                valTotal += Convert.ToDecimal(drFFs["value"]);
            ...
            foreach (DataRow drFF in dsGc.Tables[0].Rows)
            {
                ...
                //                stCrtDtaPnt += Convert.ToString(Math.Round(((Convert.ToDecimal(drFF["value"]) / valTotal) * 100), 2)) + "},";
            }
    ...
    private void AddTotalRow(string labelText, string totvalue, string disprice, string free, string netvalue, string value, string stk)
    {
        ...
        row.Cells.AddRange(new TableCell[7] { new TableCell (), //Empty Cell
                                        new TableCell { Text = labelText, HorizontalAlign = HorizontalAlign.Right,ColumnSpan=9},
    ...
    public class ord
    {
        public DataSet view_total_order_view(string div_code, string sf_code, string date)
        {
            ...
            string strQry = "exec [mgr_TODAY_ORDER_VIEW_DISCOUNT_FREEVAL] '" + sf_code + "','" + div_code + "','" + date + "'";
            try
            {
        ...
        public DataSet view_total_order_view_categorywise(string div_code, string sf_code, string date, string subdivcode)
        {
            ...
            string strQry = "exec [mgr_TODAY_ORDER_CATEGORY_VIEW] '" + sf_code + "','" + div_code + "','" + date + "','" + subdivcode + "'";
            try
            {
                dsAdmin = db_ER.Exec_DataSet(strQry);
            }
