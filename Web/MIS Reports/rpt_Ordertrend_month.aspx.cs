...
public partial class MIS_Reports_rpt_Ordertrend_month : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Retails Trend Analysis Details for  " + strFMonthName + " " + FYear + "To" + " " + strTMonthName + " " + TYear;
        if (viewby == "Statewise")
        {
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        Order pf = new Order();
                        dsDoc = pf.Order_statewise_Trend_analysis_value_singleProduct(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code);
                ...
                tr_det.Cells.Add(tc_lst_month1);
                        ...
                        dsDoc = pf.Order_statewise_Trend_analysis_value_singleProduct_total(divcode, cmonth, cyear, subdivision, State_Code);
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        ...
                        Order gg = new Order();
                        dsDoc = gg.Order_statewise_Trend_analysis_value_singleProduct(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        tr_det.Cells.Add(tc_det_last6monthsum);
                        Order dff = new Order();
                        dsDoc = dff.Order_areawise_Trend_analysis_singleproduct(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code, Areacode);
                ...
                tr_det.Cells.Add(tc_lst_month1);
                        ...
                        dsDoc = dff.Order_areawise_Trend_analysis_singleproduct_total(divcode, cmonth, cyear, subdivision, State_Code, Areacode);
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        ...
                        Order pf = new Order();
                        dsDoc = pf.Order_areawise_Trend_analysis_singleproduct(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code, Areacode);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
  sec_total = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
 if (sec_total!= "")
            ...
            tc_tot_monthd.Attributes.Add("Class", "rptCellBorder");
            tr_total.Cells.Add(tc_tot_monthd);
 iTotLstnetval=0;iTotLstCount = 0;
TableCell tc_tot_monthdg = new TableCell();
 tc_tot_monthdg.Attributes.Add("Class", "rptCellBorder");
   tr_total.Cells.Add(tc_tot_monthdg);
                    cmonth = cmonth + 1;
...
tc_DR_Code_monthe.Style.Add("text-align","center");
                    //tc_DR_Code_month.RowSpan = 1;
                    Literal lit_DR_Code_mone = new Literal();
                    lit_DR_Code_mone.Text = "<center>%Contribution</center>";
                    tc_DR_Code_monthe.Controls.Add(lit_DR_Code_mone);
                    tc_DR_Code_monthe.Attributes.Add("Class", "rptCellBorder");
                    tc_DR_Code_monthe.BorderColor = System.Drawing.Color.Black;
                    //tc_DR_Code.Visible = false;
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        Order df = new Order();
                        dsDoc = df.Order_Zonewise_Trend_analysis_totalvalue_singleprdt(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code, Areacode, ZoneCode);
...
TableCell tc_lst_month1 = new TableCell();
                ...
                tr_det.Cells.Add(tc_lst_month1);
                        ...
                        tr_det.Cells.Add(tc_det_currentmonth);
                        Order jj = new Order();
                        dsDoc = jj.Order_Zonewise_Trend_analysis_totalvalue_singleprdt_total(divcode, cmonth, cyear, subdivision, State_Code, Areacode, ZoneCode);
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        ...
                        Order ff = new Order();
                        dsDoc = ff.Order_Zonewise_Trend_analysis_totalvalue_singleprdt(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code, Areacode, ZoneCode);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
sec_total = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
 if (sec_total!= "")
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        Order ss = new Order();
                        dsDoc = ss.Order_Territorywise_Trend_analysis_totalvalue_singleprt(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, subdivision, State_Code, Areacode, ZoneCode, Territory_Code);
 ...
 sec_value = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
          ...
          TableCell tc_lst_month1 = new TableCell();
                ...
                tr_det.Cells.Add(tc_lst_month1);
                        ...
                        Order ff = new Order();
                        dsDoc = ff.Order_Territorywise_Trend_analysis_totalvalue_singleprt_total(divcode, cmonth, cyear, subdivision, State_Code, Areacode, ZoneCode, Territory_Code);
