...
public partial class MIS_Reports_report_purchase_brand_daywise : System.Web.UI.Page
{
...
string dtype=string.Empty;
   ...
   sf_code = Session["sf_code"].ToString();
        ...
        brand_code = Request.QueryString["Product_Brand_Code"].ToString();              
...
dtype = Request.QueryString["Dtype"].ToString();
        ...
        lblHead.Text = "BrandWise Details for the Month of " + strFMonthName + " " + FYear;
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drdo in gg.Tables[0].Rows)
                    {
                        ...
                        //dsDrr = sd.purchase_Brandview_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code,brand_code);
   					      dsDrr = sd.Purchase_Brand_value_categorywise(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, day: jj, cat_code: cat_code, stockistcode: distributor, brandcode: brand_code, product_code: drdoctor["Product_Detail_Code"].ToString());
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    //    dsDrr = sd.purchase_Brandview_daywise_details_total(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
 dsDrr = sd.Purchase_Brand_value_categorywise(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, day: jj, cat_code: cat_code, stockistcode: distributor, product_code: drdoctor["Product_Detail_Code"].ToString());
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
