...
public partial class MIS_Reports_rpt_saledistributor_daywise : System.Web.UI.Page
{
...
//Stock_name = Request.QueryString["stockist_name"].ToString();
        ...
        lblHead.Text = " Sale Register Distributorwise Product Detail DayWise for the Month of " + strFMonthName + " " + FYear;
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                //    dsDrr = sd.sales_register_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
    dsDrr = sd.sales_register_productdetail( divcode: div_code, Month: FMonth, Year: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(), distributor: distributor,sf_code:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    DateTime ht = Convert.ToDateTime(f);
                    string jj = ht.ToString("yyyy-MM-dd");
                   // dsDrr = sd.sales_register_productdetail_total_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString());
      dsDrr = sd.sales_register_productdetail(divcode: div_code, Month: FMonth, Year: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(),sf_code:sf_code);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
