...
public partial class MIS_Reports_view_countofshop : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Billed Outlet View Details For "+ Request.QueryString["month"].ToString() +"-"+ year;
    }
   ...
   [WebMethod(EnableSession = true)]
    public static string getdatanshp()
    {
        ...
        DataSet dsProd = dv.Get_Product_shopcnt_dtl(Div_code, Sub_Div, SF_Code,productcode, year);
        return JsonConvert.SerializeObject(dsProd.Tables[0]);
    }
    public class shopdt
    {
        public DataSet Get_Product_shopcnt_dtl(string Div_Code, string Sub_Div, string SF_Code, string Product_Code, string year)
        {
            DB_EReporting db_ER = new DB_EReporting();
            DataSet dsState = null;
            string strQry = "EXEC SKY_Analysis_shopcount_detail '" + Div_Code + "','" + Sub_Div + "','" + SF_Code + "','" + Product_Code + "','" + year + "','" + month + "'";
            try
            {
