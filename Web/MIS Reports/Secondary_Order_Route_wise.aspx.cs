...
public partial class MIS_Reports_Secondary_Order_Route_wise : System.Web.UI.Page
{
    ...
    private void FillSF()
    {
        ...
        SalesForce sf = new SalesForce();
            dsSalesForce = sf.GetRouteName_Distributorwise(stockist_code,divcode);
        ...
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                stURL = "Secondary_Order_Retailer_wise.aspx?Stockist_Code=" + stockist_code + "&Stockist_name=" + Stock_name + " &cat_Code=" + drFF["Territory_Code"] + " &cat_name=" + drFF["Territory_Name"].ToString() + "&FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + " &subdivision=" + subdivision + "";

                //stURL = "sec_od_routeweekwise.aspx?Stockist_Code=" + stockist_code + "&Stockist_name=" + Stock_name + " &retailer_code=" + drFF["Territory_Code"] + " &retailer_name=" + drFF["Territory_Name"].ToString() + "&FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + "&subdivision=" + subdivision + "";
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        if (tot_dr != "0")
                        {
                            ...
                            stURL = "sec_od_routeweekwise.aspx?Stockist_Code=" + stockist_code + "&Stockist_name=" + Stock_name + " &route_code=" + drFF["Territory_Code"] + " &route_name=" + drFF["Territory_Name"].ToString() + "&FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + "&subdivision=" + subdivision + "";
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        //int iTotLstCount = 0;
                        foreach (DataRow dr1FF in dsSalesForce.Tables[0].Rows)
                        {

                            //dsDoc = sf.getProduct_Exp_count(drFF["sf_code"].ToString(), divcode, cmonth, cyear, Convert.ToInt16(Prod), sCurrentDate);

                            dsDoc = sf.Route_wise_sale_value_stockist(dr1FF["Territory_Code"].ToString(), divcode, cmonth, cyear, sCurrentDate, stockist_code);
                            if (dsDoc.Tables[0].Rows.Count > 0)
                            {
