<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RptAllenSuperStockList.aspx.cs" Inherits="MIS_Reports_RptAllenSuperStockList" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Super Stock List Report </title>
        ...
        jQuery(document).ready(function ($) {
           ...
           // var fmonth = $("#<%=hfmonth.ClientID%>").val();
            ...
            $.ajax({
                ...
                async: false,
                url: "RptAllenSuperStockList.aspx/GetAllData",
                ...
                success: function (data) {
                    ...
                    if (prod.length > 0) {
                        var tbl = $('#tblFF');
                        //var str = '<th>Submitted Date</th><th>Submitted Time </th><th>State</th><th>H.Q</th><th>Field Force</th><th>Distributor Code</th><th>Distributor ERP Code</th><th>Distributor Name</th><th>Product Name </th><th class="">Case</th><th class="">Piece</th><th>Total Units</th><th style="height: 5%;width: 2%;">Pictures</th>';
                        var str = '<th>Submitted Date</th><th>State</th><th>H.Q</th><th>Field Force</th><th>Distributor Code</th><th>Distributor ERP Code</th><th>Distributor Name</th><th>Product Name </th><th class="">Quantity</th><th class="">Total Quantity</th><th>Total Value</th><th style="height: 5%;width: 2%;">Pictures</th>';
                        ...
                        for (var j = 0; j < prod.length; j++) {
                            ...
                            var today = new Date(prod[j].SubmittedDate);
                            ...
                            //str = '<td>' + today + '</td><td>' + prod[j].SubmittedTime + '</td><td>' + prod[j].State + '</td><td>' + prod[j].HQ + '</td><td>' + prod[j].FieldForce + '</td><td>' + prod[j].DistributorCode + '</td><td>' + prod[j].Erp_Code + '</td><td>' + prod[j].DistributorName + '</td><td>' + prod[j].ProductName + '</td><td class="">' + prod[j].CaseQty + '</td><td class="">' + prod[j].PieceQty + '</td><td>' + prod[j].TotalUnits + '</td><td>' + ((prod[j].Imgurl != '') ? $imgtg : '') + '</td>';

                            str = '<td>' + today + '</td><td>' + prod[j].State + '</td><td>' + prod[j].HQ + '</td><td>' + prod[j].FieldForce + '</td><td>' + prod[j].DistributorCode + '</td><td>' + prod[j].Erp_Code + '</td><td>' + prod[j].DistributorName + '</td><td>' + prod[j].ProductName + '</td><td class="">' + prod[j].CaseQty + '</td><td class="">' + prod[j].PieceQty + '</td><td>' + prod[j].TotalUnits + '</td><td>' + ((prod[j].Imgurl != '') ? $imgtg : '') + '</td>';
