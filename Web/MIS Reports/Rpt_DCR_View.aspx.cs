...
try
{
	   ...
	   DateTime result1 = DateTime.ParseExact(Tdate, "yyyy-MM-dd", CultureInfo.InvariantCulture);
           ...
           lblTitle.Text = "Daily Closing Report (<span style='color:Red'>" + "Stock View" + "</span>) for the month of " + sMonth;
            ...
            if (sf_code.Contains("MR"))
            {
                ...
                if (dsGV.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        for (int j = 0; j < dsGV.Tables[0].Rows.Count; j++)
                        {
        ...
        else if (strMode == "SKU Summary")
        {
            ...
            lblTitle.Text = "<h1>Daily Call Report - " + " Date : <span style='color:Red'> " + dt + " </span>  </h1>";
            //lblFieldForceName.Text = Sf_Name;
            DataSet dsGV = new DataSet();
            DCR dc = new DCR();
    ...
    protected void OnRowCreated(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            HeaderGridRow0.Cells.Add(HeaderCell);
			 ...
			 HeaderGridRow1.HorizontalAlign = HorizontalAlign.Right;
                         ...
                         if (dsdoc.Tables[0].Rows.Count > 0)
                         {
                            ...
                            form1.Controls.Add(tbldetail_main3);
                             ...
                             foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                             {
                                 ...
                                 DCR sf = new DCR();
                                 dsDrr = sf.dcr_Gettransno(sf_code, div_code, hdate3, drdoctor["Trans_Detail_Info_Code"].ToString(), drdoctor1["stockist_code"].ToString());
                                 int[] fi = new int[500];
                                 int j = 0;
                                 foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                                 {
