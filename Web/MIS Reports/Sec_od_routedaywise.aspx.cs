...
public partial class MIS_Reports_Sec_od_routedaywise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Secondary Order Distributor Daywise Details for the Month of " + strFMonthName + " " + FYear;
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
            {
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    DateTime ht = Convert.ToDateTime(f);
                    string jj = ht.ToString("yyyy-MM-dd");
                    dsDrr = sd.retail_routewise_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor,route_code);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
...
foreach (DataRow drdo in gg.Tables[0].Rows)
{
  ...
  foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
    ...
    dsDrr = sd.retail_routewise_view_daywise_details(div_code, FMonth, FYear, jjj, drdoctor["Product_Detail_Code"].ToString(), distributor,route_code);
    if (dsDrr.Tables[0].Rows.Count > 0)
    {
