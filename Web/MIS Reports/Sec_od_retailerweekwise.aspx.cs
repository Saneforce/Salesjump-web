...
public partial class MIS_Reports_Sec_od_retailerweekwise : System.Web.UI.Page
{
 ...
 Stock_name = Request.QueryString["Stockist_name"].ToString();
        ...
        lblHead.Text = "Secondary Order Retailerwise Weekwise for the Month of " + strFMonthName + " " + FYear;
               

           lblRoute.Text=route_name;
          lblret.Text=retailer_name;
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                        ...
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
                                ...
                                lit_det_dr_name.Text = Convert.ToDecimal(drrdoctor["Rec_Qty"]).ToString("0.00");
                                ...
                                lit_det_dr_hq.Text = Convert.ToDecimal(drrdoctor["VALUE"]).ToString("0.00");
...
leave_utility.Style.Add("text-align","center");
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
                ...
                lit_catg_namee.Text = " Week-" + i.ToString();
                dtype = "value";
                sURL = "Sec_od_retailerdaywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&distributor=" + distributor + "&Subdivision=" + subdivision + "&route_code=" + route_code + "&route_name=" + route_name + "&retailer_code=" + retailer_code + "&retailer_name=" + retailer_name + "&Dtype=" + dtype + "";
                lit_catg_namee.Attributes["onclick"] = "javascript:window.open('" + sURL + "' ,'','resizable=yes,toolbar=no,menubar=no,scrollbars=1,status=no,width=500,height=600,left=0,top=0,zoom=50%');";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                        dsDrr = sd.retail_retailer_view_weekise_details(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, route_code, retailer_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {     						
                    ...
                    foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
                    {
                        ...
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drr1doctor in dsDrr.Tables[0].Rows)
                            {
                                ...
                                quantot =drr1doctor["Quantity"] ==DBNull.Value ? drr1doctor["Quantity"].ToString(): Convert.ToDecimal( drr1doctor["Quantity"]).ToString("0.00"); 
// Convert.ToDecimal( drr1doctor["Quantity"]).ToString("0.00");
