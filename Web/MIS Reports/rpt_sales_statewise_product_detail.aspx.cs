...
public partial class MIS_Reports_rpt_sales_statewise_product_detail : System.Web.UI.Page
{
  ...
  System.Globalization.DateTimeFormatInfo mfi = new System.Globalization.DateTimeFormatInfo();
      ...
      if (stype == "1")
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Sales Register statewise Product Detail for the Period of " + strFMonthName + " " + Year + " ";
...
leave_utility.Style.Add("text-align","center");
 ...
 dtype="total";
                sURL = "rpt_salestate_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&State_Code=" + statecode + "&State_Name=" + State_name + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                      ...
                      //  dsDrr = sd.Sale_Statewise_purchase_productdetail_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString());
  dsDrr = sd.Sale_Statewise_value(div_code: div_code, subdivision: subdivision, sf_code: sf_code, cyear: FYear, cmonth: FMonth, week: i, product_code: drdoctor["Product_Detail_Code"].ToString());
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
            {
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                    //    dsDrr = sd.Sale_Statewise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), statecode);
  dsDrr = sd.Sale_Statewise_value(div_code: div_code, subdivision: subdivision, sf_code: sf_code, cyear: FYear, cmonth: FMonth, week: i, statecode: statecode, product_code: drdoctor["Product_Detail_Code"].ToString());
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
