...
public partial class MIS_Reports_rptPurchas_Register_Brand_wise : System.Web.UI.Page
{
...
string subdivision=string.Empty;
   ...
   subdivision = Request.QueryString["subdivision"].ToString();
        ...
        lblHead.Text = "Purchase Register -Brand Wise for  " + strFMonthName + " " + FYear + " " + "To" + " " + strTMonthName + " " + TYear;
...
stCrtDtaPnt+="{label:\""+ drFF["Product_Brd_Name"].ToString()+"\",y:";
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                       ...
                       // dsDoc = sf.Purchase_Brand_value_categorywise(drFF["Product_Brd_Code"].ToString(), divcode, cmonth, cyear, sCurrentDate, cat_code,stockist_code);

						dsDoc = sf.Purchase_Brand_value_categorywise(product_code: "", divcode: divcode, sf_code: sfCode, cyear: cyear, cmonth: cmonth, stockistcode: stockist_code, cat_code: cat_code, brandcode: drFF["Product_Brd_Code"].ToString());
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
                        ...
                        if (tot_dr != "0")
                        {
                            ...
                            stype = "1";
          stURL = "rpt_purchase_brandwise_weekwise.aspx?Stockist_Code=" + stockist_code + "&Stockist_name=" + Stock_name + "&Brand_Code=" + drFF["Product_Brd_Code"] + "&Brand_name=" + drFF["Product_Brd_Name"].ToString() + " &cat_Code=" + cat_code + " &cat_name=" + cat_name + "&Year=" + cyear + "&Month=" + cmonth + " &stype=" + stype + "&subdivision=" + subdivision + "";
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        ...
                        //dsDoc = sf.Purchase_Brand_value_categorywise(drFF["Product_Brd_Code"].ToString(), divcode, cmonth, cyear, sCurrentDate, cat_code,stockist_code);
					      dsDoc = sf.Purchase_Brand_value_categorywise(divcode: divcode, sf_code: sfCode, cyear: cyear, cmonth: cmonth, stockistcode: stockist_code, cat_code: cat_code, brandcode: drFF["Product_Brd_Code"].ToString());
                        if (dsDoc.Tables[0].Rows.Count > 0)
                        tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
                        ...
                        if (tot_dr != "")
                        {
							...
							stype = "2";
                            stURL = "rpt_purchase_brandwise_weekwise.aspx?Stockist_Code=" + stockist_code + "&Stockist_name=" + Stock_name + "&Brand_Code=" + drFF["Product_Brd_Code"] + "&Brand_name=" + drFF["Product_Brd_Name"].ToString() + " &cat_Code=" + cat_code + " &cat_name=" + cat_name + "&Year=" + cyear + "&Month=" + cmonth + " &sCurrentDate=" + sCurrentDate + "&stype=" + stype + "&subdivision=" + subdivision + "";
