...
public partial class MIS_Reports_rpt_retail_Distributor_wise_view : System.Web.UI.Page
{
  ...
  if (stype == "1")
        {
             ...
             dsDr = dr.retail_register_productdetail(div_code, stockist, Convert.ToInt16(Year), Convert.ToInt16(Month));
        }
        if (stype == "2")
        {
            dsDr = dr.retail_register_productdetail_total(div_code,Convert.ToInt16(Year), Convert.ToInt16(Month));
        }
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                lit_det_dr_name.Text =  drdoctor["Quantity"] ==DBNull.Value ? drdoctor["Quantity"].ToString() : Convert.ToDecimal( drdoctor["Quantity"]).ToString("0.00") ;
