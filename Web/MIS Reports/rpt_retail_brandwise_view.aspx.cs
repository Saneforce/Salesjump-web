...
public partial class MIS_Reports_rpt_retail_brandwise_view : System.Web.UI.Page
{
    ...
    private void CreateDynamicTable()
    {
        ...
        if (stype == "1")
        {
            dsDr = dr.retail_register_brandwisedetail_categorywise_stockist(div_code, Brand, Convert.ToInt16(Year), Convert.ToInt16(Month), catcode, stockist_code);
        }

        if (type == "2")
        {
            dsDr = dr.retail_register_brandwisedetail_categorywise(div_code, Convert.ToInt16(Year), Convert.ToInt16(Month), catcode, stockist_code);
        }
