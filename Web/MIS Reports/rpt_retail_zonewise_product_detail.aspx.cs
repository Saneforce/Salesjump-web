...
public partial class MIS_Reports_rpt_retail_zonewise_product_detail : System.Web.UI.Page
{
    DataSet dsDr = null;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Retail Register Zonewise Product Detail  for the Period of " + strMonthName + " " + Year + " ";
...
if (stype == "1")
        {
 dsDr = dr.retail_zonewise_purchase_productdetail(div_code, zonecode, Convert.ToInt16(Year), Convert.ToInt16(Month),subdivision);
        }

        if (stype == "2")
        {
 dsDr = dr.retail_zonewise_purchase_productdetail_total(div_code, Convert.ToInt16(Year), Convert.ToInt16(Month),subdivision);
        }
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            lit_det_head_qua.Text = "<b>Quantity</b>";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                lit_det_dr_name.Text = drdoctor["Quantity"]==DBNull.Value ? drdoctor["Quantity"].ToString():Convert.ToDecimal( drdoctor["Quantity"]).ToString("0.00");// drdoctor["Quantity"].ToString();
...
tc_det_dr_name.HorizontalAlign = HorizontalAlign.Right;
                ...
                lit_det_dr_hq.Text = drdoctor["value"]==DBNull.Value ? drdoctor["value"].ToString():Convert.ToDecimal( drdoctor["value"]).ToString("0.00");
