...
public partial class MIS_Reports_rpt_Customer_sales_analysis_inv : System.Web.UI.Page
{
    ...
    private void FillSF()
    {
        ...
        foreach (DataRow dr in DsRoute.Tables[0].Rows)
        {
            ...
            if (drow.Length > 0)
            {
                ...
                foreach (DataRow row in drow)
                {
                    ...
                    {
                        dsData.Rows.Add(row["ListedDrCode"].ToString(), row["ListedDr_Name"].ToString(), row["Doc_cat_ShortName"].ToString(), row["Doc_Special_Name"].ToString(), row["phone"].ToString(), row["Address1"].ToString(), dr["Territory_Name"].ToString(), "", jan_val, feb_val, mar_val, apr_val, may_val, jun_val, jul_val, aug_val, sep_val, oct_val, nov_val, dec_val, cur_tot);
                    }
        ...
        int cou = 4;
        //if (Session["div_code"].ToString()=="20")
        //{
        //    cou = 4;
        //}
        //else
