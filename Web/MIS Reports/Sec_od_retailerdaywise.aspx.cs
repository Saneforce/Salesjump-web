...
public partial class MIS_Reports_Sec_od_retailerdaywise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Secondary Order Retailer Daywise Details for the Month of " + strFMonthName + " " + FYear;
    ...
    private void Fillproduct_total()
    {
        ...
        lit_DR_utility.Text = "<center>Daywise View</center>";
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
            {
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    dsDrr = sd.purchase_disributor_view_daywise_details_total(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString());
    ...
    private void FillSF_Leave_Type()
    {
		...
		tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    DateTime ht = Convert.ToDateTime(f);
                    string jj = ht.ToString("yyyy-MM-dd");
                    dsDrr = sd.retail_retailerwise_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor, route_code, retailer_code);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
...
foreach (DataRow drdo in gg.Tables[0].Rows)
{
    ...
    string f = drdo["DATE"].ToString();
    DateTime ht = Convert.ToDateTime(f);
    string jjj = ht.ToString("yyyy-MM-dd");
  foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
            {
              
   dsDrr = sd.retail_retailerwise_view_daywise_details(div_code, FMonth, FYear, jjj, drdoctor["Product_Detail_Code"].ToString(), distributor, route_code, retailer_code);
    if (dsDrr.Tables[0].Rows.Count > 0)
    {
        foreach (DataRow drr1doctor in dsDrr.Tables[0].Rows)
        {
            ...
            {
                iTotLstCount += (decimal.Parse(quantot));

            }
            valtot = drr1doctor["VALUE"].ToString();
            if (valtot != "" && valtot != "0")
            {
                iTotLstCount2 += (decimal.Parse(valtot));

            }
            nettot = drr1doctor["net_value"].ToString();
            if (nettot != "" && nettot != "0")
            {
                iTotLstCount1 += (decimal.Parse(nettot));
