...
public partial class MIS_Reports_rpt_area_daywise : System.Web.UI.Page
{
...
string sf_code = string.Empty;
    ...
    sf_code = Session["sf_code"].ToString();
        ...
        lblHead.Text = "Purchase Register Areawise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                  ...
                  //  dsDrr = sd.areawise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(),areacode );
				
					if (stype == "value")
                    {
                        dsDrr = sd.areawise_purchase_value(areacode: areacode, div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, product_code: drdoctor["Product_Detail_Code"].ToString(), day: jj);
                    }
                    else //if (stype == "total")
                    {
                        dsDrr = sd.areawise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, product_code: drdoctor["Product_Detail_Code"].ToString(), day: jj);
                    }
