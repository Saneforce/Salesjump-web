...
System.Globalization.DateTimeFormatInfo mfi = new System.Globalization.DateTimeFormatInfo();
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Sale Register Territorywise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
...
leave_utility.Style.Add("text-align","center");
        ...
        int dff = Convert.ToInt32(getweek.Tables[0].Rows[0].ItemArray.GetValue(0));

        int weei = dff + 1;

        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < weei; i++)
            {
      ...
      dtype="total";
                sURL = "rpt_saleterritory_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&Territory_Code=" + territorycode + "&Territory_name=" + Territory_name + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < weei; i++)
                    {
                       ...
                       // dsDrr = sd.sale_Territorywise_purchase_productdetail_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString());
   dsDrr = sd.sale_Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear,week: i, product_code: drdoctor["Product_Detail_Code"].ToString(),sf_code:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                       ...
                       // dsDrr = sd.sale_Territorywise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), territorycode);
 dsDrr = sd.sale_Territorywise_purchase_value( div_code: div_code, cmonth: FMonth, cyear: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), territorycode: territorycode ,sf_code:sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
