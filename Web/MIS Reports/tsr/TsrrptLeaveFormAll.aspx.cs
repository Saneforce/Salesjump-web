...
public partial class MIS_Reports_tsr_TsrrptLeaveFormAll : System.Web.UI.Page
{
    ...
    public static TotOrder[] GetLeaveSF(string SF_Code, string FYear, string SubDiv)
    {
        ...
        foreach (DataRow row in dsSalesForce.Tables[0].Rows)
        {
			...
			vl.sfZone = row["Zone_name"].ToString();
			vl.sfEmployeeId = row["Employee_Id"].ToString();
