        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
                ...
                dtype="total";
				  ...
				  sURL = "rpt_purchase_view_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&distributor=" + distributor + "&Subdivision=" + subdivision + "&dist_name="+ Stock_name +"&Dtype="+ dtype +"";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                        ...
                        dsDrr = sd.purchase_disributor_view_weekise_details_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(),sfcode:sf_code);
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                        dsDrr = sd.purchase_disributor_view_weekise_details_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor :distributor);
                        if(dsDrr.Tables[0].Rows.Count>0)
                                {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
