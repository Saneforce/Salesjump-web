<%@ Page Title="" Language="C#" MasterPageFile="~/Master.master" AutoEventWireup="true" CodeFile="Rpt_RetailerWise_Secondary_Qty.aspx.cs" Inherits="MIS_Reports_Rpt_RetailerWise_Secondary_Qty" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="row">
        <div class="col-lg-12 sub-header">
            Retailer Wise Secondary Report<span style="float: right; margin-right: 15px;">
    ...
    <%--<div><input type="button" value="Load Data" id="btn_load" class="btn btn-primary"/></div>--%>
    <div class="card">
        <div class="card-body table-responsive" style="overflow: auto; max-height: 500px; padding: 0px !important">
            <table class="table table-bordered" id="tblRet_wise">
    ...
    <script type="text/javascript">        
        var AllOrders = []; var Orders = []; var FDt = '', str = '';
        var TDt = ''; var Allstate = [], Allclustr = [], Allsf = [], AllHyrListSf = [], AllBrndName = [];
        var Brnd = [], items = [], Allsf_Suvdiv = [], AllRetailers = [], rpt_sf = [], GetBrndName = [];
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                async: false,
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/GetBrandName",
                dataType: "json",
            ...
            $('#ddlstate').on('change', function () {
                ...
                if (statecode != '' || statecode != 'Select State') {
                    $.ajax({
                        ...
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "Rpt_RetailerWise_Secondary_Qty.aspx/Getcluster",
                        ...
                        success: function (data) {
                            ...
                            if (Allclustr.length > 0) {
                                ...
                                st.append('<option selected="selected" >No Cluster Found by State</option>');
            ...
            $('#ddlclstr').on('change', function () {
                ...
                if (terr_code == '' || terr_code == 'Select cluster') {
                    ...
                    $.ajax({
                        ...
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "Rpt_RetailerWise_Secondary_Qty.aspx/getsubDiv",
        ...
        function GetMGR() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/GetMGR",
                dataType: "json",
                success: function (data) {
        ...
        function Get_allbrnd() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/GetAllBrandName",
                dataType: "json",
                success: function (data) {
        ...
        function fillstate() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/getStates",
        ...
        function GetDiv() {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/fillsubdivision",
                datatype: "json",
                success: function (data) {
        ...
        function AllHyrList() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/Gethyr",
                dataType: "json",
                success: function (data) {
        ...
        //            url: "Rpt_RetailerWise_Secondary_Qty.aspx/GetRetailers_det",
        //            dataType: "json",
        //            success: function (data) {
        //                AllRetailers = JSON.parse(data.d) || [];
        //                if (AllRetailers.length > 0) {
        //                    ReloadTable(AllHyrListSf);
        //                }
        //                else {
        //                    $('#loadover').hide();
        //                    alert("All Datas are already loaded");
        //                    return false;
        //                }                        
        //            },
        //            error: function (Res) {
        ...
        function AppendHeader(BrndHeader) {
            ...
            //    return a.BrndNo == 1;
            //});
            styl = "style='text-align: left; color: #fff'";
            str = "<tr style='background-color: #37a4c6; color: #fff; white-space: nowrap;'><th " + styl + ">Field Force</th>\
                            <th "+ styl + ">Retailer</th>";
        ...
        function loadData() {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_RetailerWise_Secondary_Qty.aspx/GetRetailerWise_Qty",
        ...
        function ReloadTable(AllHyrSf, BrndHeader) {
            ...
            slno = 0; str = ''; var Chkstr = '', Chk = '', Val = 0;
            if (AllHyrSf.length > 0) {
                ...
                for (var i = 0; i < AllHyrSf.length; i++) {
                    if (AllHyrSf[i].SFNA != Chk) {
                        ...
                        var ret = Orders.filter(function (b) {
                            return b.SF_Code == AllHyrSf[i].Sf_Code;
                        });
                        var arr = AllHyrSf.filter(function (itm) {
                        ...
                        if (AllHyrSf.length > 1 && arr.length != 0) {
                            ...
                            BrndHeader.sort((a, b) => b.ListedDrCode - a.ListedDrCode);
                            BrndChkstr = '';
                            for (var l = 0; l < BrndHeader.length; l++) {
                                if (BrndHeader[l].BrandName != BrndChkstr) {
                                    BrndChkstr = BrndHeader[l].BrandName;
                                    Val = 0;
                                    //Val = getValue(AllHyrSf[i].Sf_Code);//, AllBrndName[l].BrandName);
                                    str += ("<td>" + Val + "</td>");
                                }
                            }
                            str += "</tr>";
                        ...
                        if (ret.length > 0) {
                            ret.sort((a, b) => b.ListedDrCode - a.ListedDrCode);//ret.sort();
                            for (var j = 0; j < ret.length; j++) {
                                if (ret[j].ListedDr_Name != Chkstr) {
                                    Chkstr = ret[j].ListedDr_Name;
                                    str += ((arr.length != 0) ? "<td></td>" : "");
                                    str += "" + ((j == 0) ? "" : "<tr><td></td>") + "<td>" + ret[j].ListedDr_Name + "</td>";
                                    BrndHeader.sort((a, b) => b.BrandName - a.BrandName);
                                    BrndChkstr = '';
                                    for (var k = 0; k < BrndHeader.length; k++) {
                                        if (BrndHeader[k].BrandName != BrndChkstr) {
                                            BrndChkstr = BrndHeader[k].BrandName;
                                            BrVal = 0;
                                            var arr = Orders.filter(function (a) {
                                                return (a.SF_Code == AllHyrSf[i].Sf_Code && a.ListedDrCode == ret[j].ListedDrCode && a.Product_Brd_Name == BrndHeader[k].BrandName);
                                            });
                                            str += ((arr.length > 0) ? "<td>" + arr[0].Quantity + "</td>" : "<td>" + 0 + "</td>");
                                        }
                                    }
                                    //str += "</tr>";
                                    //ret.splice(j, 1);
                        ...
                        if (ret.length == 0) {
                            Chkstr = '';
                            if (arr.length == 0) {
                                str += "<td>No Retailer Found</td>";
                                BrndHeader.sort((a, b) => b.BrandName - a.BrandName);
                                for (var k = 0; k < BrndHeader.length; k++) {
                                    if (BrndHeader[k].BrandName != Chkstr) {
                                        Chkstr = BrndHeader[k].BrandName;
                                        str += "<td>" + 0 + "</td>";
                                    }
                                }
                            }
                ...
                str = "<tr><td colspan='4' style='font-size: 25px;color: grey;'>No Data Found !</td></tr>";
            }
        ...
        function getValue(SF, BrndNm) {
            ...
            for (var i = 0; i < arr.length; i++) {
                Brnd = Orders.filter(function (a) {
                    return (a.SF_Code == arr[i].Sf_Code);// && a.Product_Brd_Name == BrndNm);
                });
                if (Brnd.length > 0) {
