<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Master.master" CodeFile="mgrscore_card.aspx.cs" Inherits="MIS_Reports_mgrscore_card" %>
    ...
    <head>
        ...
        <script type="text/javascript">
            function NewWindow() {
                ...
                var ddlFF = $('#<%=ddlmgr.ClientID%> :selected').text();
                if (ddlFF == "---Select---") { alert("Select FieldForce."); $('#<%=ddlmgr.ClientID%>').focus(); return false; }

                var subdiv = $('#<%=subdiv.ClientID%> :selected').val();
                if (subdiv == "--- Select ---") { alert("Select Subdivision."); $('#<%=subdiv.ClientID%>').focus(); return false; }


                var ddlFF = $('#<%=ddlmgr.ClientID%> :selected').val();
                var ddlFF_Name = $('#<%=ddlmgr.ClientID%> :selected').text();
                var FMonth = $('#<%=ddlFMonth.ClientID%> :selected').val();
                var FYear = $('#<%=ddlFYear.ClientID%> :selected').text();

                var imgpth = $('#ctl00_ContentPlaceHolder1_lblpath').text();

                window.open("Rpt_mgrscore_card.aspx?&Month=" + FMonth + "&Year=" + FYear + "&subdiv=" + subdiv + "&sf_code=" + ddlFF + "&Sf_Name=" + ddlFF_Name, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
