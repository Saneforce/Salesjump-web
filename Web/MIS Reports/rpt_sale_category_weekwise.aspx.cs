...
public partial class MIS_Reports_rpt_sale_category_weekwise : System.Web.UI.Page
{
...
leave_utility.Style.Add("text-align","center");
      ...
      dtype="total";
                sURL = "rpt_salecategory_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&dist_code=" + distributor + "&Product_Cat_Code=" + cat_code + "&subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                   ...
                   //     dsDrr = sd.sale_categoryview_weekise_details_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor);
 dsDrr = sd.sale_catogory_wise_sale_value_stockist( divcode: div_code, cmonth: FMonth,cyear: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(),stockistcode: distributor,sf_code:sf_code);
...
leave_utility.Style.Add("text-align","center");
      ...
      dtype="value";
                sURL = "rpt_salecategory_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&dist_code=" + distributor + "&stockist_name=" + Stock_name + "&Product_Cat_Code=" + cat_code + "&subdivision=" + subdivision + "&Dtype="+ dtype +"";
                lit_catg_namee.Attributes["onclick"] = "javascript:window.open('" + sURL + "' ,'','resizable=yes,toolbar=no,menubar=no,scrollbars=1,status=no,width=500,height=600,left=0,top=0,zoom=50%');";
                lit_catg_namee.Style.Add("Color", "Blue");
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                 //       dsDrr = sd.sale_categoryview_weekise_details(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
        dsDrr = sd.sale_catogory_wise_sale_value_stockist(divcode: div_code, cmonth: FMonth, cyear: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), productcatname:cat_code, stockistcode: distributor, sf_code: sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
