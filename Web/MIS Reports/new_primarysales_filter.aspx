<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/all_rpts.master" CodeFile="new_primarysales_filter.aspx.cs" Inherits="MIS_Reports_new_primarysales_filter" %>
     ...
     <div class="row">
        ...
        <div class="row">
            <div class="col-lg-12 sub-header"  style="margin-top: 4px";>
                Primary Sales Order <div style="float: right; padding-top: 3px;">
        ...
        $(document).ready(function () {
            ...
            function fillcom() {
                ...
                $.ajax({
                    ...
                    async: false,
                    url: "new_primarysales_filter.aspx/Binddivname",
            ...
            function countdtl() {
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "new_primarysales_filter.aspx/totalcnt",
                    data: "{'divcode':'<%=Session["div_code"]%>','fdate':'" + fdt + "','tdate':'" + tdt + "','invdtv':'" + invdt + "','ordtv':'" + ordt + "','orbyv':'" + orby + "','cusvv':'" + cusv + "','selinvdtv':'" + selinvdt + "','selordtv':'" + selordt + "','selorbvalv':'" + selorbval + "','selcusvv':'" + selcusv + "','arrayval':'" + arrayp + "'}",
                    dataType: "json",
                    success: function (data) {
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "new_primarysales_filter.aspx/Bindfillqty_view",
                    data: "{'divcode':'<%=Session["div_code"]%>','pageindex':'" + plimt + "','plimt':'" + plim + "','fdate':'" + fdt + "','tdate':'" + tdt + "','invdtv':'" + invdt + "','ordtv':'" + ordt + "','orbyv':'" + orby + "','cusvv':'" + cusv + "','selinvdtv':'" + selinvdt + "','selordtv':'" + selordt + "','selorbvalv':'" + selorbval + "','selcusvv':'" + selcusv + "','arrayval':'" + arrayp + "'}",
                    dataType: "json",
                    success: function (data) {
            ...
            function selectfields() {
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "new_primarysales_filter.aspx/get_selectfields",
                    data: "{'divcode':'<%=Session["div_code"]%>'}",
                    dataType: "json",
                    success: function (data) {
            ...
            function ReloadTable() {
                ...
                if (tempData.length > 0) {
                    ...
                    sth += "<th  class='rw' rowspan='2'>Sub Total</th><th  class='rw' rowspan='2'>Tax Total</th><th  class='rw' rowspan='2'>Gross Total</th>";
                }
                else {
                    sth += "<tr><th class='rw' rowspan='2'>S.NO</th><th  class='rw' rowspan='2'>GRN No</th><th  class='rw' rowspan='2'>GRN Date</th><th  class='rw' rowspan='2'>Po_No</th><th  class='rw' rowspan='2'>Supp_Name</th><th  class='rw' rowspan='2'>Sub Total</th><th  class='rw' rowspan='2'>Tax Total</th><th  class='rw' rowspan='2'>Gross Total</th>";
                }
                ...
                for (i = 0; i <= st + PgRecords; i++) {
                    if (i < proddtl.length) {
                        ...
                        if (tempData.length > 0) {
                            ...
                            str += "<td>" + proddtl[i].Net_Tot_Goods + "</td><td>" + proddtl[i].Net_Tot_Tax + "</td><td>" + proddtl[i].Net_Tot_Value + "</td>";
                        }
                        else {
                            str += "<tr style='text-align: left;'><td>" + slno + "</td><td>" + proddtl[i].GRN_No + "</td><td>" + proddtl[i].GRN_Date + "</td><td>" + proddtl[i].Po_No + "</td><td>" + proddtl[i].Supp_Name + "</td><td>" + proddtl[i].Net_Tot_Goods + "</td><td>" + proddtl[i].Net_Tot_Tax + "</td><td>" + proddtl[i].Net_Tot_Value + "</td>";
                        }
            ...
            $('#setif').on('click', function () {
                $('#sortable1').html('');
                var fld = '';
                fld += "<li class='ui-state-default'><input class='tgl tgl-skewed' type='checkbox' id='GRN_No' value='GRN_No' name='GRN_No'/> " +
                    "<label class= 'tgl' for='GRN_No' style='height: 1px; '>GRN No</label></li>";
                fld += "<li class='ui-state-default'><input class='tgl tgl-skewed' type='checkbox' id='GRN_Date' value='GRN_Date' name='GRN_Date'/> " +
                    "<label class= 'tgl' for='GRN_Date' style='height: 1px; '>GRN Date</label></li>";
                fld += "<li class='ui-state-default'><input class='tgl tgl-skewed' type='checkbox' id='Po_No'  value='Po_No' name='Po_No'/> " +
                    "<label class= 'tgl' for='Po_No' style='height: 1px; '>Po_No</label></li>";
                fld += "<li class='ui-state-default'><input class='tgl tgl-skewed' type='checkbox' id='Supp_Name' value='Supp_Name' name='Supp_Name'/> " +
                    "<label class= 'tgl' for='Supp_Name' style='height: 1px; '>Supp_Name</label></li>";
            ...
            $("#bcir").click(function () {
                if (iCnt <= 3) {
                    ...
                    str += `<label style='white-space: nowrap; margin-left: 57px;'></label><select id="txtfilter${iCnt}" name='ddfilter' style='width: 207px; margin-top: 11px;'> +`
                        + `<option value='0'>Select Fields</option><option value='GRN_No'>GRN No</option><option value='GRN_Date'>GRN Date</option> +`
                        + `<option value='Po_No'>Po_No</option><option value='Supp_Code'>Supp_Name</option>+`
                        + `</select>`;
                    str += `</div></br></br>`;
                    $('#d1').append(str);
                    ...
                    $('#txtfilter1').on('change', function () {
                        ...
                        var sf = $("#mselect1");
                        sf.empty();
                        if ($('#txtfilter1').val() == 'GRN_No') {
                            orderdateval();
                            arrayod = HQ.filter(function (el) {
                                const val = mymap.get(el.GRN_No);
                                if (val) {
                                    return false;
                                }
                                mymap.set(el.GRN_No, el.GRN_No);
                                return true;
                            }).map(function (a) { return a; }).sort();
                            for (var j = 0; j < arrayod.length; j++) {
                                sf.append($('<option value="' + arrayod[j].GRN_No + '">' + arrayod[j].GRN_No + '</option>'));
                            }

                        }
                        if ($('#txtfilter1').val() == 'GRN_Date') {
                            orderdateval();
                            arrayst = HQ.filter(function (el) {
                                const val = mymap.get(el.GRN_Date);
                                if (val) {
                                    return false;
                                }
                                mymap.set(el.GRN_Date, el.GRN_Date);
                                return true;
                            }).map(function (a) { return a; }).sort();
                            for (var i = 0; i < arrayst.length; i++) {
                                sf.append($('<option value="' + arrayst[i].GRN_Date + '">' + arrayst[i].GRN_Date + '</option>'));
                            }

                        }
                        if ($('#txtfilter1').val() == 'Po_No') {
                            orderdateval();
                            arrayob = HQ.filter(function (el) {
                                const val = mymap.get(el.Po_No);
                                if (val) {
                                    return false;
                                }
                                mymap.set(el.Po_No, el.Po_No);
                                return true;
                            }).map(function (a) { return a; }).sort();
                            for (var i = 0; i < arrayob.length; i++) {
                                sf.append($('<option value="' + arrayob[i].Po_No + '">' + arrayob[i].Po_No + '</option>'));
                            }

                        }
                        if ($('#txtfilter1').val() == 'Supp_Code') {
                            orderdateval();
                            arrayrt = HQ.filter(function (el) {
                                const val = mymap.get(el.Supp_Code);
                                if (val) {
                                    return false;
                                }
                                mymap.set(el.Supp_Code, el.Supp_Name);
                                return true;
                            }).map(function (a) { return a; }).sort();
                            for (var i = 0; i < arrayrt.length; i++) {
                                sf.append($('<option value="' + arrayrt[i].Supp_Code + '">' + arrayrt[i].Supp_Name + '</option>'));
                            }
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "new_primarysales_filter.aspx/Getorderdateval",
                    data: "{'divcode':'<%=Session["div_code"]%>','fdate':'" + fdt + "','tdate':'" + tdt + "','invdtv':'" + invdt + "','ordtv':'" + ordt + "','orbyv':'" + orby + "','cusvv':'" + cusv + "','selinvdtv':'" + selinvdt + "','selordtv':'" + selordt + "','selorbvalv':'" + selorbval + "','selcusvv':'" + selcusv + "'}",
                    dataType: "json",
                    success: function (data) {
            ...
            function saveTemplateList(templatelist) {
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "new_primarysales_filter.aspx/savetemplate",
