...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
		...
		CalcAmt(itr);
                ...
                var unitvalpeice = $(this).attr('rateinpeice');
                ...
                NewOrd[idx].Rate_in_peice = parseFloat(unitvalpeice).toFixed(2);
            ...
            $(document).on("change", "#To_Address", function () {
                if ($('#To_Address').val() != '0') {
                    if (Allrate.length > 0) {
                        ...
                        $('#To_Address').val($('#To_Address option:first').val()).trigger('chosen:updated');
                        return false;
                    }
                }
            });
            ...
            function AddRow(type) {
                ...
                itm = {}
                itm.PCd = ''; itm.sPCd = ''; itm.s_pcode = ''; itm.PName = ''; itm.Unit = ''; itm.Rate = "0"; itm.Rate_in_peice = "0"; itm.Qty = 1; itm.Qty_c = 1; itm.Free = "0"; itm.Discount = "0"; itm.Dis_value = "0"; itm.Total_Tax = 0; itm.produnit = 0; itm.eQty = 0;
			...
			$(itr).find('.second_row_div').text('');
				...
				//$('#yourTableId').off('change', '.ddlProd'); 
				//$('#yourTableId').off('change', '.ddlProd'); 
			   ...
			   $("#OrderEntry").on("change", ".ddlProd", function () {
				  ...
				  event.stopPropagation(); 
					...
					if(Prod.length==0){
					$(itr).find('.txtQty').val('');
					//$('#Td1').append('<span>not</span>');
					//$.confirm({
                       //     title: 'Error!',
                         //   content: 'Rate Not Available For This Product',
                          //  type: 'red',
                           // typeAnimated: true,
                           //icon: 'fa fa-warning ',
                           // buttons: {
...
//
                        ...
                        if (Prod.length > 0) {
                            ...
                            for (var z = 0; z < filter_unit.length; z++) {
                                if (filter_unit[z].Move_MailFolder_Id == Prod[0].Base_Unit_code) {
                                    unitrate = Prod[0].PTS;
                                    unitpeice = Prod[0].PTS;
                                    unitconfac = filter_unit[z].Sample_Erp_Code;
                                    units += "<li id=" + unitrate + " conf=" + unitconfac + "  rateinpeice=" + unitpeice +" name='itms' value='" + filter_unit[z].Move_MailFolder_Id + "'>" + filter_unit[z].Move_MailFolder_Name + "</li>";
                                    units1 += "<option  id=" + unitrate + " conf=" + unitconfac + " rateinpeice=" + unitpeice +" value='" + filter_unit[z].Move_MailFolder_Id + "'>" + filter_unit[z].Move_MailFolder_Name + "</option>";
                                }
                                ...
                                //}
                                    ...
                                    unitpeice = Prod[0].PTS;
                                    units += "<li id=" + unitrate + " name='itms' rateinpeice=" + unitpeice +" conf=" + unitconfac + " value='" + filter_unit[z].Move_MailFolder_Id + "'>" + filter_unit[z].Move_MailFolder_Name + "</li>";
                                    units1 += "<option  id=" + unitrate + " conf=" + unitconfac + "  rateinpeice=" + unitpeice +" value='" + filter_unit[z].Move_MailFolder_Id + "'>" + filter_unit[z].Move_MailFolder_Name + "</option>";
		...
		$.confirm({
                            title: 'Error!',
                            content: 'Please select any product to delete',
                            type: 'red',
                            typeAnimated: true,
                            autoClose: 'action|8000',
                            icon: 'fa fa-warning ',
...
return false;
            ...
            $(document).on('keyup', '.txtQty', function (e) {
               ...
               // getscheme(pCode, CQ, un, tr, ff, '', opcode);
                ...
                NewOrd[idx].of_Pro_Code = $(tr).find('.fre').attr('freepro');
            ...
            svOrder = function () {
 			 ...
 			 approve = 0;
                        $.confirm({
                            title: 'Error!',
                            content: 'Please Select Company Name',
                            type: 'red',
                            typeAnimated: true,
                            autoClose: 'action|8000',
                            icon: 'fa fa-warning ',
