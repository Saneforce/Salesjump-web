...
public partial class Stockist_Counter_Sales : System.Web.UI.Page
{
   ...
   [WebMethod(EnableSession = true)]
    public static string getrate_new(string Div_Code, string Stockist_Code)
    {
        string State = HttpContext.Current.Session["State"].ToString();
        DataSet ds = new DataSet();
        ds = get_ratebystk(Div_Code.TrimEnd(','), Stockist_Code, State);
        return JsonConvert.SerializeObject(ds.Tables[0]);
    }
   
    public static DataSet get_ratebystk(string Div_Code, string Stockist_Code, string State)
    {
        DataSet ds = null;
        DB_EReporting db_ER = new DB_EReporting();

        string strQry = "Exec get_con_sal_rate_new '" + Div_Code + "','" + Stockist_Code + "','" + State + "'";
