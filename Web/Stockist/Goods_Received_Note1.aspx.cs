...
public partial class MasterFiles_Goods_Received_Note : System.Web.UI.Page
{
                          ...
                          // string strmfgdt = mfgdt.ToString("MM-dd-yyyy");
                           ...
                           string strmfgdt = dtgrn.ToString("MM-dd-yyy");
                            ...
                            int Dtls_No = Insert_GoodsReceived_Details(Trans_No.ToString(), GRN_details[i].PCode.ToString(), GRN_details[i].PDetails.ToString(), 
                                GRN_details[i].UOM.ToString(), GRN_details[i].Batch_No.ToString(), GRN_details[i].POQTY.ToString(), GRN_details[i].Price.ToString(),GRN_details[i].Rate_in_peice.ToString(),
    ...
    public static int Insert_GoodsReceived_Details(string Trans_No, string Pro_Code, string Pro_Name, string uom, string batchno, string qty, string price, string Rate_in_peice, string good, string damage, string groosval, string netval, string uom_name, string mfgDate, string remarks, string Offer_pro_code, string Offer_pro_name, string offer_pro_unit, string con_factor, string free, string dis, string dis_val, string Tax)
    {
        int iReturn = -1;
        int iSlNo = -1;
        try
        {
            ...
            strQry = " INSERT INTO Trans_GRN_Details (Trans_Dtls_Sl_No,Trans_Sl_No,PCode,PDetails,UOM,Batch_No,POQTY,Price,Good,Damaged,Gross_Value,Net_Value,uom_name,mfgdate,remark,Offer_pro_code,Offer_pro_name,Cnv_qty,Off_pro_unit_name,Off_pro_unit_code,Tax,free,dis,dis_val) " +
                     " VALUES ('" + iSlNo + "','" + Trans_No + "','" + Pro_Code + "','" + Pro_Name + "','" + uom + "','" + batchno + "','" + qty + "','" + Rate_in_peice + "','" + good + "','" + damage + "','" + groosval + "','" + netval + "','" + uom_name + "','" + mfgDate + "','" + remarks + "','" + Offer_pro_code + "','" + Offer_pro_name + "','" + con_factor + "','" + offer_pro_unit + "','0','" + Tax + "','" + free + "','" + dis + "','" + dis_val + "')";
    ...
    public static string SavePurchaseOrder(string HeadData, string DetailsData, string TaxData , string Pono_No)
    {
        ...
        using (var scope = new TransactionScope())
        {
            try
            {
                ...
                {
                    Details += "<Prod PCode=\"" + Product_details[i].PCd + "\" PName=\"" + Product_details[i].PName + "\" Qty=\"" + Product_details[i].Qty_c + "\"  Rate=\"" + Product_details[i].Rate + "\"    Rate_in_peice=\"" + Product_details[i].Rate_in_peice + "\" Free=\"" + Product_details[i].free + "\"  Dis=\"" + Product_details[i].dis + "\"  Dis_value=\"" + Product_details[i].dis_value + "\" Off_Pro_Code=\"" + Product_details[i].Off_Pro_code + "\" Off_Pro_Name=\"" + Product_details[i].Off_Pro_name + "\" Off_Pro_Unit=\"" + Product_details[i].Off_Pro_Unit + "\" Productunitcode=\"" + Product_details[i].umo_unit + "\" tax=\"" + Product_details[i].Tax_value + "\" con_fac=\"" + Product_details[i].con_fac + "\" />";
                }
                Details += "</ROOT>";
