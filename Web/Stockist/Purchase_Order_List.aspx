<%@ Page Title="" Language="C#" MasterPageFile="~/Master_DIS.master" AutoEventWireup="true" CodeFile="Purchase_Order_List.aspx.cs" Inherits="Stockist_Purchase_Order_List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <form id="frm1" runat="server">
        ...
        <div class="row">
            ...
            <div class="col-lg-12 sub-header">
                Purchase Request<span style="float: right; margin-right: 53px;"><a href="../Stockist/Purchase_Order3.aspx" class="btn btn-primary btn" id="newsf">Add New</a></span><span style="float: right; margin-right: 15px;"><div id="reportrange" class="form-control mt-5 mb-5" style="background: #fff; cursor: pointer; padding: 5px 10px; border: 1px solid #ccc;">
    ...
    <script language="javascript" type="text/javascript">
                ...
                $.confirm({
                    ...
                    buttons: {
                        tryAgain: {
                            ...
                            btnClass: 'btn-red',
                            action: function () {
                                var inputHtml = '<input type="text" id="reason" placeholder="Enter cancellation reason" autocomplete="off" class="form-control">';
                                $.confirm({
                                    ...
                                    buttons: {
                                        confirm: {
                                            ...
                                            btnClass: 'btn-red',
                                            action: function () {
                                                var cancellationReason = $('#reason').val();
                                                if (cancellationReason.trim() === '') {
                                                    $.alert('Please enter a cancellation reason.');
                                                    return false; 
                                                } else {
                                                    cancelorder(orderid, stockist, div, cancellationReason);
                                                }
                                            }
                                        },
                                        cancel: function () {
        ...
        function cancelorder(orderid, stockist, div, cancellationReason) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Purchase_Order_List.aspx/cancelorder",
                data: "{'orderid':'" + orderid + "','stockist':'" + stockist + "','div':'" + div + "','cancelreason':'"+cancellationReason+"'}",
                dataType: "json",
        ...
        function Edit_Screen(id, stk, div, flag, cust, order_date) {
            ...
            if (flag == 1) {
                ...
                var url = "../Stockist/Purchase_Order_Edit.aspx?Order_No=" + id + "&Stockist_Code=" + stk + "&Div_Code=" + div + "&Cust_Code=" + cust + "&Order_Date=" + order_date + ""
            ...
            for ($i = st; $i < st + PgRecords; $i++) {
                ...
                if ($i < Orders.length) {
                    ...
                    $(tr).html("<td><input type='hidden' class='Flag' value=" + Orders[$i].Order_Flag + "><input " + ((Orders[$i].Chk == '1') ? 'checked' : '') + " type='checkbox' class='rowCheckbox' id='SelectedCheckbox'/></td><td>" + slno + "</td><td>" + Orders[$i].Order_No + "</td><td>" + Orders[$i].CompanyName + "</td><td>" + Orders[$i].Order_Date_Disp +
                        ...
                        "<td style='text-align:right; cursor: pointer;'><a href='#' class ='" + ((Orders[$i].Status == 'GRN Updated' || Orders[$i].Status =='Cancelled' || Orders[$i].Status =='Despatched'||Orders[$i].Status =='Waiting For Invoice' ||Orders[$i].Status =='Waiting For Invoice') ? 'edit_class' : 'edit_class1') + "' id='Btn_Edit' onclick='Edit_Screen(\"" + Orders[$i].Order_No + "\",\"" + stockist_Code + "\",\"" + Div_Code + "\",\"" + Orders[$i].Order_Flag + "\" ,\"" + Orders[$i].Cust_Code + "\" ,\"" + Orders[$i].Order_Date + "\" )'</a>Edit</td>" +
                        "<td class='cancelOrder' style='text-align:right'><a href='#'><span class='glyphicon glyphicon-remove' onClick='cancel(\"" + Orders[$i].Order_No + "\",\"" + stockist_Code + "\",\"" + Div_Code + "\",\"" + Orders[$i].Status + "\")'></span></a></td>" +
                    ...
                    if (Orders[$i].Status == 'GRN Updated' || Orders[$i].Status =='Cancelled'||Orders[$i].Status =='Despatched'||Orders[$i].Status =='Waiting For Invoice') { $('#OrderList tbody tr').find('.rowCheckbox').attr("disabled", true); }

                    if (Orders[$i].Status == 'Pending') {
                        $("td:contains('Pending')").addClass('red');
                    }
                    else if (Orders[$i].Status == 'Invoiced') {
                        $("td:contains('Invoiced')").addClass('orange');
                    }
					 else if (Orders[$i].Status == 'GRN Updated') {
                        $("td:contains('GRN Updated')").addClass('green');
                    }
					else if (Orders[$i].Status == 'Waiting For Invoice') {
                        $("td:contains('Waiting For Invoice')").addClass('yellow');
                    }
                    hq = [];
                }
            }
        ...
        function popup(orderid, stk, div) {
            ...
            var url = "../Stockist/Puchase_Order_View.aspx?Order_No=" + orderid + "&Stockist_Code=" + stk + "&Div_Code=" + div + ""
            window.open(url, '_self');
        }
        ...
        function loadData() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Purchase_Order_List.aspx/GetOrders",
