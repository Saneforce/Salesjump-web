...
function svDmsSecondaryOrder(){
	...
	$res["Trans_Detail_Slno"]=$sql;
	$dcrcode=$dtranssl[0]["Trans_Detail_Slno"];
	if (count($prod)>0){
		$PPXML = "<ROOT>";
		for ($j = 0; $j < count($prod); $j++) {
			$pqty=(intval($prod[$j]["Qty"])*intval($prod[$j]["Con_Fac"]));
			$PPXML = $PPXML . "<Prod PCode=\"".$prod[$j]["ProductCode"]."\" PName=\"".$prod[$j]["Productname"]."\" Qty=\"".$pqty."\" Val=\"".$prod[$j]["Val"]."\" Rate=\"".$prod[$j]["ProductRate"]."\" FQty=\"0\" Dval=\"".$prod[$j]["dis_value"]."\" DAmt=\"".$prod[$j]["dis"]."\" Md=\"free\" Mfg=\"\" Cl=\"\" Offer_ProductCd=\"\" Offer_ProductNm=\"\" of_Pro_Unit=\"\" Unit=\"".$prod[$j]["Productunit"]."\" tax=\"".$prod[$j]["tax_value"]."\" con_fac=\"".$prod[$j]["Con_Fac"]."\" Umo_Code=\"".$prod[$j]["UOM"]."\" Sl_No=\"".$j."\"  Tax_value=\"".$prod[$j]["tax_value"]."\" Qty_C=\"".$prod[$j]["Qty"]."\"  />";
			$SPProds = $SPProds . $prod[$j]["ProductCode"] . "~" . $pqty . "#".$prod[$j]["Val"].";";
			$nSPProds = $nSPProds . $prod[$j]["Productname"] . "~" . $pqty . "#".$prod[$j]["Val"].";";
		}
		$PPXML = $PPXML . "</ROOT>";
	}
	$sql="exec svSecOrder '".$StkCode."','".$Retcode."','".$StkCode."','".$RtCode."',0,'".$OrderDate."',".$OrderVal.",0,'0','',0,0,'','".$dcrcode."',".$Owndiv.",'".$PPXML."','4','','".$Subtot."','".$Distot."','".$Taxtot."'";
	$res["success"]=performQuery($sql);
...
if($vals['Retailerphoto']!=null && $vals['Retailerphoto']!="undefined")
         ...
         if($flagofretailerND==1){
          $ListedDr_Active_Flag=3;
            ...
            '" .str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','','" . $visitpath . "',".$Lat.",".$Long.",'" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_phone3']) . "')}";
           performQueryWP($sql,[]);
            ...
            // if (isset($tableData['Activity_POB_Report'])) {
                    ...
                    if (isset($tableData['Activity_Stk_POB_Report'])){
                        ...
                        for ($j = 0; $j < count($samp); $j++) {
							...
							$PPXML = $PPXML . "<Prod PCode=\"".$samp[$j]["ProductCode"]."\" PName=\"".$samp[$j]["Productname"]."\" Qty=\"".$samp[$j]["Qty"]."\" Rate=\"".$samp[$j]["ProductRate"]."\" Free=\"0\" Dis_value=\"".$samp[$j]["dis_value"]."\" Dis=\"".$samp[$j]["dis"]."\" Off_Pro_Code=\"\" Off_Pro_Name=\"\" Off_Pro_Unit=\"\" Productunitcode=\"".$samp[$j]["Productunit"]."\" tax=\"".$samp[$j]["tax_value"]."\" con_fac=\"".$samp[$j]["Con_Fac"]."\" PQty=\"".$samp[$j]["PQty"]."\" CL=\"".$cl_qty_stock."\"   />";

                            $SPProds = $SPProds . $samp[$j]["product_code"] . "~" . $samp[$j]["Qty"] . "#".$samp[$j]["ProductRate"].";";
                            $nSPProds = $nSPProds . $samp[$j]["product_Name"] . "~" . $samp[$j]["Qty"] . "#".$samp[$j]["ProductRate"].";";
                        }
                        $PPXML = $PPXML . "</ROOT>";
                    }
