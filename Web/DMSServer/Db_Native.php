 ...
 $sql = "insert into Trans_TP_Hist(SF_Code,Tour_Month,Tour_Year,Submission_date,Tour_Date,WorkType_Code_B,Worktype_Name_B,Tour_Schedule1,Objective,Worked_With_SF_Code,Division_Code,Territory_Code1,Worked_With_SF_Name,TP_Sf_Name,HQ_Code,HQ_Name,Confirmed,Change_Status,JointWork_Name,Retailer_Code,Retailer_Name) select '" . $sfCode . "',MONTH($tourDate),YEAR($tourDate),getdate(),$tourDate,$worktype_code,$worktype_name,$RouteCode,$objective,$worked_with_code," . $divisionCode[0] . ",$RouteName,$worked_with_name,$sfName,$HQ_Code,$HQ_Name,0,0,$ww,'".$RCode."','".$RName."'";
		...
		case "ondutyApproval":
		 ...
		 case "MissedApproval":
            ...
            $sql = "update TP_Attendance_App set End_Time=cast(Missed_punch_date+' '+Checkout_Tme as datetime)
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
 ...
 $query = "update Mas_SF_SlNo set POrdSl=(isnull(POrdSl,0)+1) where SF_Code='" . $sfCode . "'";
	...
	case "vwExtended":
        ...
        $AMod = $_GET['AMod'];
		if($AMod==1){
        $sql = "select Sf_Code,Sf_Name SFNm,Shift_Name,convert(varchar(10), Approved_Date, 103) Rejectdate,Missed_punch_date,Missed_punch_Flag,convert(varchar(10), Entry_Date, 120) Submission_date ,Reason,Checkin_Time,Checkout_Tme,
        ...
        $sql = "select Sf_Code,Shift_Name,convert(varchar(10), Approved_Date, 103) Rejectdate,Missed_punch_date,Missed_punch_Flag,convert(varchar(10), Entry_Date, 120) Submission_date ,Reason,Checkin_Time,Checkout_Tme,
		case when Missed_punch_Flag='0' then 'Pending'  when Missed_punch_Flag='2' then 'Approved' else 'Reject' end MPStatus,
		case when Missed_punch_Flag='0' then 'rgb(255,165,0) !important'  when Missed_punch_Flag='2' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,RejectReason 
		...
		$sql="select convert(varchar, Start_Time, 23) name,
			...
			when Missed_punch_Flag=2 then ' - Approved' when Missed_punch_Flag=3 then ' - Rejected' end name1,End_Time 
		 from (
		 select login_date,Sft_Name,max(Missed_punch_Flag) Missed_punch_Flag,max(WrkType) WrkTyp,
		 Min(Start_Time) Start_Time,Max(End_Time) End_Time from TP_Attendance_App T
	   ...
	   $getWF = performQuery($sql);
		...
		$sql="select convert(varchar, Start_Time, 23) name,
			...
			convert(varchar, Start_Time, 24) CInTm,End_Time 
		 from (
		 select login_date,Sft_Name,max(Missed_punch_Flag) Missed_punch_Flag,max(WrkType) WrkTyp,
		 Min(Start_Time) Start_Time,Max(End_Time) End_Time from TP_Attendance_App T
		 left outer join Mas_Shift_Timings on ShiftSelected_Id=Sft_ID
...
case "get/ClearData":
	...
	case "get/Shift_timing":
		...
		if ($sfCode!=null){
			$query = "exec getShiftTimesByS_Native '" . $sfCode . "'";
		}
		outputJSON(performQuery($query));
		break;	
case "Save/DivInf":
