...
//$query = "insert into reqDetails select 'DBV13','".$_GET["axn"]."','".$_POST['data']."'"; 
// performQuery($query);
...
function actionLogin() {
    ...
    if ($count == 1) {
        ...
        $query = "select * from dcrmain_trans WITH (NOLOCK) where sf_code='" . $arr[0]['SF_Code'] . "' and Activity_Date=cast(GETDATE() as date)";
...
function getAttndStatus(){
	$RSF = $_GET['sfCode'];
	$Status = $_GET['Status'];
    $query = "exec [GetAttnDetsByStatus] '" . $RSF . "','".$Status."'";
    return performQuery($query);
}
      ...
      $sql = "{call  svDCRMain_App(?,?,'" . $worktype . "','" . $cluster  . "',?,'Product Availability',?,'0')}";
       ...
       $sql = "SELECT isNull(max(Sl_No),0)+1 as Sl_No FROM Trans_Product_Availability";
        ...
        case "TP_Attendance":
			...
			$HMSG="";
			$sql = "select count(sf_code) HCount from TP_Attendance_App WITH (NOLOCK) where sf_code='$sfCode' and convert(date,Start_Time)=convert(date,'$eDate')  and Duty_Mode=1";
            $device = performQuery($sql);
            ...
            if($HCount==0){
				...
				$HMSG="Cannot DO Check-In for Holiday Entry ";
			}				
           /* }
            else{
...
$va =$data[1][$tempp[0]];
 ...
 if( $data[2]['ActivityCaptures']!=null && $data[2]['ActivityCaptures']!="undefined")
        ...
        case "tbTrackLoction":
            $sql = "select sf_emp_id,Employee_Id from Mas_Salesforce WITH (NOLOCK) where Sf_Code='$sfCode'";
            $sf = performQuery($sql);
 ...
 $sql = "insert into Trans_TP_Hist(SF_Code,Tour_Month,Tour_Year,Submission_date,Tour_Date,WorkType_Code_B,Worktype_Name_B,Tour_Schedule1,Objective,Worked_With_SF_Code,Division_Code,Territory_Code1,Worked_With_SF_Name,TP_Sf_Name,HQ_Code,HQ_Name,Confirmed,Change_Status,JointWork_Name,Retailer_Code,Retailer_Name) select '" . $sfCode . "',MONTH($tourDate),YEAR($tourDate),getdate(),$tourDate,$worktype_code,$worktype_name,$RouteCode,$objective,$worked_with_code," . $divisionCode[0] . ",$RouteName,$worked_with_name,$sfName,$HQ_Code,$HQ_Name,0,0,$ww,'".$RCode."','".$RName."'";
        ...
        case "LeaveApproval":
            ...
            performQuery($sql);
             ...
             $divCode = $_GET['divisionCode'];
            $sql = "SELECT sf_type FROM Mas_Salesforce WITH (NOLOCK) where Sf_Code='" . $vals['Sf_Code'] . "'";
      ...
      $sqll = "select Leave_Type from Mas_Leave_Form WITH (NOLOCK) where  Leave_Id= '$leaveid'  and Sf_Code='" . $vals['Sf_Code'] ."'";
    ...
    $query = "update MasEntitlement set LeaveAvailability=LeaveAvailability-'" . $days . "',LeaveTaken=LeaveTaken+'" . $days . "' where  SFCode='" . $vals['Sf_Code'] . "' and  DivisionCode='". str_replace(",", "",$divCode)."'  and LeaveCode=" . $LeaveType[0]['Leave_Type']. "";
      ...
      performQuery($query);
		...
		case "ondutyApproval":
            ...
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update TP_Attendance_App set Duty_Mode=1,Approved_Date='" .date('Y-m-d H:i:s'). "',Approved_by='" . $Mrsfcode . "' where id=$duty_id";
            performQuery($sql);
		 break;
		 case "ondutyApprovalR":
            ...
            $duty_id = $_GET['duty_id'];
            $Mrsfcode = $_GET['sfCode'];
            $sql = "update TP_Attendance_App set Duty_Mode=2,Approved_Date='" .date('Y-m-d H:i:s'). "',Approved_by='" . $Mrsfcode . "',Reject_reason=" . $vals['reason'] . " where id=$duty_id";
            performQuery($sql);
		 break;
  ...
  case "DCRMissedDates":
    ...
    break;
        ...
        case "LeaveForm":
            ...
            $tRw = performQuery($sql);
            $pk = (int) $tRw[0]['RwID'];
             $sql = "insert into Mas_Leave_Form(Leave_Id,Leave_Type,From_Date,To_Date,Reason,sf_code,Division_Code,Leave_Active_Flag,Created_Date,No_of_Days,Address,Half_Day_Flag,First_or_Second) select '$pk'," . $vals['Leave_Type'] . ",'" . $vals['From_Date'] . "'," . $vals['To_Date'] . "," . $vals['Reason'] . ",'$sfCode','$Owndiv','".$lvappp."','" . date('Y-m-d') . "'," . $vals['No_of_Days'] . "," . $vals['address'] . "," . $vals['HalfDay'] . "," . $vals['HalfDay_Type'] . "";
            performQuery($sql);
            if($leaveType==1){
                $sql = "SELECT sf_type FROM Mas_Salesforce WITH (NOLOCK) where Sf_Code='$sfCode'";
     ...
     $query = "update MasEntitlement set LeaveAvailability='".$LAC."' where  SFCode='$sfCode' and  DivisionCode='". str_replace(",", "",$divCode)."'  and LeaveCode=" . $vals['Leave_Type'] . "";
      ...
      performQuery($query);
			...
			$sqld = "insert into Mas_Deviation_Form(Deviation_Id,Sf_Code,Sf_Name,Deviation_Type,Deviation_Date,MobileDate,Server_Date,LatLng,Reason,Division_Code,Devi_Active_Flag)values('$pk','$sfCode','".$name."','" . $vals['Deviation_Type'] . "','" . $vals['From_Date'] . "'," . $vals['Time'] . ",'".$dt."',".$vals['LatLng']."," . $vals['reason'] . ", '$Owndiv','2')";
            performQuery($sqld);
				$MSG="";
			}else{
        ...
        if (count($productcategoryrates) > 0) {
         ...
         $result['ProductCategoryRates'] = $ProductCategoryRates;
            ...
            case "doctor_specialty":
                $query = "select Doc_Special_Code id,Doc_Special_Name name from Mas_Doctor_Speciality WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Doc_Special_Active_Flag=0";
                $results = performQuery($query);
                break;
            case "mas_doc_class":
                $query = "select Doc_ClsCode id,Doc_ClsName name from Mas_Doc_Class WITH (NOLOCK) where Division_code='" . $Owndiv . "' and Doc_Cls_ActiveFlag=0";
                $results = performQuery($query);
                break;
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
            ...
            case "vwleavetype":
                ...
                $query = "select Leave_Code id,Leave_Name name,Leave_SName from vwLeaveType WITH (NOLOCK) where Division_code='" . $Owndiv . "'";
                $results = performQuery($query);
                break;
			case "vwdeviationtype":
                $query = "select Deviation_code id,DeviationName name from vwDeviationType WITH (NOLOCK) where  Division_code='" . $Owndiv . "'";
			    $results = performQuery($query);
			...
			case "getmodeoftravel":
			    $sfCode = $_GET['sfCode'];
				$divisionCode=$_GET['divisionCode'];
				$qry="select g.Grade_ID from Mas_Salesforce s WITH (NOLOCK) inner join Mas_SF_Designation d WITH (NOLOCK) on s.Designation_Code=d.Designation_Code inner join Mas_Grade g WITH (NOLOCK) on d.Group_Name=g.Grade_ID where s.Sf_Code='".$sfCode."'";
				$res=performQuery($qry);
				$query = "select Sl_No id,Group_MOT name,StEndNeed,DriverNeed,isnull(FuelAmt,0) FuelAmt,isnull(Alw_Eligibilty,0) Alw_Eligibilty,isNull(NeedAttachment,'1') Attachment,MaxKm Max_Allowance from Mas_Modeof_Travel WITH (NOLOCK) where Active_Flag='0' and (Charindex(',".$res[0]["Grade_ID"].",',','+GradeID+',')>0 or GradeID='')";
				$results = performQuery($query);
			break;
            case "gettpdet":
                $date = date('Y-m-d');
 ...
 $query = "update Mas_SF_SlNo set POrdSl=(isnull(POrdSl,0)+1) where SF_Code='" . $sfCode . "'";
    ...
    case "vwLeaveStatus":
        $sfCode = $_GET['sfCode'];
        $sql = "select * from vwLeaveEntitle WITH (NOLOCK) where Sf_Code='$sfCode'";
    ...
    case "vwCheckLeave":
        $sfCode = $_GET['sfCode'];
        $date = date('Y-m-d');
        $sql = "select From_Date,To_Date,No_of_Days from mas_Leave_Form WITH (NOLOCK) where To_Date>='$date' and sf_code='$sfCode' and Leave_Active_Flag<>1 order by From_Date";
        $leaveDays = performQuery($sql);
        $currentDate = date_create($date);
        $disableDates = array();
        $sql = "SELECT * FROM vwActivity_Report WITH (NOLOCK) where SF_Code='" . $sfCode . "' and cast(activity_date as datetime)=cast('$date' as datetime)";
    ...
    case "vwLeave":
        ...
        $sfCode = $_GET['sfCode'];
        $sql = "select Sf_Code,Permissiondate,Approval_Flag,convert(varchar(10), Submission_date, 120) Submission_date , CONVERT(varchar(15),CAST(FromTime AS time),100) FromTime,CONVERT(varchar(15),CAST(ToTime AS time),100)  ToTime,Remarks,Noof_hours,case when Approval_Flag='2' then 'Pending'  when Approval_Flag='1' then 'Approved' else 'Reject' end PStatus,Rejected_Reason from PermissionEntry WITH (NOLOCK) where Sf_Code='$sfCode'";
        ...
        $sql = "select l.sf_code,l.Leave_Id,SF_Name SFNm,case when convert(date,getdate())< convert(date,From_Date) then 1 else 0 end showflag,convert(varchar(10), l.Created_Date, 105)  Created_Date,'0' cancelreason,Reason,(select Leave_Name from mas_Leave_Type where Leave_code=Leave_Type)Leave_Type,convert(varchar(10), From_Date, 105)  From_Date,convert(varchar(10), l.LastUpdt_Date, 105)  LastUpdt_Date,convert(varchar(10), To_Date, 105)  To_Date,No_of_Days,case when
		 Leave_Active_Flag='2' then 'Pending'  when Leave_Active_Flag='0' then 'Approved' when Leave_Active_Flag='3'  then 'CancelPedning'  else 'Reject' end LStatus ,
		 case when Leave_Active_Flag='2' then 'rgb(255,165,0) !important' when Leave_Active_Flag='3' then 'rgb(255,165,0) !important' when Leave_Active_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
        ...
        $AMod = $_GET['AMod'];
		 ...
		 case when SF_Status='2' then 'rgb(255,165,0) !important'  else 'rgb(3,192,60)  !important' end StusClr  from vwBlockReleaseStatus WITH (NOLOCK) where AuditSf_Code='$sfCode'  ";
        }else{
        $sql = "select     Sf_Name SFNm ,convert(varchar(10),convert(date,Createddate),105) Createddate,SF_Status,SF_Mobile,DeptName,blockeddate,AuditApproveddate Approveddate ,Release_Reason,AuditReason Rejected_Reason,SF_Status,AuditSf_Code,MGR_Sf_Code,case when
		 SF_Status='2' then 'Reject'  when SF_Status='1'  then 'Pending With Audit'  else 'Approved' end LStatus ,
		 case when SF_Status='2' then 'rgb(255,0,0) !important' when SF_Status='1' then 'rgb(255,165,0) !important' else 'rgb(3,192,60)  !important' end StusClr    from vwBlockReleaseStatus WITH (NOLOCK) where  MGR_Sf_Code='$sfCode'";
        }
		$PermissionST = performQuery($sql);
        outputJSON($PermissionST);
        break;
        ...
        $sql = "select sf_code,Deviation_Id,convert(varchar(10), MobileDate, 105)  Created_Date,Reason,(select DeviationName from Mas_Devation where Deviation_code=Deviation_Type)Deviation_Type,convert(varchar(10), Deviation_Date, 105)  Deviation_Date,convert(varchar(10), LastUpdt_Date, 105)  LastUpdt_Date,case when
		 Devi_Active_Flag='2' then 'Pending'  when Devi_Active_Flag='0' then 'Approved' else 'Reject' end DStatus ,
		 case when Devi_Active_Flag='2' then 'rgb(255,165,0) !important'  when Devi_Active_Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
		...
		$sql = "select l.sf_code,l.Sl_No,l.SF_Name SFNm,convert(varchar(10), l.MobileDate, 105)  Created_Date,Reason,EntryName,convert(varchar(10), JointDate, 105)  JointDate,convert(varchar(10), l.MobileDate, 105)  MobileDate,convert(varchar(10), l.LastUpdt_Date, 105)  LastUpdt_Date,case when
		 ...
		 Rejected_Reason,Flag from Trans_NewJoin l WITH (NOLOCK) inner join Mas_Salesforce s WITH (NOLOCK) on s.SF_Code=l.SF_Code
		 ...
		 Flag='2' then 'Pending'  when Flag='0' then 'Approved' else 'Reject' end DStatus ,
		 case when Flag='2' then 'rgb(255,165,0) !important'  when Flag='0' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end StusClr,
		 Rejected_Reason,Flag from Trans_NewJoin WITH (NOLOCK) where Sf_Code='$sfCode'  and Flag<>3 order by cast(JointDate as datetime) desc";
        }
...
case "get/vwVstPendingPragnancy":
    ...
    case "get/misseddates":
        ...
        $sql="select * from Mas_ListedDr WITH (NOLOCK) where Ukey='" . $UKEY . "'";
        $code=performQuery($sql);
        outputJSON($code[0]);
...
case "get/LeaveAvailabilityCheck":
    ...
    $query = "select SFCode,LeaveCode,LeaveValue,LeaveAvailability,LeaveTaken,Leave_SName,Leave_Name from MasEntitlement  WITH (NOLOCK) inner join mas_Leave_Type l WITH (NOLOCK) on Leave_code=LeaveCode where SFCode='".$sfCode. "' and year(createDate)='".$Year."'";
...
case "get/RetailerPaymentCollection":
	...
	case "get/LitterVsValues":
      ...
      $today =  $_GET['Date'];
		...
		$sql = "SELECT  UsrDfd_UserName Sf_UserName,SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,  (select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,case when Approved_By='FF' then 1 else 0 end SFFType,isnull(latlong,'12.91729615:80.23030043') AS HOLocation from mas_salesforce ms WITH (NOLOCK) inner join Mas_HQuarters m WITH (NOLOCK) on cast(HQ_ID as varchar)=SF_HQ  inner join access_master am WITH (NOLOCK) on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT WITH (NOLOCK) on At.Sf_code= ms.Sf_code where SF_Email='".$Email."' and Approved_By<>'I'";
		$res = performQuery($sql);
...
case "get/StockistGLogin":
		$Email = $_GET['Email'];  //CASE WHEN  UsrDfd_UserName='HAPHTNSAO2' THEN '18.7357416:78.9700081' ELSE '12.91729615:80.23030043' END 
		$sql = "SELECT  Username Sf_UserName,Stockist_Code Sf_Code,Cut_Off_Time,Stockist_Mobile,Stockist_Name Sf_Name,[Password],ms.Division_Code from mas_stockist ms WITH (NOLOCK) inner join access_master am WITH (NOLOCK) on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT on At.Sf_code= ms.Sf_code where SF_Email='".$Email."'";
		$res = performQuery($sql);
...
outputJSON($result);
	...
	case "Check/Version":
		...
		$result["btTitle"]="UPDATE YOUR APP";
		if($VERSION==$Ver || $VERSION=="Ver 2.1.0" || $VERSION=="Ver 2.2.0" || $VERSION=="Ver 2.2.1" || $VERSION=="Ver 2.2.2"){
			$result["Msg"] = "";
			$result["success"] = true;
		}else{
		...
		$Owndiv = (string) $divs[0];
		$today = date('Y-m-d H:i:s');
		$query = "select IHQLoc HQLoc ,convert(varchar(10),convert(date,IEntryDate), 105) [time],cast (convert(time,IServerDate) as varchar) Itime,isNull(cast (convert(time,OServerDate) as varchar),'-') Otime, IlatLng latLng ,isNull(OlatLng,'-') OlatLng from QRGateEntry WITH (NOLOCK) where   sf_code='".$SF_code."'  and convert(date,IEntryDate)='".$TodayDate."'";
