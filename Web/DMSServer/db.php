...
switch ($value[0]) {
    ...
    case "get/track":
		...
		$SF=$_GET['SF_Code'];
    	$Dt = str_replace(".000000","",$_GET['Dt']);// and cast(Auc as float)<=30
        $query = "select * from ((select replace(convert(varchar,Time,9),':000',' ') DtTime,m.lati lat,m.Long lng,Trans_Detail_Name,Doc_Spec_ShortName ChannelName,POB_Value,net_weight_value,GeoAddrs,Time DtTm,0 ordfld,1 typ ,concat(ListedDr_Address1,' ',ListedDr_Address2,' ',ListedDr_Address3, ', Phone No : ',coalesce(ListedDr_Phone,ListedDr_Mobile) ) ListedDr_Address1, c.lat geo_lat, c.long geo_log from vwActivity_MSL_Details m inner join Mas_ListedDr d on d.ListedDrCode=m.Trans_Detail_Info_Code left outer join map_GEO_Customers C on c.Cust_Code=d.ListedDrCode where  cast(convert(varchar,Time,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and Time>'$Dt'  and m.sf_code='$SF')union(select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'','','0','0','',DtTm,1,0 typ ,'' ListedDr_Address1,'' geo_lat,'' geo_log  from tbTrackLoction where SF_Code='$SF'  and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and DtTm>'$Dt')   union( select replace(convert(varchar,ModTime,9),':000',' ') DtTime,h.lati lat,h.Long lng,Trans_Detail_Name, ''ChannelName,POB_Value,'0' net_weight_value,GeoAddrs,ModTime DtTm,0 ordfld,2 typ ,'' ListedDr_Address1,c.lat geo_lat,c.long geo_log from vwActivity_CSH_Detail h left outer join Map_GEO_Distributors C on c.Cust_Code=h.Trans_Detail_Info_Code where  cast(convert(varchar,ModTime,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and ModTime>'$Dt'   and sf_code='$SF')) as t order by DtTm,ordfld";
...
$sfCode="Admin";
    ...
    case "save/tracksf":
		...
		$lat=$data['lat'];
		$lng=$data['lng'];
		$sql = "insert into tbTrackLoction(SF_code,Emp_Id,Employee_Id,DtTm,Lat,Lon,Addr,Auc,deg,DvcID) select '$sfCode','new','',getdate(),'$lat','$lng','','''','',''";
		performQuery($sql);
        break;
    case "save/trackloc":
