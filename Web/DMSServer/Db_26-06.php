      ...
      $sql = "{call  svDCRMain_App(?,?,'" . $worktype . "','" . $cluster  . "',?,'Product Availability',?,'0')}";
       ...
       $sql = "SELECT isNull(max(Sl_No),0)+1 as Sl_No FROM Trans_Product_Availability";
        ...
        case "TP_Attendance":
            ...
            if($update==0){
                $sql ="exec Attendance_entry '$sfCode','$Owndiv','$eDate',$lat,$long,'$eDate','$slfy','".$wrkType."' ,'0' ,'".$Shift_Selected_Id."','$App_Version','".$PlcID."','".$PlcNm."','".$vstRmks."' ";
                $result=performQuery($sql);     
            }
            else{
...
$resp['Query'] = $query;
	 ...
	 case "LeaveFormValidate":
				$leaveid = $_GET['leaveid'];
	$LstartDate = date('2020-06-22');
	$LstartDate = date('Y-m-d', strtotime($LstartDate));
	$LEndDate = date('2020-07-23');
	$LEndDate = date('Y-m-d', strtotime($LEndDate));
	$fromdate = date($vals['From_Date']);
	$fromdate = date('Y-m-d', strtotime($fromdate));
	$todate=date($vals['To_Date']);
	$todate = date('Y-m-d', strtotime($todate));
	if (($fromdate >= $LstartDate) && ($todate <= $LEndDate)){
	 ...
	 $resp['Msg'] = 'You Could not Apply Leave for Last Month';
	$resp["success"] = true;
	echo json_encode($resp); 
	die;
	}
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
 ...
 $query = "update Mas_SF_SlNo set POrdSl=(isnull(POrdSl,0)+1) where SF_Code='" . $sfCode . "'";
    ...
    case "vwLeave":
        ...
        $sfCode = $_GET['sfCode'];
        $sql = "select convert(varchar(10), Created_Date, 120)  Created_Date,convert(varchar(10), From_Date, 120)  From_Date,  convert(varchar(10), To_Date, 120)  To_Date,No_of_Days,case when
 Leave_Active_Flag='2' then 'Pending'  when Leave_Active_Flag='0' then 'Approved' else 'Reject' end LStatus ,case when
 Leave_Active_Flag='2' then 'rgb(255,165,0) !important'  when Leave_Active_Flag='1' then 'rgb(3,192,60)  !important' else 'rgb(255,0,0)  !important' end Color,
 Rejected_Reason,Leave_Active_Flag from mas_Leave_Form where Sf_Code='$sfCode'";
        $PermissionST = performQuery($sql);
        outputJSON($PermissionST);
        break;
...
case "get/RetailerPaymentCollection":
 ...
 case "get/GoogleLogin":
		...
		$sql = "SELECT  UsrDfd_UserName Sf_UserName,SF_Status,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,  (select count(Reporting_To_SF) from mas_salesforce where   Reporting_To_SF=ms.Sf_code)CheckCount,Geo_Fencing,case when Approved_By='FF' then 1 else 0 end SFFType,isnull(latlong,'12.91729615:80.23030043') AS HOLocation from mas_salesforce ms inner join Mas_HQuarters m on cast(HQ_ID as varchar)=SF_HQ  inner join access_master am on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT on At.Sf_code= ms.Sf_code where SF_Email='".$Email."' and Approved_By<>'I'";
		$res = performQuery($sql);
...
outputJSON($result);
	...
	case "Check/Version":
		...
		if($VERSION==$Ver || $VERSION=="Ver 2.2.0" || $VERSION=="Ver 2.2.1" || $VERSION=="Ver 2.2.2"){
			$result["Msg"] = "";
			$result["success"] = true;
		}else{
