...
function SavePrimaryOrderChk(){
	...
	if(count($StkDets)>0){
		...
		$sql = "exec svPrimaryOrder_NApp '".$DCRDetails["stockist_code"]."','".$divisionCode."','".$HPXML."','".$PPXML."','".$DCRHead["SF"]."','".$TXXML."','".$DCRDetails["cutoff_time"]."','".$DCRDetails["mode"]."','".$DCRDetails["orderId"]."','".$DCRHead["AppVer"]."','".$tcstax."','".$UKey."','".$DCRHead["loginType"]."'";
		$params=[];
		$result["Qry3"]=$sql;
		//$res=performQuery($sql);		
		if (sqlsrv_errors() != null) {
...
function SaveSuperStockistOrder(){	
	...
	$orderDate=date("Y-m-d h:i:s");	
		...
		$sql = "exec svSuperStockist_NApp '".$DCRDetails["stockist_code"]."','".$divisionCode."','".$HPXML."','".$PPXML."','".$DCRHead["SF"]."','".$TXXML."','".$DCRDetails["cutoff_time"]."','".$DCRDetails["mode"]."','".$DCRDetails["orderId"]."','".$DCRHead["AppVer"]."','".$tcstax."','".$UKey."'";	
		$params=[];	
		$result["Qry3"]=$sql;	
		$res=performQuery($sql);			
		if (sqlsrv_errors() != null) {	
...
function SavePrimaryOrder(){
	...
	$uomlist=$DCRDetails["uom_details"];
	$UKey=$DCRHead["SF"]."-".$DCRHead["UKey"];
	$sup_code=$DCRDetails["supCode"];
	$sup_name=$DCRDetails["supName"];
	$sql ="insert into PrimaryOrder_Data select '".$DCRHead["SF"]."',getdate(),'".$DCRDetails["Doc_Meet_Time"]."','".str_replace("'","",$_POST['data'])."'";
	performQuery($sql);
	...
	$sqlq = "select ERP_Code,Plant_id,0 DivERP from Mas_Stockist where Stockist_Code='".$DCRDetails["stockist_code"]."' and Stockist_Active_Flag=0";
	$StkDets=performQuery($sqlq);
	if(count($StkDets)>0){
		$sql="exec sp_validatePurchaseOrder '".$DCRHead["SF"]."','".$DCRDetails["Doc_Meet_Time"]."','".$DCRDetails["cutoff_time"]."','".$DCRDetails["groupCode"]."'";
		$orderValidate=performQuery($sql);
		...
		$HPXML = "<ROOT><Head Sf_Code=\"".$DCRHead["SF"]."\" Stk_Code=\"".$DCRDetails["stockist_code"]."\" Order_Date=\"".$DCRDetails["Doc_Meet_Time"]."\" Order_Value=\"".$DCRDetails["NetAmount"]."\"  Div_Code=\"".$divisionCode."\" Sub_Total=\"".$subtot."\" Tax_Total=\"".$taxtot."\" Dis_Total=\"".$distot."\" /></ROOT>";
		$sql = "exec svPrimaryOrder_NApp '".$DCRDetails["stockist_code"]."','".$divisionCode."','".$HPXML."','".$PPXML."','".$DCRHead["SF"]."','".$TXXML."','".$DCRDetails["cutoff_time"]."','".$DCRDetails["mode"]."','".$DCRDetails["orderId"]."','".$DCRHead["AppVer"]."','".$tcstax."','".$UKey."','".$DCRHead["loginType"]."','".$sup_code."','".$sup_name."'";
		$params=[];
		$result["Qry3"]=$sql;
		$res=performQuery($sql);		
		if (sqlsrv_errors() != null) {
...
function SaveInvoice(){
	...
	/*$sql = "exec [svDCRLstDet_NApp] '".$ACd."','".$DCRHead["SF"]."','1','".$DCRDetails["doctor_code"]."','".str_replace("'","''",$DCRDetails["doctor_name"])."','".$DCRDetails["Doc_Meet_Time"]."','".$DCRDetails["NetAmount"]."','','".$pProd."','".$npProd."','".$DCRHead["Town_code"]."','".$DCRHead["Daywise_Remarks"]."','".$DCRDetails["modified_time"]."','".$DCRDetails["Lat"]."','".$DCRDetails["Long"]."','','".$DCRDetails["orderValue"]."','0','".$DCRDetails["stockist_code"]."','".str_replace("'","''",$DCRDetails["stockist_name"])."','$divisionCode'";		
	$res=performQuery($sql);*/		
	$sql = "exec [svDCRcshDet_NApp] '".$ACd."','".$DCRHead["SF"]."','3','".$DCRDetails["doctor_code"]."','".str_replace("'","''",$DCRDetails["doctor_name"])."','".$DCRDetails["Doc_Meet_Time"]."','".$DCRDetails["NetAmount"]."','','".$pProd."','".$npProd."','','','".$DCRHead["Town_code"]."','".$DCRHead["Daywise_Remarks"]."','".$DCRDetails["modified_time"]."','".$DCRDetails["Lat"]."','".$DCRDetails["Long"]."','','".$DCRDetails["orderValue"]."','$divisionCode','0','".$DCRHead["SF"]."','',''";		
	$result["Qry2"]=$sql;		
	$res=performQuery($sql); 		
//$ACd=$res[0]["ARDCd"];		
			
			
	$sql = "exec [Sv_Primary_Invoice_App] '".$DCRDetails["doctor_code"]."','".$loginType."','".$DCRDetails["orderId"]."','".$HeadXML."','".$PPXML."','".$TXXML."','".$DCRDetails["Lat"]."','".$DCRDetails["Long"]."','".$DCRHead["UKey"]."','".$orderType."','".$DCRDetails["stockist_code"]."'";		
	$result["invqry"]=$sql;		
	$res=performQuery($sql);		
	$result["data"]=$res;
    $result["Msg"]=$res[0]['msg'];		
...
function getDistStockLedger(){
	...
	$qry="exec getDistStockLedger '$Stk','2021-04-01','".date("Y-m-d 00:00:00")."','".$Owndiv."'";
	$res = performQuery($qry);	
	if(count($res)>0){
		$result["success"]=true;
		$result["Data"]=$res;
...
function getSalesSummary(){
	...
	}else if($logintype=="SuperStockist"){	
			
		$sql = "select 0 NoOrder, isnull(sum(isnull(InvCnt,0)),0) InvCnt from (select Cus_Code,count(Cus_Code) InvCnt,isnull(sum(isnull(Total,0)),0) InvVal from Trans_Invoice_Head  WITH (NOLOCK) where Dis_Code='$SF'  and Invoice_Date>='".date('Y-m-d 00:00:00')."' and Invoice_Date<DATEADD(DAY,1,'".date('Y-m-d 00:00:00')."') group by Cus_Code) inv";	
...
function getAdvanceAmtDets(){
	$cus_code=$_GET['Customer_Code'];
	$logintype=$_GET['loginType'];
	 $sql="exec sp_Ret_adv_mod '".$cus_code."','".$logintype."' ";
      return performQuery($sql);
}
...
function getAPPSetupDms(){
	global $data;
	$data = json_decode($_POST['data'], true);
	$divisioncode=$data["div"];
        $divs = explode(",", $divisioncode . ",");
        $Owndiv = (string) $divs[0];
	$sql="select * from access_master where division_code='".$Owndiv."'";
	return performQuery($sql);
...
function getsalessummarydetail(){
	...
	if($logintype=="Distributor"){
		...
		switch ($mode) {	
			case "order":	
				$sql = "select Trans_Sl_No TransactionNo,th.Stockist_Code OutletCode,isnull(CustomerName,msc.Sup_Name) OutletName,Order_Value TransactionAmt,st.Stockist_Name FranchiseName,ISNULL(msc.Sup_Name,st.Stockist_Name)Sf_Name,CONVERT(varchar,th.Order_Date,103)+' '+CONVERT(varchar,th.Order_Date,108)Date_Time,st.Stockist_Address [Address],st.Stockist_Mobile Mobile from Trans_priOrder_Head th WITH (NOLOCK) inner join Mas_Stockist st WITH (NOLOCK) on st.Stockist_Code=th.Stockist_Code inner join Mas_SSCustomers msc on charindex(','+th.Stockist_Code+',',','+msc.Customer_code+',')>0 where msc.Sup_Code='$SF' and  cast(Order_Date as date)='".date('Y-m-d 00:00:00')."' and Order_Value>0";	
							
				
		          $psql = "select th.Trans_Sl_No TransactionNo,pd.Product_Detail_Name Product_Name,td.CQty Quantity from Trans_priOrder_Head th WITH (NOLOCK) inner join Trans_priOrder_Details td WITH (NOLOCK) on td.Trans_Sl_No=th.Trans_Sl_No inner join Mas_Product_Detail pd WITH (NOLOCK) on pd.Product_Detail_Code=td.Product_Code  inner join Mas_Stockist st WITH (NOLOCK) on st.Stockist_Code=th.Stockist_Code inner join Mas_SSCustomers msc on charindex(','+th.Stockist_Code+',',','+msc.Customer_code+',')>0 where msc.Sup_Code='$SF' and cast(Order_Date as date)='".date('Y-m-d 00:00:00')."'";	
			break;	
			case "noorder":	
					
						$sql = "select Trans_Detail_Name OutletName,st.S_Name FranchiseName,isnull(Activity_Remarks,'') Remarks,S_Name Sf_Name,CONVERT(varchar,lt.ModTime,103)+' '+CONVERT(varchar,lt.ModTime,108)Date_Time,'' [Address],st.Mobile Mobile,cast(mt.Activity_Date as date),lt.Trans_Detail_Info_Code from DCRMain_Trans mt WITH (NOLOCK) inner join DCRDetail_csh_Trans lt WITH (NOLOCK) on mt.Trans_SlNo=lt.Trans_SlNo inner join Supplier_Master st WITH (NOLOCK) on st.S_No=lt.sf_code where lt.sf_code='$SF' and cast(mt.Activity_Date as date)='".date('Y-m-d 00:00:00')."' and POB_Value=0";	
			break;	
			case "invoice":	
				$sql = "select Trans_Inv_Slno TransactionNo,Cus_Name OutletName,S_Name FranchiseName,ISNULL(sm.S_Name,'')Sf_Name,Total TransactionAmt,CONVERT(varchar,th.Invoice_Date,103)+' '+CONVERT(varchar,th.Invoice_Date,108)Date_Time,'' [Address],sm.Mobile Mobile from Trans_Invoice_Head th WITH (NOLOCK) inner join Mas_Stockist st WITH (NOLOCK) on st.Stockist_Code=th.Cus_Code inner join Supplier_Master sm WITH (NOLOCK) on sm.S_No=th.Sf_Code where th.Sf_Code='$SF' and convert(Date,Invoice_Date)='".date('Y-m-d 00:00:00')."'";	
					
							
				$psql = "select th.Trans_Inv_Slno TransactionNo,pd.Product_Detail_Name Product_Name,td.Quantity from Trans_Invoice_Head th WITH (NOLOCK) inner join Trans_Invoice_Details td WITH (NOLOCK) on td.Trans_Inv_Slno=th.Trans_Inv_Slno inner join Mas_Product_Detail pd WITH (NOLOCK) on pd.Product_Detail_Code=td.Product_Code where th.Dis_Code='$SF'  and convert(Date,Invoice_Date)='".date('Y-m-d 00:00:00')."'";	
			break;	
			case "neworder":	
			//$sql = "select Trans_Sl_No TransactionNo,th.Stockist_Code OutletCode,CustomerName OutletName,Order_Value TransactionAmt,st.Stockist_Name FranchiseName,ISNULL(sm.S_Name,st.Stockist_Name)Sf_Name,CONVERT(varchar,th.Order_Date,103)+' '+CONVERT(varchar,th.Order_Date,108)Date_Time,st.Stockist_Address [Address],st.Stockist_Mobile Mobile from Trans_priOrder_Head th WITH (NOLOCK) inner join Mas_Stockist st WITH (NOLOCK) on st.Stockist_Code=th.Stockist_Code inner join Supplier_Master sm WITH (NOLOCK) on sm.S_No=th.Sf_Code where th.Sf_Code='$SF' and cast(Order_Date as date)='".date('Y-m-d 00:00:00')."' and Order_Value>0";	
			$sql = "select Trans_Sl_No TransactionNo,th.Stockist_Code OutletCode,isnull(CustomerName,msc.Sup_Name) OutletName,Order_Value TransactionAmt,st.Stockist_Name FranchiseName,ISNULL(msc.Sup_Name,st.Stockist_Name)Sf_Name,CONVERT(varchar,th.Order_Date,103)+' '+CONVERT(varchar,th.Order_Date,108)Date_Time,st.Stockist_Address [Address],st.Stockist_Mobile Mobile from Trans_priOrder_Head th WITH (NOLOCK) inner join Mas_Stockist st WITH (NOLOCK) on st.Stockist_Code=th.Stockist_Code inner join Mas_SSCustomers msc on charindex(','+th.Stockist_Code+',',','+msc.Customer_code+',')>0 where msc.Sup_Code='$SF' and  cast(Order_Date as date)='".date('Y-m-d 00:00:00')."' and Order_Value>0";			
				
		         // $psql = "select th.Trans_Sl_No TransactionNo,pd.Product_Detail_Name Product_Name,td.CQty Quantity from Trans_priOrder_Head th WITH (NOLOCK) inner join Trans_priOrder_Details td WITH (NOLOCK) on td.Trans_Sl_No=th.Trans_Sl_No inner join Mas_Product_Detail pd WITH (NOLOCK) on pd.Product_Detail_Code=td.Product_Code where th.Sf_code='$SF' and cast(Order_Date as date)='".date('Y-m-d 00:00:00')."'";	
			 $psql = "select th.Trans_Sl_No TransactionNo,pd.Product_Detail_Name Product_Name,td.CQty Quantity from Trans_priOrder_Head th WITH (NOLOCK) inner join Trans_priOrder_Details td WITH (NOLOCK) on td.Trans_Sl_No=th.Trans_Sl_No inner join Mas_Product_Detail pd WITH (NOLOCK) on pd.Product_Detail_Code=td.Product_Code  inner join Mas_Stockist st WITH (NOLOCK) on st.Stockist_Code=th.Stockist_Code inner join Mas_SSCustomers msc on charindex(','+th.Stockist_Code+',',','+msc.Customer_code+',')>0 where msc.Sup_Code='$SF' and cast(Order_Date as date)='".date('Y-m-d 00:00:00')."'";	
...
function getOrderAndInvoice(){
	...
	//$sql = "select h.Trans_Sl_No OrderID,dr.Stockist_Name OutletName,CONVERT(varchar,h.Order_Date,103)[Date],h.Order_Value,case when h.Order_Flag=1 then 'Completed' when h.Order_Flag=0 then 'Pending' else '' end [Status] from Trans_priOrder_Head h WITH (NOLOCK) left join Mas_Stockist dr WITH (NOLOCK) on h.Stockist_Code=CAST(dr.Stockist_Code as varchar) where CONVERT(date,h.Order_Date) between '".$fdt."' and '".$tdt."'  and h.Sf_Code='".$stkcode."' and Order_Value>0 order by Order_Date";	
	$sql="select h.Trans_Sl_No OrderID,dr.Stockist_Name OutletName,CONVERT(varchar,h.Order_Date,103)[Date],h.Order_Value,case when h.Order_Flag=1 then 'Completed' when h.Order_Flag=0 then 'Pending' else '' end [Status] from Mas_SSCustomers ms left join Trans_priOrder_Head h WITH (NOLOCK) on CHARINDEX(','+h.Stockist_Code+',',','+ms.Customer_code+',')>0 left join Mas_Stockist dr WITH (NOLOCK) on h.Stockist_Code=CAST(dr.Stockist_Code as varchar) where CONVERT(date,h.Order_Date) between '".$fdt."' and '".$tdt."'   and ms.Sup_Code='".$stkcode."' and Order_Value>0 order by Order_Date";
	$Orders=performQuery($sql);	
	
	$sql = "select h.Trans_Sl_No OrderID,d.Product_Code,d.Product_Name,d.CQty Quantity from Mas_SSCustomers ms left join Trans_priOrder_Head h WITH (NOLOCK) on CHARINDEX(','+h.Stockist_Code+',',','+ms.Customer_code+',')>0 inner join Trans_priOrder_Details d WITH (NOLOCK) on h.Trans_Sl_No=d.Trans_Sl_No where CONVERT(date,h.Order_Date) between '".$fdt."' and '".$tdt."'  and ms.Sup_Code='".$stkcode."'order by Order_Date";	
	$OrdersDetails=performQuery($sql);	
		
     $sql = "select Trans_Detail_Slno OrderID,stockist_name OutletName,CONVERT(varchar,Activity_Date,103)[Date],td.POB_Value Order_Value,'No Order' [Status],'' Details from DCRMain_Trans th WITH (NOLOCK) inner join DCRDetail_csh_Trans td WITH (NOLOCK) on th.Trans_SlNo=td.Trans_SlNo inner join Mas_Stockist d WITH (NOLOCK) on d.Stockist_code=td.Trans_Detail_Info_Code where CONVERT(date,Activity_Date) between '".$fdt."' and '".$tdt."' and POB_Value<=0 and td.sf_code='".$stkcode."' order by Activity_Date";	
	$NoOrders=performQuery($sql);	
		
	
    //$sql = "select h.Trans_Inv_Slno InvoiceID,h.Order_No OrderID,dr.stockist_name ListedDr_Name,dr.stockist_code code,CONVERT(varchar,h.Invoice_Date,103)[Date],Dis_Code Stockist_Code,h.Total Order_Value,'Completed' [Status] from Trans_Invoice_Head h  WITH (NOLOCK) inner join Mas_Stockist dr WITH (NOLOCK) on h.Cus_Code=CAST(dr.stockist_code as varchar) where h.Invoice_Date>='".$fdt."' and h.Invoice_Date<DATEADD(DAY,1,'".$tdt."')   and h.Dis_Code='".$stkcode."' order by Invoice_Date";	
	$sql="select h.Trans_Inv_Slno InvoiceID,h.Order_No OrderID,dr.stockist_name ListedDr_Name,dr.stockist_code code,CONVERT(varchar,h.Invoice_Date,103)[Date],Dis_Code Stockist_Code,h.Total Order_Value,'Completed' [Status] from Mas_SSCustomers ms left join Trans_Invoice_Head h  WITH (NOLOCK) on CHARINDEX(','+h.Cus_Code+',',','+ms.Customer_code+',')>0   inner join  Mas_Stockist dr WITH (NOLOCK) on h.Cus_Code=CAST(dr.stockist_code as varchar) where h.Invoice_Date>='".$fdt."' and h.Invoice_Date<DATEADD(DAY,1,'".$tdt."') and  ms.Sup_Code='".$stkcode."' order by Invoice_Date ";
	$Invoice=performQuery($sql);	
		
	$sql = "select  h.Trans_Inv_Slno InvoiceID,d.Product_Code,d.Product_Name,d.Quantity from Mas_SSCustomers ms left join Trans_Invoice_Head h  WITH (NOLOCK) on CHARINDEX(','+h.Cus_Code+',',','+ms.Customer_code+',')>0 inner join Trans_Invoice_Details d WITH (NOLOCK) on d.Trans_Inv_Slno=h.Trans_Inv_Slno where h.Invoice_Date>='".$fdt."' and h.Invoice_Date<DATEADD(DAY,1,'".$tdt."')  and ms.Sup_Code='".$stkcode."'	order by Invoice_Date";	
	$InvoiceDetails=performQuery($sql);	
...
function getProdGrpList(){
	...
	$Stk=$data["Stk"];
	$loginType=$data["loginType"];
	$sql = "exec getProductGroupList_mod '".$Stk."','".$loginType."'";
	$res=performQuery($sql);
	return $res;
}
...
function getProdTypList(){
	...
	$Stk=$data["Stk"];
	$loginType=$data["loginType"];
	$sql = "exec getProductTypeList_mod '".$Stk."','".$loginType."'";
	$res=performQuery($sql);
	return $res;
}
...
function getProdCateList(){
	...
	$Stk=$data["Stk"];
	$loginType=$data["loginType"];
	$sql = "exec GetProdBrand_DMSApp_mod '".$Stk."','".$loginType."'";
	$res=performQuery($sql);
	return $res;
}
...
function getPrimaryProdDetList(){
	...
	//$sql = "exec GetPriProducts_App '".$Stk."'";
	//$res=performQuery($sql);	
...
function getSsBaseDistributor(){
	global $data;
	$data = json_decode($_POST['data'], true);
	$SF=$data["SS_Code"];
	$sql = "select * from Mas_stockist ms inner join Mas_SSCustomers msc on charindex(','+ms.Stockist_Code+',',','+msc.Customer_code+',')>0 where msc.Sup_Code='".$SF."'";
	$res=performQuery($sql);
	return $res;
	
}
function getDbBaseSupplier(){
	global $data;
	$data = json_decode($_POST['data'], true);
	$SF=$data["SF"];
	$sql = "select Sup_Code,Sup_Name  from Mas_stockist ms inner join Mas_SSCustomers msc on charindex(','+ms.Stockist_Code+',',','+msc.Customer_code+',')>0 where ms.Stockist_Code='".$SF."'";
	$res=performQuery($sql);
	return $res;
}
...
function getRouteList(){
	...
	$sql = "exec getRouteListByDis_mod '$Stk','".$loginType."','$Owndiv'";
	$res=performQuery($sql);
	return $res;
}
...
switch (strtolower($value[0])) {
	case "get/distributor":
		...
		break;
	case "get/ssbaasedistributor":
		outputJSON(getSsBaseDistributor());
		break;		
	case "get/dbbasesupplier":
		outputJSON(getDbBaseSupplier());
		break;		
	case "get/routelist":
	...
	case "save/invoice":
        ...
        break;
	case "save/primaryinvoice":	
		outputJSON(SavePrimaryInvoice());	
        break;	
	case "get/foodexp":
		getFoodExp();
		break;	
	...
	case "get/primaryscheme":
		...
		$DT=date("Y-m-d");
		$loginType=$data["loginType"];
		$qry = "exec get_scheme_by_stk_Mod '" . $sfCode . "','".$divisionCode."','".$DT."','".$loginType."'";
		$res=performQuery($qry);
		if (count($res) > 0) {
			//$result["success"]=true;
			//$result["Data"]=$res;
	...
	case "get/primaryproducttaxdetails":
		...
		$divisionCode = (string) $divs[0];
		$loginType=$data["loginType"];
		$qry="exec getAppPrimaryTax '$divisionCode','$sfCode','".$loginType."'";
		$res=performQuery($qry);
		if (count($res) > 0) {
			$result["success"]=true;
			$result["Data"]=$res;
	...
	case "get/customeroutstanding":
		...
		if (count($ress) > 0) {
			...
			$res[0]=array("Outstanding"=>$ress[0]["Balance"]);
                        $res[1]=array("AmountReceive"=>$ress[0]["Credit"]);
			$result["success"]=true;
			$result["Data"]=$res;
		}
		else{
	...
	case "get/primarydashboardvalues":
		...
		$Grpcode=$data["Grpcode"];
		$loginType=$data["loginType"];
		$distCode=$data["distributorcode"];
		//$sql = "exec getPrimaryOrderDashBoard '$sfcode','$Dt','$Grpcode','$loginType'";
		$sql = "exec getPrimaryOrderDashBoard_modNew '$sfcode','$Dt','$Grpcode','$loginType'";
		$res = performQuery($sql);
            //$sqls = "exec getSecOrderDashBoard '$sfcode','$Dt'";
			 $sqls = "exec getSecOrderDashBoard_mod '$sfcode','$Dt','$loginType','$distCode'";
...
case "get/secOrderDashBoard":
	...
	break;
	case "save/superstockistorder":	
        outputJSON(SaveSuperStockistOrder());	
	break;
