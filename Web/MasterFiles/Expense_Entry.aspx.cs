...
public partial class MasterFiles_Expense_Entry : System.Web.UI.Page
{
    ...
    public static Exp_App_Details[] GetAppDetails(string SF_Code, string ExpYear, string ExpMonth)
    {
        ...
        foreach (DataRow row in dsAlowtype.Tables[0].Rows)
        {
            ...
            ffd.expName = row["expName"].ToString();
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < JMTE.TED.Count; i++)
                    {
                        ...
                        for (int j = 0; j < JMTE.TED[i].adddtls.Count; j++)
                        {
							...
							if( JMTE.TED[i].adddtls[j].alw_code.ToString() != "0")
							{ 
        ...
        else
        {
            ...
            try
            {
                ...
                if (savemode == "1")
                {
                    ...
                    sqlQry = "update Trans_Expense_Head1 set Expense_Amt=@Expense_Amt, Expense_Mode=@Expense_Mode where Trans_Sl_No=@Trans_Sl_No";
                    cmd = new SqlCommand(sqlQry);
                    cmd = new SqlCommand(sqlQry);
                    cmd.Connection = con;
                    cmd.Transaction = tran;
    ...
    public static string GetDate(string SF_Code, string ExpYear, string ExpMonth)
    {
        ...
        if (trans_id != string.Empty)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                sqlQry = string.Format("select * from  Trans_Daily_Allowance_Details  where  trans_dt_slno='" + trans_id + "' and exp_date =  cast(CONVERT(VARCHAR, '" + exp_date + "', 101) as datetime)");
                con.Open();
                cmd = new SqlCommand(sqlQry, con);
                DataTable dtt = new DataTable();
