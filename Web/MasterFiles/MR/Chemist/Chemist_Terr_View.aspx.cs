...
using System.Data;
using Bus_EReport;
public partial class MasterFiles_MR_Chemist_Chemist_Terr_View : System.Web.UI.Page
{
        ...
        if (ViewState["dsSalesForce"] != null)
        {
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                foreach (DataRow dataRow in dsSalesForce.Tables[0].Rows)
                {
                    ...
                    iDRCatg = chem.getTerr_Chem_Count(drFF["Territory_Code"].ToString());
                    if (iDRCatg == 0)
                    {
                        sDRCatg_Count = " - ";
                    }
                    else
                    {
                        ...
                        sURL = "rptChemist_terr_View.aspx?sf_code=" + sf_code + "&terr_code=" + drFF["Territory_Code"].ToString() + " &terr_Name=" + drFF["Territory_Name"].ToString() + "&div=" + div_code;
                        hyp_catg_det_name.Attributes["onclick"] = "javascript:window.open('" + sURL + "',null,'resizable=yes,toolbar=no,menubar=no,scrollbars=yes,status=no,width=800,height=800,left=0,top=0');";
            ...
            foreach (DataRow dataRow in dsSalesForce.Tables[0].Rows)
            {
                ...
                lit_catg_det_name.Text = "<center>" + dataRow["Chemists_Count"].ToString() + "</center>";


                hyp_catg_det_total.Text = "<center>" + dataRow["Chemists_Count"].ToString() + "</center>";
                tot_catg = tot_catg + Convert.ToInt16(dataRow["Chemists_Count"].ToString());
            ...
            if (tot_catg > 0)
                ...
                else
                {
                    ...
                    sURL = "rptChemist_terr_View.aspx?mgr_code=" + sf_code + "&type=" + "" + "&div=" + div_code;
