...
public partial class MasterFiles_Report_TP_View_Report : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //MGRFieldForce();
        div_code = Session["div_code"].ToString();
        string subdivc = ddlDivision.SelectedValue.ToString();
        string sfccode = ddlFilterby.SelectedValue.ToString();
      

        if (subdivc == "" || subdivc == null)
        { subdivc = "0"; }

        if (sfccode == "" || sfccode == null)
        { sfccode = "0"; }

        if (Session["sf_type"].ToString() != "2")
        {
            FillFieldForce(div_code, subdivc, sfccode);
        }
        FillColor();
    ...
    private void MGRFieldForce(string div_code, string subdiv)
    {
        SalesForce sf = new SalesForce();
		loc sf1 = new loc();
        div_code = Session["div_code"].ToString();
		sfCode = Session["sf_code"].ToString();
        subdiv = ddlDivision.SelectedValue.ToString();

        if (subdiv == "" || subdiv == null)
        { subdiv = "0"; }

        DataSet dsSalesForce = new DataSet();
        dsSalesForce = sf1.SalesForceListMgrGet_MgrOnly(div_code, sfCode, subdiv);
        ...
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ...
            //ddlFilterby.Items.Insert(0, new ListItem("--Select--", "0"));
    ...
    private void FillFieldForce(string div_code, string subdiv, string sfcode)
    {
        SalesForce sf = new SalesForce();

        div_code = Session["div_code"].ToString();
        subdiv = ddlDivision.SelectedValue.ToString();
        sfcode = ddlFilterby.SelectedValue.ToString();
        ...
        if (sfcode != "0")
        {
            ...
            dsSalesForce = sf.Get_FillFieldForce(div_code, subdiv, sfcode);
            if (dsSalesForce.Tables[0].Rows.Count > 0)
            {
    ...
    protected void ddlFilterBy_SelectedChange(object sender, EventArgs e)
    {
        ...
        string subdiv = ddlDivision.SelectedValue.ToString();
        SalesForce sf = new SalesForce();
        //dsSalesForce = sf.SalesForceList(div_code, sReport, subdiv);
        dsSalesForce = new DataSet();

        FillFieldForce(div_code, subdiv, sReport);
    ...
    {
        string subdiv = ddlDivision.SelectedValue.ToString();
        string sAlpha = ddlAlpha.SelectedValue.ToString();
        string sReport = ddlFilterby.SelectedValue.ToString();
	...
	public class loc
    {
        public DataSet SalesForceListMgrGet_MgrOnly(string divcode, string sf_code, string Sub_Div_Code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsSF = null;
            string strQry = "EXEC getHyrSFList_MGR '" + divcode + "', '" + sf_code + "', '" + Sub_Div_Code + "' ";
