...
public partial class MasterFiles_Reports_New_rpt_DCR_Status_Datewise_Det : System.Web.UI.Page
{
    ...
    private void CreateDynamicTable()
    {
        ...
        if (ViewState["dsSalesForce"] != null)
        {
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                DateTime dtfrom1 = dtDCRfrom;
                dsDCR = dc.getDCR_Report_Det_New_Date(drFF["sf_code"].ToString(), div_code, dtDCRfrom.ToString("MM/dd/yyyy"), dtDCRto.ToString("MM/dd/yyyy"), sf_type);
                ...
                if (dsDCR.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow datarow in dsDCR.Tables[0].Rows)
                    {
                        ...
                        do
                            ...
                            docdate = docdate + 1;
                            dtfrom1 = dtfrom1.AddDays(1);
                            chkdate = dtfrom1.Day;
                         } while (chkdate == Convert.ToInt16(datarow["Activity_Date"].ToString()));
                    }
                    sDCR = " - ";
                    if ((tot_days + 1) >= docdate)
                    {
                        cday = docdate;
                        while (cday <= (tot_days + 1))
                        {
