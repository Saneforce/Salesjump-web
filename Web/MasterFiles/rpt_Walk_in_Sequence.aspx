<%@ Page Language="C#" AutoEventWireup="true" CodeFile="rpt_Walk_in_Sequence.aspx.cs"
    Inherits="MasterFiles_rpt_Walk_in_Sequence" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Walk_in_Sequence</title>
        ...
        $(document).ready(function () {
            ...
            genReport = function () {
                if (rDtls.length > 0 && cDtls.length > 0) {
                    ...
                    var len = 1;
                    var str = '<th rowspan="1" style="width:70%">Name</th><th rowspan="1" style="width:30%">Sequance No</th>';
                    $(tbl).find('thead').append('<tr style="background-color: #496a9a; color: white;">' + str + '</tr>');


                    for (var i = 0; i < rDtls.length; i++) {
                        var str = '<td >' + rDtls[i].rName + '</td><td></td> ';
                        ...
                        if (vR.length > 0) {
                            for (var k = 0; k < vR.length; k++) {
                                var slno = (k + 1);
                                str = '<td><input type="hidden" name="custCode" value="' + vR[k].cCode + '"/> ' + vR[k].cName + '</td><td><input type="text" name="txtsequance" oldVal="' + slno + '" class="' + rDtls[i].rCode + '" value="' + slno + '"/></td>';
            ...
            function loadData() {
                ...
                $.ajax({
                    ...
                    async: false,
                    url: "rpt_Walk_in_Sequence.aspx/getRoutes",
                    data: "{'SF_Code':'" + sfCode + "'}",
                    dataType: "json",
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "rpt_Walk_in_Sequence.aspx/getCustomerSequance",
                    data: "{'SF_Code':'" + sfCode + "'}",
                    dataType: "json",
                    success: function (data) {
                        cDtls = data.d;
            ...
            $(document).on('blur', 'input[name="txtsequance"]', function () {
            ...
            $(document).on('click', '#btnSave', function (e) {
                ...
                $('[id*=cTable] tbody .rtclass').each(function () {
                    arr.push({
                        cCode: $(this).find('input[name="custCode"]').val(),
                        sequnaceNo: $(this).find('input[name="txtsequance"]').val()
                    });
                });
                $.ajax({
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "rpt_Walk_in_Sequence.aspx/SaveSequence",
