...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <head>
		...
		$('#<%=fold.ClientID%>').on('change', function () {
                ...
                $('#<%=btnSubmit.ClientID%>').click(function () {
                    ...
                    if ($('#<%=chkboxLocation.ClientID%> input:checked').length > 0) { } else { alert('Select State'); return false; }
                    ...
                    for (i = 1; i <= $utr.length; i++) {
                        ...
                        if (sname != null && tname == 0) {
                            //alltax.push(tax);
                            alert('Select The tax type for selected states');
                            return false;
                        }
                        if (sname != null && tname != 0) {
                            if (alltax.length > 0) {
                                for (var l = 0; l < alltax.length; l++) {
                                    if (alltax[l].tname == tname) {
                                        if (alltax[l].sname.includes(sname)) {
                                            filteredArray.push(sname);
                                        } else {
                                            filteredArray = [];
                                        }
                                    }
                            ...
                            if (filteredArray.length > 0) {
                                alert('Tax is already assigned to the selected state....'); return false;
                            } else {
                                alltax.push(tax);
                            }
    ...
    <body>
        <form id="form1" runat="server">
            <div>
                <asp:HiddenField ID="hiddenval1" runat="server" />
                ...
                <td>
                           ...
                           <asp:FileUpload ID="fileUpload11" runat="server" />
                                                    ...
                                                    <asp:TemplateField HeaderText="Edit" ItemStyle-HorizontalAlign="Left" HeaderStyle-Width="40px">
                                                        ...
                                                        <EditItemTemplate>
                                                            <asp:Button ID="btnupdate" runat="server" Text="Update" CommandName="Update" OnClientClick="Confirmation()"  />
                                                        </EditItemTemplate>
                                                    </asp:TemplateField>
                                                    <asp:TemplateField HeaderText="Delete" ItemStyle-HorizontalAlign="Left" HeaderStyle-Width="40px">
                                                        <ItemTemplate>
                                                            <asp:Button ID="btndelete" runat="server" Text="Delete" OnClientClick="Confirm()" CommandName="Delete" />
                                                        </ItemTemplate>
