...
public partial class MIS_Reports_Visit_Details_Field1 : System.Web.UI.Page
{
        ...
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ...
            if (months > 0)
            {
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    tc_month.ColumnSpan = dsDoctor.Tables[0].Rows.Count +  9;
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                if (months > 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        TableCell tc_DrsMet = new TableCell();
                        Literal lit_DrsMet = new Literal();
                        lit_totDrs.Text = "<center>" + doc_met_total.ToString() + "</center>";
                        tc_DrsMet.BorderStyle = BorderStyle.Solid;
                        tc_DrsMet.BorderWidth = 1;
                        tc_DrsMet.Width = 50;
                        tc_DrsMet.Controls.Add(lit_totDrs);
