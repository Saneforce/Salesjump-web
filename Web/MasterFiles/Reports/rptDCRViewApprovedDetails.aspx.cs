...
public partial class Reports_rptDCRViewApprovedDetails : System.Web.UI.Page
{
  ...
  DataSet dsdos = null;
    ...
    private void CreateDynamicTable(int imonth, int iyear, string sf_code)
    {
        ...
        tc_day.Style.Add("font-size", "9pt");
        ...
        tr_ff.Attributes.Add("Class", "tblRow");
        ...
        dsdos= dc.dcr_product_details(sf_code, StrDate);
        string sprd = "";
 iCount = 0;
        if (dsdoc.Tables[0].Rows.Count > 0)
        {
            ...
            tr_det_head.Cells.Add(tc_det_head_spec);
             ...
             foreach (DataRow drStk in dsdos.Tables[0].Rows)
            {
  if (("," + sprd).IndexOf("," + drStk["Product_Code"].ToString() + ",") < 0)
                {

                    sprd += drStk["Product_Code"].ToString() + ",";
                ...
                TableCell tc_det_head_prod = new TableCell();
                            ...
                            lit_det_head_prod.Text = drStk["Product_Name"].ToString();
   ...
   string pro = "";
            ...
            foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
            {
                ...
                tr_det_sno.Attributes.Add("Class", "tblRow");
      ...
      foreach (DataRow drFF in dsdos.Tables[0].Rows)
                        {
                        if (("," + pro).IndexOf("," + drdoctor["ListedDrCode"]+"."+drFF["Product_Code"].ToString() + ",") < 0)
                    {

                        pro += drdoctor["ListedDrCode"] + "." + drFF["Product_Code"].ToString() + ",";

                            DataRow[] ro = dsdos.Tables[0].Select("ListedDrCode='" + drdoctor["ListedDrCode"].ToString() + "' and Product_Code='"+ drFF["Product_Code"].ToString()+"'");
                            if (ro.Length > 0)
                            {
                                TableCell tc_det_prod = new TableCell();
                                Literal lit_det_prod = new Literal();
                                lit_det_prod.Text = ro[0]["qnty"].ToString();
        ...
        dsdoc = dc.GetTable(sf_code, StrDate); //Distributor Hunting
        iCount = 0;
        if (dsdoc.Tables[0].Rows.Count > 0)
        {
        ...
        dsdoc = dc.get_Temp_and_Approved_dcr_che_details(sf_code, StrDate, 2); 
        iCount = 0;
        if (dsdoc.Tables[0].Rows.Count > 0)
        {
            TableRow tr_det_head = new TableRow();
