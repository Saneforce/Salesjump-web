...
<head runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            var list = $("#gvtotalorder tr td:nth-child(2)");
            var list1 = $("#gvtotalorder tr:last td");
            var len = $("#gvtotalorder tr:last  td").length;
            var sf = 0;
            for (i = 0; i < len; i++) {
                var cnt = i + 1;
                var list2 = $("#gvtotalorder tr td:nth-child(" + cnt + ")");
                sf = $(list1[i]).text();
                //alert(sf);
                if (sf == 0) {
                    list2.hide();
    ...
    <script type="text/javascript">
        function loadaddrs($tr) {
            ...
            var Long = parseFloat($($tr).attr('long').trim());
            var Lat = parseFloat($($tr).attr('lat').trim());
            var addrs = '';
            ...
            $($tr).text(addrs);
            i++;
            setTimeout(function () { loadaddrs($(document).find('.latlong')[i]) }, 10);
            // });
        ...
        $(document).ready(function () {
            if ($('#hmode').val() == 'Primary Detailed View') {


                $('#Primarytbltotal').hide();

                var totcnt = 6;

                $('#Primarytbl tr th').each(function (i) {

                    //                alert(($('#Viewalldcrdates1 tr').length - 1));
                    var remove = 0;
                    //select all tds in this column
                    var tds = $(this).parents('table').find('tr td:nth-child(' + (i + 7) + ')');
                ...
                var cc = $('#totalid');
                cc.attr("colspan", totcnt);

                $('#Primarytbltotal').show();
            }
            else if ($('#hmode').val() == 'Detailed View') {
                var tdrows = $('#hdetailedview').val();

                for (g = 1; g <= tdrows; g++) {
                    var $parentdt = $('#tdetailtotal' + g);
                    var $dffdt;

                    $dffdt = $parentdt.clone(true, true);


                    $('#tdetailtotal' + g).empty();



                    $('#tbldetailview' + g + ' tr th').each(function (n) {

                        //                alert(($('#Viewalldcrdates1 tr').length - 1));
                        var remove = 0;
                        //select all tds in this column
                        var tds1 = $(this).parents('table').find('tr td:nth-child(' + (n + 7) + ')');
                    ...
                    var ttdt = $('#tdetailtotal' + g);

                    $dffdt.insertAfter(ttdt);
                }
            }
            else if ($('#hmode').val() == 'Incentives Summary') {
                var trows = $('#promvalue').val();

                var worktype;
                for (h = 1; h <= trows; h++) {
                    worktype = $('#wrktypeidpro' + h + '').html();
                ...
                if (worktype == undefined) {
                    ...
                    for (k = 1; k <= trows; k++) {
                        ...
                        $('#totalrow' + k).empty();
                        $('#promotionaltbl' + k + ' tr th').each(function (i) {

                            //                alert(($('#Viewalldcrdates1 tr').length - 1));
                            var remove = 0;
                            //select all tds in this column
                            var tds = $(this).parents('table').find('tr td:nth-child(' + (i + 11) + ')');
                            console.log(tds.text());
                            tds.each(function (j) {
                                if (this.innerHTML == '') {
                                    this.innerHTML = '';
            ...
            else if ($('#hmode').val() == 'View_All_DCR_Date(s)') {
                if ($('#hdivcode').val() != '26') {
                    var trows = $('#newvalldcr').val();

                    var worktype;
                    for (h = 1; h <= trows; h++) {
                        worktype = $('#wrktypeid' + h + '').html();
                    ...
                    if (worktype == undefined) {
                        ...
                        for (k = 1; k <= trows; k++) {
                            ...
                            $('#totalrow' + k).empty();
                            $('#Viewalldcrdates' + k + ' tr th').each(function (i) {
            ...
            setTimeout(function () { loadaddrs($(document).find('.latlong')[0]) }, 10);
    ...
    <script type="text/jscript">
        $(document).ready(function () {
            ...
            //            $('#<%=gvtotalorder.ClientID%> tr td:not(:nth-child(1), :nth-child(2))').each(function () {
            //                //  alert($(this).text());
            //                $('#<%=gvtotalorder.ClientID%>').find("th:nth-child(" + 3 + "), td:nth-child(" + 3 + ")").hide();
            //                if ($(this).text() != "0")

            //                // alert(k);
            //                    $(this).html('<a href=rpt_Retailer_History1.aspx?&cus_code=' + $(this).closest('tr').find('td:eq(2)').text() + '&cus_Name=' + $(this).closest('tr').find('td:eq(1)').text() + '&Territory_Name=' + $('#<%=gvtotalorder.ClientID%>').find('th').eq(3).text() + '>' + $(this).html() + '</a>')
            //                k = k + 4;
                ...
                if (k > 0) {
                    if ($(tds[1]).text() != "TOTAL") {
                        ...
                        if ($(tds[2]).text() != "0") {
                            ...
                            if ($('#hmode').val() == 'Primary SKU Summary') {
                                $(tds[1]).html('<a class="AAA" href="Rpt_DCR_View.aspx?&sf_code=' + $(tds[2]).text() + '&Mode=Primary Detailed View&FDate=' + $('#<%=DCRdt.ClientID%>').val() + '&cur_month=0&cur_year=0&Tdate=' + $('#<%=DCRdtT.ClientID%>').val() + '&Sf_Name=' + $(tds[1]).html() + '">' + $(tds[1]).html() + '</a>')

                            }
                            else {
                                if ($('#hmode').val() == 'Promotional Value Summary') {
                                }
                                else {
                                    $(tds[1]).html('<a class="AAA" href=Rpt_DCR_View.aspx?&sf_code=' + $(tds[2]).text() + '&Mode=New_View_All_DCR_Date(s)&FDate=' + $('#<%=DCRdt.ClientID%>').val() + '&cur_month=0&cur_year=0&Tdate=' + $('#<%=DCRdtT.ClientID%>').val() + '&Sf_Name=>' + $(tds[1]).html() + '</a>')
...
<body>
    <form id="form1" runat="server">
        <asp:HiddenField runat="server" Value="" ID="newvalldcr" />
        <asp:HiddenField runat="server" Value="" ID="promvalue" />
        <asp:HiddenField runat="server" Value="" ID="hdetailedview" />
        <asp:HiddenField runat="server" Value="" ID="hmode" />
        <asp:HiddenField runat="server" Value="" ID="DCRdt" />
        <asp:HiddenField runat="server" Value="" ID="DCRdtT" />
        <asp:HiddenField runat="server" Value="" ID="hdivcode" />
        <div>
            <center>
                    ...
                    <tr>
                        <td>
                            <asp:GridView ID="GridView2" runat="server" Width="100%" ShowHeader="false" Font-Size="10pt"
                                HorizontalAlign="Center" OnDataBound="OnDataBound1" OnRowCreated="OnRowCreated1"
                                OnRowDataBound="GridView1_RowDataBound1" BorderWidth="1px" CellPadding="2" EmptyDataText="No Data found for View"
                                AutoGenerateColumns="true" BackColor="#ffffe0" RowStyleCssClass="gridpager" HeaderStyle-HorizontalAlign="Center"
                                BorderColor="Black" BorderStyle="Solid" ShowFooter="false">
                                <Columns>
                                </Columns>
                                <EmptyDataRowStyle ForeColor="Black" BackColor="AliceBlue" Height="5px" BorderColor="Black"
                                    BorderStyle="Solid" BorderWidth="2" Font-Bold="True" HorizontalAlign="Center"
                            ...
                            <asp:GridView ID="GridView3" runat="server" Width="100%" Font-Size="10pt" HorizontalAlign="Center"
                                OnDataBound="OnDataBound1" BorderWidth="1px" CellPadding="2" EmptyDataText="No Data found for View"
                                BackColor="#ffffe0" RowStyleCssClass="gridpager" HeaderStyle-HorizontalAlign="Center"
                                BorderColor="Black" BorderStyle="Solid"  Visible="false" CssClass="newStly">
                                <EmptyDataRowStyle ForeColor="Black" BackColor="AliceBlue" Height="5px" BorderColor="Black"
                                    BorderStyle="Solid" BorderWidth="2" Font-Bold="True" HorizontalAlign="Center"
                                    VerticalAlign="Middle" />
                            </asp:GridView>
                ...
                <tr>
                    <td align="center">
                        <asp:GridView ID="gvtotalorder" runat="server" Width="100%" ShowHeader="false" Font-Size="10pt"
                            ...
                            OnRowDataBound="GridView1_RowDataBound" BorderWidth="1px" CellPadding="2" EmptyDataText="No Data found for View"
                            AutoGenerateColumns="true" BackColor="#ffffe0" RowStyleCssClass="gridpager" HeaderStyle-HorizontalAlign="Center"
