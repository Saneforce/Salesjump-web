...
public partial class MasterFiles_Reports_DCR_Event_Captures_View : System.Web.UI.Page
{
    ...
    private void fillsubdivision()
    {
        ...
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ...
            ddlDivision.DataSource = dsSalesForce;
            ddlDivision.DataBind();
            ddlDivision.Items.Insert(0, new ListItem("--Select--", "0"));

        }
    }
    private void FillMRManagers(string SubDive)
    {
        SalesForce sf = new SalesForce();
        //ddlFFType.Visible = false;
        //ddlAlpha.Visible = false;
        dsSalesForce = sf.SalesForceList(div_code, sf_code, SubDive);
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ddlFieldForce.DataTextField = "sf_name";
            ddlFieldForce.DataValueField = "sf_code";
    ...
    {
        FillSF_Alpha();
        FillMRManagers(ddlDivision.SelectedValue.ToString());
        FillColor();
    }
    ...
    {
        SalesForce sf = new SalesForce();
        dsSalesForce = sf.getSalesForcelist_Alphabet(div_code, ddlDivision.SelectedValue.ToString());
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
    ...
    {
        SalesForce sf = new SalesForce();
        dsSalesForce = sf.SalesForceList(div_code, sf_code, ddlDivision.SelectedValue.ToString(), ddlAlpha.SelectedValue);
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ddlFieldForce.DataTextField = "sf_name";
            ddlFieldForce.DataValueField = "sf_code";
    ...
    protected void btnSubmit_Click1(object sender, EventArgs e)
    {
        try
        {
            ...
            string StartDate = Convert.ToDateTime(txtdate.Text).ToString("yyyy-MM-dd");
            string endDate = Convert.ToDateTime(txttdate.Text).ToString("yyyy-MM-dd");
            ...
            string sURL = "/MasterFiles/Reports/Rpt_DCR_Event_Captures_View.aspx?sf_code=" + ddlFieldForce.SelectedValue + "&div_code=" + div_code + "&cur_month=" + ddlMonth.SelectedValue + "&cur_year=" + ddlYear.SelectedItem.Text +
               "&Mode=" + rbnList.SelectedItem.Value + "&Sf_Name=" + ddlFieldForce.SelectedItem.Text + "&FDate=" + StartDate + "&TDate=" + endDate;
