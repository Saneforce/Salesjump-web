    ...
    private void FillSF_Leave_Type()
    {
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee;i++)
            {
                ...
                sURL = "rpt_secondary_sale_view_weekday.aspx?weeknumber=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&distributor=" + distri + "&dist_code=" + distributor + "&Subdivision=" + subdivision + "";
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                        ...
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
                                ...
                                lit_det_sale_value.Text = drrdoctor["saleqty"].ToString();
