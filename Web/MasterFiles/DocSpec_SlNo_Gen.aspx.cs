...
public partial class MasterFiles_DocSpec_SlNo_Gen : System.Web.UI.Page
{
    #region "Declaration"
    ...
    string Doc_Spec_Code = string.Empty;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            foreach (GridViewRow gridRow in grdDocCat.Rows)
            {
                TextBox txtSNo = (TextBox)gridRow.Cells[1].FindControl("txtSlNo");
                txtSlNo = txtSNo.Text;
                Label lDoc_Cat_Code = (Label)gridRow.Cells[0].FindControl("Doc_Spec_Code");
                Doc_Spec_Code = lDoc_Cat_Code.Text;

                // Update Division
                Doctor dv = new Doctor();
                int iReturn = dv.Update_DocSpecSno(Doc_Spec_Code, txtSlNo);
                if (iReturn > 0)
                {
