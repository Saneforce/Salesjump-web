...
public partial class MasterFiles_New_Territory_Detail : System.Web.UI.Page
{
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (Territory_Code != "" && Territory_Code != null)
            {
                ...
                if (dsTerritory.Tables[0].Rows.Count > 0)
                {
                    ...
                    hdnRoutecode.Value = dsTerritory.Tables[0].Rows[0].ItemArray.GetValue(0).ToString().Trim();
    ...
    public static string GetBindData(string TerritoryCode)
    {
        ...
        DataSet dsTerritory = sd.get_Territory(sf_code, Territory_Code, Division_code);
                    ...
                    if (Territory_Code == null)
                    {
                        ...
                        if (iReturn > 0)
                        {
                            ...
                            if (addfields.Count > 0)
                            {
                                ...
                                {
                                    if ((addfields[k].Fields == "" || addfields[k].Fields == null || addfields[k].Fields == "'undefined'" || addfields[k].Fields == "undefined"))
                                    {
                                    ...
                                    if ((addfields[k].Values == "" || addfields[k].Values == null || addfields[k].Values == "'undefined'" || addfields[k].Values == "undefined"))
                                    {
                        ...
                        {
                            msg = "Route Code Already Exist";
                            //ClientScript.RegisterStartupScript(GetType(), "Message", "<SCRIPT LANGUAGE='javascript'>alert('Route Code Already Exist');</script>");
                        }
                        else if (iReturn == -3)
                        {
                            msg = "Route Name Already Exist";
                    ...
                    else
                    {
                        ...
                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (addfields.Count > 0)
                            {
                                ...
                                //string colname = fld.ToString().TrimEnd(',');

                                //string[] cols = colname.Split(',');

                                //string colval = val.ToString().TrimEnd(',');

                                //string[] colvs = colval.Split(',');
                                //db_ER = new DB_EReporting();

                                //if (cols.Length > 0 && colvs.Length > 0)
                                //{
                                //    for (int k = 0; k < cols.Length && k < colvs.Length; k++)
                                //    {
                                //        string cn = Convert.ToString(cols[k]);
                                //        string cv = Convert.ToString(colvs[k]);

                                //        string uquery = "UPDATE Trans_Route_Custom_Field  SET " + cn + " = " + cv + " WHERE RouteID = " + Territory_Code + " ";
                                //        i = db_ER.ExecQry(uquery);
                                //    }
                                //}
                                if (i > 0)
    ...
    public class additionationalfld
    {
    ...
    public class RouteMainfld
    {
        ...
        public string min_prod { get; set; }

        public string Route_population { get; set; }
        ...
        public string route_town { get; set; }

        public string dstay { get; set; }

        public string DDL_aw_Type { get; set; }

        public List<additionationalfld> Additionsfld { get; set; }
    }
