...
public partial class MasterFiles_Options_SuperStockist_Sales_Upload : System.Web.UI.Page
{
    #region declaration
    ...
    protected void lnkDownload_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string fileName = Server.MapPath("~\\Document\\Superstockist_Sales_Value_Upload.xlsx");
            Response.AppendHeader("Content-Disposition", "attachment; filename=Superstockist_Sales_Value_Upload.xlsx");
            ...
            if (FlUploadcsv.HasFile)
            {
                ...
                if (extension == ".xls" || extension == ".xlsx")
                {
                    ...
                    using (OleDbConnection excel_con = new OleDbConnection(conString))
                    {
                        ...
                        squery += " [Item_ERP_Code],[Unit],[Pack],[Rate],[Item_Amount]";
                        squery += "  FROM [" + sheet1 + "]";
            ...
            for (int j = 0; j < Dt.Rows.Count; j++)
            {
                    ...
                    (columns[9].ToString() != "" && columns[9].ToString() != null))
                {
                    ...
                    using (SqlConnection con = new SqlConnection(Globals.ConnString))
                    {
                        ...
                        {
                            stk_Code = dslstst.Rows[0]["S_No"].ToString();
                            stk_Name = dslstst.Rows[0]["S_Name"].ToString();
                        }
                        else
                        {
                        ...
                        //DataTable dsltrans = dr.checktrans_slno(Convert.ToString(columns[0].Trim()));
                        //if (dsltrans.Rows.Count > 0)
                        //{
                        //    sMsg += "Unable to Upload Excel," + columns[0].ToString() + ". These Orderno has already entered</br>";
                        //}
                        //else
                        //{
                        //    trans_SlNo = dsltrans.Rows[0]["trans_SlNo"].ToString();
                        //}
                        ...
                        if (dslstst.Rows.Count > 0 && dslstprod.Rows.Count > 0)
                        {
                            try
                            {
                                ...
                                SqlCommand cmd = new SqlCommand("Insert_Superstockist_Sales_upload", con);
                                ...
                                new SqlParameter("@ERPItemCode", Convert.ToString(columns[5].Trim())),
                                new SqlParameter("@Unit", Convert.ToString(columns[6]).Trim()),
                                new SqlParameter("@Pack", Convert.ToString(columns[7]).Trim()),
                                new SqlParameter("@Rate", Convert.ToString(columns[8].Trim())),
                                new SqlParameter("@Sales_Value", Convert.ToDecimal(columns[9].Trim()))
                                };
                                ...
                                if (i > 0)
                                {
                                    sMsg += "" + i + "  Product Code :" + Convert.ToString(columns[4].ToString()) + " and Distributor Code : " + Convert.ToString(columns[2].Trim()) + " Uploaded Successfully..." + (j + 2) + "</br> ";
                                }
                                else if(i == -1)
                                {
                                    sMsg += "Unable to Upload:Excel Row No : " + (j + 1) + " OrderNumber already inserted... ";
                                }   
                            }
                            catch(Exception ex)
                            {
                                sMsg += "Unable to Upload:Excel Row No : " + (j + 1) + " OrderNumber already Used... ";
                            }
                ...
                else
                {
                    ...
                    sMsg += "Unable to Upload Product :Excel Row No : " + (j + 1) + " Has Empty Rows...</br>";
                }
            }
    ...
    public class lstdr
    {
        ...
        public DataTable checktrans_slno(string Contract_No)
        {
            DB_EReporting db = new DB_EReporting();
            DataTable dsAdmin = new DataTable();
            DataSet ds = db.Exec_DataSet("Select * from Dcrmain_Trans where Trans_SlNo='" + Contract_No + "'");
            dsAdmin = ds.Tables["Customers"];
            return dsAdmin;
        ...
        public DataTable Getstk_code(string stk_Name, string ERP_Code, string div_code)
        {
            ...
            string strQry = "SELECT TOP(1) * FROM Supplier_master Where  ";
            strQry += " Erp_Code=@ERP_Code   AND Act_flg=0 AND division_code = @Division_Code  ";
