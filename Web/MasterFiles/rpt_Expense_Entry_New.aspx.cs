...
public partial class MasterFiles_rpt_Expense_Entry_New : System.Web.UI.Page
{
	...
	[WebMethod]
    public static string Get_HotelBillExpense()
    {
        DataSet ds = null;
        string Divi_code = HttpContext.Current.Session["div_code"].ToString();
        DB_EReporting db_ElR = new DB_EReporting();
        string Qry = "select ('http://" + Url + "/photos/'+Isnull(imgurl,''))Img_Url,Activity_Report_Code,Convert(date,Dateandtime)Dateandtime from Activity_Event_Captures where Activity_Report_Code = '" + SF_Code + "' and Identification = 'BillAmount' and Month( Dateandtime)=" + Mn + " and Year(Dateandtime)=" + Yr + " and Division_Code='" + Divi_code + "'" +
    ...
    public static Allowances[] GetAllowanceDetails(string sf_code, string Mn, string Yr)
    {
        ...
        DataSet dsAlowtype = null;
        dsAlowtype = nt.GetAllowanceDets(sf_code,Mn,Yr);
    ...
    public static string ManualSfExpense(string SF, string Mn, string Yr)
    {
        ...
        if (Divi_code == "107")
        {
            ds = Exp.sfexpense_entry_period(SF, Mn, Yr, EmpID);
        }
        else
        {
            ds = Exp.sfexpense_entry(SF, Mn, Yr, EmpID);
        }
        return JsonConvert.SerializeObject(ds.Tables[0]);
    }

    [WebMethod]
    public static string rejectExpense(string SF, string Mn, string Yr, string RejDt, string Rtype, string sEmpID)
    {
        Expense Exp = new Expense();
        ...
        {
            msg = RejectExpense(SF, Mn, Yr, RejDt, Rtype, sEmpID, "");
        }
        return msg;
    }
