<%@ Page Language="C#" AutoEventWireup="true" CodeFile="DistributorDashbord.aspx.cs"
    Inherits="MasterFiles_DistributorDashbord" %>
...
<head runat="server">
 ...
 function popup(stcode, stname) {
		...
		$('.hidecll').closest('td').hide();
            $('.hidecell').hide();
            $('.hideclls').closest('td').hide();
            $('.hidecells').hide();

            if ('<%=Session["div_code"].ToString()%>' == '219') {
                $('.hidecell').show();
                $('.hidecll').closest('td').show();
				$('.hidecells').show();
                $('.hideclls').closest('td').show();
            }
...
<body>
        ...
        <div class="container" id="excelDiv" style="max-width: 100%; width: 98%">
            ...
            <asp:GridView ID="DistributorGrd" runat="server" class="newStly" AutoGenerateColumns="false">
                <Columns>
					...
					<asp:TemplateField HeaderText="State Name" HeaderStyle-CssClass="hidecells">
                        <ItemTemplate>
                            <asp:Label ID="Label9" runat="server" CssClass="hideclls" Text='<%# Eval("StateName") %>'>  </asp:Label>
                        </ItemTemplate>
                    </asp:TemplateField>
  ...
  <a href="javascript:popup(<%#Eval("Stockist_Code")%>,'<%# Eval("Stockist_Name")%>')">
					 ...
					 <asp:TemplateField HeaderText="Retailer Count " HeaderStyle-CssClass="hidecell">
                        <ItemTemplate>
                            <asp:Label ID="Label6" runat="server" CssClass="hidecll" Text='<%# Eval("retcnt") %>'>  </asp:Label>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
