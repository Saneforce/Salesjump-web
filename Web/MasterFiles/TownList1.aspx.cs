...
string sf_type = string.Empty;
    ...
    {
        Town dv = new Town();
        dsSubDiv = dv.TowngetSubDiv(divcode);
    ...
    private DataTable BindGridView()
    {
        ...
        Town dv = new Town();
        dtGrid = dv.TowngetSubDivisionlist_DataTable(divcode);
        return dtGrid;
    }
    ...
    protected void btnNew_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("TownCreation.aspx");
    }
    ...
    protected void grdSubDiv_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Deactivate")
        {
           // subdivcode = Convert.ToString(e.CommandArgument);
            ...
            //Deactivate
            Town dv = new Town();
            int iReturn = dv.TownDeActivate(subdivision_code);
            if (iReturn > 0)
            {                
    ...
    protected void grdSubDiv_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        Dist_cd = (grdSubDiv.Rows[e.RowIndex].FindControl("ddlState") as DropDownList).SelectedValue.ToString();
        Terr_Cd = (grdSubDiv.Rows[e.RowIndex].FindControl("ddlTerritory") as DropDownList).SelectedValue.ToString();
        Update(iIndex);
        FillSubdiv();
    }
    ...
    private void Update(int eIndex)
    {
        ...
        subdiv_name = txtSubDivName.Text;
         ...
         Dist_name = txt_State.SelectedItem.Text.Trim();
         DropDownList txt_Terr = (DropDownList)grdSubDiv.Rows[eIndex].Cells[5].FindControl("ddlTerritory");
         Terr_name = txt_Terr.SelectedItem.Text;
         if (Dist_name == "--Select--" && Terr_name == "--Select--")
       {
           ClientScript.RegisterStartupScript(GetType(), "Message", "<SCRIPT LANGUAGE='javascript'>alert('Please Select Dist Name');</script>");
       }
       else
       {
           // Update Sub Division
        ...
        protected void ExportToExcel(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=TownList.xls");
        Response.Charset = "";
    ...
    protected void grdSubDiv_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if ((e.Row.RowState & DataControlRowState.Edit) > 0)
        {
            ...
            if (ddlQual != null)
            {
                ...
                ddlQual.DataTextField = "Distname";
                ddlQual.DataValueField = "Dist_code";
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (State_Type != null)
                    {
                        ...
                        State_Type.SelectedIndex = State_Type.Items.IndexOf(State_Type.Items.FindByText(row["Dist"].ToString()));
        ...
        Town dv1 = new Town();
        dsDivision = dv1.getStatePerDivision(divcode);
        if (dsDivision.Tables[0].Rows.Count > 0)
        {
            //int i = 0;
            state_cd = string.Empty;
        ...
        Town s1 = new Town();
        dsState = s1.getterrProd(divcode);
        if ((e.Row.RowState & DataControlRowState.Edit) > 0)
        {

            DropDownList ddlQual = new DropDownList();
            ddlQual = (DropDownList)e.Row.FindControl("ddlTerritory");

            if (ddlQual != null)
            {
                ddlQual.DataSource = dsState;
                ddlQual.DataTextField = "Territory_Name";
                ddlQual.DataValueField = "Territory_Code";
                ddlQual.DataBind();
                if (e.Row.RowType == DataControlRowType.DataRow)
                {


                    DropDownList State_Type = (DropDownList)e.Row.FindControl("ddlTerritory");
                    if (State_Type != null)
                    {
                        DataRowView row = (DataRowView)e.Row.DataItem;

                        State_Type.SelectedIndex = State_Type.Items.IndexOf(State_Type.Items.FindByText(row["Territory_Name"].ToString()));
