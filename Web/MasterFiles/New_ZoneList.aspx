<%@ Page Language="C#" AutoEventWireup="true"  MasterPageFile="~/Master.master" CodeFile="New_ZoneList.aspx.cs" Inherits="MasterFiles_New_ZoneList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <form runat="server">
        <div class="row" style="margin-bottom: 1rem;">
           ...
           <div class="col-lg-12 sub-header">Zone Master<span style="float: right"><a href="ZoneCreation.aspx" class="btn btn-primary btn-update" id="newsf">Add Zone</a></span></div>
        </div>
                ...
                <table class="table table-hover" id="OrderList" style="font-size: 12px">
                    <thead class="text-warning">
                        <tr style="background-color: #37a4c6; color: #fff; white-space: nowrap;">
                            ...
                            <th style="text-align: left; color: #fff;">Zone Code</th>
                            <th style="text-align: left; color: #fff">Zone Name</th>
                            <th style="text-align: left; color: #fff">Area</th>
                            <th style="text-align: left; color: #fff">Territory Count</th>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "Zone_sname,Zone_name,Area,";
        var routes = [];
        var filtrkey = '0';
        ...
        function ReloadTable() {
            ...
            if (filtrkey != "All") {
                Orders = Orders.filter(function (a) {
                    return a.Zone_Active_Flag == filtrkey;
                })
            }
            st = PgRecords * (pgNo - 1);
            for ($i = st; $i < st + Number(PgRecords); $i++) {
                if ($i < Orders.length) {
                    ...
                    tr = $("<tr rname='" + Orders[$i].Zone_name + "' rocode='" + Orders[$i].Zone_code + "'></tr>");
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].Zone_sname + "</td><td>" + Orders[$i].Zone_name + "</td><td>" + Orders[$i].Area + "</td><td class='rocount'><a href='#'>" + Orders[$i].Sub_Coun + "</a></td><td class='roedit'><a href='#'>Edit</a></td><td class='rodeact'><a href='#'>" + Orders[$i].Status + "</a></td>");
        ...
        function fillzone() {
            $.ajax({
                ...
                async: false,
                url: "New_ZoneList.aspx/getfillzone",
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.roedit', function () {
                ...
                window.location.href = "ZoneCreation.aspx?Subdivision_Code=" + route_C + "";
            });
            $(document).on("click", ".rodeact", function () {
                var route_C = $(this).closest('tr').attr('rocode');
                let oindex = Orders.findIndex(x => x.Zone_code == route_C);
                let disstat = Orders[oindex]["Status"].toString();
                let flg = (parseInt(Orders[oindex]["Zone_Active_Flag"]) == 0) ? 1 : 0;
                ...
                if ((rocnt < 1 && flg == 1) || (flg == 0)) {
                    if (confirm("Do you want " + disstat + " the Distributor ?")) {
                        $.ajax({
                            ...
                            contentType: "application/json; charset=utf-8",
                            async: false,
                            url: "New_ZoneList.aspx/deactivatezone",
                            data: "{'zncode':'" + route_C + "','stat':'" + flg + "'}",
                            dataType: "json",
                            success: function (data) {
                                if (data.d == 'Success') {
                                    Orders[oindex]["Zone_Active_Flag"] = flg;
