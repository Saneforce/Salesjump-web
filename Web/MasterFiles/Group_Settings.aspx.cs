...
public partial class MasterFiles_Group_Setting : System.Web.UI.Page
{
    ...
    public static string GetCurrency()
    {
        ...
        string result = "";
        string strQry = "select Curr_Symbol Curr_Symbol,Symbol_name from Master..Mas_Currency where ActiveFlag=0";
        try
        {
    ...
    public static string GetData_colrow(string Unpivot)
    {
        ...
        {
            castvar += "ISNULL(CAST(" + q[i] + " AS NVARCHAR(50)),'') AS " + q[i] + ",";
        }
        castvar = castvar.Remove(castvar.Length - 1, 1);
        DataSet ds = null;
        DB_EReporting db_ElR = new DB_EReporting();
        str = "Select u.Field_Name,N'' +ISNULL(u.Value,'') AS Value from(select " + castvar + " from Access_Master where Division_Code = '" + divcode + "') tt" +
    ...
    [WebMethod]
    public static string UpdateByDefault(string FieldNm, string FieldVal)
    {        
        DB_EReporting db = new DB_EReporting();
        string strQry = "UPDATE Access_Master SET " + FieldNm + "=" + ((FieldNm == "Currency_Symbol") ? "N" : "") + "'" + FieldVal + "' WHERE Division_Code='" + divcode + "'";
    ...
    public string SubmitValues(string data)
    {
        ...
        try
        {
            ...
            else
            {
                for (int i = 0; i < str.Rows.Count; i++)
                {
                    strQry = "UPDATE Access_Master SET " + str.Rows[i]["Field_Name"] + "=" + ((str.Rows[i]["Field_Name"].ToString() == "Currency_Symbol") ? "N" : "") + "'" + str.Rows[i]["Value"] + "' WHERE Division_Code='" + divcode + "'";
                    iReturn = db.ExecQry(strQry);
                }
