...
public partial class MasterFiles_Expense_Approval : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static FieldForce_Details[] Get_FieldForce(string exp_years, string exp_month,string sfcode)
    {
	DB_EReporting db_ER = new DB_EReporting();
	...
	//	if (sfcode == "")
        ...
        //else
        //{
         //   dsDetails = nt.getSalesForce_Fare(Div_code, sfcode, exp_mode, exp_years, exp_month, Sf_Type);
        //}
	string sf = (sfcode != "") ? sfcode : Sf_Code;
	string strQry ="";
	//if(Div_code=="100")
	//strQry = "exec getExpenseDet_Div_Wise '" + sf + "'," + exp_month + "," + exp_years + ",'" + Div_code + "'";
	//else
	if (Global.ExpenseType == "2")
		strQry = "exec getExpenseFare '" + sf + "'," + exp_month + "," + exp_years + ",'" + Div_code + "'";
	else
		strQry = "exec getExpenseDet_Div_Wise '" + sf + "'," + exp_month + "," + exp_years + ",'" + Div_code + "'";
        try
        {
