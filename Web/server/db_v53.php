...
function addEntry() {
    ...
    switch ($temp[0]) {
        ...
        case "unlisted_doctor_master":
			...
			$sql = "{call  svListedDR_APP('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . ",'" . str_replace("'","",$vals['unlisted_doctor_cityname']) . "','" . str_replace("'","",$vals['unlisted_doctor_areaname']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson']) . "', ' " . str_replace("'","",$vals['unlisted_doctor_designation']) . "','" . str_replace("'","",$vals['unlisted_doctor_gst']) . "','" . str_replace("'","",$vals['unlisted_doctor_pincode']) . "'," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ",'" . str_replace("'","",$vals['unlisted_doctor_phone2']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_designation2']) . "','" . str_replace("'","",$vals['unlisted_doctor_landmark']) . "','" . str_replace("'","",$Ukey) . "','','','')}";
      		 performQueryWP($sql,[]);
...
case "DCRMissedDates":
	$sfCode = $_GET['sfCode'];
        ...
        $Owndiv = (string) $divs[0];
             ...
             $sql = "  update  DCR_MissedDates  set Status='2' where  Dcr_Missed_Date='".$vals['misseddate']."'  and   Sf_Code='" . $sfCode . "' and Division_Code='" . $Owndiv . "'";
            performQuery($sql);
            $resp["success"] = true;
            echo json_encode($resp);
            die;
    ...
    $result = array();
             $query = "select * from Trans_PriOrder_Head where Division_Code='". str_replace(",", "",$divCode)."' and OrderID='".$CallPreviewEkey."'";
...
/*                if ($ARDCd == "Exists") {
				...
				if($vTyp == 3){
		            $sqlOrd = "{call svPriOrder(?,$cCode,?," . $POB_Value . ",'" . $DetTB["intrumenttype"] . "','" . $DetTB["date_of_intrument"] . "'," . $pob . ",'" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,?,?,'".$eKeyID."','".$POT."')}";
		            $params1 = array(
						array($sfCode, SQLSRV_PARAM_IN),
		                array($today, SQLSRV_PARAM_IN),
			...
			if( $data[count($data)-2]["Activity_Event_Captures"]!=null && $data[count($data)-2]["Activity_Event_Captures"]!="undefined")
			{
				...
				for ($j = 0; $j < count($Event_Captures); $j++) {	                	
					...
					$sql = "insert into Activity_Event_Captures(Activity_Report_Code,DCRDetNo,imgurl,title,remarks,Division_Code,Identification,Insert_Date_Time) select '" . $ARCd . "','" . $ARDCd . "'," . $ev_imgurl . "," . $ev_title . "," . $ev_remarks . ",'" . $Owndiv . "' ,'DCR','".date('Y-m-d')."' ";
		            performQuery($sql);	
				}
			}
            $townCode = $data[0]['Activity_Report_APP']['Town_code'];
...
break;
 ...
 case "pendingbils":
	    ...
	    $query = "select * from ((select Cate_Code,product_detail_code,discount,rp_base_rate,rp_case_rate,mrp_rate,effective_from_date  from mas_product_category_rates  where division_code='" . $divisionCode . "') union (select 0 Cate_Code,product_detail_code,'0' discount,cast(Retailor_Price as varchar) rp_base_rate,cast(RetailCasePrice as varchar) rp_case_rate,cast(MRP_Price as varchar) mrp_rate,getdate() effective_from_date from vwProductStateRates where division_code='" . $divisionCode . "' and State_Code='".$state."')) as stb";
        ...
        if (count($productcategoryrates) > 0) {
		 ...
		 $result['ProductCategoryRates'] = $ProductCategoryRates;
            ...
            $stateCode = $_GET['State_Code'];
                ...
                $query = "select  TOP 1*  from vwTourPlan where SF_Code='$sfCode' and Tmonth=$CMn and Tyear=$CYr and Confirmed=1";
                $results = performQuery($query);
                outputJSON($results);
                die;
                break;
