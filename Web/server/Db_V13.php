		    ...
		    $respon['TP_ND'] = $arr[0]['TP_ND'];   
			 $respon['TPDCR_Deviation'] = $arr[0]['TPDCR_Deviation'];  
			  $respon['mobile_no_caption'] = $arr[0]['mobile_no_caption']; 
			   ...
			   $respon['NotifiGiftNeed'] = $arr[0]['NotifiGiftNeed'];
...
function getDtTPview() {
	...
	if( $sfCode=="MR3625")
		 $Qry = "exec spTPViewDtws_1 '$sfCode','$TpDt'";
		else
...
function getTPview() {
    ...
    $TpDt = date('Y-m-d 00:00:00', $t);
	    $sql = "select sf_emp_id,Employee_Id from Mas_Salesforce where Sf_Code='$sfCode'";
			...
			if($sfCode=="MR3625"){
			 $Qry = "SELECT convert(varchar,Tour_Date,103) [date],Worktype_Name_B wtype,replace(isnull(Territory_Code1,''),'0','') towns,replace(isnull(Tour_Schedule1,''),'0','') townsId,replace(isnull(Tour_Schedule1,''),'0','') PlnNo,replace(isnull(Worked_With_SF_Name,''),'0','') distributor,Confirmed,SF_Code sf_code from vwTrans_TP_View T where sf_code='$sfCode' and Tour_Month=month('$TpDt') and Territory_Code2='$empid' and Tour_year=year('$TpDt') order by Tour_Date";
}
else{
...
$Month=$now->format("M");
		...
		$query="select top 2 trans_sl_no,dcr_code,order_value,CONVERT(VARCHAR(10), order_date, 105) visit_date,order_date from trans_order_head  where sf_code='".$SF."' and Cust_Code='".$MSL."'    order by Order_Date desc";
		 ...
		 $LAST2ORDER=performQuery($query);
			 ...
			 $sql="select Product_Code,Product_Name,Quantity,value from Trans_Order_Details det
    ...
    switch ($temp[0]) {
        case "stockUpdation":
			...
			$date = date('Y-m-d H:i:s',strtotime($_GET['Selectdate']));
				...
				if($Owndiv=="4"&&$_GET['Selectdate']!=null && $_GET['Selectdate']!="undefined"){
...
case "SSstockUpdation":
        ...
        case "tbMyDayPlan":
			...
			if($arr[0]["Cnt"]>0){
			 ...
			 $todayyy=str_replace("'", "", $vals["dcr_activity_date"]);
			 $query="select * from mas_Leave_Form where sf_code='$sfCode' and cast('$todayyy' as date) between From_Date and To_Date and Leave_Active_Flag=0";
			 $tr=performQuery($query);
			 
if(count($tr)>0&&($Owndiv=="4"||$Owndiv=="29")){
				$result['msg'] = "Already Leave Submitted...";
				$result["success"] = false;
				outputJSON($result);
				die;
			}
...
if($vals['Retailerphoto']!=null && $vals['Retailerphoto']!="undefined")
        ...
        //}
         ...
         $ListedDr_Active_Flag=0;
            $sqls="SELECT count(RetailerApprv) cnt from Access_Table where sf_code='$sfCode' and RetailerApprv=1";
			$rs= performQuery($sqls);
			if($rs[0]["cnt"]>0){
			  $ListedDr_Active_Flag=3;
			}
		   $div = $_GET['divisionCode'];
...
case "Map_GEO_Distributor":
			...
			$val =$data[3][$temppp[0]];
			if($val["WorkType"]=="Leave"||$val["WorkType"]=="leave"){
			...
			if($divCode=="29,"||$divCode=="4,"||$divCode=="100,"){
							$entryDate=$val["Enterdate"];
	 $sql="select sf_emp_id from Mas_Salesforce where Sf_Code='" . $sfCode . "'";
		 	$empId = performQuery($sql);
					 	       $empcode=$empId[0]['sf_emp_id'];
				$sql="select Trans_Sl_No from trans_Expense_head1 where sf_code='$sfCode' and expense_month=month('$entryDate') and expense_year=year('$entryDate')
				and (approve_flag=2 or approve_flag=3 or approve_flag=1) and Applied_ExpAmnt!='0'";
				//print_r($sql);die;
		$tr=performQuery($sql);
		if(count($tr)>0){
				$entryDate=$val["Enterdate"];
			$Trans_Sl_No=$tr[0]['Trans_Sl_No'];
			$sql="select * from trans_expense_detail1 where trans_dt_slno=$Trans_Sl_No and expense_date='$entryDate'";
			$det=performQuery($sql);

$sql="select * from Trans_Rejected_Exp where trans_dt_slno=$Trans_Sl_No and rexpense_date='$entryDate'";
	$rej=performQuery($sql);
	if(count($det)>0||(count($det)==0&&count($rej)==0)){
			...
			//else if(count($det)==0&&count($rej)==0)
		}
			}
		...
		if(count($tr)>0){
				...
				$resp["msg"]= "Expense Already Submitted for Approval";
				$resp["Query"]= $sql;
				$resp["success"] = true;
				echo json_encode($resp);
				die;
 ...
 if($divCode!="100,"&&$divCode!="29,"&&$divCode!="4,"){
			$entryDate=$val["Enterdate"];
			$sql="select * from (select *,case when (select subdivision_code from mas_salesforce where sf_code=aa.Sf_Code)=(select subdivision_code from mas_salesforce where sf_code=aa.SfCode) then closing_stock else 0 end closingstock
			from (select * from (select stockiest_code,stockiest_name,count(cls.Stockist_code) closing_stock,sales.division_code,S.SF_Code,cls.SfCode from vwstockiest_Master_APP S
			inner join mas_salesforce sales on sales.sf_code=S.sf_code
			left join Trans_Stock_Updation_Details cls on S.stockiest_code=cls.Stockist_code  and month(Purchase_Date)=month(dateadd(month,1,'$entryDate')) and year(purchase_date)=year(dateadd(month,1,'$entryDate'))
			where isnull(Stockist_Status,0)=0 and sales.division_code='" . $divCode . "' and stock_maintain=1
			group by S.stockiest_code,cls.Stockist_code,stockiest_name,sales.division_code,S.SF_Code,cls.SfCode) t) aa where SF_Code='" . $sfCode . "') aaa  where closingstock=0";

			$tr= performQuery($sql);
			$stockist_name="";
			for ($j = 0; $j < count($tr); $j++) {
				$stockist_name=$stockist_name.$tr[$j]['stockiest_name'].",";
			}
			if(count($tr)>0){
				$resp["msg"]= "Please Update Closing Stock for ".$stockist_name;
				$resp["Query"]= $sql;
				$resp["success"] = true;
				echo json_encode($resp);
				die;
 ...
 $mode_of_travel=$_GET['mode_of_travel'];
			if($mode_of_travel=="1"&&$divCode=="100,"){
if($val["RejectType"]=="1"){
 $sql = "exec RejectExpense_hybrid '" . $sfCode . "' ,'" . $val["Enterdate"]."'";
			   performQuery($sql);
}}
			if($val['Remarks']==null)
              $Remarks='';
		...
		//	mot_id,StEndNeed,MaxKm,Fuel_Charge,Expense_Km,Expense_Amount
				if($val['mot_id']==null){
              $mot_id='';
		}else{
		$mot_id=$val['mot_id'];
			}
				if($val['StEndNeed']==null){
              $StEndNeed='';
		}else{
		$StEndNeed=$val['StEndNeed'];
			}
				if($val['MaxKm']==null){
              $MaxKm='';
		}else{
		$MaxKm=$val['MaxKm'];
				...
				if($val['Fuel_Charge']==null){
              $Fuel_Charge='';
		}else{
		$Fuel_Charge=$val['Fuel_Charge'];
			}
				if($val['Expense_Km']==null){
              $Expense_Km='';
		}else{
		$Expense_Km=$val['Expense_Km'];
			}
				if($val['Expense_Amount']==null){
              $Expense_Amount='';
		}else{
		$Expense_Amount=$val['Expense_Amount'];
			}
					if($val['mot_name']==null){
              $mot_name=$val["MOT"];
		}else{
		$mot_name=$val['mot_name'];
			}
			$debug="0";
			$mode_of_travel=$_GET['mode_of_travel'];
			if($mode_of_travel=="1"){

				  $sql ="exec Post_Expense_Vasu '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$mot_name."','".$Billamount."','".$stayingtype."','".$debug."','".$mot_id."','".$StEndNeed."','".$MaxKm."','".$Fuel_Charge."','".$Expense_Km."','".$Expense_Amount."'";
			}
			else{
				if($divCode=="100,"){
				$resp["msg"]= "Please download new app then submit expense";
				$resp["Query"]= $sql;
				$resp["success"] = true;
				echo json_encode($resp);
				die;
			}
  $sql ="exec Post_Expense '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."','".$mot_name."','".$Billamount."','".$stayingtype."','".$debug."','".$mot_id."','".$StEndNeed."','".$MaxKm."','".$Fuel_Charge."','".$Expense_Km."','".$Expense_Amount."'";
			}
           $result=performQuery($sql);
           $resp["msg"]= $result[0]['msg'];
           $resp["Query"]= $sql;
           $resp["success"] = true;
		   if( $result[0]['msg']!="Expense Submitted Successfully"&&$divCode=="4,"){
			     echo json_encode($resp);
            die;
		   }
			for ($i = 0; $i < count($vals); $i++)
...
$resp["Query"] = $Trans_TP_One;
 ...
 $sql = "insert into Trans_TP_Hist(SF_Code,Tour_Month,Tour_Year,Submission_date,Tour_Date,WorkType_Code_B,Worktype_Name_B,Tour_Schedule1,Objective,Worked_With_SF_Code,Division_Code,Territory_Code1,Worked_With_SF_Name,TP_Sf_Name,HQ_Code,HQ_Name,Confirmed,Change_Status,JointWork_Name,Retailer_Code,Retailer_Name,Distributor_Name,Distributor_Code,TPOB,TSOB) select '" . $sfCode . "',MONTH($tourDate),YEAR($tourDate),getdate(),$tourDate,$worktype_code,$worktype_name,$RouteCode,$objective,$worked_with_code," . $divisionCode[0] . ",$RouteName,$worked_with_name,$sfName,$HQ_Code,$HQ_Name,0,0,$ww,$RCode,$RName ,$DName,$DCode,$POB,$SOB";
		...
		/*	if( $divCode=="4,"){
                ...
                $date = $vals['From_Date'];
                for ($i = 1; $i <= $days; $i++) {
                    $query = "exec svTransToTempDCRByDt '$sfCode','$date'," . $sfType[0]['sf_type'] . ",$Owndiv";
                    $results = performQuery($query);
            ...
            $date = $date[2] . "-" . $date[1] . "-" . $date[0];
			  for ($i = 1; $i <= $days; $i++) {
				if($Owndiv=="4"){
				$query = "exec svTransToTempDCRByDt '" . $vals['Sf_Code'] . "','$date'," . $sfType[0]['sf_type'] . ",$Owndiv";
                    $results = performQuery($query);
				}
...
//if($SFCd!="MR2620"){
 ...
 /* $myfile = fopen("../server/vanstock.txt", "a+");
        ...
        case "Inshop_Activity":
			...
			$mQry="select isnull(max(cast(replace(InshopKey,'".$sfCode."-','') as numeric)),0)+1 MaxSl from Trans_Inshop_Activity where sfCode='".$sfCode."'";
			$mxRes=performQuery($mQry);
			$Inskey=$sfCode . "-" . $mxRes[0]["MaxSl"];
            for ($il = 0;$il < count($data[0]['Inshop_Activity']);$il++) {
                ...
                $sql="insert into Trans_Inshop_Activity (sfCode, SfName, State_Code, Division_Code, Promotorname,issue,starttime,endtime,Remarks,Retailer_Name,Distributors,Route,InsertDate,InshopKey)
                        values (  '".$sfCode."','".$FWFlg."', '".$State_Code."','". str_replace(",", "",$divCode)."','".$PromotorName."','".$Issue."','".$Stime."','".$Etime."','".$Remarks."','".$retailername."','".$Distributors."','".$Route."','" . $Current_Date_And_only . "','" . $Inskey . "')";
                performQuery($sql);
            ...
            for ($il = 0;$il < count($data[4]['Inshop_Activity_Orders']);$il++) {
                ...
                $sql="insert into Trans_Inshop_Activity_Orders (sfCode, SfName, State_Code, Division_Code, product_code,Product_Rx_Qty,Product_Sample_Qty,Rate,Insert_Date_And_time,Retailer_Name,InshopKey)
                        values (  '".$sfCode."','".$FWFlg."', '".$State_Code."','". str_replace(",", "",$divCode)."','".$product_code."','".$Product_Rx_Qty."','".$Product_Sample_Qty."','".$Rate."','" . $Current_Date_And_Time . "','" . $retailername . "','" . $Inskey . "')";
                performQuery($sql);
            }
            break;
...
if(count($tr)>0&&($divisionCode=="4"||$divisionCode=="29")){
				$result['msg'] = "Already Leave Submitted...";
				 $result['type'] = 2;
			//	$result["success"] = true;
				outputJSON($result);
...
where sfcode= '" . $sfCode . "' and convert(date,EntryDt)='" . $today . "' and Prod_Code='" . $transOrderDetails[$i]['product_code'] . "' ";
						...
						//$sql = "update Trans_Order_Head set Order_Value=((select sum(value) from Trans_Order_Details with(nolock) where Trans_Sl_No=Trans_Order_Head.Trans_Sl_No and Div_ID=Trans_Order_Head.Div_ID)-'$discount_price') where  Trans_Sl_No='" . $pk . "' and Div_ID='$Owndiv'";
						//$resp["HeadQryy"]=$today;
						//performQuery($sql);				
						//if($sfCode=="MR3532"){
						//sendSMS();
    ...
    case "get/doctorCount":
	 ...
	 $state = $_GET['State_Code'];
			$div = $_GET['divisionCode'];
				...
				$Owndiv = (string) $divs[0];
		$date = $_GET['date'];
$sql="select sl_no id,MOT name,* from Mas_Modeof_Travel where division_code=$Owndiv and isnull(Active_Flag,0)=0";
		$mot=performQuery($sql);
			$sql="select CONVERT(VARCHAR(10),max(Effective_Date),126) Effective_Date from Mas_Statewise_FuelCharge f
inner join mas_salesforce s on s.State_Code=f.StateCode and s.Designation_Code=f.Design_code
where sf_code='$sf' and cast(Effective_Date as date)<=cast('$date' as date)";
$maxdate=performQuery($sql);
if(count($maxdate)>0){
$maxdate1=$maxdate[0]['Effective_Date'];

		$sql="select f.* from Mas_Statewise_FuelCharge f
inner join mas_salesforce s on s.State_Code=f.StateCode and s.Designation_Code=f.Design_code
where sf_code='$sf' and (cast(Effective_Date as date)=cast('$maxdate1' as date) or (mot_id=7 and cast(Effective_Date as date)=cast('2022-12-01' as date)))";
		$fuel=performQuery($sql);
}
else{
	$fuel=array();
}
		...
		$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code=1";
		 else if($divisionCode!="1,"&&$divisionCode!="107,")
			$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code=29";
		$det=performQuery($sql);
		outputJSON($det);
		break;
    case "get/setup":
        ...
        if (count($productcategoryrates) > 0) {
         ...
         $result['ProductCategoryRates'] = $ProductCategoryRates;
            ...
            case "vwlastupdationdate":
                if ($data['desig'] == stockist) {
                    $query = "select convert(varchar,Last_Updation_Date,103) Last_Updation_Date from Trans_Current_Stock_Details where Stockist_code= '" . $sfCode . "' group by Last_Updation_Date,Stockist_code order by Last_Updation_Date";
                } else {
                    ...
                    where tab.SF_Code='$sfCode' group by c.Last_Updation_Date,c.Stockist_code order by c.Last_Updation_Date";
                }
                $results = performQuery($query);
                break;
...
$sfCode=$_GET['rSF'];
   ...
   $sql = "select sf_emp_id,Employee_Id from Mas_Salesforce where Sf_Code='$sfCode'";
            ...
            $empid = $sf[0]['sf_emp_id'];
			if($sfCode=="MR3625"){
				 $query = "select  * from vwTourPlan t inner join Mas_Salesforce S on S.Sf_Code=t.sf_code 
 where t.SF_Code='$sfCode' and Territory_Code2='$empid' and t.Tmonth=$CMn and t.Tyear=$CYr and cast(S.sf_TP_Active_Dt as date)<=cast(t.date as date) order by worktype_name";
			}
			else{
                $query = "select  * from vwTourPlan t inner join Mas_Salesforce S on S.Sf_Code=t.sf_code 
 where t.SF_Code='$sfCode' and t.Tmonth=$CMn and t.Tyear=$CYr and cast(S.sf_TP_Active_Dt as date)<=cast(t.date as date) order by worktype_name";
			}
                $results = performQuery($query);
...
$divisionCode=$_GET['divisionCode'];
	     ...
	     $query = "select  TOP 1 convert(varchar,cast(convert(varchar,Tour_Date,101)  as date),20) as date,Objective remarks,SF_Code,WorkType_Code_B worktype_code,Worktype_Name_B worktype_name,Tour_Schedule1 RouteCode,Territory_Code1 RouteRouteNameCode,Territory_Code1 RouteName,Worked_With_SF_Code Worked_with_Code,Worked_With_SF_Name Worked_with_Name,Confirmed,HQ_Code,HQ_Name,JointWork_Name,MONTH(Tour_Date) Tmonth,YEAR(Tour_Date) Tyear,Retailer_Code,Retailer_Name,isnull(Place_Inv,'')Place_Inv from Trans_TP where SF_Code='$sfCode' and cast(tour_date as date)=cast(getdate() as date) union
		 select  TOP 1 convert(varchar,cast(convert(varchar,Tour_Date,101)  as date),20) as date,Objective remarks,SF_Code,WorkType_Code_B worktype_code,Worktype_Name_B worktype_name,Tour_Schedule1 RouteCode,Territory_Code1 RouteRouteNameCode,Territory_Code1 RouteName,Worked_With_SF_Code Worked_with_Code,Worked_With_SF_Name Worked_with_Name,Confirmed,HQ_Code,HQ_Name,JointWork_Name,MONTH(Tour_Date) Tmonth,YEAR(Tour_Date) Tyear,Retailer_Code,Retailer_Name,isnull(Place_Inv,'')Place_Inv from Trans_TP_One where SF_Code='$sfCode' and cast(tour_date as date)=cast(getdate() as date) and Confirmed=1";
         $results = performQuery($query);
...
$divisionCode=$_GET['divisionCode'];
	     ...
	     $query = "select  TOP 1*  from Trans_TP where SF_Code='$sfCode' and Tour_Month=$CMn and Tour_Year=$CYr  and 
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' UNION
               select  TOP 1*  from Trans_TP_One where SF_Code='$sfCode' and Tour_Month=$CMn and Tour_Year=$CYr and Confirmed=1 and 
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";

                $results = performQuery($query);
				 $query = "select  TOP 1*  from Trans_TP where SF_Code='$sfCode' and Tour_Month=$PMn and Tour_Year=$PYr  and 
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' UNION
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
    ...
    case "vwCheckLeave":
        ...
        $disableDates = array();
        $sql = "SELECT * FROM vwActivity_Report where SF_Code='" . $sfCode . "' and cast(activity_date as date)=cast('$date' as date)";
        $dcrEntry = performQuery($sql);
        if (count($dcrEntry) > 0){
			if($dcrEntry[0]['FWFlg']=='F'){
				 $sql = "select * from vwActivity_MSL_Details where Trans_SlNo='".$dcrEntry[0]['Trans_SlNo']."'";
$msl=performQuery($sql);
$sql = "select * from vwActivity_CSH_Detail where Trans_SlNo='".$dcrEntry[0]['Trans_SlNo']."'";

$csh=performQuery($sql);

			}
			if(count($msl)==0&&count($csh)==0){
			
				 $sql = "delete from dcrmain_trans where SF_Code='" . $sfCode . "' and cast(activity_date as date)=cast('$date' as date)";
				
         performQuery($sql);
           
			}
			else{
				if( $divisionCode!="29,"&&$divisionCode!="4,")
   ...
   where (reporting_to_sf='$sfCode' or sf_code='$sfCode') and monthnum=$Month and year=$Year";
	   ...
	   break;
        ...
        $Owndiv = (string) $divs[0];
		 ...
		 if($Owndiv=="107"){
			...
			//$sql = "exec KtvGet_Expense_Details '" . $sfCode . "' ,'" . $EMonth . "' ,'" . $EYear . "','".$Owndiv."' ";
			 ...
			 $sql = "select isnull(Applied_ExpAmnt,0) Applied_ExpAmnt,Expense_Amt,trans_sl_no from   trans_expense_head1  where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'  and Emp_ID='" . $empcode . "'";
		...
		$Total = performQuery($sql);
		 ...
		 $appliedAmount=$Total[0]['Applied_ExpAmnt'];
		  $expAmount=$Total[0]['Expense_Amt'];
       $trans_sl_no=$Total[0]['trans_sl_no'];
		if($Owndiv=="109"||$Owndiv=="100"||$Owndiv=="29"){
 ...
 $sql="select cast(Add_Sub+Exp_Amnt as numeric) amt from expense_additional where transh_exp='$trans_sl_no'";
		...
		/* if($sfCode=="vasuMR0002"){
		 ...
		 if(count($val)>0){
			   ...
			   $expAmttt=$Total[0]['Applied_ExpAmnt'];
		  //$amtt=$val[0]['amt'];
		  //$Total[0]['Expense_Amt']=$Total[0]['Applied_ExpAmnt']+$amtt;
		   for($i=0;$i<count($val);$i++){
		  $amtt=$val[$i]['amt'];
		  $expAmttt= $expAmttt+$amtt;
	
		}	
		$Total[0]['Expense_Amt']=$expAmttt;
		 }
		 else
			  $Total[0]['Expense_Amt']=$Total[0]['Applied_ExpAmnt'];
		  if($Owndiv=="100"){
		  $Total[0]['Applied_ExpAmnt']=$Total[0]['Applied_ExpAmnt'];
		   $Total[0]['Expense_Amt']=  $expAmount;
...
//	print_r($sql);
    ...
    case "get/precall":
	 $divCode = $_GET['divisionCode'];
	  if( $divCode=="40000000," or  $divCode=="400000000")
        getPreCallDetTSR();
	else
        getPreCallDet();
        break;
...
$result=performQuery($sql);
 outputJSON($result);
        break;
		 case "get/precall_month_ret":
        $msl=$_GET['Msl_No'];
		$year=$_GET['year'];
		$month=$_GET['month'];
		$sql="select OrderID,Convert(varchar(10),CONVERT(date,order_date,106),103) date,order_date,order_value,trans_sl_no from trans_order_head  where  Cust_Code='$msl' and  month(order_date) ='$month'  and  year(order_date) = '$year'
		order by order_date desc";
$result=performQuery($sql);
...
/*if($sfCode=="MR3532"){
    ...
    $div = $_GET['divisionCode'];
		...
		//if($sfCode=="MR6071")
			//  $result['today']['RCCOUNT'] =  $rccount[0]['cnt']+38;
	 ...
	 $targetquery = "exec Get_FieldForce_Target_vs_Sal '" . $sfCode . "', '" . date("Y") . "','" .date('m') . "', '" .  date("Y") . "','" . date('m') . "' ,  '" . $Owndiv . "' ";
		...
		if($division=="100,"){
			$query="select TransFTslno slno from Trans_Field_Target_head_Primary with(nolock) where sf_code='$sfCode' and year='" . date("Y") . "'";
			$primaryTargetYear = performQuery($query);
			if(count($primaryTargetYear)>0){
				$slno=$primaryTargetYear[0]['slno'];
			$query="select Target_Value target from Trans_Field_Target_details_Primary with(nolock) where month='" .date('m') . "' and TransFTslno='$slno'";
 ...
 case "get/Deactivate":
    ...
    case "sv/expenseApproval":
            $sfCode = $_GET['rSF'];
		 	...
		 	$empId = performQuery($sql);
		 	       $empcode=$empId[0]['sf_emp_id'];
$sql = "select * from trans_expense_head1  where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'  and Approve_Flag='-1'";
	$tr= performQuery($sql);
$transslno=$tr[0]['Trans_Sl_No'];
$sql="update Trans_Expense_Detail1 set Reject_Type=2 where trans_dt_slno=$transslno";
 performQuery($sql);
 $sql="select top 1 Appr_SF from Approval_Hyr_Head Ahh WITH(Nolock)                  
        inner join Approval_Hyr_Detail Ahd WITH(Nolock) on Ahh.Slno=Ahd.FKSlno and sf_code='" . $sfCode . "' order by Appr_Lvl";
        $repTo= performQuery($sql);
        $repToSf=$repTo[0]['Appr_SF'];
$sql = "update trans_expense_head1 set Approve_Flag='0',Appr_By='" . $repToSf . "',Approved_By=''  where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'  and Approve_Flag='-1'";
	 performQuery($sql);
        ...
        echo json_encode($resp);
			  die;
        break;
 case "get/calenderdetails":
            $sfCode = $_GET['rSF'];
			$divisionCode = $_GET['divisionCode'];
			/*if($divisionCode=="29,"||$sfCode=="vasuMR0039"||$sfCode=="vasuMR0036"||$sfCode=="vasuMR0037"||$sfCode=="vasuMR0038"||$sfCode=="vasuMR0040"||$sfCode=="vasuMR0050"||$sfCode=="vasuMR0041"||$sfCode=="vasuMR0042"||$sfCode=="vasuMR0043"||$sfCode=="vasuMR0049"){
				$query="select month(getdate()) month1,3 month,31 no_of_days,year(getdate()) year1,2023 year,CONVERT(VARCHAR(10),getdate(),126) cdate,'2023-3-10' approvalopendate,'2023-4-31' approvalenddate";
			}
else if($sfCode=="MR4366" or $sfCode=="vasuMR0051"){
 $query="select month(getdate()) month1,4 month,30 no_of_days,year(getdate()) year1,2023 year,CONVERT(VARCHAR(10),getdate(),126) cdate,'2023-04-1' approvalopendate,'2023-4-27' approvalenddate";


}
			else{*/
         $query="select month(getdate()) month1,5 month,31 no_of_days,year(getdate()) year1,2023 year,CONVERT(VARCHAR(10),getdate(),126) cdate,'2023-06-1' approvalopendate,'2023-6-30' approvalenddate";
			//}
         $month_year = performQuery($query);
		 $EMonth=$month_year[0]['month'];
		 $EYear=$month_year[0]['year'];
		 $sql="select sf_emp_id from Mas_Salesforce where Sf_Code='" . $sfCode . "'";
		 	$empId = performQuery($sql);
					 	       $empcode=$empId[0]['sf_emp_id'];

		 $sql = "select * from trans_expense_head1 where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'";
	 $tr=performQuery($sql);
 if(count($tr)==0)
 $resp["monthwiseView"] = "101";
	 if(count($tr)>0){

		 $sql = "select * from trans_expense_head1 where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "' ";
	 $tr=performQuery($sql);
 if($tr[0]['Approve_Flag']=="-1")
		 $resp["monthwiseView"] = "101";
else
$resp["monthwiseView"] = $tr[0]['Approve_Flag'];
	 }
		 $sql = "exec Get_Expense_Details_month '" . $sfCode . "' ,'" . $EMonth . "' ,'" . $EYear . "','".$Owndiv."' ";
			  $View = performQuery($sql);
$Type = $_GET['Type'];
	$query ="update tb set tb.ClstrName=SDP_Name,tb.cluster=SDP from  TbMyDayPlan tb inner join   DCRDetail_Lst_Trans  dc on dc.sf_code=tb.sf_code where tb.sf_code='" . $sfCode . "' and ClstrName='Missed Entry'  and convert(date,tb.Pln_Date)=convert(date,dc.ModTime)";
	performQuery($query);
	...
	if($Type=='1'){
  $query = "select case when   (WorkType='Meeting' or  WorkType='Admin Work' or WorkType='Transit' ) and ClstrName=''  and  Division_Code='109' then 'HQ' ELSE Allowance_Type end Allowance_Type,replace(ClstrName,'$$',',') ClstrName,WorkType,CONVERT(VARCHAR(10),pln_date,126) as Pln_Date,CONVERT(VARCHAR(10),pln_date,101) as plndate,FWFlg  from (select  ROW_NUMBER() OVER(PARTITION BY cast(Pln_Date as date) order by Pln_Date) RW,isnull(ClstrName,'') ClstrName ,Division_Code, (select  isnull(Allowance_Type,'HQ') from Mas_Territory_Creation where   CONVERT(VARCHAR(50),Territory_Code)=isnull(TB.cluster,'')) Allowance_Type , (select Worktype_Name_B from   Mas_WorkType_BaseLevel where WorkType_Code_B=TB.wtype ) WorkType,pln_date,FWFlg from TbMyDayPlan    TB  where TB.sf_code='" . $sfCode . "' and month(TB.Pln_Date)='" . $EMonth . "' and year(TB.Pln_Date)='" . $EYear . "' ) A where RW=1 and FWFlg!='L' and FWFlg!='H' and FWFlg!='W'";
 }else{
   $query = "select case when   (WorkType='Meeting' or  WorkType='Admin Work' or WorkType='Transit') and ClstrName='' and  Division_Code='109'  then 'HQ' ELSE Allowance_Type end Allowance_Type,replace(ClstrName,'$$',',') ClstrName,WorkType,CONVERT(VARCHAR(10),pln_date,126) as Pln_Date,CONVERT(VARCHAR(10),pln_date,101) as plndate,FWFlg  from (select  ROW_NUMBER() OVER(PARTITION BY cast(Pln_Date as date) order by Pln_Date) RW,ClstrName,Division_Code,  isnull((select  Allowance_Type from Mas_Territory_Creation where CAST(Territory_Code AS varchar(50))=isnull(TB.cluster,'')),'HQ') Allowance_Type,(select Worktype_Name_M from   Mas_WorkType_Mgr where WorkType_Code_M=TB.wtype ) WorkType, pln_date,FWFlg from TbMyDayPlan  TB  where TB.sf_code='" . $sfCode . "' and month(TB.Pln_Date)='" . $EMonth . "' and year(TB.Pln_Date)='" . $EYear . "') A where RW=1 and FWFlg!='L' and FWFlg!='H' and FWFlg!='W'";
 }
  $attendanceView = performQuery($query);
  $resp["success"] = $query;
		  $resp["monthwise"] = $View;
		    $resp["month_year"] = $month_year;
			$resp["attendanceView"] = $attendanceView;
        echo json_encode($resp);
        die;
        break;
