...
function getSlabDetails() {
    ...
    $query = "select  RetSlabID id,SlabName name,SlabDesc ,SlabMinVal,SlabMaxVal  from Mas_Retail_Business_Slab where ActiveFlag='0' ";
    return performQuery($query);
}
            ...
            if($sfCode!="undefined")
            {
                ...
                for ($i = 0; $i < count($vals); $i++) 
                {
                   ...
                   $Crate= $CandPrate[0]['Distributor_Price'];
                    ...
                    $sql = "insert into Trans_Current_Stock_details(Stockist_Code,Product_Code,Product_Name,Rec_Qty,Cb_Qty,pieces,Division_Code,Last_Updation_Date,Conversion_Qty,SfCode,Mgf_date,Crate,Prate) select '" . $sfCode . "','" . $productCode . "',N'" . $productName . "',$recvQty,$cbQty,$pieces," . $Owndiv . ",'" . $date . "',$sampleErpCode,'$staffCode','$Mfg','$Crate','$Prate'";
                    performQuery($sql);
...
performQuery($missedquery);
	 ...
	 $result['msg'] = "DCR not possible kindly contact Admin";
                            outputJSON($result);
                            die;
}	
            break;
