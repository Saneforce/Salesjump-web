...
function DayReport() {
	...
	left outer join Trans_PriOrder_Head oh on h.Sf_Code=oh.Sf_Code and cast(Activity_Date as date)=cast(Order_Date as date) and cast(Trans_Detail_Info_Code as varchar)=oh.Stockist_Code
	where cast(Activity_Date as date)='".$dyDt."' and h.SF_code='" . $sfCode . "'
	) as Pri";
	$dayrep= performQuery($query);
	if(count($dayrep)>0) {
...
function getOrderDetByDt(){
	...
	$Dt = $_GET['rptDt'];
	$CusCd = $_GET['CusCd'];
	$query = "select h.SF_Code,Cust_Code,l.ListedDr_Name CusName,l.ListedDr_Address1 CusAddr,l.ListedDr_Mobile CusMobile,s.Stockist_Name StkName,s.Stockist_Address StkAddr,l.ListedDr_Mobile StkMob,h.Trans_Sl_No from Trans_Order_Head h
	...
	for ($j = 0; $j < count($Res); $j++) {
		...
		Offer_ProductCd OffPCode,iif(isnull(Offer_ProductNm,'')='',Product_Name,Offer_ProductNm) OffPName,off_pro_unit OffUntID,umo_unit OffUntName,free offQty,Conf_Fac OffConv
		from Trans_Order_Details d where Trans_Sl_No='".$Res[$j]["Trans_Sl_No"]."'";
...
$Month=$now->format("M");
		...
		$query="select top 2 trans_sl_no,dcr_code,order_value,CONVERT(VARCHAR(10), order_date, 105) visit_date,LTRIM(RIGHT(CONVERT(VARCHAR(20), order_date, 100), 7)) visit_time,order_date from trans_order_head  where sf_code='".$SF."' and Cust_Code='".$MSL."'    order by Order_Date desc";
...
$sql="select Product_Brd_Name name,sum(value) total,count(Product_Brd_Name) qty from (select product_code,product_name,quantity,value,Product_Brd_Name from Trans_Order_Details det
inner join Mas_Product_Detail prod on prod.Product_Detail_Code=det.Product_Code
...
case "SSstockUpdation":
        ...
        case "tbMyDayPlan":
			...
			performQuery($sql);
			$resp['msg'] = "Today Plan has been Submitted Successfully";
			$resp['msql'] = "Today Plan has been Submitted Successfully";
...
if($mods!="26"){
 ...
 case "DailyInventoryLoadingNative":
                            $mods=$vals["svMode"];
                            $CrryForword=$vals["CrryForword"];
                            $tblnm="Trans_Prod_DailyInventoryNative";
                            $SFCd=$vals["SF"];
                            if($vals["SF"]=="" || $vals["SF"]==null)
                            $SFCd=$vals["sfCode"];
                            ...
                            //$dailyInvsql = "insert into Trans_Prod_DailyInventoryNative (SLNo, SFCode, EntryDt, Prod_Code, CaseQty, PiceQty, ConvQty, RateC_Retail, RateP_Retail, RateC_Dist, RateP_Dist, ApproveFlag, StkId, StkNm, Totalqty, TotalCase, TotalPcs, OPCase, OPPcs, Div_Id) select '" . $pk ."','". $SFCd ."','". date('Y-m-d 00:00:00') ."','". $PDets["id"] . "','" . $PDets["opening_case_qty"] . "','" . $PDets["opening_piece_qty"]. "','" .$PDets["conversionQty"] . "',isnull(RetailCasePrice,0), isnull(Retailor_Price,0),isnull(DistCasePrice,0) DCP,isnull(Distributor_Price,0), 0,'". $vals["StkId"] . "','" . $vals["StkNm"] . "','" . $PDets["Totalqty"] . "','" . $PDets["total_case_qty"] . "','" .$PDets["total_piece_qty"]. "','" . $PDets["carry_fwd_case_qty"] . "','" .$PDets["carry_fwd_piece_qty"].  "','" . $PDets["Division_Code"] . "' from Mas_Product_Detail m left outer join vwProductStateRates v on m.Product_Detail_Code=v.Product_Detail_Code and m.Division_Code=v.Division_Code and v.State_Code=". $SFState." Where m.Product_Detail_Code='". $PDets["id"] . "'";
                            ...
                            $Ledgersql = "insert into Trans_VanStock_Ledger (Legr_ID,from_loc,fLocTyp,To_Loc,tLocTyp,eDate,PCode,GQty,Division_Code,EntryBy,CalcTyp,TNm) select '" . $MaxRwID ."','". $vals["StkId"] ."','F','". $SFCd ."','R','" . date('Y-m-d 00:00:00') . "','" . $PDets["id"] . "','" .$PDets["g_qty"]. "','". $Divc . "','DayInv','0','". $vals["StkNm"] ."'" ;
	...
	case "Activity_Report_APP":
		...
		for ($i = 1; $i < count($data); $i++) {
			...
			if (isset($tableData['Activity_Stockist_Report'])) {
				...
				if (!empty($DetTB["doctor_id"])) $doctor_id=$DetTB["doctor_id"];
				if (!empty($DetTB["superstockistid"])) $superstk=$DetTB["superstockistid"];
			...
			if (isset($tableData['Activity_Stk_POB_Report'])){
				...
				for ($j = 0; $j < count($samp); $j++) {
					...
					$PPXML = $PPXML . "<Prod PCode=\"".$samp[$j]["product_code"]."\" PName=\"".$samp[$j]["product_Name"]."\" CQty=\"".$CaseQty."\" Rate=\"".$samp[$j]["Rate"]."\" PQty=\"".$PieceQty."\" CL=\"".$cl_qty_stock."\"  Free=\"".$Free_qty."\"  discount_price=\"".$discount_price."\"  discount=\"".$discount."\"   Fname=\"".$Fname."\"  FreeP_Code=\"".$FreeP_Code."\"    tax=\"".$tax_price."\"  />";
		...
		if (!empty($data[1]["Activity_Doctor_Report"]['superstockistid'])) $superstk=$data[1]["Activity_Doctor_Report"]['superstockistid'];
		...
		if ($vTyp != 0 && $vTyp!=7 && $vTyp != 9) {
			if ($vTyp == 2 || $vTyp == 3 || $vTyp == 8) $proc = "svDCRCSHDet_App";
			if ($pob == '') $pob = '0';
			if ($POB_Value == '') $POB_Value = '0';
			...
			{
				if($DetTB["intrumenttype"]==null) $DetTB["intrumenttype"]="";
				if($DetTB["date_of_intrument"]==null) $DetTB["date_of_intrument"]="";
		...
		if ($vTyp == 1 ){
			$sql = "update DCRDetail_Lst_Trans set StartOrder_Time='$OrderStrat_Time',EndOrder_Time= '$OrderEnd_Time'  where Sf_Code='" . $sfCode . "' and Trans_Detail_Slno='$ARDCd'";        performQuery($sql);
		}
				...
				*/
				...
				for ($i = 0; $i < count($transOrderDetails); $i++) {
					...
					if($qty>0 || $free>0   || $cbQty>0  ||  $Ccb_qty>0 || $Promo>0  || $productremarks!==$remarkspd ||  $RcpaId>0 ){
						...
						$sql = "select Product_Detail_Code,case when ltrim(rtrim(isnull(Distributor_Discount_Price,'')))='' then 0 else isnull(Distributor_Discount_Price,'0') End DistDisc,case when ltrim(rtrim(isnull(MRP_Price,'')))='' then 0 else isnull(MRP_Price,'0') End DistPRate from Mas_Product_State_Rates r inner join Mas_Salesforce s on r.State_Code=s.State_Code and r.Division_Code=cast(left(s.Division_Code,charindex(',',s.Division_Code+',')-1) as int) where Sf_Code='" . $sfCode . "' and Product_Detail_Code='" . $transOrderDetails[$i]['product_code'] . "'";
						...
						if($qty>0 || $free>0   || $cbQty>0 || $Promo>0   ||  $Ccb_qty>0   ||  $RcpaId>0  ){
							...
							if($POT==2){
								...
								$sql = "update Trans_Prod_DailyInventory   set Totalqty=(select Totalqty-$qty   from Trans_Prod_DailyInventory where sfcode= '" . $sfCode . "' and EntryDt='" . $today . "' and Prod_Code='" . $transOrderDetails[$i]['product_code'] . "') where sfcode= '" . $sfCode . "' and convert(date,EntryDt)='" . $today . "' and Prod_Code='" . $transOrderDetails[$i]['product_code'] . "' ";
								performQuery($sql);
...
$divisionCode=$_GET['divisionCode'];
	     ...
	     $query = "select  TOP 1*  from Trans_TP where SF_Code='$sfCode' and Tour_Month=month(getdate()) and Tour_Year=year(getdate()) and (Confirmed=1 or Confirmed=3) and 
...
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' ";
                ...
                $current = performQuery($query);
				 $query = "select  TOP 1*  from Trans_TP where SF_Code='$sfCode' and Tour_Month=month(getdate())+1 and Tour_Year=year(getdate()) and (Confirmed=1 or Confirmed=3) and 
Worktype_Name_B!='Holiday' and Worktype_Name_B!='Leave' and Worktype_Name_B!='Weekly Off' UNION
            ...
            default:
					...
					if($tableName == "vwMyDayPlan" && ($results==null || count($results) == 0))
				{
					...
					$query = "select remarks, RouteName as ClstrName, Worked_with_Code as stockistid, Place_Inv,
						  ...
						  HQ_Code as subordinateid, HQ_Name,date as name,convert(varchar,date,20) plnDate
						  from vwTourPlan  where SF_Code = '". $sfCode ."' and date = '". $todayDate ."'";

						$results = performQuery($query);
						if(count($results)>0){
							$results[0]['tourplanDone'] = 1;
							$FWFlgQuery = "select top 1 FWFlg from vwMas_WorkType_App where type_code ='". $results[0]['worktype']."'";
...
inner join vwactivity_report r on r.sf_code=h.Sf_Code and cast(activity_date as date)=cast(order_date as date)
 ...
 Reject_reason,Aproval_Flag from Trans_Gift_Claim where Sf_Code ='" . $sfCode . "'";
    ...
    case "GetExpenseApprovals":
		...
		$sql = "exec Get_Expense_Approvals '" . $sfCode . "' ,'" . $EMonth . "' ,'" . $EYear . "','".$Owndiv."' ";
		$View = performQuery($sql);
		$sql = "select isnull(Applied_ExpAmnt,0) Applied_ExpAmnt,Expense_Amt from   trans_expense_head1  where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'";
    	$Total = performQuery($sql);
