...
function getSlabDetails() {
    ...
    $query = "select  RetSlabID id,SlabName name, SlabName  ,month(From_dt)SlabMonth,SlabMinVal,SlabMaxVal,CONVERT(varchar,To_Dt, 101)To_Date  from Mas_Retail_Business_Slab where ActiveFlag='0' and  Division_Code='" . $Owndiv . "' and isnull(Claim_deadline,convert(date,getdate()))>=convert(date,getdate()) ";
    return performQuery($query);
}
...
function GiftEnrolment() {
    ...
    $query = "select  GiftSlabID id,month(From_Date)GiftMonth,GiftName name,isnull(Claim_deadline,convert(date,getdate())) Claim_deadline , DATEDIFF(day, getdate(), isnull(Claim_deadline,convert(date,getdate())))daydif, GiftDesc GiftName,CONVERT(varchar,To_Date, 101)To_Date  from mas_gift_slab where ActiveFlag='0' and Division_Code='" . $Owndiv . "' and isnull(Claim_deadline,convert(date,getdate()))>=convert(date,getdate()) ";
    return performQuery($query);
}
...
function getPreCallDet() {
  ...
  $gift = performQuery($query);
    if (count($as) > 0) {
		...
		$result['SlabName'] = (string) $gift[0]['Slan_Name'];
...
if($countresult[0]["count"]==0){
 ...
 if( $data[2]['ActivityCaptures']!=null && $data[2]['ActivityCaptures']!="undefined")
   ...
   $sql ="exec Post_Expense '$sfCode','".$divisionCode[0]."','".$val["Enterdate"]."','".$val["TotalAmount"]."','".$val["HQ"]."','".$val["Routename"]."','".$val["Toworkplace"]."','".$val["KM"]."','".$val["WorkType"]."' ,'".$val["mydayplanWorkPlace"]."','".$val["Remarks"]."'";
   $result=performQuery($sql); 
