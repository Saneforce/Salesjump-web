...
function actionLogin() {
    ...
    if ($count == 1) {
		...
		$respon['Logo_Name'] = $arr[0]['Logo_Name'];
...
function getDailyAllow() {
	...
	$date=date('Y-m-d');
	$sfCode = $_GET['sfCode'];
    $querys = "select ID,Allowance_Name Name from Mas_Allowance_Type where Type=1 and Division_Code=". $Owndiv ." and user_enter=1 and active_flag=0";
	$ExpenseWeb=performQuery($querys);
      ...
      $sql = "{call  svDCRMain_App(?,?,'" . $worktype . "','" . $cluster  . "',?,'Product Availability',?,'0')}";
       ...
       $sql = "SELECT isNull(max(Sl_No),0)+1 as Sl_No FROM Trans_Product_Availability";
        ...
        case "TP_Attendance":
            ...
            if($update==0){
                $sql ="exec Attendance_entryzar '$sfCode','$Owndiv','$eDate',$lat,$long,'$eDate','$slfy','".$wrkType."' ,'0' ,'".$Shift_Selected_Id."','$App_Version' ";
                $result=performQuery($sql);     
            }
            else{
...
$va =$data[1][$tempp[0]];
  ...
  $sql1="insert into  Trans_Expense_Traveldetails(sf_code,Division_code,MOC,Remarks,Start_Image,FromDeparture,End_Image,ToArrival,Date_Time,Fare,FromPlace,ToPlace,Leos,Ukey,FromPlace_Name,ToPlace_Name,Bus_Support_Doc,DateofExpense)values('".$sfCode."','".$Owndiv ."','" . $va["MOT"]. "','".$va["Remarks"]."' ,'".$va["Start_image"]."','".$va["Start_Km"]."','".$va["Stop_image"]."','".$va["Stop_km"]."', '$dateTime' ,'".$va["BusFare"]."','".$va["Workplace"]."' ,'".$va["TodayworkRoute"]."' ,'" . $va["LEOS"] . "','" . $va["Ukey"] . "','".$va["Workplace_Name"]."' ,'".$va["TodayworkName"]."' ,'".$va["EventcaptureUrl"]."','".$va["DateOfExp"]."')";
        performQuery($sql1);
...
case "get/vwVstPendingPragnancy":
    ...
    case "get/misseddates":
		...
		$queryy = "exec [updAttnCheckOut] '" . $sfCode . "','" .  $dt . "','" . $data["Lattitude"] . "','" . $data["Langitude"] . "','".$slfy."' ,'".$Rmks."'";
		performQuery($queryy);
		/*
...
$Password = $_GET['Password'];			 //CASE WHEN  UsrDfd_UserName='HAPHTNSAO2' THEN '18.7357416:78.9700081' ELSE '12.91729615:80.23030043' END 
			$sql = "SELECT  UsrDfd_UserName Sf_UserName,Sf_Name,Sf_Password,ms.Division_Code,DisRad,ms.Sf_code,(select Url_Short_Name from Mas_division D where  D.Division_Code=am.division_code) Logo_Name,Geo_Fencing,isnull(latlong,'12.91729615:80.23030043') AS HOLocation from mas_salesforce ms inner join Mas_HQuarters m on cast(HQ_ID as varchar)=Hq_Code and SF_Status=0 inner join access_master am on  charindex(','+  cast(am.division_code as varchar) +',',','+ms.Division_Code+',')>0   inner join   access_table AT on At.Sf_code= ms.Sf_code where Sf_UserName='".$Email."'  and Sf_Password='".$Password."'";
             $res = performQuery($sql);
   ...
   outputJSON(performQuery($query));
       ...
       $divs = explode(",", $div . ",");
		...
		$Sf_HQ=performQuery($query);
                ...
                $query = "select HQ_ID id,HQ_Name name from Mas_HQuarters where Division_code='" . $Owndiv . "' and HQ_Active_Flag=0 ";
               outputJSON(performQuery($query));
                break;	
...
case "get/Signout_Check":
		...
		if ($Flag==0){
			$query = "select top 1 Sft_Name,Start_Time,  convert(varchar, Start_Time, 8) Edate ,End_Time,cast(convert(varchar,login_date-1,101) +' '+ convert (varchar,sft_STime,108) as datetime) as Sft_STime,sft_ETime,0 Wtp ,Sft_ID ,case when  isnull(End_Time,0)=0 then 0 else 1 end [Type],(case when ACtOffTyp=0 then cast(convert(varchar,Login_date,101) +' 23:59:59' as datetime) else dateadd(hour,ACtOffHrs,cast(convert(varchar,dateadd(day,1,login_date),101) +' '+ convert (varchar,sft_ETime,108) as datetime)) End) ACutOff  from TP_Attendance_App Tp inner join mas_shift_timings mst on  mst.Sft_ID=Tp.ShiftSelected_Id where sf_code='".$SF_code."'  order by  Start_Time  desc";//'".$today."'
			outputJSON(performQuery($query));
		}	
		else{
			$arr=[];
...
case "get/ClearData":
	...
	case "get/Shift_timing":
		...
		$sfCode = $_GET['Sf_code'];
		//;
        $query = "select Sft_ID id,Sft_Name name,Sft_STime, cast(convert(varchar,(case when Sft_STime>Sft_ETime then dateadd(day,1,getDate()) else getdate() End),101) +' '+ convert (varchar,sft_ETime,108) as datetime) as   sft_ETime from mas_shift_timings where Division_code='" . $Owndiv . "' and ActiveFlag=0"; 
		/*if ($sfCode!=null){
...
outputJSON($result);
	...
	case "Check/Version":
		...
		if($VERSION==$Ver || $VERSION=="Ver 1.0.9"){
			$result["Msg"] = "";
			$result["success"] = true;
		}else{
		...
		case "get/CheckinUsers":
        $div = $_GET['divisionCode'];
		$divs = explode(",", $div . ",");
	    $Owndiv = (string) $divs[0];
		$sfCode = $_GET['Sf_code'];
		//;
        $query = "select '1200' as field ,'5000' as totaluser,'2300' as nonfield "; 
