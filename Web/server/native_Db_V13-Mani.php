	...
	if (strpos($imgName, $sf) !== false)
		{
			...
			$imgName = $sf . "" . $_FILES["imgfile"]["name"];
		}
...
Mgf_date,batch_no,SS_Base_Rate";
	...
	case "get/brandavail":
		...
		$tr=performQuery($sql);
        $sql="select l.ListedDrCode, l.ListedDr_Name,ISNULL(evnt.imgurl, '') imgurl,outlet.Rmks from Trans_OutletReview_Details outlet
              left join Activity_Event_Captures evnt on cast(outlet.TDSlNo as varchar(20))=evnt.DCRDetNo
              inner join Mas_ListedDr l on cast(l.ListedDrCode as varchar(50))=outlet.RetID where 
