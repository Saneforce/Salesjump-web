...
function getDailyAllow() {
    ...
    $divs = explode(",", $div . ",");
    $Owndiv = (string) $divs[0];
    $query = "select ID,Allowance_Name Name from Mas_Allowance_Type where Type=1 and Division_Code=". $Owndiv ." and user_enter=1";
    return performQuery($query);
}
...
function getPreCallDet() {
    ...
    if (count($as) > 0) {
		...
		if (count($as) > 0) {
            $result['LVDt'] = date('d / m / Y', strtotime((string) $as[0]['Adate']));
			//$result['LVDt'] =Adate
...
function addEntry() {
    ...
    switch ($temp[0]) {
        ...
        case "tbMyDayPlan":
            ...
            $sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location,CustId,CustName,StkName) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'".$today."'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location','".$vals["custid"]."','".$vals["custName"]."','".$vals["stkName"]."'";
			performQuery($sql);
            if (str_replace("'", "", $vals["FWFlg"]) != "F") {
        ...
        case "unlisted_doctor_master":
			...
			$sql = "{call  svListedDR_APP('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . ",'" . str_replace("'","",$vals['unlisted_doctor_cityname']) . "','" . str_replace("'","",$vals['unlisted_doctor_areaname']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson']) . "', ' " . str_replace("'","",$vals['unlisted_doctor_designation']) . "','" . str_replace("'","",$vals['unlisted_doctor_gst']) . "','" . str_replace("'","",$vals['unlisted_doctor_pincode']) . "'," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ",'" . str_replace("'","",$vals['unlisted_doctor_phone2']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson2']) . "','" . str_replace("'","",$vals['unlisted_doctor_designation2']) . "')}";

				//$sql = "{call  svListedDR_APP('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . ",'" . str_replace("'","",$vals['unlisted_doctor_cityname']) . "','" . str_replace("'","",$vals['unlisted_doctor_areaname']) . "','" . str_replace("'","",$vals['unlisted_doctor_contactperson']) . "', ' " . str_replace("'","",$vals['unlisted_doctor_designation']) . "','" . str_replace("'","",$vals['unlisted_doctor_gst']) . "','" . str_replace("'","",$vals['unlisted_doctor_pincode']) . "'," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ")}";

//echo($sql);
			
			//$sql = "{call  svListedDR_APP('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . "," . $vals['unlisted_doctor_cityname'] . "," . $vals['unlisted_doctor_areaname'] . "," . $vals['unlisted_doctor_contactperson'] . ",  " . $vals['unlisted_doctor_designation'] . "," . $vals['unlisted_doctor_gst'] . "," . $vals['unlisted_doctor_pincode'] . "," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ")}";


			
			
			
//$sql = "{call  svListedDR_APP('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . ",'" . $vals['unlisted_doctor_cityname'] . "','" . $vals['unlisted_doctor_areaname'] . "','" . $vals['unlisted_doctor_contactperson'] . "','" . $vals['unlisted_doctor_designation'] . "', '" . $vals['unlisted_doctor_designation'] . "','" . $vals['unlisted_doctor_designation'] . "',             " . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ")}";

            //$sql = "{call  svListedDR_APP('$sfCode'," . $vals['unlisted_doctor_name'] . "," . $vals['unlisted_doctor_address'] . "," . $vals['unlisted_doctor_phone'] . "," . $vals["unlisted_specialty_code"] . "," . $vals["unlisted_cat_code"] . "," . $vals["town_code"] . ",'0','" . date('Y-m-d H:i:s') . "',$Owndiv," . $vals["unlisted_class"] . ",0," . $vals['wlkg_sequence'] . ",'',''," . $vals['DrKeyId'] . ")}";
        ...
        case "OrderReturn":
			...
			for($il=0;$il<count($vals["RetProducts"]);$il++)
			{
					...
					$sql = "insert into SalesReturn_Detail select ".$pk.",m.Product_Detail_Code," . $PDets["Qty"] . "," . 
							...
							",'" . $PDets["QType"] . "','" . $PDets["ClaimType"] . "','" . $PDets["remark"] . "' from Mas_Product_Detail m left outer join vwProductStateRates v on m.Product_Detail_Code=v.Product_Detail_Code and m.Division_Code=v.Division_Code ".
...
case "DOOR_TO_DOOR":
			...
			$divCode = $_GET['divisionCode'];
			 ...
			 $PERSIONID=1;
			 $Current_Date_And_Time =date("m-d-Y h:i:s a", time());
			 $FWFlg=$data[2]['FWFlg'];
			 
			 $Remarks=$data[1]['Remarks'];
			 $copen=$data[3]['No0fcoupen'];
			 $Retailer=$data[4]['Retailer'];
			  $Identification='Door_To_Door';
if( $data[count($data)-1]["Activity_Event_Captures"]!=null && $data[count($data)-1]["Activity_Event_Captures"]!="undefined")
			{
				$Event_Captures = $data[count($data)-1]["Activity_Event_Captures"];
				for ($j = 0; $j < count($Event_Captures); $j++) {	                	
					...
					$sql = "insert into Activity_Event_Captures(Activity_Report_Code,DCRDetNo,imgurl,title,remarks,Division_Code,Identification,Insert_Date_Time) values( '" . $divCode . "','" . $State_Code . "','" . $ev_imgurl . "','" . $ev_title . "','" . $ev_remarks . "','" . $divCode . "','" . $Identification . "','".$Current_Date_And_Time."')";
		            performQuery($sql);	
				}
			}
			  ...
			  for ($il = 0;$il < count($data[0]['DOOR_TO_DOOR']);$il++) {
				...
				$sql="insert into Trans_Door_To_Door (PersonID,sfCode, SfName, State_Code, Division_Code, Promotorname, place, taken, issue,starttime,endtime,Remarks,Retailer,Current_Date_And_Time)
values (  '".$copen."', '".$sfCode."','".$FWFlg."', '".$State_Code."','". str_replace(",", "",$divCode)."','".$PromotorName."','".$Place."','".$Taken."','".$Issue."','".$Stime."','".$Etime."','".$Remarks."','".$Retailer."','".$Current_Date_And_Time."')";
				 performQuery($sql);
...
if( $data[count($data)-1]["Activity_Event_Captures"]!=null && $data[count($data)-1]["Activity_Event_Captures"]!="undefined")
			{
			  ...
			  for ($il = 0;$il < count($data[0]['Inshop_Activity']);$il++) {
				...
				$sql="insert into Trans_Inshop_Activity (sfCode, SfName, State_Code, Division_Code, Promotorname,issue,starttime,endtime,Remarks)
values (  '".$sfCode."','".$FWFlg."', '".$State_Code."','". str_replace(",", "",$divCode)."','".$PromotorName."','".$Issue."','".$Stime."','".$Etime."','".$Remarks."')";
				 performQuery($sql);
...
$date_for_database = date ("Y-m-d H:i:s", strtotime($Bill_Date));
            ...
            $pk = (int) $tRw[0]['Trans_SlNo']; 
			 //$pk = $pk + 1;
		$query="INSERT INTO Trans_CusBill_CollectionDet   ( Trans_SlNo,Cust_Code, ColltDt, BillNo,BillDt,BillAmt,CollAmt,BalAmt,Division_Code,SF_CODE)
VALUES (  '".$pk."','".$docor_code."', '".$date."', '".$Bill_No."', '".$date_for_database."', '".$billamt."', '".$Collected_Amount."','".$Balance_Amount."','".$divisionCode."','".$datasf."'  )";
						performQuery($query);
						}
...
/*                if ($ARDCd == "Exists") {
				...
				if($vTyp == 3){
		            $sqlOrd = "{call svPriOrder(?,$cCode,?," . $POB_Value . ",'" . $DetTB["intrumenttype"] . "','" . $DetTB["date_of_intrument"] . "'," . $pob . ",'" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,?,?,'".$eKeyID."')}";
		            $params1 = array(
						array($sfCode, SQLSRV_PARAM_IN),
		                array($today, SQLSRV_PARAM_IN),
			...
			if( $data[count($data)-1]["Activity_Event_Captures"]!=null && $data[count($data)-1]["Activity_Event_Captures"]!="undefined")
			{
				...
				for ($j = 0; $j < count($Event_Captures); $j++) {	                	
					...
					$sql = "insert into Activity_Event_Captures(Activity_Report_Code,DCRDetNo,imgurl,title,remarks,Division_Code) select '" . $ARCd . "','" . $ARDCd . "'," . $ev_imgurl . "," . $ev_title . "," . $ev_remarks . ",'" . $Owndiv . "'";
		            performQuery($sql);	
				}
			}
            $townCode = $data[0]['Activity_Report_APP']['Town_code'];
            ...
            if (count($transOrderDetails) > 0) {
				...
				if(count($eRw)<1 || $eKeyID==""){
	                ...
	                $sql = "insert into Trans_Order_Head(Trans_Sl_No,Sf_Code,Cust_Code,Stockist_Code,Route_Target,Order_No,Collected_Amount,Order_Value,Order_Date,Route,DCR_Code,net_weight_value,discount_price,rateMode,Div_ID,OrderID) select '" . $pk . "','" . $sfCode . "'," . $custCode . "," . $stockistCode . ",'" . $routeTarget . "'," . $orderNo . ", $collectedAmount , $orderValue," . $orderDate . ", $townCode,'" . $DCRCode . "','$net_weight_value','$discount_price','$rateMode','$Owndiv','".$eKeyID."'";
					$resp["HeadQry"]=$sql;
	            	performQuery($sql);
	                ...
	                for ($i = 0; $i < count($transOrderDetails); $i++) {
						...
						if($qty>0 || $free>0   || $cbQty>0 || $Promo>0){
		                   ...
		                   /*  $sql = "SELECT isNull(max(cast(Trans_Order_No as numeric)),0)+1 as RwID FROM Trans_Order_Details";
		                    ...
		                    $sql = "insert into Trans_Order_Details( Trans_Order_No, Trans_Sl_No,Product_Code,Product_Name,Quantity,value,Rate,net_weight,free,discount,discount_price,rateMode,DistDisc,DistPicRate,MfgDt,ClStock,PromoVal,Div_ID) select '" . $pk1 . "','" . $pk . "','" . $transOrderDetails[$i]['product_code'] . "','" . $transOrderDetails[$i]['product_Name'] . "', $qty,$value,$rate,'$net_weight','$free','$discount','$discountprice','$rateMode',$Disc,$DistRate,'" . $transOrderDetails[$i]['Mfg_Date'] . "','". $cbQty  ."',$Promo,'$Owndiv'";
							performQuery($sql);
							$orderValue=$orderValue+$value;
						}
	                }
	                $sql = "update Trans_Order_Head set Order_Value=$orderValue Trans_Sl_No='" . $pk . "'";
					$resp["HeadQry"]=$sql;
	            	performQuery($sql);
				}
			}
...
switch ($value[0]) {
    ...
    case "get/track":
		$SF=$_GET['SF_Code'];
    	$Dt = $_GET['Dt'];
		$query = "select * from ((select replace(convert(varchar,Time,9),':000',' ') DtTime,lati lat,Long lng,Trans_Detail_Name,POB_Value,net_weight_value,GeoAddrs,Time DtTm,0 ordfld from vwActivity_MSL_Details where  cast(convert(varchar,Time,101) as datetime)=cast(convert(varchar,'$Dt',101) as datetime) and Time>'$Dt'  and sf_code='$SF')union(select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'','0','0','',DtTm,1 from tbTrackLoction where SF_Code='$SF'  and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar,'$Dt',101) as datetime) and DtTm>'$Dt' and cast(Auc as float)<=30)) as t order by DtTm,ordfld";
...
break;
 ...
 case "pendingbils":
        ...
        if (count($pendinglist) > 0) {
			...
			$pendinglistt[] = array("billno" => $pendinglist[$i]['billno'],"custcode" => $pendinglist[$i]['custcode'], "billamt" => $pendinglist[$i]['billamt'], "coll_amt" => 0,"date" => $pendinglist[$i]['inv_dt'], "BalanceAmout" => $pendinglist[$i]['bal_amt']);
		 ...
		 $result['PendingList'] = $pendinglistt;
				...
				case "productcategoryrates":
				  ...
				  $divisionCode=str_replace(",","",$_GET['divisionCode']);
					  ...
					  $query = "select Cate_Code,product_detail_code,discount,rp_base_rate,rp_case_rate,mrp_rate,effective_from_date  from mas_product_category_rates  where division_code='" . $divisionCode . "' ";
 ...
 $query = "update dcr_summary set pob_value=(pob_value-$pobValSumm)+$orderValue WHERE sf_code ='$sfCode' and cast(submission_date as date)='$datesumm'";
   ...
   performQuery($query);
	...
	case "get/retailer":
		...
		cityname='" . $data["Cityname"] . "' ,areaname='" . $data["AreaName"] . "',contactperson='" . $data["ContactPerson"] . "',designation='" . $data["Designation"] . "',gst='" . $data["GSTno"] . "',pin_code='" . $data["PINcode"] . "',ListedDr_Phone2='" . $data["Phone2"] . "',contactperson2='" . $data["ContactPerson2"] . "',designation2='" . $data["Designation2"] . "'
