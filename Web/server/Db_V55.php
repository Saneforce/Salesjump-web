...
function getSlabDetails() {
    ...
    $query = "select  RetSlabID id,SlabName name, SlabName  ,SlabMinVal,SlabMaxVal,CONVERT(varchar,To_Dt, 101)To_Date  from Mas_Retail_Business_Slab where ActiveFlag='0' and  Division_Code='" . $Owndiv . "' ";
    return performQuery($query);
}
 		...
 		$net_weight=$Nwt[0]['product_netwt'];
			...
			//$productremarks=($transOrderDetails[$i]['rx_remarks']==null? 0:$transOrderDetails[$i]['rx_remarks']);
                        ...
                        if($qty>0 || $free>0   || $cbQty>0  ||  $Ccb_qty>0 || $Promo>0  || $productremarks!==$remarkspd ||  $RcpaId>0 ){
                            ...
                            $sql = "SELECT isNull(max(cast(Trans_Order_No as numeric)),0)+1 as RwID FROM Trans_Order_Details where Div_ID='".$divisionCode."'";
                            $tRw = performQuery($sql);
                            $pk1 = (int) $tRw[0]['RwID'];
