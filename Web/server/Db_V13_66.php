 ...
 case "fileAttachment":
    ...
    case "get/doctorCount":
		...
		$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code=1";
			...
			$sql="select slno id,Allowance_Type_Name name,newname from Mas_Allowance_Types where division_code!=1 and division_code!=107";
		$det=performQuery($sql);
		outputJSON($det);
		break;
    case "get/setup":
...
$sfCode=$_GET['rSF'];
                ...
                $query = "select  * from vwTourPlan where SF_Code='$sfCode' and Tmonth=$CMn and Tyear=$CYr order by worktype_name";
                $results = performQuery($query);
 ...
 case "get/Deactivate":
    ...
    case "sv/expenseApproval":
            $sfCode = $_GET['rSF'];
		 	...
		 	$empId = performQuery($sql);
		 	       $empcode=$empId[0]['sf_emp_id'];
$sql = "update trans_expense_head1 set Approve_Flag='0'  where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'  and Emp_ID='" . $empcode . "' and Approve_Flag='-1'";
	 performQuery($sql);
		  $resp["success"] = true;
		  $resp["Query"] = $sql;
        echo json_encode($resp);
    ...
    case "get/calenderdetails":
            $sfCode = $_GET['rSF'];
         $query="select month(getdate()) month1,11 month,30 no_of_days,year(getdate()) year,CONVERT(VARCHAR(10),getdate(),126) cdate,'2022-12-09' approvalopendate,'2022-12-19' approvalenddate";
		 ...
		 $sql = "select * from trans_expense_head1 where SF_Code='" . $sfCode . "' and Expense_Month='" . $EMonth . "' and Expense_Year='" . $EYear . "'  and Emp_ID='" . $empcode . "' and Approve_Flag!='-1'";
	 $tr=performQuery($sql);
	 if(count($tr)>0){
		 $month_year[0]['approvalopendate']=$month_year[0]['2022-01-01'];
		 $month_year[0]['approvalenddate']=$month_year[0]['2022-01-01'];
	 }
...
$Type = $_GET['Type'];
	...
	if($Type=='1'){
  $query = "select case when   (WorkType='Meeting' or  WorkType='Admin Work' or WorkType='Transit' ) and ClstrName=''  and  Division_Code='109' then 'HQ' ELSE Allowance_Type end Allowance_Type,ClstrName,WorkType,CONVERT(VARCHAR(10),pln_date,126) as Pln_Date,CONVERT(VARCHAR(10),pln_date,101) as plndate  from (select  ROW_NUMBER() OVER(PARTITION BY cast(Pln_Date as date) order by Pln_Date) RW,isnull(ClstrName,'') ClstrName ,Division_Code, (select  isnull(Allowance_Type,'HQ') from Mas_Territory_Creation where   CONVERT(VARCHAR(50),Territory_Code)=isnull(TB.cluster,'')) Allowance_Type , (select Worktype_Name_B from   Mas_WorkType_BaseLevel where WorkType_Code_B=TB.wtype ) WorkType,pln_date from TbMyDayPlan    TB  where TB.sf_code='" . $sfCode . "' and month(TB.Pln_Date)='" . $EMonth . "' and year(TB.Pln_Date)='" . $EYear . "' ) A where RW=1";
 }else{
   $query = "select case when   (WorkType='Meeting' or  WorkType='Admin Work' or WorkType='Transit') and ClstrName='' and  Division_Code='109'  then 'HQ' ELSE Allowance_Type end Allowance_Type,ClstrName,WorkType,CONVERT(VARCHAR(10),pln_date,126) as Pln_Date,CONVERT(VARCHAR(10),pln_date,101) as plndate  from (select  ROW_NUMBER() OVER(PARTITION BY cast(Pln_Date as date) order by Pln_Date) RW,ClstrName,Division_Code,  isnull((select  Allowance_Type from Mas_Territory_Creation where CAST(Territory_Code AS varchar(50))=isnull(TB.cluster,'')),'HQ') Allowance_Type,(select Worktype_Name_M from   Mas_WorkType_Mgr where WorkType_Code_M=TB.wtype ) WorkType, pln_date from TbMyDayPlan  TB  where TB.sf_code='" . $sfCode . "' and month(TB.Pln_Date)='" . $EMonth . "' and year(TB.Pln_Date)='" . $EYear . "') A where RW=1 ";
 }
