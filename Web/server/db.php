...
function getFromTable($tableName, $coloumns, $divisionCode, $sfCode = null, $orderBy = null, $where = null, $join = null, $today = null, $wt = null, $desig) {
    ...
    if (!is_null($sfCode)) {
        if ($desig == "mgr" && ($tableName == "vwDoctor_Master_APP"))
...
$location=$vals["location"];
            $sql = "insert into tbMyDayPlan(sf_code,sf_member_code,Pln_Date,cluster,remarks,Division_Code,wtype,FWFlg,ClstrName,stockist,worked_with_code,worked_with_name,dcrtype,location) select '" . $sfCode . "'," . $vals["sf_member_code"] . ",'" . date('Y-m-d H:i:s') . "'," . $vals["cluster"] . "," . $vals["remarks"] . ",'" . $Owndiv . "'," . $vals["wtype"] . "," . $vals["FWFlg"] . "," . $vals["ClstrName"] . "," . $vals["stockist"].",$ww,(Select SF_Name+', ' from vwUserDetails where charindex('$$'+sf_Code+'$$','$$'+$ww+'$$')>0 for XML path('')),'$dcrtype','$location'";
            performQuery($sql);
...
$sql="select * from trans_order_head where sf_code='$sfCode' and cust_code=$custCode and stockist_code=$stockistCode and order_date=$orderDate and route=$townCode";
...
die;
            ...
            for ($i = 1; $i < count($data); $i++) {
                ...
                if (isset($tableData['Activity_Doctor_Report'])) {
					...
					$net_weight_valueLST=$net_weight_value;
                }
 ...
 $resp["success"] = true;
    echo json_encode($resp);die;
            ...
            if ($vTyp != 0) {
                ...
                if ($vTyp == 1 || $vTyp == 4)
                    $sqlsp = $sqlsp . "?,?,?,?,?,";
                $sqlsp = $sqlsp . "'" . str_replace("'", "", $vals["Town_code"]) . "','" . str_replace("'", "", $vals["Daywise_Remarks"]) . "',?,'" . str_replace("'", "", $vals["rx_t"]) . "'," . $DetTB["modified_time"] . ",?,?," . $vals["DataSF"] . ",'" . $DetTB["geoaddress"] . "'," . $POB_Value . ",$net_weight_value)}";
...
if($net_weight_valueLST>0)
                ...
                $sql = "insert into Trans_Order_Head(Trans_Sl_No,Sf_Code,Cust_Code,Stockist_Code,Route_Target,Order_No,Collected_Amount,Order_Value,Order_Date,Route,DCR_Code,net_weight_value) select '" . $pk . "','" . $sfCode . "'," . $custCode . "," . $stockistCode . ",'" . $routeTarget . "'," . $orderNo . ", $collectedAmount , $orderValue," . $orderDate . ", $townCode,'" . $DCRCode . "',0";       
    performQuery($sql);
                $totalval = 0;
            ...
            performQuery($sql);
					...
					if($net_weight_valueLST>0)
						...
						$sql = "insert into Trans_Order_Details( Trans_Order_No, Trans_Sl_No,Product_Code,Product_Name,Quantity,value,Rate,net_weight) select '" . $pk1 . "','" . $pk . "','" . $transOrderDetails[$i]['product_code'] . "','" . $transOrderDetails[$i]['product_Name'] . "', $qty,$value,0,0";
...
switch ($value[0]) {
    ...
    case "get/track":
		...
		$query = "select t.DtTime,t.lat,t.lng,t.Trans_Detail_Name,t.ChannelName,t.POB_Value,t.net_weight_value,t.GeoAddrs,t.DtTm,t.ordfld,t.typ,t.ListedDr_Address1,t.geo_lat,t.geo_log,t.StartOrder_Time,t.EndOrder_Time,t.Remarks,t.OrderType,t.addrs from ((select replace(convert(varchar,Time,9),':000',' ') DtTime,m.lati lat,m.Long lng,Trans_Detail_Name,Doc_Spec_ShortName ChannelName,isnull(Order_Value,0) POB_Value,net_weight_value,GeoAddrs,Time DtTm,0 ordfld,1 typ ,concat(d.ListedDr_Address1,' ',ListedDr_Address2,' ',ListedDr_Address3, ', Phone No : ',coalesce(ListedDr_Phone,ListedDr_Mobile) ) ListedDr_Address1,(case when taggedLat=''then c.lat  else taggedLat end)as geo_lat,(case when taggedLong=''then c.long  else taggedLong end)as geo_log,isnull(convert(varchar,cast(m.StartOrder_Time as datetime),108),'')StartOrder_Time,isnull(convert(varchar,cast(m.EndOrder_Time as datetime),108),'')EndOrder_Time,Activity_Remarks Remarks,OrderTyp OrderType,c.addrs from vwActivity_MSL_Details m with (nolock) inner join Mas_ListedDr d with (nolock) on d.ListedDrCode=m.Trans_Detail_Info_Code left outer join map_GEO_Customers C with (nolock) on c.Cust_Code=d.ListedDrCode where cast(convert(varchar,Time,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and Time>'$Dt'  and m.sf_code='$SF')union all (select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'','','0','0','',DtTm,1,0 typ ,'' ListedDr_Address1,'' geo_lat,'' geo_log,'','','','',''  from tbTrackLoction with (nolock) where SF_Code='$SF' and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and DtTm>'$Dt' and cast(Auc as float)<=100 and cast(lat as float)>0 )union all(select replace(convert(varchar,Adt,9),':000',' ') DtTime,R.lat,R.Long lng,ListedDr_Name,Doc_Spec_ShortName,'0','0','',Adt,0,1 typ ,ListedDr_Address1,g.lat geo_lat,g.long geo_log,'','','','Review',g.addrs  from Trans_OutletReview_Details R with (nolock) inner join Mas_ListedDr d with (nolock) on RetID=ListedDrCode left outer join map_GEO_Customers g with (nolock) on Cust_Code=RetID where SFCode='$SF' and cast(convert(varchar,Adt,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime)and Adt>'$Dt')union all( select replace(convert(varchar,ModTime,9),':000',' ') DtTime,h.lati lat,h.Long lng,Trans_Detail_Name, ''ChannelName,POB_Value,'0' net_weight_value,GeoAddrs,ModTime DtTm,0 ordfld,2 typ ,Address ListedDr_Address1,c.lat geo_lat,c.long geo_log,isnull(convert(varchar,cast(h.StartOrder_Time as datetime),108),'')StartOrder_Time,isnull(convert(varchar,cast(h.EndOrder_Time as datetime),108),'')EndOrder_Time,Activity_Remarks,'',c.addrs from vwActivity_CSH_Detail h with (nolock) left outer join Map_GEO_Distributors C with (nolock) on c.Cust_Code=h.Trans_Detail_Info_Code where  cast(convert(varchar,ModTime,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and ModTime>'$Dt' and sf_code='$SF')) as t group by t.DtTime,t.lat ,t.lng ,t.Trans_Detail_Name,t.ChannelName,t.POB_Value,t.net_weight_value,t.GeoAddrs,t.DtTm,t.ordfld,t.typ,t.ListedDr_Address1,t. geo_lat,t.geo_log,t.StartOrder_Time,t.EndOrder_Time,t.Remarks,t.OrderType,t.addrs order by DtTm,ordfld,StartOrder_Time";			
		//$query = "select * from ((select replace(convert(varchar,Time,9),':000',' ') DtTime,m.lati lat,m.Long lng,Trans_Detail_Name,Doc_Spec_ShortName ChannelName,POB_Value,net_weight_value,GeoAddrs,Time DtTm,0 ordfld,1 typ ,concat(ListedDr_Address1,' ',ListedDr_Address2,' ',ListedDr_Address3, ', Phone No : ',coalesce(ListedDr_Phone,ListedDr_Mobile) ) ListedDr_Address1,c.lat geo_lat, c.long geo_log,isnull(convert(varchar,cast(m.StartOrder_Time as datetime),108),'')StartOrder_Time,isnull(convert(varchar,cast(m.EndOrder_Time as datetime),108),'')EndOrder_Time,Activity_Remarks Remarks,OrderTyp OrderType from vwActivity_MSL_Details m with (nolock) inner join Mas_ListedDr d with (nolock) on d.ListedDrCode=m.Trans_Detail_Info_Code left outer join map_GEO_Customers C with (nolock) on c.Cust_Code=d.ListedDrCode where cast(convert(varchar,Time,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and Time>'$Dt'  and m.sf_code='$SF')union all(select replace(convert(varchar,DtTm,9),':000',' ') DtTime,lat,Lon lng,'','','0','0','',DtTm,1,0 typ ,'' ListedDr_Address1,'' geo_lat,'' geo_log,'','','',''  from tbTrackLoction with (nolock) where SF_Code='$SF' and cast(convert(varchar,DtTm,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and DtTm>'$Dt' and cast(Auc as float)<=100 and cast(lat as float)>0 )union all(select replace(convert(varchar,Adt,9),':000',' ') DtTime,R.lat,R.Long lng,ListedDr_Name,Doc_Spec_ShortName,'0','0','',Adt,0,1 typ ,ListedDr_Address1,g.lat geo_lat,g.long geo_log,'','','','Review'  from Trans_OutletReview_Details R with (nolock) inner join Mas_ListedDr d with (nolock) on RetID=ListedDrCode left outer join map_GEO_Customers g with (nolock) on Cust_Code=RetID where SFCode='$SF' and cast(convert(varchar,Adt,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and Adt>'$Dt')union all( select replace(convert(varchar,ModTime,9),':000',' ') DtTime,h.lati lat,h.Long lng,Trans_Detail_Name, ''ChannelName,POB_Value,'0' net_weight_value,GeoAddrs,ModTime DtTm,0 ordfld,2 typ , Address ListedDr_Address1,c.lat geo_lat,c.long geo_log,isnull(convert(varchar,cast(h.StartOrder_Time as datetime),108),'')StartOrder_Time,isnull(convert(varchar,cast(h.EndOrder_Time as datetime),108),'')EndOrder_Time,Activity_Remarks,'' from vwActivity_CSH_Detail h with (nolock) left outer join Map_GEO_Distributors C with (nolock) on c.Cust_Code=h.Trans_Detail_Info_Code where  cast(convert(varchar,ModTime,101) as datetime)=cast(convert(varchar,cast('$Dt' as datetime),101) as datetime) and ModTime>'$Dt' and sf_code='$SF')) as t order by DtTm,ordfld";
...
$result = performQuery($query);
    ...
    case "get/mytp":
		$SF=$_GET['SF_Code'];
    	$Dt = $_GET['Dt'];
		$query = "select Wtype,Stockist_Name,ClstrName from vwMyDayPlanAll v inner join Mas_Salesforce S on S.Sf_Code=v.SF_Code inner join vwMas_WorkType_all w on w.type_code=worktype and SFTyp=sf_type inner join Mas_Stockist D on D.Distributor_Code=stockist where v.SF_Code='$SF' and Pln_date='$Dt'";
        $result = performQuery($query);
		outputJSON($result);
        break;
    case "deleteEntry":
...
$sfCode="Admin";
        ...
        break;
            ...
            case "TaxMaster":
	            $divCode = $_GET['divisionCode'];
                $query = "exec getTaxMaster '$divCode'";
                $results = performQuery($query);
                outputJSON($results);
                die;
                break;
            case "ProdTaxDets":
	            $divCode = $_GET['divisionCode'];
	            $stateCode = $_GET['State_Code'];
                $query = "exec getTaxProdSturct '$stateCode','$divCode'";
                $results = performQuery($query);
                outputJSON($results);
                die;
                break;
	...
	case "error/data":
		...
		$myfile = fopen("../server/outboxfile_db.txt", "a+");
		$txt = "John Doe\n";
