...
function GetRejectedList() {
	...
	$divisionCode = explode(",", $div);
    $dt = $_GET['date'];
	//$query = "select SLNo 'id', Prod_Code, CaseQty, PiceQty, Totalqty 'CurrentStock', StkId from Trans_Prod_DailyInventoryNative where Div_Id='". $divisionCode[0] . "' and SFCode='". $sfCode . "' and cast(EntryDt as date) = '". $dt . "' and ApproveFlag = '2'";
	$query = "select SLNo 'id', Prod_Code, CaseQty, PiceQty, Totalqty 'CurrentStock', StkId, tvr.Reject_Reason  
from Trans_Prod_DailyInventoryNative inner join Trans_VanReject tvr on tvr.Sf_Code = '". $sfCode . "' 
and cast(Reject_Date as date) = '". $dt . "' where Div_Id='". $divisionCode[0] . "' and SFCode='". $sfCode . "' and 
cast(EntryDt as date) = '". $dt . "' and ApproveFlag = '2'";
...
case "SSstockUpdation":
        ...
        case "DCRDetail_Distributors_Hunting":
            if($vals['slno']!=null ){
                ...
                $sql = "{call  svNewDistirbutorsHunting(" . $vals['name'] . "," . $vals['contact'] . "," . $vals['phone'] . "," . $vals["area"] . "," . $vals["remarks"] . "," . $vals["lat"] . "," . $vals["address"] . "," . $vals["long"] . ",'". date('Y-m-d H:i:s') . "','".$sfCode."')}";
                performQueryWP($sql,[]);
            }
...
$results=array();
 ...
 $flag=str_replace("'", "", $vals["c_flag"]);
       ...
       $sql="insert into Trans_Cus_Checkin Values('".$SfCode."','".str_replace("'", "", $vals["retailer_code"])."','".str_replace("'", "", $vals["checkin_time"])."','".str_replace("'", "", $vals["entry_date"])."','".str_replace("'", "", $vals["ekey"])."','".str_replace("'", "", $vals["CheckoutTime"])."','".str_replace("'", "", $vals["c_flag"])."')";
  performQuery($sql);
  $resp["success"] = true;
        echo json_encode($resp);
        die;
...
$product_unit_conv=$samp[$j]["product_unit_conv"];
  ...
  $sql="insert into Trans_Pop_Ledger values('".$SfCode."','".$SfCode."','".$productCode."','".$Qty."','".$StockType."','".$OrderDate."','".$OrderTime."')";
                   performQuery($sql);
   $query="Exec saveOutletDcr'".$SfCode."','".$Owndiv."','".$RetailerCode."','".$productCode."','".$Qty."','".$pkk."','".$OrderDate."','".$OrderTime."' ";
...
case "Map_GEO_Distributor":
 ...
 $tr1=performQuery($sql);
//if($Owndiv=="162"){
if($Owndiv=="162" && $tr1[0]['sf_type']=="1"){
$sql="select trans_slno from dcrmain_trans where sf_code='".$sfCode."' and cast(activity_date as date)='".$val["Enterdate"]."'";
 $tr=performQuery($sql);
...
if (!empty($data[1]["Activity_Doctor_Report"]['superstockistid']))
 $superstk=$data[1]["Activity_Doctor_Report"]['superstockistid'];
            ...
            if ($vTyp==7) {
                ...
                //below stored PROCEDURE is the old method used
...
break;
    ...
    case "get/circularlist":
        $query = "select FileName,FileSubject from file_info";
        $output = performQuery($query);
        // die;
        ...
        //         $results[] = ['file'=>$value,'size'=>filesize($path)];
        //     } 
        // }
        echo filesize("Salesred.png");
        outputJSON($output);
        break;
    case "get/BrndSumm":
        $data = json_decode($_POST['data'], true);
    ...
    case "table/list":
        ...
        switch (strtolower($data['tableName'])) {
			case "vwactivity_msl_details":
		     $sfCode = $_GET['Sf_Code'];
		     $query = "select Order_Flag,Trans_Sl_No,Order_Value,Activity_Date,Trans_Detail_Name,j.Stockist_Name stockist_name from DCRMain_Trans h with(nolock) 
		...
		$sql = "delete from Trans_PriOrder_Details WHERE Trans_Sl_No='".$orderNo."'";
		 ...
		 for ($i = 0; $i < count($transOrderDetails); $i++) {
			 ...
			 $CQty = $transOrderDetails[$i]['Qty'];
			  ...
			  $query="insert into Trans_PriOrder_Details Values('".$Trans_POrd_No."','".$orderNo."','".$product_code."','".$product_Name."','".$transOrderDetails[$i]['Qty']."','".$PQty."','".$value."','".$Rate."','".$Order_Flag."','".$Division_code."','".$cb_qty."','".$free."','".$discount_price."','".$discount."','0','0','','".$product_unit_name."','".$tax_price."','','','".$rx_qty."','".$OrdConv."')";
			  performQuery($query);
...
case "get/vwVstPendingPragnancy":
    ...
    case "get/newcalls":
       ...
       $div = $_GET['divisionCode'];
        ...
        $divs = explode(",", $div . ",");
        $Owndiv = (string) $divs[0];
	   $query = "select Pln_Date Adate,
		D.Trans_SlNo Acode,
		cluster,AC RCCOUNT,
		ClstrName RouteName,
		isnull(TC,0) calls,
		isnull(EC,0) [order],
		isnull(orderValue,0) orderVal
from (
	SELECT convert(varchar,Pln_Date,103) Pln_Date,ClstrName,TP.sf_code,cluster,count(ListedDrCode) AC FROM TbMyDayPlan TP 
	left outer join (
		select Territory_Code,sf_code SF,ListedDrCode from Mas_ListedDr 
			where ListedDr_Active_Flag=0 and division_code='".$Owndiv."') as L on 
				charindex(','+ TP.cluster+',',','+ cast(L.Territory_Code as varchar)+',')>0  
				and charindex(','+  TP.sf_member_code+',',','+ SF +',')>0
	where TP.Sf_Code='".$sfCode."' and convert(date,Pln_Date)='".$date."'  
	group by Pln_Date,TP.sf_code,cluster,ClstrName
	) as t 
left outer join (
	select convert(varchar,h.Activity_Date,103) Adate,h.Trans_SlNo,SDP,
	COUNT(Trans_Detail_Info_Code) TC,COUNT(Cust_Code) EC,cast(isnull(sum(order_Value),0) as decimal) orderValue from DCRMain_Trans h
			inner join DCRDetail_Lst_Trans d on h.Trans_SlNo=d.Trans_SlNo 
			left outer join Trans_Order_Head o on DCR_Code=d.Trans_Detail_Slno and Cust_Code=Trans_Detail_Info_Code
			where h.Sf_Code='".$sfCode."' and convert(date,h.Activity_Date)='".$date."'  
			GROUP BY convert(varchar,h.Activity_Date,103),h.Trans_SlNo,SDP
) as d on t.cluster=d.SDP and Pln_Date=Adate
group by Pln_Date,D.Trans_SlNo ,cluster,AC,ClstrName,isnull(TC,0),isnull(EC,0),isnull(orderValue,0)";
		$res = performQuery($query);
