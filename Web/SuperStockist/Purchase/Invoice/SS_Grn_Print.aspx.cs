...
public partial class SuperStockist_Purchase_Invoice_SS_Grn_Print : System.Web.UI.Page
{
    ...
    public static DataSet getgrndetails(string Order_ID, string Div_Code)
    {
        ...
        DataSet ds = null;
        string strQry = " select  Po_No AS Trans_Sl_No,CONVERT(varchar, GRN_Date, 103) as GRN_Date,CONVERT(varchar, Entry_Date, 103) as Entry_Date,LEFt(Supp_Name, 50) as Supp_Name,Po_No, CONVERT(varchar, Dispatch_Date, 103) as Dispatch_Date,gh.SF_Code,gh.Division_code,Net_Tot_Goods,Net_Tot_Tax,Net_Tot_Value,Division_Name,(Division_Add1 + ',' + Division_Add2 + ',' + Division_City + '-' + Division_Pincode)address,S_Name,isnull(ms.Addr,'') AS Stockist_Address,isnull(ms.Mobile,'') Stockist_Mobile,replace(isnull('','0'),'0','') as Sf_Email from trans_grn_head gh join mas_division md on gh.division_code = md.division_code join Supplier_Master ms on ms.S_No = gh.SF_Code where po_no = '" + Order_ID + "' and gh.division_code = '" + Div_Code + "'";
        try
        {
    ...
    public static DataSet getallgrnorderbystockist(string Stockist_Code, string Div_Code, string Order_No)
    {
        ...
        string strQry = "get_SS_grnorder_ordno '" + Order_No + "','" + Stockist_Code + "','" + Div_Code + "'";
