                                ...
                                if (Prod.length > 0) {
                                    ...
                                    for (var z = 0; z < filter_unit.length; z++) {
                                        if (filter_unit[z].Move_MailFolder_Id == Prod[0].Base_Unit_code) {
                                            ...
                                            units += "<li id=" + unitrate + " name='itms'  conf=" + unitconfac + "  conf=" + unitconfac + "  value='" + filter_unit[z].Move_MailFolder_Id + "'>" + filter_unit[z].Move_MailFolder_Name + "</li>";
        ...
        $(document).on("click", ".Spinner-Modal>ul>li", function () {
            ...
            if (pro_filter.length > 0) {
                ...
                if (ans.length > 0) {
                    ...
                    ori_p_code = row.find('.ddlProd ').find('option:selected').val();
                    unit = $(this).text();
                    cq = row.find('.txtQty').val()*row.find('.Con_fac').text();
                    fr = row.find('.fre1').text();
...
$.confirm({
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.d.length > 0) {
                                    ...
                                    $.confirm({
                                        title: 'Success!',
                                        content: 'Secondary Ordered Submitted Successfully!',
                                        type: 'green',
                                        typeAnimated: true,
                                        autoClose: 'action|8000',
                                        icon: 'fa fa-check fa-2x',
        ...
        $(document).on('keyup', '.txtQty', function (e) {
            ...
            CQ = $(this).val()*$(this).closest('tr').find('.Con_fac').text();
            ...
            //}
             ...
             $('#free_table').find('tbody tr').remove();
                ...
                existingData.forEach(function (data, index) {
                    var str = "<tr><td style='width: 14%;' class='td_id'>" + (index + 1) + "</td><td style='width:21%;'><input type='hidden' value=" + data.product_code + " id='apc'/>" + data.product_code + "</td><td style='left: 201px;width: 38%;' class='td_name'><input type='hidden' value=" + data.product_name + " id='apn'/>" + data.product_name + "</td><td style='left: 454px;' class='td_free'><input type='hidden' value=" + data.freecqty + " id='fr'/>" + (data.freecqty ) + "</td></tr>";
                    $('#free_table tbody').append(str);
                });
        }
