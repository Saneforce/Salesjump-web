...
public partial class MIS_Reports_distinct_skudetail : System.Web.UI.Page
{     
    ...
    public DataTable generateSecondaryExcel(string divcode, string sf_code, string FDate, string TDate)
    {
        prexc dc = new prexc();
        DataSet ss = null;
        DataSet dsGV = null;
        ...
        foreach (DataRow dr in dsGV.Tables[0].Rows)
        {
           ...
           // rw["Distributor Name"] = dr["Stockist_name"].ToString();
            rw["Retailer Name"] = dr["ListedDr_Name"].ToString();
