            ...
            $(document).ready(function () {
                ...
                $(document).on('blur', 'input[name=txtGood]', function () {
                    ...
                    if (Number(qty) < Number($(this).val())) {
                        ...
                        $(this).focus();
                        getscheme(pcode, $(this).val() * cv, un, tr, ff, disss, opcode, 0)
                        return false;
                    }
                ...
                function getscheme(pCode, cq, un, tr, ff, disss, opcode) {
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        var oferProductNa = row.querySelector(".ofer_product_na").value;
                        var oferProductUnit = row.querySelector(".ofer_product_unit").value;
                        var oferProductCode = row.querySelector(".ofer_product_code").value;
                    ...
                    // Create an object to store the sum of freecqty for each ofer_product_code
                    //var freecqtySum = {};
                    //// Get the table element
                    //var table = document.getElementById("ProductTable");
                    ...
                    //    //var existingRow = $('#free_table').find('#apc[value="' + oferProductCode + '"]').closest('tr');
                    //    var existingRow = $('#ProductTable').find('input.ofer_product_code[value="' + oferProductCode + '"]').closest('tr');

                    //    if (existingRow.length > 0) {
                    //        // Append freecqty to the existing row
                    //        var existingFreecqty = parseInt(existingRow.find('#fr').val()||0);
                    //        existingRow.find('#fr').val(existingFreecqty + freecqty);
                    //        existingRow.find('.td_free').text((existingFreecqty + freecqty) + ' ' + oferProductUnit);
                    //    } else {
                    //        // Create a new row
                    //        var str = "<tr><td style='width: 14%;' class='td_id'>" + (i + 1) + "</td><td style='width:21%;'><input type='hidden' value=" + oferProductCode + " id='apc'/>" + oferProductCode + "</td><td style='left: 201px;width: 38%;' class='td_name'><input type='hidden' value=" + oferProductNa + " id='apn'/>" + oferProductNa + "</td><td style='left: 454px;' class='td_free'><input type='hidden' value=" + freecqty + " id='fr'/>" + (freecqty + ' ' + oferProductUnit) + "</td></tr>";
                    //        $('#free_table tbody').append(str);
                    //    }
                    //}
                ...
                //                        if (arr[c].pppcode.indexOf(opcode) >= 0) {
                //                            if (arr[c].P_Free > 0) {
                //                                arr[c].P_Free = arr[c].P_Free - parseInt(parseFloat((cq / parseInt(res[0].Scheme))) * parseInt(res[0].Free));
                //                                $(tr).find('.fre').text('#' + opcode).text('0');
                //                                $(tr).find('.fre1').text('#' + opcode).text('0');
                //                                tr.find('.of_pro_name').val(0);
                //                                tr.find('.of_pro_code').val(0);
                    ...
                    if (buttoncount == "1") {
                        ...
                        $('#ProductTable >tbody >tr').each(function () {
                            ...
                            proArr.push({
                                ...
                                Remarks: 0,
                                free: $(this).find('#free').attr('value') || 0,
