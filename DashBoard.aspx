...
.close:focus {
  ...
  cursor: pointer;
    ...
    <script type="text/javascript" src="https://www.amcharts.com/lib/3/themes/light.js"></script>--%>
        <script type="text/javascript">
            Highcharts.maps["countries/in/in-all"] = {
                "title": "India", "version": "1.1.2", "type": "FeatureCollection", "copyright": "Copyright (c) 2015 Highsoft AS, Based on data from Natural Earth", "copyrightShort": "Natural Earth", "copyrightUrl": "http://www.naturalearthdata.com", "crs": { "type": "name", "properties": { "name": "urn:ogc:def:crs:EPSG:24373" } }, "hc-transform": { "default": { "crs": "+proj=lcc +lat_1=19 +lat_0=19 +lon_0=80 +k_0=0.99878641 +x_0=2743195.592233322 +y_0=914398.5307444407 +a=6377299.36559538 +b=6356098.359005156 +to_meter=0.9143985307444408 +no_defs", "scale": 0.000208805754963, "jsonres": 15.5, "jsonmarginX": -999, "jsonmarginY": 9851.0, "xoffset": 1676654.55745, "yoffset": 3026618.02308 } },
        ...
        <script language="javascript" type="text/javascript">
            $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    async: false,
                    url: "DashBoard.aspx/Get_access_master",
                    dataType: "json",
	...
	if(parseInt('<%=DBase_EReport.Global.ExpenseType%>')>0){
		 $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                async: true,
			...
			console.log(result);
                ...
                $.ajax({
                    ...
                    success: function (data) {
						...
						var perstate = $("#opState");
                        //ddlCustomers.empty().append('<option  selected="selected" value="0">State</option>');
						perstate.empty().append('<option  selected="selected" value="All">All State</option>');
                        $.each(data.d, function () {
                            ddlCustomers.append($("<option></option>").val(this['StateCode']).html(this['stateName']));
							perstate.append($("<option></option>").val(this['StateCode']).html(this['stateName']));
                        });
                    },
                    error: function (result) {
                        //alert(JSON.stringify(result));
            ...
            var priTopMonthAll = [];
            var priTopDayAll = [];
            var jsonarrv = [], ReverseArrayv = [], prijsonarrv = [], priReverseArrayv = [], TopDayv = [], BotDayv = [], priTopDayv = [], priBotDayv = [], TopMonthv = [], BotMonthv = [], priTopMonthv = [], priBotMonthv = [];
				...
				function genChartdatav(obj1, obj2, n) {
                // console.log(n);
                var TopArray = [];
                var BotArray = [];
                var len = obj1.length;
                ...
                genBarchart2('proTop', TopArray);
                genBarchart2('proBot', BotArray);
            }
			
			function genChartdatav1(obj1, obj2, n) {
                var TopArray = [];
                var BotArray = [];
                var len = obj1.length;
                if (len < n) {
                ...
                genBarchart2('terrTop', TopArray);
                genBarchart2('terrBot', BotArray);
            }
				...
				function genchartfrstateD() {
                var opSt = $('#opState').val();
                if (opSt != 'All') {
                    //state filter for Day
                    var length = TopDay.length;//S
                    TopDay = TopDay.filter(function (a) {
                        return a.state.indexOf(opSt) > -1;
                    });
                    var j = 0;
                    for (var i = length - 1; i >= 0; i--) {
                        BotDay.push(TopDay[i]);
                    ...
                    var length = priTopDay.length;//P
                    priTopDay = priTopDay.filter(function (a) {
                        return a.state.indexOf(opSt) > -1;
                    });
                    var j = 0;
                    for (var i = length - 1; i >= 0; i--) {
			...
			function genchartfrstateM() {
                var opSt = $('#opState').val();
                if (opSt != 'All') {
                    //state filter for Month
                    var length = TopMonth.length;//S
                    TopMonth = TopMonth.filter(function (a) {
                        return a.state.indexOf(opSt) > -1;
                    });
                    var j = 0;
                    for (var i = length - 1; i >= 0; i--) {
                    ...
                    var length = priTopMonth.length;//P
                    priTopMonth = priTopMonth.filter(function (a) {
                        return a.state.indexOf(opSt) > -1;
                    });
                    var j = 0;
                    for (var i = length - 1; i >= 0; i--) {
			...
			function loadqtyarray() {
                jsonarrv.push(jsonarr);
                jsonarrv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                jsonarrv[0].reverse();
                ReverseArrayv.push(ReverseArray);
                ReverseArrayv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                prijsonarrv.push(prijsonarr);
                prijsonarrv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                prijsonarrv[0].reverse();
                priReverseArrayv.push(priReverseArray);
                priReverseArrayv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                TopDayv.push(TopDay);
                TopDayv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                TopDayv[0].reverse();
                BotDayv.push(BotDay);
                BotDayv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                priTopDayv.push(priTopDay);
                priTopDayv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                priTopDayv[0].reverse();
                priBotDayv.push(priBotDay);
                priBotDayv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                TopMonthv.push(TopMonth);
                TopMonthv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                TopMonthv[0].reverse();
                BotMonthv.push(BotMonth);
                BotMonthv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                priTopMonthv.push(priTopMonth);
                priTopMonthv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
                priTopMonthv[0].reverse();
                priBotMonthv.push(priBotMonth);
                priBotMonthv[0].sort(function (a, b) {
                    return a.x - b.x;
                });
            }
			...
			function loadQty() {
                ...
                loadqtyarray();
                genChartdatav(jsonarrv[0], ReverseArrayv[0], opVal == 0 ? jsonarrv[0].length : opVal);
                genChartdatav1(prijsonarrv[0], priReverseArrayv[0], opVal == 0 ? prijsonarrv[0].length : opVal);
                if (Number(vL) == 1) {
                    genchartfrstateD();
                    genChartdatav(TopDayv[0], BotDayv[0], opVal == 0 ? TopDayv[0].length : opVal);
                    genChartdatav1(priTopDayv[0], priBotDayv[0], opVal == 0 ? priTopDayv[0].length : opVal);
                    TopDay = TopDayAll;
                    priTopDay = priTopDayAll;
                }
                else if (Number(vL) == 2) {
                    genchartfrstateM();
                    genChartdatav(TopMonthv[0], BotMonthv[0], opVal == 0 ? TopMonthv[0].length : opVal);
                    genChartdatav1(priTopMonthv[0], priBotMonthv[0], opVal == 0 ? priTopMonthv[0].length : opVal);
                    TopMonth = TopMonthAll;
                    priTopMonth = priTopMonthAll;
                }
            }
			
			 function loadvaluearray() {
                ...
                prijsonarr.sort(function (a, b) {
                    return a.y - b.y;
                });
                priReverseArray.sort(function (a, b) {
                    return a.y - b.y;
                });
                TopDay.sort(function (a, b) {
                    return a.y - b.y;
                });
                BotDay.sort(function (a, b) {
                    return a.y - b.y;
                });
                priTopDay.sort(function (a, b) {
                    return a.y - b.y;
                });
                priBotDay.sort(function (a, b) {
                    return a.y - b.y;
                });
                TopMonth.sort(function (a, b) {
                    return a.y - b.y;
                }); jsonarr.sort(function (a, b) {
                    return a.y - b.y;
                });
                BotMonth.sort(function (a, b) {
                    return a.y - b.y;
                });
                priTopMonth.sort(function (a, b) {
                    return a.y - b.y;
                });
                priBotMonth.sort(function (a, b) {
                    return a.y - b.y;
                });
            }
			...
			function loadValue() {
                ...
                var vL = $('#opTionType').val();
                loadvaluearray();
                genChartdata(jsonarr.reverse(), ReverseArray, opVal == 0 ? jsonarr.length : opVal);
                genChartdata1(prijsonarr.reverse(), priReverseArray, opVal == 0 ? prijsonarr.length : opVal);
                if (Number(vL) == 1) {
                    genchartfrstateD();
                    genChartdata(TopDay.reverse(), BotDay, opVal == 0 ? TopDay.length : opVal);
                    genChartdata1(priTopDay.reverse(), priBotDay, opVal == 0 ? priTopDay.length : opVal);
                    TopDay = TopDayAll;
                    priTopDay = priTopDayAll;
                }
                else if (Number(vL) == 2) {
                    genchartfrstateM();
                    genChartdata(TopMonth.reverse(), BotMonth, opVal == 0 ? TopMonth.length : opVal);
                    genChartdata1(priTopMonth.reverse(), priBotMonth, opVal == 0 ? priTopMonth.length : opVal);
                    TopMonth = TopMonthAll;
                    priTopMonth = priTopMonthAll;
                }
            }
			
			 $(document).on('click', '.pergrp', function () {
			...
			$(document).on('change', '#opState', function () {
                if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Qty") {
                        loadQty();
                    }
                if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Value") {
                    loadValue();
                }
				...
				$(document).on('click', '#proTop', function () {
                if ('<%= Session["division_code"] %>'=='29,') {
                    ...
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Qty") {
                        loadqtyarray();
                        genBarchart2('popupContent', jsonarrv[0]);
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart2('popupContent', TopDayv[0]);
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart2('popupContent', TopMonthv[0]);
                        }
                    }
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Value") {
                        loadvaluearray();
                        genBarchart1('popupContent', jsonarr.reverse());
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart1('popupContent', TopDay.reverse());
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart1('popupContent', TopMonth.reverse());
                        }
                    }
                    $('#modelabel').text("Secondary Order Product Wise TopOrder");
                }
               
                
            });
			
			$(document).on('click', '#proBot', function () {
                if ('<%= Session["division_code"] %>' == '29,') {
                    $('#myModal').modal('toggle');
                    var vL = $('#opTionType').val();
                    var modal = document.getElementById('myModal');
                    ...
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Qty") {
                        loadqtyarray();
                        genBarchart2('popupContent', ReverseArrayv[0]);
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart2('popupContent', BotDayv[0]);
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart2('popupContent', BotMonthv[0]);
                        }
                    }
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Value") {
                        loadvaluearray();
                        genBarchart1('popupContent', ReverseArray);
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart1('popupContent', BotDay);
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart1('popupContent', BotMonth);
                        }
                    }
                    $('#modelabel').text("Secondary Order Product Wise BottomOrder");
                }
            });
			
			$(document).on('click', '#terrTop', function () {
                if ('<%= Session["division_code"] %>' == '29,') {
                    $('#myModal').modal('toggle');
                    var vL = $('#opTionType').val();
                    var modal = document.getElementById('myModal');
                    ...
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Qty") {
                        loadqtyarray();
                        genBarchart2('popupContent', prijsonarrv[0]);
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart2('popupContent', priTopDayv[0]);
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart2('popupContent', priTopMonthv[0]);
                        }
                    }
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Value") {
                        loadvaluearray();
                        genBarchart1('popupContent', prijsonarr.reverse());
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart1('popupContent', priTopDay.reverse());
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart1('popupContent', priTopMonth.reverse());
                        }
                    }
                    $('#modelabel').text("Primary Order Product Wise TopOrder");
                }
            });
			
			$(document).on('click', '#terrBot', function () {
                if ('<%= Session["division_code"] %>' == '29,') {
                    $('#myModal').modal('toggle');
                    var vL = $('#opTionType').val();
                    var modal = document.getElementById('myModal');
                    ...
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Qty") {
                        loadqtyarray();
                        genBarchart2('popupContent', priReverseArrayv[0]);
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart2('popupContent', priBotDayv[0]);
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart2('popupContent', priBotMonthv[0]);
                        }
                    }
                    if ($('.pergrp').parent('li').parent('ul').siblings('a').find('span').text().trim() == "Value") {
                        loadvaluearray();
                        genBarchart1('popupContent', priReverseArray);
                        if (Number(vL) == 1) {
                            genchartfrstateD();
                            genBarchart1('popupContent', priBotDay);
                        }
                        else if (Number(vL) == 2) {
                            genchartfrstateM();
                            genBarchart1('popupContent', priBotMonth);
                        }
                    }
                    $('#modelabel').text("Primary Order Product Wise BottomOrder");
                }
            });
			...
			$('#btntimesClose').on('click', function () {
                ...
                $(document).on('click', '#BtnOrderP', function () {
                    ...
                    var sURL = "MIS Reports/rpt_Pri_Order_View.aspx?sf_code=" + sfCode + "&cur_month=" + TMonth + "&cur_year=" + TYear +
                        "&Mode=" + "TPMYDayPlan" + "&Sf_Name=" + sfCode + "&Date=" + CDate + "&ToDate=" + CDate + "&Type=" + sfType + "&subdiv=" + "<%=sub_divc%>";
                        window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
                    }
                });
                ...
                $(document).on('click', '#BtnOrder1', function () {


                    var sURL = "MIS Reports/SS_rpt_Total_Order_View.aspx?sf_code=" + sfCode + "&cur_month=" + TMonth + "&cur_year=" + TYear +
                        "&Mode=" + "TPMYDayPlan" + "&Sf_Name=" + sfCode + "&Date=" + CDate + "&Type=" + sfType + "&Sub_Div=" + "<%=sub_divc%>";
                    window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');

                });
		$(document).on('click', '#BtnOrderPs', function () {
                    
                    var sURL = "MIS Reports/rpt_SS_Pri_Order_View.aspx?sf_code=" + sfCode + "&cur_month=" + TMonth + "&cur_year=" + TYear +
                        "&Mode=" + "TPMYDayPlan" + "&Sf_Name=" + sfCode + "&Date=" + CDate + "&ToDate=" + CDate + "&Type=" + sfType + "&subdiv=" + "<%=sub_divc%>";
                        window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
                ...
                var stkchp = get_access_master[0].MR_SStockist_Nd;
                if (stkchp != '1') {
                    $('.stkup').hide();
                    $('.oldstk').removeClass('col-lg-2');
                    $('.oldstk').removeClass('col-md-2');

                    // Add the col-lg-4 class
                    $('.oldstk').addClass('col-lg-3');
                    $('.oldstk').addClass('col-md-3');
                }
                else {
                     $('.stkup').show();
                    $('.oldstk').removeClass('col-lg-3');
                    $('.oldstk').removeClass('col-md-3');

                    // Add the col-lg-4 class
                    $('.oldstk').addClass('col-lg-2');
                    $('.oldstk').addClass('col-md-2');
                }
        ...
        <div class="row1" style="position: absolute; top: 64px; right: 20px;">
            ...
            <input type="text" id="datepicker" class="form-control" style="min-width: 110px !important; width: 110px; display: inline-block; border: none; background-color: transparent; color: #336277; font-family: Verdana; font-weight: bold; font-size: 14px;" />
            <label class="caret" for="datepicker" style="position: relative; top: 7px;"></label>
        </div>
        <div class="row">
            <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12 oldstk" style="padding-right: 0;">
            ...
            <%-- superstockist start--%>
            <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12 stkup" style="padding-right: 0;">
                <div id="BtnOrderPs" class="info-box  hover-expand-effect" style="background-color:#5cb85c ">
                    <div class="icon">
                        <i class="fa fa-suitcase" aria-hidden="true"></i>
                    </div>
                    <div class="content">
                        <div class="number count-to" data-from="0" data-to="125" data-speed="15" data-fresh-interval="20">
                            <asp:Label ID="lbl_ssp" runat="server" Text="0"></asp:Label>
                        </div>
                        <div class="text">
                            SUPERSTOCKIST ORDERS
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12" style="padding-right: 0;display:none;">
                <div id="BtnOrderRs" class="info-box bg-orange  hover-expand-effect" style="background-color:#428bca">
                    <div class="icon">
                        <i class="fa fa-briefcase" aria-hidden="true"></i>
                    </div>
                    <div class="content">
                        <div class="number count-to" data-from="0" data-to="125" data-speed="15" data-fresh-interval="20">
                            <asp:Label ID="lbl_ssS" runat="server"></asp:Label>
                        </div>
                        <div class="text">
                            SS PRIMARY ORDERS
                        </div>
            ...
            <div class="col-md-12" style="padding: 0px;">
                ...
                <div class="tab-content clearfix">
                                    ...
                                    <div class="row" style="padding: 0px;">
                                        <ul class="nav navbar-nav">
                                            <li class="dropdown user user-menu"><a href="#" id="CATG" class="dropdown-toggle"
                                                data-toggle="dropdown" style="padding: 4px 18px"><i class="fa fa-bar-chart-o fa-fw"></i><span id="txt1">Category</span><i class="caret"></i> </a>
                    ...
                    <div class="tab-pane" id="2b">
					 ...
					 <div id="myModal" class="modal fade" style="z-index: 10000000; overflow-y: auto;" tabindex="0" aria-hidden="true" data-keyboard="false" data-backdrop="static">
                      <div class="modal-dialog" role="document" style="width: 80% !important;height:90% !important">
  <div class="modal-content" style="height:80%">
      <div class="modal-header">
                        <h5 class="modal-title" id="modelabel"></h5>
                    </div>
      <div class="modal-body" style="padding-top: 10px">
     <button type="button" id="btntimesClose" style="margin-top: 20px;float:right;" data-dismiss="modal" aria-label="Close"><span class="cls" style="color:red;">&times;</span></button>
        
        <div id="popupContent" class="ChartTop amchart-container"></div>
      </div>
  </div>
                    </div>
</div>
                        ...
                        <div class="col-md-12" style="padding: 0px;">
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    <div class="row">
                                        <div class="col-md-4">
                                            Secondary Order Product Wise
                                        </div>
                                        <div class="col-md-8" style="text-align: right">
										<ul class="nav navbar-nav">
                                            <li class="dropdown user user-menu"><a href="#" id="per" class="dropdown-toggle"
                                                data-toggle="dropdown" style="padding: 4px 10px"><i class="fa fa-bar-chart-o fa-fw"></i><span id="pertext">Value</span><i class="caret"></i> </a>
                                                <ul class="dropdown-menu dropdown-custom dropdown-menu-right">
                                                    <li><a href="#" class="pergrp">Qty</a></li>
                                                </ul>
                                            </li>
                                        </ul>
                                        <select id="opState">
        ...
        <div class="col-md-4">
            <div class="panel panel-primary">
                ...
                <a href="#" id="view_subwi" style="display: none;">
                    <div class="panel-footer">
                        <span class="pull-left">Division wise Worktype!</span> <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                        <div class="clearfix">
                        </div>
                    </div>
                </a>
                            ...
                            <div class="col-xs-3 text-right">
                                <div class="huge">
                                    <a href="#" id="view_New_Outlets" style="color: #fff">
                                        <asp:Label ID="Lbl_Sch_Call" runat="server" Text="0"></asp:Label></a>
                                </div>
                                ...
                                <ItemTemplate>
                                    ...
                                    <asp:Label ID="lblInput" runat="server" Width="255px" Style="padding-top: 10px; padding-left: 5px;"
                                        Text='<%# Eval("comment") %>'></asp:Label><span class="pull-right text-muted small"><em><asp:Label
                                            ID="daytime" runat="server" Text='<%# Eval("timee") %>' Style="font-style: bold; font-size: 10px; color: #a8b0b3;"></asp:Label><i class="fa fa-clock-o" style="color: #a6aeb1;"></i></em>
                                        </span>
        ...
        <script type="text/javascript">
			...
			function genBarchart2(Obj, arrDta) {
