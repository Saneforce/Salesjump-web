...
public partial class MIS_Reports_report_purchase_category_daywise : System.Web.UI.Page
{
...
leave_utility.HorizontalAlign = HorizontalAlign.Right;
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drdo in gg.Tables[0].Rows)
                    {
                       ...
                       // dsDrr = sd.purchase_categoryview_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
						  dsDrr = sd.catogory_wise_sale_value_stockist(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, day:jj , stockistcode: distributor, cat_code: cat_code, product_code: drdoctor["Product_Detail_Code"].ToString());
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    //    dsDrr = sd.purchase_categoryview_daywise_details_total(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
						    dsDrr = sd.catogory_wise_sale_value_stockist(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, day: jj, stockistcode: distributor,  product_code: drdoctor["Product_Detail_Code"].ToString());
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
