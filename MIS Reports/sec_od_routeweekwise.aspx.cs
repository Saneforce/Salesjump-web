...
public partial class MIS_Reports_sec_od_routeweekwise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
     ...
     Stock_name = Request.QueryString["Stockist_name"].ToString();
       ...
       // subdivision = Request.QueryString["subdivision"].ToString();
        ...
        lblHead.Text = "Secondary Order Routewise Weekwise for the Month of " + strFMonthName + " " + FYear;
...
leave_utility.Style.Add("text-align","center");
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
                ...
                lit_catg_namee.Text = " Week-" + i.ToString();
                dtype = "value";
                sURL = "Sec_od_routedaywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&distributor=" + distributor + "&Subdivision=" + subdivision + "&route_code=" + route_code + "&route_name=" + route_name + "&Dtype=" + dtype + "";
                lit_catg_namee.Attributes["onclick"] = "javascript:window.open('" + sURL + "' ,'','resizable=yes,toolbar=no,menubar=no,scrollbars=1,status=no,width=500,height=600,left=0,top=0,zoom=50%');";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                        dsDrr = sd.retail_route_view_weekise_details(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, route_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
