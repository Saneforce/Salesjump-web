...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drdo in gg.Tables[0].Rows)
                    {
                        ...
                        dsDrr = sd.Secondary_sales_product_value_daywise(div_code, FMonth, FYear,jj , drdoctor["Product_Detail_Code"].ToString(), distributor,subdivision);
 if (dsDrr.Tables[0].Rows.Count > 0)
                        ...
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
                            ...
                            lit_det_sale_value.Text =drrdoctor["sale"]==DBNull.Value ? drrdoctor["sale"].ToString(): Convert.ToDecimal(drrdoctor["sale"]).ToString("0.00");
