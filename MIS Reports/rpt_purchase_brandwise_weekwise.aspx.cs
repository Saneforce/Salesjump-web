...
public partial class MIS_Reports_rpt_purchase_brandwise_weekwise : System.Web.UI.Page
{
...
Brand_name = Request.QueryString["Brand_name"].ToString();
 ...
 subdivision = Request.QueryString["subdivision"].ToString();
        ...
        lblHead.Text = "Purchase BrandWise Details for the Month of " + strFMonthName + " " + FYear;
...
leave_utility.HorizontalAlign = HorizontalAlign.Center;
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
                ...
                dtype="total";
                sURL = "report_purchase_brand_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&dist_code=" + distributor + "&Product_Brand_Code=" + Brandcode+ "&Product_Cat_code="+cat_code+"&Dtype="+ dtype +"&subdivision="+subdivision+"";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                     ...
                     //   dsDrr = sd.purchase_Brandview_weekise_details_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
						  dsDrr = sd.Purchase_Brand_value_categorywise(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, week: i, cat_code: cat_code, stockistcode: distributor, product_code: drdoctor["Product_Detail_Code"].ToString());
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                      //  dsDrr = sd.purchase_Brandview_weekise_details(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code,Brandcode);
				         dsDrr = sd.Purchase_Brand_value_categorywise(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, week: i, cat_code: cat_code, brandcode: Brandcode, stockistcode: distributor, product_code: drdoctor["Product_Detail_Code"].ToString());
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
