...
public partial class MIS_Reports_rpt_retail_statewise_product_detail : System.Web.UI.Page
{
    DataSet dsDr = null;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       stype = Request.QueryString["stype"];
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Retail Register statewise Product Detail for the Period of " + strMonthName + " " + Year + " ";
...
if (stype == "1")
        {
   dsDr = dr.retail_Statewise_purchase_productdetail(div_code, statecode, Convert.ToInt16(Year), Convert.ToInt16(Month),subdivision);
        }

        if (stype == "2")
        {
dsDr = dr.retail_Statewise_purchase_productdetail_total(div_code, Convert.ToInt16(Year), Convert.ToInt16(Month),subdivision);
        }
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                lit_det_dr_hq.Text =drdoctor["value"]==DBNull.Value ? drdoctor["value"].ToString():Convert.ToDecimal( drdoctor["value"]).ToString("0.00");// drdoctor["value"].ToString();
