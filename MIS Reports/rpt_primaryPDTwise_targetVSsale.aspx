<%@ Page Title="" Language="C#" AutoEventWireup="true" CodeFile="rpt_primaryPDTwise_targetVSsale.aspx.cs"
    Inherits="MIS_Reports_rpt_primaryPDTwise_targetVSsale" %>
<!DOCTYPE html>
        ...
        $(document).ready(function () {
            ...
            genReport = function () {
                if (pDs.length > 0) {
                    ...
                    var ogVSTotQ = [];
					var stkcnt = 0;
					 var nmofstk = Dtls.filter(function (o) {
                        if (o.rwnum == '1') {
                            stkcnt++;
                            return o;
                        }
					 ...
					 for (var s = 0; s < stkcnt; s++) {
                        var stkrw = '<tr><td style="color:red;"><b>' + nmofstk[s].stknm + '</b></td></tr>';
                        $(tbl).find('tbody').append(stkrw);
                        stkrw = "";
                    ...
                    for (var i = 0; i < pDs.length; i++) {
                        ...
                        while (md <= td) {
                            cR = Dtls.filter(function (obj) {
                                return (Number(obj.cmonth) === Number((md.getMonth() + 1)) && Number(obj.cyear) === Number(md.getFullYear()) && obj.pCode === pDs[i].product_id && obj.stknm===nmofstk[s].stknm);
                            });
                            var T = '';
                            var S = '';
                            var TQ = '';
                            ...
                            if (md.getMonth() % 2 == 0) {
                                str += '<td  style="background-color: #b1adae;text-align: right;">' + TQ + '</td><td  style="background-color: #b1adae;text-align: right;">' + TPQ + '</td><td  style="background-color: #b1adae;text-align: right;">' + T + '</td><td style="background-color: #b1adae;text-align: right;">' + SQ + '</td><td style="background-color: #b1adae;text-align: right;">' + SPQ + '</td><td  style="background-color: #b1adae;text-align: right;">' + S + '</td>';
                            }
                            else {
                                str += '<td style="text-align: right;">' + TQ + '</td><td style="text-align: right;">' + TPQ + '</td><td style="text-align: right;">' + T + '</td><td style="text-align: right;">' + SQ + '</td><td style="text-align: right;">' + SPQ + '</td><td style="text-align: right;">' + S + '</td>';
                            }
						...
						if (str != "" && tst==1) {
                        $(tbl).find('tbody').append('<tr>' + str + ' <td style="background-color: #b3a46d; color: white;text-align: right;"> ' + TTQ + ' </td><td style="background-color: #b3a46d; color: white;text-align: right;"> ' + TTPQ + ' </td> <td style="background-color: #b3a46d; color: white;text-align: right;"> ' + TT.toFixed(2) + ' </td><td style="background-color: #b3a46d; color: white;text-align: right;">' + TSQ + '</td><td style="background-color: #b3a46d; color: white;text-align: right;">' + TSPQ + '</td> <td style="background-color: #b3a46d; color: white;text-align: right;">' + TS.toFixed(2) + '</td> </tr> ');
						}
            ...
            $.ajax({
                ...
                async: false,
                url: "rpt_primaryPDTwise_targetVSsale.aspx/getProductTargetSale",
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "rpt_primaryPDTwise_targetVSsale.aspx/getdata",
                data: "{'SubDiv':'" + SubDiv + "'}",
                dataType: "json",
                success: function (data) {
                    pDs = data.d;
