...
public partial class MIS_Reports_rpt_single_retail_channel : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Doc_Special_Code = Request.QueryString["product_code"].ToString();
        ...
        lblHead.Text = "Retail Trend Analysis Details for  " + strFMonthName + " " + FYear + "To" + " " + strTMonthName + " " + TYear;
    ...
    private void FillChannelwise()
    {
        ...
        //public DataSet Retail_Trend_analysis_Channelwise_month(string subdivision, string div_code, string Doc_Special_Code, string fromdate="", string todate="")
        dsSalesForce = sf.Retail_Trend_analysis_Channelwise_month(subdivision, divcode, Doc_Special_Code);
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
            ...
            {
                tot_per += row["value"] == DBNull.Value ? 0 : Convert.ToDecimal(row["value"]);
            }
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                      ...
                      //   public DataSet Retail_Trend_analysis_Channelwise_month(string subdivision, string div_code, string Doc_Special_Code, string cmonth = "", string cyear = "", string ListedDrCode="")

                        dsDoc = sf.Retail_Trend_analysis_Channelwise_month(div_code: divcode, subdivision: subdivision, cmonth: cmonth, cyear: cyear, Doc_Special_Code: Doc_Special_Code,ListedDrCode: drFF["ListedDrCode"].ToString());//(drFF["ListedDrCode"].ToString(), divcode, cmonth, cyear, Stockist_Code);


                        if (dsDoc.Tables[0].Rows.Count > 0)
                        {
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(2).ToString();
                            sec_value = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(3).ToString();
                        }
                        else
                        {
                            tot_dr = tot_dr = string.Empty;
                            sec_value = tot_dr = string.Empty;
                        }
                        if (tot_dr != "")
                        {
                        ...
                        tot_value = tot_per.ToString("0.00") ;
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        dsDoc = sf.Retail_Trend_analysis_Channelwise_month(div_code: divcode, subdivision: subdivision, cmonth: cmonth, cyear: cyear, Doc_Special_Code: Doc_Special_Code, ListedDrCode: drFF["ListedDrCode"].ToString());
                        
                        //dsDoc = sf.Retail_Trend_analysis_stockist_monthwise_distribution(drFF["Product_Detail_Code"].ToString(), divcode, cmonth, cyear, Stockist_Code);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                        {
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(2).ToString();
                            sec_value = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(3).ToString();
                        ...
                        {
                            tot_dr = string.Empty;
                            sec_value = string.Empty;
                        }
                        if (sec_value != "")
                        {
                            iTotLstnetval += (Decimal.Parse(sec_value));
                        }
                    ...
                    TableCell tc_det_currentmonth_tot = new TableCell();
                    tc_det_currentmonth_tot.Width = 200;

                    tc_det_currentmonth_tot.HorizontalAlign = HorizontalAlign.Right;
                    Literal lit_det_mon_tot = new Literal();
                    //lit_det_FF.Text = "&nbsp;" + drFF["Stockist_Name"].ToString();
                    tc_det_currentmonth_tot.BorderStyle = BorderStyle.Solid;
                    tc_det_currentmonth_tot.BorderWidth = 1;
                    tc_det_currentmonth_tot.Style.Add("font-size", "10pt");
                    tc_det_currentmonth_tot.Attributes.Add("Class", "rptCellBorder");
                    tc_det_currentmonth_tot.Controls.Add(lit_det_mon_tot);
                    tc_det_currentmonth_tot.Attributes.Add("style", "font-weight:bold;");
                    tr_total.Cells.Add(tc_det_currentmonth_tot);

                    tot_value = tot_per.ToString("0.00");
                    if (tot_value != "" && tot_value != "0")
                    {
                        value2 = (Decimal.Parse(tot_value));
                        decimal per = iTotLstCount / value2;
                        val = ( per * 100);
                        decimal value = Math.Round(val, 0);
                        lit_det_mon_tot.Text = value.ToString("0.00");

                    }
                    else
                    {
                        lit_det_mon_tot.Text = "";
                    }
