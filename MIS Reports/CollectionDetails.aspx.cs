...
public partial class MasterFiles_CollectionDetails : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void btnGo_Click(object sender, EventArgs e)
    {
        ...
        string FromDt = Convert.ToDateTime(txtFromDate.Value).ToString("yyyy/MM/dd");
        string ToDt = Convert.ToDateTime(txtToDate.Value).ToString("yyyy/MM/dd");
        string SubDiv = subdiv.SelectedValue.ToString();
        DataSet dsBillPending = iv.getCollectionDetailsSFWise(div_code, sf_code, FromDt, ToDt, SubDiv);
        ...
        if (dsBillPending.Tables[0].Rows.Count > 0)
        {
            ...
            decimal total = dsBillPending.Tables[0].AsEnumerable().Sum(row => row.Field<decimal>("BillAmt"));
