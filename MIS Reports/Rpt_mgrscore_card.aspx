<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Rpt_mgrscore_card.aspx.cs" Inherits="MIS_Reports_Rpt_mgrscore_card" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>mgrscorecard</title>
...
<body>
    <form id="form1" runat="server">
        ...
        <div class="row">
            <div class="col-sm-8">
                <asp:Label ID="Label1" runat="server" Text="Managers Score card" Style="margin-left: 10px; font-size: x-large"></asp:Label>
    ...
    <script type="text/javascript">
        ...
        var tcpob = []; var sfusers = []; var newstr = []; var tlsdcnt = []; var bt = []; 
        ...
        var fd = $('#<%=hFdate.ClientID%>').val();
        var td = $('#<%=htdate.ClientID%>').val();
        ...
        function gettcpobval() {
            $.ajax({
                ...
                async: false,
                url: "Rpt_mgrscore_card.aspx/gettcpob",
                data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
        ...
        function getUsers() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Rpt_mgrscore_card.aspx/getSFdets",
                data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {
         ...
         function getnewstore() {
                ...
                contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "Rpt_mgrscore_card.aspx/getnewstor",
                    data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {
        ...
        function getunbilledstore() {
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                    url: "Rpt_mgrscore_card.aspx/gettlsdcount",
                    data: "{'divcode':'" + div_code + "','sfc':'" + sf + "','fdate':'" + fd + "','tdate':'" + td + "','sub':'" + sdv + "'}",
                dataType: "json",
                success: function (data) {
        ...
        function ReloadTable() {
            ...
            var sth = '';
            var str = '';
            sth += "<tr><th rowspan='2'>S.NO</th><th rowspan='2'>Managers Name</th><th rowspan='2'>No of So/Fo/Asm</th><th rowspan='2'>Total Working Days</th><th rowspan='2'>No of Days Worked</th><th rowspan='2'>AC</th>";
            sth += "<th rowspan='2'>TC</th><th rowspan='2'>PC</th><th rowspan='2'>%</th><th rowspan='2'>Value</th>";
            sth += "<th rowspan='2'>Unbilled Stores</th><th rowspan='2'>% To AC</th><th rowspan='2'>new Stores</th><th rowspan='2'>Value</th><th rowspan='2'>TLSD</th><th rowspan='2'>No of Ws</th><th rowspan='2'>Value</th>";
            sth += "</tr>";
            ...
            for (var i = 0; i < sfusers.length; i++) {
                ...
                str += "<tr colspan=2><td>" + (i + 1) + "</td><td>" + sfusers[i].rsf + "</td><td>" + sfusers[i].ff + "</td><td>" + sfusers[i].Total_Days + "</td><td>" + sfusers[i].wdays + "</td><td>" + sfusers[i].ac + "</td>";
               
                let filtarr = tcpob.filter(function (a) {
                    return a.rsf == sfusers[i].rsfc;
                    });
                pctcper = (filtarr.length > 0 ? ((filtarr[0].PC / filtarr[0].TC) * 100).toFixed(2) : 0);

                let filtlsd = tlsdcnt.filter(function (a) {
                    return a.sf_Code == sfusers[i].rsfc;
                });
            ...
            $('#btnExport').click(function () {
                ...
                var tets = 'managerscorecard' + '.xls';   //create fname
