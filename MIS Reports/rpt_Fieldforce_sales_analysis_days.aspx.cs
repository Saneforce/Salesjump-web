...
public partial class MIS_Reports_rpt_Fieldforce_sales_analysis_days : System.Web.UI.Page
{
    ...
    private void FillSF()
    {
        ...
        DataSet DsRetailer = Get_Fieldforce_sal_Days(divcode, FYear, FMonth, sfCode);
        ...
        for (int k = 1; k < DsRetailer.Tables[0].Columns.Count - 4; k++)
        {
            dsData.Columns.Add((k).ToString(), typeof(string));
        }
        dsData.Columns.Add("Total", typeof(string));
        dsData.Rows.Add("1", "Order Given FieldForce");
        ...
        foreach (DataRow dr in DsState.Tables[0].Rows)
        {
            ...
            if (drow.Length > 0)
            {
                ...
                {
                    string f1 = row[0].ToString();
                    string f2 = row[1].ToString();
                    string f3 = row[2].ToString();                                  
                    string f4 = row[3].ToString();                                  
                     
                    //string f5 = row[5].ToString();
                   // string f6 = "";
                    dsData.Rows.Add(f1, f2, f3,f4);
                    int count = dsData.Rows.Count;
                    int m = 0;
                    decimal rtot = 0;
                    for (int k = 5; k < DsRetailer.Tables[0].Columns.Count; k++)
                    {
                        decimal str = row[k] == DBNull.Value ? 0 : Convert.ToDecimal(row[k]);
                        dsData.Rows[count - 1][k - 1] = str;
                        if (str > 0)
                            over_count[k - 5]++;
                        tot[k - 5] += str <= 0 ? 0 : Convert.ToDecimal(str);
                        rtot += str;
                        m = k;
                    }
                    dsData.Rows[count - 1][m] = rtot;
                }
            }
            ...
            for (int k = 5; k < DsRetailer.Tables[0].Columns.Count; k++)
            {
                dsData.Rows[counts - 1][k - 1] = tot[k - 5].ToString();
                over_tot[k - 5] += Convert.ToDecimal(tot[k - 5]);
                trtot += tot[k - 5];
                mm = k;
            }
            dsData.Rows[counts - 1][mm] = trtot;
        }
        ...
        for (int k = 5; k < DsRetailer.Tables[0].Columns.Count; k++)
        {
            dsData.Rows[1][k - 1] = over_tot[k - 5];
            ttot += over_tot[k - 5];
            j = k;
        }
        dsData.Rows[1][j] = ttot;
        ...
        for (int k = 5; k < DsRetailer.Tables[0].Columns.Count; k++)
        {
            dsData.Rows[0][k - 1] = over_count[k - 5];
            ctot += over_count[k - 5];
            j = k;
        }
        dsData.Rows[0][j] = ctot;
    ...
    public DataSet Get_Fieldforce_sal_Days(string Div_Code, string fyear, string months, string SF_Code = "0")
    {
        string strQry = string.Empty;
        DataSet dsAdmin = null;
        DBase_EReport.DB_EReporting db_ER = new DBase_EReport.DB_EReporting();

        strQry = "exec Get_Fieldforce_sal_days '" + Div_Code + "','" + fyear + "','" + months + "','" + SF_Code + "'";
    ...
    public DataSet get_State_Name(string divcode, string sf_Code, string subdiv)
    {
        string strQry = string.Empty;
