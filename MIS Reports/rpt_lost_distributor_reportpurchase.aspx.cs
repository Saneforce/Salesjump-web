...
public partial class MIS_Reports_rpt_lost_distributor_reportpurchase : System.Web.UI.Page
{
    ...
    string feildforcecode = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       string fcode = Request.QueryString["feildforce"].ToString();
     feildforcecode = fcode.Trim();
        System.Globalization.DateTimeFormatInfo mfi = new System.Globalization.DateTimeFormatInfo();
        string strFMonthName = mfi.GetMonthName(Convert.ToInt16(FMonth)).ToString().Substring(0, 3);
        string strTMonthName = mfi.GetMonthName(Convert.ToInt16(TMonth)).ToString().Substring(0, 3);

        lblHead.Text = "Lost Distributor  Details for  " + strFMonthName + " " + FYear + " " + "To" + " " + strTMonthName + " " + TYear;


    
        lblfield.Text=feildforcecode;
    ...
    private void FillSF()
    {
        ...
        SalesForce sf = new SalesForce();
        Order sd = new Order();
        dsSalesForce = sd.lost_distributor_feildforcewise(divcode, feildforcecode);
        ...
        if (dsSalesForce.Tables[0].Rows.Count > 0)
        {
           ...
           tr_header.BackColor = System.Drawing.Color.FromName("#496a9a");
            ...
            tc_DR_Name.RowSpan = 1;
            Literal lit_DR_Name = new Literal();
            lit_DR_Name.Text = "<center>Fieldforce Name</center>";
            tc_DR_Name.BorderColor = System.Drawing.Color.Black;
            tc_DR_Name.Attributes.Add("Class", "rptCellBorder");
            tc_DR_Name.Controls.Add(lit_DR_Name);
            tr_header.Cells.Add(tc_DR_Name);
            ...
            lit_month.Text = "Non-Purchased Distributor";
            tc_month.Attributes.Add("Class", "rptCellBorder");
            tc_month.BorderStyle = BorderStyle.Solid;
            tc_month.BorderWidth = 1;
            tc_month.HorizontalAlign = HorizontalAlign.Center;
            ...
            foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
            {
                ...
                dsDoc = sf.lost_value_stockist_purch(drFF["Sf_Code"].ToString(), divcode, st, tom, Subdivision);
                ...
                {
                    hyp_lst_month.Text = tot_dr;
                   sURL = "Rpt_lost_distributo_view.aspx?Sf_Code=" + drFF["Sf_Code"] + "&Sf_name=" + drFF["sf_name"].ToString() + " &FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + "&subdivision=" + Subdivision + "&tdate=" + tom + "&fdate=" + st + "";
                   hyp_lst_month.Attributes["onclick"] = "javascript:window.open('" + sURL + "' ,'','resizable=yes,toolbar=no,menubar=no,scrollbars=1,status=no,width=500,height=600,left=0,top=0,zoom=50%');";
                   hyp_lst_month.NavigateUrl = "#";
                    if (tot_dr != "")
                    {
