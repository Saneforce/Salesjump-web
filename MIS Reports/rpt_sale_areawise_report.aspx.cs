...
public partial class MIS_Reports_rpt_sale_areawise_report : System.Web.UI.Page
{
...
sfCode = Session["sf_code"].ToString();
        ...
        statecode = Request.QueryString["State_Code"].ToString();
       statename =Request.QueryString["State_name"].ToString();
        ...
        lblHead.Text = "Sale Register -Area Wise for  " + strFMonthName + " " + FYear + " " + "To" + " " + strTMonthName + " " + TYear;
    ...
    private void FillSF()
    {
     ...
     string stCrtDtaPnt = string.Empty;
        ...
        State st = new State();
        dsSalesForce = st.get_area_Details_Statewise(statecode, divcode);
...
stCrtDtaPnt+="{label:\""+ drFF["Area_Name"].ToString()+"\",y:";
                ...
                sURL = "rpt_sale_zonewise_report.aspx?Area_Code=" + drFF["Area_code"] + "&Area_name=" + drFF["Area_Name"].ToString() + "&statename="+ statename +"&FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + "&sCurrentDate=" + sCurrentDate + "&subdivision=" + subdivision + "";
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                     ...
                     //   dsDoc = sf.sale_areawise_purchase_value(drFF["Area_code"].ToString(), divcode, cmonth, cyear, subdivision);
          dsDoc = sf.sale_areawise_purchase_value( areacode: drFF["Area_code"].ToString(), div_code: divcode, cmonth: cmonth,cyear: cyear,subdivision: subdivision,sf_cdoe:sfCode);
 ...
 stype = "1";
                            ...
                            sURL = "rpt_sale_areawise_product_detail.aspx?Area_Code=" + drFF["Area_Code"] + "&Area_name=" + drFF["Area_Name"].ToString() + " &CYear=" + cyear + "&CMonth=" + cmonth + "&stype=" + stype + "&subdivision=" + subdivision + "";
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                       ...
                       // dsDoc = sf.sale_areawise_purchase_value(drFF["Area_Code"].ToString(), divcode, cmonth, cyear, subdivision);
                        ...
                        //dsDoc = sf.Purchase_Distributor_value(drFF["Stockist_Code"].ToString(), divcode, cmonth, cyear, subdivision);
          dsDoc = sf.sale_areawise_purchase_value( areacode: drFF["Area_code"].ToString(), div_code: divcode, cmonth: cmonth,cyear: cyear,subdivision: subdivision,sf_cdoe:sfCode);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
