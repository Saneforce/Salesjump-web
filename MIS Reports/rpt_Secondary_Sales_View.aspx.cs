       ...
       leave_utility.HorizontalAlign = HorizontalAlign.Center;
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee;i++)
            {
                ...
                sURL = "rpt_secondary_sale_view_weekday.aspx?weeknumber=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&fldforcename="+feild_force_name+"&distributor=" + distri + "&dist_code=" + distributor + "&Subdivision=" + subdivision + "";
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            if (getweek.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 1; i < wee; i++)
                {
                    ...
                    lit_det_head_OP.Text = "<b>Opening stock</b>";
                    ...
                    lit_det_head_quai.Text = "<b>Closing stock</b>";
               ...
               // tc_det_sfcode.Attributes.Add("Class", "rptCellBorder");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                        ...
                        dsDrr = sd.Secondary_sales_product_value(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor,subdivision);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
                                ...
                                lit_det_dr_name.Text =drrdoctor["opening"]==DBNull.Value ? drrdoctor["opening"].ToString(): Convert.ToDecimal(drrdoctor["opening"]).ToString("0.00");
                                ...
                                tc_det_Closing.HorizontalAlign = HorizontalAlign.Right;
                                tc_det_Closing.Width = 20;
                                Literal lit_det_dr_closing = new Literal();
                                lit_det_dr_closing.Text =drrdoctor["closing"]==DBNull.Value ? drrdoctor["closing"].ToString():Convert.ToDecimal( drrdoctor["closing"]).ToString("0.00");
                                ...
                                tc_det_sale_value.HorizontalAlign = HorizontalAlign.Right;
                                tc_det_sale_value.Width = 20;
                                Literal lit_det_sale_value = new Literal();
                                lit_det_sale_value.Text =drrdoctor["saleqty"]==DBNull.Value ? drrdoctor["saleqty"].ToString():Convert.ToDecimal( drrdoctor["saleqty"]).ToString("0.00");
                                ...
                                tc_det_tot_value.HorizontalAlign = HorizontalAlign.Right;
                                tc_det_tot_value.Width = 20;
                                Literal lit_det_tot_value = new Literal();
                                lit_det_tot_value.Text =drrdoctor["value"]==DBNull.Value ? drrdoctor["value"].ToString(): Convert.ToDecimal(drrdoctor["value"]).ToString("0.00");
                                // tc_det_sale_value.Attributes.Add("Class", "tblRow");
                ...
                if (FlgEmtyRow==0) DGVFFO.Rows.Add(tr_det_sno);
            }
