...
public partial class rpt_purchase_category_view_weekwise : System.Web.UI.Page
{
...
string subdivision=string.Empty;
 ...
 subdivision = Request.QueryString["subdivision"].ToString();
        ...
        lblHead.Text = "CategoryWise Details for the Month of " + strFMonthName + " " + FYear;
...
leave_utility.HorizontalAlign = HorizontalAlign.Center;
      ...
      dtype="value";
                sURL = "report_purchase_category_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&dist_code=" + distributor + "&Product_Cat_Code=" + cat_code + "&subdivision="+subdivision+"&Dtype="+ dtype +"";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                       ...
                       // dsDrr = sd.purchase_categoryview_weekise_details(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
    dsDrr = sd.catogory_wise_sale_value_stockist(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, week: i, stockistcode: distributor, cat_code: cat_code, product_code: drdoctor["Product_Detail_Code"].ToString());
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        SalesForce sd = new SalesForce();
                       // dsDrr = sd.purchase_categoryview_weekise_details_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
						  dsDrr = sd.catogory_wise_sale_value_stockist(divcode: div_code, sf_code: sf_code, cyear: FYear, cmonth: FMonth, week: i, stockistcode: distributor, product_code: drdoctor["Product_Detail_Code"].ToString());
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
