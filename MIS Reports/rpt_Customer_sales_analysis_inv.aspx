<%@ Page Language="C#" AutoEventWireup="true" CodeFile="rpt_Customer_sales_analysis_inv.aspx.cs" Inherits="MIS_Reports_rpt_Customer_sales_analysis_inv" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                async: false,
                url: "rpt_Customer_sales_analysis_inv.aspx/GetVisitedDtls",
                    ...
                    $('#<%=GV_DATA.ClientID%> tr').each(function () {
                        ...
                        if (vD.length > 0) {
                            $(vD).each(function (index, D) {
                                $(mRow).find('td').eq(7).text('Yes');
                                if (Number($(mRow).find('td').eq(Number(D.months) + 7).text()) <= 0) {
                                    $(mRow).find('td').eq(Number(D.months) + 7).css('background-color', '#8ebf8e');
                                    $(mRow).find('td').eq(Number(D.months) + 7).text(Number($(mRow).find('td').eq(Number(D.months) + 7).text()));
                                }
                            });
                        }
