...
public partial class MIS_Reports_rptPurchas_Register_Distributor_wise : System.Web.UI.Page
{
   ...
   string subdivision_code = string.Empty; 
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       subdivision_code = Request.QueryString["subdivision"].ToString();
        ...
        lblHead.Text = "Purchase Register -Distributor Wise for  " + strFMonthName + " " + FYear + " " + "To" + " " + strTMonthName + " " + TYear;
    ...
    private void FillSF_MR()
    {
        ...
        dsSalesForce = sf.GetDistNamewise_MR(divcode, subdivision_code, sfCode);
...
tc_DR_Des1.HorizontalAlign = HorizontalAlign.Center;
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                sURL = "rptPurchas_Register_Category_wise.aspx?Stockist_Code=" + drFF["Stockist_Code"] + "&Stockist_name=" + drFF["Stockist_Name"].ToString() + " &FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + "&sCurrentDate=" + sCurrentDate + "&subdivision=" + subdivision_code + "";
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        dsDoc = sf.Purchase_Distributor_value(drFF["Stockist_Code"].ToString(), divcode, cmonth, cyear, sCurrentDate,subdivision_code,sf_code:sfCode);
                        ...
                        if (tot_dr != "0")
                        {
                            ...
                            stype = "1";
                            stURL = "rpt_Purchase_view1.aspx?Stockist_Code=" + drFF["Stockist_Code"] + "&Stockist_name=" + drFF["Stockist_Name"].ToString() + " &Year=" + cyear + "&Month=" + cmonth + " &stype=" + stype + "&subdivision=" + subdivision_code + "";
                    ...
                    if (iTotLstCount1 != 0)
                    {
                        ...
                        Decimal per = Math.Round(Convert.ToDecimal(iTotLstCount1.ToString()), 2);
                        hyp_lst_month12.Text = per.ToString("0.00");
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        ...
                        dsDoc = sf.Purchase_Distributor_value(drFF["Stockist_Code"].ToString(), divcode, cmonth, cyear, sCurrentDate,subdivision_code,sfCode);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
                ...
                Decimal per = Math.Round(Convert.ToDecimal(iTotLstCount2.ToString()), 2);
                hyp_monthq.Text = per.ToString("0.00");
    ...
    private void FillSF()
    {
        ...
        {
          //  dsSalesForce = sf.GetStockist_subdivisionwise(divcode, subdivision_code, sfCode);
		     dsSalesForce = sf.GetStockist_Sub_New(divcode: divcode, subdivision: subdivision_code, FYEAR: Convert.ToInt16(FYear), FMONTH: Convert.ToInt16(FMonth), TYEAR: Convert.ToInt16(TYear), TMONTH: Convert.ToInt16(TMonth), sf_code: sfCode);
        }
        else
        {
 dsSalesForce = sf.GetStockist_Sub_New(divcode, subdivision_code, Convert.ToInt16(FYear), Convert.ToInt16(FMonth), Convert.ToInt16(TYear), Convert.ToInt16(TMonth));
//            dsSalesForce = sf.GetStockist_subdivisionwise(divcode, subdivision_code);
        }
