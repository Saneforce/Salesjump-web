...
public partial class MIS_Reports_rpt_territorywise_product_detail : System.Web.UI.Page
{
...
sf_code = Session["sf_code"].ToString();
   ...
   if (stype == "1")
        ...
        if (!Page.IsPostBack)
        {
            ...
            lblProd.Text = "Purchase Register Territorywise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
              ...
              lit_catg_namee.Style.Add("Color", "White");
               ...
               dtype="total";
                sURL = "rpt_territory_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&Territory_Code=" + territorycode + "&Territory_name=" + Territory_name + "&Subdivision=" + subdivision + "&Dtype="+ dtype +"";
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                       ...
                       // dsDrr = sd.Territorywise_purchase_productdetail_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString());
    dsDrr = sd.Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, product_code: drdoctor["Product_Detail_Code"].ToString(), week: i);
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                      ...
                      //  dsDrr = sd.Territorywise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), territorycode);
   dsDrr = sd.Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, territorycode: territorycode, product_code: drdoctor["Product_Detail_Code"].ToString(), week: i);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
