...
public partial class MIS_Reports_rpt_sales_register_distributor_wise : System.Web.UI.Page
{
   ...
   string stype = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Sales Register -Distributor Wise for  " + strFMonthName + " " + FYear + " " + "To" + " " + strTMonthName + " " + TYear;
    ...
    private void FillSF_MR()
    {
  		 ...
  		 if (sf_type == "1")
        {
            ...
            dsSalesForce = sf.GetStockName_Customer(divcode, sfCode);

        }
        else
        {

            dsSalesForce = sf.GetStockName_Cus_MGR(divcode, sfCode);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                sURL = "rpt_sale_Register_Category_wise.aspx?Stockist_Code=" + drFF["Stockist_Code"] + "&Stockist_name=" + drFF["Stockist_Name"].ToString() + " &FYear=" + FYear + "&FMonth=" + FMonth + " &TYear=" + TYear + "&TMonth=" + TMonth + "&sCurrentDate=" + sCurrentDate + "&subdivision=" + subdivision_code + "";
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                        ...
                        dsDoc = sf.Sales_Distributor_value(drFF["Stockist_Code"].ToString(), cmonth, cyear, divcode);
                        ...
                        if (tot_dr != "0")
                        {
                            try
                            {
                                Decimal per1 = Math.Round(Convert.ToDecimal(tot_dr), 2);
                                hyp_lst_month.Text = per1.ToString("0.00");
                            }
                            catch
                            {
                                hyp_lst_month.Text = tot_dr.ToString();
                            }
                            ...
                            stURL = "rpt_sale_purchase_view.aspx?Stockist_Code=" + drFF["Stockist_Code"] + "&Stockist_name=" + drFF["Stockist_Name"].ToString() + " &Year=" + cyear + "&Month=" + cmonth + " &stype=" + stype + "&subdivision=" + subdivision_code + "";
            ...
            if (months >= 0)
            {
                ...
                for (int j = 1; j <= months + 1; j++)
                {
                    ...
                    foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                    {
                        dsDoc = sf.Sales_Distributor_value(drFF["Stockist_Code"].ToString(), cmonth, cyear, sCurrentDate);
                        if (dsDoc.Tables[0].Rows.Count > 0)
                            tot_dr = dsDoc.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
                        ...
                        if (tot_dr != "")
                        {
                            ...
                            try
                            {
                                Decimal per1 = Math.Round(Convert.ToDecimal(iTotLstCount), 2);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (months >= 0)
                {
                    ...
                    for (int j = 1; j <= months + 1; j++)
                    {
                     ...
                     //   dsDoc = sf.Sales_Distributor_value(drFF["Stockist_Code"].ToString(), cmonth, cyear, sCurrentDate);
					 dsDoc = sf.Sales_Distributor_value( stockistcode: drFF["Stockist_Code"].ToString(), cmonth: cmonth, cyear: cyear, divcode: divcode, sf_code: sfCode);
