...
public partial class MIS_Reports_rpt_salecategory_daywise : System.Web.UI.Page
{
...
string dtype=string.Empty;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        cat_code = Request.QueryString["Product_Cat_Code"].ToString();
        ...
        lblHead.Text = "Sale Register CategoryWise Product Daywise Details for the Month of " + strFMonthName + " " + FYear;
...
tc_det_SNo.HorizontalAlign = HorizontalAlign.Right;
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    //    dsDrr = sd.sale_categoryview_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
   dsDrr = sd.sale_catogory_wise_sale_value_stockist( divcode: div_code, cmonth: FMonth, cyear: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(), stockistcode: distributor, productcatname: cat_code ,sf_code:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweekdays.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drdo in gg.Tables[0].Rows)
                    {
                      ...
                      //  dsDrr = sd.sale_categoryview_daywise_details_total(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
    dsDrr = sd.sale_catogory_wise_sale_value_stockist(divcode: div_code, cmonth: FMonth, cyear: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(), stockistcode: distributor,  sf_code: sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
