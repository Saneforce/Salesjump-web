...
<html xmlns="https://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            function loadData() {
                $.when(getStartEnd(), getTpdates(), getDayPlan(), getProdCalls(), getNewRetailerPOB(), getRetailerCount(), getOrders()).then(function () {
                    ReloadData();
            ...
            function ReloadData() {
                ...
                for (var i = 0; i < tpdates.length; i++) {
                    for (var j = 0; j < sfusers.length; j++) {
                        ...
                        let nretailers = 0;
                        let nretailerspob = 0;
                        str += `<tr><td>${++slno}</td><td>${tpdates[i].actdt}</td><td>${sfusers[j].StateName}</td><td>${sfusers[j].sf_hq}</td><td>${sfusers[j].Sf_Name}</td><td>${sfusers[j].Designation}</td><td>${sfusers[j].SF_Mobile}</td>`;
                        ...
                        if (tbday.length > 0) {
                            ...
                            str += `<td>${todayWtype}</td><td>${jointwrk}</td><td>${remarks}</td><td>${stkscd}</td><td>${stkadr}</td><td>${beats}</td><td>${actcalls}</td>`;
                        }
                        else {
