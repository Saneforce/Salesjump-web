...
public partial class MIS_Reports_rpt_salearea_daywise : System.Web.UI.Page
{
 ...
 string dtype=string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Sale Register Areawise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                   ...
                   // dsDrr = sd.sale_areawise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), areacode);
dsDrr = sd.sale_areawise_purchase_value(div_code: div_code,cmonth: FMonth, cyear: FYear,day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(), areacode: areacode,sf_cdoe:sf_code);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    string jj = ht.ToString("yyyy-MM-dd");
                    //dsDrr = sd.purchase_disributor_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
            //        dsDrr = sd.sale_areawise_purchase_productdetail_total_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString());
dsDrr = sd.sale_areawise_purchase_value( div_code: div_code, cmonth: FMonth,cyear: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(),sf_cdoe:sf_code);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
