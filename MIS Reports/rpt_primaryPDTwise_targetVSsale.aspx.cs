...
public partial class MIS_Reports_rpt_primaryPDTwise_targetVSsale : System.Web.UI.Page
{
    ...
    public static targetvsSales[] getProductTargetSale(string sf_Code, string fYear, string fMonth, string tYear, string tMonth)
    {
        ...
        foreach (DataRow row in dsAccessmas.Tables[0].Rows)
        {
			...
			pds.stknm = row["Stockist_Name"].ToString();
            pds.stkcd = row["Stockist_code"].ToString();
            pds.rwnum = row["row_num"].ToString();
