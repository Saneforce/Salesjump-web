...
public partial class MIS_Reports_sec_od_distDaywise : System.Web.UI.Page
{
    DataSet dsDr = null;
    DataSet dsDrr = null;
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    DateTime ht = Convert.ToDateTime(f);
                    string jj = ht.ToString("yyyy-MM-dd");
                    dsDrr = sd.retail_disributor_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
...
foreach (DataRow drdo in gg.Tables[0].Rows)
{
  ...
  foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
    ...
    dsDrr = sd.retail_disributor_view_daywise_details(div_code, FMonth, FYear, jjj, drdoctor["Product_Detail_Code"].ToString(), distributor);
    if (dsDrr.Tables[0].Rows.Count > 0)
    {
