...
using Bus_EReport;
using System.Net;
public partial class MIS_Reports_rpt_sale_brandwise_weekwise : System.Web.UI.Page
{
...
string sURL = string.Empty;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Brandcode = Request.QueryString["Brand_Code"].ToString();
        ...
        lblHead.Text = "Sale Register Brandwise Product Details for the Month of " + strFMonthName + " " + FYear;
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                     ...
                     //   dsDrr = sd.sale_register_Branddetail_stockistwise_caterywise_total(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code);
   dsDrr = sd.sale_Brand_value_categorywise(div_code: div_code, cmonth: FMonth, cyear: FYear, week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), stockistcode: distributor, catcod: cat_code, sf_code: sf_code);
...
tc_det_dr_name.HorizontalAlign = HorizontalAlign.Right;
                                ...
                                lit_det_dr_name.Text =drrdoctor["Sale_Qty"]==DBNull.Value ? drrdoctor["Sale_Qty"].ToString(): Convert.ToDecimal(drrdoctor["Sale_Qty"]).ToString("0.00");
                ...
                foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
                {
                    ...
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drr1doctor in dsDrr.Tables[0].Rows)
                        {
                            ...
                            quantot = drr1doctor["Sale_Qty"].ToString();
...
leave_utility.Style.Add("text-align","center");
        ...
        if (getweek.Tables[0].Rows.Count > 0)
        {
            for (int i = 1; i < wee; i++)
            {
                ...
                lit_catg_namee.Text = " Week-" + i.ToString();
                sURL = "rpt_salebrand_daywise.aspx?weekno=" + i + " &FYear=" + FYear + "&FMonth=" + FMonth + "&dist_code=" + distributor + "&Product_Brand_Code=" + Brandcode + "&Product_Cat_code=" + cat_code + "";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                      ...
                      //  dsDrr = sd.sale_register_Branddetail_stockistwise_caterywise(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), distributor, cat_code, Brandcode);
  dsDrr = sd.sale_Brand_value_categorywise( div_code: div_code,cmonth: FMonth, cyear: FYear,week: i, product_code: drdoctor["Product_Detail_Code"].ToString(), stockistcode: distributor, catcod: cat_code, brandcode: Brandcode,sf_code:sf_code);
                        if (dsDrr.Tables[0].Rows.Count > 0)
                        {
                            foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                            {
