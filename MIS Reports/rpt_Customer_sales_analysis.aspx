<%@ Page Language="C#" AutoEventWireup="true" CodeFile="rpt_Customer_sales_analysis.aspx.cs"
    EnableEventValidation="false" Inherits="MIS_Reports_rpt_Customer_sales_analysis" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                async: false,
                url: "rpt_Customer_sales_analysis.aspx/GetVisitedDtls",
            ...
            $('#<%=GV_DATA.ClientID%> tr:nth-child(1)').find(' th').each(function () {
                ...
                if ($(this).index() > 6 && $(this).index() <= 18) {
                    $($(this)).html('<a class="AAA" href="rpt_Customer_sales_analysis_days.aspx?&SF_Code=' + $('#<%=hidn_sf_code.ClientID%>').val() + '&FYear=' + $('#<%=lblyear.ClientID%>').text() + '&SF_Name=' + $('#<%=lblsf_name.ClientID%>').text() + '&FMonth=' + ($(this).index() - 6) + '&MonthNa=' + $(this).text() + '">' + $(this).text() + '</a>')
                }
            ...
            $('#<%=GV_DATA.ClientID%> tr:nth-child(3)').find('td').each(function () {
                if ($(this).index() > 6 && $(this).index() <= 18) {
                    $($(this)).html('<a class="AAA" href="rpt_Customer_sales_analysis_productwise.aspx?&SF_Code=' + $('#<%=hidn_sf_code.ClientID%>').val() + '&FYear=' + $('#<%=lblyear.ClientID%>').text() + '&SF_Name=' + $('#<%=lblsf_name.ClientID%>').text() + '&FMonth=' + ($(this).index() - 6) + '&MonthNa=' + $(this).closest('table').find('th').eq($(this).index()).text() + '">' + $(this).text() + '</a>')
                }
            ...
            $(document).on('click', "#btnExcel", function (e) {
              //  window.open('data:application/vnd.ms-excel,' + encodeURIComponent($('div[id$=divExcel]').html()));
               // e.preventDefault();
                    ...
                    blob = new Blob([$('div[id$=divExcel]').html()], {
...
<body>
                        ...
                        <table>
                            <tr>
                                <td>
                                    ...
                                    <asp:LinkButton ID="btnExcel" runat="Server" Style="padding: 0px 20px;" class="btn btnExcel" Visible="true"/>
									<asp:LinkButton ID="bdnexcel" runat="Server" Style="padding: 0px 20px;" class="btn btnExcel" OnClick="btnExcel_Click" Visible="false"/>
    ...
    <asp:Panel ID="pnlContents" runat="server" Width="100%">
        <div id="divExcel">
            <center>
                <div>
                    CUSTOMER WISE SALES ANALYSIS REPORT of - <b>
