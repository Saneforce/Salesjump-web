<%@ Page Title="" Language="C#" MasterPageFile="~/Master.master" AutoEventWireup="true" CodeFile="New_Retailers_Report.aspx.cs" Inherits="MIS_Reports_New_Retailers_Report" %>
...
<Style>
    ...
    <form runat="server">
        <div class="row">
            ...
            <div class="col-lg-12 sub-header">
                New Retailers Report <span style="float: right; margin-right: 15px;">
                ...
                <table class="table table-hover" id="OrderList" style="font-size: 12px">
                    <thead class="text-warning">
                        <tr style="background-color: #37a4c6; color: #fff;white-space:nowrap;">
                            ...
                            <th style="text-align: left; color: #fff">Created Date</th>
                            ...
                            <th style="text-align: left; color: #fff">DOB</th>
                            <th style="text-align: left; color: #fff">Pin Code</th>
                            <th style="text-align: left; color: #fff">Mobile</th>
                            <th style="text-align: left; color: #fff">Contact Person Name</th>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var sortid = '';
        var asc = true;//
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "ListedDrCode,ListedDr_Name,sf_Code,dist_name,Created_Date,Territory_Name,ListedDr_Phone,";
        ...
        function ReloadTable() {
            ...
            for ($i = st; $i < st + Number(PgRecords); $i++) {
                if ($i < Orders.length) {
                    ...
                    var drstk = Orders[$i].dist_name;
                    drstk = drstk.split(',');
                    var stk_name = '';
                    for ($j = 0; $j < drstk.length; $j++) {
                        var dstk = AllOrders2.filter(function (a) {
                            return a.Stockist_Code == drstk[$j];
                        });
                        stk_name += dstk[0].Stockist_Name+',';
                    }
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].Created_Date + "</td><td>" + Orders[$i].ListedDrCode + "</td><td>" + drsfn[0].StateName + "</td><td>" + drsfn[0].sf_hq + "</td><td>" + drsfn[0].Reporting_To + "</td><td>" + sf_name + "</td><td>" + Orders[$i].ListedDr_Name + "</td><td>" + Orders[$i].Territory_Name + "</td><td>" + Orders[$i].Address + "</td><td>" + Orders[$i].DOB + "</td><td>" + Orders[$i].PinCode + "</td><td>" + Orders[$i].ListedDr_Phone + "</td><td>" + Orders[$i].contactperson + "</td><td>" + stk_name + "</td>");
                    //$(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].ListedDrCode + "</td><td>" + Orders[$i].ListedDr_Name + "</td><td>" + sf_name + "</td><td>" + stk_name + "</td><td>" + Orders[$i].Created_Date + "</td><td>" + Orders[$i].Territory_Name + "</td><td>" + Orders[$i].ListedDr_Phone + "</td>");
        ...
        function loadData1() {
            $.ajax({
                ...
                async: false,
                url: "New_Retailers_Report.aspx/GetSFDetails",
        ...
        function loadData2() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "New_Retailers_Report.aspx/GetStkDetails",
                data: "{'SF':'<%=Session["Sf_Code"]%>','Div':'<%=Session["Division_Code"]%>'}",
                dataType: "json",
                success: function (data) {
        ...
        function loadData() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "New_Retailers_Report.aspx/GetDetails",
