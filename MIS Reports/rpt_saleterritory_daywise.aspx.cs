...
public partial class MIS_Reports_rpt_saleterritory_daywise : System.Web.UI.Page
{
...
dtype = Request.QueryString["Dtype"].ToString();
        ...
        lblHead.Text = "Sale Register territorywise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                if (getweek.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < wee; i++)
                    {
                        ...
                        dsDrr = sd.areawise_purchase_productdetail(div_code, FMonth, FYear, i, drdoctor["Product_Detail_Code"].ToString(), territorycode);
...
tc_det_SNo.Style.Add("text-align","right");
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                  ...
                  //  dsDrr = sd.sale_Territorywise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), territorycode);



					  if (dtype == "value")
                    {
                        dsDrr = sd.sale_Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(), territorycode: territorycode, sf_code: sf_code);
                    }
                    else
                    {
                        dsDrr = sd.sale_Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, day: jj, product_code: drdoctor["Product_Detail_Code"].ToString(),  sf_code: sf_code);
                    }   
