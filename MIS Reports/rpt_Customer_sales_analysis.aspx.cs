...
public partial class MIS_Reports_rpt_Customer_sales_analysis : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblyear.Text = FYear;
		 if (divcode == "90")
        {
            bdnexcel.Visible = true;
            btnExcel.Visible = false;
        }
        else
        {
            bdnexcel.Visible = false;
        }
    ...
    private void FillSF()
    {
        ...
        int i = 1;
        dsData.Rows.Add("1", "Order Given Customers (Out of Total " + DsRetailer.Tables[0].Rows.Count.ToString() + ")");
            ...
            if (drow.Length > 0)
            {
                ...
                foreach (DataRow row in drow)
                {
                    ...
                    decimal cur_tot = jan_val + feb_val + mar_val + apr_val + may_val + jun_val + jul_val + aug_val + sep_val + oct_val + nov_val + dec_val;
                     if (divcode == "109")
                    {
                        dsData.Rows.Add(row["ListedDrCode"].ToString(), row["ListedDr_Name"].ToString(), row["Doc_cat_ShortName"].ToString(), row["Doc_Special_Name"].ToString(), row["phone"].ToString(), row["Address1"].ToString(), "", jdt, fdt, mdt, adt, mydt, jndt, jldt, audt, sedt, ocdt, nodt, dedt, cur_tot);
                    }
                    else
                    {
                        dsData.Rows.Add(row["ListedDrCode"].ToString(), row["ListedDr_Name"].ToString(), row["Doc_cat_ShortName"].ToString(), row["Doc_Special_Name"].ToString(), row["phone"].ToString(), row["Address1"].ToString(), "", jan_val, feb_val, mar_val, apr_val, may_val, jun_val, jul_val, aug_val, sep_val, oct_val, nov_val, dec_val, cur_tot);
                    }
