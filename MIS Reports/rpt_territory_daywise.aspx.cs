...
public partial class MIS_Reports_rpt_territory_daywise : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead.Text = "Purchase Register territorywise Product Detail  for the Period of " + strFMonthName + " " + Year + " ";
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
            {
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    //    dsDrr = sd.Territorywise_purchase_productdetail_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), territorycode);
                    dsDrr = sd.Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, territorycode: territorycode, product_code: drdoctor["Product_Detail_Code"].ToString(), day: jj);
        ...
        if (dsDr.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow drdoctor in dsDr.Tables[0].Rows)
            {
                ...
                foreach (DataRow drdo in gg.Tables[0].Rows)
                {
                    ...
                    string jj = ht.ToString("yyyy-MM-dd");
                    //dsDrr = sd.purchase_disributor_view_daywise_details(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString(), distributor);
                   // dsDrr = sd.Territorywise_purchase_productdetail_total_daywise(div_code, FMonth, FYear, jj, drdoctor["Product_Detail_Code"].ToString());
                    dsDrr = sd.Territorywise_purchase_value(div_code: div_code, cmonth: FMonth, cyear: FYear, subdivision: subdivision, sf_code: sf_code, product_code: drdoctor["Product_Detail_Code"].ToString(), day: jj);
                    if (dsDrr.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow drrdoctor in dsDrr.Tables[0].Rows)
                        {
