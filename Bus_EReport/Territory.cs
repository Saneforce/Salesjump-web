...
namespace Bus_EReport
{
    public class Territory
    {
           ...
           //     strQry = "UPDATE Mas_Territory_Creation " +
             //               " SET territory_active_flag=1  " +
               //             " WHERE Territory_Code = '" + terr_Route_Code + "' ";
				strQry = "exec Route_Deactivation '" + terr_Route_Code + "' ";
        ...
        public int RecordUpdate(string Terr_code, string Route_Code, string Territory_Name, string Territory_Type, string sf_code, string Target, string min, string Div_code, string Route_Population, string territory_sname, string dist_name, string Allowance_Type, string dllstay,string ddl_town)
        {
            int iReturn = -1;
                        ...
                        strQry = "UPDATE Mas_Territory_Creation " +
                                    ...
                                    " Territory_Name = '" + Territory_Name + "', " +
                                      ...
                                      " Target = '" + Target + "', LastUpdt_Date= getdate(), " +
                                       " Min_Prod ='" + min + "'  ,  Population = '" + Route_Population + "' ," +
                                       " Territory_SName ='" + territory_sname + "' ," +
                                       " Allowance_Type ='" + Allowance_Type + "'," +
                                       " Territory_SNo ='" + dllstay + "'," +
                                       " Route_Town_Code ='" + ddl_town + "'" +
        ...
        public DataSet getTerritory(string Sf_Code)
        {
          ...
          //  strQry = " SELECT a.Territory_Code,a.Route_Code,a.Territory_Name, a.Dist_Name,a.Target,a.Min_Prod," +
                 ...
                 //    " FROM  Mas_Territory_Creation a where (a.Sf_Code = '" + Sf_Code + "' or a.Dist_Name='" + Sf_Code + "')  and a.Territory_Active_Flag=0" +
        ...
        public int RecordAdd(string Territorys_Route_Code, string Territory_Name, string Territory_Type, string Sf_Code, string Target, string min_prod, string Div_code, string Route_Population, string territory_sname, string dist_name, string Allowance_Type, string DDLStay, string ddl_town)
        {
            int iReturn = -1;
  DB_EReporting db = new DB_EReporting();

            string route = null;
            strQry = "select  Division_SName from mas_division where Division_Code ='" + Div_code + "'";
            route = db.Exec_Scalar_s(strQry);
            strQry = "select '" + route + "' +cast(isnull(max(Replace(Route_Code,'" + route + "','')),0)+1 as varchar) from Mas_Territory_Creation where Division_Code='" + Div_code + "' and isnumeric(Replace(Route_Code,'" + route + "',''))>0";

            string rr = db.Exec_Scalar_s(strQry);
 	  

            if (!cRecordExist(rr,Territory_Name, Div_code))
            {
                if (!RecordExist(Territory_Name, rr,Div_code))
                {
                    ...
                    try
                    {
                       ...
                       // DB_EReporting db = new DB_EReporting();
                        ...
                        strQry = "insert into Mas_Territory_Creation (Territory_Code,Territory_Name,Dist_Name,division_Code, " +
                                 " Sf_Code,Territory_Active_Flag,Created_date,Target,Min_Prod,Route_Code,Town_Code,Population,Territory_SName,Allowance_Type,Territory_SNo,Route_Town_Code) " +
                                 " VALUES('" + Territory_Code + "', '" + Territory_Name + "', '" + dist_name + "',  " +
                                 " '" + Div_code + "', '" + Sf_Code + "', 0, getdate(),'" + Target + "','" + min_prod + "','" + rr+ "','" + Town_code + "','" + Route_Population + "','" + territory_sname + "','" + Allowance_Type + "','" + DDLStay + "','" + ddl_town + "')";
        ...
        public DataSet getDSM_Code(string div_Route_Code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            strQry = "SELECT '0' as DSM_code, '---Select---' as DSM_name " +
                     " UNION " +
                     " select DSM_code,DSM_name from Mas_DSM where DSM_Active_Flag=0 and DSM_code ='" + div_Route_Code + "'";
            try
            {
                dsProCat = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getSF_Code_distributor(string div_Route_Code)
        {
            ...
            strQry = " SELECT '' as Stockist_Code, '--Select--' as stockist_Name " +
                     " UNION " +
                " select Stockist_Code,stockist_Name from Mas_Stockist  where Stockist_Active_Flag = 0 and Division_Code='" + div_Route_Code + "'";
            try
            {
                dsProCat = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getSF_Code_Route(string div_Route_Code, string distributor)
        {
            ...
            strQry = " SELECT 0 as Territory_Code, '---Select---' as Territory_Name " +
                     " UNION " +
                     "select [Territory_Code],[Territory_Name] from Mas_Territory_Creation where Territory_SName='" + distributor + "' and Territory_Active_Flag='0'  and Division_Code='" + div_Route_Code + "'ORDER BY 2";
        ...
        public DataSet getTerritory_dm(string sfname)
        {
            ...
            strQry = "  select Stockist_Name,Stockist_Code from Mas_Stockist where Stockist_Code='" + sfname + "'";
            try
            {
                dsTerr = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getTerritory_dm_MR(string sfname)
        {
            DB_EReporting db_ER = new DB_EReporting();
        ...
        public DataSet getTerritorydiv(string Sf_Code, string Div_code)
        {
            ...
            strQry = " SELECT a.Territory_Code,a.Route_Code,a.Territory_Name, a.Dist_Name,a.Target,a.Min_Prod,a.Population," +


                     " (select COUNT(c.Territory_Code)  from Mas_ListedDr c where c.Sf_Code=a.Dist_Name and c.Territory_Code=a.Territory_Code and c.ListedDr_Active_Flag=0) ListedDR_Count,  " +
                     " (select COUNT(b.Dist_Name) from Mas_Territory_Creation b where b.Territory_Code=a.Territory_Code and " +
                     "  b.Territory_Active_Flag=0) Chemists_Count, " +
                     " (select Stockist_Name from Mas_Stockist e where e.Stockist_Code=a.Dist_Name)Dst_Name " +
        ...
        public DataSet getTerritorydiv1(string Div_code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            strQry = " SELECT a.Territory_Code,a.Route_Code,a.Territory_Name, a.Dist_Name,a.Target,a.Min_Prod,a.Population," +
                     ...
                     " (select COUNT(b.ListedDrCode)  from Mas_ListedDr b where  b.Territory_Code = a.Territory_Code " +
                     " and b.ListedDr_Active_Flag=0) ListedDR_Count," +
                     " (select COUNT(b.Dist_Name) from Mas_Territory_Creation b where b.Territory_Code=a.Territory_Code and " +
                     "  b.Territory_Active_Flag=0) Chemists_Count, " +
                     " (select Stockist_Name from Mas_Stockist e where e.Stockist_Code=a.Dist_Name)Dst_Name " +
                     " FROM  Mas_Territory_Creation a where a.Territory_Active_Flag=0 and a.Division_Code ='" + Div_code + "' " +
                     " order by Territory_SNo";
            //strQry = " SELECT Territory_Code,Route_Code,Territory_Name,Dist_Name,Target,Min_Prod " +
            //         " FROM  Mas_Territory_Creation a where a.Sf_Code = '" + Sf_Code + "'  and a.Territory_Active_Flag=0 " +
            //         " order by Territory_Code";
        ...
        public DataSet getSF_Code_distributor_code(string sf_code, string div_Route_Code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsProCat = null;

            strQry = " select Stockist_Code,stockist_Name from Mas_Stockist  where Field_Code='" + sf_code + "' and Division_Code='" + div_Route_Code + "'and Stockist_Active_Flag = 0";
            try
            {
                dsProCat = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet get_Territory(string divcode, string terr_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsDivision = null;
            strQry = "select * from Mas_Territory_Creation where territory_active_flag=0 and Division_Code='" + divcode + "' and  Territory_SName='" + terr_code + "' ORDER BY Territory_name";
            try
            {
        ...
        public DataSet getEmptyTerritory()
        {
            ...
            //         " FROM  sys.tables ";
            strQry = "SELECT TOP 10 ''Route_Code,'' Territory_Name,'' Dist_Name,'' Target,'' Min_Prod ,''Population " +
                     "FROM  sys.tables ";
            try
            {
                dsProCat = db_ER.Exec_DataSet(strQry);
        ...
        public DataSet getTerritorySF(string TerritoryCode, string Div_code, string subDive = "0", string sAlpha = "",string sf_code="0")
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsTerr = null;

            //strQry = " select a.Territory_Code,a.Route_Code,a.Territory_Name,a.Dist_Name,a.Target,a.Min_Prod,a.Population,count(b.ListedDrCode) as ListedDR_Count,  " +
            //         " (select Stockist_Name+', ' from Mas_Stockist e where charindex(','+e.Stockist_Code+',',','+a.Dist_Name+',')>0 for xml path(''))Dst_Name,a.Allowance_Type,a.Territory_SNo  from Mas_Territory_Creation a left outer join Mas_ListedDr b on  b.Territory_Code = cast(a.Territory_Code as varchar) and b.ListedDr_Active_Flag=0 where   charindex(','+ cast(Territory_SName as varchar) +',', ','+'" + Sf_Code + "'+',') > 0 and " +
            //         " a.Territory_Active_Flag=0 ";

            //if (sAlpha != string.Empty)
            //{
            //    strQry += " AND LEFT(a.Territory_Name,1) = '" + sAlpha + "' ";
            //}

            //strQry += "group by a.Territory_Code,a.Route_Code,a.Territory_Name,a.Dist_Name,a.Target,a.Min_Prod,a.Population,a.Allowance_Type,a.Territory_SNo order by a.Territory_Name";


            strQry = " exec getRouteList '" + Div_code + "','" + TerritoryCode + "','" + subDive + "','" + sAlpha + "','" + sf_code + "'";
        ...
        public DataSet getProdName(string div_Route_Code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsProCat = null;
            if (div_Route_Code == "11" || div_Route_Code == "13" || div_Route_Code == "8")
            {
                strQry = "select Product_Detail_Code,Product_Detail_Name from Mas_Product_Detail where Division_Code='" + div_Route_Code + "' and Product_Active_Flag='0' ORDER BY Product_Detail_Code";
            }
            else
            {
                strQry = "select Product_Detail_Code,Product_Short_Name from Mas_Product_Detail where Division_Code='" + div_Route_Code + "' and Product_Active_Flag='0' ORDER BY Product_Detail_Code";
            }
            try
            {
                dsProCat = db_ER.Exec_DataSet(strQry);
        ...
        public DataSet getTerritory_Alphabet_List(string divcode, string sf_Code = "",string SubDiv="0")
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            strQry = "select '1' val,'All' sf_name " +
                     " union " +
                     " select distinct LEFT(Territory_Name,1) val, LEFT(Territory_Name,1) sf_name" +
                     " FROM Mas_Territory_Creation a inner join Mas_Salesforce s on  charindex(','+ cast(s.sf_code as varchar) +',', ','+a.sf_code+',') > 0 " +
                     " WHERE a.Division_Code = '" + divcode + "'" +
                     " AND Territory_Active_Flag = 0 and  charindex(','+ cast(Territory_SName as varchar) +',', ','+'" + sf_Code + "'+',') > 0  and   ('0'='" + SubDiv + "' or  charindex(','+'" + SubDiv + "' +',', ','+ s.subdivision_code+',') > 0 )" +
                     " ORDER BY 1";
            try
            {
                dsDivision = db_ER.Exec_DataSet(strQry);
        ...
        public DataTable getTerritorydiv1_Ex(string Div_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataTable dsTerr = null;
            ...
            //strQry =  " select  ROW_NUMBER() Over (Order by Route_Code) As [S.N.],a.Route_Code,a.Territory_Name as Route_Name,(select Stockist_Name+', ' from Mas_Stockist e where charindex(','+e.Stockist_Code+',',','+a.Dist_Name+',')>0 for xml path(''))Dst_Name ,  (select COUNT(b.ListedDrCode)  from Mas_ListedDr b where  b.Territory_Code = a.Territory_Code  and b.ListedDr_Active_Flag=0) Retailer_Count,a.Target,a.Min_Prod,a.Population,b.Territory_name   from Mas_Territory_Creation a " +
            //         " left outer join Mas_Territory b on a.Territory_SName =b.Territory_Code " +
            //         " where  division_code='"+Div_code+"'  and a.Territory_Active_Flag=0  order by 1,a.Territory_Name";


            //without sfName 
            //strQry = " select  ROW_NUMBER() Over (Order by Route_Code) As [S.N.],a.Route_Code,a.Territory_Name as Route_Name,(select Stockist_Name+', ' from Mas_Stockist e where charindex(','+e.Stockist_Code+',',','+a.Dist_Name+',')>0 for xml path(''))Dst_Name ,  "+
            //         " COUNT(c.ListedDrCode)  Retailer_Count,a.Target,a.Min_Prod,a.Population,b.Territory_name   from Mas_Territory_Creation a inner  join Mas_Territory b on a.Territory_SName =b.Territory_Code "+
            //         " left outer join Mas_ListedDr c on  c.Territory_Code = a.Territory_Code and c.ListedDr_Active_Flag=0  where  a.division_code='" + Div_code + "'  and a.Territory_Active_Flag=0 " +
            //         " group by a.Route_Code,a.Territory_Name, a.Target,a.Min_Prod,a.Population,b.Territory_name  ,a.Dist_Name  order by 1,a.Territory_Name ";
            if (Div_code == "52")
            {
                strQry = "select ROW_NUMBER() Over (Order by Route_Code) As [S.N.],Rut.Route_Code,Rut.Territory_Name as Route_Name, " +
                        " (select Stockist_Name from Mas_Stockist e where e.Stockist_Code=Rut.Dist_Name)Dst_Name, " +
                        " (select SF_Name from Mas_salesforce ms where ms.sf_code=Rut.sf_code)SF_Name,isnull(Cnt,0) Retailer_Count, " +
                        " Rut.Target,Rut.Min_Prod,Rut.Population,HQ.Territory_name from Mas_Territory_Creation Rut " +
                        " inner  join Mas_Territory HQ on cast(Rut.Territory_SName as int) =HQ.Territory_Code " +
                        " left outer join (select Territory_Code,count(ListedDrCode) Cnt from Mas_ListedDr group by Territory_Code) as ST " +
                        " on ST.Territory_Code=cast(Rut.Territory_Code as varchar) " +
                        " where Rut.Division_Code='" + Div_code + "'  and Rut.Territory_Active_Flag=0 order by 1,Rut.Territory_Name ";
            }
            else
            {
//              strQry = " select  ROW_NUMBER() Over (Order by Route_Code) As [S.N.],a.Territory_Code,a.Route_Code,a.Territory_Name as Route_Name, (select Stockist_Name+', ' from Mas_Stockist e where charindex(','+e.Stockist_Code+',',','+a.Dist_Name+',')>0 for xml path(''))Dst_Name ,  " +
  //                      " (select SF_Name+', ' from Mas_salesforce ms where charindex(','+ms.sf_code+',',','+a.sf_code+',')>0 for xml path(''))SF_Name ,   COUNT(c.ListedDrCode)  Retailer_Count,a.Target,a.Min_Prod,a.Population,b.Territory_name   from Mas_Territory_Creation a inner  join Mas_Territory b on a.Territory_SName =b.Territory_Code " +
    //                    " left outer join Mas_ListedDr c on  c.Territory_Code = cast(a.Territory_Code as varchar) and c.ListedDr_Active_Flag=0  where  a.division_code='" + Div_code + "'  and a.Territory_Active_Flag=0  group by a.Route_Code,a.Territory_Code,a.Territory_Name, a.Target,a.Min_Prod,a.Population,b.Territory_name  ,a.Dist_Name,a.sf_code  order by 1,a.Territory_Name ";
				strQry = " exec getRoute_ExDownload1 '" + Div_code + "'";
            }

            try
            {
                dsTerr = db_ER.Exec_DataTable(strQry);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public DataSet getTerritory_OS(string Div_code, string Terr_code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            strQry = "select '0' Territory_Code ,'-- select --' Territory_Name  " +
                     " union " +
                    "select ro.Territory_Code,ro.Territory_Name from Mas_Territory_Creation Ro inner join Mas_Territory Tr on Ro.Territory_SName = Tr.Territory_Code where division_code='" + Div_code + "' and  Allowance_Type='os' and  Tr.Territory_code='" + Terr_code + "'";
        ...
        public int Allowance_type_updation(string routcode, string route_name, string alw_type, string stay_place)
        {
            int iReturn = -1;
            try
            {
                DB_EReporting db = new DB_EReporting();
                strQry = "update Mas_Territory_Creation set Territory_Name='" + route_name + "', Allowance_Type='" + alw_type + "', Territory_SNo='" + stay_place + "' where Territory_Code='" + routcode + "'";
                iReturn = db.ExecQry(strQry);
            }
            catch (Exception ex)
            {
        ...
        public DataSet getTerritorySF1(string dist, string Sf_Code, string Div_code, string sAlpha = "")
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsTerr = null;

            strQry = " select a.Territory_Code,a.Route_Code,a.Territory_Name,a.Dist_Name,a.Target,a.Min_Prod,a.Population,COUNT(b.ListedDrCode) ListedDR_Count,  " +
                     " (select Stockist_Name+', ' from Mas_Stockist e where charindex(','+e.Stockist_Code+',',','+a.Dist_Name+',')>0 for xml path(''))Dst_Name,a.Allowance_Type,a.Territory_SNo  from Mas_Territory_Creation a left outer join Mas_ListedDr b on  b.Territory_Code = a.Territory_Code and b.ListedDr_Active_Flag=0 where   charindex(','+ cast(Territory_SName as varchar) +',', ','+'" + Sf_Code + "'+',') > 0 and " +
                     " charindex(','+'" + dist + "'+',',',' + a.SF_Code+',')>0 and " +
                     " a.Territory_Active_Flag=0 ";
        ...
        public DataSet getProdName_detailed_for_company(string fdate, string todate, string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsProCat = null;

            strQry = "select  distinct C.PRODUCT_SHORT_NAME,c.Product_Detail_Name,c.Product_Detail_Code from  " +
               "Mas_Product_Detail c inner join  Trans_Order_Head h  " +
    "inner join Trans_Order_Details d  on h.Trans_Sl_No=d.Trans_Sl_No  on " +
          "  c.Product_Detail_Code=d.Product_Code where h.Sf_code='" + sf_code + "'  and " +
         " CAST(CONVERT(VARCHAR, h.Order_date, 101) AS DATETIME)  between  '" + fdate + "' and '" + todate + "' and PRODUCT_ACTIVE_FLAG=0   group by Product_Detail_Name,C.PRODUCT_SHORT_NAME,c.Product_Detail_Code order by  Product_Detail_Name  ";
        ...
        public DataSet getProdName_dcrviewalldates(string sf_code, string div_code, string activitydate, string route_code, string DistCode)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsProCat = null;

            strQry = "SELECT distinct Product_Code as Product_Detail_Code,product_name as Product_Detail_Name  FROM Trans_Order_Head H inner join Trans_Order_Details D on H.Trans_Sl_No=D.Trans_Sl_No where Sf_Code='" + sf_code + "'";
            //strQry +=" and route='" + route_code + "' ";
            strQry += " and Stockist_Code='" + DistCode + "' and cast(convert(varchar,Order_Date,101) as datetime)='" + activitydate + " 00:00:00.000' group by Product_Code,product_name";
        ...
        public DataSet getProdName_prmy_for_company(string fdate, string todate, string sf_code)
        {
            DB_EReporting db_ER = new DB_EReporting();
            ...
            strQry = "select  distinct C.PRODUCT_SHORT_NAME,c.Product_Detail_Name,c.Product_Detail_Code from  " +
               "Mas_Product_Detail c inner join Trans_PriOrder_Head h   " +
           " inner join Trans_PriOrder_Details d   on h.Trans_Sl_No=d.Trans_Sl_No  on " +
          "  c.Product_Detail_Code=d.Product_Code where h.Sf_code='" + sf_code + "'  and " +
         " CAST(CONVERT(VARCHAR, h.Order_date, 101) AS DATETIME)  between   '" + fdate + "' and '" + todate + "' and PRODUCT_ACTIVE_FLAG=0   group by Product_Detail_Name,C.PRODUCT_SHORT_NAME,c.Product_Detail_Code order by  Product_Detail_Name  ";
        ...
        public DataSet get_Excel_Format(string ToolName, string DivCode)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsProCat = null;
            strQry = "select * from Mas_Upload_Setting where Tool_Name='" + ToolName + "'";
            try
            {
                dsProCat = db_ER.Exec_DataSet(strQry);
            }
        ...
        public DataSet getRouteDetails(string divcode, string sf_Code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsAdmin = null;

            strQry = "exec getRouteDetails '"+ sf_Code +"','" + divcode + "'";
        ...
        public DataSet getSalesforce(string divcode)
        {
            ...
            strQry = "select Sf_Code,Sf_Name from Mas_Salesforce where CHARINDEX('," + divcode.TrimEnd(',') + ",',','+Division_Code+',')>0 and SF_Status<>2";
        ...
        public DataSet getStockist(string divcode)
        {
            ...
            strQry = "select Stockist_Code,Stockist_Name from Mas_Stockist where Division_Code=" + divcode.TrimEnd(',') + " and Stockist_Active_Flag=0";
        ...
        public DataSet getRoute_Retailers(string divcode,string route_code)
        {
            DB_EReporting db_ER = new DB_EReporting();

            DataSet dsAdmin = null;

            strQry = "select * from Mas_ListedDr where Division_Code='"+ divcode + "' and ListedDr_Active_Flag=0 and Territory_Code='"+ route_code + "' order by ListedDr_Name";
        ...
        public int DeActivate(string terr_Route_Code,string stat)
        {
            int iReturn = -1;
            try
            {
                DB_EReporting db = new DB_EReporting();
                strQry = "exec Route_Deactivation '" + terr_Route_Code + "',"+ stat + " ";
                iReturn = db.ExecQry(strQry);
            }
            catch (Exception ex)
            {
