...
public partial class SuperStockist_Purchase_Invoice_SS_Goods_Received_Note : System.Web.UI.Page
{
    ...
    public static OrderNO[] GetOrderNo()
    {
        ...
        //dspono = sm.GetPOoRDERS(Stockist_Code, Div_Code);
        dspono = GetPOoRDERS(Stockist_Code, Div_Code);
        if (dspono.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow row in dspono.Tables[0].Rows)
            {
                OrderNO on = new OrderNO();
                on.Order_NO = row["Trans_Sl_No"].ToString();
                orderno.Add(on);
            }
        }
        return orderno.ToArray();
    }
    public static DataSet GetPOoRDERS(string Stockist_Code, string Div_Code)
    {
        DB_EReporting db_ER = new DB_EReporting();
        DataSet ds = null;
        string strQry = "select Trans_Sl_No, Order_Date,sap_code from Trans_SPriOrder_Head where Stockist_Code =  '" + Stockist_Code + "' and Order_Value> 0 and Division_Code = '" + Div_Code + "' and Order_Flag < 1 order by Trans_Sl_No  ";
        try
        {
    ...
    public static DataSet getproductname(string Orderid, string div_code, string Stockist_Code)
    {
        ...
        string strQry = "EXEC [SS_get_grn_pro_details] '" + Orderid + "','" + div_code + "','" + Stockist_Code + "'";
        ...
        using (var scope = new TransactionScope())
        {
            try
            {
                ...
                if (dsGoods.Tables[0].Rows.Count <= 0)
                {
                    ...
                    sqlQry = "update Trans_SPriOrder_Head set Order_Flag='1' where Trans_sl_no='" + GRN_Head[0].Po_No + "' and Division_Code='" + Div_Code.TrimEnd(',') + "' and Stockist_Code='" + SF_Code + "'";
    ...
    public static DataSet Save_Primary(string Stockist_Code, string div_code, string Head, string Details, string Tax_Details, string sf_code, string pono_no)
    {
       ...
       // string strQry = "Exec [svPending_PrimaryOrder] '" + Stockist_Code + "','" + div_code + "','" + Head + "','" + Details + "','" + Tax_Details + "','" + sf_code + "','" + pono_no + "'";
        string strQry = "Exec [sv_SS_PENDing_PrimaryOrder] '" + Stockist_Code + "','" + div_code + "','" + Head + "','" + Details + "','" + Tax_Details + "','" + sf_code + "','" + pono_no + "'";
