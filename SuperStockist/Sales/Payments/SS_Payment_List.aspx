<%@ Page Title="" Language="C#" MasterPageFile="~/Master_SS.master" AutoEventWireup="true" CodeFile="SS_Payment_List.aspx.cs" Inherits="SuperStockist_Sales_Payment_SS_Payment_List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
      ...
      <form id="frm1" runat="server">
        ...
        <div class="row">
            <div class="col-lg-12 sub-header">
                Payment<span style="float: right"><a href="../Payments/SS_Payment.aspx" class="btn btn-primary btn" id="newsf">Add New</a></span><span style="float: right; margin-right: 15px;">
                ...
                <table class="table table-hover" id="OrderList">
                    <thead class="text-warning" style="white-space: nowrap;">
                        <tr>
                            ...
                            <th style="text-align: left;display:none;">Advance Amount</th>
                            <th style="text-align: left;display:none;">credit Adjust</th>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "Cus_Name,Pay_Mode,type,Amount,";var path = ''; var page = '';
        var flag = 0;
        ...
        function ReloadTable() {
            ...
            for ($i = st; $i < st + PgRecords; $i++) {
                ...
                if ($i < Orders.length) {
                    ...
                    slno = $i + 1;
                   // $(tr).html("<td>" + slno + "</td><td style='display:none;'>" + Orders[$i].Sl_No + "</td><td style='display:none;'>" + Orders[$i].Cust_Id + "</td><td>" + Orders[$i].Cus_Name + "</td><td>" + Orders[$i].Pay_Date1 +
                   //     "</td><td>" + Orders[$i].Pay_Mode + "</td><td>" + Orders[$i].type + "</td><td>" + parseFloat(Orders[$i].Amount).toFixed(2) + "</td><td><a href = 'Payment_View.aspx?Order_No=" + Orders[$i].Sl_No + "' ><span class='glyphicon glyphicon-eye-open'></span></td>");
				   
				   $(tr).html("<td>" + slno + "</td><td style='display:none;'>" + Orders[$i].Sl_No + "</td><td style='display:none;'>" + Orders[$i].Cust_Id + "</td><td>" + Orders[$i].Cus_Name + "</td><td>" + Orders[$i].Pay_Date1 +
                        "</td><td>" + Orders[$i].Pay_Mode + "</td><td>" + Orders[$i].type + "</td><td>" + parseFloat(Orders[$i].Amount).toFixed(2) + "</td><td style='display:none;'>" + parseFloat(Orders[$i].advance_pay || 0).toFixed(2) + "</td><td style='display:none;'>" + parseFloat(Orders[$i].Adv_ajusted || 0).toFixed(2) + "</td><td><a id='myButton' href='#' onClick='popup(\"" + Orders[$i].Sl_No + "\")'><span class='glyphicon glyphicon-eye-open'></span></a></td>");
                       tota = parseFloat(Orders[$i].Amount || 0) + parseFloat(tota)
                    $("#OrderList TBODY").append(tr);
                    hq = [];
                }
            }
            $("#orders_info").html("Showing " + (st + 1) + " to " + (((st + PgRecords) < Orders.length) ? (st + PgRecords) : Orders.length) + " of " + Orders.length + " entries")
            $("#OrderList tfoot").html("<tr><td colspan='5' style='font-weight: bold;'>Total</td><td colspan='2' style='align-content: right;font-weight: bold;'>" + tota.toFixed(2) + "</td></tr>");
            loadPgNos();
             $('.spinnner_div').hide();
        }
		...
		function popup(Order_No) {
            ...
            var url = "../Stockist/Payment_View.aspx?Order_No=" + Order_No + ""
        ...
        function loadData() {
            ...
            $.ajax({
                ...
                async: false,
                url: "SS_Payment_List.aspx/Get_Payment_detail",
