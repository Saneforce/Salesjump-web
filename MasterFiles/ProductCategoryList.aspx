<%@ Page Title="Category" Language="C#" AutoEventWireup="true" MasterPageFile="~/Master.master" CodeFile="ProductCategoryList.aspx.cs"
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <form id="form1" runat="server">
        ...
        <div class="col-lg-12 sub-header">Category<span style="float: right"><a href="ProductCategory.aspx" class="btn btn-primary btn-update" id="newsf">Add New</a></span><div style="float: right;padding-top: 3px;">
                    <ul class="segment">
        ...
        <div class="card">
                    ...
                    <select class="data-table-basic_length" aria-controls="data-table-basic">
                        ...
                        <table class="table table-hover" id="CatList" style="font-size: 12px">
                            <thead class="text-warning">
                                <tr>
                                    <th style="text-align: left">Sl.No</th>
                                    <th id="Category Code" style="text-align: left">Category Code</th>
                                    <th id="Category Name" style="text-align: left">Category Name</th>
    ...
    <script type="text/javascript" src="../js/plugins/datatables/jquery.dataTables.js"></script>
            ...
            <script language="javascript" type="text/javascript">
                var AllOrders = [];
                var filtrkey = '0';
                var Orders = [], pgNo = 1, PgRecords = 10, TotalPg = 0, searchKeys = "Product_Cat_Code,Product_Cat_Name,Product_Cat_Div_Name,";
                ...
                function ReloadTable() {
                    ...
                    if (filtrkey != "All") {
                        Orders = Orders.filter(function (a) {
                            return a.Product_Cat_Active_Flag == filtrkey;
                        });
                    }
                    st = PgRecords * (pgNo - 1); slno = 0;
                    for ($i = st; $i < st + PgRecords; $i++) {
                        if ($i < Orders.length) {
                            tr = $("<tr rname='" + Orders[$i].Product_Cat_Name + "'  rocode='" + Orders[$i].Product_Cat_Code + "'></tr>");
                            slno = $i + 1;
                            $(tr).html('<td>' + slno + "</td><td>" + Orders[$i].Product_Cat_SName + '</td><td>' + Orders[$i].Product_Cat_Name + "</td><td>" + Orders[$i].Product_Cat_Div_Name +
                            /*"</td><td input type='hidden' class='cat_count'><a href='#'>" + Orders[$i].cat_count + "</a></td><td id= " + Orders[$i].Designation_Code + " > <a href='#'>Inline Edit</a></td ><td id=" + Orders[$i].Product_Cat_Code +*/
                                "</td><td class='scount'><input type='hidden' class='pcod_hidd' value='" + Orders[$i].cat_count + "'><a href='#'>" + Orders[$i].cat_count + "</a></td><td id=" + Orders[$i].Product_Cat_Code +
                                " class= 'roedit' > <a href='#'>Edit</a></td > <td class='rodeact'><a href='#' style=" + ((parseInt(Orders[$i].Product_Cat_Active_Flag) == 0) ? 'color:green;' : 'color:red;') + " >" + ((parseInt(Orders[$i].Product_Cat_Active_Flag) == 0) ? 'Active' : 'Deactive') + "</a></td>");
                            $("#CatList TBODY").append(tr);
                            hq = [];
                        }
                    }
                ...
                function loadData(divcode) {
                    ...
                    $.ajax({
                        ...
                        async: false,
                        url: "ProductCategoryList.aspx/GetDetails",
                ...
                function fillRoutes(div_code) {
                    ...
                    $.ajax({
                        ...
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "ProductCategoryList.aspx/Prod_Count",
                        data: "{'div_code':'<%=Session["div_code"]%>','ProdCatCode':'" + div_code + "'}",
                        dataType: "json",
                        success: function (data) {
                ...
                $(document).ready(function () {
                    ...
                    $(document).on('click', '.roedit', function () {
                        x = this.id;
                        window.location.href = "ProductCategory.aspx?Product_Cat_Code=" + x;
                    });
                    ...
                    $(document).on("click", ".rodeact", function () {
                        ...
                        var route_C = $(this).closest('tr').attr('rocode');
                        let oindex = Orders.findIndex(x => x.Product_Cat_Code == route_C);
                        let disstat = (parseInt(Orders[oindex]["Product_Cat_Active_Flag"]) == 0) ? 'Deactive' : 'Active';
                        let flg = (parseInt(Orders[oindex]["Product_Cat_Active_Flag"]) == 0) ? 1 : 0;
                        let flag = 0;
                        if ((flg == 1) || (flg == 0)) {
                            if (disstat == "Deactive") {
                                ...
                                if (res > 0) {
                                    flag = flag + 1;
                                    alert('This Category mapped with Product! Cannot deactivate!');
                                }
                            }
                            if (flag == 0) {
                                if (confirm("Do you want " + disstat + " the Division ?")) {
                                    ...
                                    $.ajax({
                                        ...
                                        contentType: "application/json; charset=utf-8",
                                        async: false,
                                        url: "ProductCategoryList.aspx/deactivate",
                                        data: "{'ProCatCode':'" + route_C + "','stat':'" + flg + "'}",
                                        dataType: "json",
                                        success: function (data) {
                                            if (data.d == 'Success') {
                                                Orders[oindex]["Product_Cat_Active_Flag"] = flg;
                                                ReloadTable();
