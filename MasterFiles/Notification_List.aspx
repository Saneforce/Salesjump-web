<%@ Page Title="" Language="C#" MasterPageFile="~/Master.master" AutoEventWireup="true" CodeFile="Notification_List.aspx.cs" Inherits="MasterFiles_Notification_List" %>
    ...
    <script language="javascript" type="text/javascript">
	 ...
	 if (confirm("do you want Delete the Notification??")) {
            $.ajax({
                ...
                async: false,
                url: "Notification_List.aspx/notifyDelete",
  ...
  alert(JSON.stringify(result));
        ...
        function ReloadTable() {
            ...
            for ($i = st; $i < st + PgRecords; $i++) {
                ...
                if ($i < Orders.length) {
                    ...
                    $(tr).html('<td>' + slno + '</td><td>' + Orders[$i].Noti_Send_Date + '</td><td>' + Orders[$i].SF_Name + '</td><td>' + Orders[$i].Notification_Message + '</td><td>' + Orders[$i].start_Date + '</td><td>' + Orders[$i].End_Date + '</td><td id=' + Orders[$i].Trans_Sl_No + ' class="sfedit" align="center"><a href="#">Edit</a></td><td align="center" onclick="sfdeact(' + Orders[$i].Trans_Sl_No + ')"><a href="#">Delete</a></td>');
        ...
        function loadData() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Notification_List.aspx/getnotice_board",               
                dataType: "json",
                success: function (data) {
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '#newsf', function () {
                window.location.href = "Notification_Message.aspx";
            });

            $(document).on('click', '.sfedit', function () {
                x = this.id;
                window.location.href = "Notification_Message.aspx?Trans_sl_no=" + x;
            });
