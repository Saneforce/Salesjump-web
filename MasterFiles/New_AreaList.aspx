<%@ Page Language="C#" AutoEventWireup="true"  MasterPageFile="~/Master.master" CodeFile="New_AreaList.aspx.cs" Inherits="MasterFiles_New_AreaList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <form runat="server">
        <div class="row" style="margin-bottom: 1rem;">
           ...
           <div class="col-lg-12 sub-header">Area Master<span style="float: right"><a href="AreaCreation.aspx" class="btn btn-primary btn-update" id="newsf">Add Area</a></span></div>
        </div>
                ...
                <table class="table table-hover" id="OrderList" style="font-size: 12px">
                    <thead class="text-warning">
                        <tr style="background-color: #37a4c6; color: #fff; white-space: nowrap;">
                            ...
                            <th style="text-align: left; color: #fff;">Area Code</th>
                            <th style="text-align: left; color: #fff">Area Name</th>
                            <th style="text-align: left; color: #fff">State</th>
                            <th style="text-align: left; color: #fff">Zone Count</th>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function ReloadTable() {
            ...
            if (filtrkey != "All") {
                Orders = Orders.filter(function (a) {
                    return a.Area_Active_Flag == filtrkey;
                })
            }
            st = PgRecords * (pgNo - 1);
            for ($i = st; $i < st + Number(PgRecords); $i++) {
                if ($i < Orders.length) {
                    ...
                    tr = $("<tr rname='" + Orders[$i].Area_name + "' rocode='" + Orders[$i].Area_code + "'></tr>");
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].Area_sname + "</td><td>" + Orders[$i].Area_name + "</td><td>" + Orders[$i].state + "</td><td class='rocount'><a href='#'>" + Orders[$i].Sub_Coun + "</a></td><td class='roedit'><a href='#'>Edit</a></td><td class='rodeact'><a href='#'>" + Orders[$i].Status + "</a></td>");
        ...
        function fillarea() {
            $.ajax({
                ...
                async: false,
                url: "New_AreaList.aspx/getfillarea",
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.roedit', function () {
                ...
                window.location.href = "AreaCreation.aspx?Subdivision_Code=" + route_C + "";
            });
            $(document).on("click", ".rodeact", function () {
                var route_C = $(this).closest('tr').attr('rocode');
                let oindex = Orders.findIndex(x => x.Area_code == route_C);
                let disstat = Orders[oindex]["Status"].toString();
                let flg = (parseInt(Orders[oindex]["Area_Active_Flag"]) == 0) ? 1 : 0;
                ...
                if ((rocnt < 1 && flg == 1) || (flg == 0)) {
                    if (confirm("Do you want " + disstat + " the Distributor ?")) {
                        $.ajax({
                            ...
                            contentType: "application/json; charset=utf-8",
                            async: false,
                            url: "New_AreaList.aspx/deactivatearea",
                            data: "{'arcode':'" + route_C + "','stat':'" + flg + "'}",
                            dataType: "json",
                            success: function (data) {
                                if (data.d == 'Success') {
                                    Orders[oindex]["Area_Active_Flag"] = flg;
