...
public partial class MasterFiles_ProductReminderList : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            dsEffDate = pv.getEffDate(div_code);
    ...
    private void FillGift()
    {
        ...
        else
        {
            ...
            if(Dt.Length>0)
            {
                EffFrom = Dt[0];
                EffTo = Dt[2];
               dsGift = dv.getGift(div_code,Convert.ToDateTime(EffFrom),Convert.ToDateTime(EffTo));
    ...
    private DataTable BindGridView()
    {
        ...
        if (sCmd == "All")
        {
            dtGrid = dv.getGiftlist_DataTable(div_code);
        }
        else if(sCmd != "")
        {
            sCmd = Session["GetcmdArgChar"].ToString();
            dtGrid = dv.getGiftlist_DataTable(div_code, sCmd);
        }
        else if (txtsearch.Text != "")
        {
            dtGrid = dv.getGiftlist_DataTable(div_code, txtsearch.Text);
        }
    ...
    private void Search()
    {
        ...
        if (search == 2)
        {
            dsGift = dv.getGiftName(div_code, txtsearch.Text);
            if (dsGift.Tables[0].Rows.Count > 0)
            {
                dlAlpha.Visible = true;
                grdGift.Visible = true;
        ...
        if (search == 3)
        {
            ...
            dsGift = dv1.getStategift(div_code, Convert.ToInt32(ddlSrc2.SelectedValue.ToString()));
            if (dsGift.Tables[0].Rows.Count > 0)
            {
                dlAlpha.Visible = true;
                grdGift.Visible = true;
   ...
   //end
    ...
    {
        System.Threading.Thread.Sleep(time);
        Response.Redirect("Reactivate_Gifts.aspx");
    }
}