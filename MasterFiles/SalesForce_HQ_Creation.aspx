<%@ Page Title="" Language="C#" MasterPageFile="~/Master.master" AutoEventWireup="true" CodeFile="SalesForce_HQ_Creation.aspx.cs" Inherits="MasterFiles_SalesForce_HQ_Creation" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <div class="row">
        <div class="col-lg-12 sub-header">HQ Master<span style="float: right"><a href="#" class="btn btn-primary btn-update" id="newsf">Add New</a></span><div style="float: right;padding-top: 3px;">
                    <ul class="segment">
            ...
            <table class="table table-hover" id="OrderList">
                <thead class="text-warning">
                    <tr>
                        ...
                        <th style="text-align: left">HQ ID</th>
                        <th style="text-align: left">HQ Name</th>
                        <th style="text-align: left">HQ Type</th>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "HQ_ID,HQ_Name,HQ_Type,Status";
        ...
        function ReloadTable() {
            ...
            if (filtrkey != "All") {
                Orders = Orders.filter(function (a) {
                    return a.HQ_Active_Flag == filtrkey;
                })
            }
            ...
            for ($i = st; $i < st + PgRecords ; $i++) {
                ...
                if ($i < Orders.length) {
                    ...
                    slno = $i + 1;
                    $(tr).html('<td>' + slno + '</td><td>' + Orders[$i].HQ_ID + '</td><td>' + Orders[$i].HQ_Name + '</td><td>' + Orders[$i].HQ_Type +
                    '</td><td id=' + Orders[$i].HQ_ID +
            ...
            $(".ddlStatus>li>a").on("click", function () {
                ...
                stus = $(this).attr("v");
                $indx = Orders.findIndex(x => x.HQ_ID == hqid);
                cStusNm = $(this).text();
                if (confirm("Do you want change status " + $(cStus).text() + " to " + cStusNm + " ?")) {
                    sf = Orders[$indx].HQ_ID;
                    $.ajax({
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "SalesForce_HQ_Creation.aspx/SetNewStatus",
                        data: "{'SF':'" + sf + "','stus':'" + stus + "'}",
                        dataType: "json",
                        success: function (data) {
                            Orders[$indx].HQ_Active_Flag = stus;
        ...
        function loadData(divcode, sf) {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "SalesForce_HQ_Creation.aspx/getHQ",
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '#newsf', function () {
                window.location.href = "HQ_Creation.aspx";
            });
            $(document).on('click', '.sfedit', function () {
                x = this.id;
                window.location.href = "HQ_Creation.aspx?hqcode=" + x;
            });
        });
