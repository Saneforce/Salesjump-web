...
public partial class MasterFiles_ProductDetail : System.Web.UI.Page
{
    ...
    private void Bindprodlunch()
    {
        using (SqlConnection con = new SqlConnection(strCon))
        {
            ...
            con.Open();
            using (SqlCommand cmd = new SqlCommand("select ID,File_Name,File_Type,convert(varchar(10),Effective_From,105)as Effect_From,convert(varchar(10),Effective_To,105) as Effect_To from Product_New_Launch where Prod_Code='" + ProdCode + "' "))
            {
    ...
    private void BindGridviewData()
    {
        using (SqlConnection con = new SqlConnection(strCon))
        {
            ...
            con.Open();
            using (SqlCommand cmd = new SqlCommand("select ID,File_Name,File_Type,convert(varchar(10),Effect_From,105)as Effect_From,convert(varchar(10),Effect_To,105) as Effect_To from Product_Td_Detailing where Prod_Code='" + ProdCode + "' and  isnull(File_status,0)=0"))
            {
                using (SqlDataAdapter sda = new SqlDataAdapter())
                {
                    cmd.Connection = con;
