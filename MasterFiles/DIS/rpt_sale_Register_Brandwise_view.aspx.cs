...
public partial class MIS_Reports_rpt_sale_Register_Brandwise_view : System.Web.UI.Page
{
    ...
    private void CreateDynamicTable()
    {
        ...
        if (type == "1")
        {
            dsDr = dr.sale_register_Branddetail_stockistwise_caterywise(div_code, Brand, Convert.ToInt16(Year), Convert.ToInt16(Month), sCurrentDate, catcode, stockist_code);
        }

        if (type == "2")
        {
            dsDr = dr.sale_register_brandwisedetail_categorywise(div_code, Brand, Convert.ToInt16(Year), Convert.ToInt16(Month), sCurrentDate, catcode, stockist_code);
        }
