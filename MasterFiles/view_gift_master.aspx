<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Master.master" CodeFile="view_gift_master.aspx.cs" Inherits="MasterFiles_view_gift_master" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <form runat="server">
        <div class="row" style="margin-bottom: 1rem;">
           ...
           <div class="col-lg-12 sub-header">Gift Master<span style="float: right"><a href="gift_master.aspx" class="btn btn-primary btn-update" id="newsf">Add Gift</a></span></div>
        </div>
                ...
                <table class="table table-hover" id="OrderList" style="font-size: 12px">
                    <thead class="text-warning">
                        <tr style="background-color: #37a4c6; color: #fff; white-space: nowrap;">
                            ...
                            <th style="text-align: left; color: #fff;">Gift Name</th>
                            <th style="text-align: left; color: #fff">Points Value</th>
                            <th style="text-align: left; color: #fff">Value Of Gift</th>
                            <th style="text-align: left; color: #fff">Effective From</th>
                            <th style="text-align: left; color: #fff">Effective To</th>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var Orders = []; pgNo = 1; PgRecords = 10; TotalPg = 0; searchKeys = "Gift_Name,Effective_from,Effective_To,";
        var routes = [];
        var filtrkey = '0';
        ...
        function ReloadTable() {
            ...
            for ($i = st; $i < st + Number(PgRecords); $i++) {
                if ($i < Orders.length) {
                    ...
                    tr = $("<tr rname='" + Orders[$i].Gift_Name + "' rocode='" + Orders[$i].Gift_Code + "'></tr>");
                    $(tr).html("<td>" + ($i + 1) + "</td><td>" + Orders[$i].Gift_Name + "</td><td>" + Orders[$i].Points_Value + "</td><td>" + Orders[$i].Gift_val + "</td><td>" + Orders[$i].Effective_from + "</td><td>" + Orders[$i].Effective_To + "</td><td class='roedit'><a href='#'>Edit</a></td><td class='rodeact'><a href='#'>" + Orders[$i].Status + "</a></td>");
        ...
        function fillgift() {
            $.ajax({
                ...
                async: false,
                url: "view_gift_master.aspx/getfillgft",
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.roedit', function () {
                ...
                window.location.href = "gift_master.aspx?gift_code=" + route_C + "";
            });
            $(document).on("click", ".rodeact", function () {
                var route_C = $(this).closest('tr').attr('rocode');
                let oindex = Orders.findIndex(x => x.Gift_Code == route_C);
                ...
                if ((flg == 1) || (flg == 0)) {
                    if (confirm("Do you want " + disstat + " the Distributor ?")) {
                        $.ajax({
                            ...
                            contentType: "application/json; charset=utf-8",
                            async: false,
                            url: "view_gift_master.aspx/deactivategift",
                            data: "{'arcode':'" + route_C + "','stat':'" + flg + "'}",
                            dataType: "json",
                            success: function (data) {
                                if (data.d == 'Success') {
                                    Orders[oindex]["Active_flag"] = flg;
