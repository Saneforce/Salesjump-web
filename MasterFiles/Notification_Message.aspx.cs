...
using DBase_EReport;
public partial class MasterFiles_Notification_Message : System.Web.UI.Page
{
 ...
 hdnslno.Value = trans_sl_no;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            iReturn = sf.AddNotification(div_code, SF_Code, SF_Name, txtmsg.Text, Edate); //from date getting insert as today date
        }
        else
        {
            iReturn=sf.Notification_update( SF_Code, SF_Name, txtmsg.Text, Edate, trans_sl_no,div_code);//from date getting updated as today date
        }
        if (iReturn > 0)
        {
    ...
    public static string fillData()
    {
        ...
        DataSet ds = new DataSet();
         if (trans_sl_no == "" || trans_sl_no == null)
        {
            ds = sf.getNotification(Div_Code.Trim(','));
        }
        else
        {
            ds = sf.getNotification(Div_Code.Trim(','), trans_sl_no);
    ...
    {
        public int AddNotification(string div_code, string SF_Code, string SF_Name, string Notification_Message, string enddate)
        {
            int iReturn = -1;
            int Trans_Sl_No = -1;
            ...
            try
            {
                ...
                string strQry = "SELECT ISNULL(max(Trans_Sl_No),0)+1 FROM Mas_Notification ";
        ...
        public int Notification_update(string SF_Code, string SF_Name, string Notification_Message, string enddate, string trans_sl_no, string div)
        {
            int iReturn = -1;
            DB_EReporting db = new DB_EReporting();
            string strQry = "Exec Notification_update '" + SF_Code + "','" + SF_Name + "' , '" + Notification_Message + "','" + enddate + "' ,'" + trans_sl_no + "' ,'" + div + "'";
            try
            {
