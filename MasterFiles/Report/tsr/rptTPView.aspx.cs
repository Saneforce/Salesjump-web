...
public partial class MasterFiles_Temp_rptTPView : System.Web.UI.Page
{
    ...
    DataSet dsToursfdets = new DataSet();
    DataSet dsTP = null;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("select sf_emp_id,Sf_Name,Sf_HQ from Mas_Salesforce where Sf_Code='" + sfCode + "'", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dsToursfdets);
            lbleid.Text = ":" + dsToursfdets.Tables[0].Rows[0].ItemArray.GetValue(0).ToString();
            lblename.Text = ":" + dsToursfdets.Tables[0].Rows[0].ItemArray.GetValue(1).ToString();
            lblhqq.Text = ":" + dsToursfdets.Tables[0].Rows[0].ItemArray.GetValue(2).ToString();
    ...
    protected void TourPlan()
    {
        try
        {
            ...
            if (dsTerritory.Tables[0].Rows.Count > 0)
            {
                ...
                if (strTPView == "3")
                {
                    grdTP.Columns[0].Visible = true;
                    grdTP.Columns[1].Visible = true;
                    grdTP.Columns[2].Visible = true;
