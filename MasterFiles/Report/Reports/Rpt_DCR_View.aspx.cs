...
public partial class Reports_Rpt_DCR_View : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        hdivcode.Value = div_code;
        sf_code = Request.QueryString["sf_code"].ToString();
        Sf_Name = Request.QueryString["Sf_Name"].ToString();
        strMode = Request.QueryString["Mode"].ToString();
        Fdate1 = Request.QueryString["FDate"].ToString();
        cmonth = Convert.ToInt16(Request.QueryString["cur_month"].ToString());
        cyear = Convert.ToInt16(Request.QueryString["cur_year"].ToString());
        Tdate1 = (Request.QueryString["TDate"].ToString() == "") ? Fdate : Request.QueryString["TDate"].ToString();

        DateTime result4 = DateTime.ParseExact(Fdate1, "d/MM/yyyy", CultureInfo.InvariantCulture);
        Fdate = result4.ToString("yyyy-MM-dd");

        DateTime result10 = DateTime.ParseExact(Tdate1, "d/MM/yyyy", CultureInfo.InvariantCulture);
        Tdate = result10.ToString("yyyy-MM-dd");

        strMode = strMode.Trim();
        hmode.Value = strMode.Trim().ToString();
        ...
        try
        {
            DateTime result = DateTime.ParseExact(Fdate, "yyyy-MM-d", CultureInfo.InvariantCulture);
            dt = result.ToString("dd/MM/yyyy");

            DateTime result1 = DateTime.ParseExact(Tdate, "yyyy-MM-d", CultureInfo.InvariantCulture);
        ...
        else if (strMode == "Detailed View")
        {
            ...
            //lblTitle1.Text = "Daily Call Report (<span style='color:Red'>" + "From :" + dt + " To :" + dt1 + "" + "</span>) Primary Detail View ";
        ...
        else if (strMode == "TP MY Day Plan")
        {
            ...
            {
                dsGV = dc.GetTPDayMap_MR(div_code, sf_code, Fdate, Tdate);
                if (dsGV.Tables[0].Rows.Count > 0)
                {
            ...
            {
                dsGV = dc.GetTPDayMap_MGR(div_code, sf_code, Fdate, Tdate);
                if (dsGV.Tables[0].Rows.Count > 0)
                {
                    gvMyDayPlan.DataSource = dsGV;
                    gvMyDayPlan.DataBind();
        ...
        else if (strMode == "Closing Stock View")
        {
            ...
            lblTitle.Text = "Daily Closing Report (<span style='color:Red'>" + "Stock View" + "</span>) for the month of " + sMonth;
            //lblFieldForceName.Text = Sf_Name;
            DataSet dsGV = new DataSet();
            DCR dc = new DCR();
            if (sf_code.Contains("MR"))
                ...
                if (dsGV.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < dsGV.Tables[0].Rows.Count; i++)
                    {
                        for (int j = 0; j < dsGV.Tables[0].Columns.Count; j++)
                        {
                            ...
                            {
                                if (j > 2)
                                    dsGV.Tables[0].Rows[i][j] = Math.Round(Convert.ToDecimal(dsGV.Tables[0].Rows[i][j]), 2).ToString();
                            }
                    ...
                    if (dsGV.Tables[0].Rows.Count > 0)
                    {
                        ...
                        //for (int k = 3; k < dsGV.Tables[0].Columns.Count; k++)
                        //{
                        ...
                        //    //GridView3.FooterRow.Cells[k].Font.Bold = true;
                        //    //GridView3.FooterRow.Cells[k].HorizontalAlign = HorizontalAlign.Left;
                        //    //GridView3.FooterRow.Cells[k].Text = total.ToString();
                        //    //GridView3.FooterRow.Cells[k].Font.Bold = true;
            ...
            {
                dsGV = dc.Get_Close_stock_MGR(sf_code, div_code, cmonth, cyear);
                if (dsGV.Tables[0].Rows.Count > 0)
                {
        ...
        else if (strMode == "Sales Return")
        {

            lblTitle.Text = "Sales Return(<span style='color:Red'>" + "Stock View" + "</span>) for the month of " + sMonth;
            //lblFieldForceName.Text = Sf_Name;
            DataSet dsGV = new DataSet();
            DCR dc = new DCR();
            if (sf_code.Contains("MR"))
            {
                ...
                dsGV = dc.Get_SalerRT(sf_code, div_code, cmonth, cyear);
                if (dsGV.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < dsGV.Tables[0].Rows.Count; i++)
                    {
                    ...
                    if (dsGV.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int k = 2; k < dsGV.Tables[0].Columns.Count; k++)
                        {
                            // Int32 td = (tbl1.Columns[k] == null ) ? 0 : Convert.ToInt32(tbl1.Columns[k].ToString());
        ...
        else if (strMode.Trim() == "View_All_DCR_Date(s)")
        {
            SalesForce sf1 = new SalesForce();
            DataSet dssf1 = sf1.getFW(sf_code, div_code);
            if (dssf1.Tables[0].Rows.Count > 0)
            {
                if ("NS" == dssf1.Tables[0].Rows[0].ItemArray.GetValue(0).ToString())
                {
                    ...
                    {
                        lblTitle.Text = "Daily Call Report (<span style='color:Red'>" + " - From :'" + dt + "' To :'" + dt + "'" + "</span>)";
            ...
            else
            {
                if (chk == "1")
                {
                    ...
                    CreateDynamicTableDCRDate2(Fdate, Tdate, sf_code);
                    //FillSalesForce(sf_code, cmonth, cyear);
                    SalesForce sf = new SalesForce();
                    DataSet dssf = sf.getSfName(sf_code);
                    if (dssf.Tables[0].Rows.Count > 0)
        ...
        else if (strMode == "SKU Summary")
        {
            ...
            lblTitle.Text = "<h1>SKU Summary Report - " + " From : <span style='color:Red'> " + dt + " </span> " + " To : <span style='color:Red'> " + dt1 + " </span> </h1>";
            ...
            tbl1 = dc1.view_MGR_order_viewyy_tbl2(div_code, sf_code, Fdate, Tdate);
            ...
            if (tbl1.Rows.Count > 0)
            {
                ...
                // decimal total = 0; ;
                gvtotalorder.FooterRow.Cells[1].Text = "TOTAL";
                gvtotalorder.FooterRow.Cells[1].Font.Bold = true;
                gvtotalorder.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Center;
        ...
        //Promotional Value Summary

        else if (strMode.Trim() == "Incentives Summary")
        {
            SalesForce sf1 = new SalesForce();
            DataSet dssf1 = sf1.getFW(sf_code, div_code);
            if (dssf1.Tables[0].Rows.Count > 0)
            {
                if ("NS" == dssf1.Tables[0].Rows[0].ItemArray.GetValue(0).ToString())
                {
                    DataSet dsGV = new DataSet();
                    SalesForce dc = new SalesForce();
                    Label1.Text = "Incentives Summary (<span style='color:Red'>" + "all Dates" + "</span>) view for the month of " + sMonth;
                    dsGV = dc.GetTable(sf_code, div_code);
                    if (dsGV.Tables[0].Rows.Count > 0)
                    {
                        GridView1.DataSource = dsGV;
                    ...
                    {
                        lblTitle.Text = "Incentives Summary (<span style='color:Red'>" + " - From :'" + dt + "' To :'" + dt + "'" + "</span>)";
                        CreateDynamicTableDCRDate_pro(Fdate, Tdate, sf_code);
                        //FillSalesForce(sf_code, cmonth, cyear);
                        SalesForce sf = new SalesForce();
                        DataSet dssf = sf.getSfName(sf_code);
                        if (dssf.Tables[0].Rows.Count > 0)
                ...
                {
                    lblTitle.Text = "Incentives Summary -" + "From :<span style='color:Red'>" + dt + " To :" + dt1 + "" + "</span>)";
                    CreateDynamicTableDCRDate_pro(Fdate, Tdate, sf_code);
                    //FillSalesForce(sf_code, cmonth, cyear);
                    SalesForce sf = new SalesForce();
                    DataSet dssf = sf.getSfName(sf_code);
            ...
            {
                lblTitle.Text = "<h1>Incentives Summary - " + " From : <span style='color:Red'> " + dt + " </span> " + " To : <span style='color:Red'> " + dt1 + " </span> </h1>";
                CreateDynamicTableDCRDate_pro(Fdate, Tdate, sf_code);
                //FillSalesForce(sf_code, cmonth, cyear);
                SalesForce sf = new SalesForce();
                DataSet dssf = sf.getSfName(sf_code);
        ...
        else if (strMode == "Primary SKU Summary")
        {

            lblTitle.Text = "<h1>Primary SKU Summary Report - " + " From : <span style='color:Red'> " + dt + " </span> " + " To : <span style='color:Red'> " + dt1 + " </span> </h1>";
            ...
            //dsGV = dc1.view_MGR_order_viewyy(div_code, sf_code, Fdate);
            tbl1 = dc1.view_MGR_order_viewyy_tbl3(div_code, sf_code, Fdate, Tdate);
            //   DataTable dt = new DataTable();
    ...
    protected void OnRowCreated(object sender, GridViewRowEventArgs e)
    {
        ...
        SalesForce SF = new SalesForce();
        DataSet ff = new DataSet();
        if (div_code == "26" || div_code == "32" || div_code == "35")
        {


            ff = SF.GetProduct_Name_for_cmy(Fdate, Tdate, div_code, strMode);
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            if (strMode == "Promotional Value Summary")
            {
    ...
    protected void OnRowCreated1(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            HeaderCell.HorizontalAlign = HorizontalAlign.Center;
            HeaderCell.BackColor = System.Drawing.Color.FromName("#33CCCC");
            HeaderCell.Text = (div_code != "70") ? "Retailer Name" : "Customer Name";
            //HeaderCell. = "Andalus";
            HeaderGridRow0.Cells.Add(HeaderCell);
    ...
    private void CreateDynamicTableDCRDate_pro(string Fdate, string Tdate, string sf_code)
    {
        DCR dc = new DCR();
        int tbcount = 0;
            ...
            foreach (DataRow drdoc in dsDCR.Tables[0].Rows)
            {
                ...
                if (dsdoc.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                    {
                        ...
                        tc_det_SNo.ID = "wrktypeidpro" + tbcount;
                        iCount += 1;
                        ...
                        if (sf_code.Contains("MR"))
                        {
                            ...
                            //tc_Terr.Controls.Add(tbl);
                            //tr_ff.Cells.Add(tc_Terr);
                            //tbl.Rows.Add(tr_ff);
                            ...
                            tbldetail.ID = "promotionaltbl" + tbcount;
                ...
                else
                {
                    ...
                    {
                        dsdoc = dc.get_dcr_details1(sf_code, drdoc["Activity_Date"].ToString(), 1, drdoctor1["stockist_code"].ToString()); //1-Listed Doctor
                        iCount = 0;
                        ...
                        if (dsdoc.Tables[0].Rows.Count > 0)
                        {
                            ...
                            tc_det_head_doc.HorizontalAlign = HorizontalAlign.Center;
                            Literal lit_det_head_doc = new Literal();
                            lit_det_head_doc.Text = (div_code != "70") ? "Retailer Name" : "Customer Name";
                            tc_det_head_doc.Attributes.Add("Class", "tr_det_head");
                            tc_det_head_doc.Controls.Add(lit_det_head_doc);
                            ...
                            TableCell tc_det_head_Add = new TableCell();
                            tc_det_head_Add.BorderStyle = BorderStyle.Solid;
                            tc_det_head_Add.BorderWidth = 1;
                            tc_det_head_Add.HorizontalAlign = HorizontalAlign.Center;
                            Literal lit_det_head_Add = new Literal();
                            lit_det_head_Add.Text = "<b>Address</b>";
                            tc_det_head_Add.Attributes.Add("Class", "tr_det_head");
                            tc_det_head_Add.Controls.Add(lit_det_head_Add);
                            tr_det_head.Cells.Add(tc_det_head_Add);

                            TableCell tc_det_head_cpn = new TableCell();
                            tc_det_head_cpn.BorderStyle = BorderStyle.Solid;
                            tc_det_head_cpn.BorderWidth = 1;
                            tc_det_head_cpn.HorizontalAlign = HorizontalAlign.Center;
                            Literal lit_det_head_cpn = new Literal();
                            lit_det_head_cpn.Text = "<b>Contact Person</b>";
                            tc_det_head_cpn.Attributes.Add("Class", "tr_det_head");
                            tc_det_head_cpn.Controls.Add(lit_det_head_cpn);
                            tr_det_head.Cells.Add(tc_det_head_cpn);

                            TableCell tc_det_head_mob = new TableCell();
                            tc_det_head_mob.BorderStyle = BorderStyle.Solid;
                            tc_det_head_mob.BorderWidth = 1;
                            tc_det_head_mob.HorizontalAlign = HorizontalAlign.Center;
                            Literal lit_det_head_mob = new Literal();
                            lit_det_head_mob.Text = "<b>Mobile</b>";
                            tc_det_head_mob.Attributes.Add("Class", "tr_det_head");
                            tc_det_head_mob.Controls.Add(lit_det_head_mob);
                            tr_det_head.Cells.Add(tc_det_head_mob);
                            ...
                            if (div_code == "35" || div_code == "32")
                            {
                                ...
                                dsTerritory = terr.getProdName_dcrviewalldates(sf_code, div_code, adate, dsdoc.Tables[0].Rows[0]["Plan_No"].ToString(), drdoctor1["stockist_code"].ToString());
                            ...
                            foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                            {
                                ...
                                if (div_code == "11" || div_code == "13" || div_code == "8" || div_code == "35" || div_code == "32")
                                {
                            ...
                            lit_det_head_gift.Text = "<b>Total</b>";
                            ...
                            foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                            {
                                ...
                                lit_det_Add.Text = "&nbsp;&nbsp;" + drdoctor["ListedDr_Address1"].ToString();
                                tc_det_Add.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_Add.BorderStyle = BorderStyle.Solid;
                                tc_det_Add.BorderWidth = 1;
                                tc_det_Add.Controls.Add(lit_det_Add);
                                tr_det_sno.Cells.Add(tc_det_Add);

                                TableCell tc_det_cpn = new TableCell();
                                Literal lit_det_cpn = new Literal();
                                lit_det_cpn.Text = "&nbsp;&nbsp;" + drdoctor["Contact_Person_Name"].ToString();
                                tc_det_cpn.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_cpn.BorderStyle = BorderStyle.Solid;
                                tc_det_cpn.BorderWidth = 1;
                                tc_det_cpn.Controls.Add(lit_det_cpn);
                                tr_det_sno.Cells.Add(tc_det_cpn);

                                TableCell tc_det_mob = new TableCell();
                                Literal lit_det_mob = new Literal();
                                lit_det_mob.Text = "&nbsp;&nbsp;" + drdoctor["ListedDr_Mobile"].ToString();
                                tc_det_mob.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_mob.BorderStyle = BorderStyle.Solid;
                                tc_det_mob.BorderWidth = 1;
                                tc_det_mob.Controls.Add(lit_det_mob);
                                tr_det_sno.Cells.Add(tc_det_mob);
                                ...
                                DCR sf = new DCR();
                                dsDrr = sf.dcr_Gettransno_pro(sf_code, div_code, hdate3, drdoctor["Trans_Detail_Info_Code"].ToString(), drdoctor1["stockist_code"].ToString());
                                int[] fi = new int[500];
                                int j = 0;
                                foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                                {
                                ...
                                lit_det_gift.Text = Tot.ToString();
                                ...
                                iTotLstCount += Decimal.Parse(Tot.ToString());
                            ...
                            foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                            {
                                ...
                                hyp_mon.Text = na[k].ToString();// Tot_Sec.ToString();
                                if (hyp_mon.Text == "0" && div_code != "26")
                                {
                                    tc_tot.Visible = false;
                    ...
                    dsdoc = dc.get_dcr_stk_details(sf_code, drdoc["Activity_Date"].ToString(), 3); //3-Stockist-P
                    iCount = 0;
                    int[] na1 = new int[500];
                    double qvaltotale = 0;
                    int totalcolspancount = 0;
                    if (dsdoc.Tables[0].Rows.Count > 0)
                    {
                        Table tbldetail_main3 = new Table();
                        //tbldetail_main3.BorderStyle = BorderStyle.None;
                        ...
                        Territory terr = new Territory();
                        if (div_code == "35" || div_code == "26" || div_code == "32")
                        {
                            dsTerritory = terr.getProdName_prmy_for_company(Fdate, Tdate, sf_code);
                        ...
                        foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                        {
                            totalcolspancount = dsTerritory.Tables[0].Rows.Count;
                            TableHeaderCell tc = new TableHeaderCell();
                        ...
                        foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                        {
                            ...
                            int j = 0;
                            double qval = 0;
                            lcou = dsTerritory.Tables[0].Rows.Count;
                            foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                            {
                                string pCode = pro1["Product_Detail_Code"].ToString();
                                var DSRows = (from w in dsDrr.Tables[0].AsEnumerable() where w.Field<string>("Product_Code") == pCode select w);
                        ...
                        TableRow tr_total = new TableRow();
                        tr_total.ID = "Primarytbltotal1";
                        TableCell tc_Count_Total = new TableCell();
                        tc_Count_Total.ID = "totalid1";
                        tc_Count_Total.BackColor = System.Drawing.Color.Beige;
                        tc_Count_Total.BorderStyle = BorderStyle.Solid;
                        tc_Count_Total.BorderWidth = 1;
                        tc_Count_Total.ColumnSpan = lcou + 4;
                        ...
                        lit_Lst_Count_Total.Text = Convert.ToString(qvaltotale);
                        ...
                        tbldetailstk.Rows.Add(tr_total);
                        ...
                        //tbldetail_main.Rows.Add(tr_total);
    ...
    private void CreateDynamicTableDCRDate1(string Fdate, string Tdate, string sf_code)
    {
        DCR dc = new DCR();
        int tbcount = 0;
        dsDCR1 = dc.Getsfcode_all(div_code, sf_code);


        foreach (DataRow sf_code1 in dsDCR1.Tables[0].Rows)
        {

            detorderval = 0;
            detnetval = 0;
            sf_code = sf_code1["Sf_Code"].ToString();
            dsDCR = dc.get_dcr_DCRPendingdate(sf_code, div_code, Fdate, Tdate);
            ...
            if (dsDCR.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    tbcount += 1;
                    newvalldcr.Value = tbcount.ToString();
                    ...
                    if (dsdoc.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                        {
                            TableRow tr_det_sno = new TableRow();
                            TableCell tc_det_SNo = new TableCell();
                            tc_det_SNo.ID = "wrktypeid" + tbcount;
                            iCount += 1;
                            Literal lit_det_SNo2 = new Literal();
                            if (sf_code.Contains("MR"))
                            {
                                ...
                                Table tbldetail = new Table();
                                tbldetail.ID = "Viewalldcrdates" + tbcount;
                                tbldetail.BorderStyle = BorderStyle.Solid;
                                tbldetail.BorderWidth = 1;
                                tbldetail.GridLines = GridLines.Both;
                                tbldetail.Width = 1000;
                    ...
                    else
                    {
                        ...
                        {
                            dsdoc1 = dc.get_dcr_details_mr(sf_code, drdoc["Activity_Date"].ToString(), 1); //1-Listed Doctor
                        }
                        iCount = 0;
                        ...
                        foreach (DataRow drdoctor1 in dsdoc1.Tables[0].Rows)
                        {
                            ...
                            if (dsdoc.Tables[0].Rows.Count > 0)
                            {
                                ...
                                TableCell tc_det_head_Code = new TableCell();
                                tc_det_head_Code.BorderStyle = BorderStyle.Solid;
                                tc_det_head_Code.BorderWidth = 1;
                                tc_det_head_Code.HorizontalAlign = HorizontalAlign.Center;
                                Literal lit_det_head_Code = new Literal();
                                tc_det_head_Code.Attributes.Add("Class", "tr_det_head");
                                lit_det_head_Code.Text = "<b>ERP Code</b>";
                                tc_det_head_Code.Controls.Add(lit_det_head_Code);
                                tr_det_head.Cells.Add(tc_det_head_Code);
                                ...
                                TableCell tc_ordertype = new TableCell();
                                tc_ordertype.BorderStyle = BorderStyle.Solid;
                                tc_ordertype.BorderWidth = 1;
                                tc_ordertype.HorizontalAlign = HorizontalAlign.Center;
                                Literal lit_ordertype = new Literal();
                                lit_ordertype.Text = "<b>OrderType</b>";
                                tc_ordertype.Attributes.Add("Class", "tr_det_head");
                                tc_ordertype.Controls.Add(lit_ordertype);
                                tr_det_head.Cells.Add(tc_ordertype);

                                TableCell tc_det_head_Geo = new TableCell();
                                tc_det_head_Geo.BorderStyle = BorderStyle.Solid;
                                tc_det_head_Geo.BorderWidth = 1;
                                tc_det_head_Geo.HorizontalAlign = HorizontalAlign.Center;
                                Literal lit_det_head_Geo = new Literal();
                                lit_det_head_Geo.Text = "<b>Geo Address</b>";
                                tc_det_head_Geo.Attributes.Add("Class", "tr_det_head");
                                tc_det_head_Geo.Controls.Add(lit_det_head_Geo);
                                tr_det_head.Cells.Add(tc_det_head_Geo);
                                ...
                                Territory terr = new Territory();
                                if (div_code == "35" || div_code == "32" || div_code == "45" || div_code == "4" || div_code == "53" || div_code == "52" || div_code == "59" || div_code == "19" || div_code == "20" || div_code == "57")
                                {
                                    string adate = string.Empty;
                                    adate = drdoc["Activity_Date"].ToString().Trim();
                                ...
                                foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                                {
                                    ...
                                    tc.HorizontalAlign = HorizontalAlign.Center;
                                    Literal lic = new Literal();
                                    if (div_code == "11" || div_code == "13" || div_code == "8" || div_code == "35" || div_code == "32" || div_code == "45" || div_code == "4" || div_code == "53" || div_code == "52" || div_code == "59" || div_code == "19" || div_code == "20" || div_code == "57")
                                    {
                                        lic.Text = "<b>" + pro1["Product_Detail_Name"].ToString() + "</b>";
                                    }
                                    else
                                ...
                                foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                                {
                                    ...
                                    lit_det_Code.Text = "&nbsp;&nbsp;" + drdoctor["Code"].ToString();
                                    tc_det_Code.Attributes.Add("Class", "tbldetail_Data");
                                    ...
                                    TableCell tc_ordertyp = new TableCell();
                                    Literal lit_ordertyp = new Literal();
                                    lit_ordertyp.Text = "&nbsp;&nbsp;" + drdoctor["OrderType"].ToString();
                                    tc_ordertyp.Attributes.Add("Class", "tbldetail_Data");
                                    tc_ordertyp.BorderStyle = BorderStyle.Solid;
                                    tc_ordertyp.BorderWidth = 1;
                                    tc_ordertyp.Width = 500;
                                    tc_ordertyp.Controls.Add(lit_ordertyp);
                                    tr_det_sno.Cells.Add(tc_ordertyp);



                                    TableCell tc_det_Geo = new TableCell();
                                    Literal lit_det_Geo = new Literal();
                                    //lit_det_Geo.Text = "&nbsp;&nbsp;" + drdoctor["Geo"].ToString();
                                    //if (drdoctor["GeoAddrs"].ToString().Trim() == "NA" && drdoctor["lati"] != "")
                                    //{
                                    //    lit_det_Geo.Text = strlongname;
                                    //}
                                    //else if (drdoctor["GeoAddrs"].ToString().Trim() != "NA")
                                    //{
                                    //    lit_det_Geo.Text = "&nbsp;&nbsp;" + drdoctor["GeoAddrs"].ToString();
                                    //}
                                    //else if (drdoctor["GeoAddrs"].ToString().Trim() == "NA")
                                    //{
                                    //    lit_det_Geo.Text = "";
                                    ...
                                    if (drdoctor["GeoAddrs"].ToString().Trim() == "NA" && drdoctor["lati"] != "")
                                    {
                                        lit_det_Geo.Text = ""; //getLocationByGeoLocation(drdoctor["long"].ToString(), drdoctor["lati"].ToString());
                                    }
                                    else
                                    {
                                        lit_det_Geo.Text = "&nbsp;&nbsp;" + drdoctor["GeoAddrs"].ToString();
                                    ...
                                    tc_det_Geo.Attributes.Add("Class", "tbldetail_Data");
                                    tc_det_Geo.Attributes.Add("Class", "latlong");
                                    tc_det_Geo.Attributes.Add("lat", drdoctor["lati"].ToString());
                                    tc_det_Geo.Attributes.Add("long", drdoctor["long"].ToString());
                                    tc_det_Geo.BorderStyle = BorderStyle.Solid;
                                    tc_det_Geo.BorderWidth = 1;
                                    tc_det_Geo.Width = 500;
                                    tc_det_Geo.Controls.Add(lit_det_Geo);
                                    tr_det_sno.Cells.Add(tc_det_Geo);
                                    ...
                                    DCR sf = new DCR();
                                    dsDrr = sf.dcr_Gettransno(drdoctor1["sf_code"].ToString(), div_code, hdate3, drdoctor["Trans_Detail_Info_Code"].ToString(), drdoctor1["stockist_code"].ToString());
                                    int[] fi = new int[500];
                                    int j = 0;
                                    foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                                    {
                                    ...
                                    tc_det_gift.Controls.Add(lit_det_gift);
                                    tr_det_sno.Cells.Add(tc_det_gift);
                                    stURL = "rpt_dcrproductdetail.aspx?Sf_Code=" + drdoctor1["sf_code"].ToString() + "&Activity_date=" + drdoc["Activity_Date"].ToString() + "&div_code=" + div_code + "&Sf_Name=" + Sf_Name + "&retailer_name=" + drdoctor["ListedDr_Name"].ToString() + "&retailer_code=" + drdoctor["Trans_Detail_Info_Code"].ToString() + "";
                                    lit_det_gift.Attributes["onclick"] = "javascript:window.open('" + stURL + "' ,'','resizable=yes,toolbar=no,menubar=no,scrollbars=1,status=no,width=500,height=600,left=0,top=0,zoom=50%');";
                                    iTotLstCount += Decimal.Parse(drdoctor["POB_Value"].ToString());
                                    Session["gsf_code"] = drdoctor1["sf_code"].ToString();
                                    lit_det_gift.NavigateUrl = "#";
                                    TableCell tc_det_CallFeedBack = new TableCell();
                                    Literal lit_det_CallFeedBack = new Literal();
                                ...
                                foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                                {
                                    ...
                                    hyp_mon.Text = na[k].ToString();// Tot_Sec.ToString();
                                    if (hyp_mon.Text == "0" && div_code != "26" && div_code != "45" && div_code != "4" && div_code != "53" && div_code != "52" && div_code != "20" && div_code != "19" && div_code != "57")
                                    {
                                        tc_tot.Visible = false;
                        ...
                        //        DCR sf6 = new DCR();
                        //        dsDrr = sf6.dcr_Getcount(Session["gsf_code"].ToString(), div_code, drdoc["Activity_Date"].ToString());
                        ...
                        //            lit_det_head_hq3.Text = "<b>Category Wise Product</b>";
                        ...
                        //        DCR sf3 = new DCR();
                        //        dsDrr = sf3.dcr_Getcount1(Session["gsf_code"].ToString(), div_code, drdoc["Activity_Date"].ToString());
                        ...
                        //        DCR sf8 = new DCR();
                        //        dsDrr = sf8.dcr_Item_sum(Session["gsf_code"].ToString(), div_code, hdate1);
                        ...
                        dsDemo = dc.get_trans_demo_head(sf_code, drdoc["Activity_Date"].ToString());
                        if (dsDemo.Tables[0].Rows.Count > 0)
                        {
                            Table tbldemo = new Table();
                            tbldemo.BorderStyle = BorderStyle.Solid;
                            tbldemo.BorderWidth = 1;
                            tbldemo.GridLines = GridLines.Both;
                            tbldemo.Width = 1000;
                            tbldemo.Style.Add("border-collapse", "collapse");
                            tbldemo.Style.Add("border", "solid 1px Black");

                            TableRow trdemohead = new TableRow();
                            TableCell tcdemohead = new TableCell();
                            tcdemohead.BorderStyle = BorderStyle.Solid;
                            tcdemohead.HorizontalAlign = HorizontalAlign.Center;
                            tcdemohead.BorderWidth = 1;
                            Literal lbldemohead = new Literal();
                            lbldemohead.Text = "<b>S.No</b>";
                            tcdemohead.Attributes.Add("Class", "tr_det_head");
                            tcdemohead.Controls.Add(lbldemohead);
                            trdemohead.Cells.Add(tcdemohead);

                            tcdemohead = new TableCell();
                            tcdemohead.BorderStyle = BorderStyle.Solid;
                            tcdemohead.HorizontalAlign = HorizontalAlign.Center;
                            tcdemohead.BorderWidth = 1;
                            lbldemohead = new Literal();
                            lbldemohead.Text = "<b>Date</b>";
                            tcdemohead.Attributes.Add("Class", "tr_det_head");
                            tcdemohead.Controls.Add(lbldemohead);
                            trdemohead.Cells.Add(tcdemohead);
                            ...
                            tcdemohead.BorderWidth = 1;
                            lbldemohead = new Literal();
                            lbldemohead.Text = "<b>Contact Name</b>";
                            tcdemohead.Attributes.Add("Class", "tr_det_head");
                            tcdemohead.Controls.Add(lbldemohead);
                            trdemohead.Cells.Add(tcdemohead);
                            ...
                            tcdemohead.BorderWidth = 1;
                            lbldemohead = new Literal();
                            lbldemohead.Text = "<b>Contact No.</b>";
                            tcdemohead.Attributes.Add("Class", "tr_det_head");
                            tcdemohead.Controls.Add(lbldemohead);
                            trdemohead.Cells.Add(tcdemohead);
                            ...
                            tcdemohead.BorderWidth = 1;
                            lbldemohead = new Literal();
                            lbldemohead.Text = "<b>No. of Animals</b>";
                            tcdemohead.Attributes.Add("Class", "tr_det_head");
                            tcdemohead.Controls.Add(lbldemohead);
                            trdemohead.Cells.Add(tcdemohead);
                            ...
                            tcdemohead.BorderWidth = 1;
                            lbldemohead = new Literal();
                            lbldemohead.Text = "<b>Used Semen</b>";
                            tcdemohead.Attributes.Add("Class", "tr_det_head");
                            tcdemohead.Controls.Add(lbldemohead);
                            trdemohead.Cells.Add(tcdemohead);
                            ...
                            tbldemo.Rows.Add(trdemohead);

                            iCount = 0;
                            foreach (DataRow drdoctor in dsDemo.Tables[0].Rows)
                            {
                                TableRow tr_det_sno = new TableRow();
                                TableCell tc_det_dr_name = new TableCell();
                                Literal lit_det_dr_name = new Literal();
                                iCount += 1;
                                lit_det_dr_name.Text = "<center>" + iCount.ToString() + "</center>";
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + Convert.ToDateTime(drdoctor["current_dateand_time"]).ToString("dd/MM/yyyy");
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["Route"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["formarname"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["contactnumber"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["Address"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["category"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["crop"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                                ...
                                tc_det_dr_name = new TableCell();
                                lit_det_dr_name = new Literal();
                                lit_det_dr_name.Text = "&nbsp;&nbsp;" + drdoctor["semen"].ToString();
                                tc_det_dr_name.Attributes.Add("Class", "tbldetail_Data");
                                tc_det_dr_name.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_name.BorderWidth = 1;
                                tc_det_dr_name.Controls.Add(lit_det_dr_name);
                        ...
                        dsdoc = dc.get_dcr_stk_details(sf_code, drdoc["Activity_Date"].ToString(), 3); //3-Stockist-Pr
                        iCount = 0;
                        decimal[] na1 = new decimal[500];
                        double qvaltotale = 0;
                        int totalcolspancount = 0;
                        if (dsdoc.Tables[0].Rows.Count > 0)
                        {
                            ...
                            tc_det_head_add.BorderWidth = 1;
                            Literal lit_det_head_add = new Literal();
                            lit_det_head_add.Text = "<b>Stockist Address</b>";
                            tc_det_head_add.Attributes.Add("Class", "tr_det_head");
                            tc_det_head_add.Controls.Add(lit_det_head_add);
                            tr_det_head.Cells.Add(tc_det_head_add);
                            ...
                            lit_det_head_LastUpdate.Text = "<b>Remarks</b>";
                            ...
                            foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                            {
                                ...
                                TableCell tc_det_dr_add = new TableCell();
                                Literal lit_det_dr_add = new Literal();
                                lit_det_dr_add.Text = "&nbsp;&nbsp;" + drdoctor["Stockist_Address"].ToString();
                                tc_det_dr_add.BorderStyle = BorderStyle.Solid;
                                tc_det_dr_add.BorderWidth = 1;
                                tc_det_dr_add.Controls.Add(lit_det_dr_add);
                                tr_det_sno.Cells.Add(tc_det_dr_add);
                                ...
                                foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                                {
                                    ...
                                    tc.Controls.Add(lic);
                                    tr_det_sno.Cells.Add(tc);
                                    fi[j] += sQty == "" ? 0 : Convert.ToDecimal(sQty);
                                    j++;
                                }
                                for (int l = 0; l < dsTerritory.Tables[0].Rows.Count; l++)
                                {
                                    na1[l] += Convert.ToDecimal(fi[l]);
                                }
                                ...
                                lit_det_dr_LastUpdate.Text = "&nbsp;&nbsp;" + drdoctor["Activity_Remarks"].ToString();
                            ...
                            TableRow tr_total = new TableRow();
                            tr_total.ID = "Primarytbltotal";
                            TableCell tc_Count_Total = new TableCell();
                            tc_Count_Total.ID = "totalid";
                            tc_Count_Total.BackColor = System.Drawing.Color.Beige;
                            tc_Count_Total.BorderStyle = BorderStyle.Solid;
                            tc_Count_Total.BorderWidth = 1;
                            tc_Count_Total.ColumnSpan = lcou + 5;
                            //tc_catg_Total.Width = 25;
                            Literal lit_Count_Total = new Literal();
    ...
    private void CreateDynamicTableDCRDate2(string Fdate, string Tdate, string sf_code)
    {
        DCR dc = new DCR();
        int tbcount = 0;
        ...
        //detnetval = 0;
        //sf_code = sf_code1["Sf_Code"].ToString();
        dsDCR = dc.get_dcr_DCRPendingdate1(sf_code, div_code, Fdate, Tdate);
                ...
                else
                {
                    ...
                    foreach (DataRow drdoctor1 in dsdoc1.Tables[0].Rows)
                    {
                        ...
                        if (dsdoc.Tables[0].Rows.Count > 0)
                        {
                            ...
                            Territory terr = new Territory();
                            if (div_code == "35" || div_code == "32" || div_code == "45" || div_code == "4" || div_code == "53" || div_code == "52" || div_code == "59")
                            {
                                string adate = string.Empty;
                                adate = drdoc["Activity_Date"].ToString().Trim();
                            ...
                            foreach (DataRow pro1 in dsTerritory.Tables[0].Rows)
                            {
                                ...
                                tc.HorizontalAlign = HorizontalAlign.Center;
                                Literal lic = new Literal();
                                if (div_code == "11" || div_code == "13" || div_code == "8" || div_code == "35" || div_code == "32" || div_code == "45" || div_code == "4" || div_code == "53" || div_code == "52" || div_code == "59")
                                {
                                    lic.Text = "<b>" + pro1["Product_Detail_Name"].ToString() + "</b>";
                                }
                                else
                    ...
                    dsdoc = dc.get_dcr_stk_details(sf_code, drdoc["Activity_Date"].ToString(), 3); //3-Stockist-Pr2
                    ...
                    if (dsdoc.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                        {
                            ...
                            if (sf_code.Contains("MGR"))
                            {
                                ...
                                tc_det_spec.Controls.Add(lit_det_spec);
                                tr_det_sno.Cells.Add(tc_det_spec);
                                qvaltotale1 += (drdoctor["POB"] == DBNull.Value ? 0 : Decimal.Parse(drdoctor["POB"].ToString()));
                                tbldetailstk.Rows.Add(tr_det_sno);
                        ...
                        {
                            lit_Lst_Count_Total.Text = Convert.ToString(qvaltotale1);
                        }
                        tc_Lst_Count_Total.Controls.Add(lit_Lst_Count_Total);
                        tc_Lst_Count_Total.HorizontalAlign = HorizontalAlign.Center;
                        tc_Lst_Count_Total.VerticalAlign = VerticalAlign.Middle;
                    ...
                    foreach (DataRow drdoc in dsDCR.Tables[0].Rows)
                    {
                        ...
                        if (dsdoc.Tables[0].Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                            {
                                ...
                                tblvalue.HorizontalAlign = HorizontalAlign.Center;
                                Literal lblvalue = new Literal();
                                string value = Convert.ToString(drdoctor["POB_Value"].ToString());

                                qvaltotale += Convert.ToDouble(drdoctor["POB_Value"].ToString());
                                lblvalue.Text = value;
                                tblvalue.BorderStyle = BorderStyle.Solid;
                                tblvalue.Attributes.Add("Class", "tbldetail_main");
                                //tc_det_lvisit.BorderWidth = 1;
                            ...
                            tr_total.ID = "Primarytbltotal";
                            TableCell tc_Count_Total = new TableCell();
                            tc_Count_Total.ID = "totalid13";
                            tc_Count_Total.BackColor = System.Drawing.Color.Beige;
                            tc_Count_Total.BorderStyle = BorderStyle.Solid;
                            tc_Count_Total.BorderWidth = 1;
                            tc_Count_Total.ColumnSpan = totalcolspancount + 4;
                            //tc_catg_Total.Width = 25;
                            Literal lit_Count_Total = new Literal();
                        ...
                        //    Territory terr = new Territory();
                        //    //dsTerritory = terr.getWorkAreaName(div_code);
                        //    lit_det_head_Ses.Text = "<b>DB Name</b>";
                        //    tc_det_head_Ses.Attributes.Add("Class", "tr_det_head");
                        //    tc_det_head_Ses.Controls.Add(lit_det_head_Ses);
                        //    tr_det_head.Cells.Add(tc_det_head_Ses);
                        ...
                        //        TableCell tc_det_dr_name = new TableCell();
                        //        Literal lit_det_dr_name = new Literal();
                        //        tc_det_dr_name.Style.Add("width", "55px");
    ...
    private void CreateDynamicDCRDetailedView(string Fdate, string Tdate, string sf_code)
    {
        try
        {
            ...
            {
                tbdcount += 1;
                hdetailedview.Value = tbdcount.ToString();
                detorderval = 0;
                detnetval = 0;
                sf_code = sf_code1["Sf_Code"].ToString();
                dsDCR = dc.get_dcr_DCRPendingdate(sf_code, div_code, Fdate, Tdate);
                if (dsDCR.Tables[0].Rows.Count > 0)
                {
                    ...
                    Table tbl = new Table();
                    tbl.Style.Add("padding", "2px 5px");
                    tbl.Style.Add("white-space", "nowrap");
                    //tbl.Width = 1000;
                    ...
                    tbldetail_main.BorderStyle = BorderStyle.None;
                    tbldetail_main.GridLines = GridLines.Both;
                    tbldetail_main.Style.Add("padding", "2px 5px");
                    tbldetail_main.Style.Add("white-space", "nowrap");
                    //tbldetail_main.Width = 1000;
                    //tbldetail_main.Style.Add("border-collapse", "collapse");
                    //tbldetail_main.Style.Add("border", "solid 1px Black");
                    //tbldetail_main.Style.Add("margin-left", "100px");
                    ...
                    Table tbldetail = new Table();
                    tbldetail.ID = "tbldetailview" + tbdcount;
                    tbldetail.BorderStyle = BorderStyle.Solid;
                    tbldetail.BorderWidth = 1;
                    tbldetail.GridLines = GridLines.Both;
                    //tbldetail.Width = 1000;
                    ...
                    if (dsdoc.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (div_code == "26" || div_code == "32" || div_code == "35")
                        {
                            dsTerritory = terr1.getProdName_detailed_for_company(Fdate, Tdate, sf_code);
                        }
                        else
                        {
                        ...
                        tc_det_head_gift.HorizontalAlign = HorizontalAlign.Center;
                        Literal lit_det_head_gift = new Literal();
                        lit_det_head_gift.Text = "<b>Net Value</b>";
                        tc_det_head_gift.Attributes.Add("Class", "tr_det_head");
                        tc_det_head_gift.Controls.Add(lit_det_head_gift);
                        tr_det_head.Cells.Add(tc_det_head_gift);
                        ...
                        TableCell tc_det_head_gift1 = new TableCell();
                        tc_det_head_gift1.BorderStyle = BorderStyle.Solid;
                        tc_det_head_gift1.BorderWidth = 1;
                        tc_det_head_gift1.HorizontalAlign = HorizontalAlign.Center;
                        Literal lit_det_head_gift1 = new Literal();
                        lit_det_head_gift1.Text = "<b>achieved value</b>";
                        tc_det_head_gift1.Attributes.Add("Class", "tr_det_head");
                        tc_det_head_gift1.Controls.Add(lit_det_head_gift1);
                        tr_det_head.Cells.Add(tc_det_head_gift1);
                        ...
                        foreach (DataRow drdoctor in dsdoc.Tables[0].Rows)
                        {
                            ...
                            TableCell tc_det_Ses = new TableCell();
                            HyperLink lit_det_Ses = new HyperLink();
                            if (drdoctor["Activity_Date"].ToString() == date)
                            {
                                lit_det_Ses.Text = "";
                                tc_det_Ses.Attributes.Add("Class", "tbldetail_main");
                            ...
                            {
                                lit_det_Ses.Text = drdoctor["Activity_Date"].ToString();
                                date = drdoctor["Activity_Date"].ToString();
                                tc_det_Ses.Attributes.Add("Class", "tbldetail_main");
                            ...
                            {
                                if ((strWorktypeName != "Holiday" && strWorktypeName != "Group work" && strWorktypeName != "Meeting" && strWorktypeName != "Weekly Off" && strWorktypeName != "Transit" && strWorktypeName != "Leave" && strWorktypeName != "Camp Work"))
                                {
                                    iFieldWrkCount += 1;
                                    DateTime NewDate = Convert.ToDateTime(drdoctor["Submission_Date"].ToString());
                                    string Mdate = NewDate.Month.ToString();
                                    string Ydate = NewDate.Year.ToString();
                                    sURL = "rptDCRViewApprovedDetails.aspx?sf_Name=" + "&sf_code=" + sf_code + "&Year=" + Ydate + "&Month=" + Mdate + "&div_code=" + div_code + " &Day=" + lit_det_Ses.Text + "";
                                    ...
                                    if (Hyllit_det_spec.Text != "0")
                                    {
                                        sURL = "rptDCRViewCompleted.aspx?sf_Name=" + "&sf_code=" + sf_code + "&Year=" + Ydate + "&Month=" + Mdate + "&Day=" + lit_det_Ses.Text + "&Type=" + 1 + "";
                                    ...
                                    string datet3 = string.Empty;
                                    string hdate3 = string.Empty;
                                    Activity_date3 = drdoctor["Activity_Dat"].ToString();
                                    datet3 = Activity_date3.Trim();
                                    DateTime dtt3 = Convert.ToDateTime(datet3);
                                    hdate3 = dtt3.ToString("yyyy-MM-dd");
                                    ...
                                    HyperLink hyllit_det_prod = new HyperLink();
                                    DCR sf6 = new DCR();
                                    dsDrr = sf6.dcr_Getcount(sf_code, div_code, drdoctor["Activity_Dat"].ToString());
                                    foreach (DataRow coun in dsDrr.Tables[0].Rows)
                                    {
                                        ...
                                        tc_det_prod1.Attributes.Add("style", "font-weight:bold;");
                                        ...
                                        tc_det_Che_POB.Attributes.Add("style", "font-weight:bold;");
                                        //tc_det_head_POB.Visible = false;
                                        tc_det_Che_POB.HorizontalAlign = HorizontalAlign.Center;
                                        tc_det_Che_POB.BorderWidth = 1;
                                        tc_det_Che_POB.Controls.Add(lit_det_Che_POB);
                                    ...
                                    tc_det_gift.HorizontalAlign = HorizontalAlign.Center;
                                    tc_det_gift.Attributes.Add("Class", "tbldetail_main");
                                    tc_det_gift.Attributes.Add("style", "font-weight:bold;");
                                    tc_det_gift.BorderWidth = 1;
                                    tc_det_gift.Controls.Add(hyllit_det_gift);
                                    tr_det_sno.Cells.Add(tc_det_gift);
                                    ...
                                    dsDrr1 = sf6.dcr_GetAch(sf_code, div_code, drdoctor["Activity_Dat"].ToString());
                                    foreach (DataRow coun1 in dsDrr1.Tables[0].Rows)
                                    {
                                        hyllit_det_UnDoc.Text = coun1["Ach"].ToString();
                                        if (hyllit_det_UnDoc.Text != "0")
                                        {
                                        ...
                                        tc_det_UnDoc.HorizontalAlign = HorizontalAlign.Center;
                                        tc_det_UnDoc.Attributes.Add("Class", "tbldetail_main");
                                        tc_det_UnDoc.Attributes.Add("style", "font-weight:bold;");
                                        //tc_det_UnDoc.BorderWidth = 1;
                                        tc_det_UnDoc.Controls.Add(hyllit_det_UnDoc);
                                        tr_det_sno.Cells.Add(tc_det_UnDoc);
                                        iTotUnLstCal += Convert.ToDouble(hyllit_det_UnDoc.Text);
                                    }
                        ...
                        TableRow tr_total = new TableRow();
                        tr_total.ID = "tdetailtotal" + tbdcount;
                        TableCell tc_Count_Total = new TableCell();
                        tc_Count_Total.BorderStyle = BorderStyle.Solid;
                        tc_Count_Total.BorderWidth = 1;
                        //tc_catg_Total.Width = 25;
                        ...
                        //tc_catg_Total.Width = 25;
                        Literal lit_UnLst_Count_Total = new Literal();
                        lit_UnLst_Count_Total.Text = Convert.ToString(iTotUnLstCal);
                        tc_UnLst_Count_Total.Controls.Add(lit_UnLst_Count_Total);
                        tc_UnLst_Count_Total.HorizontalAlign = HorizontalAlign.Center;
                        tc_UnLst_Count_Total.Attributes.Add("Class", "tbldetail_main");
                        tc_UnLst_Count_Total.Font.Bold.ToString();
    ...
    private string getLocationByGeoLocation(string longitude, string latitude)
    {
        ...
        try
        {
            ...
            string url = string.Format("https://maps.googleapis.com/maps/api/geocode/xml?latlng={0},{1}&key=123AAP_AIzaSyAER5hPywUW-5DRlyKJZEfsqgZlaqytxoU", latitude, longitude);
            ...
            using (WebResponse response = (HttpWebResponse)request.GetResponse())
            {
                using (StreamReader reader = new StreamReader(response.GetResponseStream(), Encoding.UTF8))
                {
                    ...
                    try
                    {
                        ...
                        //    locationName +=  row["formatted_address"].ToString() + "</br>" ;
                        //}
