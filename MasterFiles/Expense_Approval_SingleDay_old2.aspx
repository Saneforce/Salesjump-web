...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        var UserExp = [];
        var rejdt = '', SF_Code = '';
        var SF_Name = '', ExpFare = '', ExpDist = '', ExpDA = '', ExpDaAmnt = '', ExpTotal = '';
	...
	$(document).on('click', '.picc', function () {
        ...
        function Exp(x) {
            ...
            ExpDist = $(x).closest('tr').find('td').eq(6).text();
            ExpDA = $(x).closest('tr').find('td').eq(7).text();
            ExpDaAmnt = $(x).closest('tr').find('td').eq(8).text();
            ExpFare = $(x).closest('tr').find('td').eq(9).text();
            ExpTotal = $(x).closest('tr').find('td').eq(11).text();
        ...
        function View() {
            ...
            str = "<div>" + SF_Name + "</div><div class='card'><div class='card-body table-responsive' style='overflow: scroll;height: 320px;'>\
		 ...
		 imgsrt = '';
                for (var j = 0; j < img.length; j++) {		    
                    if (img[j] != "") {
                        imgsrt += '<img src="' + img[j] + '" alt="" class="picc" style="height: 35px;width: 25px;" />'
                    }
                }
                getUstr += "<tr><td>" + UserExp[i].expName + "</td><td>" + imgsrt + "</td><td>" + UserExp[i].Amt + "</td></tr>"
		}
            }
            ustr = "<table>" + getUstr + "</table>"
            str = "<tr>\
                       <td>"+ ExpDA + "</td>\
                       <td>"+ ExpDaAmnt + "</td>\
                       <td>"+ ExpDist + "</td>\
