...
public partial class MasterFiles_WrkTypeWise_Allowance : System.Web.UI.Page
{
    ...
    {
        TourPlan tp = new TourPlan();
        dsSalesForce = tp.GetFlag(SesSFCode, div_code, mon, year);

        foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
        {
            //string val = 
            sndhqfl = Convert.ToInt32(drFF[0].ToString());
            if (sndhqfl == 1 || sndhqfl==0)
            {
    ...
    private void FillBaseLevel()
    {
        ...
        {
            Response.Redirect("WrkTypeWise_Allowance_date.aspx");
        }
    }
    ...
    private void saveData(string flag)
    {
        ...
        if (iReturn >= 0)
        {
            terr.Expense_App(sfCode, Month, Year, flag, div_code, sf_name);
        }
        else
        { }
        if (iReturn == 0)
