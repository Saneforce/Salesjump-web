...
public partial class MasterFiles_Rpt_Secondary_Order_ : System.Web.UI.Page
{
    ...
    private void bindgrid()
    {
        ...
        sf_code = Convert.ToString(ddlFieldForce.SelectedValue);
        dsST = sd.get_secStockist(div_code, sf_code);
        SalesForce sf = new SalesForce();
        dsSf = sf.get_secOrder(sf_code, div_code, hdnffdate.Value, hdnttdate.Value);
    ...
    private void FillManager()
    {
        ...
        SalesForce sf = new SalesForce();
        dsSalesForce = sf.get_secfieldforce(div_code, hdnffdate.Value, hdnttdate.Value);
...
[WebMethod]
public static List<SecRetail> getretail(string sfcode)
    {
        string div_code = "";
        try
        {
        ...
        List<SecRetail> secRetails1 = new List<SecRetail>();
        Order sd = new Order();
        DataSet dsST;
        dsST = sd.get_secStockist(div_code, sfcode);
        foreach(DataRow row in dsST.Tables[0].Rows)
        {
            SecRetail retail = new SecRetail();
            retail.retailcode = row["Stockist_Code"].ToString();
            retail.retailname = row["Stockist_Name"].ToString();
            secRetails1.Add(retail);
        }
        return secRetails1;
    }

    [WebMethod]
    public static string updateSecorder(string secorder)
    {
        ...
        IList<SecOrder_values> prilist = JsonConvert.DeserializeObject<IList<SecOrder_values>>(secorder); ;
        Order sd = new Order();
        int ret = -1;
        try
        {
            ...
            {
                ret = sd.updatesecorder(prilist[i].order_flag, div_code, prilist[i].stockistcode, prilist[i].trans_sl_no);
            }
            sucess = "updated";
