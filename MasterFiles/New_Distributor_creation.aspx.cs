...
public partial class Stockist_New_Distributor_creation : System.Web.UI.Page
{
    ...
    public static DataSet getRates(string div_code)
    {
		...
		strQry = "select Price_list_Sl_No, Price_list_Name from Mas_Product_Wise_Bulk_rate_head where Division_Code = '" + div_code + "'   AND convert(date, Effective_To_Date) >= convert(date,getDATE()) order by Price_list_Name";
    ...
    public static string submit_distributor(string Div_Code, string Stock_code, string userentry, string stockist_code, string erp_code, string ContactPerson, string Stockist_Name, string sfusrname,
        ...
        string sfCategory, string sfGSTN, string stock, string sub_div,string sfcity)
    {
        ...
        if (stockist_code == "" || stockist_code == "null")
        {
            ...
            if (stkname == "")
            {
                iReturn = RecordAdd(Div_Code, userentry, Stockist_Name, sfAddress, ContactPerson, sfDesignation, sfMobile, erp_code, sfDistrictname, sfDistrict,
                          ...
                          sfCategory, sfemail, sfddlTalukname, sfddlTaluk, sfRate,sfcity);
            }
            else
            {
                iReturn = -2;
        ...
        else
        {
            ...
            if (stkname == "")
            {
                iReturn = RecordUpdate(Div_Code, stockist_code, userentry, Stockist_Name, sfAddress, ContactPerson, sfDesignation,
                   ...
                   sfHead, sfType, sfGSTN, sfCategory, sfemail, sfddlTalukname, sfddlTaluk, sfRate,sfcity);
            }
            else
            {
                iReturn = -2;
    ...
    public static int RecordAdd(string divcode, string SF_Name, string stockist_name, string stockist_Address, string stockist_ContactPerson,
        ...
        string type, string gstnNo, string dis_cat_code, string stkemail, string Taluk_Name, string Taluk_code, string Rate,string sfcity)
    {
        int iReturn = -1;
        //if (!NRecordExist(Territor_Code, stockist_name, divcode))
        //{
    ...
    public static int RecordUpdate(string divcode, string stockist_code, string Stock_code, string stockist_name, string stockist_Address, string stockist_ContactPerson, string stockist_Designation, string stockist_mobilno, string ERP_Code, string Town_Name, string Town_code, string Territor_Code, string Territory_Name, string Username, string Password, int norm, string Fo_Name, string Fo_Code, string sub_division, string headquarters, string type, string gstnNo, string Dis_Cat_Code, string stkemail, string Taluk_Name, string Taluk_code, string Rate,string sfcity)
    {
        int iReturn = -1;
        try
        {
            ...
            strQry = "UPDATE mas_stockist " +
                     ...
                     " Territory_Code = '" + Territor_Code + "' , " +
                      ...
                      " Price_list_Name='" + Rate + "', stockist_city='" + sfcity+"'," +
