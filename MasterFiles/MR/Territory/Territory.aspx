<%@ Page Title="Route List" Language="C#" AutoEventWireup="true"  MasterPageFile="~/Master.master" CodeFile="Territory.aspx.cs" Inherits="MasterFiles_MR_Territory" EnableEventValidation="false" %>
...
<head>
    <title>TerrTerritory List</title>
        ...
        $(document).ready(function () {
            ...
            $('#btnGo').click(function () {
 			 ...
 			 $('#<%=btnGo.ClientID%>').click(function () {
                var st = $('#<%=subdiv.ClientID%> :selected').text();
                if (st == "--Select--") { alert("Select Division Name."); $('#<%=subdiv.ClientID%>').focus(); return false; }

                var st = $('#<%=salesforcelist.ClientID%> :selected').text();
                if (st == "--Select--") { alert("Select FieldForce Name."); $('#<%=salesforcelist.ClientID%>').focus(); return false; }
            ...
            $('#<%=btnAdd.ClientID%>').click(function () {
                ...
                var st = $('#<%=ddlSFCode.ClientID%> :selected').text();
                if (st == "---ALL---") { alert("Select Distributor Name."); $('#<%=ddlSFCode.ClientID%>').focus(); return false; }
                
            });
			$('#<%=btnallowance.ClientID%>').click(function () {
                var st = $('#<%=subdiv.ClientID%> :selected').text();
                if (st == "--Select--") { alert("Select Division Name."); $('#<%=subdiv.ClientID%>').focus(); return false; }
            ...
            $('#<%=btnNew.ClientID%>').click(function () {
			  ...
			  $('.lstCount').each(function () {
                var terrhq = $('#<%=Territory.ClientID%> :selected').val()
                var rout_code = $.trim($(this).closest('tr').children('td:nth-child(2)').text());
                var rout_code = $.trim($(this).closest('tr').children('td:nth-child(2)').find('.terr_cde').text());
                var rout_name = $.trim($(this).closest('tr').children('td:nth-child(3)').text());
                if ($(this).text() != 0) {
                    $($(this)).html('<a class="AAA" href="rpt_Rout_Names.aspx?&Route_Code=' + rout_code + '&Route_Name=' + rout_name + '&Terr_Hq_Name=' + terrhq + '">' + $(this).text() + '</a>')
                }
            });
            $(".AAA").click(function () {
      ...
      <asp:ImageButton ID="ImageButton1" runat="server" align="right" ImageUrl="~/img/Excel-icon.png"
                ...
                <td>
                    <asp:Panel ID="pnlAdmin" runat="server">
                         ...
                         onselectedindexchanged="subdiv_SelectedIndexChanged" >
                          ...
                          <asp:DropDownList ID="Territory" runat="server" SkinID="ddlRequired"  CssClass="ddl" Visible="true" >                         
                         </asp:DropDownList>
                        ...
                        <asp:DropDownList ID="salesforcelist" runat="server" SkinID="ddlRequired" 
                            onselectedindexchanged="salesforcelist_SelectedIndexChanged" AutoPostBack="true" Visible="false">
                        <asp:ListItem Value="0">---Select---</asp:ListItem>
                        </asp:DropDownList>
                        <asp:DropDownList ID="ddlSFCode" runat="server" SkinID="ddlRequired" Visible="false">
                        <asp:ListItem Value="0">---Select---</asp:ListItem>
                        </asp:DropDownList>
                        <asp:Button ID="btnGo" runat="server"  Text="Go" CssClass="btn btn-primary btn-md"
        ...
        <table width="90%" cellpadding="5" cellspacing="5">
            <tr>
                <td style="width: 8.2%" />
                <td>
                <asp:Button ID="btnAdd" runat="server" CssClass="btn btn-primary btn-md"
                        Text="Add Route" Enabled="false" onclick="btnAdd_Click"  />
                 &nbsp;&nbsp;
					 <asp:Button ID="btnallowance" runat="server" CssClass="btn btn-primary btn-md" Text="Add Allowance Types"   OnClick="btnallowance_Click" />
                    <asp:Button ID="btnNew" runat="server" CssClass="BUTTON" Text="Bulk Add Route" visible="false" Enabled="false" Width="150px"
                ...
                <td align="right" >           
                 <asp:Label ID="Label1" runat="server" Font-Size="12px" Font-Names="Verdana"
                                Visible="false"></asp:Label>
                               <asp:Label ID="lblFilter" runat="server" Font-Bold="true" ForeColor="Purple" Visible="false" Text="Filter By FO Name"></asp:Label>&nbsp;&nbsp;
                                    
                                    <asp:DropDownList ID="ddlFilter" SkinID="ddlRequired" runat="server" Visible="false">
                                    </asp:DropDownList>
                            <asp:DropDownList ID="ddlSF" runat="server" Visible="false" SkinID="ddlRequired">
                            </asp:DropDownList>
                            &nbsp;
                                    <asp:Button ID="Button1" runat="server" CssClass="btn btn-primary btn-md" Visible="false" 
                    ...
                    <td colspan="2" align="center">
                       <asp:GridView ID="grdTerritory" runat="server" Width="85%" HorizontalAlign="Center"
                            AllowSorting="true" OnSorting="grdTerritory_Sorting" EmptyDataText="No Records Found"
                             OnPageIndexChanging="grdTerritory_PageIndexChanging" onrowcommand="grdTerritory_RowCommand"                          
                            AutoGenerateColumns="false" AllowPaging="True" 
                            PageSize="10" GridLines="None" CssClass="mGrid" PagerStyle-CssClass="pgr"
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="Route Code" Visible="true">
                                   <%-- <EditItemTemplate>
                                    ...
                                    <ItemTemplate>
                                        ...
                                        <asp:Label ID="lblTerritorys_Code1" CssClass="terr_cde" runat="server" Text='<%#Eval("Territory_Code")%>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderStyle-Width="400px" HeaderStyle-HorizontalAlign="Center"
                                    HeaderText="Route Name  " ItemStyle-HorizontalAlign="Left"
                                        ...
                                        <asp:Table ID="lblTerritory_Name" runat="server">
                                            <asp:TableRow>
                                                <asp:TableCell BorderStyle="None" Width="30px">
                                                    <asp:Label ID="LabTerritory_Name" runat="server" Width="200px" Text='<%# Bind("Territory_Name") %>'></asp:Label>
                                                </asp:TableCell>  
                                ...
                                <asp:TemplateField HeaderText="Distributor Name" HeaderStyle-Width="280px"  HeaderStyle-ForeColor="White"
                                    Visible="true">
                                  
                                    <ItemTemplate>
                                       <asp:Label ID="lblDist_Territory" runat="server" Width="280px"  Visible="false" Text='<%# Bind("Dist_Name") %>'></asp:Label>
                                        <asp:Label ID="Dist_Name" runat="server" Width="300px"  Visible="true" Text='<%# Bind("Dst_Name") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>

							  <asp:TemplateField HeaderText="Retailer Cnt" HeaderStyle-Width="90"  HeaderStyle-ForeColor="White" ItemStyle-HorizontalAlign="right" 
                                    Visible="true">
                                  
                                    <ItemTemplate>
                                       <asp:Label ID="lblListedDRCnt" class="lstCount" runat="server" ForeColor="DarkBlue" Text='<%# Bind("ListedDR_Count") %>'></asp:Label>        
                                    </ItemTemplate>
                                </asp:TemplateField>



                                <asp:TemplateField HeaderText="Target" HeaderStyle-Width="100px" ItemStyle-HorizontalAlign="Left">                                  
                                    <ItemTemplate>
                                        <asp:Label ID="lblTerritory_Target" runat="server" Text='<%# Bind("Target") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="MinProd %" HeaderStyle-Width="120px"  ItemStyle-HorizontalAlign="Left">
                                  
                                    <ItemTemplate>
                                        <asp:Label ID="lblTerritory_minprod" runat="server" Text='<%# Bind("Min_Prod") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="Route Population" HeaderStyle-Width="90px" ItemStyle-HorizontalAlign="Left">
                                    
                                    <ItemTemplate>
                                        <asp:Label ID="lblRoute_Population" runat="server" Text='<%# Bind("Population") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
