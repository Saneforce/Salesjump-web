...
using System.Data;
using Bus_EReport;
public partial class MasterFiles_MR_Leave_Status : System.Web.UI.Page
{
    ...
    protected void btnView_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            if (FYear <= TYear)
            {
                ...
                dsTP = adm.GetStatus_MGR(div_code, sf_code, ddlFMonth.SelectedValue, ddlFYear.SelectedValue, ddlTMonth.SelectedValue, ddlTYear.SelectedValue);

                if (dsTP.Tables[0].Rows.Count > 0)
                {
                    grdLeave.Visible = true;
                    grdLeave.DataSource = dsTP;
                    grdLeave.DataBind();
                }
                else
                {
