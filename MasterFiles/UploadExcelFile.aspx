...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
                    ...
                    if ($("input:checkbox[name=fields]:checked").length > 0) {
                        ...
                        if (ExcelName == "Tax") {
                            $.ajax({
                                ...
                                async: false,
                                data: "{}",
                                url: "UploadExcelFile.aspx/Download_Click",
                                dataType: "json",
                                success: function (data) {
                                    var dataa1 = JSON.parse(data.d);
                                    $("#htmlTable TBODY").html("");
                                    html = $('#htmlTable');
                                    $(html).find('tbody tr').remove();
                                    if (dataa1.length > 0) {
                                        for (i = 0; i < dataa1.length; i++) {
                                            res = "<tr><td>" + dataa1[i].ProductName + "</td><td>" + dataa1[i].ProductCode + "</td><td>" + dataa1[i].ERP_Code +
                                                "</td><td>" + dataa1[i].StateName + "</td><td>" + dataa1[i].TaxName + "</td><td>" + dataa1[i].TaxValue + "</td></tr>";
                                            $("#htmlTable").append(res);
                                        }
                                    }
                                    var htmltable = document.getElementById('htmlTable');
                                    var file = XLSX.utils.table_to_book(htmltable, { sheet: "Tax" });
                                    XLSX.write(file, { bookType: 'xlsx', bookSST: true, type: 'base64' });
                                    XLSX.writeFile(file, 'Tax.xlsx');
                                }
                            });
                        }
                        else {
        ...
        function BindExcel(data, sheetname) {
			...
			if (sheetname == 'TP Upload') {
			excelRows = XLSX.utils.sheet_to_json(workbook.Sheets[sheetname], {
                    raw: false, // This ensures that Excel numbers are interpreted as dates
                });

                for (var i = 0; i < excelRows.length; i++) {
                    var dateComponents = excelRows[i].Tour_Date.split('/');
                    ...
                    // Convert the year to a 4-digit format (assuming 20xx for 'YY' years)
                    var formattedYear = '20' + year;

                    // Create a new date string in "YYYY-MM-DD" format
                    var formattedDateString = `${formattedYear}-${month}-${day}`;
                    excelRows[i].Tour_Date = formattedDateString;
                }
			}
			else {
            ...
            for ($i = st; $i < st + PgRecords; $i++) {
                if ($i < jsondata.length) {
                    ...
                    for (var colIndex = 0; colIndex < columns.length; colIndex++) {
					    if (jsondata[$i][columns[colIndex]] == undefined) {
                            jsondata[$i][columns[colIndex]] = "";
                        }
                        else if (((jsondata[$i][columns[colIndex]]).toString()) == null) {
                            jsondata[$i][columns[colIndex]] = "";
                        }
                        var cellValue = jsondata[$i][columns[colIndex]];
                        if (columns[i] != 'Success') {
						     if (jsondata[$i][columns[colIndex]] != "") {
                                cellValue = jsondata[$i][columns[colIndex]];
                                //alert(cellValue);
                                row$.append($("<td colName=" + (columns[colIndex].trim()) + ">" + ($.trim(cellValue)) + "</td>")); //.html(($.trim(cellValue))));
                                //row$.append("<td></td>");
                            }
                            //if (cellValue == null)
                              //  cellValue = "";
                            ...
                            //row$.append($('<td colName="' + (columns[colIndex].trim()) + '"></td>').html((cellValue)));
                            cellValue = jsondata[$i][columns[colIndex]]
                            row$.append($("<td colName=" + (columns[colIndex].trim()) + ">" + ($.trim(cellValue)) + "</td>"));
                            //row$.append($('<td colName="' + (columns[colIndex]).trim() + '"></td>').html(($.trim(cellValue))));
                        }
                    }
					
					 if ((excelRows[0]["Success"] == undefined && cellValue != "")) {
                        row$.append("<td></td>");
                    }
