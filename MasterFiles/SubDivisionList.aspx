<%@ Page Title="Division List" Language="C#" AutoEventWireup="true" MasterPageFile="~/Master.master" CodeFile="SubDivisionList.aspx.cs"
    Inherits="MasterFiles_SubDivisionList" EnableEventValidation="false" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
        ...
        <script type="text/javascript">
            ...
            $('form').live("submit", function () {
                ...
                <div class="row">
        <div class="col-lg-12 sub-header">Division<span style="float: right"><a href="SubDivisionCreation.aspx" class="btn btn-primary btn-update" id="newsf">Add New</a></span><div style="float: right;padding-top: 3px;">
                    <ul class="segment">
    ...
    <div class="modal fade" id="FieldModal" style="z-index: 10000000; background: transparent; overflow-y: scroll;" tabindex="0" aria-hidden="true">
            <div class="modal-dialog" role="document" style="width: 80% !important">
                ...
                <div class="card">
                    ...
                    <select class="data-table-basic_length" aria-controls="data-table-basic">
                        ...
                        <table class="table table-hover" id="DivList" style="font-size: 12px">
                            <thead class="text-warning">
                                <tr>
                                    <th style="text-align: left">Sl.No</th>
                                    <th id="Division_Code" style="text-align: left">Division Code</th>
                                    <th id="Division_Name" style="text-align: left">Division Name</th>
                                    <th id="Productwise_Count" style="text-align: left">Productwise Count</th>
                                    <th id="Fieldforcewise_Count" style="text-align: left">Fieldforcewise Count</th>
            ...
            <script language="javascript" type="text/javascript">
                var AllOrders = [];
                var filtrkey = '0';
                var Orders = [], pgNo = 1, PgRecords = 10, TotalPg = 0, searchKeys = "subdivision_code,subdivision_name,";
                ...
                function ReloadTable() {
                    ...
                    if (filtrkey != "All") {
                        Orders = Orders.filter(function (a) {
                            return a.SubDivision_Active_Flag == filtrkey;
                        });
                    }
                    st = PgRecords * (pgNo - 1); slno = 0;
                    for ($i = st; $i < st + PgRecords; $i++) {
                        if ($i < Orders.length) {
                            tr = $("<tr rname='" + Orders[$i].subdivision_name + "'  rocode='" + Orders[$i].subdivision_code + "'></tr>");
                            slno = $i + 1;
                            $(tr).html('<td>' + slno + "</td><td>" + Orders[$i].subdivision_code + '</td><td>' + Orders[$i].subdivision_name + "</td><td class='scount'><input type='hidden' class='pcod_hidd' value='" + Orders[$i].Sub_Count + "'><a href='#'>" + Orders[$i].Sub_Count +
                                "</a></td><td class='sfcount'><a href='#'>" + Orders[$i].SubField_Count + "</a></td><td id=" + Orders[$i].subdivision_code +
                                " class= 'roedit' > <a href='#'>Edit</a></td > <td class='rodeact'><a href='#' style=" + ((parseInt(Orders[$i].SubDivision_Active_Flag) == 0) ? 'color:green;' : 'color:red;')+" >" + ((parseInt(Orders[$i].SubDivision_Active_Flag) == 0) ? 'Active' : 'Deactive') + "</a></td>");
                            $("#DivList TBODY").append(tr);
                            hq = [];
                        }
                    }
                ...
                function fillRoutes(divcode) {
                    $('#routedets tbody').html('');
                    $.ajax({
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "SubDivisionList.aspx/Prod_Count",
                        data: "{'divcode':'<%=Session["div_code"]%>','sdcode':'" + divcode + "'}",
                        dataType: "json",
                        success: function (data) {
                            ...
                            for ($i = 0; $i < AllOrders2.length; $i++) {
                                if (AllOrders2.length > 0) {
                                    slno += 1;
                                    tr = $("<tr></tr>");
                                    $(tr).html("<td>" + slno + "</td><td>" + AllOrders2[$i].Product_Detail_Code + "</td><td>" + AllOrders2[$i].Product_Detail_Name + "</td><td>" + AllOrders2[$i].Product_Description + "</td><td>" + AllOrders2[$i].Product_Sale_Unit + "</td>");
                ...
                function fillFields(divcode) {
                    ...
                    $.ajax({
                        ...
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "SubDivisionList.aspx/Field_Count",
                        data: "{'divcode':'<%=Session["div_code"]%>','sdcode':'" + divcode + "'}",
                        dataType: "json",
                        success: function (data) {
                            var AllOrders2 = JSON.parse(data.d) || [];
                ...
                function loadData(divcode) {
                    ...
                    $.ajax({
                        ...
                        contentType: "application/json; charset=utf-8",
                        async: false,
                        url: "SubDivisionList.aspx/GetDetails",
                ...
                $(document).ready(function () {
                    ...
                    $(document).on('click', '.roedit', function () {
                        x = this.id;
                        window.location.href = "SubDivisionCreation.aspx?hqcode=" + x;
                    });
					...
					var row = $(this).closest('tr');
                        var route_C = $(this).closest('tr').attr('rocode');
                        let oindex = Orders.findIndex(x => x.subdivision_code == route_C);
                        let disstat = (parseInt(Orders[oindex]["SubDivision_Active_Flag"]) == 0) ? 'Deactive' : 'Active';
                        let flg = (parseInt(Orders[oindex]["SubDivision_Active_Flag"]) == 0) ? 1 : 0;
						let flag = 0;
                        if ((flg == 1) || (flg == 0)) {
						if (disstat == "Deactive") {
                                ...
                                if (res > 0) {
                                    flag = flag + 1;
                                    alert('This Division mapped with Product! Cannot deactivate!');
                                }
                            }
							if (flag == 0) {
                            if (confirm("Do you want " + disstat + " the Division ?")) {
                                ...
                                $.ajax({
                                    ...
                                    contentType: "application/json; charset=utf-8",
                                    async: false,
                                    url: "SubDivisionList.aspx/deactivate",
                                    data: "{'sdcode':'" + route_C + "','stat':'" + flg + "'}",
                                    dataType: "json",
                                    success: function (data) {
                                        if (data.d == 'Success') {
                                            Orders[oindex]["SubDivision_Active_Flag"] = flg;
                                            ReloadTable();
