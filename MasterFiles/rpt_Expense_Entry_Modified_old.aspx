...
<body style="margin: 15px; margin-top: 50px">
    ...
    <form id="form1" runat="server">
        ...
        <script language="javascript" type="text/javascript">
            ...
            var Type = 0; let dailyAdd_Amt = 0; let dailyD_Amt = 0, click_key = 0;
            ...
            function reject($rdt, rReason) {
                ...
                $.ajax({
                    ...
                    data: "{'SF':'<%=SF_Code%>','Mn':'<%=Mn %>','Yr':'<%=Yr%>','RejDt':'" + rejdt[0] + "','Rtype':'" + rtype + "','sEmpID':'<%=EmpID%>','RjectRm':'" + rReason + "','Type':'" + Type + "'}",
                    dataType: "json",
                    success: function (data) {
                ...
                for ($i = 0; $i < ExpDets.length; $i++) {
                    ...
                    if (ExpItem.ADate != NAdt) {
                        if ((ExpItem.ADate != frecord) && (ExpItem.ADate != lrecord)) {
                            ...
                            $s += "</table></td>";
                            if (ExpItem.WType != 'No Claim' && ExpItem.WType != 'Not Claimed' && '<%=Session["div_code"]%>' == '162') {
            ...
            function distchange($chdist, $Fulchrg) {
                ...
                var chfare = cdist * (($($chdist).closest('tr').find(".Atyp").val() == 'HQ') ? 0 : $($chdist).closest('tr').find(".E_Mot_typ").val());//$Fulchrg;;
                ...
                $($chdist).closest('tr').find('.E_Dis>span').text(($($chdist).closest('tr').find(".Atyp").val() == 'HQ') ? 0 : cdist);
                ...
                if ($($chdist).closest('tr').find(".Atyp").val() == 'HQ') {
                ...
                data: "{'EHead':'" + JSON.stringify($ExpHead) + "','ExDetails':'" + JSON.stringify(dydata) + "','DDetails':'" + JSON.stringify(mdydata) + "','MDetails':'" + JSON.stringify(mMtdata) + "','ExpAdd':'" + JSON.stringify(Exp_Add) + "','MAuto':'" + $MAuto + "','ApExp':'" + $Atvexp + "','ExpDAdd':'" + JSON.stringify(dyAdde) + "','AppSFC':'" + $chksfc + "','sEmpId':'<%=EmpID%>'}",
                    ...
                    success: function (data) {
                        ...
                        if (data.d == 'Expense Saved for Approval' || data.d == 'Expense Submitted for Approval') {
                            if ('<%=Session["sf_code"]%>' == 'admin' && $MAuto != 0) {
                                contents = 'Expense Approved';
                            }
                            else {
                                contents = data.d;
                                //alert(data.d);
