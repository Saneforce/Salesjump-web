<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Rpt_DCR_View.aspx.cs" Inherits="MasterFiles_Reports_tsr_Rpt_DCR_View" EnableEventValidation = "false" %>
...
//            var mapOptions = {
//                center: new google.maps.LatLng(markers[0].lat, markers[0].lat),
//                zoom: 12,
...
<%--<script type="text/javascript">
        $(function () {
            ...
            var list = $("#gvtotalorder tr td:nth-child(6)");
     ...
     <script type="text/jscript">
         $(document).ready(function () {
             var j = 0;
             $('#<%=GridView4.ClientID%>').find('tr').each(function () {


                 tds1 = $(this).find("td");
                 $(tds1[2]).hide();
                 if (j > 0) {
                     if ($(tds1[1]).text() != "TOTAL") {
                         $(tds1[0]).text(j);

                         if (Number($(this).find("td:last").text()||0) > 0) {
                             $(this).find("td:last").html('<a class="AAA" href="rptBrandECview.aspx?&sf_code=' + $(tds1[2]).text() + '&date=' + $('#<%=DCRdt.ClientID%>').val() + '&Sf_Name=' + $(tds1[1]).text() + '&cur_month=0&cur_year=0&Tdate=' + $('#<%=DCRdt.ClientID%>').val() + '">' + $(this).find("td:last").text() + '</a>')
                         }
             ...
             $('#<%=GridView2.ClientID%>').find('tr').each(function () {
                 tds1 = $(this).find("td");
				 var clsby = $(tds1[2]).text();
                 $td = $(`<td>${clsby}</td>`);
                 if (clsby == "Closing By") {
                     $($td).css('background-color', '#33CCCC');
                 }
                 $(tds1[2]).hide();
                 $($(tds1)[4]).after($td)
             });
             ...
             $('#<%=gvtotalorder.ClientID%>').find('tr').each(function () {
                 ...
                 tds = $(this).find("td");
                 $(tds[4]).css('background-color', 'MistyRose');
                 $(tds[5]).css('background-color', 'MistyRose');
                 //                 $(tds[6]).hide();
                 //                 $(tds[7]).hide();
                 //                 $(tds[8]).hide();
                 if (k > 0) {
                     if ($(tds[6]).text() != "TOTAL") {
                         ...
                         if ($(tds[6]).text() != "0") {
                             $(tds[1]).html('<a class="AAA" href=Rpt_DCR_View.aspx?&sf_code=' + $(tds[6]).text() + '&Mode=New_View_All_DCR_Date(s)&FDate=' + $('#<%=DCRdt.ClientID%>').val() + '&cur_month=0&cur_year=0&Tdate=' + $('#<%=DCRdt.ClientID%>').val() + '&Sf_Name=>' + $(tds[1]).html() + '</a>')
 ...
 <asp:HiddenField runat="server" Value="" ID="DCRdt" />
        ...
        <center>
                        ...
                        <td>
                            <asp:GridView ID="gvMyDayPlan" runat="server" Width="100%" HorizontalAlign="Center"
                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="Emp ID" ItemStyle-Width="70" HeaderStyle-BorderWidth="1"
                                        HeaderStyle-Font-Size="8pt" ItemStyle-BorderWidth="1" ItemStyle-HorizontalAlign="Center">
                                        <ItemTemplate>
                                            <asp:Label ID="Pln_Date" runat="server" Font-Size="9pt" Text='<%#Eval("SF_Emp_ID")%>'></asp:Label>
                                        </ItemTemplate>
                                    </asp:TemplateField>
                                    ...
                                    <asp:TemplateField HeaderText="App Version" ItemStyle-Width="70" HeaderStyle-BorderWidth="1"
                                        HeaderStyle-Font-Size="8pt" ItemStyle-BorderWidth="1" ItemStyle-HorizontalAlign="Center">
                                        <ItemTemplate>
                                            <asp:Label ID="txtappversion" runat="server" Font-Size="9pt" Text='<%#Eval("App_version")%>'></asp:Label>
                                        </ItemTemplate>
                                    </asp:TemplateField>
            ...
            <asp:Panel ID="Panel2" runat="server" Width="100%">
                <table border="0" width="90%">
                    <tr align="right">
                        <td align="left" 
                            style="font-size: small; font-weight: bold; font-family: Andalus;">
                            FieldForce Name:
                            <asp:Label ID="Feild" runat="server" ForeColor="Red" Text=""></asp:Label>
                        </td>
                    </tr>
                    <tr>
                        <td align="center">
                    ...
                    <tr>
                        <td>
                            <asp:GridView ID="GridView2" runat="server" Width="100%"   ShowHeader="false" Font-Size="10pt"
                                ...
                                HeaderStyle-HorizontalAlign="Center" BorderColor="Black" BorderStyle="Solid" Style="padding: 3px 6px; white-space: nowrap;" CssClass="mGrid"  ShowFooter="true">                               
                                <Columns>
  ...
  <asp:Panel ID="Panel4" runat="server" Width="100%">
                <table border="0" width="90%">
                    <tr align="center">
                        <td>
                            <asp:Label ID="Label3" runat="server" Font-Size="Small" Font-Bold="True" Font-Underline="true"></asp:Label>
                            <span style="color: Red"></span>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>
                            <asp:GridView ID="GridView3" runat="server" Width="100%"   ShowHeader="false" Font-Size="10pt"
                                HorizontalAlign="Center" OnDataBound = "OnDataBound2" OnRowCreated = "OnRowCreated2"   OnRowDataBound="GridView1_RowDataBound2"
                                BorderWidth="1px" CellPadding="2" EmptyDataText="No Data found for View" 
                                AutoGenerateColumns="true" BackColor="#ffffe0"   RowStyleCssClass="gridpager"
      ...
      <asp:Table ID="Table1" runat="server" Height="100%" Width="100%">
                    ...
                    <tr>
                        <td>
                             <asp:GridView ID="gvtotalorder" runat="server" Width="100%"   
                                ...
                                HorizontalAlign="Center" OnDataBound = "OnDataBound" 
                                 OnRowCreated = "OnRowCreated"   OnRowDataBound="GridView1_RowDataBound"
                                BorderWidth="1px" CellPadding="2" EmptyDataText="No Data found for View" 
                                ...
                                HeaderStyle-HorizontalAlign="Center" BorderColor="Black" 
                                 BorderStyle="Solid"  CssClass="mGrid" ShowFooter="true" 
                                 onprerender="gvtotalorder_PreRender">                               
                                <Columns>
           ...
           <asp:Table ID="Table2" runat="server" Height="100%" Width="100%">
            <asp:TableRow ID="TableRow4" runat="server">
                ...
                <asp:TableCell ID="TableCell6" runat="server" style="padding-left:50%;"></asp:TableCell>
            <%--    <asp:TableCell ID="TableCell3" runat="server"></asp:TableCell>--%>
            </asp:TableRow>
            <asp:TableRow ID="TableRow5" runat="server">
                <asp:TableCell ID="TableCell9" runat="server"></asp:TableCell>
                <asp:TableCell ID="TableCell10" runat="server" style="padding-left:50%;"></asp:TableCell>
               <%-- <asp:TableCell ID="TableCell6" runat="server"></asp:TableCell>--%>
            </asp:TableRow>
            <asp:TableRow ID="TableRow6" runat="server">
                <asp:TableCell ID="TableCell11" runat="server"></asp:TableCell>
                <asp:TableCell ID="TableCell12" runat="server" style="padding-left:50%;"></asp:TableCell>
              <%--  <asp:TableCell ID="TableCell9" runat="server"></asp:TableCell>--%>
            </asp:TableRow>
        </asp:Table>
                <table border="0" width="100%">
