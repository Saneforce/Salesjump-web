<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Retailer_Report_Format_new.aspx.cs" MasterPageFile="~/Master.master" Inherits="MasterFiles_Reports_tsr_Retailer_Report_Format_new" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        var ddlstate = [], ddlzone = [], ddlso = [], ddlTsr = [], ddldisbutr = [], ddlFieldForce = [], ddlevent = [];
        $(document).ready(function () {
        
            retrievestate(); retrieveZone(); retrieveDistributor(); retrieveIdentification();
            var mod = '<%=Mode%>';
            if (mod == 'Event Captures View' || mod == 'TSR Event Captures View') {
                $('#ddmodeev').show();
            }
			else{
			    $('#ddmodeev').hide();
			}
            if (mod == 'TP MY Day Plan' || mod == 'Event Captures View' || mod == 'TSR Event Captures View') {
                $('#ddb').hide();
            }
			else{
			    $('#ddb').show();
        ...
        $('#btnsubmit').on('click', function () { 
           ...
           // var mod = '<%=Mode%>';
            ...
            if (mod == "SKU Summary") {
                var sURL = "/MasterFiles/Reports/tsr/rptSKU_Summarys_rep1.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> +
                ...
                sURL = "/MasterFiles/Reports/tsr/viewpromotion.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> + "&Mode=" + mod + "&Sf_Name=" + sfn + "&FDate=" + fdt + "&TDate=" + tdt
                    + "&Dst_name=" + distrunm + "&Dst_code=" + distru + "&st_name=" + statn + "&st_code=" + stat;
                window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
			...
			<%--else if (mod == "TP MY Day Plan") {
			    ...
			    sURL = "/MasterFiles/Reports/tsr/Retailwise_sales_report.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> +
                    "&Mode= " + mod + "&Sf_Name=" + sfn + "&FDate=" + fdt + "&TDate=" + tdt
                    + "&Dst_name=" + distrunm + "&Dst_code=" + distru + "&st_name=" + statn + "&st_code=" + stat;
                window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
			    ...
			    sURL = "/MasterFiles/Reports/tsr/Retailwise_closing_report.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> +
                    "&Mode= " + mod + "&Sf_Name=" + sfn + "&FDate=" + fdt + "&TDate=" + tdt
                    + "&Dst_name=" + distrunm + "&Dst_code=" + distru + "&st_name=" + statn + "&st_code=" + stat;                 
                window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
			    ...
			    sURL = "/MasterFiles/Reports/tsr/Rpt_Retailer_Tag_Status.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> +
                    "&Mode= " + mod + "&Sf_Name=" + sfn + "&FDate=" + fdt + "&TDate=" + tdt
                    + "&Dst_name=" + distrunm + "&Dst_code=" + distru + "&st_name=" + statn + "&st_code=" + stat;                 
                window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
            
            }
			else if (mod == "Event Captures View" || mod == "TSR Event Captures View") {
                sURL = "/MasterFiles/Reports/tsr/Rpt_DCR_Event_Captures_View.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> +
                    "&Mode= " + mod + "&Sf_Name=" + sfn + "&FDate=" + fdt + "&TDate=" + tdt
                    + "&Dst_name=" + distrunm + "&Dst_code=" + distru + "&st_name=" + statn + "&st_code=" + stat+ "&mode_identi=" + ddlevent;                 
                window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');            
            }
            else {
			    sURL = "/MasterFiles/Reports/tsr/Rpt_DCR_View.aspx?sf_code=" + sf + "&div_code=" + <%=div_code%> +
                    "&Mode= " + mod + "&Sf_Name=" + sfn + "&FDate=" + fdt + "&TDate=" + tdt
                    + "&Dst_name=" + distrunm + "&Dst_code=" + distru + "&st_name=" + statn + "&st_code=" + stat;
                window.open(sURL, null, 'resizable=yes,toolbar=no,scrollbars=yes,menubar=no,status=no,width=800,height=600,left=0,top=0');
        ...
        function retrievestate() {
            $.ajax({
                ...
                async: false,
                url: "Retailer_Report_Format_new.aspx/getstate",
        ...
        function retrieveZone() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Retailer_Report_Format_new.aspx/getzone",
                data: "{'divcode':'<%=Session["div_code"]%>'}",
                dataType: "json",
                success: function (data) {
        ...
        function retrieveSO($mgrid) {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Retailer_Report_Format_new.aspx/getSO",
        ...
        function retrieveDistributor() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Retailer_Report_Format_new.aspx/getDistributor",
                data: "{'divcode':'<%=Session["div_code"]%>'}",
                dataType: "json",
                success: function (data) {
        ...
        function retrieveIdentification() {
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                async: false,
                url: "Retailer_Report_Format_new.aspx/getIdentification",
                data: "{'divcode':'<%=Session["div_code"]%>'}",
                dataType: "json",
                success: function (data) {
                    console.log(data);
