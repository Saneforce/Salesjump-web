...
public partial class MIS_Reports_rpt_priorder_detailwise : System.Web.UI.Page

{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblHead1.Text = " Primary Orders Product Detail View for  " + hdate + "";
    ...
    private void FillSF()
    {
        ...
        DCR sf = new DCR();
        dsDrr = Gettransno1(Sf_Code, divcode, hdate, Stockist_Code);
        foreach (DataRow d in dsDrr.Tables[0].Rows)
        {
            ...
            if (dsSalesForce.Tables[0].Rows.Count > 0)
            {
                ...
                foreach (DataRow drFF in dsSalesForce.Tables[0].Rows)
                {
                    ...
                    TableCell tc_det_dr_tax = new TableCell();
                    tc_det_dr_tax.HorizontalAlign = HorizontalAlign.Center;
                    Literal lit_det_dr_tax = new Literal();
                    lit_det_dr_tax.Text = drFF["tax"].ToString();
                    iTotLsttax += Decimal.Parse(drFF["tax"].ToString());
                    tc_det_dr_tax.Attributes.Add("Class", "tblRow");
                    tc_det_dr_tax.BorderStyle = BorderStyle.Solid;
                    tc_det_dr_tax.BorderWidth = 1;
                    tc_det_dr_tax.Controls.Add(lit_det_dr_tax);
                    tr_det.Cells.Add(tc_det_dr_tax);
                ...
                hyp_tax.Text = iTotLsttax.ToString();
    ...
    public DataSet Gettransno1(string sfcode, string div_code, string activitydate, string Stockist_Code)
    {
        DB_EReporting db_ER = new DB_EReporting();

        DataSet dsSF = null;
        string strQry = "select Trans_Sl_No from Trans_priOrder_Head  where Sf_code='" + sfcode + "' and  CONVERT(VARCHAR(25), Order_Date, 126)  like '" + activitydate + "%' and Stockist_Code='" + Stockist_Code + "'";
    ...
    public DataSet product_detaill(string Trans_Sl_no)
    {
        ...
        string strQry = "select d.Product_Code,d.Product_Name, ISNULL(d.Qty,d.CQty)Quantity,(ISNULL(d.Qty,d.CQty) * Product_Unit_Value) Quantity1,d.Rate,d.free,d.discount_price,d.value,isnull(d.tax,0) tax," +
                        "(CAST(ISNULL(d.Qty,d.CQty) AS VARCHAR(10)) + ' ' + d.product_Unit_Name)netpack from Trans_priOrder_Head h " +
