...
public partial class MasterFiles_Reports_Retailer_GeoTag_Status : System.Web.UI.Page
{
    ...
    public void filldist()
    {
        SalesForce sd = new SalesForce();
        DataSet ds = new DataSet();
        ds = sd.SalesForceList(divCode, Session["Sf_Code"].ToString());
        if (ds.Tables[0].Rows.Count > 0)
        {
    ...
    protected void btnview_Click(object sender, EventArgs e)
    {
        try
        {
            string sURL = "Rpt_Retailer_Tag_Status.aspx?SF_code=" + ddldist.SelectedValue + "&div_code=" + divCode +
               "&subdiv=" + ddldiv.SelectedItem.Value + "&SF_Name=" + ddldist.SelectedItem.Text ;
