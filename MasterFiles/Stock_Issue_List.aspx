<%@ Page Title="Issue Slip List" Language="C#" MasterPageFile="~/Master.master"
    AutoEventWireup="true" CodeFile="Stock_Issue_List.aspx.cs" Inherits="MasterFiles_Stock_Issue_List" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                async: false,
                url: "Stock_Issue_List.aspx/GetDistributor",
                dataType: "json",
            ...
            $(document).on('click', '#btnGo', function () {
                ...
                $.ajax({
                    ...
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    url: "Stock_Issue_List.aspx/Get_Stock_Transfer_List",
                    ...
                    success: function (data) {
                        ...
                        if (data.d.length > 0) {
                            for (var i = 0; i < data.d.length; i++) {
                                var strFF = "<td>" + (i + 1) + "</td><td>" + data.d[i].Transfer_No + "</td><td>" + data.d[i].Transfer_Date + "</td><td><input type='hidden' name='supp_Code' value='" + data.d[i].Transfer_From + "'/>" + data.d[i].Transfer_From_Nm + "</td><td><input type='hidden' name='supp_Code' value='" + data.d[i].Transfer_To + "'/>" + data.d[i].Transfer_To + "</td><td><a class='btn btn-primary btn-md' href='Stock_Issue.aspx?Mode=1&Code=" + data.d[i].Trans_SlNo + "'><span class='glyphicon glyphicon-edit'> Edit</span></a></td><td><a class='btn btn-primary btn-md' href='Stock_Issue_View.aspx?Mode=1&Code=" + data.d[i].Trans_SlNo + "'><span class='glyphicon glyphicon-eye-open'> View</span></a></td>";
                                $("#StockTransferList tbody").append("<tr class='gvRow'>" + strFF + "</tr>");
                            }
                        }
                        else {

                            $("#StockTransferList tbody").append("<tr class='gvRow'><td colspan='5'> No Records Found..!</td></tr>");
                        }
    ...
    <div class="container">
       <div class="row">
            <div class="col-lg-12">
                <a class='btn btn-success btn-md' href='Stock_Issue.aspx?Mode=0'>
                    New Issue Slip</span></a>
            </div>
        </div>
        <br />
        <br />
        <div class="row">
            ...
            <div class="col-sm-4" style="text-align:right">
                <div class="form-inline">
                    <div class="form-group">
                        <asp:Label ID="Label2" runat="server" Text="Transfer From" Visible="false"></asp:Label>
                        <select id="ddlFilter" name="ddlFilter" class="form-control"  style="display:none;">                          
                        </select>
                    </div>
                </div>
            </div>
