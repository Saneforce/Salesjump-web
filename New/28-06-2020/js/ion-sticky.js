        ...
        return {
            ...
            link: function ($scope, $element, $attr, $ionicScroll) {
                ...
                var createStickyClone = function ($element) {
                    ...
                    $attr.ionStickyClass = ($attr.ionStickyClass) ? $attr.ionStickyClass : 'positive';
                ...
                var divHeight = $ionicPosition.offset($element[0].getElementsByClassName($attr.ionSticky || "item-divider")).height;
                    ...
                    for (var i = 0; i < dividers.length; ++i) { // can be changed to binary search
                        if ($ionicPosition.offset(dividers[i]).top - $ionicPosition.offset($element).top < 0) { // this equals to jquery outerHeight  - (divHeight-35) 
                            if (i === dividers.length - 1 || $ionicPosition.offset(dividers[i + 1]).top -(
                                 ($ionicPosition.offset($element).top + dividers[i+1].prop('offsetHeight')) + (divHeight-35)) > 0) { //when clone covers new header
                                active = dividers[i][0];
                                break;
                            }
                        }
