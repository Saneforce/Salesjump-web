    ...
    fmcgAPI.addMAData = function(method, url, choice, data) {
        ...
        switch (choice) {
            ...
            case "2":
                ...
                unlisted_doctor_master.lat=data.lat;
                unlisted_doctor_master.long=data.long;
            ...
            Multiretailername = '',MultiretailerCode='';
                ...
                Tour_Plan.Multiretailername = Multiretailername;
                Tour_Plan.MultiretailerCode =MultiretailerCode;
...
case "42":
            ...
            if(data.GiftCardAryra!=undefined){
                ...
                for (var i = 0, len = data['GiftCardAryra'].length; i < len; i++) {
                    ...
                    //tempObb['name']=addQuotes(data['GiftCardAryra'][i]['name']+'-'+data['GiftCardAryra'][i]['Qty']);
                    //tempObb['QTY']=addQuotes(data['GiftCardAryra'][i]['Qty']);
                 if (i != 0)
                    tempObb +="#"
                    tempObb +=data['GiftCardAryra'][i]['name']+'~'+data['GiftCardAryra'][i]['Qty'];
                   
                    //GiftCardAryra.push(pushKey(tempObb));                      
                            }
                             }
             ...
             for (var t = 0, jwlen = data.BreedCategory.length; t < jwlen; t++) {
                ...
                resultData.push({'ContactNumber':data['ContactNumber']});
                resultData.push({'Remarks':data['remarks']});
                resultData.push({'Seeman':data['Semen']});
                resultData.push({'Address':data['Address']});
               
                resultData.push({'Catogoryformer':data.speciality.selected.id});

               
               resultData.push({'Crop':data['Crop']});
                resultData.push({'Route':data['Route']});
                resultData.push({'Breed':jointWorkString});
                resultData.push({'Class':data.class.selected.id});
                resultData.push({'Ukey':data.Ukey});
                resultData.push({'unlisted_cat_code':"null"});*/
    ...
    fmcgAPI.saveDCRData = function(method, url, data, update) {
        ...
        switch (selectedC) {
            case "1":
                ...
                activity_doctor_rep['discount_price'] = data['discount_price'];
